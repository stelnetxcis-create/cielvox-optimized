// stelnettts_backend_pocket_tts.cpp -- adapter for kyutai/pocket-tts
...
// bare-name resolution in the vibevoice / cielvox2-tts adapters. Absolute paths and
...
static std::string resolve_pocket_voice_path(const whisper_params& params) {
    ...
    if (voice_path.find("..") != std::string::npos || voice_path.find('\0') != std::string::npos) {
        fprintf(stderr, "stelnettts[pocket-tts]: voice name '%s' contains illegal characters (.. or NUL)\n",
                voice_path.c_str());
        return voice_path;
    }
    ...
    if (file_exists(wav_path))
        return wav_path;
    fprintf(stderr, "stelnettts[pocket-tts]: warning: voice '%s' not found at '%s'\n", voice_path.c_str(),
            wav_path.c_str());
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[pocket-tts]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        model_path = stelnettts_resolve_model_cli(model_path, p.backend, p.no_prints, p.cache_dir, p.auto_download, "");

        ctx_ = pocket_tts_init_from_file(model_path.c_str(), cp);
        if (!ctx_) {
            fprintf(stderr, "stelnettts[pocket-tts]: failed to load model '%s'\n", model_path.c_str());
            return false;
        }
        ...
        if (p.tts_voice.empty()) {
            ...
            for (const char** fb = fallbacks; *fb; fb++) {
                ...
                if (f) {
                    ...
                    int ref_sr = 0;
                    if (stelnettts::core::read_wav_mono_pcm16(*fb, ref_pcm, ref_sr) && !ref_pcm.empty()) {
                        if (ref_sr != 24000)
                            ref_pcm =
                        ...
                        if (pocket_tts_set_voice(ctx_, ref_pcm.data(), (int)ref_pcm.size()) == 0) {
                            if (!p.no_prints)
                                fprintf(stderr, "stelnettts[pocket-tts]: auto-loaded voice from '%s'\n", *fb);
                            loaded = true;
                            break;
                        }
                    }
            ...
            if (!loaded && !p.no_prints) {
                fprintf(stderr, "stelnettts[pocket-tts]: WARNING: no --voice specified; "
    ...
    std::vector<float> synthesize(const std::string& text, const whisper_params& params) override {
        ...
        if (!voice_path.empty() && voice_path != last_voice_key_) {
            std::vector<float> ref_pcm;
            int ref_sr = 0;
            if (!stelnettts::core::read_wav_mono_pcm16(voice_path, ref_pcm, ref_sr)) {
                fprintf(stderr, "stelnettts[pocket-tts]: failed to read voice reference '%s'\n", voice_path.c_str());
            } else {
                ...
                if (rc != 0 && !params.no_prints) {
                    fprintf(stderr, "stelnettts[pocket-tts]: voice cloning failed (rc=%d)\n", rc);
                }
            }
