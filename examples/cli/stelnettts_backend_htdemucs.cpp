// stelnettts_backend_htdemucs.cpp — HTDemucs source-separation backend shim.
//
// Source separation is its OWN task (audio out, N named stems), not
// transcription. It is exposed through the `--separate` dispatcher
// (examples/cli/stelnettts_separate_cli.cpp), which resolves + resamples the
...
public:
    ...
    bool init(const whisper_params&) override {
        fprintf(stderr, "stelnettts: htdemucs is a source-separation model — run it with --separate\n"
                        "  (e.g. `stelnettts --separate -m <htdemucs.gguf> -f mix.wav`), not as a\n"
