// stelnettts_backend_higgs_stt.cpp — adapter for bosonai/higgs-audio-v3-stt.
//
...
public:
    ...
    uint32_t capabilities() const override {
        ...
        // stelnettts_run.cpp auto-chunk gate must NOT fire: a CLI-level window
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[higgs-stt]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (!params.ask.empty()) {
            ...
            const std::string instr = "Transcribe the speech in " + stelnettts_iso_to_english_lang(params.language) +
