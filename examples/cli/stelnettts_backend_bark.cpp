// stelnettts_backend_bark.cpp -- adapter for Suno Bark TTS (MIT).
//
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[bark]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[bark]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
    ...
    void load_speaker(const std::string& voice) {
        ...
        if (voice.size() > 4 && voice.substr(voice.size() - 4) == ".npz") {
            if (bark_set_speaker_npz(ctx_, voice.c_str()) == 0) {
                ...
                fprintf(stderr, "stelnettts[bark]: failed to load speaker from '%s'\n", voice.c_str());
            }
        } else {
            ...
            fprintf(stderr, "stelnettts[bark]: speaker presets not yet supported, "
                            "use --voice <path>.npz\n");
        }
    }
};
