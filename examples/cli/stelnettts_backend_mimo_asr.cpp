// stelnettts_backend_mimo_asr.cpp — XiaomiMiMo/MiMo-V2.5-ASR backend.
//
...
public:
    ...
    bool init(const whisper_params& params) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[mimo-asr]: failed to load LM model '%s'\n", params.model.c_str());
            return false;
        }
        ...
        if (tok_path.empty()) {
            fprintf(stderr, "stelnettts[mimo-asr]: audio tokenizer GGUF not found next to LM. Options:\n"
                            ...
                            "  3. Download manually:\n"
                            "       hf download Xenna/mimo-tokenizer-GGUF mimo-tokenizer-q4_k.gguf "
                            "--local-dir <dir-of-LM>\n");
            return false;
        }
        if (mimo_asr_set_tokenizer_path(ctx_, tok_path.c_str()) != 0) {
            fprintf(stderr, "stelnettts[mimo-asr]: failed to register tokenizer path '%s'\n", tok_path.c_str());
            return false;
        }
        if (!params.no_prints)
            fprintf(stderr, "stelnettts[mimo-asr]: tokenizer at '%s'\n", tok_path.c_str());
        return true;
    }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (!params.ask.empty()) {
            ...
            const std::string instr =
                "Please transcribe this audio in " + stelnettts_iso_to_english_lang(params.language) + ".";
