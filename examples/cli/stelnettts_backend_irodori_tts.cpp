// stelnettts_backend_irodori_tts.cpp — adapter for Aratako/Irodori-TTS.
//
...
//
// Usage:
//   stelnettts --backend irodori-tts -m auto --tts "こんにちは" -o out.wav
...
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            std::fprintf(stderr, "stelnettts[irodori-tts]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        ...
        if (const char* e = std::getenv("STELNETTTS_IRODORI_ODE_STEPS"))
            irodori_tts_set_ode_steps(ctx_, std::atoi(e));
        if (const char* e = std::getenv("STELNETTTS_IRODORI_CFG_TEXT"))
            irodori_tts_set_cfg_scale_text(ctx_, (float)std::atof(e));
        if (const char* e = std::getenv("STELNETTTS_IRODORI_CFG_SPEAKER"))
            irodori_tts_set_cfg_scale_speaker(ctx_, (float)std::atof(e));
        if (const char* e = std::getenv("STELNETTTS_IRODORI_CFG_CAPTION"))
        ...
        if (!codec_path.empty()) {
            ...
            if (!p.no_prints) {
                std::fprintf(stderr, "stelnettts[irodori-tts]: codec path = '%s'\n", codec_path.c_str());
            }
        } else if (!p.no_prints) {
            std::fprintf(stderr, "stelnettts[irodori-tts]: no DAC-VAE codec found. Pass --codec-model PATH or place "
    ...
    std::vector<float> synthesize(const std::string& text, const whisper_params& p) override {
        ...
        else if (const char* cap = std::getenv("STELNETTTS_IRODORI_CAPTION"))
...
private:
    ...
    void apply_reference(const whisper_params& p) {
        ...
        if (!stelnettts::core::read_wav_mono_pcm16(p.tts_voice, pcm, sr) || pcm.empty() || sr <= 0) {
            std::fprintf(stderr, "stelnettts[irodori-tts]: failed to load reference audio '%s' (cloning disabled)\n",
                         p.tts_voice.c_str());
            irodori_tts_clear_reference(ctx_);
        ...
        if (!p.no_prints) {
            std::fprintf(stderr, "stelnettts[irodori-tts]: reference voice '%s' (%d samples, %d Hz)\n",
