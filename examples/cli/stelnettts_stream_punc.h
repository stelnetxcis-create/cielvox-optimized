// stelnettts_stream_punc.h — streaming punctuation placement policy
...
// the two places `stelnettts_run.cpp` calls into it. Extracted into a
...
inline bool stelnettts_stream_punc_partials_enabled(const std::string& mode) {
    return mode == "partial";
}

inline bool stelnettts_stream_punc_finals_enabled(const std::string& mode) {
...
/// truth-source.
inline bool stelnettts_stream_punc_mode_valid(const std::string& mode) {
