...
// unified `stelnettts_write_*` writers already support (txt/srt/vtt/csv/lrc/
...
// as CAP_* bits the wrapper does NOT advertise. `stelnettts_run.cpp` then
...
// `kWhisperCaps` constant in stelnettts_backend.cpp because the wrapper didn't
...
public:
    ...
    uint32_t capabilities() const override {
        ...
        // stelnettts_run_backend. Features exclusive to the historical
        ...
        // CAP_PUNCTUATION_NATIVE so stelnettts_should_auto_enable_punctuation does
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[whisper]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        whisper_full_params wp =
            whisper_full_default_params(p.beam_size > 1 ? STELNETTTS_SAMPLING_BEAM_SEARCH : STELNETTTS_SAMPLING_GREEDY);
        ...
        // slicing the stelnettts dispatcher does upstream — that one produces
        // `stelnettts_audio_slice` batches that we transcribe one slice at a
        ...
        // StelnetTTS's dispatch layer (stelnettts_compute_audio_slices) already
        ...
        //
        // Always disable whisper-internal VAD; the StelnetTTS dispatch handles
        ...
        if (use_grammar) {
            ...
            if (it != p.grammar_parsed.symbol_ids.end()) {
                ...
                wp.strategy = STELNETTTS_SAMPLING_BEAM_SEARCH;
            } else {
                fprintf(stderr, "stelnettts[whisper]: grammar rule '%s' not found — ignoring\n", p.grammar_rule.c_str());
            }
        }
        ...
        if (rc != 0) {
            fprintf(stderr, "stelnettts[whisper]: %s failed\n", nproc > 1 ? "whisper_full_parallel" : "whisper_full");
            return out;
        }
    ...
    // dispatcher-level post-step (stelnettts_diarize.cpp). The whisper
