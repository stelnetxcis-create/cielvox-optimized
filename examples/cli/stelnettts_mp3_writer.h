// stelnettts_mp3_writer.h — MP3 serializer for TTS float32 PCM, built on
// our in-tree glint encoder (glint/). Header-only like
// stelnettts_wav_writer.h so the unit tests can exercise it without
...
// libmp3lame (STELNETTTS_HAVE_LAME), it serves as an optional fallback:
// used automatically if glint fails, or forced with
// STELNETTTS_MP3_ENCODER=lame (A/B comparisons).
//
// AI-generated provenance: the encoded stream is prefixed with the
// ID3v2 TXXX tag from stelnettts_make_id3v2_ai_tag() (stelnettts_wav_writer.h).
...
inline int stelnettts_mp3_target_rate(int sr) {
...
// clamped (same semantics as stelnettts_make_wav_int16). Non-MP3 sample
...
// failure.
inline std::string stelnettts_make_mp3_glint(const float* pcm, int n_samples, int sample_rate, int bitrate_kbps = 128) {
    if (!pcm || n_samples <= 0 || sample_rate <= 0)
        return {};

    const int enc_rate = stelnettts_mp3_target_rate(sample_rate);
    ...
    std::string out = stelnettts_make_id3v2_ai_tag();
...
#ifdef STELNETTTS_HAVE_LAME
// Optional libmp3lame path (fallback / STELNETTTS_MP3_ENCODER=lame).
...
// resamples instead).
inline std::string stelnettts_make_mp3_lame(const float* pcm, int n_samples, int sample_rate, int bitrate_kbps = 128) {
    if (!pcm || n_samples <= 0 || sample_rate <= 0)
        return {};
    ...
    std::string id3 = stelnettts_make_id3v2_ai_tag();
    id3.append((const char*)mp3_buf.data(), (size_t)(written + flushed));
    return id3;
}
#endif // STELNETTTS_HAVE_LAME

// Encoder dispatch: glint by default; libmp3lame (when compiled in) as
// automatic fallback on glint failure, or forced via
// STELNETTTS_MP3_ENCODER=lame. A forced-lame failure still falls back to
// glint rather than returning empty.
inline std::string stelnettts_make_mp3(const float* pcm, int n_samples, int sample_rate, int bitrate_kbps = 128) {
    if (!pcm || n_samples <= 0 || sample_rate <= 0)
        return {};
#ifdef STELNETTTS_HAVE_LAME
    const char* pref = std::getenv("STELNETTTS_MP3_ENCODER");
    if (pref && std::strcmp(pref, "lame") == 0) {
        std::string out = stelnettts_make_mp3_lame(pcm, n_samples, sample_rate, bitrate_kbps);
        if (!out.empty())
            return out;
    }
#endif
    std::string out = stelnettts_make_mp3_glint(pcm, n_samples, sample_rate, bitrate_kbps);
#ifdef STELNETTTS_HAVE_LAME
    if (out.empty())
        out = stelnettts_make_mp3_lame(pcm, n_samples, sample_rate, bitrate_kbps);
#endif
    return out;
}
