// stelnettts_backend_omniasr.cpp — OmniASR backend adapter (CTC + LLM).
...
public:
    ...
    bool init(const whisper_params& params) override {
        ...
        if (stelnettts_env::get("STELNETTTS_OMNIASR_DEBUG"))
            cp.verbosity = 2;
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (is_ctc && n_samples > kCtcMaxSamples) {
            stelnettts_ctc_logits merged_logits;
        ...
        if (!params.no_prints && n_runs > 1 && r)
            fprintf(stderr, "stelnettts[omniasr]: best-of-%d picked score=%.4f\n", n_runs, best_score);
    ...
    const stelnettts_ctc_logits* last_ctc_logits() const override {
