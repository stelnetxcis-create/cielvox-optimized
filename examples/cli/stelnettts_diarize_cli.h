// stelnettts_diarize_cli.h — CLI-side diarization shim.
//
// The four in-process diarization methods (energy, xcorr, vad-turns,
// pyannote-native) live in `src/stelnettts_diarize.h` so every StelnetTTS
...
#include "stelnettts_diarize.h" // from src/ via whisper target's PUBLIC include dir
#include "stelnettts_backend.h"
#include "stelnettts_vad.h" // stelnettts_audio_slice
...
/// `"(speaker N) "` to match the historical stelnettts convention.
///
...
///
/// That re-walk is the subtle part: `stelnettts_apply_diarize` GROWS the vector
