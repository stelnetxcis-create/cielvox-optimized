// stelnettts_backend_moss_tts_local.cpp — CLI adapter for
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[moss-tts-local]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[moss-tts-local]: failed to load backbone '%s'\n", p.model.c_str());
            return false;
        }
        ...
        if (codec_path.empty()) {
            fprintf(stderr, "stelnettts[moss-tts-local]: no codec model found. Pass --codec-model PATH or place "
                            "<stem>-codec.gguf next to the backbone.\n");
            return false;
        }
        if (!moss_tts_local_set_codec_path(ctx_, codec_path.c_str())) {
            fprintf(stderr, "stelnettts[moss-tts-local]: failed to load codec '%s'\n", codec_path.c_str());
            return false;
        }
        if (!p.no_prints)
            fprintf(stderr, "stelnettts[moss-tts-local]: codec loaded from '%s'\n", codec_path.c_str());
        return true;
    }
    ...
    void prepare_voice(const whisper_params& params) {
        ...
        if (!moss_tts_local_can_clone(ctx_)) {
            fprintf(stderr, "stelnettts[moss-tts-local]: this codec GGUF is decode-only (no encoder tensors) — "
        ...
        if (!stelnettts::core::read_wav_mono_pcm16(params.tts_voice, ref, sr) || ref.empty()) {
            fprintf(stderr, "stelnettts[moss-tts-local]: failed to load reference audio '%s'\n",
        ...
        if (!codes) {
            fprintf(stderr, "stelnettts[moss-tts-local]: reference encode failed for '%s'\n", params.tts_voice.c_str());
            last_voice_.clear();
            return;
        }
        ...
        if (!params.no_prints)
            fprintf(stderr, "stelnettts[moss-tts-local]: cloning voice from '%s' (%d frames)\n", params.tts_voice.c_str(),
                    t_audio);
    }
    ...
    std::vector<float> synthesize(const std::string& text, const whisper_params& params) override {
        ...
        if (params.tts_min_speech_tokens >= 0) {
            ...
            win.frame_cap = sp.max_new_frames > 0 ? sp.max_new_frames : stelnettts_speech_window::kDefaultFrameCap;
            const std::string warn = stelnettts_speech_window::diagnose(win);
            if (!warn.empty())
                fprintf(stderr, "stelnettts[moss-tts-local]: warning: %s\n", warn.c_str());
        }
