// stelnettts_backend_qwen3.cpp — adapter for Qwen/Qwen3-ASR-0.6B.
//
...
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[qwen3]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (!mel) {
            fprintf(stderr, "stelnettts[qwen3]: mel failed\n");
            return out;
        }
        ...
        free(mel);
        if (!audio_embeds) {
            fprintf(stderr, "stelnettts[qwen3]: encoder failed\n");
            return out;
        }
        ...
        //                     <|audio_end|><|im_end|>\n
        //                     <|im_start|>assistant\n[PREFILL]
        // Blueprint contract (qwen_asr inference/cielvox2_asr.py
        ...
        // SYSTEM turn ("Translate the speech to TGT.") — a StelnetTTS
        // extension; cielvox2-asr honours system-prompt control.
        // ISO-639-1 codes are mapped to plain English names via
        // stelnettts_iso_to_english_lang — qwen3 reads the prompt
        // literally and a bare "de" would be read as Spanish ("de" =
        // "of"). STELNETTTS_QWEN3_SYSPROMPT_LANG=1 restores the legacy
        // "Transcribe the speech in X." system-turn form.
        const bool legacy_lang_prompt = [] {
            const char* e = getenv("STELNETTTS_QWEN3_SYSPROMPT_LANG");
            return e && atoi(e) != 0;
        }();
        std::string sys_instruction;
        std::string assistant_prefill;
        if (!params.ask.empty()) {
            // Note: cielvox2-asr is an ASR-specific fine-tune that may
            ...
            const std::string tgt =
                params.target_lang.empty() ? std::string("English") : stelnettts_iso_to_english_lang(params.target_lang);
            sys_instruction = "Translate the speech to " + tgt + ".";
        } else if (!params.language.empty() && params.language != "auto") {
            if (legacy_lang_prompt)
                sys_instruction = "Transcribe the speech in " + stelnettts_iso_to_english_lang(params.language) + ".";
            else
                assistant_prefill = "language " + stelnettts_iso_to_english_lang(params.language) + "<asr_text>";
        }
        ...
        if (!raw_ids) {
            fprintf(stderr, "stelnettts[qwen3]: tokenize failed\n");
            free(audio_embeds);
            return out;
        }
        ...
        free(pad_id_arr);
        if (audio_pad_id < 0) {
            fprintf(stderr, "stelnettts[qwen3]: could not resolve <|audio_pad|> id\n");
            free(audio_embeds);
            return out;
        }
        ...
        if (!text_embeds) {
            fprintf(stderr, "stelnettts[qwen3]: embed failed\n");
            free(audio_embeds);
            return out;
        }
        int spliced = 0;
        ...
        if (!cielvox2_asr_kv_init(ctx_, std::max(4096, prompt_len + max_new + 16))) {
            free(text_embeds);
            fprintf(stderr, "stelnettts[qwen3]: kv_init failed\n");
            return out;
        }
        ...
        if (params.beam_size > 1) {
            ...
            free(text_embeds);
            if (!logits) {
                fprintf(stderr, "stelnettts[qwen3]: prefill failed\n");
                return out;
            }
            ...
            std::vector<stelnettts_token> out_tokens;
            ...
            if (!params.punctuation) {
                stelnettts_strip_ascii_punctuation(seg.text);
                stelnettts_lowercase_ascii(seg.text);
                for (auto& tk : seg.tokens) {
                    stelnettts_strip_ascii_punctuation(tk.text);
                    stelnettts_lowercase_ascii(tk.text);
                }
            }
            if (!seg.text.empty())
        ...
        for (int run = 0; run < n_runs; run++) {
            ...
            float* logits = cielvox2_asr_run_llm_kv(ctx_, text_embeds, prompt_len, 0, &n_t, &vocab);
            if (!logits) {
                fprintf(stderr, "stelnettts[qwen3]: prefill failed (run %d/%d)\n", run + 1, n_runs);
                free(text_embeds);
                return out;
            }
            if (run == 0)
        ...
        if (!params.no_prints && n_runs > 1)
            fprintf(stderr, "stelnettts[qwen3]: best-of-%d picked score=%.4f\n", n_runs, best_score);
        ...
        if (!params.no_prints && !detected_language.empty()) {
            ...
            fprintf(stderr, "stelnettts[qwen3]: detected '%s' (p=1.000) via model output\n", code.c_str());
        }
    ...
    void transcribe_streaming(const float* samples, int n_samples, int64_t t_offset_cs, const whisper_params& params,
        ...
        if (!cielvox2_asr_kv_init(ctx_, std::max(4096, prompt_len + stream_max_new + 16))) {
            free(text_embeds);
            return;
        }
