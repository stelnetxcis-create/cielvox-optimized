// stelnettts_backend_voxcpm2_tts.cpp — adapter for openbmb/VoxCPM2 TTS.
//
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[voxcpm2-tts]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        const char* env_steps = stelnettts_env::get("STELNETTTS_VOXCPM2_INFERENCE_STEPS");
        if (env_steps)
            cp.inference_steps = atoi(env_steps);

        const char* env_cfg = stelnettts_env::get("STELNETTTS_VOXCPM2_CFG_VALUE");
        if (env_cfg)
            cp.cfg_value = (float)atof(env_cfg);

        const char* env_max = stelnettts_env::get("STELNETTTS_VOXCPM2_MAX_LEN");
        if (env_max)
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[voxcpm2-tts]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
    ...
    std::vector<float> synthesize(const std::string& text, const whisper_params& params) override {
        ...
        if (!ref_path.empty()) {
            int sr = 0;
            if (stelnettts::core::read_wav_mono_pcm16(ref_path, ref_pcm, sr)) {
                if (sr != 16000 && sr > 0) {
                    ref_pcm = core_audio::resample_polyphase(ref_pcm.data(), (int)ref_pcm.size(), sr, 16000);
                    if (!params.no_prints) {
                        fprintf(stderr, "stelnettts[voxcpm2-tts]: resampled reference '%s' from %d Hz to 16000 Hz\n",
                                ref_path.c_str(), sr);
                    }
                }
                if (!params.no_prints) {
                    fprintf(stderr, "stelnettts[voxcpm2-tts]: loaded reference audio '%s' (%zu samples @ 16 kHz)\n",
                            ref_path.c_str(), ref_pcm.size());
                }
            } else {
                fprintf(stderr,
                        "stelnettts[voxcpm2-tts]: failed to load reference audio '%s' — falling back to zero-shot\n",
                        ref_path.c_str());
            }
        }
        ...
        if (!pcm || n <= 0) {
            fprintf(stderr, "stelnettts[voxcpm2-tts]: synthesis returned no audio\n");
            return {};
        }
