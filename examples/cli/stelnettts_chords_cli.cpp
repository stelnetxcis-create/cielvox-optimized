// stelnettts_chords_cli.cpp — see stelnettts_chords_cli.h.
...
int stelnettts_run_chords(const whisper_params& params) {
    if (params.fname_inp.empty()) {
        fprintf(stderr, "stelnettts: --chords needs an input file (-f)\n");
        return 2;
    }

    const bool json = params.chords_format == "json";
    if (!json && !params.chords_format.empty() && params.chords_format != "text") {
        fprintf(stderr, "stelnettts: --chords-format: unknown format '%s' (expected text or json)\n",
                params.chords_format.c_str());
        return 2;
    }

    // Defaults to the `btc-chords` registry entry, which is the 170-class
    // model: it can be reduced to maj/min with STELNETTTS_BTC_MAJ_MIN=1, whereas
    // a 25-class model could never be expanded.
    const std::string backend_key = params.backend.empty() ? "btc-chords" : params.backend;
    const std::string model = stelnettts_resolve_model_cli(params.model, backend_key, params.no_prints, params.cache_dir,
                                                         params.auto_download, "", params.accept_license);
    if (model.empty()) {
        fprintf(stderr, "stelnettts: --chords: could not resolve a model.\n"
    ...
    if (!meta) {
        fprintf(stderr, "stelnettts: --chords: cannot open '%s'\n", model.c_str());
        return 2;
    }
    const std::string arch = core_gguf::kv_str(meta, "general.architecture", "");
    core_gguf::free_metadata(meta);
    if (arch != "btc") {
        fprintf(stderr, "stelnettts: --chords: '%s' is not a chord model (arch='%s'). Expected btc.\n", model.c_str(),
                arch.c_str());
        return 2;
    }
    ...
    if (!ctx) {
        fprintf(stderr, "stelnettts: --chords: failed to load '%s'\n", model.c_str());
        return 2;
    }
    ...
    for (const auto& fname : params.fname_inp) {
        ...
        if (!r) {
            fprintf(stderr, "stelnettts: --chords failed on '%s'\n", fname.c_str());
            rc = 1;
            continue;
        }
        ...
        if (!params.no_prints)
            fprintf(stderr, "stelnettts: %s: %d chord spans (%d-class vocabulary)\n", fname.c_str(), r->n_spans,
                    r->n_chords);
