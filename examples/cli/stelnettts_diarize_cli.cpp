// stelnettts_diarize_cli.cpp — CLI-side diarization shim.
//
// Routes the four in-process diarization methods to the shared library
// (`src/stelnettts_diarize.cpp`) and keeps the sherpa-ONNX subprocess
...
std::string scratch_dir() {
    ...
    std::string d = (env && *env) ? std::string(env) : stelnettts_cache::dir() + "/scratch";
    mkdir(d.c_str(), 0755);
    return d;
}
...
std::string write_temp_mono_wav(const float* samples, int n_samples) {
    std::string tmpl_s = scratch_dir() + "/stelnettts-sherpa-XXXXXX.wav";
...
void assign_speakers_from_sherpa(std::vector<stelnettts_segment>& segs, const std::vector<SherpaSegment>& sherpa) {
    if (sherpa.empty())
        return;
    for (auto& seg : segs) {
...
bool apply_sherpa(const std::vector<float>& mono, int64_t slice_t0_cs, std::vector<stelnettts_segment>& segs,
    ...
    if (params.sherpa_segment_model.empty() || params.sherpa_embedding_model.empty()) {
        fprintf(stderr, "stelnettts[diarize]: sherpa needs --sherpa-segment-model and\n"
    ...
    if (bin.find('/') != std::string::npos || bin.find('\\') != std::string::npos) {
        ...
        if (::stat(bin.c_str(), &st) != 0) {
            fprintf(stderr,
                    "stelnettts[diarize]: sherpa binary '%s' not found — pass "
    ...
    if (wav_path.empty()) {
        fprintf(stderr, "stelnettts[diarize]: failed to write temp wav\n");
        return false;
    }

    const auto args = make_sherpa_args(bin, params, wav_path);
    if (!params.no_prints)
        fprintf(stderr, "stelnettts[diarize]: %s\n", stelnettts_cli_process::join_cmdline(args).c_str());

    const int timeout_sec =
        stelnettts_cli_process::timeout_from_audio_samples("STELNETTTS_SHERPA_TIMEOUT_SEC", (int)mono.size());
    const auto run = stelnettts_cli_process::run_capture_stdout(args, timeout_sec);
    if (run.timed_out) {
        fprintf(stderr, "stelnettts[diarize]: sherpa subprocess timed out after %d s\n", timeout_sec);
        std::remove(wav_path.c_str());
        return false;
    }
    if (run.exit_code != 0) {
        fprintf(stderr, "stelnettts[diarize]: sherpa subprocess failed with exit code %d\n", run.exit_code);
        std::remove(wav_path.c_str());
        return false;
    }
    ...
    if (parsed.empty()) {
        fprintf(stderr, "stelnettts[diarize]: sherpa subprocess produced no parseable "
    ...
    if (!params.no_prints) {
        fprintf(stderr, "stelnettts[diarize]: sherpa → %zu speaker regions over %zu ASR segments\n", parsed.size(),
                segs.size());
    }
    return true;
}
...
std::string resolve_pyannote_model(const whisper_params& params) {
    ...
    if (mp.empty() || mp == "auto") {
        ...
        // in stelnettts_model_registry.cpp). The effect was that `--diarize-method
        ...
        mp = stelnettts_managed_download(
            "pyannote-seg-3.0.gguf",
            "https://huggingface.co/Xenna/pyannote-v3-segmentation-GGUF/resolve/main/pyannote-seg-3.0.gguf", "mit",
            params.no_prints, "stelnettts[diarize]", params.cache_dir, params.accept_license);
    }
    ...
    if (mp.empty() || mp == "auto") {
        ...
        if (stelnettts_registry_lookup("wespeaker", entry)) {
            if (stelnettts_license_requires_acceptance(entry.license) &&
                ...
                fprintf(stderr,
                        "stelnettts[diarize]: refusing restricted WeSpeaker weights without --accept-license %s\n",
                        stelnettts_license_tag(entry.license).c_str());
                return {};
            }
            if (!params.no_prints)
                fprintf(stderr, "stelnettts[diarize]: weights license: %s\n", entry.license.c_str());
        }
        mp = stelnettts_cache::ensure_cached_file(
            "wespeaker-resnet34-lm.gguf",
            "https://huggingface.co/Xenna/wespeaker-resnet34-lm-GGUF/resolve/main/wespeaker-resnet34-lm.gguf",
            params.no_prints, "stelnettts[diarize]", params.cache_dir);
    }
    return mp;
}
...
void split_segments_on_pyannote_turns(std::vector<stelnettts_segment>& segs, const CrispasrPyannoteCache& cache) {
    ...
    for (auto& seg : segs) {
        ...
        const auto runs =
            stelnettts_diarize_internal::group_words_into_speaker_runs(word_spk, word_t0, word_t1, MIN_RUN_CS);
        ...
        for (size_t ri = 0; ri < runs.size(); ri++) {
            ...
            // handling matches the canonical StelnetTTS convention:
...
bool stelnettts_compute_sherpa_cache(const float* full_audio, int n_samples, const whisper_params& params,
    ...
    const std::string bin =
        params.sherpa_bin.empty() ? std::string("sherpa-onnx-offline-speaker-diarization") : params.sherpa_bin;
    if (params.sherpa_segment_model.empty() || params.sherpa_embedding_model.empty()) {
        fprintf(stderr, "stelnettts[diarize]: sherpa global cache needs --sherpa-segment-model and\n"
                        "                   --sherpa-embedding-model.\n");
        return false;
    }

    if (!params.no_prints)
        fprintf(stderr, "stelnettts[diarize]: computing global sherpa timeline over %d samples (%.1f s)...\n", n_samples,
                (double)n_samples / 16000.0);

    const std::string wav_path = write_temp_mono_wav(full_audio, n_samples);
    if (wav_path.empty()) {
        fprintf(stderr, "stelnettts[diarize]: failed to write temp wav for global sherpa\n");
        return false;
    }
    ...
    const int timeout_sec = stelnettts_cli_process::timeout_from_audio_samples("STELNETTTS_SHERPA_TIMEOUT_SEC", n_samples);
    const auto run = stelnettts_cli_process::run_capture_stdout(args, timeout_sec);
    if (run.timed_out) {
        fprintf(stderr, "stelnettts[diarize]: sherpa global run timed out after %d s\n", timeout_sec);
        std::remove(wav_path.c_str());
        return false;
    }
    if (run.exit_code != 0) {
        fprintf(stderr, "stelnettts[diarize]: sherpa global run failed with exit code %d\n", run.exit_code);
        std::remove(wav_path.c_str());
        return false;
    }
    ...
    if (out.segments.empty()) {
        fprintf(stderr, "stelnettts[diarize]: sherpa global run produced no segments\n");
        return false;
    }

    if (!params.no_prints)
        fprintf(stderr, "stelnettts[diarize]: sherpa global → %zu speaker regions\n", out.segments.size());
    return true;
}

void stelnettts_diarize_merged_by_slice(
    std::vector<stelnettts_segment>& segs, const std::vector<stelnettts_audio_slice>& slices,
    const std::function<void(const stelnettts_audio_slice&, std::vector<stelnettts_segment>&)>& diarize_slice) {
    ...
    for (size_t i = 0; i < slices.size(); ++i) {
        ...
        std::vector<stelnettts_segment> slice_segs(
...
bool stelnettts_apply_foxnose_global(std::vector<stelnettts_segment>& all_segs, const std::vector<float>& samples,
                                   const whisper_params& params) {
    if (!params.diarize || !params.diarize_embedder_is_foxnose() || all_segs.empty() || samples.empty())
        return false;
    if (params.diarize_embedder.empty()) {
        fprintf(stderr, "stelnettts[diarize]: foxnose needs --diarize-embedder <wespeaker.gguf>\n");
        return false;
    }
    ...
    if (!stelnettts_diarize_segments(pcm, pcm, (int)samples.size(), /*is_stereo=*/false, lib_segs, opts, &turns))
        return false;
    apply_int_speakers_to_stelnettts_segments(lib_segs, all_segs);
    split_segments_on_foxnose_turns(all_segs, turns, /*slice_t0_cs=*/0);
    if (!params.no_prints)
        fprintf(stderr, "stelnettts[diarize]: foxnose global pass — %zu turn(s) over %.1f s\n", turns.size(),
...
bool stelnettts_apply_diarize(const std::vector<float>& left, const std::vector<float>& right, bool is_stereo,
    ...
    if (use_lib && lib_method == CrispasrDiarizeMethod::Pyannote && pyannote_cache && pyannote_cache->valid()) {
        ...
        // overrides per sub-segment via word-range scoring).
        {
            ...
            stelnettts_diarize_internal::assign_speakers_from_log_posteriors(
                pyannote_cache->log_probs.data(), pyannote_cache->T, pyannote_cache->frame_dur_s,
                /*slice_t0_cs=*/0, lib_segs);
            apply_int_speakers_to_stelnettts_segments(lib_segs, segs);
        }
    ...
    fprintf(stderr,
            "stelnettts[diarize]: unknown --diarize-method '%s'. Known: energy, xcorr, "
...
static int stelnettts_embed_workers(size_t n_candidates) {
    if (const char* e = std::getenv("STELNETTTS_SPEAKER_EMBED_WORKERS")) {
...
static void stelnettts_embed_segments(const std::vector<stelnettts_segment>& segs, const float* full_audio, int n_samples,
    ...
    if (std::getenv("STELNETTTS_DIARIZE_DEBUG")) {
        ...
        fprintf(stderr, "stelnettts[diarize]: embed %zu segments in %.0f ms (%.1f ms/seg, %zu worker%s)\n", jobs.size(),
...
void stelnettts_remap_speakers_via_embeddings(std::vector<stelnettts_segment>& segs, const float* full_audio, int n_samples,
    ...
    std::vector<float> embeddings;
    stelnettts_embed_segments(segs, full_audio, n_samples, embedder, d, embed_idx, embeddings);
    ...
    if (params.diarize_cluster_threshold_explicit) {
        labels = stelnettts_agglomerative_cluster(embeddings, n_emb, d, thr, max_spk);
    } else {
        ...
        if (std::getenv("STELNETTTS_DIARIZE_DEBUG"))
            fprintf(stderr, "stelnettts[diarize]: n_emb=%d dim=%d -> k=%d (min_spk=%d, %s, cos_p10=%.4f, pca=%d)\n",
...
int stelnettts_identify_speaker_clusters(std::vector<stelnettts_segment>& segs, const CrispasrClusterEmbeddings& ce,
    ...
    const auto centroids = stelnettts_cluster_centroids(ce.embeddings, ce.labels, (int)ce.seg_idx.size(), ce.dim);
    ...
    for (int c = 0; c < ce.n_clusters; c++) {
        ...
        if (!name) {
            if (!no_prints)
                fprintf(stderr, "stelnettts: speaker-db: cluster %d -> unmatched, keeps (speaker %d) (best cos %.2f)\n",
                        c, c, score);
            continue;
        }
        ...
        if (!no_prints)
            fprintf(stderr, "stelnettts: speaker-db: cluster %d -> '%s' (cos %.2f)\n", c, name, score);
        n_named++;
    }
    return n_named;
}
...
bool stelnettts_identify_single_speaker(std::vector<stelnettts_segment>& segs, const float* full_audio, int n_samples,
    ...
    const auto centroid = stelnettts_cluster_centroids(embeddings, labels, (int)embed_idx.size(), d);
    ...
    if (!name) {
        if (!no_prints)
            fprintf(stderr, "stelnettts: speaker-db: recording unmatched, labels stay anonymous (best cos %.2f)\n",
                    score);
        return false;
    }
    ...
    for (auto& seg : segs)
        seg.speaker = label;
    if (!no_prints)
        fprintf(stderr, "stelnettts: speaker-db: recording -> '%s' (cos %.2f)\n", name, score);
    return true;
}
