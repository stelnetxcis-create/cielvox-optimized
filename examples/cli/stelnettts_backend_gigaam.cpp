// stelnettts_backend_gigaam.cpp — adapter for ai-sage/GigaAM-v3 (Russian ASR).
//
// Wraps gigaam_init_from_file + gigaam_transcribe_ex and converts the native
// gigaam_result into std::vector<stelnettts_segment>.
...
public:
    ...
    uint32_t capabilities() const override {
        ...
        // gates stelnettts_should_auto_enable_punctuation().
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[gigaam]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        // GigaAM-v3 is Russian-only; a language flag cannot steer it.
        if (!p.language.empty() && p.language != "auto" && p.language != "ru") {
            fprintf(stderr, "stelnettts[gigaam]: '%s' ignored — GigaAM-v3 is a Russian-only model\n", p.language.c_str());
        }
        return true;
    }
