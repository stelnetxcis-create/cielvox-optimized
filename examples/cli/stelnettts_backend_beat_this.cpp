// stelnettts_backend_beat_this.cpp — Beat This! beat-tracking backend shim.
//
...
// precedent that settled this for stems, stelnettts_backend_crepe.cpp for the
// pitch equivalent and stelnettts_backend_btc.cpp for chords. It is exposed
// through the `--beats` dispatcher (examples/cli/stelnettts_beats_cli.cpp), which
...
// stelnettts_backend.cpp rather than after the fact: btc-chords shipped its
...
public:
    ...
    bool init(const whisper_params&) override {
        fprintf(stderr, "stelnettts: beat-this is a beat-tracking model — run it with --beats\n"
                        "  (e.g. `stelnettts --beats -m <beat-this-f16.gguf> -f song.wav`), not as a\n"
