// stelnettts_mp4_writer.h — minimal ISO-BMFF (MP4/M4A) muxer for a single audio
...
#include "stelnettts_aac_writer.h" // stelnettts_make_aac (ADTS) + target-rate

namespace stelnettts_mp4 {
...
inline std::string make_aac_mp4(const float* pcm, int n_samples, int sample_rate, int bitrate_kbps = 96) {
    std::string adts = stelnettts_make_aac(pcm, n_samples, sample_rate, bitrate_kbps);
    if (adts.empty())
        return {};
