// stelnettts_backend_wav2vec2.cpp — adapter for Wav2Vec2ForCTC models.
//
...
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!wav2vec2_load(p.model.c_str(), *model_)) {
            fprintf(stderr, "stelnettts[wav2vec2]: failed to load '%s'\n", p.model.c_str());
            model_.reset();
            return false;
        }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (logits.empty()) {
            fprintf(stderr, "stelnettts[wav2vec2]: compute_logits failed\n");
            return out;
        }
        ...
        if (stelnettts_env::get("STELNETTTS_WAV2VEC2_BENCH"))
