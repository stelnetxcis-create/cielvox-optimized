// stelnettts_backend_nemotron.cpp — adapter for nvidia/nemotron-3.5-asr-streaming-0.6b.
//
// Wraps nemotron_init_from_file + nemotron_transcribe_ex and converts the
// native nemotron_result into std::vector<stelnettts_segment>.
...
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[nemotron]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        // MAES beam search (env: STELNETTTS_NEMOTRON_MAES=1).
        // Requires beam_size > 1. Configurable via env vars.
        {
            const char* maes_env = std::getenv("STELNETTTS_NEMOTRON_MAES");
