// stelnettts_backend_dia.cpp — adapter for nari-labs/Dia-1.6B
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[dia]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[dia]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        ...
        if (!codec_path.empty()) {
            ...
            if (!p.no_prints) {
                fprintf(stderr, "stelnettts[dia]: DAC codec path = '%s'\n", codec_path.c_str());
            }
        } else if (!p.no_prints) {
            fprintf(stderr, "stelnettts[dia]: no separate DAC codec found (expecting embedded in main GGUF)\n");
        }
