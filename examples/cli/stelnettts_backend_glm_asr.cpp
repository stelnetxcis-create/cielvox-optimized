// stelnettts_backend_glm_asr.cpp — GLM-ASR-Nano backend adapter.
...
public:
    ...
    uint32_t capabilities() const override {
        ...
        // — see stelnettts_backend_parakeet.cpp for the same reasoning.
    ...
    bool init(const whisper_params& params) override {
        ...
        cp.use_gpu = stelnettts_backend_should_use_gpu(params);
        ...
        if (!params.no_prints && !params.translate && !params.language.empty() && params.language != "auto") {
            ...
            if (!glm_lang) {
                fprintf(stderr,
                        "stelnettts[glm-asr]: WARNING: this model only transcribes Chinese (+ Chinese "
                        ...
                        l.c_str(), stelnettts_iso_to_english_lang(l).c_str());
            }
        }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (!params.ask.empty()) {
            glm_asr_set_ask(ctx_, params.ask.c_str());
                   ...
                   !params.translate) {
            const std::string instr = "Please transcribe in " + stelnettts_iso_to_english_lang(params.language) + ".";
        ...
        if (!params.no_prints && n_runs > 1)
            fprintf(stderr, "stelnettts[glm-asr]: best-of-%d picked score=%.4f\n", n_runs, best_score);
        ...
        std::vector<stelnettts_token> all_tokens;
        all_tokens.reserve((size_t)r->n_tokens);
        for (int i = 0; i < r->n_tokens; i++) {
            stelnettts_token tok;
        ...
        if (!params.punctuation) {
            ...
            for (auto& tok : seg.tokens) {
                stelnettts_strip_ascii_punctuation(tok.text);
                stelnettts_lowercase_ascii(tok.text);
            }
        }
    ...
    void transcribe_streaming(const float* samples, int n_samples, int64_t /*t_offset_cs*/,
        ...
        ids.push_back(59253);     // <|user|>
        {
            ...
            if (!params.ask.empty()) {
                ...
                instr = "\nPlease transcribe in " + stelnettts_iso_to_english_lang(params.language) + ".";
            }
            bool emitted = false;
            if (!instr.empty()) {
                int n_instr = 0;
        ...
        for (int step = 0; step < 512; step++) {
            ...
            if (raw) {
                ...
                if (!params.punctuation) {
                    stelnettts_strip_ascii_punctuation(piece);
                    stelnettts_lowercase_ascii(piece);
                }
                accumulated += piece;
