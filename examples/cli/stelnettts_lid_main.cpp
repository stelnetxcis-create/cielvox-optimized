...
//
// Companion to stelnettts (audio ASR) for tagging the language of a
...
// Usage:
//
//   stelnettts-lid -m /path/to/lid-glotlid-f16.gguf --text "Hello world"
//   stelnettts-lid -m /path/to/cld3-f16.gguf --text "Hallo Welt"
//   echo "Bonjour le monde" | stelnettts-lid -m lid-glotlid-f16.gguf
//   stelnettts-lid -m cld3-f16.gguf -k 5 < transcript.txt
//
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        auto need_arg = [&](const char* flag) -> const char* {
            if (i + 1 >= argc) {
                std::fprintf(stderr, "stelnettts-lid: missing value for %s\n", flag);
                std::exit(2);
            }
            return argv[++i];
        };
        if (a == "-h" || a == "--help") {
            ...
            std::fprintf(stderr, "stelnettts-lid: unknown arg '%s'\n", a.c_str());
            usage(stderr, argv[0]);
            return 2;
        }
    }
    ...
    if (model_path.empty()) {
        std::fprintf(stderr, "stelnettts-lid: -m/--model is required\n");
        usage(stderr, argv[0]);
        return 2;
    }
    if (!have_text) {
    ...
    if (text.empty()) {
        std::fprintf(stderr, "stelnettts-lid: empty input text\n");
        return 2;
    }
    ...
    if (topk == 1) {
        ...
        if (!lab) {
            std::fprintf(stderr, "stelnettts-lid: prediction failed\n");
            text_lid_free(ctx);
            return 1;
        }
        std::printf("%s\t%.6f\n", lab, conf);
    ...
    if (!quiet) {
        std::fprintf(stderr, "stelnettts-lid: backend=%s variant=%s dim=%d %d labels\n", text_lid_backend(ctx),
