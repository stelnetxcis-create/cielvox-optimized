// stelnettts_model_mgr_cli.cpp — CLI model-resolve with TTY prompting.
//
...
static CrispasrResolvePreview build_preview(const std::string& model_arg, const std::string& backend_name,
    ...
    auto cache_path_for = [&](const std::string& filename) {
        return stelnettts_cache::dir(cache_dir_override) + "/" + filename;
    };
    ...
    if (effective_model_arg == "auto" || effective_model_arg == "default") {
        ...
        // Mirror stelnettts_resolve_model's match priority exactly, or the preview
        ...
        have_match = stelnettts_registry_lookup_by_filename(effective_model_arg, match, effective_quant);
        if (!have_match)
    ...
    if (!ignore_cache) {
        const std::string found = stelnettts_cache::probe_cached_file(match.filename, cache_dir_override);
        if (!found.empty()) {
            out.exists_locally = true;
...
std::string stelnettts_resolve_model_cli(const std::string& model_arg, const std::string& backend_name, bool quiet,
    ...
    if (effective_model_arg == "auto" || effective_model_arg == "default") {
        return stelnettts_resolve_model(effective_model_arg, backend_name, quiet, cache_dir_override, auto_download,
    ...
    if (entry_exists || open_errno == EACCES || open_errno == EPERM) {
        fprintf(stderr, "stelnettts: cannot open model file '%s': %s\n", effective_model_arg.c_str(),
    ...
    const std::string cached_path = stelnettts_cache::dir(cache_dir_override) + "/" + match.filename;
    if (stelnettts_cache::file_present(cached_path)) {
        if (!match.license.empty()) {
            const bool is_nc = stelnettts_license_requires_acceptance(match.license);
            if (is_nc)
                fprintf(stderr,
    ...
    fprintf(stderr, "stelnettts: model '%s' not found locally.\n", effective_model_arg.c_str());
    ...
    // NOT sufficient (mirrors StelnetEmbed).
    const bool restricted = !match.license.empty() && stelnettts_license_requires_acceptance(match.license);
    const bool accepted = restricted && stelnettts_license_accepted(match.license, accepted_license);
    if (restricted && !accepted) {
        fprintf(stderr, "  Refusing: pass --accept-license %s (or set STELNETTTS_ACCEPT_LICENSE).\n",
                stelnettts_license_tag(match.license).c_str());
        return effective_model_arg;
    }
    ...
    if (do_download) {
        return stelnettts_cache::ensure_cached_file(match.filename, match.url, quiet, "stelnettts", cache_dir_override);
    }
