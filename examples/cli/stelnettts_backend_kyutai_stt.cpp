// stelnettts_backend_kyutai_stt.cpp — Kyutai STT backend adapter.
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (!params.language.empty() && !kyutai_stt_supports_language(ctx_, params.language.c_str()))
            fprintf(stderr, "stelnettts[kyutai-stt]: model supports %s; language='%s' ignored\n",
    ...
    std::vector<stelnettts_segment> transcribe_one(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (!params.no_prints && n_runs > 1)
            fprintf(stderr, "stelnettts[kyutai-stt]: best-of-%d picked score=%.4f\n", n_runs, best_score);
        ...
        if (!params.punctuation) {
            ...
            for (auto& w : seg.words) {
                stelnettts_strip_ascii_punctuation(w.text);
                stelnettts_lowercase_ascii(w.text);
            }
        }
