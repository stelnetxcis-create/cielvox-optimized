// stelnettts_run.cpp — top-level dispatch for non-whisper backends.
//
...
// historical stelnettts behaviour is bit-identical.

#include "stelnettts_backend.h"
#include "stelnettts_cache.h"
#include "stelnettts_gap_fill.h"
#include "stelnettts_split_pipeline.h"
#include "tada_encoder.h"

#include <sys/stat.h>
#include "stelnettts_chunk_context_gate.h"
#include "stelnettts_lcs_dedup.h"
#include "stelnettts_long_audio_fallback.h"
#include "stelnettts_mic_cli.h"
#include "stelnettts_speaker.h"
#include "stelnettts_popen.h"
#include "stelnettts_beats_cli.h"
#include "stelnettts_chords_cli.h"
#include "stelnettts_tab_cli.h"
#include "stelnettts_piano_cli.h"
#include "stelnettts_pitch_cli.h"
#include "stelnettts_separate_cli.h"
#include "stelnettts_vad_cli.h"
#include "stelnettts_output.h"
#include "stelnettts_strict.h"          // #311: shared strict-pipeline reqs (also used by the server)
#include "stelnettts_phonemes_policy.h" // #316: who can be driven by phonemes
#include "stelnettts_punctuation_policy.h"
#include "stelnettts_punc_loader.h"
#include "stelnettts_truecase_loader.h"
...
#include "stelnettts_lid_cli.h"
#include "stelnettts_lid.h" // stelnettts_lid_free_cache()
#include "stelnettts_diarize_cli.h"
#include "stelnettts_speaker_embedder.h"
#include "tiron_link.h"
#include "stelnettts_mem.h"
#include "stelnettts_stream_finalize.h"
#include "stelnettts_stream_partial_decode.h"
#include "stelnettts_stream_punc.h"
...
#include "stelnettts_mp3_writer.h"  // MP3 output via in-tree glint encoder
#include "stelnettts_aac_writer.h"  // AAC-LC (ADTS) output via in-tree glint encoder
#include "stelnettts_mp4_writer.h"  // AAC/Opus-in-MP4 muxer (C2PA-capable container)
#include "stelnettts_opus_writer.h" // Ogg Opus output via in-tree glint encoder
...
static std::string stelnettts_resolve_watermark_model(const whisper_params& params) {
    if (params.watermark_model.empty())
        return "";
    if (params.watermark_model == "auto" || params.watermark_model == "default")
        return stelnettts_resolve_model_cli("auto", "audioseal", params.no_prints, params.cache_dir, params.auto_download,
                                          "");
    return params.watermark_model;
}

// True if the container implied by `out_path` will carry a C2PA manifest under
// the current build and STELNETTTS_NO_C2PA_REMUX setting. WAV/MP3/M4A/MP4 always
// can; raw ADTS .aac / Ogg .opus can only when remux to MP4 is enabled (the
// default). When C2PA is compiled out (STELNETTTS_NO_C2PA_NATIVE and no c2pa-rs)
...
// it (see stelnettts_wm_dispatch::set_forced). Must mirror the container decision
// in stelnettts_write_synth_audio below.
static bool stelnettts_output_carries_c2pa(const std::string& out_path, bool no_c2pa = false) {
    if (no_c2pa)
...
#if defined(STELNETTTS_HAVE_C2PA) || !defined(STELNETTTS_NO_C2PA_NATIVE)
    ...
    if (is_aac || is_opus)
        return std::getenv("STELNETTTS_NO_C2PA_REMUX") == nullptr; // raw container ⇒ no manifest
...
// watermark on (overriding --no-watermark / STELNETTTS_NO_WATERMARK) so at least
...
static void stelnettts_enforce_cli_watermark_floor(const std::string& out_path, const whisper_params& params) {
    const bool carries = stelnettts_output_carries_c2pa(out_path, params.tts_no_c2pa);
    stelnettts_wm_dispatch::set_forced(!carries);
    if (!carries && (params.tts_no_watermark || std::getenv("STELNETTTS_NO_WATERMARK") != nullptr)) {
        fprintf(stderr,
                "stelnettts: note: '%s' can't carry a C2PA manifest, so --no-watermark is "
...
static int stelnettts_check_marking_attestation(const whisper_params& params) {
    ...
    if (!params.tts_marking_responsibility_accepted) {
        fprintf(stderr,
                "stelnettts: error: %s requires --accept-marking-responsibility.\n"
                ...
                "  Usage: stelnettts --tts \"text\" %s --accept-marking-responsibility\n",
                which, which);
        return 12;
    }
...
// (caller's exit code).
static int stelnettts_write_synth_audio(const std::string& out_path, const float* pcm, int n_samples, int sample_rate,
    ...
    // .m4a/.mp4 output is always AAC-in-MP4. Set STELNETTTS_NO_C2PA_REMUX=1 to keep
    // the raw .aac/.opus container (watermark + metadata provenance only).
#if defined(STELNETTTS_HAVE_C2PA) || !defined(STELNETTTS_NO_C2PA_NATIVE)
    const bool c2pa_active = std::getenv("STELNETTTS_NO_C2PA_REMUX") == nullptr;
#else
    ...
    if (aac_mp4 || opus_mp4) {
        blob = opus_mp4 ? stelnettts_mp4::make_opus_mp4(pcm, n_samples, sample_rate)
                        : stelnettts_mp4::make_aac_mp4(pcm, n_samples, sample_rate);
        if (blob.empty()) {
            fprintf(stderr, "stelnettts: error: MP4 encoding failed for '%s'\n", path.c_str());
            return 16;
        }
        c2pa_fmt = "audio/mp4";
        if (is_aac) {
            ...
            path += ".m4a";
            fprintf(stderr,
                    "stelnettts: note: emitting '%s' (AAC-in-MP4) so C2PA can embed a manifest; "
                    "set STELNETTTS_NO_C2PA_REMUX=1 for raw .aac\n",
                    path.c_str());
        } else if (is_opus) {
            ...
            path += ".mp4";
            fprintf(stderr,
                    "stelnettts: note: emitting '%s' (Opus-in-MP4) so C2PA can embed a manifest; "
                    "set STELNETTTS_NO_C2PA_REMUX=1 for raw .opus\n",
                    path.c_str());
        }
    } else if (is_mp3) {
        blob = stelnettts_make_mp3(pcm, n_samples, sample_rate);
        c2pa_fmt = "audio/mpeg";
        if (blob.empty()) {
            fprintf(stderr, "stelnettts: error: MP3 encoding failed for '%s'\n", path.c_str());
            return 16;
        }
    } else if (is_aac || is_opus) {
        // C2PA remux opted out — raw ADTS/Ogg, watermark + tag only.
        blob =
            is_aac ? stelnettts_make_aac(pcm, n_samples, sample_rate) : stelnettts_make_opus(pcm, n_samples, sample_rate);
        c2pa_fmt = "";
        if (blob.empty()) {
            fprintf(stderr, "stelnettts: error: %s encoding failed for '%s'\n", is_aac ? "AAC" : "Opus", path.c_str());
            return 16;
        }
    } else {
        blob = stelnettts_make_wav_int16(pcm, n_samples, sample_rate);
    }
    ...
    if (!sign_c2pa) {
        ...
        // is forced on (stelnettts_enforce_cli_watermark_floor) so output is still
        // marked; here we simply skip embedding the manifest.
        fprintf(stderr,
                "stelnettts: note: C2PA signing disabled (--no-c2pa); '%s' written without a manifest "
                "(audio watermark still applied)\n",
                path.c_str());
    } else if (c2pa_fmt && *c2pa_fmt) {
        stelnettts_c2pa_sign_auto(blob, c2pa_fmt, c2pa_cert, c2pa_key, cache_dir);
    } else if (!c2pa_cert.empty() || !c2pa_key.empty()) {
        fprintf(stderr,
                "stelnettts: note: C2PA cannot embed a manifest in this container; "
...
static void apply_punc_model(fireredpunc_context* punc_ctx, std::vector<stelnettts_segment>& segs) {
    if (!punc_ctx)
        return;
    for (auto& seg : segs) {
...
static bool stream_punc_partials_enabled(const whisper_params& params) {
    return stelnettts_stream_punc_partials_enabled(params.stream_punc);
}

static bool stream_punc_finals_enabled(const whisper_params& params) {
    return stelnettts_stream_punc_finals_enabled(params.stream_punc);
}

// Apply PCS (punctuation + capitalization + segmentation) to all segments.
static void apply_pcs_model(pcs_context* pcs_ctx, std::vector<stelnettts_segment>& segs) {
    if (!pcs_ctx)
        return;
    for (auto& seg : segs) {
...
static void apply_truecase_model(truecaser_context* tc_ctx, std::vector<stelnettts_segment>& segs) {
    if (!tc_ctx)
        return;
    for (auto& seg : segs) {
...
static void apply_truecase_crf_model(truecaser_crf_context* tc_crf_ctx, std::vector<stelnettts_segment>& segs) {
    if (!tc_crf_ctx)
        return;
    for (auto& seg : segs) {
...
static void apply_truecase_lstm_model(truecaser_lstm_context* tc_lstm_ctx, std::vector<stelnettts_segment>& segs) {
    if (!tc_lstm_ctx)
        return;
    for (auto& seg : segs) {
...
int warn_unsupported(const StelnetAsrBackend& backend, const whisper_params& p) {
    ...
    auto warn = [&](const char* feature) {
        fprintf(stderr, "stelnettts: warning: backend '%s' does not support %s — ignoring\n", backend.name(), feature);
        warns++;
    };

    // Diarize is now handled at the dispatcher level via the generic
    // stelnettts_apply_diarize() post-step (energy / xcorr / future
...
std::vector<stelnettts_segment> merge_segments(std::vector<std::vector<stelnettts_segment>>&& per_slice,
    ...
    if (!core_seg_hygiene::any_enabled(hy)) {
        stelnettts_warn_if_segments_backward(out, "slice merge");
        return out;
    }
    ...
    std::vector<stelnettts_segment> res;
    res.reserve(pick.size());
    for (size_t i = 0; i < pick.size(); i++) {
        stelnettts_segment seg = std::move(out[pick[i]]);
    ...
    stelnettts_warn_if_segments_backward(res, "slice merge + hygiene");
    return res;
}

bool stelnettts_words_have_positive_span(const std::vector<stelnettts_word>& words) {
...
static bool stelnettts_segs_have_text(const std::vector<stelnettts_segment>& segs) {
...
static void stelnettts_warn_if_empty_transcript(bool have_text, const std::vector<float>& samples, double audio_s,
    ...
    if (peak < 0.01f) // ~ -40 dBFS: treat as silence, no speech expected
        return;
    fprintf(stderr,
            "stelnettts: WARNING: no text produced for %.1fs of non-silent audio (peak %.2f). "
...
// (stelnettts_tiron_link_transcript) so the CLI, session C-ABI, and server all
// share ONE implementation. Returns true if the segments were tiron output.
static bool stelnettts_apply_tiron_linking(std::vector<stelnettts_segment>& segs, const std::vector<float>& samples,
                                         const whisper_params& params) {
    ...
    const int n_spk = stelnettts_tiron_link_transcript(ts, samples.data(), (int)samples.size(), spec.c_str(),
    ...
    std::vector<stelnettts_segment> kept;
    ...
    if (n_spk > 0 && !params.no_prints) {
        fprintf(stderr, "stelnettts[tiron]: linked across windows -> %d meeting-level speakers\n", n_spk);
    }
    return true;
}

static void stelnettts_apply_global_speaker_stages(std::vector<stelnettts_segment>& all_segs,
    ...
    if (stelnettts_apply_tiron_linking(all_segs, samples, params))
        return;

    // #324: foxnose runs here, once, over the whole audio — this is what makes
    // its speaker numbering consistent across slices.
    if (stelnettts_apply_foxnose_global(all_segs, samples, params))
    ...
    auto embedder = stelnettts_make_speaker_embedder(spec, params.n_threads, params.cache_dir);
    if (!embedder)
        return;
    ...
    if (claimed <= 0) {
        fprintf(stderr,
                "stelnettts: speaker-db: none of the claimed speakers (--expect-speakers '%s') are\n"
                "  enrolled in '%s' — all labels stay anonymous\n",
                params.expect_speakers.c_str(), params.speaker_db.c_str());
    } else if (want_cluster) {
        stelnettts_identify_speaker_clusters(all_segs, clusters, db, params.speaker_threshold, params.no_prints);
    } else {
        stelnettts_identify_single_speaker(all_segs, samples.data(), (int)samples.size(), embedder.get(), db,
...
enum stelnettts_strict_rc {
    STELNETTTS_STRICT_RC_VAD = 30,   // required VAD model failed to load / run
    STELNETTTS_STRICT_RC_WORDS = 31, // required word timestamps missing on a non-empty segment
    STELNETTTS_STRICT_RC_PUNC = 32,  // required punctuation model failed to load
};

// `stelnettts_strict_reqs` + `stelnettts_compute_strict_reqs` + the missing-word-ts
// counter are shared with the HTTP server via stelnettts_strict.h (included above)
...
// satisfied / not required, else STELNETTTS_STRICT_RC_WORDS after printing a
// machine-stable error. Empty/no-speech transcripts pass vacuously.
static int stelnettts_strict_check_words(const std::vector<stelnettts_segment>& segs, bool required,
                                       const std::string& fname_inp, bool aligner_load_failed = false) {
    if (!required)
        return 0;
    if (aligner_load_failed) {
        fprintf(stderr,
                "stelnettts: error: the explicitly requested forced aligner failed to load for '%s' "
                "(--require-word-timestamps/--strict-pipeline).\n",
                fname_inp.c_str());
        return STELNETTTS_STRICT_RC_WORDS;
    }
    int missing = stelnettts_count_missing_word_ts(segs);
    if (missing > 0) {
        fprintf(stderr,
                "stelnettts: error: word timestamps required (--require-word-timestamps/--strict-pipeline) but %d "
...
int process_one_input(StelnetAsrBackend& backend, const std::string& fname_inp, const std::string& fname_out,
    ...
    auto out_path = [&](const char* ext) -> std::string {
        ...
        return stelnettts_make_out_path(fname_inp, ext);
    };
    ...
    if (!read_audio_data(fname_inp, samples, stereo, want_stereo, native_rate)) {
        fprintf(stderr, "stelnettts: error: failed to read audio '%s'\n", fname_inp.c_str());
        return 20;
    }
    ...
    // they stay in original-audio time.
    {
        ...
        if (win.active && win.past_end) {
            fprintf(stderr, "stelnettts: error: --offset-t %d ms is past the end of '%s' (%.1f s)\n", params.offset_t_ms,
        ...
        if (win.active) {
            ...
            if (!params.no_prints) {
                fprintf(stderr, "stelnettts: processing window [%.2f s, %.2f s) of '%s'\n",
    ...
    // When --verbose (-v) or STELNETTTS_VERBOSE=1 is set, activate ALL
    ...
    if (params.verbose || (getenv("STELNETTTS_VERBOSE") && getenv("STELNETTTS_VERBOSE")[0])) {
        params.verbose = true;
        ...
        setenv_safe("CIELVOX2_TTS_BENCH", "1");
        setenv_safe("CIELVOX2_TTS_DEBUG", "1");
        setenv_safe("FIREREDPUNC_DEBUG", "1");
    }

    stelnettts_log_mem(params.verbose, "after audio decode");
    if (params.verbose) {
        double dur = (double)samples.size() / (double)native_rate;
        double est = stelnettts_estimate_mem_mb(dur, backend.name());
        fprintf(stderr, "stelnettts[verbose]: audio %.1fs (%zu samples, %.1f MB PCM), est encoder mem ~%.0f MB\n", dur,
    ...
    if (!params.no_prints) {
        fprintf(stderr, "stelnettts: audio: %d samples (%.1f s) @ %d Hz, %d threads\n", (int)samples.size(),
    ...
    if (!params.enroll_speaker.empty()) {
        ...
        if (!params.speaker_db_consent) {
            fprintf(stderr, "stelnettts: error: --enroll-speaker requires --speaker-db-consent.\n"
        ...
        if (tmodel.empty() || tmodel == "auto") {
            tmodel =
                stelnettts_resolve_model("auto", "titanet", params.no_prints, params.cache_dir, params.auto_download, "");
        }
        if (tmodel.empty()) {
            fprintf(stderr, "stelnettts: error: cannot resolve TitaNet model for enrollment\n");
            return 21;
        }
        auto* tctx = titanet_init(tmodel.c_str(), params.n_threads);
        if (!tctx) {
            fprintf(stderr, "stelnettts: error: failed to load TitaNet model '%s'\n", tmodel.c_str());
            return 22;
        }
        float emb[192];
        ...
        titanet_free(tctx);
        if (dim <= 0) {
            fprintf(stderr, "stelnettts: error: TitaNet embedding extraction failed\n");
            return 23;
        }
        std::string db_dir = params.speaker_db;
        if (db_dir.empty())
            db_dir = params.cache_dir.empty()
                         ? std::string(getenv("HOME") ? getenv("HOME") : ".") + "/.cache/stelnettts/speakers"
        ...
        if (!speaker_db_enroll(db_dir.c_str(), params.enroll_speaker.c_str(), emb, dim,
            ...
            fprintf(stderr, "stelnettts: error: failed to enroll speaker '%s'\n", params.enroll_speaker.c_str());
            return 24;
        }
        return 0;
    }
    ...
    stelnettts_lid_info lid_info; // stored for JSON output
    ...
    if (const char* sole_lang = backend.sole_language();
        ...
        if (!params.no_prints)
            fprintf(stderr, "stelnettts: %s is %s-only — skipping language detection\n", backend.name(), sole_lang);
    }
    ...
    if (want_auto_lang && !has_native_lid && !lid_disabled) {
        stelnettts_lid_result probe;
        if (stelnettts_backend_probe_language(backend, samples.data(), (int)samples.size(), params, probe)) {
            ...
            probed_ok = true;
            if (!params.no_prints) {
                fprintf(stderr, "stelnettts: LID -> language = '%s' (%s, p=%.3f)\n", probe.lang_code.c_str(),
    ...
    if (want_auto_lang && !has_native_lid && !lid_disabled && !probed_ok) {
        stelnettts_lid_result lid;
        if (stelnettts_detect_language_cli(samples.data(), (int)samples.size(), params, lid)) {
            ...
            if (params.language == "auto") {
                if (!params.no_prints) {
                    fprintf(stderr, "stelnettts: LID failed and no -l was set — "
                ...
                fprintf(stderr, "stelnettts: LID failed, falling back to params.language='%s'\n",
    ...
    if (stelnettts_chunk_context::backend_self_chunks_on_explicit((backend.capabilities() & CAP_INTERNAL_CHUNKING) != 0,
    ...
    if (backend.prefers_vad() && (is_long_audio || exceeds_backend_window) && !params.vad && params.vad_model.empty() &&
        ...
        if (!params.no_prints) {
            fprintf(stderr,
                    "stelnettts: %s backend auto-enabling --vad on %.1fs audio "
    ...
    const bool long_audio_no_vad =
        !params.chunk_seconds_explicit &&
        stelnettts_long_audio::should_auto_chunk_long(effective_chunk_seconds, wants_vad, backend.capabilities(),
    ...
    if (long_audio_no_vad) {
        ...
        if (!params.no_prints) {
            fprintf(stderr,
                    "stelnettts: %s backend on %.1fs audio without --vad or --chunk-seconds — "
        ...
        if (effective_chunk_seconds == 0 && (backend.capabilities() & CAP_UNBOUNDED_INPUT)) {
            fprintf(stderr,
                    "stelnettts: %s backend — full-audio / library-internal streaming "
            ...
            fprintf(stderr,
                    "stelnettts: %s backend — chunking at %ds may reduce quality; "
    ...
    if (!params.vad_import_file.empty()) {
        ...
        if (!in) {
            fprintf(stderr, "stelnettts: error: cannot open --vad-import file '%s'\n", params.vad_import_file.c_str());
            return 1;
        }
        ...
        if (!stelnettts_parse_vad_slices(ss.str(), slices, &imported_sr, &imported_chunk, &imported_raw)) {
            fprintf(stderr, "stelnettts: error: malformed --vad-import file '%s'\n", params.vad_import_file.c_str());
            return 1;
        }
        ...
        if (!imported_raw && stelnettts_vad_chunk_mismatch(imported_chunk, requested_chunk)) {
            ...
            fprintf(stderr,
                    "stelnettts: %s: --vad-import file was exported at --chunk-seconds %.2f but this run requests "
                    "%.2f.\n"
        ...
        if (imported_sr > 0 && imported_sr != SR) {
            fprintf(stderr, "stelnettts: --vad-import boundaries were computed at %d Hz, this run is %d Hz — rescaling\n",
                    imported_sr, SR);
            for (auto& s : slices) {
        ...
        std::vector<stelnettts_audio_slice> clean;
        ...
        if (imported_raw && slice_chunk_seconds > 0) {
            slices = stelnettts_rechunk_slices(slices, samples.data(), (int)samples.size(), SR, slice_chunk_seconds);
        }
        if (!params.no_prints) {
            fprintf(stderr, "stelnettts: imported %zu %s from '%s'%s\n", slices.size(),
        ...
        slices = stelnettts_compute_audio_slices(samples.data(), (int)samples.size(), SR, slice_chunk_seconds, params,
                                               &vad_load_failed);
        ...
        if (vad_load_failed && stelnettts_compute_strict_reqs(params).vad) {
            fprintf(stderr,
                    "stelnettts: error: required VAD model '%s' failed to load for '%s' "
                    "(--require-vad/--strict-pipeline) — refusing to fall back to fixed chunking.\n",
                    params.vad_model.c_str(), fname_inp.c_str());
            return STELNETTTS_STRICT_RC_VAD;
        }
    }

    // NOTE: --vad-export is now handled before backend init
    // (stelnettts_run_backend, issue #227). This site is no longer reached
    ...
    if (slices.empty()) {
        ...
        fprintf(stderr, "stelnettts: warning: no speech detected in '%s'\n", fname_inp.c_str());
        return 0;
    }

    if (!params.no_prints && slices.size() > 1) {
        fprintf(stderr, "stelnettts: processing %zu slice(s)\n", slices.size());
    }

    auto t_start = std::chrono::steady_clock::now();

    // --------------- VAD stitching path (stelnettts-style) ---------------
    ...
    if (use_stitching) {
        auto stitched = stelnettts_stitch_vad_slices(samples.data(), (int)samples.size(), SR, slices);
        if (!params.no_prints) {
            fprintf(stderr, "stelnettts: stitched %zu VAD segments → %.1fs (from %.1fs original)\n", slices.size(),
        ...
        if (params.verbose) {
            fprintf(stderr, "stelnettts[verbose]: align[stitched]: aligner='%s' caps_ctc=%d force=%d -> want=%d\n",
        ...
        if (want_align) {
            for (auto& seg : segs) {
                ...
                if (e > s) {
                    ...
                    auto words = stelnettts_ctc_align(params.aligner_model, seg.text, samples.data() + s, e - s, seg.t0,
                                                    params.n_threads, &load_failed);
                    aligner_load_failed = aligner_load_failed || load_failed;
                    if (stelnettts_words_have_positive_span(words)) {
        ...
        if (params.diarize && !segs.empty()) {
            ...
            if (have_stereo) {
                ...
                stelnettts_apply_diarize(samples, samples, /*is_stereo=*/false, 0, segs, params, pya_ptr, shp_ptr);
            }
        }

        // Fall through to the shared output path below by wrapping
        // the stitched result into per_slice / all_segs.
        std::vector<std::vector<stelnettts_segment>> stitched_per_slice(1);
        ...
        stelnettts_apply_global_speaker_stages(all_segs, samples, params);
        ...
        if (!params.punctuation) {
            for (auto& seg : all_segs)
                stelnettts_strip_punctuation(seg);
        }

        const auto disp = stelnettts_make_disp_segments(all_segs, params.max_len, params.split_on_punct);
            ...
            (params.output_srt || params.output_vtt || params.max_len > 0 || params.print_colors || params.diarize);
        {
            ...
            if (!params.no_prints) {
                fprintf(stderr, "stelnettts: transcribed %.1fs audio in %.2fs (%.1fx realtime)\n", audio_s, t_total,
                        audio_s / std::max(t_total, 0.001));
            }
            stelnettts_warn_if_empty_transcript(stelnettts_segs_have_text(all_segs), samples, audio_s, params);
            std::lock_guard<std::mutex> lock(g_stdout_mutex);
            stelnettts_print_stdout(disp, show_timestamps);
            if (params.show_alternatives)
                stelnettts_print_alternatives(all_segs, params.n_alternatives);
            else if (params.print_confidence)
                stelnettts_print_confidence(all_segs);
        }
        // #311: fail before writing output files if required word timestamps are missing.
        if (int rc = stelnettts_strict_check_words(all_segs, stelnettts_compute_strict_reqs(params).words, fname_inp,
                                                 aligner_load_failed))
            return rc;
        if (params.output_txt)
            stelnettts_write_txt(out_path(".txt"), disp);
        if (params.output_srt)
            stelnettts_write_srt(out_path(".srt"), disp);
        if (params.output_vtt)
            stelnettts_write_vtt(out_path(".vtt"), disp);
        if (params.output_csv)
            stelnettts_write_csv(out_path(".csv"), disp);
        if (params.output_lrc)
            stelnettts_write_lrc(out_path(".lrc"), disp);
        if (params.output_jsn)
            stelnettts_write_json(out_path(".json"), all_segs, backend.name(), params.model, params.language,
        ...
        if (params.return_logits) {
            if (const auto* logits = backend.last_ctc_logits()) {
                stelnettts_write_ctc_logits_json(out_path(".ctc-logits.json"), *logits, backend.name());
            } else if (!params.no_prints) {
                fprintf(stderr, "stelnettts: warning: backend '%s' did not produce CTC logits\n", backend.name());
            }
        }
        return 0;
    }
    ...
    std::vector<std::vector<stelnettts_segment>> per_slice(slices.size());
    std::vector<stelnettts_ctc_logits> per_slice_logits(slices.size());
    ...
    if (params.diarize &&
        ...
        if (!stelnettts_compute_sherpa_cache(full, n_samples_full, params, sherpa_cache)) {
    ...
    if (params.diarize && params.diarize_method == "pyannote" && !samples.empty()) {
        ...
        const int n_samples_full = (int)samples.size();
        if (!stelnettts_compute_pyannote_cache(full, n_samples_full, params, pyannote_cache)) {
            // Cache build failed (model missing, etc.). Fall back to
            // per-slice apply_pyannote — same code path as before P2a.
            // stelnettts_apply_diarize handles the missing-cache case by
    ...
    // Issue #114 — gate lives in stelnettts_chunk_context_gate.h so the
    ...
    // in stelnettts_chunk_context_gate.h; tools/check-overlap-save-bug.sh
    // is the A/B sweep that surfaces new offenders.
    const bool backend_ok = stelnettts_chunk_context::backend_allows_chunk_context(backend.name());
    const bool use_chunk_context = stelnettts_chunk_context::should_use_chunk_context(
    ...
    auto tick_slice_progress = [&]() {
        if (params.print_progress && slices.size() > 1) {
            ...
            fprintf(stderr, "stelnettts: progress = %3d%% (%zu/%zu slices)\n", pct, done, slices.size());
        }
    };
    ...
    auto finish_slice = [&](size_t i, std::vector<stelnettts_segment> segs, StelnetAsrBackend& be,
        ...
        if (use_chunk_context && !segs.empty()) {
            ...
            for (auto& seg : segs) {
                ...
                std::vector<stelnettts_word> kept;
                for (auto& w : seg.words) {
            ...
            segs.erase(
                std::remove_if(segs.begin(), segs.end(), [](const stelnettts_segment& s) { return s.text.empty(); }),
                segs.end());
        }

        // Issue #89 gap-fill second pass (bounded-window backends only).
        if (be.vad_slice_cap_seconds() > 0) {
            const char* gf = getenv("STELNETTTS_GAP_FILL");
            if (!gf || atoi(gf) != 0)
                stelnettts_gap_fill_slice(be, params, samples.data(), (int)samples.size(), SR, sl, segs);
        }
        ...
        const bool want_align =
            !params.aligner_model.empty() && ((backend.capabilities() & CAP_TIMESTAMPS_CTC) || params.force_aligner);
        if (params.verbose) {
            fprintf(stderr, "stelnettts[verbose]: align[slice]: aligner='%s' caps_ctc=%d force=%d -> want=%d\n",
                    params.aligner_model.c_str(), !!(backend.capabilities() & CAP_TIMESTAMPS_CTC),
                    params.force_aligner ? 1 : 0, want_align ? 1 : 0);
        }
        if (want_align) {
            for (auto& seg : segs) {
                if (!seg.words.empty() && !params.force_aligner)
                    continue;
                bool load_failed = false;
                auto words = stelnettts_ctc_align(params.aligner_model, seg.text, samples.data() + sl.start,
        ...
        if (params.diarize && !segs.empty()) {
            ...
            if (have_stereo) {
                ...
                stelnettts_apply_diarize(sl_l, sl_r, /*is_stereo=*/true, sl.t0_cs, segs, params, pya_ptr, shp_ptr);
            } else {
                std::vector<float> mono_slice(samples.begin() + sl.start, samples.begin() + sl.end);
                stelnettts_apply_diarize(mono_slice, mono_slice,
        ...
        // stelnettts_apply_global_speaker_stages().
    ...
    //
    // The conditions live in stelnettts_split_pipeline.h so there is one place to
    ...
    stelnettts_split::Inputs split_in;
    ...
    const bool use_split_pipeline = stelnettts_split::enabled(split_in, getenv("STELNETTTS_SLICE_PIPELINE"));
    ...
    auto pipeline_run = [&](size_t lo, size_t hi) {
        ...
        std::vector<std::vector<stelnettts_segment>> decoded(hi - lo);
        for (size_t i = lo; i < hi; i++) {
            pending p;
            {
                std::unique_lock<std::mutex> lk(m);
    ...
    if (use_split_pipeline) {
        ...
        if (!params.no_prints && params.verbose)
            fprintf(stderr, "stelnettts: pipelining encode/decode over %zu/%zu slices\n", n_ok, slices.size());
        ...
        if (!params.no_prints) {
            fprintf(stderr, "stelnettts: parallel processing %zu slices with %d workers\n", slices.size(), n_workers);
        }
        ...
        for (int w = 1; w < n_workers; w++) {
            auto wb = stelnettts_create_backend(params.backend);
            if (!wb || !wb->init(params)) {
                if (!params.no_prints)
                    fprintf(stderr, "stelnettts: warning: failed to create worker %d, reducing parallelism\n", w);
                pool_ok = false;
                break;
            }
        ...
        for (size_t i = 0; i < slices.size(); i++) {
            ...
            auto disp = stelnettts_make_disp_segments(slice_segs, params.max_len, params.split_on_punct);
        ...
        // Timing
        {
            ...
            stelnettts_warn_if_empty_transcript(progressive_any_text, samples, audio_s, params);
        }
        ...
        if (params.output_txt || params.output_vtt || params.output_csv || params.output_lrc || params.output_jsn) {
            ...
            std::vector<std::vector<stelnettts_segment>> per_slice_redo(slices.size());
            for (size_t i = 0; i < slices.size(); i++) {
                process_slice(i, backend);
            ...
            if (!params.punctuation)
                for (auto& seg : all_segs)
                    stelnettts_strip_punctuation(seg);
            auto disp_all = stelnettts_make_disp_segments(all_segs, params.max_len, params.split_on_punct);
            ...
            if (int rc = stelnettts_strict_check_words(all_segs, stelnettts_compute_strict_reqs(params).words, fname_inp,
                ...
                return rc;
            if (params.output_txt)
                stelnettts_write_txt(out_path(".txt"), disp_all);
            if (params.output_srt)
                stelnettts_write_srt(out_path(".srt"), disp_all);
            if (params.output_vtt)
                stelnettts_write_vtt(out_path(".vtt"), disp_all);
            if (params.output_csv)
                stelnettts_write_csv(out_path(".csv"), disp_all);
            if (params.output_lrc)
                stelnettts_write_lrc(out_path(".lrc"), disp_all);
            if (params.output_jsn)
                stelnettts_write_json(out_path(".json"), all_segs, backend.name(), params.model, params.language,
                                    params.output_jsn_full, lid_info.lang_code.empty() ? nullptr : &lid_info);
            if (params.return_logits) {
    ...
    if (lcs_active) {
        ...
        stelnettts_lcs::apply_lcs_chunk_dedup(per_slice, delay_tokens > 0 ? delay_tokens : 1, params.lcs_min_length);
    }
...
static int tada_run_aligner_pipeline(const whisper_params& params, const std::string& audio_path,
    ...
    const std::string aux_base = (params.backend == "tada-1b" || params.backend == "tada-tts-1b")
                                     ? "https://huggingface.co/Xenna/tada-tts-1b-GGUF/resolve/main/"
                                     : "https://huggingface.co/Xenna/tada-tts-3b-ml-GGUF/resolve/main/";
    auto resolve_aux = [&](const std::string& name) -> std::string {
        std::string local = dir_of(params.model) + "/" + name;
        struct stat st;
        if (stat(local.c_str(), &st) == 0)
            return local;
        std::string cached = stelnettts_cache::dir(params.cache_dir) + "/" + name;
        if (stelnettts_cache::file_present(cached))
            return cached;
        if (params.auto_download)
            return stelnettts_cache::ensure_cached_file(name, aux_base + name, params.no_prints, "stelnettts",
                                                      params.cache_dir);
        return std::string();
    };
    ...
    if (encoder_path.empty()) {
        fprintf(stderr,
                "stelnettts[%s]: cannot find tada-encoder GGUF. Add --auto-download to fetch it, pass "
    ...
    if (aligner_path.empty()) {
        fprintf(stderr,
                "stelnettts[%s]: cannot find tada-aligner GGUF. Add --auto-download to fetch it, pass "
                "--make-ref-aligner <path>, or place tada-aligner-en.gguf next to the model.\n",
                label);
        return 20;
    }
    fprintf(stderr, "stelnettts[%s]: encoder=%s\n", label, encoder_path.c_str());
    fprintf(stderr, "stelnettts[%s]: aligner=%s\n", label, aligner_path.c_str());
    fprintf(stderr, "stelnettts[%s]: voice=%s text='%s'\n", label, audio_path.c_str(), transcript.c_str());
    ...
    if (!read_audio_data(audio_path, ref_audio, stereo_dummy, false)) {
        fprintf(stderr, "stelnettts[%s]: failed to load audio '%s'\n", label, audio_path.c_str());
        return 20;
    }
    ...
    fprintf(stderr, "stelnettts[%s]: audio %.2fs @ 24kHz (%d samples)\n", label, n_24k / 24000.0f, n_24k);
...
// matching the file-mode `prefix_speaker()` convention in stelnettts_output.cpp.
...
static inline void stelnettts_stream_append_seg(std::string& out, const stelnettts_segment& s) {
    out += s.speaker;
    out += s.text;
}
...
static std::string stelnettts_stream_common_speaker(const std::vector<stelnettts_segment>& segs) {
    std::string spk;
    for (const auto& s : segs) {
        if (s.speaker.empty())
            continue;
...
int stelnettts_run_backend(const whisper_params& params_in) {
    ...
    // stelnettts_phonemes_policy.h). Checked HERE, before any model is loaded,
    ...
    if (!params.tts_phonemes.empty() && !stelnettts_phonemes_policy::backend_supports(params.backend)) {
        fprintf(stderr, "stelnettts: error: %s\n", stelnettts_phonemes_policy::unsupported_message(params.backend).c_str());
        return 2;
    }

    if (params.require_vad && !(params.vad || !params.vad_model.empty())) {
        fprintf(stderr, "stelnettts: error: --require-vad needs VAD to be requested (pass --vad or --vad-model/-vm).\n");
        return 2;
    }
    if (params.require_punctuation && params.punc_model.empty()) {
        fprintf(stderr, "stelnettts: error: --require-punctuation needs a punctuation model (pass --punc-model).\n");
        return 2;
    }
    ...
    if (params.stream && (!params.speaker_db.empty() || !params.enroll_speaker.empty())) {
        fprintf(stderr, "stelnettts: error: --speaker-db/--enroll-speaker are not available in streaming mode.\n"
    ...
    if (!params.speaker_db.empty() && params.enroll_speaker.empty()) {
        if (!params.speaker_db_consent) {
            fprintf(stderr, "stelnettts: --speaker-db ignored: matching named voiceprints is biometric\n"
            ...
            fprintf(stderr, "stelnettts: error: --speaker-db requires --expect-speakers \"NameA,NameB\".\n"
            ...
            if (!params.no_prints)
                fprintf(stderr, "stelnettts: --speaker-db with --diarize enables global speaker clustering "
    ...
    // skip unknowns with `if stelnettts --print-speaker-identity f; then ...`.
    if (!params.print_speaker_identity_file.empty()) {
        ...
        const stelnettts_voice::SpeakerIdentity stamped = stelnettts_voice::read_model_speaker_identity(path);
        const stelnettts_voice::PackProvenance pack = stelnettts_voice::read_pack_provenance(path);
        ...
        stelnettts_voice::SpeakerIdentity table = stelnettts_voice::identity_for_voice_pack(path);
        if (table == stelnettts_voice::SpeakerIdentity::Unknown) {
            const std::string arch = stelnettts_voice::read_gguf_architecture(path);
            if (!arch.empty())
                table = stelnettts_voice::identity_for_model(stelnettts_voice::backend_for_architecture(arch), path);
        }
        const stelnettts_voice::SpeakerIdentity id = stelnettts_voice::resolve_speaker_identity(
            /*override=*/stelnettts_voice::parse_speaker_identity(params.tts_speaker_identity),
            /*pack=*/pack.identity == stelnettts_voice::SpeakerIdentity::Unknown ? table : pack.identity,
            /*backend=*/stelnettts_voice::SpeakerIdentity::Unknown, stamped);
        printf("%s\n", stelnettts_voice::to_string(id));
        if (!params.no_prints) {
            fprintf(stderr, "stelnettts: '%s' -> speaker_identity=%s (stamp=%s, table=%s)\n", path.c_str(),
                    stelnettts_voice::to_string(id), stelnettts_voice::to_string(stamped),
                    stelnettts_voice::to_string(table));
        }
        return id == stelnettts_voice::SpeakerIdentity::Unknown ? 3 : 0;
    }
    ...
    if (!params.detect_watermark_file.empty()) {
        ...
        if (!fin) {
            fprintf(stderr, "stelnettts: error: cannot open '%s'\n", wav_path.c_str());
            return 1;
        }
        ...
        if (file_size < 44) {
            fprintf(stderr, "stelnettts: error: '%s' is too small to be a valid WAV\n", wav_path.c_str());
            fclose(fin);
            return 1;
        }
        ...
        if (fread(&wav_sr, 4, 1, fin) != 1) {
            fprintf(stderr, "stelnettts: error: cannot read sample rate from '%s'\n", wav_path.c_str());
            fclose(fin);
            return 1;
        }
        ...
        if ((int)fread(pcm_i16.data(), sizeof(int16_t), n_samples, fin) != n_samples) {
            fprintf(stderr, "stelnettts: error: short read from '%s'\n", wav_path.c_str());
            fclose(fin);
            return 1;
        }
        fclose(fin);
        ...
        stelnettts_wm_dispatch::init(stelnettts_resolve_watermark_model(params));

        float confidence = stelnettts_wm_dispatch::detect(pcm.data(), n_samples, (int)wav_sr);

        const bool neural = stelnettts_wm_dispatch::get_ctx() != nullptr;
        ...
        if (neural) {
            ...
            // the binomial null in stelnettts_watermark_stats.h does not apply.
            fprintf(stdout, "Result: %s\n",
                    confidence > 0.5f ? "AI-GENERATED WATERMARK DETECTED"
                                      : "No watermark detected (this does NOT mean the audio is human-made)");
        } else {
            // Spread-spectrum: the score is the fraction of STELNETTTS_WATERMARK_NBINS
            ...
            if (stelnettts_watermark_detect_uses_frames()) {
                ...
                const auto verdict = stelnettts_wm_stats::classify_frames(confidence);
                fprintf(stdout, "Statistic: per-frame t + decoy specificity\n");
                fprintf(stdout, "Result: %s\n", stelnettts_wm_stats::verdict_line(verdict));
                if (verdict != stelnettts_wm_stats::Verdict::Detected && dur_s < 1.0) {
                    fprintf(stdout,
                            ...
                            dur_s, stelnettts_wm::kDetectMinFrames);
                }
            } else {
                const double p = stelnettts_wm_stats::p_value(confidence, STELNETTTS_WATERMARK_NBINS);
                const auto verdict = stelnettts_wm_stats::classify(confidence, STELNETTTS_WATERMARK_NBINS);
                fprintf(stdout, "Statistic: bin-sign agreement (legacy)\n");
                fprintf(stdout, "Chance of this score without a watermark: %.2g\n", p);
                fprintf(stdout, "Result: %s\n", stelnettts_wm_stats::verdict_line(verdict));
                if (verdict != stelnettts_wm_stats::Verdict::Detected && dur_s < 10.0) {
                    fprintf(stdout,
                            ...
                            "      clips clear the 0.65 bar). Unset STELNETTTS_WATERMARK_DETECT to use the\n"
        ...
        stelnettts_wm_dispatch::shutdown();
        return 0;
    }
    ...
    if (params.align_only) {
        ...
        if (am.empty() || am == "auto" || am == "default") {
            am = stelnettts_resolve_model_cli(am.empty() ? "auto" : am, "canary-ctc-aligner", params.no_prints,
                                            params.cache_dir, params.auto_download);
        } else {
            const std::string resolved =
                stelnettts_resolve_model_cli(am, "", params.no_prints, params.cache_dir, params.auto_download);
            if (!resolved.empty())
                am = resolved;
        }
        if (am.empty()) {
            fprintf(stderr, "stelnettts[align-only]: no aligner model. Pass -am <path.gguf> "
        ...
        if (gran != "auto" && gran != "word" && gran != "segment") {
            fprintf(stderr, "stelnettts[align-only]: invalid --align-granularity '%s' (auto|word|segment)\n",
                    gran.c_str());
            return 10;
        }
        ...
        if (!params.text_file.empty()) {
            ...
            if (params.text_file == "-") {
                ...
                while ((n = fread(buf, 1, sizeof(buf), stdin)) > 0)
                    raw.append(buf, n);
                if (ferror(stdin)) {
                    fprintf(stderr, "stelnettts[align-only]: error reading transcript from stdin\n");
                    return 10;
                }
                if (raw.empty()) {
                    fprintf(stderr, "stelnettts[align-only]: --text-file - was given but stdin was empty.\n");
                    return 10;
                }
            } else {
                FILE* tf = fopen(params.text_file.c_str(), "rb");
                if (!tf) {
                    fprintf(stderr, "stelnettts[align-only]: cannot open text file '%s'\n", params.text_file.c_str());
                    return 10;
                }
                fseek(tf, 0, SEEK_END);
                long sz = ftell(tf);
                ...
                if ((long)fread(&raw[0], 1, sz, tf) != sz) {
                    fprintf(stderr, "stelnettts[align-only]: short read from '%s'\n", params.text_file.c_str());
                    fclose(tf);
                    return 10;
                }
                fclose(tf);
            ...
            if (is_srt_input) {
                for (auto& cue : stelnettts_parse_srt_cues(raw))
            ...
            fprintf(stderr, "stelnettts[align-only]: no transcript given — alignment needs the text to align.\n"
        ...
        if (transcript.empty()) {
            fprintf(stderr, "stelnettts[align-only]: transcript is empty.\n");
            return 10;
        }
        ...
        if (params.fname_inp.empty()) {
            fprintf(stderr, "stelnettts[align-only]: requires an audio file (-f <audio.wav>).\n");
            return 10;
        }
        std::vector<float> samples;
        std::vector<std::vector<float>> stereo_dummy;
        if (!read_audio_data(params.fname_inp[0], samples, stereo_dummy, false)) {
            fprintf(stderr, "stelnettts[align-only]: failed to load audio '%s'\n", params.fname_inp[0].c_str());
            return 10;
        }
        if (!params.no_prints) {
            fprintf(stderr, "stelnettts[align-only]: aligner=%s\n", am.c_str());
            fprintf(stderr, "stelnettts[align-only]: audio=%.2fs (%d samples @ 16kHz)\n",
                    (float)samples.size() / 16000.0f, (int)samples.size());
            fprintf(stderr, "stelnettts[align-only]: transcript='%.80s%s'\n", transcript.c_str(),
                    transcript.size() > 80 ? "…" : "");
        }

        // Run alignment.
        auto aligned = stelnettts_align_words(am, transcript, samples.data(), (int)samples.size(), /*t_offset_cs=*/0,
                                            params.n_threads);
        if (aligned.empty()) {
            fprintf(stderr, "stelnettts[align-only]: alignment failed or produced no words.\n");
            return 10;
        }
        ...
        if (segment_mode) {
            ...
            if (segments.empty()) {
                fprintf(stderr, "stelnettts[align-only]: segment grouping produced no segments.\n");
                return 10;
            }
        }
        ...
        if (params.align_output.empty()) {
            ...
            if (!f) {
                fprintf(stderr, "stelnettts[align-only]: cannot write '%s'\n", params.align_output.c_str());
                return 10;
            }
            ...
            if (!params.no_prints) {
                if (segment_mode)
                    fprintf(stderr, "stelnettts[align-only]: %zu words in %zu segments → %s\n", aligned.size(),
                            segments.size(), params.align_output.c_str());
                else
                    fprintf(stderr, "stelnettts[align-only]: %zu words → %s\n", aligned.size(),
                            params.align_output.c_str());
            }
        }
    ...
    if (params.verbose) {
        fprintf(stderr, "stelnettts[verbose]: model arg          = '%s'\n", params.model.c_str());
        fprintf(stderr, "stelnettts[verbose]: backend arg        = '%s'\n",
                params.backend.empty() ? "auto" : params.backend.c_str());
        fprintf(stderr, "stelnettts[verbose]: use_gpu            = %s\n", params.use_gpu ? "true" : "false");
        fprintf(stderr, "stelnettts[verbose]: gpu_backend        = '%s'\n",
                params.gpu_backend.empty() ? "auto" : params.gpu_backend.c_str());
        fprintf(stderr, "stelnettts[verbose]: gpu_device         = %d\n", params.gpu_device);
        fprintf(stderr, "stelnettts[verbose]: cache_dir override = '%s'\n",
                params.cache_dir.empty() ? "(default)" : params.cache_dir.c_str());
        fprintf(stderr, "stelnettts[verbose]: auto_download      = %s\n", params.auto_download ? "true" : "false");
        fprintf(stderr, "stelnettts[verbose]: n_threads          = %d\n", params.n_threads);
        fprintf(stderr, "stelnettts[verbose]: flash_attn         = %s\n", params.flash_attn ? "true" : "false");
    }
    ...
    if (backend_name.empty() || backend_name == "auto") {
        if (model_is_auto) {
            ...
            if (stelnettts_find_cached_model(cached, params.cache_dir, params.model_quant)) {
                ...
                if (!params.no_prints) {
                    fprintf(stderr, "stelnettts: -m auto — using cached %s model (%s)\n", backend_name.c_str(),
                            cached.filename.c_str());
                }
            } else {
                backend_name = "whisper";
                if (!params.no_prints) {
                    fprintf(stderr, "stelnettts: -m auto with no cached model — defaulting to whisper\n");
                }
            }
        } else {
            ...
            if (backend_name.empty()) {
                fprintf(stderr,
                        "stelnettts: error: could not auto-detect backend from '%s'. "
                        "Use --backend NAME to force one.\n",
                        params.model.c_str());
                return 10;
            }
            if (!params.no_prints) {
                fprintf(stderr, "stelnettts: detected backend '%s' from GGUF metadata\n", backend_name.c_str());
            }
        }
    }
    ...
    if (model_is_auto && !params.language.empty() && params.language != "auto") {
        ...
        if (is_en_chatterbox(backend_name)) {
            ...
            if (!routed.empty()) {
                if (!params.no_prints) {
                    fprintf(stderr, "stelnettts: -l %s with --backend %s — auto-routing to %s\n", params.language.c_str(),
    ...
    if (backend_name == "cohere-ar" && (params.language.empty() || params.language == "auto")) {
        params.language = "ar";
        if (!params.no_prints) {
            fprintf(stderr, "stelnettts: --backend cohere-ar — defaulting language to 'ar' (pass -l to override)\n");
        }
    }

    // MUST STAY ABOVE stelnettts_resolve_model_cli(): that call downloads the
    ...
    if (!params.vad_export_file.empty()) {
        ...
        for (size_t fi = 0; fi < params.fname_inp.size(); fi++) {
            ...
            if (!read_audio_data(fname, samples, stereo_dummy, false)) {
                fprintf(stderr, "stelnettts: error: cannot read audio '%s'\n", fname.c_str());
                vad_rc = 20;
                continue;
            }
            ...
            auto slices = stelnettts_compute_audio_slices(samples.data(), (int)samples.size(), SR, (int)slice_chunk,
            ...
            if (export_vad_load_failed && stelnettts_compute_strict_reqs(params).vad) {
                fprintf(stderr,
                        "stelnettts: error: required VAD model '%s' failed to load for '%s' "
            ...
            if (params.fname_inp.size() > 1) {
                export_path = stelnettts_make_out_path(fname, ".vad.json");
            }
            std::ofstream out(export_path, std::ios::binary | std::ios::trunc);
            if (!out) {
                fprintf(stderr, "stelnettts: warning: cannot write --vad-export file '%s'\n", export_path.c_str());
                vad_rc = 1;
            } else {
                out << stelnettts_serialize_vad_slices(slices, SR, slice_chunk, params.vad_export_raw);
                if (!params.no_prints) {
                    ...
                    fprintf(stderr, "stelnettts: exported %zu %s to '%s'\n", slices.size(),
    ...
    const std::string resolved = stelnettts_resolve_model_cli(params.model, backend_name, params.no_prints,
                                                            params.cache_dir, params.auto_download, params.model_quant);
    if (params.verbose) {
        fprintf(stderr, "stelnettts[verbose]: resolved model     = '%s'\n", resolved.c_str());
    }
    if (resolved.empty()) {
        return 11;
    }
    ...
    if (backend_name == "parakeet" && stelnettts_gguf_is_pure_ctc(params.model)) {
        if (!params.no_prints) {
            fprintf(stderr,
                    "stelnettts: '%s' is a pure-CTC model (no RNN-T decoder) — the parakeet\n"
                    "stelnettts: backend is transducer-only; auto-routing to --backend fastconformer-ctc\n",
                    params.model.c_str());
        }
    ...
    // orpheus / indextts / cielvox2-tts), so ASR backends with a hard
        ...
        if (stelnettts_registry_lookup(backend_name, entry, params.model_quant) && !entry.companion_filename.empty()) {
            ...
            if (!companion_found) {
                const std::string cached =
                    stelnettts_cache::probe_cached_file(entry.companion_filename, params.cache_dir);
                if (!cached.empty())
                    companion_found = true;
            }

            if (!companion_found) {
                const std::string resolved_companion = stelnettts_resolve_model_cli(
                ...
                if (params.verbose) {
                    fprintf(stderr, "stelnettts[verbose]: resolved companion = '%s'\n", resolved_companion.c_str());
                }
            } else if (params.verbose) {
                fprintf(stderr, "stelnettts[verbose]: companion '%s' found locally, skipping download\n",
    ...
    if (backend_name == "canary" && params.aligner_model.empty() && !params.no_auto_aligner && !params.stream &&
        ...
        if (wants_word_ts) {
            ...
            if (!params.no_prints) {
                fprintf(stderr, "stelnettts: canary auto-aligner: enabling `-am auto --force-aligner` "
                                "(canary-ctc-aligner, ~442 MB; ~78 ms MAE vs ~414 ms for native DTW). "
                                "Pass --no-auto-aligner to disable.\n");
            }
            if (params.verbose) {
                fprintf(stderr, "stelnettts[verbose]: auto-aligner: backend=canary wants_word_ts=1 "
    ...
    if (params.aligner_model == "auto" || params.aligner_model == "default") {
        const std::string resolved_aligner = stelnettts_resolve_model_cli(
            params.aligner_model, "canary-ctc-aligner", params.no_prints, params.cache_dir, params.auto_download);
        if (resolved_aligner.empty()) {
            fprintf(stderr, "stelnettts: error: failed to resolve `-am auto` (canary-ctc-aligner)\n");
            return 19;
        }
        if (params.verbose) {
            fprintf(stderr, "stelnettts[verbose]: resolved aligner   = '%s'\n", resolved_aligner.c_str());
        }
        params.aligner_model = resolved_aligner;
    } else if (!params.aligner_model.empty()) {
        const std::string resolved_aligner = stelnettts_resolve_model_cli(params.aligner_model, "", params.no_prints,
                                                                        params.cache_dir, params.auto_download);
    ...
    // --ref-text (the transcript drives the alignment). #201 follow-up.
    {
        ...
        if (is_tada && !params.tts_text.empty() && voice_is_wav && !params.make_ref && !params.align) {
            if (params.tts_ref_text.empty()) {
                fprintf(stderr, "stelnettts[tada]: cloning from a .wav needs --ref-text \"exact transcript of %s\".\n",
                        v.c_str());
                return 20;
            }
            ...
            std::string tmp = stelnettts_cache::dir(params.cache_dir) + "/tada-inline-voice.gguf";
            if (tada_encoder_write_ref_gguf(tmp.c_str(), result, params.tts_ref_text.c_str(),
                ...
                fprintf(stderr, "stelnettts[tada-clone]: failed to write reference GGUF\n");
                return 20;
            }
            ...
            if (stat(tmp.c_str(), &rst) != 0 || rst.st_size == 0) {
                fprintf(stderr,
                        "stelnettts[tada-clone]: baked ref did not land at '%s' (cache dir unwritable?). "
                        "Pass --cache-dir <writable-dir>.\n",
                        tmp.c_str());
                return 20;
            }
            if (!params.no_prints)
                fprintf(stderr, "stelnettts[tada-clone]: baked voice ref (%d tokens) → %s\n", result.n_tokens,
                        tmp.c_str());
    ...
    if (!backend->init(params)) {
        fprintf(stderr, "stelnettts: error: failed to initialise backend '%s'\n", backend_name.c_str());
        return 13;
    }
    if (params.verbose) {
        fprintf(stderr, "stelnettts[verbose]: backend '%s' initialised OK\n", backend_name.c_str());
    }

    // #80e: optional warmup — transcribe a short silence buffer to
    // amortize first-call overhead (graph alloc, GPU kernel compile).
    // Enabled via --warmup or STELNETTTS_WARMUP=1.
    if (params.warmup || getenv("STELNETTTS_WARMUP")) {
        ...
        if (params.verbose || !params.no_prints) {
            ...
            fprintf(stderr, "stelnettts: warmup completed in %.0f ms\n", dt * 1000.0);
        }
    }
    ...
    // alignment dispatch loop (stelnettts_run.cpp:305 / :398 / :1252)
    ...
    if (!params.aligner_model.empty() && !params.force_aligner && (backend->capabilities() & CAP_WORD_TIMESTAMPS) &&
        !params.no_prints) {
        fprintf(stderr,
                "stelnettts: warning: --aligner-model is set, but backend '%s' already produces "
    ...
    if (!params.text_input.empty()) {
        if (!(backend->capabilities() & CAP_TRANSLATE)) {
            fprintf(stderr,
                    "stelnettts: error: backend '%s' does not support text-to-text translation "
        ...
        if (src.empty() || tgt.empty()) {
            fprintf(stderr, "stelnettts: error: --text requires source + target language. Pass `-sl <code> "
                            "-tl <code>` (or `--tr-sl` / `--tr-tl` for 2-stage pipes).\n");
            return 17;
        }
        std::string out = backend->translate_text(params.text_input, src, tgt, params);
        if (out.empty()) {
            fprintf(stderr, "stelnettts: error: translation failed\n");
            return 18;
        }
        printf("%s\n", out.c_str());
        return 0;
    ...
    if (params.make_ref || params.align) {
        ...
        if (params.tts_voice.empty()) {
            fprintf(stderr, "stelnettts[%s]: requires --voice <audio.wav>\n", verb);
            return 20;
        }
        if (params.tts_ref_text.empty()) {
            fprintf(stderr, "stelnettts[%s]: requires --ref-text \"transcript of the audio\"\n", verb);
            return 20;
        }
        ...
        if (params.make_ref && !params.tts_voice_clone_consent) {
            fprintf(stderr,
                    "stelnettts[make-ref]: building a voice reference requires the --i-have-rights flag.\n"
                    "\n"
        ...
        if (params.align) {
            ...
            if (params.align_output.empty()) {
                ...
                FILE* f = fopen(params.align_output.c_str(), "wb");
                if (!f) {
                    fprintf(stderr, "stelnettts[align]: cannot write '%s'\n", params.align_output.c_str());
                    return 20;
                }
                fwrite(out.data(), 1, out.size(), f);
                fclose(f);
                fprintf(stderr, "stelnettts[align]: %d tokens → %zu words → %s\n", result.n_tokens, words.size(),
                        params.align_output.c_str());
            }
            return 0;
        }
        ...
        fprintf(stderr, "stelnettts[make-ref]: %d tokens × %d-d → %s\n", result.n_tokens, result.embed_dim,
                out_path.c_str());
        ...
        if (rc != 0) {
            fprintf(stderr, "stelnettts[make-ref]: failed to write GGUF (rc=%d)\n", rc);
            return 20;
        }
        fprintf(stderr, "stelnettts[make-ref]: saved %s\n", out_path.c_str());
        return 0;
    }
    ...
    if (!params.tts_text.empty()) {
        if (!(backend->capabilities() & CAP_TTS)) {
            fprintf(stderr, "stelnettts: error: backend '%s' does not support TTS\n", backend_name.c_str());
            return 14;
        }
        ...
        if (int rc = stelnettts_check_marking_attestation(params))
            return rc;
        // Honor the --no-watermark opt-out (equivalent to STELNETTTS_NO_WATERMARK).
        stelnettts_wm_dispatch::set_disabled(params.tts_no_watermark);
        ...
        stelnettts_consent::init_log_path_from_env();
        ...
        // (chatterbox has no .wav path at all). See stelnettts_voice_clone_policy.h.
        ...
        if (is_voice_clone && !params.tts_voice_clone_consent) {
            fprintf(stderr,
                    "stelnettts: error: voice cloning requires the --i-have-rights flag.\n"
                    "\n"
                    "  By passing --i-have-rights you attest:\n"
                    "  \"I have the consent of the speaker whose voice this clones,\n"
                    "   or it is my own voice.\"\n"
                    "\n"
                    "  Usage: stelnettts --tts \"text\" --voice speaker.wav --i-have-rights\n"
        ...
        if (is_voice_clone) {
            ...
            // full path would. See stelnettts_consent_record.h.
            ...
            const std::string ref_target =
                !params.tts_voice_source_recording.empty()
                    ...
                    : stelnettts_voice::resolve_voice_path(params.tts_voice, params.tts_voice_dir);
            std::string ref = stelnettts_consent::file_sha256(ref_target);
        ...
        // the consent gate above; see stelnettts_speaker_identity.h.
        bool identity_recognised = true;
        const stelnettts_voice::SpeakerIdentity identity_override =
            stelnettts_voice::parse_speaker_identity(params.tts_speaker_identity, &identity_recognised);
        if (!identity_recognised) {
            ...
            fprintf(stderr,
                    "stelnettts: warning: unrecognised --speaker-identity '%s'; treating as 'unknown'. "
        ...
        const bool needs_spoken_disclosure =
            stelnettts_voice::requires_spoken_disclosure(is_voice_clone, speaker_identity);
        if (stelnettts_voice::should_warn_unknown_identity(is_voice_clone, speaker_identity) &&
            stelnettts_voice::claim_unknown_identity_warning(backend_name)) {
            fprintf(stderr, "%s\n", stelnettts_voice::unknown_identity_warning(backend_name).c_str());
        }
        ...
        if (params.tts_stream) {
            if (!params.no_prints)
                fprintf(stderr, "stelnettts: streaming TTS as s16le mono @ %d Hz to stdout\n", sr_in);
            ...
            stelnettts_enforce_cli_watermark_floor("", params);
            auto emit = [&](std::vector<float>& pcm) {
                if (pcm.empty())
                    return;
                stelnettts_wm_dispatch::embed(pcm.data(), (int)pcm.size(), sr_in);
            ...
            if (needs_spoken_disclosure && !params.tts_no_spoken_disclaimer) {
                const auto& disc = stelnettts_tts_get_disclaimer(backend.get(), params);
                if (!disc.empty()) {
            ...
            const std::vector<std::string> stream_chunks =
                stelnettts_tts_plan_chunks_for_backend(params.tts_text, backend->name());
            bool any = false;
            ...
            stelnettts_wm_dispatch::shutdown();
            if (!any) {
                fprintf(stderr, "stelnettts: error: TTS synthesis failed\n");
                return 15;
            }
            if (!params.no_prints)
                fprintf(stderr, "stelnettts: TTS stream complete\n");
            return 0;
        }
        ...
        std::vector<float> audio;
        {
            ...
            for (size_t ci = 0; ci < chunks_txt.size(); ci++) {
                if (params.verbose && chunks_txt.size() > 1)
                    fprintf(stderr, "stelnettts[tts]: chunk %zu/%zu (%zu chars)\n", ci + 1, chunks_txt.size(),
                            chunks_txt[ci].size());
                std::vector<float> c = backend->synthesize(chunks_txt[ci], params);
                if (!c.empty())
                    chunk_pcm.push_back(std::move(c));
            }
            audio = stelnettts_tts_concat_with_silence(chunk_pcm, sr_in / 5);
        }
        if (audio.empty()) {
            fprintf(stderr, "stelnettts: error: TTS synthesis failed\n");
            return 15;
        ...
        if (needs_spoken_disclosure && !params.tts_no_spoken_disclaimer) {
            stelnettts_tts_prepend_disclaimer(audio, backend.get(), params);
        }
        ...
        if (params.tts_trim_silence) {
            ...
            if (cut > 0) {
                if (!params.no_prints)
                    fprintf(stderr, "stelnettts: trimmed %.2fs of leading silence\n", (double)cut / (double)sr_in);
        ...
        stelnettts_enforce_cli_watermark_floor(out_path, params);

        // Embed watermark (AudioSeal if loaded, otherwise spread-spectrum)
        stelnettts_wm_dispatch::embed(audio.data(), (int)audio.size(), sr_in);

        // Write output audio (backend-native sample rate, mono) — WAV by
        // default, MP3/AAC when --tts-output ends in .mp3/.aac.
        if (int rc = stelnettts_write_synth_audio(out_path, audio.data(), (int)audio.size(), sr_in, params.c2pa_cert,
        ...
        // number means. See stelnettts_watermark_stats.h.
        if (!stelnettts_wm_dispatch::is_disabled() && stelnettts_wm_dispatch::get_ctx() == nullptr) {
            const double dur_s = sr_in > 0 ? (double)audio.size() / (double)sr_in : 0.0;
            const float conf = stelnettts_wm_dispatch::detect(audio.data(), (int)audio.size(), sr_in);
            const bool frames = stelnettts_watermark_detect_uses_frames();
            const auto verdict = frames ? stelnettts_wm_stats::classify_frames(conf)
                                        : stelnettts_wm_stats::classify(conf, STELNETTTS_WATERMARK_NBINS);
            ...
            if (dur_s >= min_dur && verdict == stelnettts_wm_stats::Verdict::NotDetected) {
                if (frames) {
                    fprintf(stderr,
                            "stelnettts: warning: watermark self-check did not find the mark it just embedded "
                    ...
                    fprintf(stderr,
                            ...
                            conf, dur_s, stelnettts_wm_stats::p_value(conf, STELNETTTS_WATERMARK_NBINS));
                }
            }
        }
        ...
        if (is_voice_clone) {
            ...
            char ts[64];
            std::strftime(ts, sizeof(ts), "%Y-%m-%dT%H:%M:%S%z", std::localtime(&t));
            std::string out_hash = stelnettts_consent::file_sha256(out_path);
            stelnettts_consent::emit("CONSENT-OUTPUT", ts,
                                   {{"output", out_path},
        ...
        if (!params.no_prints)
            fprintf(stderr, "stelnettts: TTS output written to '%s' (%zu samples @ %d Hz, %.2f sec)\n", out_path.c_str(),
        ...
        if (params.tts_play) {
            stelnettts_speaker* spk = stelnettts_speaker_open(sr_in, 1, params.tts_play_device);
            if (!spk) {
                fprintf(stderr, "stelnettts: warning: --tts-play: could not open playback device\n");
            } else {
                if (!params.no_prints)
                    fprintf(stderr, "stelnettts: playing on '%s'\n", stelnettts_speaker_default_device_name());
                if (stelnettts_speaker_play(spk, audio.data(), (int)audio.size(), sr_in, 1) == 0)
                    stelnettts_speaker_wait(spk);
                else
                    fprintf(stderr, "stelnettts: warning: --tts-play: playback failed\n");
                stelnettts_speaker_close(spk);
            }
        }
    ...
    if (params.s2s) {
        if (!(backend->capabilities() & CAP_S2S)) {
            fprintf(stderr, "stelnettts: error: backend '%s' does not support S2S\n", backend_name.c_str());
            return 14;
        }
        if (params.fname_inp.empty() || params.fname_inp[0].empty()) {
            fprintf(stderr, "stelnettts: error: S2S requires audio input (-f <file>)\n");
            return 3;
        }
        ...
        if (audio.empty()) {
            fprintf(stderr, "stelnettts: error: S2S synthesis failed\n");
            return 15;
        }
        ...
        if (!transcript.empty()) {
            if (!params.no_prints)
                fprintf(stderr, "stelnettts: S2S transcript: %s\n", transcript.c_str());
            printf("%s\n", transcript.c_str());
        }
        ...
        // Embed watermark
        stelnettts_wm_dispatch::embed(audio.data(), (int)audio.size(), sr_out);

        // Write output audio — WAV by default, MP3/AAC when --s2s-output
        // ends in .mp3/.aac.
        if (int rc = stelnettts_write_synth_audio(out_path, audio.data(), (int)audio.size(), sr_out, params.c2pa_cert,
                                                params.c2pa_key, params.cache_dir, !params.tts_no_c2pa))
            return rc;

        if (!params.no_prints)
            fprintf(stderr, "stelnettts: S2S output written to '%s' (%zu samples @ %d Hz, %.2f sec)\n", out_path.c_str(),
    ...
    if (stelnettts_should_auto_enable_punctuation(backend->capabilities(), params)) {
        params.punc_model = "auto";
        if (!params.no_prints)
            fprintf(stderr, "stelnettts: auto-enabling punctuation restoration for backend '%s'\n", backend->name());
    }
    ...
    // in stelnettts_punc_loader.h, so the two front-ends can't drift apart.
    const stelnettts_punc_spec punc_spec = stelnettts_resolve_punc_model(params.punc_model);
    std::unique_ptr<fireredpunc_context, decltype(&fireredpunc_free)> punc_ctx(nullptr, fireredpunc_free);
    if (punc_spec.kind == stelnettts_punc_kind::fireredpunc) {
        std::string punc_path = punc_spec.direct_path;
        if (punc_path.empty() && !punc_spec.cache_filename.empty())
            punc_path = stelnettts_cache::ensure_cached_file(punc_spec.cache_filename, punc_spec.url, params.no_prints,
                                                           "stelnettts[punc]", params.cache_dir);
        if (!punc_path.empty()) {
            punc_ctx.reset(fireredpunc_init(punc_path.c_str()));
            if (!punc_ctx) {
                fprintf(stderr, "stelnettts: warning: failed to load punc model '%s' — continuing without\n",
                        punc_path.c_str());
            } else if (!params.no_prints) {
                fprintf(stderr, "stelnettts: loaded punctuation model '%s'\n", punc_path.c_str());
            }
        }
    }
    ...
    if (punc_spec.kind == stelnettts_punc_kind::pcs) {
        std::string pcs_path = punc_spec.direct_path;
        if (pcs_path.empty() && !punc_spec.cache_filename.empty())
            pcs_path = stelnettts_cache::ensure_cached_file(punc_spec.cache_filename, punc_spec.url, params.no_prints,
                                                          "stelnettts[pcs]", params.cache_dir);
        if (!pcs_path.empty()) {
            pcs_ctx.reset(pcs_init(pcs_path.c_str()));
            if (pcs_ctx) {
                punc_ctx.reset(); // PCS replaces fireredpunc
                if (!params.no_prints)
                    fprintf(stderr, "stelnettts: loaded PCS model '%s'\n", pcs_path.c_str());
            }
        }
    }
    ...
    if (stelnettts_compute_strict_reqs(params).punc && !punc_ctx && !pcs_ctx) {
        fprintf(stderr,
                "stelnettts: error: required punctuation model '%s' failed to load "
                "(--require-punctuation/--strict-pipeline).\n",
                params.punc_model.c_str());
        return STELNETTTS_STRICT_RC_PUNC;
    }
    ...
    stelnettts_load_truecase(params.truecase_model, params.no_prints, params.cache_dir, tc_ctx, tc_crf_ctx, tc_lstm_ctx,
                           "stelnettts[tc]");
    ...
    if (params.stream) {
        ...
        const std::string stream_vad_path = stelnettts_resolve_vad_model(params);

        stelnettts_vad_options stream_vad_opts;
        ...
        if (params.stream_json) {
            stream_vad_opts.post_merge_policy = stelnettts_vad_post_merge_policy::streaming_json;
        ...
        if (params.mic) {
            fprintf(stderr, "stelnettts[mic]: capturing from default microphone...\n");
            fprintf(stderr, "stelnettts[mic]: press Ctrl+C to stop\n\n");
...
#elif defined(_WIN32)
            {
                ...
                // — see issue #70 follow-up. stelnettts_popen widens to
                // wchar_t and calls _wpopen so the device name survives.
                const char* dev = stelnettts_mic_default_device_name();
                std::string dshow_arg = stelnettts_windows_dshow_audio_arg_from_name(dev);
                std::string cmd = "ffmpeg -f dshow -i " + dshow_arg + " -f s16le -ar 16000 -ac 1 -";
                if (params.stream_monitor || !params.no_prints) {
                    fprintf(stderr, "stelnettts[mic]: device=%s\n", dev && *dev ? dev : "(default)");
                    fprintf(stderr, "stelnettts[mic]: ffmpeg cmd: %s\n", cmd.c_str());
                }
                // Suppress ffmpeg stderr on the wire but leave it user-
                // discoverable (we just printed the command above).
                cmd += " 2>NUL";
                mic_pipe = stelnettts::stelnettts_popen(cmd, "rb");
            }
#else
...
#endif
            if (!mic_pipe) {
                fprintf(stderr, "stelnettts[mic]: failed to open microphone. Install sox, ffmpeg, or arecord.\n");
                return 20;
            }
        } else {
            fprintf(stderr, "stelnettts[stream]: reading raw s16le 16kHz mono PCM from stdin\n");
            fprintf(stderr, "stelnettts[stream]: step=%dms length=%dms keep=%dms\n", params.stream_step_ms,
                    params.stream_length_ms, params.stream_keep_ms);
            fprintf(stderr, "stelnettts[stream]: pipe audio in, e.g.:\n");
            fprintf(stderr, "  ffmpeg -i input.wav -f s16le -ar 16000 -ac 1 - | stelnettts --stream -m model.gguf\n\n");
        }
        ...
        const int64_t partial_decode_interval_samples =
            stelnettts_stream_partial_decode_interval_samples(params.stream_partial_decode_ms, params.stream_step_ms, SR);
        ...
        while (true) {
            ...
            if (n_read == 0) {
                if (mic_pipe && !any_samples_read) {
                    fprintf(stderr, "\nstelnettts[mic]: pipe ended before any PCM was read.\n"
            ...
            std::vector<std::pair<stelnettts_audio_slice, std::string>> step_slice_text;
            bool decoded_segments_this_step = false;
            if (!stream_vad_path.empty()) {
                const auto slices = stelnettts_compute_vad_slices(pcm_window.data(), (int)pcm_window.size(), SR,
                ...
                const bool allow_partial_decode = stelnettts_stream_partial_decode_allow(
                ...
                for (const auto& sl : slices) {
                    if (params.stream_json) {
                        ...
                        std::vector<stelnettts_segment> sl_for_text;
            ...
            if (params.stream_json && stream_vad_path.empty() && !segs.empty()) {
                ...
                for (const auto& s : segs)
                    all_text += s.text;
                stelnettts_audio_slice fake_sl{0, (int)pcm_window.size(), 0, 0};
            ...
            if (params.stream_json) {
                ...
                auto finalize_utterance = [&]() {
                    ...
                    // min — see stelnettts::kStreamRedecodeMinSamples) or
                    ...
                    if (params.stream_final_mode == "redecode") {
                        if ((int)utterance_pcm.size() >= stelnettts::kStreamRedecodeMinSamples) {
                            ...
                            for (const auto& s : utt_segs)
                                final_text += s.text;
                            final_speaker = stelnettts_stream_common_speaker(utt_segs);
                            final_text_from_redecode = !final_text.empty();
                        }
                        if (final_text.empty())
                            final_text = stelnettts::stitch_partial_accumulator(prefix_committed, last_partial_text);
                    } else {
                    ...
                    if (!final_speaker.empty())
                        spk_field = ",\"speaker\":\"" + stelnettts_json_escape(final_speaker) + "\"";
                    fprintf(stdout,
                            "{\"type\":\"final\",\"utterance_id\":%lld,\"text\":\"%s\"%s,\"t0\":%.3f,\"t1\":%.3f}\n",
                            (long long)utterance_id, stelnettts_json_escape(final_text).c_str(), spk_field.c_str(), t0,
                            t1);
                    fflush(stdout);
                ...
                auto on_partial_text = [&](const std::string& new_text) {
                    ...
                    fprintf(stdout,
                            ...
                            (long long)utterance_id, stelnettts_json_escape(new_text).c_str(), t0, t1);
                    fflush(stdout);
                };
            ...
            std::string text;
            for (const auto& s : segs)
                stelnettts_stream_append_seg(text, s);
        ...
        if (params.stream_json && have_open_utterance) {
            ...
            // in-loop finalize_utterance. See stelnettts_stream_finalize.h.
            std::string final_text;
            ...
            fprintf(stdout, "{\"type\":\"final\",\"utterance_id\":%lld,\"text\":\"%s\"%s,\"t0\":%.3f,\"t1\":%.3f}\n",
                    (long long)utterance_id, stelnettts_json_escape(final_text).c_str(), spk_field.c_str(), t0, t1);
            fflush(stdout);
        }
        fprintf(stdout, "\n");
        if (mic_pipe) {
            stelnettts::stelnettts_pclose(mic_pipe);
        }
        return 0;
    }
    ...
    // Bit-identical with the historical StelnetTTS behaviour.
    //
    ...
    if (nproc > 1 && params.fname_inp.size() > 1) {
        ...
        for (int i = 1; i < nproc; i++) {
            auto extra = stelnettts_create_backend(backend_name);
            if (!extra || !extra->init(params)) {
                fprintf(stderr,
                        "stelnettts: error: failed to spin up worker %d/%d "
        ...
        if (!params.no_prints) {
            fprintf(stderr, "stelnettts: parallel mode: %d worker(s), %zu input file(s)\n", nproc,
...
// linker drops it.
{
    ...
    std::vector<std::vector<float>> stereo;
        ...
        // a generic dispatcher post-step (stelnettts_diarize.cpp), so we
        ...
        if (!params.no_prints) {
            fprintf(stderr,
                    "stelnettts: audio: %d samples (%.1f s) @ %d Hz, %d threads\n",
                    (int)samples.size(),
                    (double)samples.size() / SR, SR, params.n_threads);
        }
        ...
        if (want_auto_lang && !has_native_lid && !lid_disabled) {
            stelnettts_lid_result lid;
            // Backend self-probe first (see stelnettts_lid_cli.h); external LID
            // only when it declines.
            const bool probed = stelnettts_backend_probe_language(*backend, samples.data(),
                                                                (int)samples.size(), params, lid);
            if (probed ||
                stelnettts_detect_language_cli(samples.data(), (int)samples.size(),
                                          params, lid)) {
                params.language = lid.lang_code;
                if (params.source_lang.empty()) {
                    params.source_lang = lid.lang_code;
                ...
                if (!params.no_prints) {
                    fprintf(stderr,
                            "stelnettts: LID -> language = '%s' (%s, p=%.3f)\n",
                            lid.lang_code.c_str(), lid.source.c_str(),
                            lid.confidence);
                }
            } else if (!params.no_prints) {
                fprintf(stderr,
                        "stelnettts: LID failed, falling back to params.language='%s'\n",
                        params.language.c_str());
            }
        }
        ...
        const auto slices = stelnettts_compute_audio_slices(
        ...
        std::vector<std::vector<stelnettts_segment>> per_slice;
        std::vector<stelnettts_ctc_logits> per_slice_logits;
        ...
        for (size_t i = 0; i < slices.size(); i++) {
            ...
            std::vector<stelnettts_segment> segs = backend->transcribe(
            ...
            if (params.verbose) {
                fprintf(stderr,
                        "stelnettts[verbose]: align: aligner='%s' caps_ctc=%d force=%d -> want=%d\n",
            ...
            if (want_align) {
                for (auto & seg : segs) {
                    ...
                    auto words = stelnettts_ctc_align(
            ...
            if (params.diarize && !segs.empty()) {
                if (have_stereo) {
                    ...
                    stelnettts_apply_diarize(sl_l, sl_r, /*is_stereo=*/true,
        ...
        if (params.return_logits) {
            ...
            if (!merged.data.empty())
                stelnettts_write_ctc_logits_json(out_path(".ctc-logits.json"), merged, backend->name());
        }
    }
