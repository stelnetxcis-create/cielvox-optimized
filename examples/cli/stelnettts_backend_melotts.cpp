// stelnettts_backend_melotts.cpp — MeloTTS (VITS2) backend adapter.
    ...
    bool init(const whisper_params& p) override {
        ...
        //   4. Auto-download from registry companion
        {
            ...
            if (!p.tts_codec_model.empty()) {
                ...
                const char* env = stelnettts_env::get("STELNETTTS_MELOTTS_BERT");
                if (env && *env) {
                    bert_path = env;
                } else {
        ...
        if (!p.tts_voice.empty()) {
            ...
            for (const char* name : {"openvoice2-tcc-f16.gguf", "openvoice2-tcc-q4_k.gguf", "openvoice2-tcc.gguf"}) {
                ...
                if (test) {
                    ...
                    if (ov2_ctx_) {
                        const char* dd = stelnettts_env::get("STELNETTTS_OV2_DUMP_DIR");
                        if (dd)
