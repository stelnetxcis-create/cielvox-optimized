// stelnettts_backend_paraformer.cpp — FunASR Paraformer (NAR-ASR) adapter.
...
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[paraformer]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (!r || !r->text) {
            ...
            fprintf(stderr, "stelnettts[paraformer]: transcribe failed\n");
            return out;
        }
