// stelnettts_stream_finalize.h — helpers shared between the streaming
// finalize_utterance / EOF flush paths in stelnettts_run.cpp and their
// unit tests.
//
// The logic here is intentionally pure (no I/O, no global state) so it
// can be tested in isolation. The streaming loop in stelnettts_run.cpp
...
// `examples/cli/stelnettts_run.cpp`):
