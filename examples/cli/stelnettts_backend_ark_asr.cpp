// stelnettts_backend_ark_asr.cpp — AutoArk-AI/ARK-ASR-3B backend.
//
...
public:
    ...
    uint32_t capabilities() const override {
        ...
        // safety cap, else internal chunks) instead of stelnettts_run's 30 s split.
    ...
    bool init(const whisper_params& params) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[ark-asr]: failed to load model '%s'\n", params.model.c_str());
            return false;
        }
        return true;
    }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        // mirrors stelnettts_backend_mimo_asr.cpp.
        if (!params.ask.empty()) {
            ark_asr_set_ask(ctx_, params.ask.c_str());
        } else if (!params.language.empty() && params.language != "auto") {
            const std::string instr = "Transcribe the audio in " + stelnettts_iso_to_english_lang(params.language) + ".";
