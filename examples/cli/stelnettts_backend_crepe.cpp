// stelnettts_backend_crepe.cpp — CREPE pitch (F0) backend shim.
//
...
// dispatcher (examples/cli/stelnettts_pitch_cli.cpp), which resolves the model,
...
public:
    ...
    bool init(const whisper_params&) override {
        fprintf(stderr, "stelnettts: crepe is a pitch (F0) model — run it with --pitch\n"
                        "  (e.g. `stelnettts --pitch -m <crepe-tiny-f16.gguf> -f audio.wav`), not as a\n"
