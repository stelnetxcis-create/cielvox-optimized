// stelnettts_tab_cli.cpp — see stelnettts_tab_cli.h.
...
int stelnettts_run_tab(const whisper_params& params) {
    if (params.fname_inp.empty()) {
        fprintf(stderr, "stelnettts: --tab needs an input file (-f)\n");
        return 2;
    }

    const bool json = params.tab_format == "json";
    if (!json && !params.tab_format.empty() && params.tab_format != "text") {
        fprintf(stderr, "stelnettts: --tab-format: unknown format '%s' (expected text or json)\n",
    ...
    if (model.empty()) {
        fprintf(stderr, "stelnettts: --tab: could not resolve a model.\n");
        return 2;
    }

    gguf_context* meta = core_gguf::open_metadata(model.c_str());
    if (!meta) {
        fprintf(stderr, "stelnettts: --tab: cannot open '%s'\n", model.c_str());
        return 2;
    }
    const std::string arch = core_gguf::kv_str(meta, "general.architecture", "");
    core_gguf::free_metadata(meta);
    if (arch != "tabcnn") {
        fprintf(stderr, "stelnettts: --tab: '%s' is not a tablature model (arch='%s'). Expected tabcnn.\n", model.c_str(),
                arch.c_str());
        return 2;
    }
    ...
    if (!ctx) {
        fprintf(stderr, "stelnettts: --tab: failed to load '%s'\n", model.c_str());
        return 2;
    }
    ...
    for (const auto& fname : params.fname_inp) {
        ...
        if (n_frames <= 0) {
            fprintf(stderr, "stelnettts: --tab: '%s' is too short\n", fname.c_str());
            rc = 1;
            continue;
        }
        ...
        if (got <= 0) {
            fprintf(stderr, "stelnettts: --tab failed on '%s'\n", fname.c_str());
            rc = 1;
            continue;
        }
        ...
        if (!params.no_prints) {
            ...
            fprintf(stderr,
                    "stelnettts: %s: %d frames (%.3f s hop), %d fretted string-frames\n"
