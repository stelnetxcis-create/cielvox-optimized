// stelnettts_backend_parler.cpp -- adapter for parler-tts/parler-tts-mini-v1.1
...
//
// Usage:
//   stelnettts -b parler-tts -m parler-tts-mini-v1.1-f16.gguf \
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[parler-tts]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[parler-tts]: failed to load model '%s'\n", model_path.c_str());
            return false;
        }
        ...
        if (rc != 0) {
            fprintf(stderr, "stelnettts[parler-tts]: failed to encode description\n");
            return false;
        }
