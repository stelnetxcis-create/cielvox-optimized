#ifndef STELNETTTS_TRUECASE_LOADER_H
#define STELNETTTS_TRUECASE_LOADER_H
...
// (stelnettts_run.cpp) and the HTTP server (stelnettts_server.cpp). There are three
...
using stelnettts_truecaser_ptr = std::unique_ptr<truecaser_context, decltype(&truecaser_free)>;
using stelnettts_truecaser_crf_ptr = std::unique_ptr<truecaser_crf_context, decltype(&truecaser_crf_free)>;
using stelnettts_truecaser_lstm_ptr = std::unique_ptr<truecaser_lstm_context, decltype(&truecaser_lstm_free)>;

// Resolve `truecase_model` and load it into exactly one of the three contexts.
// Mirrors the resolution in stelnettts_run.cpp:
//   none|off            → nothing
...
inline void stelnettts_load_truecase(const std::string& truecase_model, bool no_prints, const std::string& cache_dir,
                                   stelnettts_truecaser_ptr& tc, stelnettts_truecaser_crf_ptr& tc_crf,
                                   stelnettts_truecaser_lstm_ptr& tc_lstm, const char* label = "stelnettts") {
    ...
    if (tc_path == "lstm" || tc_path == "lstm-de" || tc_path == "lstm-en" || tc_path == "lstm-es" ||
        ...
        std::string url = "https://huggingface.co/Xenna/truecaser-de/resolve/main/" + fname;
        tc_path = stelnettts_cache::ensure_cached_file(fname, url, no_prints, label, cache_dir);
        if (!tc_path.empty()) {
    ...
    if (tc_path == "crf" || tc_path == "crf-de") {
        tc_path = stelnettts_cache::ensure_cached_file(
            "truecaser-crf-de.bin", "https://huggingface.co/Xenna/truecaser-de/resolve/main/truecaser-crf-de.bin",
    ...
    if (tc_path == "auto" || tc_path == "de") {
        tc_path = stelnettts_cache::ensure_cached_file(
            "truecaser-de.bin", "https://huggingface.co/Xenna/truecaser-de/resolve/main/truecaser-de.bin", no_prints,
            label, cache_dir);
    }
    if (!tc_path.empty()) {
...
inline void stelnettts_apply_truecase(truecaser_context* tc, truecaser_crf_context* tc_crf,
...
#endif // STELNETTTS_TRUECASE_LOADER_H
