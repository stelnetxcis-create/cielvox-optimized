// stelnettts_diagnostics.cpp — see header for the why.

#include "stelnettts_diagnostics.h"
...
#ifndef STELNETTTS_VERSION_STR
#define STELNETTTS_VERSION_STR "unknown"
#endif
#ifndef STELNETTTS_GIT_SHA1
#define STELNETTTS_GIT_SHA1 "unknown"
#endif
#ifndef STELNETTTS_GIT_DATE
#define STELNETTTS_GIT_DATE "unknown"
#endif
#ifndef STELNETTTS_GIT_SUBJECT
#define STELNETTTS_GIT_SUBJECT ""
#endif
#ifndef STELNETTTS_BUILD_DATE
#define STELNETTTS_BUILD_DATE __DATE__ " " __TIME__
#endif
#ifndef STELNETTTS_BUILD_TYPE
#define STELNETTTS_BUILD_TYPE "unknown"
#endif
#ifndef STELNETTTS_CUDA_ARCHS
#define STELNETTTS_CUDA_ARCHS ""
#endif
#ifndef STELNETTTS_CUDA_VERSION
#define STELNETTTS_CUDA_VERSION ""
#endif
...
static const char* stelnettts_compiled_backends() {
    static std::string s;
    if (!s.empty())
        return s.c_str();
    s = "cpu";
#ifdef STELNETTTS_HAVE_CUDA
    s += ",cuda";
#endif
#ifdef STELNETTTS_HAVE_METAL
    s += ",metal";
#endif
#ifdef STELNETTTS_HAVE_VULKAN
    s += ",vulkan";
#endif
#ifdef STELNETTTS_HAVE_HIP
    s += ",hip";
#endif
#ifdef STELNETTTS_HAVE_SYCL
    s += ",sycl";
#endif
#ifdef STELNETTTS_HAVE_OPENCL
    s += ",opencl";
#endif
#ifdef STELNETTTS_HAVE_BLAS
    s += ",blas";
#endif
#ifdef STELNETTTS_HAVE_MUSA
    s += ",musa";
#endif
    return s.c_str();
}

static const char* stelnettts_compiler_id() {
...
void stelnettts_print_short_banner(FILE* out) {
    std::fprintf(out, "stelnettts %s (git %s, %s) [backends: %s]\n", STELNETTTS_VERSION_STR, STELNETTTS_GIT_SHA1,
                 STELNETTTS_BUILD_TYPE, stelnettts_compiled_backends());
}

void stelnettts_print_build_info(FILE* out) {
    std::fprintf(out, "=== build info ===\n");
    std::fprintf(out, "  version       : %s\n", STELNETTTS_VERSION_STR);
    std::fprintf(out, "  git sha       : %s\n", STELNETTTS_GIT_SHA1);
    if (STELNETTTS_GIT_DATE[0] != '\0' && std::strcmp(STELNETTTS_GIT_DATE, "unknown") != 0) {
        std::fprintf(out, "  git date      : %s\n", STELNETTTS_GIT_DATE);
    }
    if (STELNETTTS_GIT_SUBJECT[0] != '\0') {
        std::fprintf(out, "  git subject   : %s\n", STELNETTTS_GIT_SUBJECT);
    }
    std::fprintf(out, "  build date    : %s\n", STELNETTTS_BUILD_DATE);
    std::fprintf(out, "  build type    : %s\n", STELNETTTS_BUILD_TYPE);
    std::fprintf(out, "  compiler      : %s\n", stelnettts_compiler_id());
...
#endif
    std::fprintf(out, "  ggml backends : %s\n", stelnettts_compiled_backends());
#ifdef STELNETTTS_HAVE_CUDA
    if (STELNETTTS_CUDA_VERSION[0] != '\0') {
        std::fprintf(out, "  cuda toolkit  : %s\n", STELNETTTS_CUDA_VERSION);
    }
    if (STELNETTTS_CUDA_ARCHS[0] != '\0') {
        std::fprintf(out, "  cuda archs    : %s\n", STELNETTTS_CUDA_ARCHS);
    }
#endif
    std::fprintf(out, "\n");
}
...
void stelnettts_print_runtime_env(FILE* out) {
    ...
    print_env_if_set(out, "STELNETTTS_USE_CUDA_COMPAT");
    print_env_if_set(out, "STELNETTTS_BACKEND");
    print_env_if_set(out, "STELNETTTS_CACHE_DIR");
...
void stelnettts_print_devices(FILE* out) {
...
void stelnettts_print_full_diagnostics(FILE* out) {
    stelnettts_print_build_info(out);
    stelnettts_print_runtime_env(out);
    stelnettts_print_devices(out);
}
