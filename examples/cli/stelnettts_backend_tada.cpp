// stelnettts_backend_tada.cpp — adapter for HumeAI/tada-3b-ml
...
// stelnettts_audio_load_at_rate lives in the shared lib (stelnettts_audio.cpp);
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float*, int, int64_t, const whisper_params&) override {
        fprintf(stderr, "stelnettts[tada]: transcription not supported\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (const char* e = stelnettts_env::get("STELNETTTS_TADA_NUM_FM_STEPS"); e && *e) {
            int n = atoi(e);
            if (n > 0)
                cp.num_fm_steps = n;
        }
        if (const char* e = stelnettts_env::get("STELNETTTS_TADA_ACOUSTIC_CFG"); e && *e)
            cp.acoustic_cfg = (float)atof(e);
        if (const char* e = stelnettts_env::get("STELNETTTS_TADA_NOISE_TEMP"); e && *e)
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[tada]: failed to load '%s'\n", p.model.c_str());
            return false;
        }
        ...
        if (!codec_path.empty()) {
            ...
            if (!p.no_prints)
                fprintf(stderr, "stelnettts[tada]: codec = '%s'\n", codec_path.c_str());
        } else if (!p.no_prints) {
            fprintf(stderr, "stelnettts[tada]: no codec found. "
        ...
        // (stelnettts_run.cpp) consumes it, so don't load it or fail on the .wav.
        if (!p.make_ref && !p.align && !p.tts_voice.empty() && p.tts_voice != "default" && p.tts_voice != "auto") {
            ...
            if (is_wav) {
                // #201: opt-in on-the-fly clone (STELNETTTS_TADA_WAV_CLONE=1). On
                ...
                prompt_path = stelnettts_resolve_model_cli(p.tts_voice, p.backend, p.no_prints, p.cache_dir,
                                                         p.auto_download, p.model_quant);
                if (prompt_path.empty()) {
                    fprintf(stderr,
                            "stelnettts[tada]: --voice '%s' could not be resolved. Pass the path to a "
        ...
        } else if (const char* env = stelnettts_env::get("STELNETTTS_TADA_PROMPT_CACHE"); env && *env) {
            prompt_path = env;
        } else {
            ...
            if (prompt_path.empty()) {
                ...
                if (!lang.empty()) {
                    ...
                    const std::string cached = stelnettts_cache::dir(p.cache_dir) + "/" + ref_name;
                    if (stelnettts_cache::file_present(cached)) {
                        prompt_path = cached;
                    } else if (p.auto_download) {
                        prompt_path = stelnettts_managed_download(
                            ...
                            p.no_prints, "stelnettts[tada]", p.cache_dir, p.accept_license);
                    } else if (!p.no_prints) {
                        fprintf(stderr,
                                "stelnettts[tada]: no voice reference for language '%s' found. "
        ...
        if (!prompt_path.empty()) {
            if (tada_load_prompt(ctx_, prompt_path.c_str()) != 0) {
                fprintf(stderr, "stelnettts[tada]: failed to load prompt from '%s'\n", prompt_path.c_str());
            } else {
                ...
                if (!p.no_prints && !explicit_voice)
                    fprintf(stderr, "stelnettts[tada]: using default voice prompt '%s'\n", prompt_path.c_str());
            }
        }
        return true;
    }
    ...
    // Mirrors stelnettts_run.cpp's resolve_aux so the adapter (server) path finds
    ...
    std::string resolve_makeref_aux(const whisper_params& p, const std::string& configured,
        ...
        if (present(local))
            return local;
        std::string cached = stelnettts_cache::dir(p.cache_dir) + "/" + fname;
        if (stelnettts_cache::file_present(cached))
            return cached;
        if (p.auto_download) {
            const std::string base = (p.backend == "tada-1b" || p.backend == "tada-tts-1b")
                                         ? "https://huggingface.co/Xenna/tada-tts-1b-GGUF/resolve/main/"
                                         : "https://huggingface.co/Xenna/tada-tts-3b-ml-GGUF/resolve/main/";
            return stelnettts_managed_download(fname, base + fname,
    ...
    // STELNETTTS_TADA_WAV_CLONE=1 a .wav voice is rejected as before. On success
    ...
    bool clone_from_wav(const whisper_params& p) {
        ...
        if (const char* g = stelnettts_env::get("STELNETTTS_TADA_WAV_CLONE"); !g || atoi(g) == 0) {
            fprintf(stderr, "stelnettts[tada]: --voice with a .wav is opt-in — set STELNETTTS_TADA_WAV_CLONE=1 to clone "
                            "at synth time, or build a tada-ref.gguf with --make-ref.\n");
            return false;
        }
        // Resolve the WAV + optional companion transcript, mirroring the server
        // voice-dir convention (cielvox2-tts): a bare name that isn't an existing
        ...
        if (ref_text.empty()) {
            fprintf(stderr, "stelnettts[tada]: cloning from a .wav needs the transcript — pass --ref-text (CLI), the "
        ...
        if (enc.empty() || ali.empty()) {
            fprintf(stderr, "stelnettts[tada]: .wav cloning needs the encoder + aligner GGUFs (add --auto-download, "
                            "pass --make-ref-encoder/--make-ref-aligner, or place them next to the model).\n");
            return false;
        }
        float* pcm = nullptr;
        int n = 0, sr = 0;
        if (stelnettts_audio_load_at_rate(wav.c_str(), 24000, &pcm, &n, &sr) != 0 || !pcm || n <= 0) {
        ...
        free(pcm);
        if (rc != 0) {
            fprintf(stderr, "stelnettts[tada]: .wav cloning failed (rc=%d)\n", rc);
            return false;
        }
        last_voice_key_ = v;
        if (!p.no_prints)
            fprintf(stderr, "stelnettts[tada]: cloned voice in-memory from '%s'\n", wav.c_str());
        return true;
    }
    ...
    bool apply_request_voice(const whisper_params& p) {
        ...
        std::string ref =
            stelnettts_resolve_model_cli(v, p.backend, p.no_prints, p.cache_dir, p.auto_download, p.model_quant);
        if (ref.empty()) {
            fprintf(stderr, "stelnettts[tada]: voice '%s' not found; keeping current voice.\n", v.c_str());
            return false;
        }
        if (tada_load_prompt(ctx_, ref.c_str()) != 0) {
            fprintf(stderr, "stelnettts[tada]: failed to load voice '%s'; keeping current voice.\n", ref.c_str());
            return false;
        }
        last_voice_key_ = v;
        if (!p.no_prints)
            fprintf(stderr, "stelnettts[tada]: switched voice → '%s'\n", ref.c_str());
        return true;
    }
