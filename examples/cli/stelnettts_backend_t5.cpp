// stelnettts_backend_t5.cpp — adapter for T5-family encoder-decoder
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[madlad]: transcription is not supported — this is a translation backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[madlad]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
    ...
    std::string translate_text(const std::string& text, const std::string& /*src_lang*/, const std::string& tgt_lang,
        ...
        if (t5_has_token(ctx_, lang_tag.c_str())) {
            ...
            if (params.verbose && !params.no_prints) {
                fprintf(stderr,
                        "stelnettts[madlad]: target-lang tag '%s' not in vocab — passing input through "
