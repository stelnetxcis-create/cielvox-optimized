// stelnettts_backend_cohere.cpp — adapter for Cohere Transcribe.
//
...
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        // long-form (>5 min). Force flash via STELNETTTS_COHERE_FLASH=1.
        // See PERFORMANCE.md §5 (PLAN #73 closeout).
        cp.use_flash = (getenv("STELNETTTS_COHERE_FLASH") != nullptr);
        cp.use_gpu = stelnettts_backend_should_use_gpu(p);
        cp.no_punctuation = !p.punctuation;
        cp.diarize = false;
        cp.verbosity = p.no_prints ? 0 : 1;
        if (getenv("STELNETTTS_VERBOSE") || stelnettts_env::get("STELNETTTS_COHERE_BENCH"))
            cp.verbosity = 2;

        ctx_ = cohere_init_from_file(p.model.c_str(), cp);
        if (!ctx_) {
            fprintf(stderr, "stelnettts[cohere]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
    ...
    bool detect_language(const float* samples, int n_samples, const whisper_params& p, std::string& out_lang,
        ...
        if (n_supported <= 0) {
            ...
            if (!p.no_prints)
                fprintf(stderr, "stelnettts[cohere]: this GGUF carries no supported-language list "
        ...
        int max_langs = 4;
        if (const char* v = stelnettts_env::get("STELNETTTS_COHERE_PROBE_MAX_LANGS"))
            max_langs = atoi(v);
        if (!forced && n_supported > max_langs) {
            if (!p.no_prints)
                fprintf(stderr,
                        "stelnettts[cohere]: %d supported languages — skipping the self-probe "
        ...
        TextLidHook hook;
        const char* textlid_env = stelnettts_env::get("STELNETTTS_COHERE_PROBE_TEXTLID");
    ...
    bool prefers_vad() const override {
        ...
        //
        // Only arms the long-audio safeguard in stelnettts_run.cpp, and only when
