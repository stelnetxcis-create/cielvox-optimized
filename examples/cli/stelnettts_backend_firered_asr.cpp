// stelnettts_backend_firered_asr.cpp — FireRedASR2-AED backend adapter.
...
public:
    ...
    uint32_t capabilities() const override {
        ...
        // (stelnettts_run.cpp) auto-chunks at 30 s when the user passes neither
    ...
    bool init(const whisper_params& params) override {
        ...
        if (getenv("STELNETTTS_VERBOSE") || stelnettts_env::get("STELNETTTS_FIRERED_BENCH"))
            cp.verbosity = 2;
        ...
        if (!params.no_prints && !params.language.empty() && params.language != "auto") {
            ...
            if (!firered_lang) {
                fprintf(stderr,
                        "stelnettts[firered-asr]: WARNING: this model only transcribes Chinese (+ Chinese "
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        // until §W1 gave it a code-point path.
        {
            ...
            if (keep.size() < seg.tokens.size()) {
                std::vector<stelnettts_token> kept;
