// stelnettts_backend_outetts.cpp -- adapter for OuteAI/OuteTTS-0.3-1B
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[outetts]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[outetts]: failed to load talker '%s'\n", p.model.c_str());
            return false;
        }
        ...
        if (!codec_path.empty()) {
            ...
            if (!p.no_prints) {
                fprintf(stderr, "stelnettts[outetts]: codec path = '%s'\n", codec_path.c_str());
            }
        } else if (!p.no_prints) {
            fprintf(stderr, "stelnettts[outetts]: no WavTokenizer codec found. Pass --codec-model PATH or place "
        ...
        if (!params.tts_voice.empty() && params.tts_voice != last_voice_key_) {
            if (params.tts_voice.size() > 5 && params.tts_voice.substr(params.tts_voice.size() - 5) == ".json") {
                if (outetts_load_speaker(ctx_, params.tts_voice.c_str()) == 0) {
                    ...
                    fprintf(stderr, "stelnettts[outetts]: failed to load speaker from '%s'\n", params.tts_voice.c_str());
                }
            }
        }
