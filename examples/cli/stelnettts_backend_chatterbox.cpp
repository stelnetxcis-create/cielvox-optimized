// stelnettts_backend_chatterbox.cpp — adapter for Chatterbox TTS (base, turbo,
...
static std::string discover_s3gen(const whisper_params& p) {
    if (!p.tts_codec_model.empty() && p.tts_codec_model != "auto" && p.tts_codec_model != "default") {
        return stelnettts_resolve_model_cli(p.tts_codec_model, p.backend, p.no_prints, p.cache_dir, p.auto_download,
                                          p.tts_codec_quant);
    }
    ...
    const std::string cache_dir = stelnettts_cache::dir(p.cache_dir);
    ...
    if (p.auto_download || !is_auto_companion_arg(p.tts_codec_model)) {
        ...
        return stelnettts_resolve_model_cli(wanted, p.backend, p.no_prints, p.cache_dir, p.auto_download,
                                          p.tts_codec_quant);
    }
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[chatterbox]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[chatterbox]: failed to load T3 model '%s'\n", p.model.c_str());
            return false;
        }
        std::string s3gen_path = discover_s3gen(p);
        if (!s3gen_path.empty()) {
            if (chatterbox_set_s3gen_path(ctx_, s3gen_path.c_str()) != 0) {
                fprintf(stderr, "stelnettts[chatterbox]: failed to load S3Gen '%s'\n", s3gen_path.c_str());
                return false;
            }
            s3gen_loaded_ = true;
            if (!p.no_prints) {
                fprintf(stderr, "stelnettts[chatterbox]: S3Gen loaded from '%s'\n", s3gen_path.c_str());
            }
        }
        return true;
    }
    ...
    std::vector<float> synthesize(const std::string& text, const whisper_params& params) override {
        if (!ctx_ || text.empty())
            return {};
        if (!s3gen_loaded_) {
            fprintf(stderr, "stelnettts[chatterbox]: S3Gen not loaded. Pass --codec-model <s3gen.gguf>\n");
            return {};
        }
        ...
        if (!params.tts_voice.empty() && params.tts_voice != last_voice_key_) {
            if (chatterbox_set_voice_from_wav(ctx_, params.tts_voice.c_str()) == 0) {
                ...
                if (!params.no_prints) {
                    fprintf(stderr, "stelnettts[chatterbox]: voice '%s' loaded\n", params.tts_voice.c_str());
                }
            } else {
