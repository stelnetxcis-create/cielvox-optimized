// stelnettts_backend_kokoro.cpp — adapter for hexgrad/Kokoro-82M and
...
// stelnettts_kokoro_* C ABI so wrappers (Python, Rust, Go, ...) can reuse
...
std::string kokoro_resolve_model(const std::string& lang, const std::string& model_path) {
    char out[1024] = {0};
    int rc = stelnettts_kokoro_resolve_model_for_lang(model_path.c_str(), lang.c_str(), out, (int)sizeof(out));
    if (rc == 0) {
        fprintf(stderr,
                "stelnettts[kokoro]: language 'de' — preferring German-trained "
...
std::string kokoro_resolve_fallback_voice(const std::string& lang, const std::string& model_path,
    ...
    char path[1024] = {0};
    char picked[64] = {0};
    int rc = stelnettts_kokoro_resolve_fallback_voice(model_path.c_str(), lang.c_str(), path, (int)sizeof(path), picked,
                                                    (int)sizeof(picked));
    if (rc != 0)
        return {};
    if (out_picked_name)
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[kokoro]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[kokoro]: failed to load model '%s'\n", resolved_model.c_str());
            return false;
        }
        return true;
    }
    ...
    std::vector<float> synthesize(const std::string& text, const whisper_params& params) override {
        ...
        if (voice_path.empty() && !params.language.empty() && params.language != "auto" &&
            !stelnettts_kokoro_lang_has_native_voice(params.language.c_str())) {
            std::string picked;
            voice_path = kokoro_resolve_fallback_voice(params.language, params.model, &picked);
            if (!voice_path.empty() && stelnettts_kokoro_lang_is_german(params.language.c_str())) {
                const bool is_kikiri = picked == "df_victoria";
                if (is_german_backbone_) {
                    fprintf(stderr,
                            "stelnettts[kokoro]: language 'de' — using %s voicepack on the "
                ...
                fprintf(stderr,
                        "stelnettts[kokoro]: no native Kokoro-82M voice for language '%s'; "
        ...
        if (!voice_loaded_ && !voice_path.empty()) {
            if (kokoro_load_voice_pack(ctx_, voice_path.c_str()) != 0) {
                fprintf(stderr, "stelnettts[kokoro]: failed to load voice pack '%s'\n", voice_path.c_str());
                return {};
            }
            voice_loaded_ = true;
        }
