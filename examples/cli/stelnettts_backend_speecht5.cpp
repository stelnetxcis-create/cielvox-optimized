// stelnettts_backend_speecht5.cpp -- adapter for Microsoft SpeechT5 TTS.
//
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[speecht5]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        sp.use_gpu = stelnettts_backend_should_use_gpu(p);

        std::string model_path = p.model;
        ctx_ = speecht5_tts_init(model_path.c_str(), sp);
        if (!ctx_) {
            fprintf(stderr, "stelnettts[speecht5]: failed to load model '%s'\n", model_path.c_str());
            return false;
        }
        ...
        if (!p.tts_voice.empty()) {
            ...
            if (!p.no_prints) {
                fprintf(stderr, "stelnettts[speecht5]: using zero speaker embedding. "
...
private:
    ...
    void load_speaker(const std::string& voice_path, bool quiet) {
        ...
        if (data.empty()) {
            if (!quiet) {
                fprintf(stderr, "stelnettts[speecht5]: failed to load speaker from '%s'\n", voice_path.c_str());
            }
            return;
        }
        ...
        if (!quiet) {
            fprintf(stderr, "stelnettts[speecht5]: loaded speaker embedding (%d dims) from '%s'\n", (int)data.size(),
                    voice_path.c_str());
        }
    }
};
