// stelnettts_strict.h — shared strict-pipeline requirement logic (issue #311).
//
// Both the CLI (stelnettts_run.cpp) and the HTTP server (stelnettts_server.cpp)
...
#include "stelnettts_backend.h" // stelnettts_segment
...
inline stelnettts_strict_reqs stelnettts_compute_strict_reqs(const whisper_params& p) {
...
inline int stelnettts_count_missing_word_ts(const std::vector<stelnettts_segment>& segs) {
    int missing = 0;
