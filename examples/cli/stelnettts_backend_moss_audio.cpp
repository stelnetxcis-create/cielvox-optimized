// stelnettts_backend_moss_audio.cpp — adapter for MOSS-Audio-4B-Instruct.
//
...
//
// First audio-understanding (not just ASR) backend in StelnetTTS —
...
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[moss-audio]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        if (p.seed > 0)
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (!params.prompt.empty()) {
            ...
            prompt_buf = "Transcribe this audio in " + stelnettts_iso_to_english_lang(params.language) + ".";
    ...
    void transcribe_streaming(const float* samples, int n_samples, int64_t /*t_offset_cs*/,
        ...
        if (!params.prompt.empty()) {
            ...
            prompt_buf_s = "Transcribe this audio in " + stelnettts_iso_to_english_lang(params.language) + ".";
