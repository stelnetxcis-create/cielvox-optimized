// stelnettts_lid_cli.cpp — CLI LID shim.
//
...
#include "stelnettts_lid.h" // shared library header (src/)
...
constexpr const char* kSileroLidDefaultUrl =
    "https://huggingface.co/Xenna/silero-lid-lang95-GGUF/resolve/main/silero-lid-lang95-f32.gguf";
...
constexpr const char* kEcapaLidDefaultUrl =
    "https://huggingface.co/Xenna/ecapa-lid-107-GGUF/resolve/main/ecapa-lid-107.gguf";
...
std::string resolve_whisper_lid_model(const whisper_params& p) {
    ...
    return stelnettts_cache::ensure_cached_file(kWhisperLidDefaultFile, kWhisperLidDefaultUrl, p.no_prints,
                                              "stelnettts[lid]", p.cache_dir);
}

std::string resolve_silero_lid_model(const whisper_params& p) {
    if (!p.lid_model.empty() && p.lid_model != "auto")
        return expand_home(p.lid_model);
    return stelnettts_cache::ensure_cached_file(kSileroLidDefaultFile, kSileroLidDefaultUrl, p.no_prints,
                                              "stelnettts[lid-silero]", p.cache_dir);
}

std::string resolve_firered_lid_model(const whisper_params& p) {
    if (!p.lid_model.empty() && p.lid_model != "auto")
        return expand_home(p.lid_model);
    return stelnettts_cache::ensure_cached_file(kFireredLidDefaultFile, kFireredLidDefaultUrl, p.no_prints,
                                              "stelnettts[lid-firered]", p.cache_dir);
}

std::string resolve_ecapa_lid_model(const whisper_params& p) {
    if (!p.lid_model.empty() && p.lid_model != "auto")
        return expand_home(p.lid_model);
    return stelnettts_cache::ensure_cached_file(kEcapaLidDefaultFile, kEcapaLidDefaultUrl, p.no_prints,
                                              "stelnettts[lid-ecapa]", p.cache_dir);
}
...
bool detect_with_sherpa(const float* samples, int n_samples, const whisper_params& p, stelnettts_lid_result& out) {
    const char* env_bin = std::getenv("STELNETTTS_SHERPA_LID_BIN");
    const std::string bin =
        env_bin && *env_bin ? std::string(env_bin) : std::string("sherpa-onnx-offline-language-identification");

    std::string tmpl_s = scratch_dir() + "/stelnettts-lid-XXXXXX.wav";
    std::vector<char> tmpl(tmpl_s.begin(), tmpl_s.end());
    tmpl.push_back('\0');
    int fd = mkstemps(tmpl.data(), 4);
    if (fd < 0) {
        fprintf(stderr, "stelnettts[lid]: mkstemps failed\n");
        return false;
    }
    close(fd);
    ...
    if (!p.no_prints)
        fprintf(stderr, "stelnettts[lid]: %s\n", stelnettts_cli_process::join_cmdline(args).c_str());

    const int timeout_sec =
        stelnettts_cli_process::timeout_from_audio_samples("STELNETTTS_SHERPA_LID_TIMEOUT_SEC", n_samples);
    const auto run = stelnettts_cli_process::run_capture_stdout(args, timeout_sec, true);
    if (run.timed_out) {
        fprintf(stderr, "stelnettts[lid]: sherpa subprocess timed out after %d s\n", timeout_sec);
        std::remove(wav_path.c_str());
        return false;
    }
    if (run.spawn_failed) {
        fprintf(stderr, "stelnettts[lid]: failed to spawn sherpa LID subprocess\n");
        std::remove(wav_path.c_str());
        return false;
    }
    if (run.exit_code != 0) {
        fprintf(stderr, "stelnettts[lid]: sherpa subprocess exited with code %d\n", run.exit_code);
        std::remove(wav_path.c_str());
        return false;
    }
    ...
    if (detected.empty()) {
        fprintf(stderr, "stelnettts[lid]: sherpa subprocess produced no 'Detected language:' line\n");
        return false;
    }
    ...
    out.source = "silero";
    if (!p.no_prints)
        fprintf(stderr, "stelnettts[lid]: sherpa → %s (%s)\n", detected.c_str(), out.lang_code.c_str());
    return true;
}
...
bool stelnettts_detect_language_cli(const float* samples, int n_samples, const whisper_params& params,
    ...
    if (be == "probe" || be == "self") {
        ...
        // stelnettts_backend_probe_language() already had its turn. Fall back to
        // the external default rather than erroring on a valid flag value.
        fprintf(stderr, "stelnettts[lid]: this backend has no self-probe (or it returned nothing) — "
    ...
    if (be == "silero") {
        ...
        if (!model_path.empty() && model_path.size() >= 5 &&
            ...
            fprintf(stderr, "stelnettts[lid]: silero-native returned null — "
    ...
    if (be == "firered") {
        ...
        fprintf(stderr, "stelnettts[lid]: firered LID failed\n");
        return false;
    }
    ...
    if (be == "ecapa") {
        ...
        fprintf(stderr, "stelnettts[lid]: ecapa LID failed\n");
        return false;
    }

    fprintf(stderr, "stelnettts[lid]: unknown --lid-backend '%s' (expected 'whisper', 'silero', 'firered', or 'ecapa')\n",
            be.c_str());
    return false;
}
