// stelnettts_backend_sensevoice.cpp — FunAudioLLM/SenseVoiceSmall adapter.
//
...
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[sensevoice]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        // Map stelnettts's language code to SenseVoice's known set. SenseVoice
        ...
        if (!r) {
            fprintf(stderr, "stelnettts[sensevoice]: transcribe failed\n");
            return out;
        }
