// stelnettts_server.cpp — HTTP server with persistent model for all backends.
//
...
//
// Usage:
//   stelnettts --server -m model.gguf [--port 8080] [--host 127.0.0.1]
//
// Endpoints:
...
#include "stelnettts_lid.h"
#include "stelnettts_lid_cli.h"
#include "stelnettts_output.h"
#include "stelnettts_model_mgr_cli.h"
#include "stelnettts_vad_cli.h"
#include "stelnettts_cpu_isa.h" // #261: CPU instruction-set self-check (soft-degrade VAD/diarize)
#include "stelnettts_aligner_cli.h"
#include "whisper_params.h"
#include "stelnettts_strict.h"             // #311: shared strict-pipeline requirements (parity with the CLI)
...
#include "stelnettts_cache.h"              // ensure_cached_file() for resolving the punc model
#include "stelnettts_punc_loader.h"        // shared --punc-model alias resolution (CLI parity)
#include "stelnettts_truecase_loader.h"    // shared --truecase-model resolution + apply (CLI parity)
#include "stelnettts_phonemes_policy.h"    // #316
#include "stelnettts_punctuation_policy.h" // stelnettts_should_auto_enable_punctuation()

#include "common-stelnettts.h"           // read_audio_data
#include "stelnettts_chat.h"             // /v1/chat/completions
...
#include "stelnettts_marking_policy.h" // #312: who may skip the spoken AI-disclaimer
...
// always available via the in-tree glint encoder (stelnettts_mp3_writer.h,
// with an optional libmp3lame fallback behind STELNETTTS_HAVE_LAME).
#ifdef STELNETTTS_HAVE_OPUS
#include <opus/opus.h>
#endif
...
static std::string scratch_dir() {
    ...
    std::string d;
    if (const char* env = std::getenv("STELNETTTS_SCRATCH_DIR"); env && *env)
        ...
        d = std::string(cache) + "/stelnettts/scratch";
    else if (const char* home = std::getenv("HOME"); home && *home)
        d = std::string(home) + "/.cache/stelnettts/scratch";
    else
        d = ".cache/stelnettts/scratch";

    std::error_code ec;
    std::filesystem::create_directories(d, ec);
    if (ec || !std::filesystem::is_directory(d, ec)) {
        std::error_code tec;
        std::filesystem::path fallback = std::filesystem::temp_directory_path(tec) / "stelnettts-scratch";
...
static std::string write_temp_audio(const char* data, size_t size, const std::string& original_filename = "") {
    ...
    return final_path;
#else
    std::string tmpl_s = scratch_dir() + "/stelnettts-XXXXXX" + ext;
...
static void json_error(httplib::Response& res, int status, const std::string& message, const std::string& code = "",
    ...
    res.status = status;
    std::string body =
        "{\"error\": {\"message\": \"" + stelnettts_json_escape(message) + "\", \"type\": \"invalid_request_error\"";
    if (!code.empty())
        body += ", \"code\": \"" + stelnettts_json_escape(code) + "\"";
    if (!param.empty())
        body += ", \"param\": \"" + stelnettts_json_escape(param) + "\"";
    body += "}}";
...
static void append_ctc_logits(stelnettts_ctc_logits& dst, const stelnettts_ctc_logits* src) {
...
static std::string add_ctc_logits_to_json(const std::string& base, const stelnettts_ctc_logits& logits) {
    if (logits.data.empty())
        return base;
    try {
        auto obj = nlohmann::json::parse(base);
        obj["ctc_logits"] = nlohmann::json::parse(stelnettts_ctc_logits_to_json(logits));
        return obj.dump();
    } catch (...) {
        return base;
    }
...
static transcription_result do_transcribe(const httplib::MultipartFormData& audio_file, CrispasrBackend* backend,
    ...
    if (rp.verbose)
        fprintf(stderr, "stelnettts-server: processing '%s' (%zu bytes)\n", log_sanitize(audio_file.filename).c_str(),
    ...
    const stelnettts_strict_reqs strict = stelnettts_compute_strict_reqs(rp);
    ...
    if (strict.punc && !punc_ctx && !pcs_ctx) {
        result.error = "punctuation required (require_punctuation/strict_pipeline) but the server has no punctuation "
                       "model loaded — start stelnettts-server with --punc-model";
        return result;
    }

    // #91: offset_t_ms / duration_ms request params — restrict processing to a
    // time window of the upload (mirrors the CLI dispatcher stelnettts_run.cpp,
    ...
    // Mirror the CLI (stelnettts_run.cpp): when VAD is active on a
    ...
    // chunking drops from 30 s to the cap. Mirrors stelnettts_run.cpp.
    ...
    // blob. Mirrors the CLI dispatcher (stelnettts_run.cpp
    ...
    if (!rp.vad_import_json.empty()) {
        ...
        // CLI's --vad-import (stelnettts_run.cpp); same wire format.
        int imported_sr = 0;
        float imported_chunk = 0.0f;
        bool imported_raw = false;
        if (!stelnettts_parse_vad_slices(rp.vad_import_json, slices, &imported_sr, &imported_chunk, &imported_raw)) {
        ...
        // effective value (see stelnettts_run.cpp).
        ...
        if (!imported_raw && stelnettts_vad_chunk_mismatch(imported_chunk, requested_chunk)) {
            ...
            fprintf(stderr,
                    "stelnettts-server: warning: vad_import exported at chunk_seconds %.2f, request uses %.2f — "
        ...
        if (imported_raw && effective_chunk_seconds > 0)
            slices = stelnettts_rechunk_slices(slices, pcmf32.data(), n_samples, SR, effective_chunk_seconds);
    } else {
        // #261: Silero/MarbleNet VAD runs on the CPU ggml backend inside slicing —
        // breadcrumb it so a native-instruction fault (SIGILL) is attributed
        // correctly by the fatal-signal handler.
        ...
        slices =
            stelnettts_compute_audio_slices(pcmf32.data(), n_samples, SR, effective_chunk_seconds, rp, &vad_load_failed);
    ...
    if (rp.vad_export_inline) {
        ...
        result.vad_segments_json = stelnettts_serialize_vad_slices(slices, SR, exp_chunk, rp.vad_export_raw);
    }
...

    {
        ...
        if (want_auto_lang && !has_native_lid && !lid_disabled) {
            ...
            if (have_vad_slices) {
                lid_speech = stelnettts_lid_speech_prefix(pcmf32, slices);
                if (!lid_speech.empty()) {
            ...
            const bool probed = stelnettts_backend_probe_language(*backend, lid_samples, lid_n_samples, rp, lid);
            if (probed || stelnettts_detect_language_cli(lid_samples, lid_n_samples, rp, lid)) {
                rp.language = lid.lang_code;
                if (rp.source_lang.empty() || rp.source_lang == "auto")
                    rp.source_lang = lid.lang_code;
                if (!rp.no_prints) {
                    fprintf(stderr, "stelnettts-server: LID -> language = '%s' (%s, p=%.3f)\n", lid.lang_code.c_str(),
                            lid.source.c_str(), lid.confidence);
                    if (used_speech_lid)
                        fprintf(stderr, "stelnettts-server: LID input = %.2fs speech from %zu VAD slice(s)\n",
                                (double)lid_n_samples / 16000.0, slices.size());
                }
            } else if (rp.language == "auto") {
                if (!rp.no_prints) {
                    fprintf(stderr, "stelnettts-server: LID failed and no -l was set — "
                ...
                fprintf(stderr, "stelnettts-server: LID failed, falling back to language='%s'\n", rp.language.c_str());
            }
        ...
        if (!rp.no_prints && slices.size() > 1) {
            fprintf(stderr, "stelnettts-server: processing %zu slice(s)\n", slices.size());
        }
        ...
        if (rp.verbose) {
            fprintf(stderr,
                    "stelnettts-server[verbose]: align: need_ts=%d aligner='%s' caps_ctc=%d force=%d -> want=%d\n",
        ...
        for (size_t i = 0; i < slices.size(); ++i) {
            ...
            // same policy as the CLI dispatcher (stelnettts_gap_fill.h).
            if (vad_cap > 0) {
                const char* gf = getenv("STELNETTTS_GAP_FILL");
                if (!gf || atoi(gf) != 0)
                    stelnettts_gap_fill_slice(*backend, rp, pcmf32.data(), n_samples, SR, sl, segs);
            }
            ...
            if (want_align) {
                for (auto& seg : segs) {
                    ...
                    auto words = stelnettts_ctc_align(rp.aligner_model, seg.text, pcmf32.data() + sl.start,
            ...
            if (!rp.no_prints && slices.size() > 1) {
                ...
                fprintf(stderr, "stelnettts-server: slice %zu/%zu done (%.1fs audio in %.1fs)\n", i + 1, slices.size(),
        ...
        stelnettts_warn_if_segments_backward(result.segs, "slice append");
        ...
        if (!result.segs.empty()) {
            ...
            const int n_spk = stelnettts_tiron_link_transcript(ts, pcmf32.data(), n_samples, spec.c_str(), rp.n_threads,
                                                             rp.cache_dir.c_str());
            if (n_spk >= 0) {
                tiron_handled = true;
        ...
        // Mirrors the CLI path in stelnettts_run.cpp:732-743.
        if (!tiron_handled && rp.diarize && !result.segs.empty()) {
            ...
            try {
                ...
                // the per-slice path must stand down. Mirrors stelnettts_run.cpp.
                ...
                stelnettts_diarize_merged_by_slice(
                    ...
                    [&](const stelnettts_audio_slice& sl, std::vector<stelnettts_segment>& slice_segs) {
                        if (have_stereo) {
                            ...
                            stelnettts_apply_diarize(sl_l, sl_r, /*is_stereo=*/true, sl.t0_cs, slice_segs, rp, pya_ptr,
                                                   shp_ptr);
                        } else {
                ...
                // VAD slice. Mirrors stelnettts_apply_global_speaker_stages in
                // the CLI runner; foxnose owns the labels, so the TitaNet remap
                // below must not re-run.
                const bool foxnose_global_ran = stelnettts_apply_foxnose_global(result.segs, pcmf32, rp);
                ...
                if (!foxnose_global_ran && !rp.diarize_embedder.empty() && !pcmf32.empty() &&
                    ...
                    auto embedder = stelnettts_make_speaker_embedder(rp.diarize_embedder, rp.n_threads, rp.cache_dir);
                    if (embedder) {
                        stelnettts_remap_speakers_via_embeddings(result.segs, pcmf32.data(), n_samples, embedder.get(),
                                                               rp);
                    }
                }
            } catch (const std::exception& e) {
                fprintf(stderr,
                        "stelnettts-server: warning: diarization failed (%s) — returning transcription "
                        "without speaker labels\n",
                        e.what());
            } catch (...) {
                fprintf(stderr, "stelnettts-server: warning: diarization failed (unknown error) — returning "
        ...
        if (!pcs_ctx && (tc_ctx || tc_crf_ctx || tc_lstm_ctx)) {
            ...
            for (auto& seg : result.segs)
                stelnettts_apply_truecase(tc_ctx, tc_crf_ctx, tc_lstm_ctx, seg.text);
        }
    ...
    // (native or aligned). Parity with the CLI's stelnettts_strict_check_words.
    if (strict.words) {
        ...
        const int missing = stelnettts_count_missing_word_ts(result.segs);
        if (missing > 0) {
...
// stelnettts_make_wav_int16 lives in stelnettts_wav_writer.h so the unit
// tests can exercise it without linking the server translation unit.

// ---------------------------------------------------------------------------
// 75e: MP3 / Opus encoding helpers. MP3 (stelnettts_make_mp3, glint) lives
// in stelnettts_mp3_writer.h and is always available; Opus stays gated on
// libopus.
// ---------------------------------------------------------------------------
...
static std::string stelnettts_encode_opus(const float* pcm, int n_samples, int sample_rate, int bitrate = 64000) {
...
// output is now always available). STELNETTTS_OPUS_ENCODER=libopus selects the
...
static std::string stelnettts_opus_response(const float* pcm, int n_samples, int sample_rate, const char** content_type) {
#ifdef STELNETTTS_HAVE_OPUS
    const char* pref = std::getenv("STELNETTTS_OPUS_ENCODER");
    if (pref && std::strcmp(pref, "libopus") == 0) {
        *content_type = "audio/opus";
        return stelnettts_encode_opus(pcm, n_samples, sample_rate);
    }
#endif
    *content_type = "audio/ogg";
    return stelnettts_make_opus(pcm, n_samples, sample_rate);
}
...
static void stelnettts_sig_write(const char* s) {
    if (!s)
        return;
    size_t n = 0;
    while (s[n])
...
static void stelnettts_fatal_signal_handler(int sig) {
    ...
    stelnettts_sig_write("\n*** stelnettts-server: FATAL ");
    stelnettts_sig_write(name);
    stelnettts_sig_write(" during stage: ");
    stelnettts_sig_write(g_current_stage ? g_current_stage : "unknown");
    stelnettts_sig_write(" ***\n");
    if (sig == SIGILL) {
        stelnettts_sig_write("This almost always means the binary uses CPU instructions this host does\n"
...
static void stelnettts_install_fatal_signal_handlers() {
#ifndef _WIN32
    struct sigaction sa;
    memset(&sa, 0, sizeof(sa));
    sa.sa_handler = stelnettts_fatal_signal_handler;
...
#endif
#else
    signal(SIGILL, stelnettts_fatal_signal_handler);
    signal(SIGSEGV, stelnettts_fatal_signal_handler);
    signal(SIGFPE, stelnettts_fatal_signal_handler);
#endif
}
...
int stelnettts_run_server(whisper_params& params, const std::string& host, int port) {
    using namespace httplib;

    stelnettts_install_fatal_signal_handlers();
    ...
    if (!stelnettts_consent::log_path().empty())
        fprintf(stderr,
                "stelnettts-server: consent records -> %s (JSON Lines; append-only storage is yours to provide)\n",
                stelnettts_consent::log_path().c_str());
    ...
    const stelnettts_cpu_isa::IsaCheck cpu_isa = stelnettts_cpu_isa::check();
    fprintf(stderr, "%s\n", stelnettts_cpu_isa::banner(cpu_isa).c_str());

    stelnettts_c2pa_startup_check();
    if (!params.watermark_model.empty()) {
        stelnettts_wm_dispatch::init(params.watermark_model);
    }
    ...
    if ((params.tts_no_watermark || params.tts_no_c2pa) && !params.tts_marking_responsibility_accepted) {
        fprintf(stderr,
                "stelnettts: error: server launched with %s requires --accept-marking-responsibility "
    ...
    if (const char* env_keys = getenv("STELNETTTS_API_KEYS")) {
    ...
    // Initial model load
    {
        ...
        if (backend_name.empty() || backend_name == "auto") {
            if (model_is_auto) {
                ...
                if (!params.no_prints) {
                    fprintf(stderr, "stelnettts-server: -m auto with no backend — defaulting to whisper\n");
                }
            } else {
        ...
        if (backend_name.empty()) {
            fprintf(stderr, "stelnettts-server: cannot detect backend from '%s'\n", params.model.c_str());
            return 1;
        }
        ...
        if (resolved.empty()) {
            fprintf(stderr, "stelnettts-server: failed to resolve model '%s' for backend '%s'\n", params.model.c_str(),
                    backend_name.c_str());
            return 1;
        }
        ...
        if (params.aligner_model == "auto" || params.aligner_model == "default") {
            ...
            if (resolved_aligner.empty()) {
                fprintf(stderr, "stelnettts-server: failed to resolve aligner '%s'\n", params.aligner_model.c_str());
                return 1;
            }
            params.aligner_model = resolved_aligner;
        } else if (!params.aligner_model.empty()) {
            params.aligner_model = stelnettts_resolve_model_cli(params.aligner_model, "", params.no_prints,
                                                              params.cache_dir, params.auto_download);
        }

        backend = stelnettts_create_backend(backend_name);
        if (!backend || !backend->init(params)) {
            fprintf(stderr, "stelnettts-server: failed to init backend '%s'\n", backend_name.c_str());
            return 1;
        }
        // #80e: warmup in server mode — on by default (amortized over many
        // requests). Opt out with --no-warmup (or STELNETTTS_NO_WARMUP=1): some
        ...
        const bool skip_warmup = params.no_warmup || [] {
            const char* e = std::getenv("STELNETTTS_NO_WARMUP");
            return e && e[0] && e[0] != '0';
        }();
        if (skip_warmup) {
            fprintf(stderr, "stelnettts-server: warmup skipped (--no-warmup)\n");
        } else {
            auto t0 = std::chrono::steady_clock::now();
            try {
                backend->warmup();
                auto dt = std::chrono::duration<double>(std::chrono::steady_clock::now() - t0).count();
                fprintf(stderr, "stelnettts-server: warmup completed in %.0f ms\n", dt * 1000.0);
            } catch (const std::exception& e) {
                fprintf(stderr, "stelnettts-server: warning: warmup failed (%s) — continuing without warmup\n", e.what());
            } catch (...) {
                fprintf(stderr, "stelnettts-server: warning: warmup failed — continuing without warmup\n");
            }
        }
        ...
        // The --server-workers flag sets the default; STELNETTTS_SERVER_WORKERS,
        // when present, overrides it (env stays the ultimate gate).
        int n_workers = std::max(1, params.server_workers);
        if (const char* e = std::getenv("STELNETTTS_SERVER_WORKERS"))
        ...
        // extra throughput; 4 workers with the 1.7B cielvox2-tts is 4 x ~1.95 GiB
        ...
        if (n_workers > 1 && (backend->capabilities() & CAP_TTS)) {
            fprintf(stderr,
                    "stelnettts-server: note — the worker pool serves pure-ASR requests only; "
        ...
        if (n_workers > 1) {
            ...
            for (int i = 0; i < n_workers; ++i) {
                ...
                w->backend = stelnettts_create_backend(backend_name);
                if (!w->backend || !w->backend->init(params)) {
                    fprintf(stderr, "stelnettts-server: worker %d init failed — running single-instance\n", i);
                    workers.clear();
                    break;
                }
                if (!skip_warmup) {
            ...
            if (!workers.empty()) {
                ...
                fprintf(stderr, "stelnettts-server: ASR worker pool = %zu workers (pure-ASR requests run concurrently)\n",
                        asr_pool->size());
            }
        }
        ready.store(true);
        fprintf(stderr, "stelnettts-server: backend '%s' loaded, model '%s'\n", backend_name.c_str(),
                params.model.c_str());
    }
    ...
    if (stelnettts_should_auto_enable_punctuation(backend->capabilities(), params)) {
        ...
        fprintf(stderr, "stelnettts-server: auto-enabling punctuation restoration for backend '%s'\n", backend->name());
    }
    ...
    {
        const stelnettts_punc_spec spec = stelnettts_resolve_punc_model(params.punc_model);
        std::string path = spec.direct_path;
        if (path.empty() && !spec.cache_filename.empty())
            path = stelnettts_cache::ensure_cached_file(spec.cache_filename, spec.url, params.no_prints, "stelnettts[punc]",
                                                      params.cache_dir);
        if (spec.kind == stelnettts_punc_kind::fireredpunc && !path.empty()) {
            punc_ctx.reset(fireredpunc_init(path.c_str()));
            if (!punc_ctx)
                fprintf(stderr, "stelnettts-server: warning: failed to load punc model '%s' — continuing without\n",
                        path.c_str());
            else
                fprintf(stderr, "stelnettts-server: loaded punctuation model '%s'\n", path.c_str());
        } else if (spec.kind == stelnettts_punc_kind::pcs && !path.empty()) {
            pcs_ctx.reset(pcs_init(path.c_str()));
            if (!pcs_ctx)
                fprintf(stderr, "stelnettts-server: warning: failed to load PCS model '%s' — continuing without\n",
                        path.c_str());
            else
                fprintf(stderr, "stelnettts-server: loaded PCS model '%s'\n", path.c_str());
        }
    }
    ...
    if (!cors_origin.empty())
        fprintf(stderr, "stelnettts-server: CORS enabled (Allow-Origin: %s)\n", cors_origin.c_str());
    ...
    // -----------------------------------------------------------------------
    // POST /inference — native StelnetTTS transcription endpoint
    // -----------------------------------------------------------------------
    svr.Post("/inference", [&](const Request& req, Response& res) {
        ...
        fprintf(stderr, "stelnettts-server: /inference received '%s' (%zu bytes)\n",
        ...
        // still fully specify the decode.
        {
            ...
            if (!sens.empty()) {
                ...
                if (core_sensitivity::parse_preset(sens, sp)) {
                    ...
                    fprintf(stderr, "stelnettts[server]: ignoring unknown sensitivity '%s' (expected: %s)\n",
        ...
        fprintf(stderr, "stelnettts-server: transcribed %.1fs audio in %.2fs (%.1fx realtime)\n", result.duration_s,
                result.elapsed_s, result.elapsed_s > 0 ? result.duration_s / result.elapsed_s : 0.0);

        std::string json = stelnettts_segments_to_native_json(result.segs, backend_name, result.duration_s);
        if (rp.return_logits)
    ...
    //
    // StelnetTTS extension fields (ignored by vanilla OpenAI clients):
    ...
    svr.Post("/v1/audio/transcriptions", [&](const Request& req, Response& res) {
        ...
        auto audio_file = req.get_file_value("file");
        fprintf(stderr, "stelnettts-server: /v1/audio/transcriptions received '%s' (%zu bytes)\n",
                audio_file.filename.c_str(), audio_file.content.size());

        // Parse OpenAI form fields + StelnetTTS extensions.
                        ...
                        pcmf32.data(), pcmf32.size(), 0, rp, [&](const std::string& partial, bool is_final) {
                            if (!partial.empty() || is_final) {
                                ...
                                if (!diff.empty()) {
                                    ...
                                    js << "data: {\"text\": \"" << stelnettts_json_escape(diff) << "\"}\n\n";
        ...
        fprintf(stderr, "stelnettts-server: transcribed %.1fs audio in %.2fs (%.1fx realtime), format=%s\n",
        ...
        if (rp.diarize && (response_format == "text" || response_format == "json")) {
            const bool labelled = std::any_of(result.segs.begin(), result.segs.end(),
                                              [](const stelnettts_segment& s) { return !s.speaker.empty(); });
            if (labelled) {
                fprintf(stderr,
                        "stelnettts-server: diarization produced speaker labels but response_format='%s' "
        ...
        if (response_format == "text") {
            res.set_content(stelnettts_segments_to_text(result.segs), "text/plain; charset=utf-8");
        } else if (response_format == "srt") {
            res.set_content(stelnettts_segments_to_srt(result.segs), "application/x-subrip; charset=utf-8");
        } else if (response_format == "vtt") {
            res.set_content(stelnettts_segments_to_vtt(result.segs), "text/vtt; charset=utf-8");
        } else if (response_format == "verbose_json") {
            std::string task = rp.translate ? "translate" : "transcribe";
            std::string json = stelnettts_segments_to_openai_verbose_json(result.segs, result.duration_s, result.language,
                                                                        task, temperature);
            if (rp.return_logits)
                json = add_ctc_logits_to_json(json, result.logits);
            json = add_vad_segments_to_json(json, result.vad_segments_json);
            ...
            std::string task = rp.translate ? "translate" : "transcribe";
            std::string json =
                stelnettts_segments_to_diarized_json(result.segs, result.duration_s, result.language, task, temperature);
            if (rp.return_logits)
                json = add_ctc_logits_to_json(json, result.logits);
            json = add_vad_segments_to_json(json, result.vad_segments_json);
            res.set_content(json, "application/json");
            ...
            // Default: json — {"text": "..."}
            std::string json = stelnettts_segments_to_openai_json(result.segs);
            if (rp.return_logits)
                json = add_ctc_logits_to_json(json, result.logits);
            json = add_vad_segments_to_json(json, result.vad_segments_json);
            res.set_content(json, "application/json");
    ...
    svr.Post("/load", [&](const Request& req, Response& res) {
        ...
        // when running with STELNETTTS_SERVER_WORKERS>1).
        if (asr_pool) {
            json_error(res, 409, "/load is not supported with STELNETTTS_SERVER_WORKERS>1 — restart to change models");
            return;
        }
        std::lock_guard<std::mutex> lock(model_mutex);
        ready.store(false);
        ...
        if (new_backend.empty())
            new_backend = stelnettts_detect_backend_from_gguf(new_model);
        ...
        const std::string resolved_model = stelnettts_resolve_model_cli(
        ...
        auto nb = stelnettts_create_backend(new_backend);
        if (!nb || !nb->init(np)) {
        ...
        fprintf(stderr, "stelnettts-server: hot-swapped to '%s' backend, model '%s'\n", new_backend.c_str(),
                resolved_model.c_str());
        res.set_content("{\"status\": \"ok\", \"backend\": \"" + stelnettts_json_escape(new_backend) + "\"}",
                        "application/json");
    });
    ...
    svr.Get("/health", [&](const Request&, Response& res) {
        if (ready.load()) {
            res.set_content("{\"status\": \"ok\", \"backend\": \"" + stelnettts_json_escape(backend_name) + "\"}",
                            "application/json");
        } else {
            res.status = 503;
    ...
    svr.Get("/backends", [&](const Request& req, Response& res) {
        if (!require_auth(req, res))
            return;
        auto names = stelnettts_list_backends();
        std::ostringstream js;
        js << "{\"backends\": [";
        for (size_t i = 0; i < names.size(); i++) {
            if (i)
                js << ", ";
            js << "\"" << stelnettts_json_escape(names[i]) << "\"";
        }
        js << "], \"active\": \"" << stelnettts_json_escape(backend_name) << "\"}";
    ...
    svr.Get("/v1/models", [&](const Request& req, Response& res) {
        ...
        std::ostringstream js;
        js << "{\"object\": \"list\", \"data\": [{";
        js << "\"id\": \"" << stelnettts_json_escape(params.model) << "\", ";
        js << "\"object\": \"model\", ";
        js << "\"owned_by\": \"stelnettts\", ";
        js << "\"backend\": \"" << stelnettts_json_escape(backend_name) << "\"";
        js << "}]}";
        res.set_content(js.str(), "application/json");
    });
    ...
    svr.Post("/v1/translate", [&](const Request& req, Response& res) {
        ...
        std::ostringstream js;
        js << "{\"text\": \"" << stelnettts_json_escape(out) << "\"}";
        res.set_content(js.str(), "application/json");
    });
    ...
    //                                                     cielvox2-tts and moss-tts act on it;
    ...
    //     stream (no header). `f32` is the stelnettts extension that
    //     emits raw float32 for downstream DSP.
    //   - `instructions` maps to params.tts_instruct (cielvox2-tts
    ...
    // own terms — cielvox2-tts CustomVoice resolves it as a speaker
    // name, orpheus resolves "tara"/"leah" as presets, cielvox2-tts
    ...
    svr.Post("/v1/audio/speech", [&](const Request& req, Response& res) {
        ...
        if (!(backend->capabilities() & CAP_TTS)) {
            json_error(res, 400,
                       "loaded backend '" + backend_name +
                           ...
                           "(e.g. cielvox2-tts, kokoro, vibevoice, orpheus) via POST /load");
            return;
        }
        ...
        // string came off a socket. See stelnettts_voice_clone_policy.h; the
        ...
        // gated by STELNETTTS_TADA_WAV_CLONE). Passed through to the backend as
        ...
        // the reasoning behind it, live in stelnettts_marking_policy.h so they can
        ...
        // See stelnettts_speaker_identity.h.
        bool identity_recognised = true;
        const stelnettts_voice::SpeakerIdentity identity_override =
            stelnettts_voice::parse_speaker_identity(body.value("speaker_identity", std::string()), &identity_recognised);
        ...
        const stelnettts_marking::Decision marking =
            stelnettts_marking::decide(needs_spoken_disclosure, spoken_disclaimer, body.value("marking_attestation", ""),
        ...
        if (is_voice_clone) {
            ...
            // stelnettts_consent_record.h); req is the per-request correlation id,
            // without which a consent line and the response it authorised are
            // unlinkable on a server serving many.
            const std::string resolved_ref = stelnettts_voice::resolve_voice_path(voice_name, params.tts_voice_dir);
            std::string ref_hash = stelnettts_consent::file_sha256(resolved_ref);
            stelnettts_consent::emit("CONSENT", ts,
                                   {{"voice", log_sanitize(voice_name)},
                                    ...
                                    {"req", stelnettts_consent::request_correlation_id()}});
        ...
        if (!tts_phonemes.empty()) {
            if (!stelnettts_phonemes_policy::backend_supports(backend_name)) {
                json_error(res, 400, stelnettts_phonemes_policy::unsupported_message(backend_name),
        ...
        auto t0 = std::chrono::steady_clock::now();
        const std::vector<std::string> sentences = stelnettts_tts_plan_chunks_for_backend(text, backend->name());
        ...
        if (stream) {
            ...
            auto push_pcm = [sq, sr_out, speed, enqueue](const float* pcm, int n_samples) {
                ...
                stelnettts_wm_dispatch::embed(chunk.data(), (int)chunk.size(), sr_out, /*force=*/true);
                enqueue(stelnettts_make_pcm_int16_le(chunk.data(), (int)chunk.size()));
            };
            ...
            std::thread worker([&backend, &model_mutex, sentences, rp, apply_spoken_disclaimer, silence_s16,
                ...
                try {
                    ...
                    if (apply_spoken_disclaimer) {
                        const auto& disc = stelnettts_tts_get_disclaimer(backend.get(), rp);
                        if (!disc.empty()) {
                    ...
                    fprintf(stderr, "stelnettts-server: streaming worker exception: %s\n", e.what());
                    std::lock_guard<std::mutex> lk(sq->m);
                    sq->failed = true;
                } catch (...) {
                    fprintf(stderr, "stelnettts-server: streaming worker unknown exception\n");
                    std::lock_guard<std::mutex> lk(sq->m);
                    sq->failed = true;
                }
                {
                ...
                fprintf(stderr, "stelnettts-server: streaming synthesis finished in %.2fs\n", el);
            });
            worker.detach();
        ...
        std::vector<float> pcm = stelnettts_tts_concat_with_silence(chunks, sr_out / 5);
        auto t1 = std::chrono::steady_clock::now();
        ...
        if (apply_spoken_disclaimer) {
            stelnettts_tts_prepend_disclaimer(pcm, backend.get(), rp);
        }
        set_marking_headers(res);
        ...
        const stelnettts_marking::ContainerMarking cmark =
            stelnettts_marking::container_marking_for_format(response_format);
        stelnettts_wm_dispatch::embed(pcm.data(), (int)pcm.size(), sr_out, /*force=*/!cmark.carries_c2pa);

        const double elapsed_s = std::chrono::duration<double>(t1 - t0).count();
        const double audio_s = (double)pcm.size() / (double)sr_out;
        fprintf(stderr,
                "stelnettts-server: synthesized %.1fs audio in %.2fs (RTF=%.2f) "
        ...
        if (response_format == "f32") {
            ...
            std::string raw = stelnettts_make_pcm_int16_le(pcm.data(), (int)pcm.size());
            res.set_content(std::move(raw), "audio/pcm");
        } else if (response_format == "mp3") {
            std::string mp3 = stelnettts_make_mp3(pcm.data(), (int)pcm.size(), sr_out);
            if (mp3.empty()) {
            ...
            if (!params.tts_no_c2pa)
                stelnettts_c2pa_sign_auto(mp3, cmark.c2pa_mime, params.c2pa_cert, params.c2pa_key, params.cache_dir);
            res.set_content(std::move(mp3), "audio/mpeg");
        } else if (response_format == "aac") {
            std::string aac = stelnettts_make_aac(pcm.data(), (int)pcm.size(), sr_out);
            if (aac.empty()) {
            ...
            std::string opus = stelnettts_opus_response(pcm.data(), (int)pcm.size(), sr_out, &ct);
            if (opus.empty()) {
            ...
            std::string wav = stelnettts_make_wav_int16(pcm.data(), (int)pcm.size(), sr_out);
            ...
            if (!params.tts_no_c2pa) // --no-c2pa: attested opt-out of the manifest floor
                stelnettts_c2pa_sign_auto(wav, cmark.c2pa_mime, params.c2pa_cert, params.c2pa_key, params.cache_dir);
    ...
    svr.Post("/v1/audio/speech-to-speech", [&](const Request& req, Response& res) {
        ...
        std::vector<float> pcm;
        {
            ...
            fprintf(stderr, "stelnettts-server: S2S %.1fs in → %.1fs out in %.2fs transcript='%s'\n", in_dur_s, out_dur_s,
    ...
    auto handle_list_voices = [&](const Request& req, Response& res) {
        ...
        if (!params.tts_voice_dir.empty()) {
            ...
            for (const auto& entry : std::filesystem::directory_iterator(params.tts_voice_dir, ec)) {
                ...
                if (!first)
                    js << ", ";
                js << "{\"name\": \"" << stelnettts_json_escape(stem) << "\", \"format\": \"" << fmt << "\"}";
                first = false;
            }
        }
        js << "]}";
        ...
        // just "what was synthesized with it afterwards".
        {
            ...
            stelnettts_consent::emit(
                ...
                {{"scope", "voice-upload"},
                 ...
                 {"ref_sha256", stelnettts_consent::bytes_sha256(voice_file.content.data(), voice_file.content.size())},
                 {"req", stelnettts_consent::request_correlation_id()}});
        }
        fprintf(stderr, "stelnettts-server: uploaded voice '%s' (%zu bytes)\n", voice_name.c_str(),
    ...
    svr.Delete(R"(/v1/voices/([a-zA-Z0-9_-]+))", [&](const Request& req, Response& res) {
        ...
        res.set_content(resp.dump(), "application/json");
        fprintf(stderr, "stelnettts-server: deleted voice '%s'\n", voice_name.c_str());
    });
    ...
    //     "top_k":            int,                          (optional, stelnettts ext.)
    ...
    //
    // Backed by the shared stelnettts_chat_* C ABI (one process-wide session
    ...
    // -----------------------------------------------------------------------
    std::shared_ptr<stelnettts_chat_session> chat_sess(nullptr, &stelnettts_chat_close);
    ...
    auto ensure_chat_session = [&]() -> stelnettts_chat_session_t {
        ...
        stelnettts_chat_session_t s = stelnettts_chat_open(params.chat_model.c_str(), &op, &err);
        if (!s) {
            fprintf(stderr, "stelnettts-server: chat session open failed: %s\n", err.message);
            return nullptr;
        }
        fprintf(stderr, "stelnettts-server: /v1/chat/completions ready — model '%s', template '%s', ctx %d\n",
                params.chat_model.c_str(), stelnettts_chat_template_name(s), stelnettts_chat_n_ctx(s));
        chat_sess.reset(s, &stelnettts_chat_close);
        return s;
    };
    ...
    auto parse_generate_params = [](const nlohmann::json& body, stelnettts_chat_generate_params& gp,
    ...
    svr.Post("/v1/chat/completions", [&](const Request& req, Response& res) {
        ...
        stelnettts_chat_session_t s = ensure_chat_session();
        if (!s) {
        ...
        std::vector<stelnettts_chat_message> msgs;
        ...
        stelnettts_chat_error rerr{};
        if (stelnettts_chat_reset(s, &rerr) != 0) {
        ...
        if (!stream) {
            stelnettts_chat_error gerr{};
            char* out = stelnettts_chat_generate(s, msgs.data(), msgs.size(), &gp, &gerr);
            if (!out) {
            ...
            js << "{\"id\": \"" << completion_id << "\", " << "\"object\": \"chat.completion\", "
               << "\"created\": " << created_str << ", " << "\"model\": \"" << stelnettts_json_escape(model_id) << "\", "
               << "\"choices\": [{" << "\"index\": 0, " << "\"message\": {\"role\": \"assistant\", \"content\": \""
               << stelnettts_json_escape(reply) << "\"}, " << "\"finish_reason\": \"stop\"" << "}]}";
            res.set_content(js.str(), "application/json");
            return;
        }
        ...
        if (stelnettts_chat_generate_stream(s, msgs.data(), msgs.size(), &gp, on_tok, &state, &gerr) != 0) {
        ...
        for (const auto& delta : state.deltas) {
            ...
            js << "{\"id\": \"" << completion_id << "\", " << "\"object\": \"chat.completion.chunk\", "
               << "\"created\": " << created_str << ", " << "\"model\": \"" << stelnettts_json_escape(model_id) << "\", "
               << "\"choices\": [{\"index\": 0, " << "\"delta\": {\"content\": \"" << stelnettts_json_escape(delta)
               << "\"}, " << "\"finish_reason\": null}]}";
            sse << "data: " << js.str() << "\n\n";
        }
    ...
    svr.set_error_handler([&](const Request& req, Response& res) {
        if (!res.body.empty())
            return;
        fprintf(stderr, "stelnettts-server: %s %s → %d (no matching route)\n", req.method.c_str(), req.path.c_str(),
                res.status);
    ...
    svr.set_exception_handler([&](const Request& req, Response& res, std::exception_ptr ep) {
        ...
        fprintf(stderr, "stelnettts-server: %s %s → 500 (exception: %s)\n", req.method.c_str(), req.path.c_str(),
                what.c_str());
        res.status = 500;
    ...
    fprintf(stderr, "\nstelnettts-server: listening on %s:%d\n", host.c_str(), port);
    ...
    if (tts) {
        ...
        if (params.tts_voice_dir.empty()) {
            fprintf(stderr, "stelnettts-server: warning: --voice-dir not set; /v1/voices will return empty "
    ...
    if (!api_keys.empty())
        fprintf(stderr, "stelnettts-server: API key authentication enabled\n");
    ...
    // session API (stelnettts_session_stream_*); clients send binary 16 kHz mono
    ...
    if (params.wyoming_port > 0) {
        if (wyoming_start(backend.get(), model_mutex, params, params.wyoming_port) == 0) {
            ...
            fprintf(stderr, "stelnettts-server: warning: failed to start Wyoming server on port %d\n",
                    params.wyoming_port);
        }
    }
    if (params.server_ws_port >= 0) {
        ...
        if (ws_stream_start(params.model.c_str(), ws_port, params.n_threads) == 0) {
            ...
            fprintf(stderr, "stelnettts-server: warning: failed to start WebSocket streaming on port %d\n", ws_port);
        }
        ...
        if (realtime_server_start(backend.get(), model_mutex, params, rt_port) == 0) {
            ...
            fprintf(stderr, "stelnettts-server: warning: failed to start vLLM Realtime API on port %d\n", rt_port);
        }
    }
    fprintf(stderr, "\n");
