// stelnettts_llm_pipeline.h — templated audio-LLM pipeline shared across
...
// Returns a vector with exactly one stelnettts_segment (text + offset times)
// on success, empty on failure. Prints errors to stderr.
template <typename Ops>
std::vector<stelnettts_segment> stelnettts_run_voxtral_style_pipeline(typename Ops::CtxT* ctx, const float* samples,
    ...
    if (!mel) {
        fprintf(stderr, "stelnettts[%s]: mel failed\n", BE);
        return out;
    }
    ...
    free(mel);
    if (!audio_embeds) {
        fprintf(stderr, "stelnettts[%s]: encoder failed\n", BE);
        return out;
    }
    ...
    if (!pid || !sid) {
        fprintf(stderr, "stelnettts[%s]: tokenize failed\n", BE);
        free(pid);
        free(sid);
        free(audio_embeds);
        return out;
    ...
    if (!text_embeds) {
        fprintf(stderr, "stelnettts[%s]: embed failed\n", BE);
        free(audio_embeds);
        return out;
    }
    ...
    if (!Ops::kv_init(ctx, 4096)) {
        free(text_embeds);
        fprintf(stderr, "stelnettts[%s]: kv_init failed\n", BE);
        return out;
    }
    ...
    for (int run = 0; run < n_runs; run++) {
        ...
        if (!logits) {
            fprintf(stderr, "stelnettts[%s]: prefill failed (run %d/%d)\n", BE, run + 1, n_runs);
            free(text_embeds);
            return out;
        }
        if (run == 0)
    ...
    if (!params.no_prints && n_runs > 1) {
        fprintf(stderr, "stelnettts[%s]: best-of-%d picked score=%.4f\n", BE, n_runs, best_score);
    }
...
// approach was to slice at 30 s in `stelnettts_run.cpp` and call the
...
// rather than batched (StelnetTTS's runtime is single-graph), but the
...
template <typename Ops>
std::vector<stelnettts_segment> stelnettts_run_voxtral_style_pipeline_streamed(typename Ops::CtxT* ctx,
    ...
    if (n_samples <= kChunkSamples) {
        return stelnettts_run_voxtral_style_pipeline<Ops>(ctx, samples, n_samples, t_offset_cs, params);
    }
    ...
    for (int c = 0; c < n_chunks; c++) {
        ...
        if (!mel) {
            fprintf(stderr, "stelnettts[%s][streamed]: mel failed on chunk %d/%d\n", BE, c + 1, n_chunks);
            return out;
        }
        ...
        free(mel);
        if (!audio_embeds) {
            fprintf(stderr, "stelnettts[%s][streamed]: encoder failed on chunk %d/%d\n", BE, c + 1, n_chunks);
            return out;
        }
        ...
        if (c == 0) {
            ...
            fprintf(stderr, "stelnettts[%s][streamed]: chunk %d shape (%d,%d) != chunk 0 (%d,%d)\n", BE, c, N_enc, pdim,
    ...
    if (per_chunk_dim <= 0) {
        fprintf(stderr, "stelnettts[%s][streamed]: no chunks emitted (per_chunk_dim=0)\n", BE);
        return out;
    }
    const int N_enc_total = (int)(audio_embeds_all.size() / (size_t)per_chunk_dim);

    if (!params.no_prints) {
        fprintf(stderr, "stelnettts[%s][streamed]: %d chunk(s), %d audio tokens, then single LLM decode\n", BE, n_chunks,
                N_enc_total);
    }
    ...
    int32_t* sid = Ops::tokenize(ctx, suffix.c_str(), &n_suffix);
    if (!pid || !sid) {
        fprintf(stderr, "stelnettts[%s][streamed]: tokenize failed\n", BE);
        free(pid);
        free(sid);
        return out;
    }
    ...
    float* text_embeds = Ops::embed_tokens(ctx, ids.data(), T_prompt);
    if (!text_embeds) {
        fprintf(stderr, "stelnettts[%s][streamed]: embed failed\n", BE);
        return out;
    }
    ...
    if (!params.no_prints && max_new > params.max_new_tokens) {
        fprintf(stderr, "stelnettts[%s][streamed]: scaling max_new_tokens %d -> %d for %d s of audio\n", BE,
                params.max_new_tokens, max_new, audio_seconds);
    }
    const int kv_budget = T_prompt + max_new + 64;
    if (!Ops::kv_init(ctx, kv_budget)) {
        free(text_embeds);
        fprintf(stderr, "stelnettts[%s][streamed]: kv_init(%d) failed\n", BE, kv_budget);
        return out;
    }
    Ops::kv_reset(ctx);
    ...
    float* logits = Ops::run_llm_kv(ctx, text_embeds, T_prompt, 0, &n_tokens_out, &vocab);
    if (!logits) {
        fprintf(stderr, "stelnettts[%s][streamed]: prefill failed\n", BE);
        free(text_embeds);
        return out;
    }
...
void stelnettts_run_voxtral_style_pipeline_streamed_cb(typename Ops::CtxT* ctx, const float* samples, int n_samples,
    ...
    float* logits = Ops::run_llm_kv(ctx, text_embeds, T_prompt, 0, &n_tokens_out, &vocab);
    if (!logits) {
        fprintf(stderr, "stelnettts[%s]: prefill failed\n", BE);
        free(text_embeds);
        return;
    }
