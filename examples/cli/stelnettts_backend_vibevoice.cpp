// stelnettts_backend_vibevoice.cpp — adapter for Microsoft VibeVoice-ASR.
//
// The runtime itself expects 24 kHz mono PCM. The unified StelnetTTS CLI
...
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[vibevoice]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        ...
        if (backend_name_ == "vibevoice" && !vibevoice_has_asr(ctx_)) {
            fprintf(stderr,
                    "stelnettts[vibevoice]: error: '%s' is a TTS-only model (no at_enc.*/st_enc.* tensors).\n"
                    "  Use --backend vibevoice-tts for this model, or download the ASR model:\n"
                    "  stelnettts -m auto --backend vibevoice --auto-download\n",
                    p.model.c_str());
            vibevoice_free(ctx_);
            ctx_ = nullptr;
            return false;
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        //
        // STELNETTTS_VIBEVOICE_RAW_TRANSCRIPT=1 restores the pre-#300 behaviour
    ...
    std::vector<float> synthesize(const std::string& text, const whisper_params& params) override {
        ...
        //      (matches cielvox2-tts post-d35940b — server callers can pass the same
        ...
        // request). Path-traversal sanitization mirrors the cielvox2-tts adapter.
        if (!voice_path.empty() && !params.tts_voice_dir.empty() && voice_path.find('/') == std::string::npos &&
            ...
            if (voice_path.find("..") != std::string::npos || voice_path.find('\0') != std::string::npos) {
                fprintf(stderr, "stelnettts[%s]: voice name '%s' contains illegal characters (.. or NUL)\n",
            ...
            if (file_exists(gguf_path)) {
                ...
                fprintf(stderr, "stelnettts[%s]: warning: neither '%s' nor '%s' were found on disk\n",
        ...
        if (!voice_path.empty() && voice_path != last_voice_key_) {
            if (vibevoice_load_voice(ctx_, voice_path.c_str()) == 0) {
                ...
                if (params.tts_voice.empty() || params.tts_voice != voice_path)
                    fprintf(stderr, "stelnettts[%s]: voice loaded '%s'\n", backend_name_.c_str(), voice_path.c_str());
            } else {
                fprintf(stderr,
                        "stelnettts[%s]: voice '%s' could not be loaded; refusing to synthesise without a "
                        "voice prompt.\n",
                        backend_name_.c_str(), voice_path.c_str());
                return {};
            }
        ...
        if (!allow_generic_no_voice_ && last_voice_key_.empty() && wav_ref_path_.empty()) {
            const char* voice_wav_env = stelnettts_env::get("STELNETTTS_VIBEVOICE_VOICE_AUDIO");
            if (!voice_wav_env || !voice_wav_env[0]) {
                fprintf(stderr,
                        "stelnettts[%s]: no voice prompt resolved (pass --voice <path.gguf>, --voice <path.wav>, "
