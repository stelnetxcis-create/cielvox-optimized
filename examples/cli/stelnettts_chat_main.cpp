// stelnettts_chat_main.cpp — reference CLI for the stelnettts_chat_* C ABI.
//
...
//
// Usage:
//   stelnettts-chat -m model.gguf [--system "..."] [-c 4096]
...
int main(int argc, char** argv) {
    ...
    stelnettts_chat_session_t s = stelnettts_chat_open(args.model.c_str(), &op, &err);
    if (!s) {
        std::fprintf(stderr, "stelnettts_chat_open failed: %s\n", err.message);
        return 1;
    }
    ...
    if (one_shot) {
        ...
        std::string acc;
        if (stelnettts_chat_generate_stream(s, msgs.data(), msgs.size(), &gp, stream_cb, &acc, &err) != 0) {
            std::fprintf(stderr, "\ngenerate failed: %s\n", err.message);
    ...
    std::fprintf(stderr, "Loaded %s — template '%s', ctx %d. Empty line to quit.\n", args.model.c_str(),
                 stelnettts_chat_template_name(s), stelnettts_chat_n_ctx(s));
    ...
    while (true) {
        ...
        std::fflush(stdout);
        std::string reply;
        if (stelnettts_chat_generate_stream(s, msgs.data(), msgs.size(), &gp, stream_cb, &reply, &err) != 0) {
