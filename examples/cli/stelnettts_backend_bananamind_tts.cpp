// stelnettts_backend_bananamind_tts.cpp -- adapter for BananaMind-TTS-V2.1.
//
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[bananamind-tts]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[bananamind-tts]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
