// stelnettts_consent_record.h — what a `[CONSENT]` line has to carry to be worth
// anything, and the one place all four surfaces build it.
//
// The consent GATE (stelnettts_voice_clone_policy.h) decides whether cloning is
...
// controller here — StelnetTTS is a tool they run (docs/eu-ai-act.md 6.x).
//
...
#include "core/stelnettts_sha256.h"
...
// Where `--consent-log` / STELNETTTS_CONSENT_LOG routes the machine-readable
...
inline void init_log_path_from_env() {
    if (!log_path().empty())
        return;
    const char* e = std::getenv("STELNETTTS_CONSENT_LOG");
    if (e && *e)
        log_path() = e;
}
