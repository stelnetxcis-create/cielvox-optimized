// stelnettts_backend_canary.cpp — adapter for nvidia/canary-1b-v2.
//
...
class CanaryBackend : public StelnetAsrBackend {
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[canary]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (const char* e = stelnettts_env::get("STELNETTTS_CANARY_STREAM_THRESHOLD_S")) {
...
std::unique_ptr<StelnetAsrBackend> stelnettts_make_canary_backend() {
    return std::unique_ptr<StelnetAsrBackend>(new CanaryBackend());
}
