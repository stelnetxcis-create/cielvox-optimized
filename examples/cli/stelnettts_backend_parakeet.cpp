// stelnettts_backend_parakeet.cpp — adapter for nvidia/parakeet-tdt-0.6b-v3.
//
// Wraps parakeet_init_from_file + parakeet_transcribe_ex and converts the
// native parakeet_result into a std::vector<stelnettts_segment>. One segment
...
public:
    ...
    uint32_t capabilities() const override {
        ...
        // (stelnettts_run.cpp:`!has_native_lid`), so users wanting LID
        ...
        // Override with STELNETTTS_PARAKEET_INTERNAL_CHUNKING=0 to fall back to
        ...
        if (const char* e = getenv("STELNETTTS_PARAKEET_INTERNAL_CHUNKING"))
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[parakeet]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        ...
        if (p.parakeet_decoder == "ctc") {
            if (parakeet_has_ctc(ctx_)) {
                ...
                if (!p.no_prints)
                    fprintf(stderr, "stelnettts[parakeet]: using CTC decoder\n");
            } else {
                fprintf(stderr, "stelnettts[parakeet]: --parakeet-decoder ctc requested but model has no CTC head\n");
            }
        }
        return true;
    }
    ...
    void apply_sticky_params(const whisper_params& params) {
        ...
        // MAES beam search (env: STELNETTTS_PARAKEET_MAES=1, or --decode maes).
        // Requires beam_size > 1. Configurable via env vars.
        {
            const char* maes_env = std::getenv("STELNETTTS_PARAKEET_MAES");
            bool use_maes = (maes_env && atoi(maes_env) > 0) || params.parakeet_decoder == "maes";
            if (use_maes && params.beam_size > 1) {
                int num_steps = 2;
                float gamma = 2.3f;
                int beta = 2;
                if (const char* v = std::getenv("STELNETTTS_MAES_NUM_STEPS"))
                    num_steps = atoi(v);
                if (const char* v = std::getenv("STELNETTTS_MAES_GAMMA"))
                    gamma = (float)atof(v);
                if (const char* v = std::getenv("STELNETTTS_MAES_BETA"))
                    beta = atoi(v);
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        // STELNETTTS_PARAKEET_* env knobs and takes JA-ness from is_ja_model_.
    ...
    std::vector<stelnettts_segment> decode_slice(encoded_slice e, int64_t t_offset_cs,
                                               const whisper_params& /*params*/) override {
        std::vector<stelnettts_segment> out;
    ...
    void repair_slice(const float* samples, int n_samples, int64_t t_offset_cs, std::vector<stelnettts_segment>& segs,
                      const whisper_params& params) override {
    ...
    static parakeet_seg parakeet_seg_from_seg(const stelnettts_segment& seg) {
        parakeet_seg ps;
        ps.text = seg.text;
        ps.t0 = seg.t0;
        ps.t1 = seg.t1;
    ...
    static stelnettts_segment seg_from_parakeet_seg(const parakeet_seg& ps) {
