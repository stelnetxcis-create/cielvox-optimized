// stelnettts_backend_mini_omni2.cpp — adapter for Mini-Omni2.
//
...
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        mp.use_gpu = stelnettts_backend_should_use_gpu(p);
        if (p.temperature > 0.0f)
            mp.temperature = p.temperature;

        ctx_ = mini_omni2_init_from_file(p.model.c_str(), mp);
        if (!ctx_) {
            fprintf(stderr, "stelnettts[mini-omni2]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        ...
        if (!codec_path.empty()) {
            if (mini_omni2_load_snac(ctx_, codec_path.c_str())) {
                snac_loaded_ = true;
                if (!p.no_prints)
                    fprintf(stderr, "stelnettts[mini-omni2]: SNAC codec loaded from '%s'\n", codec_path.c_str());
            }
        } else if (!p.no_prints) {
            fprintf(stderr, "stelnettts[mini-omni2]: no SNAC codec found. "
    ...
    std::vector<float> speech_to_speech(const float* samples, int n_samples, std::string* out_text,
        ...
        if (!ctx_)
            return {};
        if (!snac_loaded_) {
            fprintf(stderr, "stelnettts[mini-omni2]: SNAC codec required for S2S. "
    ...
    std::vector<float> synthesize(const std::string& text, const whisper_params& params) override {
        if (!ctx_ || text.empty())
            return {};
        if (!snac_loaded_) {
            fprintf(stderr, "stelnettts[mini-omni2]: SNAC codec required for TTS. "
                            "Pass --codec-model snac-24khz.gguf\n");
            return {};
        }
