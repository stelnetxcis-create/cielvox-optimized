// stelnettts_backend_gemma4_e2b.cpp — Gemma-4-E2B ASR backend adapter.
...
public:
    ...
    uint32_t capabilities() const override {
        ...
        //   CAP_AUTO_DOWNLOAD        registry entry in src/stelnettts_model_registry.cpp
        ...
        // pre-step LID gate (stelnettts_run.cpp:`!has_native_lid`), so users
    ...
    bool init(const whisper_params& params) override {
        ...
        if (getenv("STELNETTTS_VERBOSE") || stelnettts_env::get("STELNETTTS_GEMMA4_E2B_BENCH"))
            cp.verbosity = 2;
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        // via --vad. Opt-in: set STELNETTTS_GEMMA4_AUTO_CHUNK=1 to chunk
        ...
        if (n_samples > kMaxSamples) {
            const bool auto_chunk = std::getenv("STELNETTTS_GEMMA4_AUTO_CHUNK") != nullptr;
            if (!auto_chunk) {
                fprintf(stderr,
                        "stelnettts[gemma4-e2b]: input is %.1f s (> %.0f s training window). "
                        "Use --vad to segment, chunk externally, or set "
                        "STELNETTTS_GEMMA4_AUTO_CHUNK=1 to chunk internally "
    ...
    std::vector<stelnettts_segment> transcribe_one(const float* samples, int n_samples, int64_t t_offset_cs,
