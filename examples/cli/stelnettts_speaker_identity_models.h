// stelnettts_speaker_identity_models.h — the researched verdicts.
//
// stelnettts_speaker_identity.h is the MECHANISM: three values, two duties, a
...
// plumbing. A verdict scattered across `stelnettts_backend_*.cpp` is a verdict
// nobody re-reads.
//
...
// ----------------------------------
// One StelnetTTS backend serves many checkpoints — `orpheus` runs Canopy's base
...
//
// The durable fix EXISTS NOW: a `stelnettts.voice.speaker_identity` stamp in the
// GGUF itself, written by `models/convert-*.py --speaker-identity` and read by
// stelnettts_voice::read_model_speaker_identity(). A stamped checkpoint answers
...
// stays until the published Xenna/ mirrors are re-converted with the flag. Note
...
inline SpeakerIdentity identity_for_model(const std::string& backend, const std::string& model_path) {
    ...
    // recorded 2016-17 for LibriVox — and StelnetTTS ships only the English
    ...
    // StelnetTTS's own card is explicit — "This is a base model, not a voice. It
...
// stelnettts.voice.speaker_identity stamp answers for itself and never reaches
// here.
//
...
//
// EMPTY as of 2026-08-03: every TTS backend StelnetTTS ships has been checked
