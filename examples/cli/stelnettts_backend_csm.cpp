// stelnettts_backend_csm.cpp -- adapter for Sesame CSM-1B TTS.
//
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[csm-tts]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[csm-tts]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
