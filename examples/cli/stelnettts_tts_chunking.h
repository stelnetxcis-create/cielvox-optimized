// stelnettts_tts_chunking.h — long-form input helpers for /v1/audio/speech.
//
// The talker LM in every TTS backend has a finite training horizon
// (cielvox2-tts-1.7b-base degrades past ~600 chars / 200 codec frames
...
std::vector<std::string> stelnettts_tts_split_sentences(const std::string& text, std::size_t max_chars = 600);

// Server policy wrapper around stelnettts_tts_split_sentences. VibeVoice
...
std::vector<float> stelnettts_tts_concat_with_silence(const std::vector<std::vector<float>>& chunks, int silence_samples);
