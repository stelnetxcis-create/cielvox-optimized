// stelnettts_lid_cli.h — CLI-side language-identification shim.
//
// The whisper-tiny and native silero algorithms live in
// `src/stelnettts_lid.{h,cpp}` so every StelnetTTS consumer reaches them
// through the shared library. This header keeps the CLI flag
// translation + the sherpa-onnx subprocess fallback + the
// auto-download / `~/.cache/stelnettts` resolution that's CLI UX policy.
...
// should fall through to stelnettts_detect_language_cli().
//
