// stelnettts_piano_cli.cpp — see stelnettts_piano_cli.h.
...
int stelnettts_run_piano(const whisper_params& params) {
    if (params.fname_inp.empty()) {
        fprintf(stderr, "stelnettts: --piano needs an input file (-f)\n");
        return 2;
    }

    const bool json = params.piano_format == "json";
    if (!json && !params.piano_format.empty() && params.piano_format != "text") {
        fprintf(stderr, "stelnettts: --piano-format: unknown format '%s' (expected text or json)\n",
    ...
    if (model.empty()) {
        fprintf(stderr, "stelnettts: --piano: could not resolve a model.\n");
        return 2;
    }

    gguf_context* meta = core_gguf::open_metadata(model.c_str());
    if (!meta) {
        fprintf(stderr, "stelnettts: --piano: cannot open '%s'\n", model.c_str());
        return 2;
    }
    ...
    if (arch != "piano-transcription" && arch != "piano_transcription") {
        fprintf(stderr, "stelnettts: --piano: '%s' is not a piano model (arch='%s').\n", model.c_str(), arch.c_str());
        return 2;
    }
    ...
    if (!ctx) {
        fprintf(stderr, "stelnettts: --piano: failed to load '%s'\n", model.c_str());
        return 2;
    }
    ...
    for (const auto& fname : params.fname_inp) {
        ...
        if (piano_transcription_transcribe(ctx, mono.data(), (int)mono.size(), &res) != 0) {
            fprintf(stderr, "stelnettts: --piano failed on '%s'\n", fname.c_str());
            rc = 1;
            continue;
        }
        ...
        if (!params.no_prints)
            fprintf(stderr, "stelnettts: %s: %d notes, %d pedal events\n", fname.c_str(), res.n_notes, res.n_pedals);
