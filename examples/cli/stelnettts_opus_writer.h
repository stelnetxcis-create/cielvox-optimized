// stelnettts_opus_writer.h — Ogg Opus serializer for TTS float32 PCM, built on
// the in-tree glint Opus encoder (glint_opus_encode_file). Header-only like
// stelnettts_mp3_writer.h / stelnettts_aac_writer.h; consumers link the `glint`
// static library.
//
...
inline uint32_t stelnettts_ogg_crc32(const uint8_t* data, size_t len) {
...
inline bool stelnettts_opus_inject_provenance(std::string& ogg) {
    ...
    while (pos + 27 <= n) {
        ...
        if (body_len >= 8 && std::memcmp(d + body_off, "OpusTags", 8) == 0) {
            ...
            comments.emplace_back("GENERATOR=StelnetTTS");
            ...
            page += pkt;
            uint32_t crc = stelnettts_ogg_crc32(reinterpret_cast<const uint8_t*>(page.data()), page.size());
...
inline std::string stelnettts_make_opus_glint(const float* pcm, int n_samples, int sample_rate, int bitrate_bps = 64000) {
    ...
    stelnettts_opus_inject_provenance(out);
    return out;
}

// Public entry: glint Ogg Opus. Kept as a thin wrapper so a future libopus A/B
// path can slot in behind STELNETTTS_OPUS_ENCODER without touching call sites.
inline std::string stelnettts_make_opus(const float* pcm, int n_samples, int sample_rate, int bitrate_bps = 64000) {
    return stelnettts_make_opus_glint(pcm, n_samples, sample_rate, bitrate_bps);
}
