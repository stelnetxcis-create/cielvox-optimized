...
inline bool stelnettts_backend_should_use_gpu(const whisper_params& params) {
...
inline void stelnettts_strip_ascii_punctuation(std::string& s) {
    std::string out;
    out.reserve(s.size());
    for (char c : s) {
...
inline void stelnettts_lowercase_ascii(std::string& s) {
    for (char& c : s) {
        unsigned char u = (unsigned char)c;
        if (u >= 'A' && u <= 'Z')
            c = (char)(u + 32);
...
inline std::string stelnettts_iso_to_english_lang(const std::string& code) {
