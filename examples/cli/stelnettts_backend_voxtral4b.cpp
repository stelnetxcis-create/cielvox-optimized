// stelnettts_backend_voxtral4b.cpp — adapter for Voxtral-Mini-4B-Realtime-2602.
//
...
// stelnettts_llm_pipeline.h template. The pipeline is implemented inline here,
...
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[voxtral4b]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (!mel) {
            fprintf(stderr, "stelnettts[voxtral4b]: mel failed\n");
            return out;
        }
        ...
        free(mel);
        if (!audio_embeds) {
            fprintf(stderr, "stelnettts[voxtral4b]: encoder failed\n");
            return out;
        }
        ...
        if (!prompt_embeds) {
            free(audio_embeds);
            fprintf(stderr, "stelnettts[voxtral4b]: embed failed\n");
            return out;
        }
        ...
        if (!voxtral4b_kv_init(ctx_, 4096)) {
            free(prompt_embeds);
            free(audio_embeds);
            fprintf(stderr, "stelnettts[voxtral4b]: kv_init failed\n");
            return out;
        }
        ...
        for (int run = 0; run < n_runs; run++) {
            ...
            if (!logits) {
                free(prompt_embeds);
                free(audio_embeds);
                fprintf(stderr, "stelnettts[voxtral4b]: prefill failed (run %d/%d)\n", run + 1, n_runs);
                return out;
            }
            if (run == 0)
        ...
        if (!params.no_prints && n_runs > 1)
            fprintf(stderr, "stelnettts[voxtral4b]: best-of-%d picked score=%.4f\n", n_runs, best_score);

        // ---- Detokenize, filtering streaming control tokens (id < 1000) ----
        std::string transcript;
        std::vector<stelnettts_token> tok_vec;
