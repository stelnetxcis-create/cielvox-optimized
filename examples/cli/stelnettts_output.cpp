// stelnettts_output.cpp — output writers for non-whisper backends.
//
...
std::string stelnettts_to_timestamp(int64_t cs, bool comma) {
    int64_t msec = cs * 10;
...
static bool words_carry_sentence_ends(const std::vector<stelnettts_word>& words) {
...
int stelnettts_first_backward_segment(const std::vector<stelnettts_segment>& segments, int64_t* prev_cs, int64_t* cur_cs) {
...
std::vector<stelnettts_disp_segment> stelnettts_make_disp_segments(const std::vector<stelnettts_segment>& segments,
    ...
    std::vector<stelnettts_disp_segment> out;
...
bool stelnettts_write_txt(const std::string& path, const std::vector<stelnettts_disp_segment>& segs) {
    std::ofstream f(path);
    if (!f) {
        fprintf(stderr, "stelnettts: warning: cannot write TXT '%s'\n", path.c_str());
        return false;
    }
...
bool stelnettts_write_srt(const std::string& path, const std::vector<stelnettts_disp_segment>& segs) {
    std::ofstream f(path);
    if (!f) {
        fprintf(stderr, "stelnettts: warning: cannot write SRT '%s'\n", path.c_str());
        return false;
    }
    for (size_t i = 0; i < segs.size(); i++) {
        f << (i + 1) << "\n"
          << stelnettts_to_timestamp(segs[i].t0, /*comma=*/true) << " --> "
          << stelnettts_to_timestamp(segs[i].t1, /*comma=*/true) << "\n"
...
bool stelnettts_write_vtt(const std::string& path, const std::vector<stelnettts_disp_segment>& segs) {
    std::ofstream f(path);
    if (!f) {
        fprintf(stderr, "stelnettts: warning: cannot write VTT '%s'\n", path.c_str());
        return false;
    }
    f << "WEBVTT\n\n";
    for (const auto& s : segs) {
        f << stelnettts_to_timestamp(s.t0) << " --> " << stelnettts_to_timestamp(s.t1) << "\n"
...
bool stelnettts_write_csv(const std::string& path, const std::vector<stelnettts_disp_segment>& segs) {
    std::ofstream f(path);
    if (!f) {
        fprintf(stderr, "stelnettts: warning: cannot write CSV '%s'\n", path.c_str());
        return false;
    }
    f << "start,end,text\n";
    for (const auto& s : segs) {
...
// Exposed publicly as stelnettts_json_escape(); the static alias keeps
...
static inline const std::string json_escape(const std::string& s) {
    return stelnettts_json_escape(s);
}
...
bool stelnettts_write_json(const std::string& path, const std::vector<stelnettts_segment>& segs,
                         ...
                         bool full, const stelnettts_lid_info* lid) {
    std::ofstream f(path);
    if (!f) {
        fprintf(stderr, "stelnettts: warning: cannot write JSON '%s'\n", path.c_str());
        return false;
    }
    f << "{\n";
    f << "  \"stelnettts\": {\n";
    ...
    for (size_t i = 0; i < segs.size(); i++) {
        const auto& s = segs[i];
        f << "    {\n";
        f << "      \"timestamps\": { \"from\": \"" << stelnettts_to_timestamp(s.t0, true) << "\", \"to\": \""
          << stelnettts_to_timestamp(s.t1, true) << "\" },\n";
        ...
        // No "emotion" key by design — see stelnettts_backend.h / docs/eu-ai-act.md.
        if (!s.itn_flag.empty())
...
bool stelnettts_write_lrc(const std::string& path, const std::vector<stelnettts_disp_segment>& segs) {
    std::ofstream f(path);
    if (!f) {
        fprintf(stderr, "stelnettts: warning: cannot write LRC '%s'\n", path.c_str());
        return false;
    }
    f << "[by:stelnettts]\n";
    for (const auto& s : segs) {
...
bool stelnettts_write_ctc_logits_json(const std::string& path, const stelnettts_ctc_logits& logits,
    ...
    std::ofstream f(path);
    if (!f) {
        fprintf(stderr, "stelnettts: warning: cannot write CTC logits JSON '%s'\n", path.c_str());
        return false;
    }
    f << "{\n";
    f << "  \"backend\": \"" << json_escape(backend_name) << "\",\n";
    f << "  \"ctc_logits\": " << stelnettts_ctc_logits_to_json(logits) << "\n";
    f << "}\n";
    return true;
}
...
void stelnettts_print_stdout(const std::vector<stelnettts_disp_segment>& segs, bool show_timestamps) {
    if (show_timestamps) {
        for (const auto& s : segs) {
            printf("[%s --> %s]  %s%s\n", stelnettts_to_timestamp(s.t0).c_str(), stelnettts_to_timestamp(s.t1).c_str(),
...
std::string stelnettts_segments_to_srt(const std::vector<stelnettts_segment>& segs, int max_len) {
    auto disp = stelnettts_make_disp_segments(segs, max_len);
    std::ostringstream out;
    for (size_t i = 0; i < disp.size(); i++) {
        out << (i + 1) << "\n"
            << stelnettts_to_timestamp(disp[i].t0, /*comma=*/true) << " --> "
            << stelnettts_to_timestamp(disp[i].t1, /*comma=*/true) << "\n"
            << prefix_speaker(disp[i].speaker) << disp[i].text << "\n\n";
    }
    return out.str();
}

std::string stelnettts_segments_to_vtt(const std::vector<stelnettts_segment>& segs, int max_len) {
    auto disp = stelnettts_make_disp_segments(segs, max_len);
    std::ostringstream out;
    out << "WEBVTT\n\n";
    for (const auto& s : disp) {
        out << stelnettts_to_timestamp(s.t0) << " --> " << stelnettts_to_timestamp(s.t1) << "\n"
            << prefix_speaker(s.speaker) << s.text << "\n\n";
    }
    return out.str();
}
...
std::string stelnettts_segments_to_openai_json(const std::vector<stelnettts_segment>& segs) {
    std::string text = stelnettts_segments_to_text(segs);
...
std::string stelnettts_segments_to_openai_verbose_json(const std::vector<stelnettts_segment>& segs, double duration_s,
                                                     ...
                                                     float temperature) {
    std::string full_text = stelnettts_segments_to_text(segs);
