// stelnettts_backend_orpheus.cpp — adapter for canopylabs/orpheus-3b-0.1-ft
...
class OrpheusBackend : public StelnetAsrBackend {
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[orpheus]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[orpheus]: failed to load talker '%s'\n", p.model.c_str());
            return false;
        }
        ...
        if (!codec_path.empty()) {
            ...
            if (!p.no_prints) {
                fprintf(stderr, "stelnettts[orpheus]: codec path = '%s'\n", codec_path.c_str());
            }
        } else if (!p.no_prints) {
            fprintf(stderr, "stelnettts[orpheus]: no SNAC codec found. Pass --codec-model PATH or place "
        ...
        if (!voice_loaded_) {
            ...
            if (orpheus_is_fixed_speaker(ctx_)) {
                if (spk_name.empty()) {
                    ...
                    if (!first) {
                        fprintf(stderr,
                                "stelnettts[orpheus]: fixed_speaker variant has no speakers in the GGUF metadata\n");
                        return {};
                    }
                    spk_name = first;
                }
                if (orpheus_set_speaker_by_name(ctx_, spk_name.c_str()) != 0) {
                    fprintf(stderr, "stelnettts[orpheus]: unknown speaker '%s' (available: ", spk_name.c_str());
                ...
                if (!params.no_prints) {
                    fprintf(stderr, "stelnettts[orpheus]: speaker = '%s'\n", spk_name.c_str());
                }
            }
            voice_loaded_ = true;
        }
...
std::unique_ptr<StelnetAsrBackend> stelnettts_make_orpheus_backend() {
    return std::unique_ptr<StelnetAsrBackend>(new OrpheusBackend());
}
