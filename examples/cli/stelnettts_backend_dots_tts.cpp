// stelnettts_backend_dots_tts.cpp — adapter for rednote-hilab/dots.tts
...
class DotsTtsBackend : public StelnetAsrBackend {
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[dots-tts]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[dots-tts]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        ...
        if (!voc_path.empty() && voc_path != "auto" && voc_path != "default") {
            voc_path = stelnettts_resolve_model_cli(voc_path, p.backend, p.no_prints, p.cache_dir, p.auto_download,
                                                  p.tts_codec_quant);
        } else {
            voc_path.clear();
        }
        ...
        if (voc_path.empty()) {
            ...
            if (stelnettts_registry_lookup(p.backend, entry, p.tts_codec_quant) && !entry.companion_filename.empty()) {
                voc_path = stelnettts_resolve_model_cli(entry.companion_filename, p.backend, p.no_prints, p.cache_dir,
        ...
        if (!voc_path.empty()) {
            ...
            if (!p.no_prints) {
                fprintf(stderr, "stelnettts[dots-tts]: vocoder = '%s'\n", voc_path.c_str());
            }
        } else if (!p.no_prints) {
            fprintf(stderr, "stelnettts[dots-tts]: WARNING: vocoder GGUF not found — synthesis will fail\n");
        }
        ...
        if (!p.tts_voice.empty()) {
            ...
            if (spk_path.empty()) {
                fprintf(stderr, "stelnettts[dots-tts]: WARNING: --voice given but speaker encoder GGUF not found "
                ...
                fprintf(stderr, "stelnettts[dots-tts]: WARNING: failed to apply voice prompt '%s'\n",
                        p.tts_voice.c_str());
            } else if (!p.no_prints) {
                fprintf(stderr, "stelnettts[dots-tts]: voice = '%s' (encoder '%s')\n", p.tts_voice.c_str(),
                        spk_path.c_str());
            }
        }
...
std::unique_ptr<StelnetAsrBackend> stelnettts_make_dots_tts_backend() {
    return std::unique_ptr<StelnetAsrBackend>(new DotsTtsBackend());
}
