// stelnettts_diff_main.cpp — CLI frontend for the ground-truth diff harness.
//
// Companion to tools/dump_reference.py. Given a reference GGUF archive
// produced by the Python dumper and a stelnettts backend + model, runs the
...
// stelnettts_diff.{h,cpp}.
//
// Usage:
//   stelnettts-diff <backend> <model.gguf> <reference.gguf> <audio.wav>
//
// Example:
...
static void print_tada_fm_rows(const stelnettts_diff::Ref& ref, const char* name, const std::vector<float>& data,
...
static StageResult qwen3_mel(cielvox2_asr_context* ctx, const float* samples, int n_samples) {
    StageResult r;
    int n_mels = 0, T_mel = 0;
...
static StageResult parakeet_encoder_with_ref_mel_r(parakeet_context* ctx, const stelnettts_diff::Ref& ref) {
    StageResult r;
...
static StageResult canary_encoder_with_ref_mel_r(canary_context* ctx, const stelnettts_diff::Ref& ref) {
    StageResult r;
    auto pair = ref.get_f32("mel_spectrogram");
    auto shp = ref.shape("mel_spectrogram");
    if (!pair.first || shp.size() < 2) {
...
// ---- cielvox2-tts (Qwen3 talker, codec_head, code_predictor) ----

static StageResult cielvox2_tts_text_proj_r(cielvox2_tts_context* ctx, const int32_t* ids, int n_tokens) {
    StageResult r;
    int T = 0, d = 0;
    float* h = cielvox2_tts_run_text_proj(ctx, ids, n_tokens, &T, &d);
    if (!h) {
        r.note = "cielvox2_tts_run_text_proj returned null";
        return r;
    }
    r.shape = {T, d};
...
static StageResult cielvox2_tts_talker_logits_r(cielvox2_tts_context* ctx, const float* embeds, int n_tokens) {
    StageResult r;
    int vocab = 0;
    float* logits = cielvox2_tts_run_talker_with_embeds(ctx, embeds, n_tokens, &vocab);
    if (!logits) {
        r.note = "cielvox2_tts_run_talker_with_embeds returned null";
        return r;
    }
    r.shape = {1, vocab};
...
static void print_row(const char* name, const stelnettts_diff::Report& r, float cos_threshold, const char* extra = "") {
...
static void print_row_exact(const char* name, const stelnettts_diff::Report& r, float cos_threshold,
...
static stelnettts_diff::Report compare_logits_strided(const stelnettts_diff::Ref& ref, const std::string& name,
                                                    const float* data, size_t n_cpp_elem, int stride_cpp,
                                                    int stride_ref) {
    stelnettts_diff::Report r;
...
static stelnettts_diff::Report compare_with_row_width(const stelnettts_diff::Ref& ref, const std::string& name,
...
static int tiron_diff(const std::string& model, const std::string& ref_path, const std::string& /*audio*/) {
    ...
    whisper_full_params p = whisper_full_default_params(STELNETTTS_SAMPLING_GREEDY);
...
int main(int argc, char** argv) {
    ...
    if (argc < 5) {
        fprintf(stderr,
                ...
                "\n"
                "  backend       one of: voxtral, voxtral4b, qwen3, cielvox2-tts, cielvox2-tts-codec, omnivoice, tada-tts, "
                ...
                "  model.gguf    stelnettts-compatible model weights\n"
    ...
    if (backend_name == "piano" || backend_name == "piano-transcription") {
        ...
        if (!read_audio_data(audio_path, pcm, stereo_unused, /*stereo=*/false)) {
            fprintf(stderr, "stelnettts-diff: failed to read audio '%s'\n", audio_path.c_str());
            return 2;
        }
    ...
    if (!ref_backend.empty() && ref_backend != backend_name) {
        fprintf(stderr,
                "stelnettts-diff: warning: reference archive was dumped for backend '%s' "
                "but you asked for '%s'\n",
    ...
    if (backend_name == "voxtral") {
        ...
        auto scale_ok = [&](const stelnettts_diff::Report& r) {
            return std::isfinite(r.norm_ratio) && r.norm_ratio >= CHATTERBOX_NORM_RATIO_MIN &&
                   r.norm_ratio <= CHATTERBOX_NORM_RATIO_MAX;
        };
        auto print_row_mean = [&](const char* name, const stelnettts_diff::Report& r, float cos_threshold,
                                  const char* extra = "") {
        ...
        auto record_mean = [&](const stelnettts_diff::Report& r, float cos_threshold) {
            if (!r.found) {
                n_skip++;
        ...
        auto print_row_strict = [&](const char* name, const stelnettts_diff::Report& r, float cos_mean_threshold,
        ...
        auto record_strict = [&](const stelnettts_diff::Report& r, float cos_mean_threshold, float cos_min_threshold) {
            if (!r.found) {
                n_skip++;
        ...
        cp.verbosity = 0;
        cp.use_gpu = false;
        // STELNETTTS_DIFF_USE_GPU=1 flips the chatterbox C++ side onto the GPU
        ...
        if (const char* env_gpu = std::getenv("STELNETTTS_DIFF_USE_GPU")) {
            if (env_gpu[0] == '1' || env_gpu[0] == 't' || env_gpu[0] == 'T' || env_gpu[0] == 'y' || env_gpu[0] == 'Y') {
                cp.use_gpu = true;
                fprintf(stderr, "[stelnettts-diff] STELNETTTS_DIFF_USE_GPU=1 -> chatterbox use_gpu=true\n");
            }
        }
        ...
        if (const char* env_lang = stelnettts_env::get("STELNETTTS_CHATTERBOX_LANG")) {
            if (*env_lang)
                chatterbox_lang = env_lang;
        }
        if (!chatterbox_lang.empty()) {
            chatterbox_set_language(ctx, chatterbox_lang.c_str());
            fprintf(stderr, "[stelnettts-diff] CHATTERBOX_LANG=%s -> multilingual path\n", chatterbox_lang.c_str());
        }
        ...
        // ---- t3_cond_emb + t3_prefill_emb (deterministic, compare before stochastic T3) ----
        {
            ...
            // (`stelnettts.ref.chatterbox_syn_text`) — that's the text the
            ...
            const char* syn_text = stelnettts_env::get("STELNETTTS_CHATTERBOX_SYN_TEXT");
            ...
            if (!pemb) {
                ...
                if (prefill_ref.empty()) {
                    ...
                    if (stelnettts_env::get("STELNETTTS_CHATTERBOX_DEBUG")) {
                ...
                if (C_mel == 80) {
                    ...
                    if (ref_source_stft && T_src > 0) {
                        ...
                        for (const auto& s : voc_stages) {
                            ...
                            if (stelnettts_env::get("STELNETTTS_CHATTERBOX_DEBUG") && rep.found && rep.cos_mean < 0.999f) {
        ...
        cp.n_threads = 4;
        cp.verbosity = 0;
        // STELNETTTS_DIFF_NO_GPU=1 forces the CPU backend — discriminates
        // GPU-numerics divergence from port-logic divergence.
        cp.use_gpu = std::getenv("STELNETTTS_DIFF_NO_GPU") == nullptr;
        // STELNETTTS_DIFF_STAGES=mel,conv,enc_ref,enc_own,ids,logits selects
        ...
        auto stage_on = [](const char* key) {
            const char* s = std::getenv("STELNETTTS_DIFF_STAGES");
            if (!s || !*s)
                return true;
        ...
        // Reference mel (layout f*T + t — matches cielvox2_asr_run_* input).
        ...
        if (stage_on("conv") && ref_mel.first && ref.has("conv_out")) {
            int B = 0, Tc = 0, d = 0;
            float* conv = cielvox2_asr_run_conv(ctx, ref_mel.first, ref_n_mels, ref_T_mel, &B, &Tc, &d);
            if (conv) {
                ...
                printf("[ERR ] conv_out(ref_mel)       cielvox2_asr_run_conv returned null\n");
                n_fail++;
            }
        }
        ...
        // STELNETTTS_DIFF_NO_GPU=1 — set_output snapshots lie on Metal.
        if (stage_on("enc_stages") && ref_mel.first) {
            ...
            if (dir) {
                ...
                int N = 0, pd = 0;
                float* enc = cielvox2_asr_run_encoder(ctx, ref_mel.first, ref_n_mels, ref_T_mel, &N, &pd);
        ...
        if (stage_on("enc_ref") && ref_mel.first && ref.has("encoder_output")) {
            ...
            if (enc) {
                ...
                printf("[ERR ] encoder_output(ref_mel) cielvox2_asr_run_encoder returned null\n");
                n_fail++;
            }
        }
        ...
        if ((stage_on("enc_own") || stage_on("logits")) && mel_r.ok && ref.has("encoder_output")) {
            ...
            float* enc = cielvox2_asr_run_encoder(ctx, mel_r.data.data(), mel_r.shape[0], T_own, &N_enc_own, &pdim_own);
            if (enc) {
                ...
                printf("[ERR ] encoder_output(own_mel) cielvox2_asr_run_encoder returned null\n");
                n_fail++;
            }
        }
        ...
        if ((stage_on("ids") || stage_on("logits")) && ref.has("trace_input_ids")) {
            ...
            text += "<|audio_end|><|im_end|>\n"
                    "<|im_start|>assistant\n";
            int n_ids = 0;
            int32_t* ids = cielvox2_asr_tokenize(ctx, text.c_str(), &n_ids);
            if (ids) {
        ...
        if (stage_on("llm") && ref.has("trace_inputs_embeds") && ref.has("trace_first_logits")) {
            ...
            if (emb.first && emb_shape.size() >= 2) {
                ...
                if (cielvox2_asr_kv_init(ctx, std::max(4096, T_prompt + 64))) {
                    cielvox2_asr_kv_reset(ctx);
                    int n_t = 0, vocab = 0;
                    float* logits = cielvox2_asr_run_llm_kv(ctx, const_cast<float*>(emb.first), T_prompt, 0, &n_t, &vocab);
                    if (logits) {
        ...
        if (stage_on("logits") && !enc_own.empty() && !own_ids.empty() && ref.has("trace_first_logits")) {
            float* embeds = cielvox2_asr_embed_tokens(ctx, own_ids.data(), (int)own_ids.size());
            if (embeds) {
                int n_pad_id = 0;
                int32_t* pad_arr = cielvox2_asr_tokenize(ctx, "<|audio_pad|>", &n_pad_id);
                ...
                if (cielvox2_asr_kv_init(ctx, std::max(4096, T_prompt + 64))) {
                    cielvox2_asr_kv_reset(ctx);
                    int n_t = 0, vocab = 0;
                    float* logits = cielvox2_asr_run_llm_kv(ctx, embeds, T_prompt, 0, &n_t, &vocab);
                    if (logits) {
                        const float* last = logits + (size_t)(n_t - 1) * vocab;
                        auto rep = ref.compare("trace_first_logits", last, (size_t)vocab);
                        int own_arg = 0;
    ...
    } else if (backend_name == "cielvox2-tts") {
        ...
        auto cp = cielvox2_tts_context_default_params();
        cp.n_threads = 4;
        cp.verbosity = 0;
        cielvox2_tts_context* ctx = cielvox2_tts_init_from_file(model_path.c_str(), cp);
        if (!ctx) {
            fprintf(stderr, "failed to load cielvox2-tts model\n");
            return 4;
        }
        const char* codec_gguf = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_GGUF");
        if (codec_gguf && *codec_gguf) {
            if (cielvox2_tts_set_codec_path(ctx, codec_gguf) != 0) {
                fprintf(stderr, "failed to load cielvox2-tts codec '%s'\n", codec_gguf);
                cielvox2_tts_free(ctx);
                return 4;
            }
        }
        ...
        // and feed them through cielvox2_tts_run_text_proj.
        ...
        if (!ids_pair.first) {
            ...
            auto tp_r = cielvox2_tts_text_proj_r(ctx, ids.data(), (int)ids.size());
            if (tp_r.ok) {
        ...
        if (!embeds_pair.first || embeds_shape.size() < 2) {
            ...
            (void)d;
            auto tl_r = cielvox2_tts_talker_logits_r(ctx, embeds_pair.first, T);
            if (tl_r.ok) {
        ...
        const std::string syn_text = ref.meta("cielvox2_tts_syn_text");
        const std::string ref_text = ref.meta("cielvox2_tts_ref_text");
        if (syn_text.empty() || ref_text.empty()) {
            printf("[SKIP] talker_logits_via_icl  cielvox2_tts_syn_text / cielvox2_tts_ref_text not in reference (set "
                   "env vars at dump time)\n");
            n_skip++;
        } else {
            // Need a voice pack — load the canonical clone pack.
            int rc = cielvox2_tts_load_voice_pack(ctx, "/tmp/cielvox2-tts-voice-pack.gguf");
            if (rc != 0) {
                printf("[SKIP] talker_logits_via_icl  voice pack not loaded (run bake-cielvox2-tts-voice-pack first)\n");
                n_skip++;
            } else {
                int Tprefill = 0;
                float* my_prefill = cielvox2_tts_build_icl_prefill(ctx, syn_text.c_str(), ref_text.c_str(), &Tprefill);
                if (!my_prefill) {
                    ...
                    auto tl_r = cielvox2_tts_talker_logits_r(ctx, my_prefill, Tprefill);
                    free(my_prefill);
                    if (tl_r.ok) {
                        auto ref_logits_pair = ref.get_f32("talker_logits");
                        auto ref_logits_shape = ref.shape("talker_logits");
        ...
        if (syn_text.empty() || ref_text.empty()) {
            printf("[SKIP] runtime_voice_prompt    cielvox2_tts_syn_text / cielvox2_tts_ref_text not in reference\n");
            n_skip++;
        } else {
            int rc = cielvox2_tts_set_voice_prompt_with_text(ctx, audio_path.c_str(), ref_text.c_str());
            if (rc != 0) {
                ...
                int n_spk = 0;
                const float* my_spk = cielvox2_tts_get_runtime_spk_emb(ctx, &n_spk);
                ...
                int n_codes = 0;
                const int32_t* my_codes = cielvox2_tts_get_runtime_ref_codes(ctx, &n_codes);
                ...
                int Tprefill_rt = 0;
                float* rt_prefill = cielvox2_tts_build_icl_prefill(ctx, syn_text.c_str(), ref_text.c_str(), &Tprefill_rt);
                if (!rt_prefill) {
                    ...
                    auto tl_r = cielvox2_tts_talker_logits_r(ctx, rt_prefill, Tprefill_rt);
                    free(rt_prefill);
                    if (tl_r.ok) {
                        auto ref_logits_pair = ref.get_f32("talker_logits");
                        auto ref_logits_shape = ref.shape("talker_logits");
        ...
        for (int k = 0; k < 15; k++) {
            ...
            int vocab = 0;
            float* logits = cielvox2_tts_run_code_pred_step(ctx, in_pair.first, T_in, n_past, /*lm_head_idx=*/k, &vocab);
            if (!logits || vocab <= 0) {
                printf("[ERR ] %-15s cielvox2_tts_run_code_pred_step returned null\n", stage_label);
                n_fail++;
                break;
            }
        ...
        // ---- cielvox2-tts-spk (ECAPA speaker encoder) ----
        // Uses the same 440 Hz sine wave as the Python reference backend.
    } else if (backend_name == "cielvox2-tts-spk") {
        auto cp = cielvox2_tts_context_default_params();
        cp.n_threads = 4;
        cp.verbosity = 0;
        cp.use_gpu = false;
        ...
        float* mel = cielvox2_tts_compute_speaker_mel(ctx, audio.data(), n, &T_mel, &n_mels);
        if (!mel) {
        ...
        // Read spk_mel from the reference archive (Python-computed).
        {
            ...
            if (mel_pair.first && mel_shape.size() >= 2) {
                ...
                int dim2 = 0;
                float* emb2 = cielvox2_tts_run_speaker_enc_on_mel(ctx, mel_TC.data(), T, &dim2);
                if (emb2) {
        ...
        int dim = 0;
        float* emb = cielvox2_tts_compute_speaker_embedding(ctx, audio.data(), n, &dim);
        if (!emb) {
        ...
        // ---- cielvox2-tts-cenc (codec ENCODER: audio → codes) ----
        // Uses the same fixed 3s slice of clone.wav as the Python reference.
    } else if (backend_name == "cielvox2-tts-cenc") {
        const char* codec_gguf = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_GGUF");
        if (!codec_gguf) {
            fprintf(stderr, "cielvox2-tts-cenc: set CIELVOX2_TTS_CODEC_GGUF=<codec.gguf>\n");
            return 4;
        }
        auto cp = cielvox2_tts_context_default_params();
        cp.n_threads = 4;
        cp.verbosity = 0;
        cp.use_gpu = false;
        cielvox2_tts_context* qctx = cielvox2_tts_init_from_file(model_path.c_str(), cp);
        if (!qctx) {
            fprintf(stderr, "failed to load talker\n");
            return 4;
        }
        if (cielvox2_tts_set_codec_path(qctx, codec_gguf) != 0) {
            fprintf(stderr, "failed to load codec\n");
            cielvox2_tts_free(qctx);
            return 4;
        }
        ...
        for (const char* s : stages) {
            int n = 0;
            float* mine = cielvox2_tts_cenc_extract_stage(qctx, audio_buf.data(), n_samp, s, &n);
            if (!mine) {
        ...
        {
            int n = 0;
            float* mine = cielvox2_tts_cenc_extract_stage(qctx, audio_buf.data(), n_samp, "cenc_codes", &n);
            if (!mine) {
        ...
        {
            int rc = cielvox2_tts_set_voice_prompt(qctx, audio_path.c_str());
            if (rc != 0) {
                ...
                int n_codes = 0;
                const int32_t* my_codes = cielvox2_tts_get_runtime_ref_codes(qctx, &n_codes);
                if (!ref_codes_pair.first || ref_codes_shape.size() < 2 || !my_codes) {
    ...
    } else if (backend_name == "cielvox2-tts-codec") {
        const char* codec_gguf = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_GGUF");
        if (!codec_gguf) {
            fprintf(stderr, "cielvox2-tts-codec: set CIELVOX2_TTS_CODEC_GGUF=<path/to/codec.gguf>\n");
            return 4;
        }
        auto cp = cielvox2_tts_context_default_params();
        cp.n_threads = 4;
        ...
        cielvox2_tts_context* ctx = cielvox2_tts_init_from_file(model_path.c_str(), cp);
        if (!ctx) {
            fprintf(stderr, "failed to load cielvox2-tts model '%s'\n", model_path.c_str());
            return 4;
        }
        if (cielvox2_tts_set_codec_path(ctx, codec_gguf) != 0) {
        ...
        // Stage list matches DEFAULT_STAGES in cielvox2_tts_codec.py
        ...
        for (int si = 0; si < 8; si++) {
            ...
            int n_stage = 0;
            float* our_data = cielvox2_tts_codec_extract_stage(ctx, codes.data(), T_codec * N_Q, stage, &n_stage);
            if (!our_data) {
                printf("[ERR ] %-22s  extract returned null\n", ref_name);
                n_fail++;
                continue;
        ...
        const char* env_codec = stelnettts_env::get("STELNETTTS_TADA_CODEC_GGUF");
        ...
        const char* env_text = stelnettts_env::get("STELNETTTS_TADA_DIFF_TEXT");
        ...
        if (const char* prompt = stelnettts_env::get("STELNETTTS_TADA_PROMPT_CACHE"); prompt && *prompt)
            prompt_path = prompt;
        ...
        for (const char* stage : gen_stages) {
            ...
            auto rep = ref.compare(stage, our_data, (size_t)n_stage, stelnettts_diff::Ref::COS_LAST_DIM);
        ...
        if (gen_pcm && n_pcm > 0) {
            auto rep = ref.compare("codec_pcm", gen_pcm, (size_t)n_pcm, stelnettts_diff::Ref::COS_LAST_DIM);
        ...
        if (!gen_acoustic_features.empty() && ref.has("acoustic_features")) {
            auto rep = ref.compare("acoustic_features", gen_acoustic_features.data(), gen_acoustic_features.size(),
                                   stelnettts_diff::Ref::COS_LAST_DIM);
        ...
        struct TadaStage {
            ...
            stelnettts_diff::Ref::CompareMode mode;
        };
        static const TadaStage stages[] = {
            {"codec_proj", "dump_proj", stelnettts_diff::Ref::COS_LAST_DIM},
            {"codec_attn_out", "dump_attn", stelnettts_diff::Ref::COS_LAST_DIM},
            {"codec_pcm", "pcm", stelnettts_diff::Ref::COS_LAST_DIM},
        };
        ...
        // verified safe. The cielvox2-tts kernel_conv_transpose_1d watchdog hang
        // shape is comparable to MiMo's conv2 / down_sample. Opt in with
        // MIMO_TOKENIZER_GPU=1.
        cp.use_gpu = stelnettts_env::get("STELNETTTS_MIMO_TOKENIZER_GPU") != nullptr;
        ...
        cp.n_threads = 4;
        cp.verbosity = 0;
        cp.use_gpu = stelnettts_env::get("STELNETTTS_MIMO_ASR_GPU") != nullptr;
        ...
        cp.n_threads = 4;
        cp.verbosity = 0;
        cp.use_gpu = stelnettts_env::get("STELNETTTS_ARKASR_GPU") != nullptr;
        ...
        // ---- every residual stage + stats + embedding, in one forward ----
        {
            ...
            if (rc != 0) {
                ...
                for (const char* nm : {"stem_out", "layer1_out", "layer2_out", "layer3_out", "layer4_out", "stats"}) {
                    ...
                    // brittle statistic here. stelnettts_diff skips any row whose
        ...
        //
        // Opt in with `STELNETTTS_DIFF_SLICES=s0:e0,s1:e1,...` (sample
        ...
        if (const char* slice_env = std::getenv("STELNETTTS_DIFF_SLICES")) {
            auto ref_enc_pair = ref.get_f32("encoder_output");
            auto ref_enc_shp = ref.shape("encoder_output");
            if (!ref_enc_pair.first || ref_enc_shp.size() < 2) {
                ...
                printf("\nper-slice diff (STELNETTTS_DIFF_SLICES, %d slice(s), per-frame cos vs ref)\n",
        ...
        if (phonemes.empty()) {
            fprintf(stderr, "stelnettts-diff kokoro: reference is missing kokoro_phonemes metadata. "
                            "Re-dump with KOKORO_PHONEMES=<ipa> set.\n");
            return 4;
        }
        const char* voice_env = stelnettts_env::get("STELNETTTS_KOKORO_VOICE_GGUF");
        ...
        const char* gpu_env = stelnettts_env::get("STELNETTTS_KOKORO_USE_GPU");
        ...
        printf("stelnettts-diff kokoro: phonemes=%s  voice=%s\n", phonemes.c_str(), voice_gguf.c_str());
        ...
        const char* dump_dir = stelnettts_env::get("STELNETTTS_KOKORO_DUMP_STAGES");
        ...
        //
        // model_path is the SNAC GGUF (Xenna/snac-24khz-GGUF or built
        ...
        sp.n_threads = 4;
        sp.verbosity = 0;
        sp.use_gpu = stelnettts_env::get("STELNETTTS_ORPHEUS_SNAC_GPU") != nullptr;
        ...
        const int T_super = stelnettts_env::get("STELNETTTS_ORPHEUS_SNAC_T_SUPER")
                                ? std::atoi(stelnettts_env::get("STELNETTTS_ORPHEUS_SNAC_T_SUPER"))
                                : 4;
        const int fill_code = stelnettts_env::get("STELNETTTS_ORPHEUS_SNAC_CODE")
                                  ? std::atoi(stelnettts_env::get("STELNETTTS_ORPHEUS_SNAC_CODE"))
                                  : 0;
        ...
        printf("stelnettts-diff: orpheus SNAC: T_super=%d  code=%d  → %zu+%zu+%zu codebook entries\n", T_super, code,
        ...
        if (!pid || pid_n == 0) {
            fprintf(stderr, "stelnettts-diff orpheus-talker: reference missing prompt_ids\n");
            return 4;
        }
        std::string ids_str;
        ...
        cp.use_gpu = stelnettts_env::get("STELNETTTS_ORPHEUS_DIFF_GPU") != nullptr;
        ...
        {
            const char* mg = stelnettts_env::get("STELNETTTS_ORPHEUS_DIFF_MAXGEN");
        ...
        if (stelnettts_env::get("STELNETTTS_VOXCPM2_CPU_ONLY")) {
            cp.use_gpu = false;
        }
        ...
        if (const char* env = stelnettts_env::get("STELNETTTS_CV3_FLOW_GGUF"); env && *env) {
            flow_path = env;
        } else {
        ...
        if (const char* env = stelnettts_env::get("STELNETTTS_CV3_HIFT_GGUF"); env && *env) {
            hift_path = env;
        } else {
        ...
        {
            std::string s3_path;
            if (const char* env = stelnettts_env::get("STELNETTTS_CV3_S3TOK_GGUF"); env && *env) {
                s3_path = env;
            } else {
                s3_path = model_path;
        ...
        if (!ref.shape("clone_spk_emb").empty() || !ref.shape("clone_prompt_feat_24k").empty() ||
            ...
            if (const char* env = stelnettts_env::get("STELNETTTS_CV3_CAMPPLUS_GGUF"); env && *env)
                camp_path = env;
            else {
            ...
            if (!ref.shape("clone_spk_emb").empty()) {
                ...
                if (camp_ok && cosyvoice3_tts_extract_spk_emb(ctx, audio_path.c_str(), emb) == 0) {
                    ...
                    printf("[SKIP] %-30s  campplus gguf '%s' not loaded (set STELNETTTS_CV3_CAMPPLUS_GGUF)\n",
            ...
            if (!ref.shape("clone_speech_tokens").empty()) {
                ...
                if (tk && n_tok > 0) {
                    ...
                    printf("[SKIP] %-30s  s3tok gguf '%s' not loaded (set STELNETTTS_CV3_S3TOK_GGUF)\n",
        ...
        // CSM_WAV_OUT is set (reuses this build since the main CLI's stelnettts
        ...
        if (const char* wav_out = stelnettts_env::get("STELNETTTS_CSM_WAV_OUT")) {
            const char* wtext = stelnettts_env::get("STELNETTTS_CSM_WAV_TEXT");
            std::string syn_text = wtext ? wtext : (text.empty() ? "Hello, how are you?" : text);
            float temp = stelnettts_env::get("STELNETTTS_CSM_WAV_TEMP")
                             ? (float)atof(stelnettts_env::get("STELNETTTS_CSM_WAV_TEMP"))
                             : 0.9f;
            int fcap =
                stelnettts_env::get("STELNETTTS_CSM_WAV_FRAMES") ? atoi(stelnettts_env::get("STELNETTTS_CSM_WAV_FRAMES")) : 64;
        ...
        if (desc_text.empty() || tts_text.empty()) {
            fprintf(stderr, "stelnettts-diff parler-tts: reference is missing parler_desc / parler_text metadata.\n");
            return 4;
        }
        printf("stelnettts-diff parler-tts: desc='%.60s...' text='%s'\n", desc_text.c_str(), tts_text.c_str());
        ...
        if (!ref_desc_data || !ref_prompt_data) {
            fprintf(stderr, "stelnettts-diff parler-tts: reference missing description_ids or prompt_ids\n");
            return 4;
        }
        ...
        {
            const char* mg = stelnettts_env::get("STELNETTTS_PARLER_DIFF_MAXGEN");
        ...
        kp.use_gpu = true;
        kp.flash_attn = true;
        if (stelnettts_env::get("STELNETTTS_KUGELAUDIO_CPU_ONLY"))
            kp.use_gpu = false;
        ...
        kugelaudio_free(ctx);
#else
        fprintf(stderr, "stelnettts-diff: kugelaudio backend not compiled in\n");
        return 4;
#endif
        ...
        cp.n_threads = 4;
        cp.verbosity = 1;
        if (const char* g = std::getenv("STELNETTTS_DIFF_USE_GPU"); g && g[0] == '1') {
            cp.use_gpu = true;
            fprintf(stderr, "[stelnettts-diff] STELNETTTS_DIFF_USE_GPU=1 -> moss_transcribe use_gpu=true\n");
        }
        ...
        //   stelnettts-diff melotts melotts-en-f16.gguf /tmp/melotts-ref.gguf dummy.wav

        // Read melotts metadata directly from reference GGUF
        gguf_context* ref_meta = core_gguf::open_metadata(ref_path.c_str());
        if (!ref_meta) {
            fprintf(stderr, "stelnettts-diff melotts: failed to open reference '%s'\n", ref_path.c_str());
            return 4;
        }
        ...
        printf("stelnettts-diff melotts: text=\"%s\"  seed=%u  speaker=%u\n", text.c_str(), seed, spk);
        ...
        if (!ctx) {
            fprintf(stderr, "stelnettts-diff melotts: failed to load '%s'\n", model_path.c_str());
            return 4;
        }

        // Enable intermediate dumps
        char dump_dir[] = "/tmp/stelnettts-diff-melotts-XXXXXX";
        if (!mkdtemp(dump_dir)) {
            fprintf(stderr, "stelnettts-diff melotts: mkdtemp failed\n");
            melotts_free(ctx);
            return 4;
        }
        ...
        if (n <= 0 || !pcm) {
            fprintf(stderr, "stelnettts-diff melotts: synthesis failed\n");
            melotts_free(ctx);
            return 4;
        }
        ...
        if (!ctx) {
            fprintf(stderr, "stelnettts-diff: failed to load zonos-tts model '%s'\n", model_path.c_str());
            return 4;
        }

        // Resolve synthesis text
        std::string syn_text;
        const char* env_text = stelnettts_env::get("STELNETTTS_ZONOS_TTS_TEXT");
        ...
        printf("stelnettts-diff[zonos-tts]: text = %s\n", syn_text.c_str());

        // How many AR steps to compare (must match Python ZONOS_DIFF_N_STEPS)
        int n_diff_steps = 10;
        if (const char* ns = stelnettts_env::get("STELNETTTS_ZONOS_DIFF_N_STEPS"))
        ...
        // Run the AR dump and compare slot by slot.
        {
            ...
            if (!ar_dump) {
                ...
                auto print_row_logits = [&](const char* nm, const stelnettts_diff::Report& rpt) {
                ...
                auto record_logits = [&](const stelnettts_diff::Report& rpt) {
                    if (!rpt.found) {
                        n_skip++;
                        return;
                    }
        ...
        lp.n_threads = 4;
        lp.verbosity = 0;
        // STELNETTTS_DIFF_USE_GPU=1 runs the lfm2 C++ stages on the GPU backend so
        // the per-stage diff can isolate CPU-vs-GPU divergence.
        if (const char* eg = std::getenv("STELNETTTS_DIFF_USE_GPU")) {
            if (eg[0] == '1' || eg[0] == 't' || eg[0] == 'T' || eg[0] == 'y' || eg[0] == 'Y') {
                lp.use_gpu = true;
                fprintf(stderr, "[stelnettts-diff] STELNETTTS_DIFF_USE_GPU=1 -> lfm2-audio use_gpu=true\n");
            }
        }
        ...
        //   Xenna/kyutai-stt-2.6b-en-GGUF / kyutai-stt-2.6b-ref.gguf
        ...
        std::string tok_path;
        if (const char* env_tok = stelnettts_env::get("STELNETTTS_OMNIVOICE_TOKENIZER_GGUF")) {
            tok_path = env_tok;
        } else {
        ...
        if (tok_path.empty() || omnivoice_set_tokenizer_path(ctx, tok_path.c_str()) != 0) {
            fprintf(stderr,
                    ...
                    "STELNETTTS_OMNIVOICE_TOKENIZER_GGUF or place omnivoice-tokenizer-f16.gguf next to the model\n",
                    tok_path.c_str());
            omnivoice_free(ctx);
            return 4;
        }
        ...
        // conv_stem_out: dump via env, compare against ref
        {
            ...
            setenv("STELNETTTS_MOSS_DIARIZE_CONV_DUMP", conv_dump.c_str(), 1);
        }
        ...
        fprintf(stderr,
                "stelnettts-diff: backend '%s' is not recognised. "
                "Supported: voxtral, voxtral4b, qwen3, cielvox2-tts, cielvox2-tts-codec, omnivoice, kokoro, granite, "
