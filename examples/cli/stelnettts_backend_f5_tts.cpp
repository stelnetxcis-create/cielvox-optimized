// stelnettts_backend_f5_tts.cpp — adapter for SWivid/F5-TTS.
//
...
#include "stelnettts_tts_ref_text.h"
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[f5-tts]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[f5-tts]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
    ...
    void prepare_voice(const whisper_params& p) {
        ...
        if (ref_pcm.empty() || wav_sr <= 0) {
            fprintf(stderr, "stelnettts[f5-tts]: failed to load reference audio '%s'\n", p.tts_voice.c_str());
            last_voice_.clear();
            return;
        }
        ...
        if (ref_text_str.empty()) {
            ...
            // Whisper entirely. Disable with STELNETTTS_TTS_REF_CACHE=0.
            const std::string cache_path = stelnettts_ref_cache::path_for(p.tts_voice, ".f5reftext");
            const bool cache_enabled = !stelnettts_ref_cache::disabled();
            std::vector<uint32_t> shape;
            std::vector<uint8_t> payload;
            if (cache_enabled && stelnettts_ref_cache::load(cache_path, p.tts_voice, "f5-reftext", shape, payload)) {
                ref_text_str.assign((const char*)payload.data(), payload.size());
                if (!p.no_prints) {
                    fprintf(stderr, "stelnettts[f5-tts]: using cached ref transcript '%s': '%s'\n", cache_path.c_str(),
                            ref_text_str.c_str());
                }
            } else {
                ...
                if (!p.no_prints) {
                    fprintf(stderr, "stelnettts[f5-tts]: --ref-text not set, auto-transcribing via %s...\n",
                            asr_name.c_str());
                }
                ref_text_str = stelnettts_ref_text::transcribe(ref_16k, p, asr_name, "stelnettts[f5-tts]");
                if (cache_enabled && !ref_text_str.empty()) {
                    stelnettts_ref_cache::save(cache_path, "f5-reftext", {(uint32_t)ref_text_str.size()},
                                             ref_text_str.data(), ref_text_str.size());
                }
            }
            if (ref_text_str.empty()) {
                if (!p.no_prints) {
                    fprintf(stderr, "stelnettts[f5-tts]: auto-transcription returned empty; "
                                    "duration estimate may be inaccurate\n");
                }
            } else if (!p.no_prints) {
                fprintf(stderr, "stelnettts[f5-tts]: auto-transcribed ref: '%s'\n", ref_text_str.c_str());
            }
        }
        ...
        if (f5_tts_set_reference(ctx_, ref_24k.data(), (int)ref_24k.size(), ref_text) != 0) {
            fprintf(stderr, "stelnettts[f5-tts]: failed to set reference audio\n");
            last_voice_.clear();
            return;
        }

        if (!p.no_prints) {
            fprintf(stderr, "stelnettts[f5-tts]: loaded ref audio '%s' (%d@%dHz → %d@24kHz) ref_text='%s'\n",
