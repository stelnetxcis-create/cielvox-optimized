// stelnettts_voice_clone_policy.h — what counts as a voice CLONE.
//
...
// same reason stelnettts_marking_policy.h is: a gate nobody can test on the tier
...
//      REWRITES --voice to point at it (stelnettts_run.cpp) before the gate runs.
...
// WHAT IS *NOT* A CLONE
// ---------------------
// Not every voice pack is a clone. kokoro / cielvox2-tts / miotts / vibevoice ship
...
// they are: the bakers stamp `stelnettts.voice.cloned_from_recording` into packs
// they derive from a user recording, and the gate reads it back.
//
// The honest limitation: a pack baked by a StelnetTTS older than that stamp
...
// or pointed at by STELNETTTS_COSYVOICE3_VOICES_PATH, and --voice selects an
// entry inside it BY NAME (stelnettts_backend_cosyvoice3.cpp — whose own header
...
#include "stelnettts_speaker_identity.h" // SpeakerIdentity, carried on BankFacts

#include <string>

namespace stelnettts_voice {
...
// (stelnettts_backend_zonos.cpp), and adding a decoder to another adapter is a
...
// recording. Read back by the gate; see stelnettts_voice_provenance.h.
//
...
inline const char* provenance_key() {
    return "stelnettts.voice.cloned_from_recording";
}
...
inline std::string bank_provenance_key_for(const std::string& voice_name) {
    return "stelnettts.voice." + voice_name + ".cloned_from_recording";
}
...
inline const char* bank_stamped_key() {
    return "stelnettts.voice.bank_stamped";
}
...
//   cielvox2tts.voicepack   models/bake-cielvox2-tts-voice-pack.py       (name:wav:text)
...
// tada-ref case: StelnetTTS ships no cosyvoice3 bank and auto-downloads none, so
...
//   stelnettts.reference (tada)       — AMBIGUOUS: both the shipped
...
inline bool architecture_is_recording_derived(const std::string& arch) {
    return arch == "chatterbox-voice" || arch == "cielvox2tts.voicepack" || arch == "cosyvoice3-voices";
}
...
inline const char* provenance_attestation_key() {
    return "stelnettts.voice.consent_attestation";
}
...
// read_bank_provenance() (stelnettts_voice_provenance.h) when the backend selects
// voices by name from a bundle rather than by path; all-default otherwise.
struct BankFacts {
    // Whose voice this entry is (stelnettts.voice.<name>.speaker_identity, then
