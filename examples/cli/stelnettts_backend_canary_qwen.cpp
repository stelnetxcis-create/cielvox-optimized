// stelnettts_backend_canary_qwen.cpp — adapter for nvidia/canary-qwen-2.5b.
//
...
public:
    ...
    //   - with --chunk-seconds: the #257 gate in stelnettts_run.cpp handed the
    ...
    // stelnettts_chunk_context_gate.h (#218) — that gate is correct and unrelated.
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[canary-qwen]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
