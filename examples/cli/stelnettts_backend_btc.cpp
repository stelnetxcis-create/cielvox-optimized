// stelnettts_backend_btc.cpp — BTC chord-recognition backend shim.
//
...
// precedent that settled this for stems, and stelnettts_backend_crepe.cpp for the
// pitch equivalent. It is exposed through the `--chords` dispatcher
// (examples/cli/stelnettts_chords_cli.cpp), which resolves the model, decodes the
...
// table in stelnettts_backend.cpp — so `--list-backends` did not know the backend
...
// this shim plus the four registration sites in stelnettts_backend.cpp.
...
public:
    ...
    bool init(const whisper_params&) override {
        fprintf(stderr, "stelnettts: btc-chords is a chord-recognition model — run it with --chords\n"
                        "  (e.g. `stelnettts --chords -m <btc-chords-large-f16.gguf> -f song.wav`), not as a\n"
    ...
    std::vector<stelnettts_segment> transcribe(const float*, int, int64_t, const whisper_params&) override { return {}; }
