// stelnettts_backend_moss_tts.cpp — CLI adapter for MOSS-TTS-v1.5 (MossTTSDelay).
//
...
class MossTtsBackend : public StelnetAsrBackend {
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[moss-tts]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[moss-tts]: failed to load backbone '%s'\n", p.model.c_str());
            return false;
        }
        ...
        if (codec_path.empty()) {
            fprintf(stderr, "stelnettts[moss-tts]: no codec model found. Pass --codec-model PATH or place "
                            "<stem>-codec.gguf next to the backbone.\n");
            return false;
        }
        if (!moss_tts_set_codec_path(ctx_, codec_path.c_str())) {
            fprintf(stderr, "stelnettts[moss-tts]: failed to load codec '%s'\n", codec_path.c_str());
            return false;
        }
        if (!p.no_prints)
            fprintf(stderr, "stelnettts[moss-tts]: codec loaded from '%s'\n", codec_path.c_str());
        return true;
    }
    ...
    void prepare_voice(const whisper_params& params) {
        ...
        int sr = 0;
        if (!stelnettts::core::read_wav_mono_pcm16(params.tts_voice, ref, sr) || ref.empty()) {
            fprintf(stderr, "stelnettts[moss-tts]: failed to load reference audio '%s'\n", params.tts_voice.c_str());
            last_voice_.clear();
            return;
        }
        ...
        if (sr != target && sr > 0) {
            ...
            if (!params.no_prints)
                fprintf(stderr, "stelnettts[moss-tts]: resampled reference %d->%d Hz\n", sr, target);
        }
        if (!moss_tts_set_reference_wav(ctx_, ref.data(), (int)ref.size())) {
            fprintf(stderr, "stelnettts[moss-tts]: reference encode failed for '%s'\n", params.tts_voice.c_str());
            last_voice_.clear();
        } else if (!params.no_prints) {
            fprintf(stderr, "stelnettts[moss-tts]: cloning voice from '%s'\n", params.tts_voice.c_str());
        }
    }
    ...
    std::vector<float> synthesize(const std::string& text, const whisper_params& params) override {
        ...
        if (params.tts_min_speech_tokens >= 0) {
            ...
            stelnettts_speech_window::Window win;
            ...
            win.frame_cap = sp.max_new_tokens > 0 ? sp.max_new_tokens : stelnettts_speech_window::kDefaultFrameCap;
            const std::string warn = stelnettts_speech_window::diagnose(win);
            if (!warn.empty())
                fprintf(stderr, "stelnettts[moss-tts]: warning: %s\n", warn.c_str());
        }
        ...
        if (!params.language.empty() && params.language != "auto") {
            lang = stelnettts_iso_to_english_lang(params.language);
...
std::unique_ptr<StelnetAsrBackend> stelnettts_make_moss_tts_backend() {
    return std::unique_ptr<StelnetAsrBackend>(new MossTtsBackend());
}
