// stelnettts_backend_indextts.cpp -- adapter for IndexTTS-1.5 TTS.
//
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[indextts]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[indextts]: failed to load GPT model '%s'\n", p.model.c_str());
            return false;
        }
        ...
        if (!vocoder_path.empty() && vocoder_path != "auto" && vocoder_path != "default") {
            vocoder_path = stelnettts_resolve_model_cli(vocoder_path, p.backend, p.no_prints, p.cache_dir,
        ...
        if (!vocoder_path.empty()) {
            ...
            if (!p.no_prints) {
                fprintf(stderr, "stelnettts[indextts]: vocoder path = '%s'\n", vocoder_path.c_str());
            }
        } else if (!p.no_prints) {
            fprintf(stderr, "stelnettts[indextts]: no BigVGAN vocoder found. Pass --codec-model PATH or place "
    ...
    std::vector<float> synthesize(const std::string& text, const whisper_params& params) override {
        ...
        // reference after the first). Disable the cache with STELNETTTS_TTS_REF_CACHE=0.
        const float* ref_pcm = nullptr;
        int ref_n_samples = 0;
        std::vector<float> ref_audio;
        if (!voice_path_.empty() && !ref_ready_) {
            int sr = 0;
            if (stelnettts::core::read_wav_mono_pcm16(voice_path_, ref_audio, sr)) {
                if (sr != 24000 && sr > 0) {
                    ref_audio = core_audio::resample_polyphase(ref_audio.data(), (int)ref_audio.size(), sr, 24000);
                    if (!params.no_prints)
                        fprintf(stderr, "stelnettts[indextts]: resampled reference audio from %d Hz to 24000 Hz\n", sr);
                    sr = 24000;
                }
                ref_pcm = ref_audio.data();
                ref_n_samples = (int)ref_audio.size();
                if (!params.no_prints)
                    fprintf(stderr, "stelnettts[indextts]: loaded reference audio '%s' (%d samples, %d Hz)\n",
                            voice_path_.c_str(), ref_n_samples, sr);
            } else {
                fprintf(stderr, "stelnettts[indextts]: failed to load reference audio '%s'\n", voice_path_.c_str());
            }
        }
        ...
        if (!voice_path_.empty() && pcm && n > 0)
            ref_ready_ = true;
        if (!pcm || n <= 0) {
            fprintf(stderr, "stelnettts[indextts]: synthesis failed\n");
            return {};
        }
