// stelnettts_backend_fastconformer_ctc.cpp — adapter for standalone
...
// fastconformer-ctc`` flag and produces a single `stelnettts_segment`
...
public:
    ...
    uint32_t capabilities() const override {
        ...
        // CAP_INTERNAL_CHUNKING: skip stelnettts_run.cpp's 30 s auto-chunk
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[fastconformer-ctc]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (rc != 0 || !logits) {
            fprintf(stderr, "stelnettts[fastconformer-ctc]: compute_logits failed (%d)\n", rc);
            return out;
        }
        ...
        std::free(logits);
        if (!r || !r->text) {
            fprintf(stderr, "stelnettts[fastconformer-ctc]: greedy_decode failed\n");
            if (r)
