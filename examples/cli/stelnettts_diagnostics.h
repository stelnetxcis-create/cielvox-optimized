// stelnettts_diagnostics.h — startup diagnostics + --version / --diagnostics
//
...
//
//   stelnettts_print_build_info  — version, git SHA, build date, compiler,
//                                compiled-in ggml backends, supported
//                                CUDA archs. Pure compile-time info.
//   stelnettts_print_runtime_env — env vars relevant to GPU bring-up,
//                                LD_LIBRARY_PATH, /etc/ld.so.conf.d
//                                contents, /usr/local/cuda/compat presence.
//   stelnettts_print_devices     — calls ggml_backend_load_all() and
...
// Compact one-line build banner ("stelnettts <version> (<git>) <backends>").
