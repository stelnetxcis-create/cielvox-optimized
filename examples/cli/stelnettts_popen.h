// stelnettts_popen.h — UTF-8-safe popen/pclose wrappers.
//
...
//
// Returns nullptr on failure. The caller closes via `stelnettts_pclose`.
inline FILE* stelnettts_popen(const std::string& cmd, const char* mode) {
#if defined(_WIN32)
...
inline int stelnettts_pclose(FILE* pipe) {
#if defined(_WIN32)
    return _pclose(pipe);
#else
    return ::pclose(pipe);
