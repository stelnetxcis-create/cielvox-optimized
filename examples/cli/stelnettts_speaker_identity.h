// stelnettts_speaker_identity.h — whose voice is a PRESET voice?
//
// The voice-clone gate (stelnettts_voice_clone_policy.h) answers "did this voice
...
// Sits alongside stelnettts.voice.cloned_from_recording; the two are independent
// (a pack can be a non-clone preset AND a real person).
inline const char* speaker_identity_key() {
    return "stelnettts.voice.speaker_identity";
}
...
inline std::string speaker_identity_key_for(const std::string& voice_name) {
    return "stelnettts.voice." + voice_name + ".speaker_identity";
}
...
inline std::string unknown_identity_warning(const std::string& model_id) {
    return "stelnettts: warning: '" + (model_id.empty() ? std::string("<unknown-model>") : model_id) +
           ...
           "  Pass --speaker-identity real_person to have StelnetTTS prepend the disclosure, or\n"
