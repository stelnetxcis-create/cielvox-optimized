// stelnettts_gap_fill.h — issue #89 gap-fill second pass, shared between the
// CLI dispatcher (stelnettts_run.cpp) and the HTTP server (stelnettts_server.cpp).
//
...
// STELNETTTS_GAP_FILL=0 disables, STELNETTTS_GAP_FILL_MIN_CS tunes the trigger
...
// (#205 / 617cd02).
inline std::string stelnettts_rebuild_text_from_words(const std::vector<stelnettts_word>& words) {
    std::string rebuilt;
...
// in stelnettts_run.cpp. A covering segment with no words on the far side of the
...
// stelnettts_make_disp_segments builds every SRT/VTT cue from the word
...
inline void stelnettts_gap_fill_resolve_overlaps(std::vector<stelnettts_segment>& segs) {
    const auto by_time = [](const stelnettts_segment& a, const stelnettts_segment& b) {
    ...
    while (changed) {
        ...
        for (size_t i = 1; i < segs.size(); i++) {
            stelnettts_segment& prev = segs[i - 1];
            stelnettts_segment& cur = segs[i];
            if (cur.t0 >= prev.t1)
                continue;
            ...
            if (interleaved && !cur.words.empty()) {
                ...
                std::stable_sort(prev.words.begin(), prev.words.end(),
                                 [](const stelnettts_word& a, const stelnettts_word& b) { return a.t0 < b.t0; });
                std::stable_sort(prev.tokens.begin(), prev.tokens.end(),
                                 [](const stelnettts_token& a, const stelnettts_token& b) { return a.t0 < b.t0; });
                prev.text = stelnettts_rebuild_text_from_words(prev.words);
            ...
            if (!prev.words.empty() && prev.words.back().t0 >= cur.t1) {
                stelnettts_segment tail = prev; // copy metadata (speaker, chunk_id, ...)
                std::vector<stelnettts_word> head_words, tail_words;
                ...
                std::vector<stelnettts_token> head_tokens, tail_tokens, untimed_tokens;
                ...
                tail.text = stelnettts_rebuild_text_from_words(tail.words);
...
inline void stelnettts_gap_fill_slice(CrispasrBackend& be, const whisper_params& params, const float* samples,
    ...
    for (int round = 0; round < max_rounds; round++) {
        ...
        for (auto& g : gaps) {
            ...
            if (rec.words.empty())
                continue;
            rec.text = stelnettts_rebuild_text_from_words(rec.words);
            if (rec.text.empty())
                continue;
        ...
        stelnettts_gap_fill_resolve_overlaps(segs);
    }
}
