// stelnettts_backend_granite.cpp — adapter for ibm-granite/granite-4.0-1b-speech.
//
...
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[granite]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (!mel) {
            fprintf(stderr, "stelnettts[granite]: mel failed\n");
            return out;
        }
        ...
        free(mel);
        if (!enc) {
            fprintf(stderr, "stelnettts[granite]: encoder failed\n");
            return out;
        }
        ...
        free(enc);
        if (!proj) {
            fprintf(stderr, "stelnettts[granite]: projector failed\n");
            return out;
        }
        ...
        auto iso_to_eng = [](const std::string& c) -> std::string { return stelnettts_iso_to_english_lang(c); };
        ...
        if (prefix_ids.empty() || suffix_ids.empty()) {
            fprintf(stderr, "stelnettts[granite]: tokenize failed — re-convert the GGUF "
        ...
        if (!all_embeds) {
            free(proj);
            fprintf(stderr, "stelnettts[granite]: embed failed\n");
            return out;
        }
        ...
        if (!granite_speech_kv_init(ctx_, 4096)) {
            free(all_embeds);
            fprintf(stderr, "stelnettts[granite]: kv init failed\n");
            return out;
        }
        ...
        if (params.beam_size > 1) {
            ...
            if (!logits) {
                fprintf(stderr, "stelnettts[granite]: prefill failed\n");
                free(all_embeds);
                return out;
            }
            free(all_embeds);
            ...
            // collapse decision made on seg.text above.
            {
                ...
                if (keep.size() != seg.tokens.size()) {
                    std::vector<stelnettts_token> filtered;
        ...
        for (int run = 0; run < n_runs; run++) {
            ...
            float* logits = granite_speech_run_llm_kv(ctx_, all_embeds, total_prompt, 0, nullptr, &vocab);
            if (!logits) {
                fprintf(stderr, "stelnettts[granite]: prefill failed (run %d/%d)\n", run + 1, n_runs);
                free(all_embeds);
                return out;
            }
            if (run == 0)
        ...
        if (!params.no_prints && n_runs > 1)
            fprintf(stderr, "stelnettts[granite]: best-of-%d picked score=%.4f\n", n_runs, best_score);
        ...
        auto parse_ts = [&](const std::string& text, std::vector<stelnettts_word>& words) -> std::string {
        ...
        if (want_saa && !seg.text.empty()) {
            ...
            size_t last_pos = 0;
            std::vector<stelnettts_segment> saa_segs;
                ...
                if (want_ts) {
                    ...
                    if (!words.empty()) {
                        ...
                        if (keep.size() != words.size()) {
                            std::vector<stelnettts_word> filtered;
                            filtered.reserve(keep.size());
                            for (int idx : keep)
