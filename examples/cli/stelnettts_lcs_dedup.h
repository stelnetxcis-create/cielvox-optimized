// stelnettts_lcs_dedup.h — LCS-based deduplication driver for overlap-save
// chunk boundaries (issue #89 / #114 follow-up).
//
// Wraps the pure-algorithm `stelnettts_lcs::lcs_dedup_prefix_count` with a
// segment-aware driver that knows how to walk the per-slice
// `std::vector<stelnettts_segment>` we get from the parakeet (and other
...
//
// The function is split into this header from `stelnettts_run.cpp` so the
// unit test in `tests/test-lcs-dedup-driver.cpp` can exercise it
// against synthetic segment fixtures without a model load.

#pragma once

#include "stelnettts_backend.h" // stelnettts_segment / _word / _token
...
namespace stelnettts_lcs {

// Flatten the last `tail_tokens` token ids of a single chunk's segments,
// preserving order. tail_tokens <= 0 → all tokens.
inline std::vector<int32_t> collect_tail_token_ids(const std::vector<stelnettts_segment>& segs, int tail_tokens) {
...
inline int drop_leading_tokens(std::vector<stelnettts_segment>& segs, int n) {
    if (n <= 0)
        return 0;
    int dropped = 0;

    auto rebuild_text_from_tokens = [](stelnettts_segment& s) {
        s.text.clear();
        for (const auto& t : s.tokens)
            s.text += t.text;
    ...
    while (dropped < n && !segs.empty()) {
        ...
        auto first_keep =
            std::find_if(s.words.begin(), s.words.end(), [&](const stelnettts_word& w) { return w.t0 > cutoff_cs - 1; });
...
inline void apply_lcs_chunk_dedup(std::vector<std::vector<stelnettts_segment>>& per_slice, int delay_tokens,
