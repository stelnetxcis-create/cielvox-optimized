// stelnettts_backend_moss_transcribe_diarize.cpp — adapter for MOSS-Transcribe-Diarize-0.9B.
//
...
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[moss-diarize]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        if (!p.hotwords.empty())
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        std::vector<stelnettts_segment> result;
