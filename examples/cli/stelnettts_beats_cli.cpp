// stelnettts_beats_cli.cpp — see stelnettts_beats_cli.h.
...
int stelnettts_run_beats(const whisper_params& params) {
    if (params.fname_inp.empty()) {
        fprintf(stderr, "stelnettts: --beats needs an input file (-f)\n");
        return 2;
    }

    const bool json = params.beats_format == "json";
    if (!json && !params.beats_format.empty() && params.beats_format != "text") {
        fprintf(stderr, "stelnettts: --beats-format: unknown format '%s' (expected text or json)\n",
    ...
    if (model.empty()) {
        fprintf(stderr, "stelnettts: --beats: could not resolve a model (try --auto-download).\n");
        return 2;
    }

    gguf_context* meta = core_gguf::open_metadata(model.c_str());
    if (!meta) {
        fprintf(stderr, "stelnettts: --beats: cannot open '%s'\n", model.c_str());
        return 2;
    }
    const std::string arch = core_gguf::kv_str(meta, "general.architecture", "");
    core_gguf::free_metadata(meta);
    if (arch != "beat-this") {
        fprintf(stderr, "stelnettts: --beats: '%s' is not a beat model (arch='%s'). Expected beat-this.\n", model.c_str(),
                arch.c_str());
        return 2;
    }
    ...
    if (!ctx) {
        fprintf(stderr, "stelnettts: --beats: failed to load '%s'\n", model.c_str());
        return 2;
    }
    ...
    for (const auto& fname : params.fname_inp) {
        ...
        if (n < 0) {
            fprintf(stderr, "stelnettts: --beats failed on '%s'\n", fname.c_str());
            rc = 1;
            continue;
        }
        ...
        if (!params.no_prints) {
            ...
            fprintf(stderr, "stelnettts: %s: %d beats (%d downbeats), %.1f BPM\n", fname.c_str(), n, n_db,
