// stelnettts_pitch_cli.cpp — see stelnettts_pitch_cli.h.
...
int stelnettts_run_pitch(const whisper_params& params) {
    if (params.fname_inp.empty()) {
        fprintf(stderr, "stelnettts: --pitch needs an input file (-f)\n");
        return 2;
    }

    const bool json = params.pitch_format == "json";
    if (!json && !params.pitch_format.empty() && params.pitch_format != "text") {
        fprintf(stderr, "stelnettts: --pitch-format: unknown format '%s' (expected text or json)\n",
    ...
    if (model.empty()) {
        fprintf(stderr, "stelnettts: --pitch: could not resolve a model (pass -m <gguf> or "
    ...
    if (!meta) {
        fprintf(stderr, "stelnettts: --pitch: cannot open '%s'\n", model.c_str());
        return 2;
    }
    ...
    if (arch != "crepe") {
        fprintf(stderr,
                "stelnettts: --pitch: '%s' is not a pitch model (arch='%s'). "
                "Expected crepe.\n",
    ...
    if (!ctx) {
        fprintf(stderr, "stelnettts: --pitch: failed to load '%s'\n", model.c_str());
        return 2;
    }
    ...
    for (const auto& fname : params.fname_inp) {
        ...
        if (n_max <= 0) {
            fprintf(stderr, "stelnettts: --pitch: no frames for '%s'\n", fname.c_str());
            rc = 1;
            continue;
        }
        ...
        if (n <= 0) {
            fprintf(stderr, "stelnettts: --pitch failed on '%s'\n", fname.c_str());
            rc = 1;
            continue;
        }
        ...
        if (!params.no_prints)
            fprintf(stderr, "stelnettts: %s: %d pitch frames (crepe-%s, hop %.1f ms)\n", fname.c_str(), n,
