// stelnettts_backend_moonshine_streaming.cpp — Moonshine Streaming ASR backend adapter.
...
public:
    ...
    bool init(const whisper_params& params) override {
        ...
        if (const char* g = stelnettts_env::get("STELNETTTS_MOONSHINE_STREAMING_GPU"))
            cp.use_gpu = (g[0] == '1');
        if (getenv("STELNETTTS_VERBOSE") || stelnettts_env::get("STELNETTTS_MOONSHINE_STREAMING_BENCH"))
            cp.verbosity = 2;
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (!params.language.empty() && params.language != "auto" && params.language != "en")
            fprintf(stderr, "stelnettts[moonshine-streaming]: English-only model; language='%s' ignored\n",
