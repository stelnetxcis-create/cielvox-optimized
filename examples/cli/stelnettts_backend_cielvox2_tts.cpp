// stelnettts_backend_cielvox2_tts.cpp — adapter for Qwen3-TTS-12Hz.
//
...
// as an extra companion when the user runs --model auto --backend cielvox2-tts).
static std::string discover_default_voice(const std::string& model_path) {
    const std::string dir = dir_of(model_path);
    static const char* candidates[] = {
        "cielvox2-tts-voice-default.gguf",
        "cielvox2-tts-voice-default-q8_0.gguf",
        "cielvox2-tts-voice-default-f16.gguf",
    };
    for (const char* name : candidates) {
        std::string p = dir + "/" + name;
        if (file_exists(p))
...
public:
    ...
    const char* name() const override { return "cielvox2-tts"; }
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[cielvox2-tts]: transcription is not supported by this backend\n");
        return {};
    }

    bool init(const whisper_params& p) override {
        cielvox2_tts_context_params cp = cielvox2_tts_context_default_params();
        cp.n_threads = p.n_threads;
        cp.verbosity = p.no_prints ? 0 : 1;
        cp.use_gpu = stelnettts_backend_should_use_gpu(p);
        cp.temperature = p.temperature;
        cp.seed = p.seed;
        ctx_ = cielvox2_tts_init_from_file(p.model.c_str(), cp);
        if (!ctx_) {
            fprintf(stderr, "stelnettts[cielvox2-tts]: failed to load talker '%s'\n", p.model.c_str());
            return false;
        }
        model_path_ = p.model;
        ...
        if (!codec_path.empty() && codec_path != "auto" && codec_path != "default") {
            codec_path = stelnettts_resolve_model_cli(codec_path, p.backend, p.no_prints, p.cache_dir, p.auto_download,
                                                    p.tts_codec_quant);
        } else {
            codec_path.clear();
        }
        ...
        if (codec_path.empty()) {
            fprintf(stderr, "stelnettts[cielvox2-tts]: no codec model found. Pass --codec-model PATH or place "
                            "cielvox2-tts-tokenizer-12hz.gguf next to the talker.\n");
            return false;
        }
        if (cielvox2_tts_set_codec_path(ctx_, codec_path.c_str()) != 0) {
            fprintf(stderr, "stelnettts[cielvox2-tts]: failed to load codec '%s'\n", codec_path.c_str());
            return false;
        }
        if (!p.no_prints)
            fprintf(stderr, "stelnettts[cielvox2-tts]: codec loaded from '%s'\n", codec_path.c_str());
        return true;
    }
    ...
    bool prepare_synthesis(const std::string& text, const whisper_params& params) {
        if (!ctx_ || text.empty())
            return false;
        cielvox2_tts_set_temperature(ctx_, params.temperature);
        cielvox2_tts_set_seed(ctx_, params.seed);

        // Output language hint. Map the ISO-639-1 code to the English name
        // cielvox2tts.codec_language_names is keyed by ("German", "Chinese",
        ...
        if (!out_lang.empty() && out_lang != "auto") {
            const std::string lang_name = stelnettts_iso_to_english_lang(out_lang);
            if (cielvox2_tts_set_language_by_name(ctx_, lang_name.c_str()) != 0 && !params.no_prints)
                fprintf(stderr,
                        "stelnettts[cielvox2-tts]: language '%s' not in the model's codec_language table; using auto\n",
                        lang_name.c_str());
        }
        ...
        if (!cielvox2_tts_is_voice_design(ctx_) && !cielvox2_tts_is_custom_voice(ctx_) && !params.tts_voice.empty() &&
            ...
            if (is_bare_name) {
                if (v.find("..") != std::string::npos || v.find('\0') != std::string::npos) {
                    fprintf(stderr, "stelnettts[cielvox2-tts]: voice name '%s' contains illegal characters (.. or NUL)\n",
                            v.c_str());
                    return false;
                }
        ...
        if (cielvox2_tts_is_voice_design(ctx_)) {
            voice_key = "vd:" + params.tts_instruct;
        } else if (cielvox2_tts_is_custom_voice(ctx_)) {
        ...
        if (voice_key != last_voice_key_) {
            if (cielvox2_tts_is_voice_design(ctx_)) {
                ...
                if (!params.tts_voice.empty() && !params.no_prints) {
                    fprintf(stderr, "stelnettts[cielvox2-tts]: VoiceDesign uses --instruct, not --voice — ignoring '%s'\n",
                            params.tts_voice.c_str());
                }
                if (params.tts_instruct.empty()) {
                    fprintf(stderr, "stelnettts[cielvox2-tts]: VoiceDesign requires --instruct \"<voice description>\"\n");
                    return false;
                }
                if (cielvox2_tts_set_instruct(ctx_, params.tts_instruct.c_str()) != 0) {
                    return false;
                }
                if (!params.no_prints) {
                    fprintf(stderr, "stelnettts[cielvox2-tts]: VoiceDesign instruct = \"%s\"\n",
                ...
                if (spk_name.empty() || ends_with_ci(spk_name, ".wav") || ends_with_ci(spk_name, ".gguf")) {
                    if (!spk_name.empty() && !params.no_prints) {
                        fprintf(stderr,
                                "stelnettts[cielvox2-tts]: CustomVoice expects a speaker NAME for --voice, "
                                "got '%s' — falling back to first speaker.\n",
                                spk_name.c_str());
                    }
                    const char* first = cielvox2_tts_get_speaker_name(ctx_, 0);
                    if (!first) {
                        fprintf(stderr,
                                "stelnettts[cielvox2-tts]: CustomVoice model has no speakers in the GGUF metadata\n");
                        return false;
                    }
                    spk_name = first;
                }
                if (cielvox2_tts_set_speaker_by_name(ctx_, spk_name.c_str()) != 0) {
                    return false;
                }
                if (!params.no_prints) {
                    fprintf(stderr, "stelnettts[cielvox2-tts]: CustomVoice speaker = '%s' (available: ", spk_name.c_str());
                    int n = cielvox2_tts_n_speakers(ctx_);
                    for (int i = 0; i < n; i++) {
                        fprintf(stderr, "%s%s", i ? ", " : "", cielvox2_tts_get_speaker_name(ctx_, i));
                    }
                    fprintf(stderr, ")\n");
                }
                // Style control for CustomVoice 1.7B (issue #91).
                // --instruct is optional; pass "" to clear any previous style.
                cielvox2_tts_set_cv_style_instruct(ctx_, params.tts_instruct.c_str());
                if (!params.tts_instruct.empty() && !params.no_prints) {
                    fprintf(stderr, "stelnettts[cielvox2-tts]: CustomVoice style instruct = \"%s\"\n",
                            params.tts_instruct.c_str());
                }
                ...
                if (ends_with_ci(v, ".wav")) {
                    if (resolved_ref_text.empty()) {
                        fprintf(stderr, "stelnettts[cielvox2-tts]: --voice is a WAV but --ref-text was not set. "
                                        "Provide the reference transcription so the talker can match it.\n");
                        return false;
                    }
                    if (cielvox2_tts_set_voice_prompt_with_text(ctx_, v.c_str(), resolved_ref_text.c_str()) != 0) {
                        fprintf(stderr, "stelnettts[cielvox2-tts]: failed to set voice prompt from '%s'\n", v.c_str());
                        return false;
                    }
                } else {
                    if (cielvox2_tts_load_voice_pack(ctx_, v.c_str()) != 0) {
                        fprintf(stderr, "stelnettts[cielvox2-tts]: failed to load voice pack '%s'\n", v.c_str());
                        return false;
                    }
                }
            } else {
                ...
                if (!def.empty()) {
                    if (!params.no_prints)
                        fprintf(stderr, "stelnettts[cielvox2-tts]: no --voice given, using default voice '%s'\n",
                                def.c_str());
                    if (cielvox2_tts_load_voice_pack(ctx_, def.c_str()) != 0) {
                        fprintf(stderr, "stelnettts[cielvox2-tts]: failed to load default voice '%s'\n", def.c_str());
                        return false;
                    }
                    ...
                    fprintf(stderr,
                            "stelnettts[cielvox2-tts]: no voice specified for Base model. Options:\n"
                            "  --voice ref.wav --ref-text \"...\"   — clone from a WAV file\n"
                            "  --voice voices/name.gguf           — load a baked voice pack\n"
                            "  --backend cielvox2-tts-customvoice    — use built-in fixed speakers (no --voice needed)\n"
                            "A default voice pack (cielvox2-tts-voice-default.gguf) can be auto-downloaded "
                            "with: stelnettts --model auto --backend cielvox2-tts\n");
                    return false;
                }
            }
    ...
    std::vector<float> synthesize(const std::string& text, const whisper_params& params) override {
        ...
        int n = 0;
        float* pcm = cielvox2_tts_synthesize(ctx_, text.c_str(), &n);
        if (!pcm || n <= 0)
            return {};
        std::vector<float> out(pcm, pcm + n);
        cielvox2_tts_pcm_free(pcm);
        return out;
    }

    void synthesize_streaming(const std::string& text, const whisper_params& params,
                              stelnettts_pcm_stream_callback cb) override {
        if (!prepare_synthesis(text, params))
            return;
        ...
        auto trampoline = [](const float* pcm, int n_samples, int is_final, void* user_data) {
            auto* fn = static_cast<stelnettts_pcm_stream_callback*>(user_data);
        ...
        // balance.
        //
        // Platform note (measured M1 Metal, cielvox2-tts-0.6b, 2026-06-20): the
        ...
        float* full = cielvox2_tts_synthesize_streaming(ctx_, text.c_str(), 8, 96, trampoline, &cb, &n);
        // The full buffer was already delivered via the callback chunks; free it.
        cielvox2_tts_pcm_free(full);
    }

    void shutdown() override {
        if (ctx_) {
            cielvox2_tts_free(ctx_);
            ctx_ = nullptr;
        }
...
private:
    bool is_base_ = false;
    cielvox2_tts_context* ctx_ = nullptr;
