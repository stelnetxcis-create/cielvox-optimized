// stelnettts_backend_zonos.cpp -- adapter for Zyphra/Zonos-v0.1-transformer
...
class ZonosBackend : public StelnetAsrBackend {
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[zonos]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        zp.use_gpu = stelnettts_backend_should_use_gpu(p);
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[zonos]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        ...
        if (!codec_path.empty()) {
            ...
            if (!p.no_prints) {
                fprintf(stderr, "stelnettts[zonos]: codec path = '%s'\n", codec_path.c_str());
            }
        } else if (!p.no_prints) {
            fprintf(stderr, "stelnettts[zonos]: no DAC codec found. Pass --codec-model PATH or place "
        ...
        if (!p.tts_voice.empty()) {
            ...
            if (v.size() > 4 && (v.substr(v.size() - 4) == ".wav" || v.substr(v.size() - 4) == ".mp3" ||
                ...
                if (zonos_tts_set_voice(ctx_, v.c_str()) != 0) {
                    fprintf(stderr, "stelnettts[zonos]: warning: failed to load voice from '%s'\n", v.c_str());
                }
            }
        }
...
std::unique_ptr<StelnetAsrBackend> stelnettts_make_zonos_backend() {
    return std::unique_ptr<StelnetAsrBackend>(new ZonosBackend());
}
