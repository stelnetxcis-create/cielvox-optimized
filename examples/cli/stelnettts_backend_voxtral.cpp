// stelnettts_backend_voxtral.cpp — adapter for Mistral Voxtral-Mini-3B-2507.
//
// The pipeline logic (mel -> encoder -> prompt splice -> KV decode) is in
// stelnettts_llm_pipeline.h. This file only provides the voxtral-specific
...
#include "stelnettts_llm_pipeline.h"
    ...
    static std::string build_suffix(const whisper_params& p) {
        ...
        if (p.translate) {
            ...
            const std::string tgt =
                p.target_lang.empty() ? std::string("English") : stelnettts_iso_to_english_lang(p.target_lang);
...
public:
    ...
    uint32_t capabilities() const override {
        ...
        // AR decode). The stelnettts_run.cpp 30-second auto-chunk gate
    ...
    bool init(const whisper_params& p) override {
        ctx_ = VoxtralOps::init(p.model.c_str(), p.n_threads, p.no_prints ? 0 : 1, stelnettts_backend_should_use_gpu(p));
        if (!ctx_) {
            fprintf(stderr, "stelnettts[voxtral]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        return stelnettts_run_voxtral_style_pipeline_streamed<VoxtralOps>(ctx_, samples, n_samples, t_offset_cs, params);
    }
    ...
    void transcribe_streaming(const float* samples, int n_samples, int64_t t_offset_cs, const whisper_params& params,
        ...
        stelnettts_run_voxtral_style_pipeline_streamed_cb<VoxtralOps>(ctx_, samples, n_samples, params, on_text);
    }
