// stelnettts_tts_ref_text.h — resolve the reference transcript for a voice clone.
//
...
namespace stelnettts_ref_text {

// Transcribe 16 kHz mono PCM with `asr_backend`. `log_tag` is the caller's
// "stelnettts[<backend>]" prefix so diagnostics name the backend the user asked
...
inline std::string transcribe(const std::vector<float>& pcm_16k, const whisper_params& p,
    ...
    auto backend = stelnettts_create_backend(asr_backend);
    if (!backend) {
        if (!p.no_prints)
    ...
    asr_p.model = stelnettts_resolve_model_cli("auto", asr_backend, /*quiet=*/true,
...
// Disable the cache with STELNETTTS_TTS_REF_CACHE=0.
inline std::string resolve_cached(const std::string& voice_path, const std::vector<float>& pcm_16k,
    ...
    const std::string cache_path = stelnettts_ref_cache::path_for(voice_path, cache_suffix);
    const bool cache_enabled = !stelnettts_ref_cache::disabled();

    if (cache_enabled) {
        std::vector<uint32_t> shape;
        std::vector<uint8_t> payload;
        if (stelnettts_ref_cache::load(cache_path, voice_path, cache_suffix, shape, payload)) {
    ...
    if (!text.empty() && cache_enabled)
        stelnettts_ref_cache::save(cache_path, cache_suffix, {(uint32_t)text.size()}, text.data(), text.size());
    return text;
}
