// stelnettts_backend_lfm2_audio.cpp — adapter for LiquidAI LFM2.5-Audio.
//
...
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        lp.use_gpu = stelnettts_backend_should_use_gpu(p);

        ctx_ = lfm2_audio_init_from_file(p.model.c_str(), lp);
        if (!ctx_) {
            fprintf(stderr, "stelnettts[lfm2-audio]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
