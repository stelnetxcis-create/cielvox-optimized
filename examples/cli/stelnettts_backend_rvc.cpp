// stelnettts_backend_rvc.cpp — RVC voice-conversion backend shim.
//
...
// which StelnetTTS does not produce. The consumer owns the content encoder, so a
// standalone command line has nothing to feed it. The real surface is the
// session C ABI (stelnettts_session_convert*).
//
...
public:
    ...
    bool init(const whisper_params&) override {
        fprintf(stderr, "stelnettts: rvc-svc is a VOICE CONVERSION model and has no CLI verb.\n"
                        "  Its input is ContentVec features, which StelnetTTS does not produce — the\n"
                        "  caller owns the content encoder. Use the session C ABI instead:\n"
                        "    stelnettts_session_convert(s, content, n_frames, f0_hz, speaker_id, NULL, NULL)\n"
