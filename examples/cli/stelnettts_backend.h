// stelnettts_backend.h — abstract backend interface for the unified stelnettts CLI.
//
// Each model in src/ (parakeet, canary, cohere, cielvox2-asr, voxtral, voxtral4b,
// granite_speech) is wrapped by a backend that converts its native result type
// into the common stelnettts_segment vector. The whisper backend is a thin
...
// or dispatches to stelnettts_run_backend() which drives the pipeline:
//   load audio -> VAD slice -> backend->transcribe() -> write outputs.

#pragma once

#include "stelnettts_speaker_identity_models.h" // declared_speaker_identity() — the researched verdicts
...
// introducing a parallel stelnettts_params, so users keep the same interface
// they already know from stelnettts.
...
struct stelnettts_token_alt {
    std::string text;
    float prob = 0.0f; // probability [0,1]
    int32_t id = -1;
};

struct stelnettts_token {
    std::string text;
    ...
    std::vector<stelnettts_token_alt> alts; // top-N alternatives (--alt mode)
};

struct stelnettts_word {
    std::string text;
    int64_t t0 = 0; // centiseconds, absolute
    int64_t t1 = 0;
};

struct stelnettts_segment {
    std::string text;
    int64_t t0 = 0; // centiseconds, absolute
    int64_t t1 = 0;
    ...
    int chunk_id = -1;
    std::vector<stelnettts_word> words;   // may be empty
    std::vector<stelnettts_token> tokens; // may be empty
...
enum stelnettts_capability : uint32_t {
    ...
    CAP_INTERNAL_CHUNKING = 1u << 20,   // backend handles its own long-audio chunking internally
                                        ...
                                        // Skip the stelnettts_run.cpp auto-chunk fallback for these.
...
public:
    ...
    // Bitmask of stelnettts_capability flags.
    ...
    // The CLI loads audio at this rate via stelnettts_audio_load_at_rate,
    ...
    // offset).
    virtual std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
    ...
    virtual const stelnettts_ctc_logits* last_ctc_logits() const { return nullptr; }
    ...
    virtual std::vector<stelnettts_segment> decode_slice(encoded_slice /*enc*/, int64_t /*t_offset_cs*/,
    ...
    virtual void repair_slice(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
                              std::vector<stelnettts_segment>& /*segs*/, const whisper_params& /*params*/) {}
    ...
    virtual std::vector<stelnettts_segment> transcribe_stereo(const float* left_samples, const float* right_samples,
    ...
    using stelnettts_pcm_stream_callback = std::function<void(const float* pcm, int n_samples, bool is_final)>;
    ...
    virtual void synthesize_streaming(const std::string& text, const whisper_params& p,
                                      stelnettts_pcm_stream_callback cb) {
    ...
    //
    // The default is a lookup in stelnettts_speaker_identity_models.h, keyed on
    ...
    // stelnettts_speaker_identity.h for why those are different duties.
    //
    // A pack or bank entry that declares its own stelnettts.voice.speaker_identity
    // outranks this, and --speaker-identity outranks both.
    virtual stelnettts_voice::SpeakerIdentity declared_speaker_identity(const std::string& model_path) const {
        return stelnettts_voice::identity_for_model(name(), model_path);
    }
    ...
    // to hand the path over. See stelnettts_voice_provenance.h.
    //
    ...
    // TTS backends (kokoro, cielvox2-tts, vibevoice, chatterbox, orpheus, indextts)
    ...
    using stelnettts_stream_callback = std::function<void(const std::string& partial_text, bool is_final)>;
    ...
    virtual void transcribe_streaming(const float* samples, int n_samples, int64_t t_offset_cs,
                                      const whisper_params& params, stelnettts_stream_callback on_text) {
        (void)samples;
        (void)n_samples;
        (void)t_offset_cs;
        (void)params;
...
int stelnettts_run_backend(const whisper_params& params);
