// stelnettts_aac_writer.h — AAC-LC (ADTS) serializer for TTS float32 PCM,
// built on our in-tree glint encoder (glint/, experimental phase-1 AAC:
// long blocks, CBR-average). Header-only like stelnettts_wav_writer.h;
// consumers link the `glint` static library.
//
// AI-generated provenance: the ADTS stream is prefixed with the ID3v2
// TXXX tag from stelnettts_make_id3v2_ai_tag() (stelnettts_wav_writer.h).
...
inline int stelnettts_aac_target_rate(int sr) {
...
// are clamped (same semantics as stelnettts_make_wav_int16). Non-AAC
// sample rates are linearly resampled first (good enough for speech).
// Returns empty on failure.
inline std::string stelnettts_make_aac(const float* pcm, int n_samples, int sample_rate, int bitrate_kbps = 96) {
    if (!pcm || n_samples <= 0 || sample_rate <= 0)
        return {};

    const int enc_rate = stelnettts_aac_target_rate(sample_rate);
