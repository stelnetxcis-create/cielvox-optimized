// stelnettts_backend_funasr.cpp — FunAudioLLM/Fun-ASR-Nano-2512 adapter.
//
...
// UTF-8 string into a single stelnettts_segment.
...
public:
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[funasr]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
    ...
    std::vector<stelnettts_segment> transcribe(const float* samples, int n_samples, int64_t t_offset_cs,
        ...
        if (!r || !r->text) {
            ...
            fprintf(stderr, "stelnettts[funasr]: transcribe failed\n");
            return out;
        }
