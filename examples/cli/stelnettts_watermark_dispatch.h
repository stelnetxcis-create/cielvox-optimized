// stelnettts_watermark_dispatch.h — unified watermark dispatch.
//
...
namespace stelnettts_wm_dispatch {
...
// The STELNETTTS_NO_WATERMARK env var is an equivalent opt-out (both are honored;
...
// against --no-watermark / STELNETTTS_NO_WATERMARK. The CLI sets this for outputs
...
inline bool is_disabled(bool force = false) {
    if (force || forced_flag())
        return false;
    return disabled_flag() || std::getenv("STELNETTTS_NO_WATERMARK") != nullptr;
}
...
inline bool init(const std::string& model_path) {
    ...
    if (!get_ctx()) {
        fprintf(stderr,
                "stelnettts: warning: could not load AudioSeal model '%s'; "
...
// CLI flag or the STELNETTTS_NO_WATERMARK env var (equivalent opt-outs). Either
...
// all. That is the watertight floor; see stelnettts_enforce_cli_watermark_floor()
...
inline void embed(float* pcm, int n_samples, int sample_rate = 24000, bool force = false) {
    if (is_disabled(force)) {
        ...
        if (!warned) {
            warned = true;
            fprintf(stderr, "stelnettts: warning: watermarking disabled. "
    ...
    stelnettts_watermark_embed_impl(pcm, n_samples);
}
