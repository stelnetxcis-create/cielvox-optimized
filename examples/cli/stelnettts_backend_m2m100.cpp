// stelnettts_backend_m2m100.cpp — adapter for M2M-100 multilingual translation.
//
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[m2m100]: transcription is not supported — this is a translation backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[m2m100]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        return true;
    }
    ...
    // Translation: called from stelnettts_run.cpp when --translate-model is active
