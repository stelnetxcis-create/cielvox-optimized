// stelnettts_diff.cpp — implementation of the ground-truth diff harness.
// See stelnettts_diff.h for the interface contract.
...
bool Ref::load(const std::string& path) {
    ...
    if (!impl_->backend) {
        fprintf(stderr, "stelnettts_diff: failed to init CPU backend\n");
        delete impl_;
        impl_ = nullptr;
        return false;
    }

    if (!core_gguf::load_weights(path.c_str(), impl_->backend, "stelnettts_diff", impl_->wl)) {
        fprintf(stderr, "stelnettts_diff: failed to load '%s'\n", path.c_str());
        delete impl_;
        impl_ = nullptr;
        return false;
    }

    // Read the string metadata (stelnettts.ref.*) directly from the GGUF
    ...
    if (gguf_context* gctx = core_gguf::open_metadata(path.c_str())) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (!k || strncmp(k, "stelnettts.ref.", 13) != 0)
                continue;
...
static const std::vector<float>& cache_f32(Ref::Impl* impl, const std::string& name) {
    ...
    if (t->type == GGML_TYPE_F32) {
        ...
        fprintf(stderr, "stelnettts_diff: tensor '%s' has type %s — Ref loader supports F32/F16/I32 only\n", name.c_str(),
...
Report Ref::compare(const std::string& name, const float* data, size_t n_elem, CompareMode mode) const {
    ...
    if (ref.empty()) {
        fprintf(stderr, "stelnettts_diff: reference '%s' is empty or non-F32\n", name.c_str());
        return r;
    }
