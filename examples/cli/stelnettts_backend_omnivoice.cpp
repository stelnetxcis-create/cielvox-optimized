// stelnettts_backend_omnivoice.cpp — adapter for k2-fsa/OmniVoice TTS.
//
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float* /*samples*/, int /*n_samples*/, int64_t /*t_offset_cs*/,
        ...
        fprintf(stderr, "stelnettts[omnivoice]: transcription is not supported by this backend\n");
        return {};
    }
    ...
    bool init(const whisper_params& p) override {
        ...
        if (!ctx_) {
            fprintf(stderr, "stelnettts[omnivoice]: failed to load model '%s'\n", p.model.c_str());
            return false;
        }
        ...
        if (!tok_path.empty()) {
            if (omnivoice_set_tokenizer_path(ctx_, tok_path.c_str()) != 0) {
                fprintf(stderr, "stelnettts[omnivoice]: failed to load tokenizer '%s'\n", tok_path.c_str());
            } else if (!p.no_prints) {
                fprintf(stderr, "stelnettts[omnivoice]: tokenizer loaded from '%s'\n", tok_path.c_str());
            }
        } else if (!p.no_prints) {
            fprintf(stderr, "stelnettts[omnivoice]: no audio tokenizer found. Pass --codec-model PATH or place "
                            "omnivoice-tokenizer.gguf next to the model.\n");
            fprintf(stderr, "stelnettts[omnivoice]: code generation will work; audio decode requires the tokenizer.\n");
        }

        // Diff-harness: OMNIVOICE_ENCODE_DIFF=<ref.gguf> runs the encode-path
        // stage diff and exits (#254 voice-clone port validation).
        if (const char* rp = stelnettts_env::get("STELNETTTS_OMNIVOICE_ENCODE_DIFF")) {
        ...
        if (!p.tts_instruct.empty()) {
            if (omnivoice_set_instruct(ctx_, p.tts_instruct.c_str()) != 0) {
                fprintf(stderr, "stelnettts[omnivoice]: --instruct was rejected (see above)\n");
                return false;
            }
        }
    ...
    std::vector<float> synthesize(const std::string& text, const whisper_params& params) override {
        ...
        // Same shape as stelnettts_backend_cosyvoice3.cpp.
        ...
        if (codes && n_codes > 0) {
            if (!params.no_prints) {
                fprintf(stderr, "stelnettts[omnivoice]: generated %d codes (audio decode requires tokenizer)\n", n_codes);
            }
...
private:
    ...
    void prepare_voice(const whisper_params& params) {
        ...
        if (omnivoice_set_voice_prompt(ctx_, params.tts_voice.c_str(), ref_text.c_str()) != 0) {
            fprintf(stderr, "stelnettts[omnivoice]: failed to set voice prompt '%s'\n", params.tts_voice.c_str());
            last_voice_.clear();
        }
    }
