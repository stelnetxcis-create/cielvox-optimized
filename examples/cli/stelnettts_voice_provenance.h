// stelnettts_voice_provenance.h — the IO half of the voice-clone gate.
//
...
// pure predicate in stelnettts_voice_clone_policy.h.
//
...
// filesystem and no link against stelnettts-core. Everything that needs a file
// lives here.

#pragma once

#include "core/gguf_loader.h"
#include "stelnettts_speaker_identity_models.h" // identity_for_voice_pack — legacy pack fallback
...
struct PackProvenance {
    // The pack carries the baker's stelnettts.voice.cloned_from_recording stamp.
    ...
    // Whose voice this is (stelnettts.voice.speaker_identity). INDEPENDENT of
...
inline stelnettts_voice::BankFacts read_bank_provenance(const std::string& bank_path, const std::string& voice_name) {
    stelnettts_voice::BankFacts f;
...
// file-name matching in stelnettts_speaker_identity_models.h: a checkpoint that
// carries `stelnettts.voice.speaker_identity` says so itself and survives being
