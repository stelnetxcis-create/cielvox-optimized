// cielvox2-asr-enc-dump — dump the Qwen3-ASR audio-encoder output for a GGUF.
//
...
//
// Usage: cielvox2-asr-enc-dump model.gguf audio.wav out.bin
...
int main(int argc, char** argv) {
    ...
    int sr = 0;
    if (!stelnettts::core::read_wav_mono_pcm16(audio_path, pcm, sr) || pcm.empty()) {
    ...
    cielvox2_asr_context* ctx = cielvox2_asr_init_from_file(model_path, p);
    if (!ctx) {
    ...
    float* mel = cielvox2_asr_compute_mel(ctx, pcm.data(), (int)pcm.size(), &n_mels, &T_mel);
    if (!mel) {
