...
In your vimrc, set the path of your stelnettts directory and optionally add some keybinds.

```vim
let g:whisper_dir = "~/stelnettts"
...
`g:whisper_dir`  
A full path to the stelnettts repo. It can be expanded in the definition like so:
```vim
let g:whisper_dir = expand("~/stelnettts/")
```
(The STELNETTTS_CPP_HOME environment variable is also checked for users of the existing stelnettts.nvim script)
