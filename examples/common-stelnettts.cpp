...
// `src/stelnettts_audio.cpp` already defines `MINIAUDIO_IMPLEMENTATION`
// for the stelnettts library (full version, with device IO for mic
// capture). Defining it here too produced duplicate `ma_*` symbols
// in both `common.lib` and `stelnettts.lib`, causing MSVC LNK2005 on
// stelnettts-cli (release.yml `build-windows-cpu` / `-legacy`) and
...
// here; consumers of `common-stelnettts.cpp` already link against
// `stelnettts` (which carries the symbols), so `ma_decoder_*` etc.
...
// stelnettts_popen wrapper widens the command to wchar_t and uses
...
#include "cli/stelnettts_popen.h"
static bool ffmpeg_subprocess_decode(const std::string& fname, std::vector<float>& pcmf32) {
    ...
    FILE* pipe = stelnettts::stelnettts_popen(cmd, "rb");
    if (!pipe) {
        fprintf(stderr, "stelnettts: ffmpeg popen failed: %s\n", strerror(errno));
        return false;
    }
    ...
    int ret = stelnettts::stelnettts_pclose(pipe);
    if (ret != 0 || buf.empty())
        return false;
...
bool read_audio_data(const std::string& fname, std::vector<float>& pcmf32, std::vector<std::vector<float>>& pcmf32s,
    ...
    const int effective_rate = (target_rate > 0) ? target_rate : STELNETTTS_SAMPLE_RATE;
    ...
    if (fname == "-") {
        ...
        // decode via the stelnettts_audio_load C ABI first — it covers these
        ...
        if (stereo) {
            ...
            float* R = nullptr;
            int fr = 0, sr = 0, ch = 0;
            if (stelnettts_audio_load_stereo(fname.c_str(), &L, &R, &fr, &sr, &ch) == 0) {
                ...
                stelnettts_audio_free(L);
                stelnettts_audio_free(R);
                return true;
            }
        } else {
            float* buf = nullptr;
            int fr = 0, sr = 0;
            if (stelnettts_audio_load_at_rate(fname.c_str(), effective_rate, &buf, &fr, &sr) == 0) {
                pcmf32.assign(buf, buf + fr);
                stelnettts_audio_free(buf);
                return true;
            }
        }
#if defined(STELNETTTS_FFMPEG)
...
// src/stelnettts_audio.cpp), the Vorbis backend isn't compiled into
// common-stelnettts.cpp at all, and re-expanding stb_vorbis.c here
// produced ld multiple-definition errors against the same set of
// symbols already provided by stelnettts_audio.cpp on Linux/macOS
// (Release / build-linux + build-macos failures on v0.6.1 release.yml).
// stelnettts_audio.cpp's `#undef STB_VORBIS_HEADER_ONLY; #include
