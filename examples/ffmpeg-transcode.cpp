...
static void convert_frame(struct SwrContext* swr, AVCodecContext* codec, AVFrame* frame, s16** data, int* size,
                          bool flush) {
    int nr_samples;
    s64 delay;
    u8* buffer;
    ...
    nr_samples = av_rescale_rnd(delay + in_samples, WAVE_SAMPLE_RATE, codec->sample_rate, AV_ROUND_UP);
    if (nr_samples <= 0)
        return;
	 ...
	 * conversion buffers...
	 */
    nr_samples = swr_convert(swr, &buffer, nr_samples, (frame && !flush) ? (const u8**)frame->data : NULL, in_samples);
