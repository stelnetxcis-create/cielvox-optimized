// stelnettts-quantize — GGUF tensor re-quantization tool.
//
...
// here when the per-model CLIs were consolidated into stelnettts.
...
// llama.cpp's `llama-imatrix`. Ported from StelnetEmbed tools/quantize.cpp.
//
...
// that emits this file — is not yet in StelnetTTS; see PLAN.md §llama.cpp
// comparison Tier-1. This is the consumer side, ported from StelnetEmbed.)
static std::map<std::string, std::vector<float>> g_imatrix;

static bool stelnettts_load_imatrix(const std::string& path) {
...
static ggml_type stelnettts_parse_type_name(const std::string& s) {
...
static ggml_type stelnettts_row_fit(ggml_type want, int64_t ncols) {
...
static bool stelnettts_model_quantize(const std::string& fname_inp, const std::string& fname_out, ggml_ftype ftype) {
    ...
    case GGML_FTYPE_MOSTLY_Q6_K:
        ...
        break;
    // IQ4_NL / IQ4_XS: 4-bit non-linear codebook quants. A/B on StelnetEmbed
    ...
    const char* env_enc_f16 = std::getenv("STELNETTTS_GRANITE_ENC_F16");
    ...
    const char* env_quant_all = std::getenv("STELNETTTS_GRANITE_QUANT_ALL");
    ...
    // STELNETTTS_OMNIASR_QUANT_ALL=1 to ship a smaller variant at the
    ...
    // by sweeping STELNETTTS_OMNIASR_KEEP_F16_HEAD ∈ {0, 4, 8, 12, 16} on
    ...
    if (is_omniasr_ctc) {
        ...
        if (const char* env_h = std::getenv("STELNETTTS_OMNIASR_KEEP_F16_HEAD"))
            omniasr_keep_head = std::max(0, atoi(env_h));
        if (const char* env_t = std::getenv("STELNETTTS_OMNIASR_KEEP_F16_TAIL"))
            omniasr_keep_tail = std::max(0, atoi(env_t));
    }
    const char* env_omniasr_all = std::getenv("STELNETTTS_OMNIASR_QUANT_ALL");
    ...
    if (is_omniasr_ctc && !omniasr_quant_all && (omniasr_keep_head + omniasr_keep_tail) > 0) {
        if (omniasr_keep_head > 0 && omniasr_keep_tail == 0) {
            printf("%s: omniasr-ctc — keeping enc.0-%d (head) at F16 to "
                   "prevent CTC drift (STELNETTTS_OMNIASR_QUANT_ALL=1 to override)\n",
    ...
    const bool is_cielvox2_tts = (arch.find("cielvox2tts") != std::string::npos);
    ...
    // with STELNETTTS_VIBEVOICE_QUANT_ALL=1.
    const bool is_vibevoice = (arch.find("vibevoice") != std::string::npos);
    const char* env_vv_all = std::getenv("STELNETTTS_VIBEVOICE_QUANT_ALL");
    ...
    // carve-outs at ~half their size cost. STELNETTTS_CIELVOX2ASR_QUANT_AUDIO=1
    // quantizes audio.* at the target type anyway (A/B gate).
    const bool is_cielvox2_asr = (arch == "cielvox2asr" || arch == "cielvox2-asr");
    const bool cielvox2asr_quant_audio = []() {
        const char* e = std::getenv("STELNETTTS_CIELVOX2ASR_QUANT_AUDIO");
        return e && *e && *e != '0';
    }();
    ...
    const char* env_tada_all = std::getenv("STELNETTTS_TADA_QUANT_ALL");
    ...
    if (is_tada) {
        ...
        if (const char* env_h = std::getenv("STELNETTTS_TADA_KEEP_F16_HEAD"))
            tada_keep_head = std::max(0, atoi(env_h));
        if (const char* env_t = std::getenv("STELNETTTS_TADA_KEEP_F16_TAIL"))
        ...
        if (!tada_quant_all && (tada_keep_head > 0 || tada_keep_tail > 0)) {
            ...
            if (tada_keep_head > 0 && tada_keep_tail > 0) {
                printf("%s: tada-tts - keeping talker.blk.0-%d (head) + talker.blk.%d-%d (tail) at source precision "
                       "(override with STELNETTTS_TADA_KEEP_F16_HEAD/TAIL, full quant with STELNETTTS_TADA_QUANT_ALL=1)\n",
    ...
    // models (n_tdt_durations==0). Override with STELNETTTS_PARAKEET_QUANT_ALL=1.
    ...
    // STELNETTTS_GIGAAM_QUANT_ALL=1.
    const bool is_gigaam = (arch == "gigaam");
    const char* env_gigaam_all = std::getenv("STELNETTTS_GIGAAM_QUANT_ALL");
    ...
    if (is_gigaam && !gigaam_quant_all) {
        printf("%s: gigaam — keeping joint.*, decoder.*, head.ctc.* and encoder.pre.* at source "
               "precision (override with STELNETTTS_GIGAAM_QUANT_ALL=1)\n",
               __func__);
    }
    ...
    const char* env_t5_keep = std::getenv("STELNETTTS_T5_KEEP_EMBED");
    ...
    if (t5_keep_embed) {
        printf("%s: t5 — keeping shared.embed.* and lm_head.* at source precision "
               "(STELNETTTS_T5_KEEP_EMBED=1; measured BIGGER and no better on madlad400 — "
    ...
    const char* env_parakeet_all = std::getenv("STELNETTTS_PARAKEET_QUANT_ALL");
    ...
    if (is_parakeet && parakeet_is_rnnt && !parakeet_quant_all) {
        printf("%s: parakeet RNNT — keeping joint.* and decoder.embed.* at source precision "
               "(override with STELNETTTS_PARAKEET_QUANT_ALL=1)\n",
               __func__);
    }
    ...
    // sampling/argmax-critical). STELNETTTS_QUANT_LMHEAD=1 lets the caller quantize
    ...
    const bool allow_lmhead = []() {
        const char* e = std::getenv("STELNETTTS_QUANT_LMHEAD");
        return e && *e && *e != '0';
    }();
    ...
    for (int i = 0; i < n_tensors; i++) {
        ...
        bool should_quantize =
            ...
            !(is_cielvox2_tts && (sname.find("speaker.") == 0 || sname.find("code_pred.token_embd") == 0 ||
        ...
        // Qwen3-ASR audio-tower Q8_0 floor (see is_cielvox2_asr note above).
        ...
        if (is_cielvox2_asr && !cielvox2asr_quant_audio && sname.find("audio.") == 0 && should_quantize &&
        ...
        for (size_t r = 0; r < g_type_overrides.size(); r++) {
            ...
            if (ggml_is_quantized(ov)) {
                ...
                ggml_type fit = stelnettts_row_fit(ov, ncols);
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (a == "--imatrix") {
            ...
            ggml_type ot = stelnettts_parse_type_name(tn);
    ...
    if (!imatrix_path.empty()) {
        stelnettts_load_imatrix(imatrix_path); // non-fatal: falls back to unweighted if empty
    }

    if (!stelnettts_model_quantize(fname_inp, fname_out, ftype)) {
