// StelnetTTS — C-ABI consumed by every StelnetTTS consumer: the CLI in
// `examples/cli/`, the Dart FFI binding in `flutter/stelnettts/`, the Python
// ctypes binding in `python/stelnettts/`, and the Rust `stelnettts-sys` crate.
...
// and also expose the higher-level StelnetTTS session/VAD/diarize surface.
//
// Every symbol here is plain C linkage, prefixed `stelnettts_` so it can't
// collide with upstream whisper.h identifiers. Keep signatures stable once
// published — these are part of StelnetTTS's published ABI contract shared
...
#include "stelnettts_vad.h"     // VAD slicing + stitching (shared with CLI)
#include "stelnettts_diarize.h" // Speaker diarization (shared with CLI)
#include "stelnettts_lid.h"     // Language identification (shared with CLI)
#if defined(STELNETTTS_RNNOISE)
#include "stelnettts_enhance.h" // RNNoise audio enhancement (shared with CLI)
#endif
#include "text_lid_dispatch.h"        // Text-LID backend-agnostic façade (CLD3 + fastText)
#include "stelnettts_aligner.h"         // CTC / forced-aligner word timings (shared with CLI)
#include "stelnettts_cache.h"           // HF download + filesystem cache (shared with CLI)
#include "stelnettts_model_registry.h"  // Known-model lookup (shared with CLI)
#include "stelnettts_punc_model.h"      // shared --punc-model alias resolution (CLI/server/C-ABI parity)
...
#include "core/stelnettts_c2pa.h"       // C2PA Content Credentials signing (shared with CLI; #260)
#include "core/stelnettts_wav_writer.h" // WAV container + AI-provenance INFO tag (interop floor)
...
#include "stelnettts_diarize_internal.h"
#include "pyannote_seg.h"

// CA_EXPORT decorates every C-ABI definition in this file. It MUST expand
// to the same linkage attributes that `STELNETTTS_API` (from
// `include/stelnettts.h`) puts on the public declaration, otherwise MSVC
...
// STELNETTTS_API to empty; shared builds set __declspec(dllexport) on the
...
#define CA_EXPORT extern "C" STELNETTTS_API
...
// stelnettts_get_progress(). The whisper progress_callback writes here;
// Dart reads it at leisure.

static std::atomic<int> g_progress{-1};

CA_EXPORT int stelnettts_get_progress(void) {
    return g_progress.load(std::memory_order_relaxed);
}

CA_EXPORT void stelnettts_reset_progress(void) {
...
CA_EXPORT void stelnettts_params_set_language(whisper_full_params* p, const char* lang) {
    if (p)
        p->language = lang; // caller must keep the string alive
}
CA_EXPORT void stelnettts_params_set_translate(whisper_full_params* p, int v) {
    if (p)
        p->translate = v != 0;
}
CA_EXPORT void stelnettts_params_set_detect_language(whisper_full_params* p, int v) {
    if (p)
        p->detect_language = v != 0;
}
CA_EXPORT void stelnettts_params_set_token_timestamps(whisper_full_params* p, int v) {
    if (p)
        p->token_timestamps = v != 0;
}
CA_EXPORT void stelnettts_params_set_n_threads(whisper_full_params* p, int n) {
    if (p)
        p->n_threads = n;
}
CA_EXPORT void stelnettts_params_set_max_len(whisper_full_params* p, int n) {
    if (p)
        p->max_len = n;
}
CA_EXPORT void stelnettts_params_set_best_of(whisper_full_params* p, int n) {
    if (p && n > 0)
        p->greedy.best_of = n;
}
CA_EXPORT void stelnettts_params_set_split_on_word(whisper_full_params* p, int v) {
    if (p)
        p->split_on_word = v != 0;
}
CA_EXPORT void stelnettts_params_set_no_context(whisper_full_params* p, int v) {
    if (p)
        p->no_context = v != 0;
}
CA_EXPORT void stelnettts_params_set_single_segment(whisper_full_params* p, int v) {
    if (p)
        p->single_segment = v != 0;
}
CA_EXPORT void stelnettts_params_set_print_realtime(whisper_full_params* p, int v) {
    if (p)
        p->print_realtime = v != 0;
}
CA_EXPORT void stelnettts_params_set_print_progress(whisper_full_params* p, int v) {
    if (p)
        p->print_progress = v != 0;
}
CA_EXPORT void stelnettts_params_set_print_timestamps(whisper_full_params* p, int v) {
    if (p)
        p->print_timestamps = v != 0;
}
CA_EXPORT void stelnettts_params_set_print_special(whisper_full_params* p, int v) {
    if (p)
        p->print_special = v != 0;
}
CA_EXPORT void stelnettts_params_set_suppress_blank(whisper_full_params* p, int v) {
    if (p)
        p->suppress_blank = v != 0;
}
CA_EXPORT void stelnettts_params_set_temperature(whisper_full_params* p, float t) {
    if (p)
        p->temperature = t;
}
CA_EXPORT void stelnettts_params_set_max_tokens(whisper_full_params* p, int n) {
    if (p)
        p->max_tokens = n > 0 ? n : 0;
}
CA_EXPORT void stelnettts_params_set_initial_prompt(whisper_full_params* p, const char* prompt) {
    if (p)
        p->initial_prompt = prompt; // caller owns the string
}

// VAD (stelnettts built-in Silero pipeline). When enabled, whisper_full
...
CA_EXPORT void stelnettts_params_set_vad(whisper_full_params* p, int v) {
    if (p)
        p->vad = v != 0;
}
CA_EXPORT void stelnettts_params_set_vad_model_path(whisper_full_params* p, const char* path) {
    if (p)
        p->vad_model_path = path; // caller owns the string
}
CA_EXPORT void stelnettts_params_set_vad_threshold(whisper_full_params* p, float t) {
    if (p)
        p->vad_params.threshold = t;
}
CA_EXPORT void stelnettts_params_set_vad_min_speech_ms(whisper_full_params* p, int ms) {
    if (p)
        p->vad_params.min_speech_duration_ms = ms;
}
CA_EXPORT void stelnettts_params_set_vad_min_silence_ms(whisper_full_params* p, int ms) {
    if (p)
...
CA_EXPORT void stelnettts_params_set_tdrz(whisper_full_params* p, int v) {
    if (p)
        p->tdrz_enable = v != 0;
}
...
CA_EXPORT void stelnettts_ctx_params_set_dtw(whisper_context_params* p, bool enable,
...
CA_EXPORT int64_t stelnettts_token_t0(whisper_context* ctx, int i_seg, int i_tok) {
    if (!ctx)
        return 0;
    return whisper_full_get_token_data(ctx, i_seg, i_tok).t0;
}
CA_EXPORT int64_t stelnettts_token_t1(whisper_context* ctx, int i_seg, int i_tok) {
    if (!ctx)
        return 0;
    return whisper_full_get_token_data(ctx, i_seg, i_tok).t1;
}
CA_EXPORT float stelnettts_token_p(whisper_context* ctx, int i_seg, int i_tok) {
    if (!ctx)
        return 0.0f;
    return whisper_full_get_token_data(ctx, i_seg, i_tok).p;
}

CA_EXPORT int64_t stelnettts_token_dtw_t(whisper_context* ctx, int i_segment, int i_token) {
    if (!ctx)
        return 0;
...
// `wparams.alt_n > 0` (set via stelnettts_params_set_alt_n), the decoder
...
CA_EXPORT void stelnettts_params_set_alt_n(whisper_full_params* p, int n) {
    if (p)
        p->alt_n = n < 0 ? 0 : (n > 32 ? 32 : n); // sanity clamp; UI caps at 5
}

CA_EXPORT int stelnettts_token_n_alts(whisper_context* ctx, int i_seg, int i_tok) {
    if (!ctx)
        return 0;
    return whisper_full_get_token_n_alts(ctx, i_seg, i_tok);
}

CA_EXPORT int32_t stelnettts_token_alt_id(whisper_context* ctx, int i_seg, int i_tok, int i_alt) {
    if (!ctx)
        return 0;
    return (int32_t)whisper_full_get_token_alt_id(ctx, i_seg, i_tok, i_alt);
}

CA_EXPORT float stelnettts_token_alt_p(whisper_context* ctx, int i_seg, int i_tok, int i_alt) {
    if (!ctx)
        return 0.0f;
...
CA_EXPORT int stelnettts_token_alt_text(whisper_context* ctx, int i_seg, int i_tok, int i_alt, char* out, int out_cap) {
...
CA_EXPORT float stelnettts_detect_language(whisper_context* ctx, const float* pcm, int n_samples, int n_threads,
...
// caller must pass the pointer back to `stelnettts_vad_free` when done. Returns
...
CA_EXPORT int stelnettts_vad_segments(const char* vad_model_path, const float* pcm, int n_samples, int sample_rate,
...
// Dispatcher-backed VAD slicing. Unlike stelnettts_vad_segments above, this
// routes through stelnettts_compute_vad_slices so GGUF VAD backends such as
...
// must pass it to stelnettts_vad_free. Returns the number of slices (>= 0), or a
// negative error.
//
//   -1  bad arguments
//   -2  allocation failed
...
// stelnettts_compute_vad_slices takes an `out_load_failed` flag for exactly this
...
CA_EXPORT int stelnettts_vad_slices(const char* vad_model_path, const float* pcm, int n_samples, int sample_rate,
    ...
    *out_spans = nullptr;
    ...
    std::vector<stelnettts_audio_slice> slices =
        stelnettts_compute_vad_slices(pcm, n_samples, sample_rate, vad_model_path, opts, &load_failed);
    if (load_failed)
...
CA_EXPORT void stelnettts_vad_free(float* spans) {
    if (spans)
        std::free(spans);
}
...
// that drive `libstelnettts` chunk-by-chunk (Go cgo, Rust, Dart FFI, Python
...
// concerns. See `src/core/stelnettts_lcs.h` for the algorithm itself; this
...
// stelnettts_watermark_load_model(), otherwise falls back to the built-in
// spread-spectrum watermark.

#include "../examples/cli/stelnettts_voice_clone_policy.h"
#include "../examples/cli/stelnettts_voice_provenance.h"
...
CA_EXPORT int stelnettts_watermark_load_model(const char* gguf_path) {
...
CA_EXPORT float stelnettts_watermark_detect(const float* pcm, int n_samples) {
    if (!pcm || n_samples <= 0)
        return 0.0f;
    if (g_audioseal_ctx) {
        int n_frames = 0;
    ...
    return ::stelnettts_watermark_detect_select(pcm, n_samples);
}

CA_EXPORT void stelnettts_watermark_embed(float* pcm, int n_samples, float alpha) {
    if (!pcm || n_samples <= 0)
        return;
    if (g_audioseal_ctx) {
    ...
    ::stelnettts_watermark_embed_impl(pcm, n_samples, alpha > 0.0f ? alpha : -1.0f);
}
...
// bytes (free with stelnettts_c2pa_free) and sets *out_len, or NULL when C2PA is
...
CA_EXPORT unsigned char* stelnettts_c2pa_sign(const unsigned char* data, size_t len, const char* format,
    ...
    if (!::stelnettts_c2pa_sign_auto(buf, format, cert, key, std::string()))
        return nullptr;
...
// Free a buffer returned by stelnettts_c2pa_sign.
CA_EXPORT void stelnettts_c2pa_free(unsigned char* p) {
    free(p);
}

// Wrap float32 mono PCM into a 16-bit WAV container that carries the AI-generated
// provenance metadata tag (a standard WAV LIST/INFO chunk: ISFT="StelnetTTS
...
// (free with stelnettts_c2pa_free) and sets *out_len, or NULL on bad input.
// Feed the result to stelnettts_c2pa_sign() to additionally embed a C2PA manifest.
CA_EXPORT unsigned char* stelnettts_pcm_to_wav(const float* pcm, int n_samples, int sample_rate, size_t* out_len) {
    if (out_len)
        *out_len = 0;
    if (!pcm || n_samples <= 0 || sample_rate <= 0)
        return nullptr;
    std::string wav = ::stelnettts_make_wav_int16(pcm, n_samples, sample_rate);
...
CA_EXPORT int stelnettts_lcs_dedup_prefix_count(const int32_t* prev_tail_tokens, int n_prev, const int32_t* curr_tokens,
    ...
    const int min_l = min_lcs_length > 0 ? min_lcs_length : stelnettts_lcs::kMinMergeSubsequenceLen;
    return stelnettts_lcs::lcs_dedup_prefix_count(prev, curr, min_l);
}
...
// is what stelnettts itself supports.
...
struct stelnettts_stream {
    ...
    // PLAN #62c — opaque kyutai_stt_stream*; when set, all stelnettts_stream_*
...
CA_EXPORT stelnettts_stream* stelnettts_stream_open(whisper_context* ctx, int n_threads, int step_ms, int length_ms,
                                                int keep_ms, const char* language, int translate) {
    if (!ctx)
        return nullptr;
    auto* s = new stelnettts_stream();
    s->ctx = ctx;
...
CA_EXPORT void stelnettts_stream_close(stelnettts_stream* s) {
    if (!s)
        return;
#if __has_include("kyutai_stt.h")
...
static int stelnettts_stream_run_decode(stelnettts_stream* s) {
...
CA_EXPORT int stelnettts_stream_feed(stelnettts_stream* s, const float* pcm, int n_samples) {
    ...
    if (stelnettts_stream_run_decode(s) != 0)
        return -2;
    return 1; // new output ready
}

CA_EXPORT int stelnettts_stream_get_text(stelnettts_stream* s, char* out_text, int out_cap, double* out_t0_s,
...
CA_EXPORT int stelnettts_stream_flush(stelnettts_stream* s) {
    if (!s)
        return -1;
#if __has_include("kyutai_stt.h")
    if (s->kyutai_stream_state) {
    ...
    if (s->accum.empty())
        return 0;
    return stelnettts_stream_run_decode(s) == 0 ? 1 : -2;
}
...
CA_EXPORT void stelnettts_stream_set_live_decode(stelnettts_stream* s, int enabled) {
    if (!s)
        return;
#if __has_include("voxtral4b.h")
    if (s->voxtral4b_stream_state) {
...
CA_EXPORT parakeet_context* stelnettts_parakeet_init(const char* model_path, int n_threads, int use_flash) {
    if (!model_path)
        return nullptr;
...
CA_EXPORT void stelnettts_parakeet_free(parakeet_context* ctx) {
    if (ctx)
        parakeet_free(ctx);
}

CA_EXPORT parakeet_result* stelnettts_parakeet_transcribe(parakeet_context* ctx, const float* pcm, int n_samples,
                                                        int64_t t_offset_cs) {
...
CA_EXPORT const char* stelnettts_parakeet_result_text(parakeet_result* r) {
    return (r && r->text) ? r->text : "";
}

CA_EXPORT int stelnettts_parakeet_result_n_words(parakeet_result* r) {
    return r ? r->n_words : 0;
}
CA_EXPORT const char* stelnettts_parakeet_result_word_text(parakeet_result* r, int i) {
    if (!r || i < 0 || i >= r->n_words)
        return "";
    return r->words[i].text;
}
CA_EXPORT int64_t stelnettts_parakeet_result_word_t0(parakeet_result* r, int i) {
    return (r && i >= 0 && i < r->n_words) ? r->words[i].t0 : 0;
}
CA_EXPORT int64_t stelnettts_parakeet_result_word_t1(parakeet_result* r, int i) {
    return (r && i >= 0 && i < r->n_words) ? r->words[i].t1 : 0;
}

CA_EXPORT int stelnettts_parakeet_result_n_tokens(parakeet_result* r) {
    return r ? r->n_tokens : 0;
}
CA_EXPORT const char* stelnettts_parakeet_result_token_text(parakeet_result* r, int i) {
    if (!r || i < 0 || i >= r->n_tokens)
        return "";
    return r->tokens[i].text;
}
CA_EXPORT int64_t stelnettts_parakeet_result_token_t0(parakeet_result* r, int i) {
    return (r && i >= 0 && i < r->n_tokens) ? r->tokens[i].t0 : 0;
}
CA_EXPORT int64_t stelnettts_parakeet_result_token_t1(parakeet_result* r, int i) {
    return (r && i >= 0 && i < r->n_tokens) ? r->tokens[i].t1 : 0;
}
CA_EXPORT float stelnettts_parakeet_result_token_p(parakeet_result* r, int i) {
    return (r && i >= 0 && i < r->n_tokens) ? r->tokens[i].p : 0.0f;
}

CA_EXPORT void stelnettts_parakeet_result_free(parakeet_result* r) {
    if (r)
...
CA_EXPORT nemotron_context* stelnettts_nemotron_init(const char* model_path, int n_threads, int use_gpu) {
    if (!model_path)
        return nullptr;
    if (use_gpu)
...
CA_EXPORT void stelnettts_nemotron_free(nemotron_context* ctx) {
    if (ctx)
        nemotron_free(ctx);
}

CA_EXPORT nemotron_result* stelnettts_nemotron_transcribe(nemotron_context* ctx, const float* pcm, int n_samples,
                                                        int64_t t_offset_cs) {
    if (!ctx || !pcm || n_samples <= 0)
        return nullptr;
...
// backend names used by StelnetTTS ("whisper" / "parakeet" / "canary" /
...
CA_EXPORT int stelnettts_detect_backend_from_gguf(const char* path, char* out_name, int out_cap) {
    ...
    // Map known architecture strings to StelnetTTS backend names.
    //
...
//
// Internally a `stelnettts_session` owns exactly one of the per-backend
...
// switch statement in `stelnettts_session_open_explicit`.

// ─────────────────────────────────────────────────────────────────────
// Open-time params (StelnetTTS 0.6.1). The previous open ABI took only
...
// through `stelnettts_session_open_explicit` via thread-local storage:
// the new `stelnettts_session_open_with_params` export sets them
...
// (`stelnettts_session_open` → detect → open_explicit) and by every
...
// Defined ahead of stelnettts_session so the session can hold its own
// streamed-segment polling buffer (see the Dart FFI polling API below).
struct stelnettts_session_seg {
    std::string text;
    ...
    // (stelnettts_segment::speaker) and the same string the CLI prefixes into
    // text/srt/vtt output. Read via stelnettts_session_result_segment_speaker().
...
struct stelnettts_session {
    ...
    // Set via stelnettts_session_set_pcm_sample_rate() so backends that
    ...
    // DEFAULT on every ABI path (stelnettts_session_synthesize / _streaming /
    // speech_to_speech), matching the CLI/server. The only way to obtain UNMARKED
    // PCM is stelnettts_session_synthesize_raw, which is hard-refused unless the
    // integrator first attests via stelnettts_session_accept_marking_responsibility()
    ...
    // True once stelnettts_session_set_voice() has been given a reference WAV,
    ...
    // stelnettts_session_get_disclaimer_pcm), so instead it records the fact,
    ...
    // consent attestation. See stelnettts_speaker_identity.h.
    stelnettts_voice::SpeakerIdentity voice_pack_identity = stelnettts_voice::SpeakerIdentity::Unknown;
    stelnettts_voice::SpeakerIdentity speaker_identity_override = stelnettts_voice::SpeakerIdentity::Unknown;
    ...
    // stelnettts_session_detected_language falls back to the source-language
    // hint or "unknown". This is the in-decode acoustic signal, distinct
    // from the backend-agnostic text-LID pass (stelnettts_text_detect_language).
    std::string detected_lang;

    // --punc-model post-processor (set via stelnettts_session_set_punc_model).
    ...
    // (voxtral / voxtral4b / cielvox2-asr). When non-empty, replaces the
    ...
    //   cielvox2-asr      — via core_beam_decode::run_with_probs (replay)
    ...
    // stelnettts_session_set_return_logits. Off by default so the normal path
    ...
    // deferred until stelnettts_session_transcribe_vad_lang has remapped the
    ...
    // matching C-ABI `stelnettts_session_set_whisper_decode_extras`.
    ...
    //
    // Lifecycle:
    //   * `stelnettts_session_set_grammar_text(s, "<gbnf>", "root", 100.0f)`
    ...
    // stelnettts_session_transcribe_chunked[_lang] sets these for the duration
    ...
    stelnettts_progress_callback progress_cb = nullptr;
    ...
    stelnettts_segment_callback segment_cb = nullptr;
    ...
    stelnettts_token_callback token_cb = nullptr;
    ...
    // polling API (stelnettts_drain_streamed_segments etc.) delegates to the
    // session that most recently streamed via the default callbacks.
    std::mutex stream_mutex;
    std::vector<stelnettts_session_seg> streamed_segments;
    std::vector<std::string> streamed_tokens;
    std::atomic<int> streamed_seg_count{0};
    std::atomic<int> streamed_tok_count{0};
    std::string token_drain_buf; // backing store for stelnettts_drain_streamed_tokens
...
#endif
#ifdef CA_HAVE_QWEN3
    cielvox2_asr_context* qwen3_ctx = nullptr;
#endif
    ...
    cielvox2_tts_context* cielvox2_tts_ctx = nullptr;
    bool cielvox2_tts_voice_loaded = false;
#endif
    ...
    // Emission scores from the last stelnettts_session_tab() call, kept alive so
    ...
    // recognize() so stelnettts_session_chords_spans can hand out a typed-array
    ...
    // contiguous float view (see stelnettts_session_piano_notes).
...
struct stelnettts_session_result {
    std::vector<stelnettts_session_seg> segments;
    std::string backend;
    // Per-frame CTC logits, populated only when the session opted in via
    // stelnettts_session_set_return_logits and the backend produces a dense CTC
...
static stelnettts_session* g_stream_session = nullptr;

static void _register_stream_session(stelnettts_session* s) {
...
CA_EXPORT int stelnettts_get_streamed_segment_count(void) {
    std::lock_guard<std::mutex> reg(g_stream_session_mtx);
    return g_stream_session ? g_stream_session->streamed_seg_count.load(std::memory_order_relaxed) : 0;
}

CA_EXPORT stelnettts_session_result* stelnettts_drain_streamed_segments(void) {
    std::lock_guard<std::mutex> reg(g_stream_session_mtx);
    stelnettts_session* s = g_stream_session;
    if (!s)
        return nullptr;
    std::lock_guard<std::mutex> lk(s->stream_mutex);
    if (s->streamed_segments.empty())
        return nullptr;
    auto* r = new stelnettts_session_result;
...
CA_EXPORT void stelnettts_reset_streamed_segments(void) {
    std::lock_guard<std::mutex> reg(g_stream_session_mtx);
    stelnettts_session* s = g_stream_session;
    if (!s)
        return;
...
CA_EXPORT int stelnettts_get_streamed_token_count() {
    std::lock_guard<std::mutex> reg(g_stream_session_mtx);
    return g_stream_session ? g_stream_session->streamed_tok_count.load(std::memory_order_relaxed) : 0;
}

CA_EXPORT const char* stelnettts_drain_streamed_tokens(int* out_count) {
    std::lock_guard<std::mutex> reg(g_stream_session_mtx);
    stelnettts_session* s = g_stream_session;
    if (!s) {
        if (out_count)
...
CA_EXPORT void stelnettts_reset_streamed_tokens() {
    std::lock_guard<std::mutex> reg(g_stream_session_mtx);
    stelnettts_session* s = g_stream_session;
    if (!s)
        return;
...
static void _fire_segment_callbacks(stelnettts_session* s, stelnettts_session_result* r) {
    if (!s || !r || !s->segment_cb)
        return;
...
struct ca_token_record {
    ...
    std::vector<stelnettts_session_seg::word_alt> alts;
};
...
static void _fire_token_callbacks(stelnettts_session* s, const std::vector<ca_token_record>& toks) {
    if (!s || !s->token_cb)
        return;
...
static std::vector<stelnettts_session_seg::word> emit_words_from_tokens(const std::vector<ca_token_record>& toks) {
    ...
    std::vector<stelnettts_session_seg::word> out;
    stelnettts_session_seg::word cur;
    bool have_cur = false;
...
static void filter_words_by_ngram_collapse(std::vector<stelnettts_session_seg::word>& words) {
    if (words.empty())
        return;
    ...
    if (keep.size() == words.size())
        return;
    std::vector<stelnettts_session_seg::word> filtered;
...
CA_EXPORT stelnettts_session* stelnettts_session_open_explicit(const char* model_path, const char* backend_name,
                                                           int n_threads) {
    ...
    // stelnettts_session_set_segment_callback after open. ud is the session
    ...
    // examples/cli/stelnettts_backend.cpp::resolve_make_fn().
    if (s->backend == "qwen3" || s->backend == "mega-asr" || s->backend == "mega_asr" || s->backend == "megaasr") {
        ...
        p.use_gpu = g_open_use_gpu_tls;
        p.flash_attn = g_open_flash_attn_tls;
        s->qwen3_ctx = cielvox2_asr_init_from_file(model_path, p);
        if (!s->qwen3_ctx) {
            delete s;
            return nullptr;
        }
...
#endif
#ifdef CA_HAVE_QWEN3_TTS
    if (s->backend == "cielvox2-tts" || s->backend == "cielvox2_tts" || s->backend == "cielvox2tts" ||
        s->backend == "cielvox2-tts-base" || s->backend == "cielvox2-tts-1.7b-base" ||
        s->backend == "cielvox2-tts-customvoice" || s->backend == "gwen-tts") {
        cielvox2_tts_context_params p = cielvox2_tts_context_default_params();
        p.n_threads = s->n_threads;
        p.verbosity = g_open_verbosity_tls;
        p.use_gpu = g_open_use_gpu_tls;
        p.flash_attn = g_open_flash_attn_tls;
        s->cielvox2_tts_ctx = cielvox2_tts_init_from_file(model_path, p);
        if (!s->cielvox2_tts_ctx) {
            delete s;
            return nullptr;
        }
        // Codec must be loaded before synthesise. Caller does so via
        // `stelnettts_session_set_codec_path` after open.
        return s;
    }
#endif
#ifdef CA_HAVE_MIOTTS
    ...
    if (s->backend == "moss-tts" || s->backend == "moss_tts" || s->backend == "mosstts") {
        ...
        // Codec loaded via `stelnettts_session_set_codec_path` after open.
        return s;
    }
#endif
#ifdef CA_HAVE_MOSS_TTS_LOCAL
    ...
    if (s->backend == "htdemucs" || s->backend == "demucs") {
        ...
        hp.use_gpu = g_open_use_gpu_tls; // stelnettts_session has no use_gpu member; use the open-time TLS flag
    ...
    if (s->backend == "kokoro" || s->backend == "kokoro-tts") {
        ...
        // `stelnettts_session_set_voice` after open.
        return s;
    }
#endif
#ifdef CA_HAVE_VOXTRAL_TTS
    if (s->backend == "voxtral-tts") {
        ...
        // Voice is an optional preset name applied via stelnettts_session_set_voice;
    ...
    if (s->backend == "chatterbox" || s->backend == "chatterbox-tts" || s->backend == "kartoffelbox" ||
        ...
        // S3Gen GGUF must be loaded via stelnettts_session_set_s3gen_path
        return s;
    }
#endif
#ifdef CA_HAVE_BANANAMIND_TTS
    ...
    if (s->backend == "tada" || s->backend == "tada-tts" || s->backend == "tada-1b" || s->backend == "tada-tts-1b" ||
        ...
        if (const char* env = stelnettts_env::get("STELNETTTS_TADA_NUM_CANDIDATES"); env && *env) {
            int n = atoi(env);
            if (n >= 1)
        ...
        if (const char* e = stelnettts_env::get("STELNETTTS_TADA_DO_SAMPLE"); e && *e)
            p.text_do_sample = !(e[0] == '0' || e[0] == 'f' || e[0] == 'F' || e[0] == 'n' || e[0] == 'N');
        if (const char* e = stelnettts_env::get("STELNETTTS_TADA_TEMPERATURE"); e && *e)
            p.temperature = (float)atof(e);
        if (const char* e = stelnettts_env::get("STELNETTTS_TADA_TOP_P"); e && *e)
            p.text_top_p = (float)atof(e);
        if (const char* e = stelnettts_env::get("STELNETTTS_TADA_TOP_K"); e && *e)
            p.text_top_k = atoi(e);
        if (const char* e = stelnettts_env::get("STELNETTTS_TADA_REPETITION_PENALTY"); e && *e)
            p.text_repetition_penalty = (float)atof(e);
        s->tada_ctx = tada_init_from_file(model_path, p);
        if (!s->tada_ctx) {
            fprintf(stderr, "stelnettts: failed to init tada from '%s'\n", model_path);
            delete s;
            return nullptr;
        }
    ...
    if (s->backend == "outetts" || s->backend == "oute-tts" || s->backend == "oute_tts") {
        ...
        // WavTokenizer codec set via stelnettts_session_set_companion_path
    ...
    if (s->backend == "dots" || s->backend == "dots-tts" || s->backend == "dots_tts" || s->backend == "dots.tts") {
        ...
        // stelnettts_session_set_voice); load it eagerly so set_voice just applies
        // the reference WAV.
        {
            std::string mp = model_path ? model_path : "";
            auto sep = mp.find_last_of("/\\");
    ...
    if (s->backend == "voxcpm2-tts" || s->backend == "voxcpm2" || s->backend == "voxcpm2_tts") {
        ...
        // stelnettts_session_set_voice (decoded from a WAV there).
        return s;
    }
#endif
#ifdef CA_HAVE_VOXCPM2_VAE
    ...
    if (s->backend == "cosyvoice3-tts" || s->backend == "cosyvoice3" || s->backend == "cosyvoice3-llm" ||
        ...
        // hardcoding 0.8 here silently ignored stelnettts_session_set_temperature.
    ...
    if (s->backend == "indextts" || s->backend == "indextts-1.5") {
        ...
        // stelnettts_session_set_codec_path. The cloning reference (24 kHz)
        // is supplied via stelnettts_session_set_voice.
        return s;
    }
#endif
#ifdef CA_HAVE_F5TTS
    if (s->backend == "f5-tts" || s->backend == "f5tts" || s->backend == "f5") {
        ...
        // silently ignored stelnettts_session's seed for f5-tts only.
    ...
    if (s->backend == "melotts" || s->backend == "melo-tts" || s->backend == "melo") {
        ...
        {
            const char* bert_env = stelnettts_env::get("STELNETTTS_MELOTTS_BERT");
    ...
    if (s->backend == "mimo-asr" || s->backend == "mimo_asr" || s->backend == "mimo") {
        ...
        // Caller does so via `stelnettts_session_set_codec_path` after open
        // (we route the tokenizer through that setter — same shape as
        // cielvox2-tts/orpheus's codec companion).
        return s;
    }
#endif
#ifdef CA_HAVE_ARK_ASR
...
CA_EXPORT stelnettts_session* stelnettts_session_open(const char* model_path, int n_threads) {
    if (!model_path)
        return nullptr;
    char detected[64] = {0};
    if (stelnettts_detect_backend_from_gguf(model_path, detected, (int)sizeof(detected)) <= 0) {
    ...
    return stelnettts_session_open_explicit(model_path, detected, n_threads);
}

// ─────────────────────────────────────────────────────────────────
// Open with explicit runtime params (StelnetTTS 0.6.1, extended 0.6.2).
//
...
// (same path as `stelnettts_session_open`).
// ─────────────────────────────────────────────────────────────────
...
CA_EXPORT void stelnettts_set_gpu_backend(const char* name) {
    stelnettts_set_gpu_backend_pref(name);
}
...
CA_EXPORT stelnettts_session* stelnettts_session_open_with_params(const char* model_path, const char* backend_name,
    ...
    // same place stelnettts_session_open does.
    int n_threads = 4;
    bool use_gpu = true;
    int verbosity = 0;
    ...
    stelnettts_session* s = nullptr;
    if (backend_name && backend_name[0] != '\0') {
        s = stelnettts_session_open_explicit(model_path, backend_name, n_threads);
    } else {
        // Explicit-detection path matches `stelnettts_session_open`.
        char detected[64] = {0};
        if (stelnettts_detect_backend_from_gguf(model_path, detected, (int)sizeof(detected)) > 0) {
            s = stelnettts_session_open_explicit(model_path, detected, n_threads);
        } else {
            ...
            if (f) {
                char magic[4] = {0};
                if (fread(magic, 1, 4, f) == 4 && (memcmp(magic, "lmgg", 4) == 0 || memcmp(magic, "ggjt", 4) == 0)) {
                    s = stelnettts_session_open_explicit(model_path, "whisper", n_threads);
                }
                fclose(f);
            }
        }
...
CA_EXPORT const char* stelnettts_session_backend(stelnettts_session* s) {
...
// backend-agnostic text-LID pass (stelnettts_text_detect_language), which runs
// CLD3 + fastText over the transcript.
CA_EXPORT int stelnettts_session_detected_language(stelnettts_session* s, char* out_buf, int out_cap) {
...
// use this with stelnettts_audio_load_at_rate() to decode audio directly at
// the model's native rate, avoiding a lossy down-then-up resample.
CA_EXPORT int stelnettts_session_input_sample_rate(stelnettts_session* s) {
    if (!s)
        return 0;
    ...
    return STELNETTTS_SAMPLE_RATE;
}

CA_EXPORT int stelnettts_session_set_pcm_sample_rate(stelnettts_session* s, int rate) {
    if (!s || rate <= 0)
        return -1;
...
// #332: the sample rate of the PCM produced by stelnettts_session_synthesize /
...
// (examples/cli/stelnettts_backend.h — default 24 kHz, overridden per backend).
...
CA_EXPORT int stelnettts_session_output_sample_rate(stelnettts_session* s) {
    if (!s)
        return 0;
...
// (stelnettts_session_separate + separate_sample_rate). Exposed as getters
...
CA_EXPORT int stelnettts_session_input_channels(stelnettts_session* s) {
    if (!s)
        return 0;
    return 1;
}

CA_EXPORT int stelnettts_session_output_channels(stelnettts_session* s) {
    if (!s)
        return 0;
    return stelnettts_session_output_sample_rate(s) > 0 ? 1 : 0;
}
...
// over stelnettts_session_result_logits. Returns 0 / "" for other backends.
CA_EXPORT int stelnettts_session_n_vocab(stelnettts_session* s) {
    if (!s)
        return 0;
#ifdef CA_HAVE_OMNIASR
...
CA_EXPORT const char* stelnettts_session_token_text(stelnettts_session* s, int id) {
    if (!s || id < 0)
        return "";
#ifdef CA_HAVE_OMNIASR
    if ((s->backend.rfind("omniasr", 0) == 0) && s->omniasr_ctx)
...
CA_EXPORT int stelnettts_session_available_backends(char* out_csv, int out_cap) {
    ...
    // StelnetWeaver's catalogue uses so the front-door
    ...
    // support) — advertise it so StelnetWeaver's strict front-door check
    ...
    // Advertised separately so StelnetWeaver's strict front-door check
...
// CLI's stelnettts_llm_pipeline.h (which depends on whisper_params and
...
static stelnettts_session_result* run_voxtral_family(Ctx* ctx, const VoxtralFamilyOps<Ctx>& ops, const float* pcm,
    ...
    // `stelnettts_session_set_ask`. With it: `[/INST]<question>` so the
...
// `stelnettts_session_transcribe`. The instruction-tuned audio-LLM backends
...
static stelnettts_session_result* transcribe_single(stelnettts_session* s, const float* pcm, int n_samples,
                                                  const char* language);
...
// STELNETTTS_SESSION_AUTOCHUNK=0 disables (default on — it fixes a degradation and
// matches the CLI); STELNETTTS_SESSION_CHUNK_SECONDS overrides the window. The
// default window is a flat 30 s for every backend; STELNETTTS_SESSION_PERBACKEND_CHUNK=1
...
static stelnettts_session_result* transcribe_autochunk(stelnettts_session* s, const float* pcm, int n_samples,
                                                     const char* language) {
    const int SR = 16000;
    bool enabled = true;
    if (const char* e = getenv("STELNETTTS_SESSION_AUTOCHUNK"))
        enabled = atoi(e) != 0;
    bool perbackend = false;
    if (const char* e = getenv("STELNETTTS_SESSION_PERBACKEND_CHUNK"))
        perbackend = atoi(e) != 0;
    int chunk_s = core_session::session_default_chunk_seconds(s->backend, perbackend);
    if (const char* e = getenv("STELNETTTS_SESSION_CHUNK_SECONDS"))
    ...
    auto* merged = new stelnettts_session_result();
    for (const auto& range : ranges) {
        const size_t b = range.first, e = range.second;
        stelnettts_session_result* part = transcribe_single(s, pcm + b, (int)(e - b), language);
        if (!part)
            continue;
        ...
        for (auto& seg : part->segments) {
            ...
            if (!seg.words.empty()) {
                ...
                if (keep.size() < seg.words.size()) {
                    std::vector<stelnettts_session_seg::word> kept;
...
// Defined further down next to stelnettts_session_set_punc_model.
static void apply_session_punc_model(stelnettts_session* s, stelnettts_session_result* r);
...
static void apply_session_hygiene(stelnettts_session_result* r, bool include_merge) {
    ...
    std::vector<stelnettts_session_seg> res;
    res.reserve(pick.size());
    for (size_t i = 0; i < pick.size(); i++) {
        stelnettts_session_seg seg = std::move(r->segments[pick[i]]);
        seg.text = kept[i].text;
        seg.t1 = kept[i].t1;
        res.push_back(std::move(seg));
    }
    if (dropped > 0 || res.size() != r->segments.size())
        fprintf(stderr, "stelnettts[hygiene]: %zu -> %zu segments (%d dropped)\n", r->segments.size(), res.size(),
                dropped);
    r->segments = std::move(res);
}

CA_EXPORT stelnettts_session_result* stelnettts_session_transcribe_lang(stelnettts_session* s, const float* pcm,
    ...
    if (n_runs <= 1) {
        stelnettts_session_result* r = transcribe_autochunk(s, pcm, n_samples, language);
    ...
    stelnettts_session_result* best = nullptr;
    double best_avg_p = -1.0;
    for (int run = 0; run < n_runs; run++) {
        stelnettts_session_result* candidate = transcribe_autochunk(s, pcm, n_samples, language);
        if (!candidate)
            continue;
...
// from the CLI adapter (examples/cli/stelnettts_backend_parakeet.cpp) so the
...
static void parakeet_session_chunked_merge(parakeet_context* ctx, const float* samples, int n_samples,
                                           ...
                                           std::vector<stelnettts_session_seg>& out,
                                           stelnettts_progress_callback prog_cb = nullptr, void* prog_ud = nullptr) {
    const int SR = 16000;
    if (chunk_samples < SR)
    ...
    for (auto& w : dedup) {
        if (!text.empty())
            text += ' ';
        text += w.text;
        stelnettts_session_seg::word sw;
        sw.text = w.text;
        sw.t0 = w.t0;
        sw.t1 = w.t1;
        sw.p = w.p;
...
// exactly equivalent to stelnettts_session_transcribe_lang — callers can use
// the chunked entry point uniformly without backend-specific branching.
CA_EXPORT stelnettts_session_result* stelnettts_session_transcribe_chunked_lang(stelnettts_session* s, const float* pcm,
    ...
    return stelnettts_session_transcribe_lang(s, pcm, n_samples, language);
}

// Language-default convenience wrapper for the chunked entry point.
CA_EXPORT stelnettts_session_result* stelnettts_session_transcribe_chunked(stelnettts_session* s, const float* pcm,
                                                                       int n_samples, int chunk_seconds,
                                                                       int overlap_seconds) {
    return stelnettts_session_transcribe_chunked_lang(s, pcm, n_samples, chunk_seconds, overlap_seconds, nullptr);
}

// Issue #208: register a per-session progress callback for long-form
// (chunked) transcription. See stelnettts_session.h for the contract.
CA_EXPORT void stelnettts_session_set_progress_callback(stelnettts_session* s, stelnettts_progress_callback cb,
                                                      void* user_data) {
    if (!s)
        return;
    s->progress_cb = cb;
...
CA_EXPORT void stelnettts_session_set_segment_callback(stelnettts_session* s, stelnettts_segment_callback cb,
                                                     void* user_data) {
    if (!s)
        return;
...
CA_EXPORT void stelnettts_session_set_token_callback(stelnettts_session* s, stelnettts_token_callback cb, void* user_data) {
    if (!s)
        return;
...
static stelnettts_session_result* transcribe_single(stelnettts_session* s, const float* pcm, int n_samples,
    ...
    // stelnettts_session_set_hotwords() — no ask-prompt injection needed.
    ...
    if (s->backend == "whisper" && s->whisper_ctx) {
        ...
        whisper_full_params wparams =
            whisper_full_default_params(use_beam ? STELNETTTS_SAMPLING_BEAM_SEARCH : STELNETTTS_SAMPLING_GREEDY);
        wparams.print_progress = false;
        wparams.print_realtime = false;
        wparams.print_timestamps = false;
        wparams.print_special = false;
        ...
        // Dart can poll via stelnettts_get_progress().
            ...
            for (int j = 0; j < n_tok; ++j) {
                ...
                if (n_alts > 0) {
                    ...
                    for (int k = 0; k < n_alts; ++k) {
                        ...
                        stelnettts_session_seg::word_alt wa;
        ...
        // stelnettts_session_detected_language can surface it (ISO-639-1). The
    ...
    if (s->backend == "parakeet" && s->parakeet_ctx) {
        ...
        // parakeet (short + 225 s). STELNETTTS_SESSION_UNIFIED_DISPATCH=0 selects the
        // legacy inline path for A/B.
        const bool _unified = [] {
            const char* e = getenv("STELNETTTS_SESSION_UNIFIED_DISPATCH");
        ...
        if (_unified && !is_ja) {
            ...
            // stelnettts_session_transcribe_chunked[_lang]; 0 is its documented
        ...
        // isolation). See examples/cli/stelnettts_run.cpp gap-fill for the
        // measured numbers (97/97/96 % recall vs 53-62 % streamed).
        auto transcribe_ja_sliced = [&](const float* audio, int n, int sr, int cap_s) -> stelnettts_session_seg {
            int64_t min_gap_cs = 100;
            if (const char* e = getenv("STELNETTTS_GAP_FILL_MIN_CS"))
                min_gap_cs = std::max((int64_t)30, (int64_t)atoi(e));
            bool gap_fill = true;
            if (const char* e = getenv("STELNETTTS_GAP_FILL"))
                gap_fill = atoi(e) != 0;
            constexpr int64_t kEdgePadCs = 20;
            constexpr int64_t kCoverSlopCs = 30;

            const auto slices = stelnettts_energy_chunk_slices(audio, n, sr, cap_s, 2.0f);
            std::vector<stelnettts_session_seg::word> words;
            auto run_window = [&](int w0, int w1, int64_t t0_cs, std::vector<stelnettts_session_seg::word>& out) {
                parakeet_result* pr = parakeet_transcribe_ex(s->parakeet_ctx, audio + w0, w1 - w0, t0_cs);
                if (!pr)
                    return;
                for (int i = 0; i < pr->n_words; ++i) {
                    stelnettts_session_seg::word w;
            ...
            for (size_t si = 0; si < slices.size(); ++si) {
                ...
                std::vector<stelnettts_session_seg::word> sw;
                ...
                for (int round = 0; gap_fill && round < 2; ++round) {
                    ...
                    for (auto& g : gaps) {
                        ...
                        if (w1 - w0 < sr / 4)
                            continue;
                        std::vector<stelnettts_session_seg::word> fw;
                    ...
                    std::sort(sw.begin(), sw.end(),
                              [](const stelnettts_session_seg::word& a, const stelnettts_session_seg::word& b) {
                                  return a.t0 < b.t0;
                              });
                }
                for (auto& w : sw)
            ...
            std::sort(
                ...
                [](const stelnettts_session_seg::word& a, const stelnettts_session_seg::word& b) { return a.t0 < b.t0; });
            stelnettts_session_seg seg;
            for (const auto& w : words) {
                if (w.text.empty())
                    continue;
        ...
        if (const char* e = getenv("STELNETTTS_PARAKEET_CHUNK_SECONDS"))
            chunk_s = std::max(4, atoi(e));
        if (const char* e = getenv("STELNETTTS_PARAKEET_CHUNK_OVERLAP"))
        ...
        // stelnettts_session_transcribe_chunked[_lang]) forces the bounded path
        ...
        // (stelnettts_session_transcribe_chunked → server / language bindings)
        ...
        // mirror of the CLI default; STELNETTTS_PARAKEET_VAD_SLICE_CAP=0
        // reverts to the old streamed path, an explicit chunked request via
        // stelnettts_session_transcribe_chunked keeps the caller's sizing).
        int ja_cap_s = 12;
        if (const char* e = getenv("STELNETTTS_PARAKEET_VAD_SLICE_CAP"))
    ...
    // alias set in stelnettts_session_open_explicit.
    if ((s->backend == "qwen3" || s->backend == "mega-asr" || s->backend == "mega_asr" || s->backend == "megaasr") &&
        s->qwen3_ctx) {
        // cielvox2-asr's runtime _transcribe() is a stub. Drive the building
        ...
        int n_mels = 0, T_mel = 0;
        float* mel = cielvox2_asr_compute_mel(s->qwen3_ctx, pcm, n_samples, &n_mels, &T_mel);
        if (!mel) {
            delete r;
            return nullptr;
        }
        int N_enc = 0, pdim = 0;
        float* audio_embeds = cielvox2_asr_run_encoder(s->qwen3_ctx, mel, n_mels, T_mel, &N_enc, &pdim);
        std::free(mel);
        if (!audio_embeds) {
            delete r;
            return nullptr;
        ...
        // adapter (stelnettts_backend_qwen3.cpp); the legacy system-turn
        // instruction stays behind STELNETTTS_QWEN3_SYSPROMPT_LANG=1.
        std::string sys_instruction;
        std::string assistant_prefill;
        if (s->ask.empty()) {
            const std::string eff_lang = lang_set ? lang : s->source_language;
            if (!eff_lang.empty() && eff_lang != "auto") {
                const char* legacy_env = getenv("STELNETTTS_QWEN3_SYSPROMPT_LANG");
        ...
        int n_prompt = 0;
        int32_t* raw_ids = cielvox2_asr_tokenize(s->qwen3_ctx, text.c_str(), &n_prompt);
        if (!raw_ids) {
            std::free(audio_embeds);
            delete r;
            return nullptr;
        ...
        int n_pad_id = 0;
        int32_t* pad_id_arr = cielvox2_asr_tokenize(s->qwen3_ctx, "<|audio_pad|>", &n_pad_id);
        ...
        float* text_embeds = cielvox2_asr_embed_tokens(s->qwen3_ctx, ids.data(), (int)ids.size());
        if (!text_embeds) {
            std::free(audio_embeds);
            delete r;
            return nullptr;
        ...
        if (!cielvox2_asr_kv_init(s->qwen3_ctx, std::max(4096, (int)ids.size() + q3_max_new + 16))) {
            std::free(text_embeds);
            delete r;
            return nullptr;
        }
        cielvox2_asr_kv_reset(s->qwen3_ctx);

        int n_t = 0, vocab = 0;
        float* logits = cielvox2_asr_run_llm_kv(s->qwen3_ctx, text_embeds, (int)ids.size(), 0, &n_t, &vocab);
        std::free(text_embeds);
        if (!logits) {
            delete r;
            return nullptr;
        ...
        int eos_id = -1;
        int n_eos = 0;
        int32_t* eos_arr = cielvox2_asr_tokenize(s->qwen3_ctx, "<|im_end|>", &n_eos);
        if (eos_arr && n_eos >= 1)
            eos_id = eos_arr[0];
        std::free(eos_arr);
        ...
        if (s->beam_size > 1) {
            // PLAN §90: session beam_size → cielvox2-asr beam decode.
        ...
        for (size_t i = 0; i < dec.tokens.size(); i++) {
            ...
            if (id == eos_id)
                break;
            const char* raw_piece = cielvox2_asr_token_text(s->qwen3_ctx, id);
    ...
    if ((s->backend == "granite" || s->backend == "granite-4.1" || s->backend == "granite-4.1-plus") &&
        ...
        // Legacy granite-4.0 fallbacks (mirrors stelnettts_backend_granite.cpp).
        if (audio_tok < 0)
            audio_tok = 100352;
        if (eos_tok < 0)
            eos_tok = 100257;
        ...
        // Language steering (mirrors stelnettts_backend_granite.cpp): when a
    ...
    if (s->backend == "wav2vec2" && s->wav2vec2_ctx) {
        ...
        // stelnettts_session_set_return_logits). Raw pre-softmax grid, frame-major
    ...
    if (s->backend == "vibevoice" && s->vibevoice_ctx) {
        ...
        // stelnettts_session_set_pcm_sample_rate + stelnettts_audio_load_at_rate.
        ...
        // only in stelnettts_backend_vibevoice.cpp would leave every binding
        // handing its callers the raw JSON blob.
        // STELNETTTS_VIBEVOICE_RAW_TRANSCRIPT=1 keeps the pre-#300 single segment.
        bool parsed = false;
        if (!stelnettts_env::truthy("STELNETTTS_VIBEVOICE_RAW_TRANSCRIPT")) {
    ...
    if ((s->backend == "fastconformer-ctc" || s->backend == "canary-ctc") && s->ctc_ctx) {
        ...
        // stelnettts_session_set_return_logits). canary_ctc_compute_logits returns
    ...
    auto package_text_only = [&](char* text, bool need_free) -> stelnettts_session_result* {
        if (!text) {
            delete r;
            return nullptr;
        }
    ...
    auto package_with_tokens = [&](char* text, std::vector<ca_token_record>&& toks) -> stelnettts_session_result* {
        if (!text) {
            delete r;
            return nullptr;
        }
    ...
    if ((s->backend == "glm-asr" || s->backend == "glmasr" || s->backend == "glm" || s->backend == "glm_asr") &&
        ...
        // ask > language instruction > default (mirrors stelnettts_backend_glm_asr.cpp).
    ...
    if ((s->backend == "kyutai-stt" || s->backend == "kyutai" || s->backend == "moshi-stt" ||
        ...
        // per-context setter so session-API consumers (StelnetWeaver's
    ...
    if ((s->backend.rfind("omniasr", 0) == 0) && s->omniasr_ctx) {
        ...
        if (s->return_logits) {
            ...
            stelnettts_session_result* res = package_text_only(text, true);
            if (res && lg) {
    ...
    // Backends without a token-prob API yet: text-only segment.
    {
        ...
        if (!text && s->backend == "mimo-asr" && s->mimo_asr_ctx) {
            ...
            // wasn't set via stelnettts_session_set_codec_path. We surface a clean
            // "no transcription" rather than hanging.
            // ask > language instruction > default (mirrors stelnettts_backend_mimo_asr.cpp).
            if (!s->ask.empty()) {
        ...
        if (!text && s->moss_audio_ctx) {
            // ask > language instruction > default (mirrors stelnettts_backend_moss_audio.cpp).
        ...
        if (!text && s->moss_diarize_ctx) {
            ...
            // #292: forward the session's max_new_tokens (stelnettts_session_set_max_new_tokens).
...
CA_EXPORT stelnettts_session_result* stelnettts_session_transcribe(stelnettts_session* s, const float* pcm, int n_samples) {
    return stelnettts_session_transcribe_lang(s, pcm, n_samples, nullptr);
}
...
// silence gaps (stelnettts-style), calls stelnettts_session_transcribe on
// the stitched buffer, and remaps segment + word timestamps from
// stitched-buffer space back to original-audio positions.
//
// The same algorithm the CLI uses (see examples/cli/stelnettts_run.cpp) is
// now reachable from every binding via a single call.
//
// Falls back to a direct stelnettts_session_transcribe(pcm) when VAD
...
CA_EXPORT stelnettts_session_result* stelnettts_session_transcribe_vad_lang(stelnettts_session* s, const float* pcm,
    ...
    if (vad_model_path && *vad_model_path) {
        slices = stelnettts_compute_vad_slices(pcm, n_samples, sample_rate, vad_model_path, opts);
    }
    if (slices.empty()) {
        return stelnettts_session_transcribe_lang(s, pcm, n_samples, language);
    }
    ...
    if (slices.size() == 1) {
        ...
        return stelnettts_session_transcribe_lang(s, pcm + sl.start, sl.end - sl.start, language);
    }

    // Multiple slices ⇒ stitch with 0.1s silence gaps, transcribe once,
    // remap timestamps back to original-audio positions.
    auto stitched = stelnettts_stitch_vad_slices(pcm, n_samples, sample_rate, slices);
    ...
    stelnettts_session_result* r =
        stelnettts_session_transcribe_lang(s, stitched.samples.data(), (int)stitched.samples.size(), language);
    if (!r)
        return nullptr;

    for (auto& seg : r->segments) {
        seg.t0 = stelnettts_vad_remap_timestamp(stitched.mapping, seg.t0);
        seg.t1 = stelnettts_vad_remap_timestamp(stitched.mapping, seg.t1);
        for (auto& w : seg.words) {
            w.t0 = stelnettts_vad_remap_timestamp(stitched.mapping, w.t0);
            w.t1 = stelnettts_vad_remap_timestamp(stitched.mapping, w.t1);
        }
    }
...
CA_EXPORT stelnettts_session_result* stelnettts_session_transcribe_vad(stelnettts_session* s, const float* pcm, int n_samples,
                                                                   int sample_rate, const char* vad_model_path,
                                                                   const stelnettts_vad_abi_opts* opts_or_null) {
    return stelnettts_session_transcribe_vad_lang(s, pcm, n_samples, sample_rate, vad_model_path, opts_or_null, nullptr);
}
...
struct stelnettts_diarize_opts_abi {
    int32_t method; // 0..4 from stelnettts_diarize_method_t
    int32_t n_threads;
    int64_t slice_t0_cs;
...
// (stelnettts-sys `diarize_abi_layout`, flutter's 48-byte buffer) with it.
//
...
static_assert(!k_abi_is_64bit || sizeof(stelnettts_diarize_seg_abi) == 24,
              "diarize seg ABI layout changed — update every binding mirror");
static_assert(!k_abi_is_64bit || sizeof(stelnettts_diarize_opts_abi) == 48,
              "diarize opts ABI layout changed — update every binding mirror");
static_assert(!k_abi_is_64bit || offsetof(stelnettts_diarize_opts_abi, pyannote_model_path) == 16,
              "diarize opts ABI layout drifted");
static_assert(!k_abi_is_64bit || offsetof(stelnettts_diarize_opts_abi, foxnose_embedder_path) == 24,
              "diarize opts ABI layout drifted");
static_assert(!k_abi_is_64bit || offsetof(stelnettts_diarize_opts_abi, min_speakers) == 32,
              "diarize opts ABI layout drifted");

CA_EXPORT int stelnettts_diarize_segments_abi(const float* left_pcm, const float* right_pcm, int32_t n_samples,
                                            int32_t is_stereo, stelnettts_diarize_seg_abi* segs, int32_t n_segs,
                                            const stelnettts_diarize_opts_abi* opts) {
    ...
    const bool ok = stelnettts_diarize_segments(left_pcm, r, n_samples, is_stereo != 0, lib_segs, lib_opts);
    if (!ok)
        return 1;
...
// ---------------------------------------------------------------------------
CA_EXPORT int stelnettts_detect_language_pcm(const float* samples, int32_t n_samples,
...
// ---------------------------------------------------------------------------
CA_EXPORT int stelnettts_enhance_audio_rnnoise(const float* in_pcm, int32_t n_samples, float* out_pcm, int32_t out_cap) {
#if defined(STELNETTTS_RNNOISE)
...
// `stelnettts_detect_language_pcm` shape: caller supplies a model
...
CA_EXPORT int stelnettts_text_detect_language(const char* text, const char* model_path, int32_t n_threads,
...
//
// Runs a CTC aligner (canary-ctc by default, cielvox2-forced-aligner when
...
// frees with `stelnettts_align_result_free`. Accessors below mirror the
...
CA_EXPORT stelnettts_align_result* stelnettts_align_words_abi(const char* aligner_model, const char* transcript,
    ...
    if (!aligner_model || !transcript || !samples || n_samples <= 0)
        return nullptr;
    auto* r = new stelnettts_align_result();
    r->words =
        stelnettts_align_words(aligner_model, transcript, samples, n_samples, t_offset_cs, n_threads > 0 ? n_threads : 4);
    if (r->words.empty()) {
        delete r;
        return nullptr;
    }
...
CA_EXPORT int stelnettts_align_result_n_words(stelnettts_align_result* r) {
    return r ? (int)r->words.size() : 0;
}

CA_EXPORT const char* stelnettts_align_result_word_text(stelnettts_align_result* r, int i) {
    return (r && i >= 0 && i < (int)r->words.size()) ? r->words[i].text.c_str() : "";
}

CA_EXPORT int64_t stelnettts_align_result_word_t0(stelnettts_align_result* r, int i) {
    return (r && i >= 0 && i < (int)r->words.size()) ? r->words[i].t0_cs : 0;
}

CA_EXPORT int64_t stelnettts_align_result_word_t1(stelnettts_align_result* r, int i) {
    return (r && i >= 0 && i < (int)r->words.size()) ? r->words[i].t1_cs : 0;
}

CA_EXPORT void stelnettts_align_result_free(stelnettts_align_result* r) {
    if (r)
        delete r;
}
...
// (~/.cache/stelnettts on POSIX, %USERPROFILE%/.cache/stelnettts on Windows).
// ---------------------------------------------------------------------------
CA_EXPORT int stelnettts_cache_ensure_file_abi(const char* filename, const char* url, int32_t quiet,
    ...
    const std::string path = stelnettts_cache::ensure_cached_file(filename, url, quiet != 0, "stelnettts", override_s);
    if (path.empty())
        return 1;
...
CA_EXPORT int stelnettts_cache_dir_abi(const char* cache_dir_override, char* out_buf, int32_t out_cap) {
    if (!out_buf || out_cap <= 0)
        return -1;
    const std::string override_s = cache_dir_override ? cache_dir_override : "";
    const std::string d = stelnettts_cache::dir(override_s);
    if (d.empty())
        return 1;
...
CA_EXPORT int stelnettts_registry_lookup_abi(const char* backend, char* out_filename, int32_t filename_cap, char* out_url,
                                           int32_t url_cap, char* out_size, int32_t size_cap) {
...
CA_EXPORT int stelnettts_registry_lookup_by_filename_abi(const char* filename, char* out_filename, int32_t filename_cap,
...
// stelnettts_registry_lookup_abi(name) for full details. Mirrors the
// shape of stelnettts_session_available_backends().
CA_EXPORT int stelnettts_registry_list_backends_abi(char* out_csv, int32_t out_cap) {
    if (!out_csv || out_cap <= 0)
        return -1;
    std::string acc;
...
CA_EXPORT int stelnettts_registry_default_bundle_info_abi(const char* backend, char* out_backend, int32_t backend_cap,
...
CA_EXPORT int stelnettts_registry_default_bundle_artifact_abi(const char* backend, int32_t index, int32_t* out_kind,
...
CA_EXPORT int stelnettts_session_result_n_segments(stelnettts_session_result* r) {
    return r ? (int)r->segments.size() : 0;
}
CA_EXPORT const char* stelnettts_session_result_segment_text(stelnettts_session_result* r, int i) {
    return (r && i >= 0 && i < (int)r->segments.size()) ? r->segments[i].text.c_str() : "";
}
CA_EXPORT int64_t stelnettts_session_result_segment_t0(stelnettts_session_result* r, int i) {
    return (r && i >= 0 && i < (int)r->segments.size()) ? r->segments[i].t0 : 0;
}
CA_EXPORT int64_t stelnettts_session_result_segment_t1(stelnettts_session_result* r, int i) {
    return (r && i >= 0 && i < (int)r->segments.size()) ? r->segments[i].t1 : 0;
}
CA_EXPORT const char* stelnettts_session_result_segment_speaker(stelnettts_session_result* r, int i) {
    return (r && i >= 0 && i < (int)r->segments.size()) ? r->segments[i].speaker.c_str() : "";
}
CA_EXPORT int stelnettts_session_result_n_words(stelnettts_session_result* r, int i_seg) {
    if (!r || i_seg < 0 || i_seg >= (int)r->segments.size())
        return 0;
    return (int)r->segments[i_seg].words.size();
}
CA_EXPORT const char* stelnettts_session_result_word_text(stelnettts_session_result* r, int i_seg, int i_word) {
    if (!r || i_seg < 0 || i_seg >= (int)r->segments.size())
        return "";
    auto& ws = r->segments[i_seg].words;
    return (i_word >= 0 && i_word < (int)ws.size()) ? ws[i_word].text.c_str() : "";
}
CA_EXPORT int64_t stelnettts_session_result_word_t0(stelnettts_session_result* r, int i_seg, int i_word) {
    if (!r || i_seg < 0 || i_seg >= (int)r->segments.size())
        return 0;
    auto& ws = r->segments[i_seg].words;
    return (i_word >= 0 && i_word < (int)ws.size()) ? ws[i_word].t0 : 0;
}
CA_EXPORT int64_t stelnettts_session_result_word_t1(stelnettts_session_result* r, int i_seg, int i_word) {
    if (!r || i_seg < 0 || i_seg >= (int)r->segments.size())
        return 0;
    auto& ws = r->segments[i_seg].words;
...
// from "100% confident".
CA_EXPORT float stelnettts_session_result_word_p(stelnettts_session_result* r, int i_seg, int i_word) {
    if (!r || i_seg < 0 || i_seg >= (int)r->segments.size())
        return -1.0f;
    auto& ws = r->segments[i_seg].words;
...
CA_EXPORT float stelnettts_session_result_segment_no_speech_prob(stelnettts_session_result* r, int i_seg) {
    if (!r || i_seg < 0 || i_seg >= (int)r->segments.size())
        return -1.0f;
...
// stelnettts_session_result_logits returns a frame-major buffer
...
// canary-ctc. Owned by `r`; freed with stelnettts_session_result_free.
CA_EXPORT int stelnettts_session_result_n_logit_frames(stelnettts_session_result* r) {
    return r ? r->n_logit_frames : 0;
}
CA_EXPORT int stelnettts_session_result_n_logit_vocab(stelnettts_session_result* r) {
    return r ? r->n_logit_vocab : 0;
}
CA_EXPORT const float* stelnettts_session_result_logits(stelnettts_session_result* r) {
...
// descending by p.
CA_EXPORT int stelnettts_session_result_word_n_alts(stelnettts_session_result* r, int i_seg, int i_word) {
    if (!r || i_seg < 0 || i_seg >= (int)r->segments.size())
        return 0;
    auto& ws = r->segments[i_seg].words;
...
CA_EXPORT const char* stelnettts_session_result_word_alt_text(stelnettts_session_result* r, int i_seg, int i_word,
                                                            int i_alt) {
    if (!r || i_seg < 0 || i_seg >= (int)r->segments.size())
        return "";
    auto& ws = r->segments[i_seg].words;
...
CA_EXPORT float stelnettts_session_result_word_alt_p(stelnettts_session_result* r, int i_seg, int i_word, int i_alt) {
    if (!r || i_seg < 0 || i_seg >= (int)r->segments.size())
        return 0.0f;
    auto& ws = r->segments[i_seg].words;
    if (i_word < 0 || i_word >= (int)ws.size())
...
CA_EXPORT void stelnettts_session_result_free(stelnettts_session_result* r) {
    if (r)
        delete r;
}

// ---------------------------------------------------------------------------
// TTS synthesis (vibevoice, cielvox2-tts)
// ---------------------------------------------------------------------------
//
// `stelnettts_session_synthesize` returns malloc'd float32 PCM at 24 kHz mono.
// `*out_n_samples` is set on success. Caller frees with `stelnettts_pcm_free`.
// Returns nullptr if the active backend doesn't support TTS or synthesis fails.
//
// `stelnettts_session_set_voice` accepts the active backend's native voice
...
// `ref_text_or_null`.
//
// `stelnettts_session_set_codec_path` forwards the companion GGUF path to the
...
CA_EXPORT int stelnettts_session_set_parakeet_att_context(stelnettts_session* s, int left, int right) {
...
CA_EXPORT int stelnettts_session_set_codec_path(stelnettts_session* s, const char* path) {
    if (!s || !path)
        return -1;
#ifdef CA_HAVE_QWEN3_TTS
    if (s->cielvox2_tts_ctx)
    ...
    // through the shared codec-path setter, same as cielvox2-tts/orpheus.
    if (s->indextts_ctx)
...
// stelnettts_audio_load lives in stelnettts_audio.cpp (same shared lib);
// forward-declare it so set_voice can decode a reference WAV without
// pulling in the audio header.
// stelnettts_audio_load_at_rate resamples directly to the requested rate,
...
extern "C" int stelnettts_audio_load_at_rate(const char* path, int target_rate, float** out_pcm, int* out_samples,
                                           int* out_sample_rate);
#endif
...
CA_EXPORT int stelnettts_session_set_voice(stelnettts_session* s, const char* path, const char* ref_text_or_null) {
    if (!s || !path)
        return -1;
    ...
    // predicate reads the baker's stelnettts.voice.cloned_from_recording stamp.
    ...
    const stelnettts_voice::CloneDecision voice_decision = stelnettts_voice::classify_voice(
    ...
    if (s->cosyvoice3_ctx) {
        ...
        if (ends_with_wav(path) && s->cosyvoice3_ref_text.empty()) {
            ...
            const std::string cache_path = stelnettts_ref_cache::path_for(path, stelnettts_ref_cache::kCv3RefTextSuffix);
            std::vector<uint32_t> shape;
            std::vector<uint8_t> payload;
            if (!stelnettts_ref_cache::disabled() &&
                stelnettts_ref_cache::load(cache_path, path, stelnettts_ref_cache::kCv3RefTextSuffix, shape, payload) &&
                !payload.empty()) {
                s->cosyvoice3_ref_text.assign((const char*)payload.data(), payload.size());
                fprintf(stderr, "stelnettts[cosyvoice3-tts]: using cached ref transcript '%s': '%s'\n",
            ...
            fprintf(stderr,
                    "stelnettts[cosyvoice3-tts]: cloning from '%s' needs a transcript of that clip, and none "
                    "was given.\n"
                    "  Pass the exact transcript as the ref_text argument, or run the clip through the CLI "
                    "once\n"
                    "  (stelnettts --backend cosyvoice3-tts --voice %s ...) which transcribes it and caches the\n"
    ...
    if (s->cielvox2_tts_ctx) {
        if (ends_with_wav(path)) {
            if (!ref_text_or_null)
                return -2;
            int rc = cielvox2_tts_set_voice_prompt_with_text(s->cielvox2_tts_ctx, path, ref_text_or_null);
            if (rc == 0)
                s->cielvox2_tts_voice_loaded = true;
            return rc;
        }
        int rc = cielvox2_tts_load_voice_pack(s->cielvox2_tts_ctx, path);
        if (rc == 0)
            s->cielvox2_tts_voice_loaded = true;
        return rc;
    }
    ...
    if (s->tada_ctx) {
        ...
        // Opt-in, default OFF: without STELNETTTS_TADA_WAV_CLONE=1 the historical
        ...
        if (const char* g = stelnettts_env::get("STELNETTTS_TADA_WAV_CLONE"); !g || atoi(g) == 0) {
            fprintf(stderr, "stelnettts: tada .wav voice cloning is opt-in — set STELNETTTS_TADA_WAV_CLONE=1 "
                            "to enable (experimental; validate the output before relying on it)\n");
            return -2;
        }
        if (!ref_text_or_null || !ref_text_or_null[0]) {
            fprintf(stderr, "stelnettts: tada .wav voice cloning requires the reference transcript "
                            "(pass it as ref_text)\n");
            return -2;
        }
        ...
        const std::string cache = stelnettts_cache::dir();
        ...
        if (enc.empty() || ali.empty()) {
            fprintf(stderr, "stelnettts: tada .wav cloning needs the encoder + aligner GGUFs — place "
                            "tada-encoder-f16.gguf + tada-aligner-<lang>.gguf next to the model or in the cache "
                            "dir, or set them via stelnettts_session_tada_set_makeref_models()\n");
            return -3;
        }
        // Decode the reference WAV straight to 24 kHz mono (the encoder's rate).
        float* pcm = nullptr;
        int n = 0, sr = 0;
        if (stelnettts_audio_load_at_rate(path, 24000, &pcm, &n, &sr) != 0 || !pcm || n <= 0) {
            fprintf(stderr, "stelnettts: tada .wav cloning: failed to decode '%s'\n", path);
            if (pcm)
                free(pcm);
            return -1;
        }
    ...
    if (s->voxcpm2_ctx) {
        ...
        float* pcm = nullptr;
        int n = 0, sr = 0;
        if (stelnettts_audio_load(path, &pcm, &n, &sr) != 0 || !pcm || n <= 0) {
            if (pcm)
                free(pcm);
            return -1;
        }
...
// voice cloning (stelnettts_session_set_voice with a `.wav` + ref_text). Either
...
CA_EXPORT int stelnettts_session_tada_set_makeref_models(stelnettts_session* s, const char* encoder_gguf,
...
CA_EXPORT int stelnettts_session_set_tts_phonemes(stelnettts_session* s, const char* phonemes) {
    if (!s)
        return -1;
...
CA_EXPORT int stelnettts_session_set_speaker_name(stelnettts_session* s, const char* name) {
    if (!s || !name)
        return -1;
#ifdef CA_HAVE_ORPHEUS
    if (s->orpheus_ctx) {
...
#endif
#ifdef CA_HAVE_QWEN3_TTS
    if (s->cielvox2_tts_ctx && cielvox2_tts_is_custom_voice(s->cielvox2_tts_ctx)) {
        int rc = cielvox2_tts_set_speaker_by_name(s->cielvox2_tts_ctx, name);
        if (rc == 0)
            s->cielvox2_tts_voice_loaded = true;
        return rc;
    }
...
// [0, stelnettts_session_n_speakers() - 1].
//
...
// contract.
CA_EXPORT int stelnettts_session_set_speaker_id(stelnettts_session* s, int id) {
    if (!s)
        return -1;
    if (id < 0)
        return -2;
...
CA_EXPORT int stelnettts_session_n_speakers(stelnettts_session* s) {
    if (!s)
        return 0;
#ifdef CA_HAVE_ORPHEUS
    if (s->orpheus_ctx)
...
#ifdef CA_HAVE_QWEN3_TTS
    if (s->cielvox2_tts_ctx && cielvox2_tts_is_custom_voice(s->cielvox2_tts_ctx))
        return cielvox2_tts_n_speakers(s->cielvox2_tts_ctx);
#endif
#ifdef CA_HAVE_MELOTTS
    if (s->melotts_ctx)
...
CA_EXPORT const char* stelnettts_session_get_speaker_name(stelnettts_session* s, int i) {
    if (!s || i < 0)
        return nullptr;
#ifdef CA_HAVE_ORPHEUS
    if (s->orpheus_ctx)
...
#ifdef CA_HAVE_QWEN3_TTS
    if (s->cielvox2_tts_ctx && cielvox2_tts_is_custom_voice(s->cielvox2_tts_ctx))
        return cielvox2_tts_get_speaker_name(s->cielvox2_tts_ctx, i);
#endif
    return nullptr;
}

// Set the voice description / style instruct for instruct-capable TTS
// backends: cielvox2-tts VoiceDesign, parler-tts, omnivoice. Required before
// stelnettts_session_synthesize when the loaded backend is VoiceDesign.
//
// ⚠ The contract differs by backend. cielvox2-tts and parler take free
...
CA_EXPORT int stelnettts_session_set_instruct(stelnettts_session* s, const char* instruct) {
    if (!s || !instruct)
        return -1;
#ifdef CA_HAVE_QWEN3_TTS
    if (s->cielvox2_tts_ctx && cielvox2_tts_is_voice_design(s->cielvox2_tts_ctx)) {
        int rc = cielvox2_tts_set_instruct(s->cielvox2_tts_ctx, instruct);
        if (rc == 0)
            s->cielvox2_tts_voice_loaded = true;
        return rc;
    }
...
// Variant detection for the cielvox2-tts backend. Returns 0/1; 0 also
// covers "active backend isn't cielvox2-tts". Lets wrappers branch on
// which voice-prompt API to call (`set_voice` vs `set_speaker_name`
// vs `set_instruct`) without parsing GGUF metadata themselves.
CA_EXPORT int stelnettts_session_is_custom_voice(stelnettts_session* s) {
    if (!s)
        return 0;
#ifdef CA_HAVE_QWEN3_TTS
    if (s->cielvox2_tts_ctx)
        return cielvox2_tts_is_custom_voice(s->cielvox2_tts_ctx);
#endif
    return 0;
}

CA_EXPORT int stelnettts_session_is_voice_design(stelnettts_session* s) {
    if (!s)
        return 0;
#ifdef CA_HAVE_QWEN3_TTS
    if (s->cielvox2_tts_ctx)
        return cielvox2_tts_is_voice_design(s->cielvox2_tts_ctx);
#endif
    return 0;
}
...
#ifdef CA_HAVE_COSYVOICE3
static bool stelnettts_session_ensure_cosyvoice3_cloning_models(stelnettts_session* s) {
...
static float* stelnettts_session_synthesize_raw_impl(stelnettts_session* s, const char* text, int* out_n_samples) {
    if (out_n_samples)
        *out_n_samples = 0;
    if (!s || !text)
        return nullptr;
    ...
    if (s->cosyvoice3_ctx) {
        ...
        if (is_wav && !stelnettts_session_ensure_cosyvoice3_cloning_models(s))
            return nullptr;
    ...
    if (s->cielvox2_tts_ctx) {
        ...
        // without a voice reference — cielvox2_tts_synthesize_codes prints
        // to stderr (invisible on Android) and returns nullptr, which the
        // Dart side can only report as a generic "no audio produced".
        if (!s->cielvox2_tts_voice_loaded) {
            const bool is_cv = cielvox2_tts_is_custom_voice(s->cielvox2_tts_ctx);
            const bool is_vd = cielvox2_tts_is_voice_design(s->cielvox2_tts_ctx);
            if (!is_cv && !is_vd) {
                s->last_synth_error = "cielvox2-tts Base requires a voice — "
                                      "select a voice pack or reference WAV in the voice picker";
                return nullptr;
            }
        }
        // Output language: target_language → source_language. cielvox2-tts keys
        // its codec_language_names table by English name ("German", ...).
        {
            const std::string tts_lang = !s->target_language.empty() ? s->target_language : s->source_language;
            if (!tts_lang.empty() && tts_lang != "auto")
                cielvox2_tts_set_language_by_name(s->cielvox2_tts_ctx, ca_iso_to_english_lang(tts_lang).c_str());
        }
        float* pcm = cielvox2_tts_synthesize(s->cielvox2_tts_ctx, text, out_n_samples);
        if (!pcm && s->last_synth_error.empty()) {
            s->last_synth_error = "cielvox2-tts synthesis failed — "
    ...
    if (s->csm_tts_ctx) {
        ...
        // backends; PCM is malloc'd and freed via stelnettts_pcm_free.
    ...
    if (s->dia_tts_ctx) {
        ...
        // via stelnettts_pcm_free, same as the other TTS backends.
...
#ifdef CA_HAVE_PARLER
    if (s->parler_ctx) {
        // Parler TTS emits 24 kHz mono float; PCM is malloc'd, freed via stelnettts_pcm_free.
        return parler_tts_synthesize(s->parler_ctx, text, out_n_samples);
    }
#endif
#ifdef CA_HAVE_SPEECHT5
    if (s->speecht5_ctx) {
        // SpeechT5 emits 16 kHz mono float; PCM is malloc'd, freed via stelnettts_pcm_free.
...
#ifdef CA_HAVE_VOXCPM2
    if (s->voxcpm2_ctx) {
        // VoxCPM2 synthesises at 48 kHz mono; every other StelnetTTS TTS
    ...
    if (s->indextts_ctx) {
        ...
        // back to dummy conditioning. Buffer is malloc'd / stelnettts_pcm_free
...
// responsibility. REQUIRED before stelnettts_session_synthesize_raw() will return
...
// session.
CA_EXPORT int stelnettts_session_accept_marking_responsibility(stelnettts_session* s, const char* attestation) {
    if (!s)
        return -1;
...
static void stelnettts_session_warn_unmarked_clone(stelnettts_session* s) {
    ...
    // Art. 3(60) does not ask them to. See stelnettts_speaker_identity.h.
    const stelnettts_voice::SpeakerIdentity identity = stelnettts_voice::resolve_speaker_identity(
        s->speaker_identity_override, s->voice_pack_identity, stelnettts_voice::SpeakerIdentity::Unknown);
    if (!stelnettts_voice::requires_spoken_disclosure(s->voice_is_clone, identity))
        return;
    ...
    fprintf(stderr,
            ...
            "  stelnettts: warning: this output is watermarked, but the ABI does not prepend the\n"
            ...
            "  stelnettts_session_get_disclaimer_pcm() (before set_voice) or\n"
            "  stelnettts_session_disclaimer_text(); call\n"
            "  stelnettts_session_accept_marking_responsibility() to silence this.\n",
            ts, s->voice_is_clone ? "yes" : "no", stelnettts_voice::to_string(identity));
}
...
// themselves via stelnettts_watermark_embed(). Because it yields unmarked PCM it is
// HARD-REFUSED (returns nullptr) unless the integrator first attests via
// stelnettts_session_accept_marking_responsibility(). Most callers should use
// stelnettts_session_synthesize() instead, which auto-watermarks by default.
CA_EXPORT float* stelnettts_session_synthesize_raw(stelnettts_session* s, const char* text, int* out_n_samples) {
    if (!s) {
        if (out_n_samples)
            *out_n_samples = 0;
        return nullptr;
    ...
    if (!s->marking_responsibility_accepted) {
        s->last_synth_error = "stelnettts_session_synthesize_raw returns UNMARKED audio and requires a prior "
                              "stelnettts_session_accept_marking_responsibility() attestation (you accept the "
                              "AI-content marking/disclosure duty). Use stelnettts_session_synthesize() for "
                              "watermarked output.";
        if (out_n_samples)
            *out_n_samples = 0;
        return nullptr;
    }
    return stelnettts_session_synthesize_raw_impl(s, text, out_n_samples);
}
...
CA_EXPORT float* stelnettts_session_synthesize(stelnettts_session* s, const char* text, int* out_n_samples) {
    stelnettts_session_warn_unmarked_clone(s);
    float* pcm = stelnettts_session_synthesize_raw_impl(s, text, out_n_samples);
    if (pcm && out_n_samples && *out_n_samples > 0) {
        stelnettts_watermark_embed(pcm, *out_n_samples, -1.0f);
    }
    return pcm;
}
...
CA_EXPORT int stelnettts_session_set_speaker_identity(stelnettts_session* s, const char* identity) {
    if (!s)
        return -1;
    bool recognised = true;
    const stelnettts_voice::SpeakerIdentity parsed =
        stelnettts_voice::parse_speaker_identity(identity ? identity : "", &recognised);
    if (!recognised)
        return -2;
...
// server prepend (examples/cli/stelnettts_tts_disclaimer.h). Exposed so an
...
CA_EXPORT const char* stelnettts_session_disclaimer_text(void) {
...
// comments in stelnettts_backend_{dots_tts,irodori_tts,voxcpm2_tts}.cpp). On the
...
// stelnettts_pcm_free(). Sample rate is the backend-native one, same as
// stelnettts_session_synthesize().
CA_EXPORT float* stelnettts_session_get_disclaimer_pcm(stelnettts_session* s, int* out_n_samples) {
    if (out_n_samples)
        *out_n_samples = 0;
    if (!s)
        return nullptr;
    if (s->voice_is_clone) {
        s->last_synth_error = "stelnettts_session_get_disclaimer_pcm must be called BEFORE stelnettts_session_set_voice() "
    ...
    if (s->disclaimer_pcm.empty()) {
        int n = 0;
        float* fresh = stelnettts_session_synthesize_raw_impl(s, stelnettts_session_disclaimer_text(), &n);
        if (!fresh || n <= 0) {
            free(fresh);
            return nullptr;
        }
...
CA_EXPORT void stelnettts_pcm_free(float* pcm) {
    free(pcm);
}
...
// stelnettts_session_synthesize (backend-native; caller-known). The PCM passed to
...
CA_EXPORT int stelnettts_session_synthesize_streaming(stelnettts_session* s, const char* text, stelnettts_pcm_stream_cb cb,
                                                    void* user_data) {
    if (!s || !text || !cb)
        return -1;
    ...
    for (size_t i = 0; i < chunks.size(); i++) {
        int n = 0;
        float* pcm = stelnettts_session_synthesize(s, chunks[i].c_str(), &n);
...
static float* stelnettts_session_speech_to_speech_impl(stelnettts_session* s, const float* in_samples, int n_in_samples,
...
// consistent with the CLI/server and stelnettts_session_synthesize. There is no
...
CA_EXPORT float* stelnettts_session_speech_to_speech(stelnettts_session* s, const float* in_samples, int n_in_samples,
                                                   char** out_text, int* out_n_samples) {
    stelnettts_session_warn_unmarked_clone(s);
    float* pcm = stelnettts_session_speech_to_speech_impl(s, in_samples, n_in_samples, out_text, out_n_samples);
    if (pcm && out_n_samples && *out_n_samples > 0)
        stelnettts_watermark_embed(pcm, *out_n_samples, -1.0f);
    return pcm;
}
...
CA_EXPORT int stelnettts_session_set_hotwords(stelnettts_session* s, const char* hotwords, float boost) {
    if (!s)
        return -1;
...
CA_EXPORT const char* stelnettts_session_last_synth_error(stelnettts_session* s) {
    if (!s)
        return "";
...
CA_EXPORT char* stelnettts_session_translate_text(stelnettts_session* s, const char* text, const char* src_lang,
...
// Free a string returned by `stelnettts_session_translate_text`.  Mirrors
// the punc-side `stelnettts_punc_free_text` symmetric-ownership pattern —
...
CA_EXPORT void stelnettts_session_translate_text_free(char* text) {
    free(text);
}

// =========================================================================
// Streaming session API (PLAN #62b — generalize stream_open from
// whisper_context* to stelnettts_session*).
//
// Today only the whisper backend is wired through (it's what the
// rolling-window engine in `stelnettts_stream_*` was built for); future
// backends (moonshine-streaming, kyutai-stt, voxtral4b) plug in here
// by routing `stelnettts_session_stream_feed` to their native streaming
// entry points.
//
// Returns nullptr if the session's backend doesn't support streaming.
// =========================================================================

CA_EXPORT stelnettts_stream* stelnettts_session_stream_open(stelnettts_session* s, int n_threads, int step_ms, int length_ms,
                                                        int keep_ms, const char* language, int translate) {
    if (!s)
        return nullptr;
    if (s->whisper_ctx)
        return stelnettts_stream_open(s->whisper_ctx, n_threads, step_ms, length_ms, keep_ms, language, translate);
#if __has_include("kyutai_stt.h")
    if (s->kyutai_ctx) {
...
CA_EXPORT int stelnettts_session_separate(stelnettts_session* s, const float* pcm_stereo, int n_samples) {
...
CA_EXPORT int stelnettts_session_separate_n_stems(stelnettts_session* s) {
    if (!s)
        return 0;
#ifdef CA_HAVE_HTDEMUCS
    if (s->htdemucs_last_result)
...
CA_EXPORT const char* stelnettts_session_separate_stem_name(stelnettts_session* s, int stem_idx) {
    if (!s)
        return nullptr;
#ifdef CA_HAVE_HTDEMUCS
...
CA_EXPORT const float* stelnettts_session_separate_stem(stelnettts_session* s, int stem_idx, int* out_n_samples) {
    if (!s)
        return nullptr;
#ifdef CA_HAVE_HTDEMUCS
    if (s->htdemucs_last_result && stem_idx >= 0 && stem_idx < s->htdemucs_last_result->n_sources) {
...
CA_EXPORT int stelnettts_session_separate_sample_rate(stelnettts_session* s) {
    if (!s)
        return 0;
#ifdef CA_HAVE_HTDEMUCS
    if (s->htdemucs_ctx)
...
//
// Mirrors the separation block above: pitch frames are not stelnettts_segments,
// so they get their own entry points rather than riding on transcribe().
// ---------------------------------------------------------------------------

CA_EXPORT int stelnettts_session_pitch(stelnettts_session* s, const float* pcm_16k, int n_samples, float hop_ms) {
...
CA_EXPORT int stelnettts_session_pitch_n_frames(stelnettts_session* s) {
    if (!s)
        return 0;
#ifdef CA_HAVE_CREPE
...
CA_EXPORT int stelnettts_session_pitch_frame(stelnettts_session* s, int idx, float* out_time_ms, float* out_f0_hz,
...
CA_EXPORT const float* stelnettts_session_pitch_frames(stelnettts_session* s, int* out_n_frames) {
    if (out_n_frames)
        *out_n_frames = 0;
    if (!s)
        return nullptr;
...
CA_EXPORT int stelnettts_session_pitch_sample_rate(stelnettts_session* s) {
    if (!s)
        return 0;
#ifdef CA_HAVE_CREPE
    if (s->crepe_ctx)
...
// is ContentVec features, which StelnetTTS does not produce (the consumer owns
...
CA_EXPORT int stelnettts_session_convert(stelnettts_session* s, const float* content, int n_frames, const float* f0_hz,
...
CA_EXPORT const float* stelnettts_session_convert_audio(stelnettts_session* s, int* out_n_samples) {
    if (out_n_samples)
        *out_n_samples = 0;
    if (!s)
        return nullptr;
...
CA_EXPORT int stelnettts_session_convert_content_dim(stelnettts_session* s) {
#ifdef CA_HAVE_RVC_SVC
    if (s && s->rvc_ctx)
...
CA_EXPORT int stelnettts_session_convert_n_speakers(stelnettts_session* s) {
#ifdef CA_HAVE_RVC_SVC
    if (s && s->rvc_ctx)
...
CA_EXPORT int stelnettts_session_convert_sample_rate(stelnettts_session* s) {
#ifdef CA_HAVE_RVC_SVC
    if (s && s->rvc_ctx)
...
//
// A chord timeline is not stelnettts_segments either, so it follows pitch and
...
CA_EXPORT int stelnettts_session_tab(stelnettts_session* s, const float* pcm, int n_samples, int sample_rate) {
    if (!s || !pcm || n_samples <= 0 || sample_rate <= 0)
        return -1;
#ifdef CA_HAVE_TABCNN
    if (s->tabcnn_ctx) {
...
CA_EXPORT int stelnettts_session_tab_n_frames(stelnettts_session* s) {
    if (!s)
        return 0;
#ifdef CA_HAVE_TABCNN
...
// the next stelnettts_session_tab call or session close.
CA_EXPORT const float* stelnettts_session_tab_emissions(stelnettts_session* s, int* out_n_frames, int* out_n_strings,
                                                      int* out_n_classes) {
    if (out_n_frames)
        *out_n_frames = 0;
    if (out_n_strings)
...
CA_EXPORT int stelnettts_session_tab_silent_class(stelnettts_session* s) {
    if (!s)
        return -1;
#ifdef CA_HAVE_TABCNN
    if (s->tabcnn_ctx)
...
CA_EXPORT float stelnettts_session_tab_frame_period(stelnettts_session* s) {
    if (!s)
        return 0.0f;
#ifdef CA_HAVE_TABCNN
    if (s->tabcnn_ctx)
...
CA_EXPORT int stelnettts_session_tab_string_open_midi(stelnettts_session* s, int string) {
    if (!s)
        return -1;
#ifdef CA_HAVE_TABCNN
    if (s->tabcnn_ctx)
...
CA_EXPORT int stelnettts_session_chords(stelnettts_session* s, const float* pcm, int n_samples, int sample_rate) {
    if (!s || !pcm || n_samples <= 0 || sample_rate <= 0)
        return -1;
#ifdef CA_HAVE_BTC_CHORDS
    if (s->btc_ctx) {
...
CA_EXPORT int stelnettts_session_chords_n_spans(stelnettts_session* s) {
    if (!s)
        return 0;
#ifdef CA_HAVE_BTC_CHORDS
...
CA_EXPORT const float* stelnettts_session_chords_spans(stelnettts_session* s, int* out_n_spans) {
    if (out_n_spans)
        *out_n_spans = 0;
    if (!s)
        return nullptr;
...
CA_EXPORT const char* stelnettts_session_chords_span_name(stelnettts_session* s, int idx) {
    if (!s || idx < 0)
        return nullptr;
#ifdef CA_HAVE_BTC_CHORDS
...
CA_EXPORT int stelnettts_session_chords_vocab_size(stelnettts_session* s) {
    if (!s)
        return 0;
#ifdef CA_HAVE_BTC_CHORDS
    if (s->btc_ctx)
...
CA_EXPORT int stelnettts_session_beats(stelnettts_session* s, const float* pcm, int n_samples, int sample_rate) {
    if (!s || !pcm || n_samples <= 0 || sample_rate <= 0)
        return -1;
#ifdef CA_HAVE_BEAT_THIS
    if (s->beat_ctx) {
...
CA_EXPORT int stelnettts_session_beats_n_events(stelnettts_session* s) {
    if (!s)
        return 0;
#ifdef CA_HAVE_BEAT_THIS
...
CA_EXPORT const float* stelnettts_session_beats_events(stelnettts_session* s, int* out_n_events) {
    if (out_n_events)
        *out_n_events = 0;
    if (!s)
        return nullptr;
...
CA_EXPORT int stelnettts_session_beats_sample_rate(stelnettts_session* s) {
    if (!s)
        return 0;
#ifdef CA_HAVE_BEAT_THIS
    if (s->beat_ctx)
...
CA_EXPORT float stelnettts_session_beats_tempo_bpm(stelnettts_session* s) {
    if (!s)
        return 0.0f;
#ifdef CA_HAVE_BEAT_THIS
...
CA_EXPORT int stelnettts_session_piano(stelnettts_session* s, const float* pcm_16k, int n_samples) {
    if (!s || !pcm_16k || n_samples <= 0)
        return -1;
#ifdef CA_HAVE_PIANO_TRANSCRIPTION
    if (s->piano_ctx) {
        ...
        for (int i = 0; i < res.n_notes; i++) {
            ...
            // (stelnettts_session_pitch_frames is ms too) so convert once here
...
CA_EXPORT int stelnettts_session_piano_n_notes(stelnettts_session* s) {
    if (!s)
        return 0;
#ifdef CA_HAVE_PIANO_TRANSCRIPTION
...
CA_EXPORT const float* stelnettts_session_piano_notes(stelnettts_session* s, int* out_n_notes) {
    if (out_n_notes)
        *out_n_notes = 0;
    if (!s)
        return nullptr;
...
CA_EXPORT int stelnettts_session_piano_sample_rate(stelnettts_session* s) {
    if (!s)
        return 0;
#ifdef CA_HAVE_PIANO_TRANSCRIPTION
    if (s->piano_ctx)
...
CA_EXPORT void stelnettts_session_close(stelnettts_session* s) {
    if (!s)
        return;
    {
...
#ifdef CA_HAVE_QWEN3_TTS
    if (s->cielvox2_tts_ctx)
        cielvox2_tts_free(s->cielvox2_tts_ctx);
#endif
#ifdef CA_HAVE_MIOTTS
    if (s->miotts_ctx)
...
#ifdef CA_HAVE_FIREREDPUNC
CA_EXPORT void* stelnettts_punc_init(const char* model_path) {
    return (void*)fireredpunc_init(model_path);
}

CA_EXPORT const char* stelnettts_punc_process(void* ctx, const char* text) {
    return fireredpunc_process((fireredpunc_context*)ctx, text);
}

CA_EXPORT void stelnettts_punc_free_text(const char* text) {
    free(const_cast<char*>(text));
}

CA_EXPORT void stelnettts_punc_free(void* ctx) {
    fireredpunc_free((fireredpunc_context*)ctx);
}
#else
CA_EXPORT void* stelnettts_punc_init(const char*) {
    return nullptr;
}
CA_EXPORT const char* stelnettts_punc_process(void*, const char*) {
    return nullptr;
}
CA_EXPORT void stelnettts_punc_free_text(const char*) {}
CA_EXPORT void stelnettts_punc_free(void*) {}
#endif

// =========================================================================
// Truecaser — standalone text post-processing (init → process → free).
// Three backends: statistical (`truecaser`), BiLSTM (`truecaser_lstm`),
// and CRF (`truecaser_crf`). The `stelnettts_truecase_init` dispatcher
...
// string; free it with `stelnettts_truecase_free_text`.
// =========================================================================

#if defined(CA_HAVE_TRUECASER_LSTM)
CA_EXPORT void* stelnettts_truecase_init(const char* model_path) {
    // Prefer LSTM when available (97.9% F1 on German).
    return (void*)truecaser_lstm_init(model_path);
}
CA_EXPORT const char* stelnettts_truecase_process(void* ctx, const char* text) {
    return truecaser_lstm_process((truecaser_lstm_context*)ctx, text);
}
CA_EXPORT void stelnettts_truecase_free_text(const char* text) {
    free(const_cast<char*>(text));
}
CA_EXPORT void stelnettts_truecase_free(void* ctx) {
...
#else
CA_EXPORT void* stelnettts_truecase_init(const char*) {
    return nullptr;
}
CA_EXPORT const char* stelnettts_truecase_process(void*, const char*) {
    return nullptr;
}
CA_EXPORT void stelnettts_truecase_free_text(const char*) {}
CA_EXPORT void stelnettts_truecase_free(void*) {}
#endif
...
#ifdef CA_HAVE_PCS
CA_EXPORT void* stelnettts_pcs_init(const char* model_path) {
    return (void*)pcs_init(model_path);
}
CA_EXPORT const char* stelnettts_pcs_process(void* ctx, const char* text) {
    return pcs_process((pcs_context*)ctx, text);
}
CA_EXPORT void stelnettts_pcs_free_text(const char* text) {
    free(const_cast<char*>(text));
}
CA_EXPORT void stelnettts_pcs_free(void* ctx) {
    pcs_free((pcs_context*)ctx);
}
#else
CA_EXPORT void* stelnettts_pcs_init(const char*) {
    return nullptr;
}
CA_EXPORT const char* stelnettts_pcs_process(void*, const char*) {
    return nullptr;
}
CA_EXPORT void stelnettts_pcs_free_text(const char*) {}
CA_EXPORT void stelnettts_pcs_free(void*) {}
#endif
...
CA_EXPORT int stelnettts_transcribe_parallel(struct whisper_context* ctx, struct whisper_full_params params,
...
CA_EXPORT const char* stelnettts_c_api_version(void) {
    ...
    // 0.6.0 — Adds StelnetWeaver parity: stelnettts_get_progress /
    // stelnettts_reset_progress (atomic progress polling for Dart FFI),
    // stelnettts_audio_load_stereo (stereo PCM decode),
    // stelnettts_transcribe_parallel (whisper_full_parallel wrapper),
    // stelnettts_ctx_params_set_dtw / stelnettts_token_dtw_t (DTW
    // timestamp init + getter). Pure addition; no symbol renames.
    // 0.5.3 — Adds `stelnettts_truecase_*` (init/process/free/free_text)
    // and `stelnettts_pcs_*` (init/process/free/free_text) standalone
    // text post-processors. Pure addition; no symbol renames.
    // 0.5.2 — Adds `stelnettts_text_detect_language` (text-LID via the
    // internal `text_lid_dispatch` façade — CLD3 + GlotLID-V3 +
    // LID-176 routed by GGUF architecture).  Mirrors the audio-side
    // `stelnettts_detect_language_pcm` return-code contract.
    // 0.5.1 — Adds `stelnettts_session_translate_text_free`.
    // Pure addition; no symbol renames or signature changes.
    return "0.7.0";
}

// Backwards-compatibility alias. The Dart smoke test and any 0.4.x-era
// consumer probed `stelnettts_dart_helpers_version`. The symbol was renamed
// when the file moved to `stelnettts_c_api.cpp` (no longer Dart-specific).
// TODO: remove once all in-tree consumers are updated and a major-version
// bump is cut.
CA_EXPORT const char* stelnettts_dart_helpers_version(void) {
    return stelnettts_c_api_version();
}

// =========================================================================
// Kokoro per-language model + voice routing — re-exports of the
// stelnettts_kokoro_* helpers from src/kokoro.cpp so they're visible to
// every wrapper. See src/kokoro.h for full semantics. (PLAN #56 opt 2b)
// =========================================================================

#ifdef CA_HAVE_KOKORO
CA_EXPORT bool stelnettts_kokoro_lang_is_german_abi(const char* lang) {
    return stelnettts_kokoro_lang_is_german(lang);
}

CA_EXPORT bool stelnettts_kokoro_lang_has_native_voice_abi(const char* lang) {
    return stelnettts_kokoro_lang_has_native_voice(lang);
}

CA_EXPORT int stelnettts_kokoro_resolve_model_for_lang_abi(const char* model_path, const char* lang, char* out_path,
                                                         int out_path_len) {
    return stelnettts_kokoro_resolve_model_for_lang(model_path, lang, out_path, out_path_len);
}

CA_EXPORT int stelnettts_kokoro_resolve_fallback_voice_abi(const char* model_path, const char* lang, char* out_path,
                                                         int out_path_len, char* out_picked, int out_picked_len) {
    return stelnettts_kokoro_resolve_fallback_voice(model_path, lang, out_path, out_path_len, out_picked, out_picked_len);
}
...
CA_EXPORT int stelnettts_session_kokoro_clear_phoneme_cache(stelnettts_session* s) {
    if (!s)
        return -1;
    if (s->kokoro_ctx)
...
#else
CA_EXPORT bool stelnettts_kokoro_lang_is_german_abi(const char*) {
    return false;
}
CA_EXPORT bool stelnettts_kokoro_lang_has_native_voice_abi(const char*) {
    return false;
}
CA_EXPORT int stelnettts_kokoro_resolve_model_for_lang_abi(const char*, const char*, char*, int) {
    return 1;
}
CA_EXPORT int stelnettts_kokoro_resolve_fallback_voice_abi(const char*, const char*, char*, int, char*, int) {
    return 2;
}
CA_EXPORT int stelnettts_session_kokoro_clear_phoneme_cache(stelnettts_session*) {
    return 0;
}
#endif
...
CA_EXPORT int stelnettts_session_set_source_language(stelnettts_session* s, const char* lang) {
    if (!s)
        return -1;
...
CA_EXPORT int stelnettts_session_set_target_language(stelnettts_session* s, const char* lang) {
    if (!s)
        return -1;
...
// stelnettts_session_set_source_language already doubles as the output-language
// fallback.
CA_EXPORT int stelnettts_session_set_tts_reference_language(stelnettts_session* s, const char* lang) {
    if (!s)
        return -1;
...
CA_EXPORT int stelnettts_session_set_punctuation(stelnettts_session* s, int enable) {
    if (!s)
        return -1;
...
CA_EXPORT int stelnettts_session_set_punc_model(stelnettts_session* s, const char* punc_model) {
    if (!s)
        return -1;
    ...
    const stelnettts_punc_spec spec = stelnettts_resolve_punc_model(punc_model ? punc_model : "");
    if (spec.kind == stelnettts_punc_kind::none)
        return 0; // unloaded / disabled

    std::string path = spec.direct_path;
    if (path.empty() && !spec.cache_filename.empty())
        path = stelnettts_cache::ensure_cached_file(spec.cache_filename, spec.url, /*quiet=*/true, "stelnettts[punc]", "");
    if (path.empty())
        return -2;

    if (spec.kind == stelnettts_punc_kind::fireredpunc) {
#ifdef CA_HAVE_FIREREDPUNC
    ...
    if (spec.kind == stelnettts_punc_kind::pcs) {
#ifdef CA_HAVE_PCS
...
CA_EXPORT int stelnettts_session_set_translate(stelnettts_session* s, int enable) {
    if (!s)
        return -1;
...
// (granite / voxtral / voxtral4b / cielvox2-asr / glm-asr / gemma4-e2b /
...
CA_EXPORT int stelnettts_session_set_ask(stelnettts_session* s, const char* prompt) {
    if (!s)
        return -1;
    s->ask = prompt ? prompt : "";
    return 0;
...
CA_EXPORT int stelnettts_session_set_temperature(stelnettts_session* s, float temperature, uint64_t seed) {
    if (!s)
        return -1;
...
#ifdef CA_HAVE_QWEN3_TTS
    if (s->cielvox2_tts_ctx) {
        // cielvox2-tts's code-predictor sampler reads cparams.temperature
        // on every step (after the 0.6.2 wiring); 0.0 means "use the
        // upstream 0.9 default" — pass any other value to override.
        cielvox2_tts_set_temperature((cielvox2_tts_context*)s->cielvox2_tts_ctx, temperature);
        (void)seed;
        touched++;
    }
#endif
...
// silently no-op (rc=-2).
CA_EXPORT int stelnettts_session_set_tts_seed(stelnettts_session* s, uint64_t seed) {
    if (!s)
        return -1;
    int touched = 0;
#ifdef CA_HAVE_VOXTRAL_TTS
...
#ifdef CA_HAVE_QWEN3_TTS
    if (s->cielvox2_tts_ctx) {
        cielvox2_tts_set_seed((cielvox2_tts_context*)s->cielvox2_tts_ctx, seed);
        touched++;
    }
#endif
#ifdef CA_HAVE_MOSS_TTS
...
// ─────────────────────────────────────────────────────────────────
// StelnetTTS 0.6.1 parity additions — TTS sampling knobs reachable at
// runtime so the StelnetWeaver Synthesize screen can drive them
...
CA_EXPORT int stelnettts_session_set_tts_steps(stelnettts_session* s, int steps) {
    if (!s)
        return -1;
    int touched = 0;
#ifdef CA_HAVE_CHATTERBOX
...
// backends no-op (rc=-2).
CA_EXPORT int stelnettts_session_set_tts_cfg_scale(stelnettts_session* s, float scale) {
    if (!s)
        return -1;
    int touched = 0;
#ifdef CA_HAVE_VIBEVOICE
    ...
    if (s->irodori_ctx) {
        ...
        // STELNETTTS_IRODORI_CFG_SPEAKER — the single generic knob can't carry it (#241).
...
CA_EXPORT int stelnettts_session_set_tts_num_candidates(stelnettts_session* s, int n) {
    if (!s)
        return -1;
    int touched = 0;
#ifdef CA_HAVE_TADA
...
CA_EXPORT int stelnettts_session_set_g2p_dict(stelnettts_session* s, const char* source) {
    if (!s)
        return -1;
...
CA_EXPORT int stelnettts_session_set_top_p(stelnettts_session* s, float top_p) {
    if (!s)
        return -1;
    int touched = 0;
#ifdef CA_HAVE_CHATTERBOX
...
CA_EXPORT int stelnettts_session_set_min_p(stelnettts_session* s, float min_p) {
    if (!s)
        return -1;
    int touched = 0;
#ifdef CA_HAVE_CHATTERBOX
...
// chatterbox.
CA_EXPORT int stelnettts_session_set_repetition_penalty(stelnettts_session* s, float r) {
    if (!s)
        return -1;
    int touched = 0;
#ifdef CA_HAVE_CHATTERBOX
...
CA_EXPORT int stelnettts_session_set_top_k(stelnettts_session* s, int top_k) {
    if (!s)
        return -1;
    int touched = 0;
#ifdef CA_HAVE_TADA
...
CA_EXPORT int stelnettts_session_set_do_sample(stelnettts_session* s, int enable) {
    if (!s)
        return -1;
    int touched = 0;
#ifdef CA_HAVE_TADA
...
// conditional path.
CA_EXPORT int stelnettts_session_set_cfg_weight(stelnettts_session* s, float cfg_weight) {
    if (!s)
        return -1;
    int touched = 0;
#ifdef CA_HAVE_CHATTERBOX
...
CA_EXPORT int stelnettts_session_set_tts_noise_temp(stelnettts_session* s, float noise_temp) {
    if (!s)
        return -1;
    int touched = 0;
#ifdef CA_HAVE_TADA
...
// flat / monotone.
CA_EXPORT int stelnettts_session_set_exaggeration(stelnettts_session* s, float exaggeration) {
    if (!s)
        return -1;
    int touched = 0;
#ifdef CA_HAVE_CHATTERBOX
...
// hallucinations.
CA_EXPORT int stelnettts_session_set_max_speech_tokens(stelnettts_session* s, int n) {
    if (!s)
        return -1;
    int touched = 0;
#ifdef CA_HAVE_CHATTERBOX
...
CA_EXPORT int stelnettts_session_set_min_speech_tokens(stelnettts_session* s, int n) {
    if (!s)
        return -1;
    int touched = 0;
#ifdef CA_HAVE_MOSS_TTS
...
CA_EXPORT int stelnettts_session_set_length_scale(stelnettts_session* s, float scale) {
    if (!s)
        return -1;
    int touched = 0;
#ifdef CA_HAVE_KOKORO
...
CA_EXPORT int stelnettts_session_set_best_of(stelnettts_session* s, int n) {
    if (!s)
        return -1;
    s->best_of = n > 0 ? n : 1;
    return 0;
...
CA_EXPORT int stelnettts_session_set_max_new_tokens(stelnettts_session* s, int n) {
    if (!s)
        return -1;
...
CA_EXPORT int stelnettts_session_set_frequency_penalty(stelnettts_session* s, float penalty) {
    if (!s)
        return -1;
...
//   whisper (native BEAM_SEARCH), cielvox2-asr / granite / voxtral (replay via
...
CA_EXPORT int stelnettts_session_set_beam_size(stelnettts_session* s, int n) {
    if (!s)
        return -1;
    s->beam_size = n > 0 ? n : 1;
    return 0;
...
CA_EXPORT int stelnettts_session_set_return_logits(stelnettts_session* s, int enable) {
    if (!s)
        return -1;
...
CA_EXPORT int stelnettts_session_set_grammar_text(stelnettts_session* s, const char* gbnf_text, const char* root_rule,
                                                float penalty) {
    if (!s)
        return -1;
...
CA_EXPORT int stelnettts_session_set_fallback_thresholds(stelnettts_session* s, float entropy_thold, float logprob_thold,
...
CA_EXPORT int stelnettts_session_set_sensitivity(stelnettts_session* s, const char* preset_name) {
    ...
    return stelnettts_session_set_fallback_thresholds(s, t.entropy_thold, t.logprob_thold, t.no_speech_thold,
                                                    t.temperature_inc);
}
...
// different API).
CA_EXPORT int stelnettts_session_set_alt_n(stelnettts_session* s, int n) {
    if (!s)
        return -1;
...
CA_EXPORT int stelnettts_session_set_whisper_decode_extras(stelnettts_session* s, int suppress_nst,
...
// standalone stelnettts_detect_language() from src/stelnettts_lid.h so
...
// failed.
CA_EXPORT int stelnettts_session_detect_language(stelnettts_session* s, const float* pcm, int n_samples,
...
CA_EXPORT void* stelnettts_titanet_init(const char* model_path, int32_t n_threads) {
    return (void*)titanet_init(model_path, n_threads);
}

CA_EXPORT void stelnettts_titanet_free(void* ctx) {
...
CA_EXPORT int32_t stelnettts_titanet_embed(void* ctx, const float* pcm_16k, int32_t n_samples, float* out) {
    return (int32_t)titanet_embed((struct titanet_context*)ctx, pcm_16k, n_samples, out);
}

CA_EXPORT float stelnettts_titanet_cosine_sim(const float* a, const float* b, int32_t dim) {
...
CA_EXPORT void* stelnettts_speaker_db_open(const char* dir_path, const char* expected_names_csv,
                                         int32_t consent_attested) {
    if (!consent_attested) {
        fprintf(stderr, "stelnettts: speaker_db_open refused: matching named voiceprints is biometric\n"
    ...
    if (!expected_names_csv || !*expected_names_csv) {
        fprintf(stderr, "stelnettts: speaker_db_open refused: a closed roster of claimed participants is\n"
...
CA_EXPORT void* stelnettts_speaker_db_load(const char* dir_path) {
    (void)dir_path;
    fprintf(stderr, "stelnettts: stelnettts_speaker_db_load was removed (#266): open 1:N identification is\n"
                    "  unsupported. Use stelnettts_speaker_db_open(dir, expected_names_csv, consent_attested)\n");
    return nullptr;
}

CA_EXPORT void stelnettts_speaker_db_free(void* db) {
    speaker_db_free((struct speaker_db*)db);
}

CA_EXPORT int32_t stelnettts_speaker_db_count(const void* db) {
...
CA_EXPORT float stelnettts_speaker_db_match(const void* db, const float* embedding, int32_t dim, float threshold,
...
CA_EXPORT int32_t stelnettts_speaker_db_enroll2(const char* dir_path, const char* name, const float* embedding,
...
CA_EXPORT int32_t stelnettts_speaker_db_enroll(const char* dir_path, const char* name, const float* embedding,
                                             int32_t dim) {
    (void)dir_path;
    (void)name;
    (void)embedding;
    (void)dim;
    fprintf(stderr, "stelnettts: stelnettts_speaker_db_enroll was removed (#266): enrollment requires a consent\n"
                    "  attestation. Use stelnettts_speaker_db_enroll2(dir, name, emb, dim, consent_attested)\n");
    return 2;
}
...
// failure. See stelnettts_speaker_embedder.h for the dispatch rules.
CA_EXPORT void* stelnettts_speaker_embedder_make_abi(const char* model_spec, int32_t n_threads, const char* cache_dir) {
    if (!model_spec)
        return nullptr;
    auto p =
        stelnettts_make_speaker_embedder(std::string(model_spec), n_threads, cache_dir ? std::string(cache_dir) : "");
...
CA_EXPORT int32_t stelnettts_speaker_embedder_embed_abi(void* embedder, const float* pcm_16k, int32_t n_samples,
                                                      float* out) {
...
// -1 on invalid args.
CA_EXPORT int32_t stelnettts_speaker_cluster_abi(const float* embeddings, int32_t n, int32_t dim, float merge_threshold,
    ...
    auto labels = stelnettts_agglomerative_cluster(e, n, dim, merge_threshold, max_speakers);
    int max_label = -1;
    for (int i = 0; i < n; i++) {
...
struct stelnettts_pyannote_cache_abi {
    std::vector<float> log_probs;
    int T = 0;
    double frame_dur_s = 0.0;
};

CA_EXPORT void* stelnettts_pyannote_cache_compute_abi(const float* full_audio, int32_t n_samples, const char* model_path,
                                                    int32_t n_threads) {
    ...
    auto* cache = new stelnettts_pyannote_cache_abi();
...
CA_EXPORT void stelnettts_pyannote_cache_free_abi(void* cache) {
    if (cache)
        delete static_cast<stelnettts_pyannote_cache_abi*>(cache);
}
...
CA_EXPORT int32_t stelnettts_pyannote_cache_apply_abi(const void* cache, int64_t slice_t0_cs,
    ...
    for (int i = 0; i < n_segs; i++)
        lib_segs.push_back({segs[i].t0_cs, segs[i].t1_cs, segs[i].speaker});
    stelnettts_diarize_internal::assign_speakers_from_log_posteriors(c->log_probs.data(), c->T, c->frame_dur_s,
