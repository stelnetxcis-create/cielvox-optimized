...
// `stelnettts_session_transcribe` a clean 16-kHz mono float32 buffer
...
// as a miniaudio custom backend (libopus/opusfile, STELNETTTS_HAVE_OPUS; see
...
// STELNETTTS_AAC_DECODER, glint primary); container AAC (M4A/ALAC/CAF) still
...
// Device IO (capture mode) is needed for `stelnettts_mic_*` (PLAN #62d);
...
// The stelnettts_mic_* C ABI is stubbed out to return failure on the
// same platforms (see stelnettts_mic.cpp). macOS keeps device IO.
//
...
// STELNETTTS_HAVE_OPUS, which CMake sets when opusfile is found (pkg-config) or
// built statically (FetchContent fallback for platforms without system libs).
#if defined(STELNETTTS_HAVE_OPUS)
#include "miniaudio_libopus.h"
namespace {
// ma_decoding_backend_libopus is already a `ma_decoding_backend_vtable*`.
ma_decoding_backend_vtable* g_stelnettts_opus_backends[] = {ma_decoding_backend_libopus};
} // namespace
#define STELNETTTS_OPUS_DECODER_CONFIG(cfg)                                                                              \
    do {                                                                                                               \
        (cfg).ppCustomBackendVTables = g_stelnettts_opus_backends;                                                       \
...
// pulling in the full stelnettts.h header (which conflicts with
// miniaudio's implementation-mode defines in this TU).
CA_EXPORT int stelnettts_audio_load(const char*, float**, int*, int*);
CA_EXPORT int stelnettts_audio_load_at_rate(const char*, int, float**, int*, int*);
CA_EXPORT int stelnettts_audio_load_stereo(const char*, float**, float**, int*, int*, int*);
CA_EXPORT void stelnettts_audio_free(float*);

namespace {
// Thread-local so stelnettts_audio_load_at_rate can set the target rate for
// the current decode without affecting other threads.  Default is 16 kHz
// (Whisper-compatible) — overridden by stelnettts_audio_load_at_rate().
...
namespace {
int stelnettts_at_decode(const char* path, int want_channels, float** out_interleaved, int* out_frames,
                       int* out_channels) {
...
int stelnettts_au_decode(const char* path, int want_channels, float** out_buf, int* out_frames, int* out_channels) {
...
#if defined(STELNETTTS_HAVE_AMR)
...
int stelnettts_amr_decode(const char* path, int want_channels, float** out_buf, int* out_frames, int* out_channels) {
    FILE* f = std::fopen(path, "rb");
    if (!f)
        return -2;
...
// in-tree; libopus is an optional fallback (built with STELNETTTS_OPUS, selected
// via STELNETTTS_OPUS_DECODER=libopus).
#if defined(STELNETTTS_HAVE_OPUS)
...
int stelnettts_webm_decode(const char* path, int want_channels, float** out_buf, int* out_frames, int* out_channels) {
    // Read entire file
    FILE* f = std::fopen(path, "rb");
    if (!f)
        return -2;
    ...
    if (is_opus) {
        ...
        // (RFC-conformant, no libopus). STELNETTTS_OPUS_DECODER=libopus selects
        // libopus (only when built with STELNETTTS_OPUS).
        bool use_libopus = false;
#if defined(STELNETTTS_HAVE_OPUS)
        if (const char* pref = std::getenv("STELNETTTS_OPUS_DECODER"))
        ...
        if (use_libopus) {
            ...
            if (const char* e = std::getenv("STELNETTTS_OPUS_DEBUG"); e && e[0] && e[0] != '0')
...
// installed, stelnettts_m4a_decode returns -2 and the caller falls through to
...
int stelnettts_m4a_decode(const char* path, int want_channels, float** out_buf, int* out_frames, int* out_channels) {
...
namespace {
int stelnettts_mf_decode(const char* path, int want_channels, float** out_buf, int* out_frames, int* out_channels) {
...
#if defined(STELNETTTS_HAVE_MEDIA_NDK)
#include <media/NdkMediaCodec.h>
#include <media/NdkMediaExtractor.h>
#include <media/NdkMediaFormat.h>

namespace {
int stelnettts_ndk_decode(const char* path, int want_channels, float** out_buf, int* out_frames, int* out_channels) {
...
#endif // STELNETTTS_HAVE_MEDIA_NDK
...
/// (libopus/opusfile, when STELNETTTS_HAVE_OPUS), Sun AU / .snd (inline, µ-law /
/// A-law / PCM), AMR-NB/WB (opencore-amr, when STELNETTTS_HAVE_AMR),
/// WebM/Matroska Opus|Vorbis (EBML demux, when STELNETTTS_HAVE_OPUS),
...
//
// Gated per the A/B convention: set STELNETTTS_AAC_DECODER=fdk|coreaudio|os to
...
// removed. STELNETTTS_AAC_DEBUG=1 prints a one-line decode summary.
//
...
int stelnettts_adts_decode_glint(const char* path, int want_channels, float** out_buf, int* out_frames,
                               int* out_channels) {
    (void)want_channels; // 16 kHz ASR path is mono; downmix here

    if (const char* pref = std::getenv("STELNETTTS_AAC_DECODER")) {
        if (pref[0] && std::strcmp(pref, "glint") != 0 && std::strcmp(pref, "auto") != 0)
            return -2; // user pinned a non-glint decoder
    }
    const bool dbg = [] {
        const char* e = std::getenv("STELNETTTS_AAC_DEBUG");
        return e && e[0] && e[0] != '0';
    }();
...
// STELNETTTS_OPUS_DECODER pins libopus — so .opus reads even in builds without
// libopus (STELNETTTS_OPUS=OFF). Returns -2 for non-Opus Ogg (e.g. Vorbis) so
...
int stelnettts_ogg_opus_decode_glint_16k(const char* path, float** out_itl, int* out_frames, int* out_channels) {
    if (const char* pref = std::getenv("STELNETTTS_OPUS_DECODER")) {
        if (pref[0] && std::strcmp(pref, "glint") != 0 && std::strcmp(pref, "auto") != 0)
            return -2; // user pinned libopus
    }
    const bool dbg = [] {
        const char* e = std::getenv("STELNETTTS_OPUS_DEBUG");
        return e && e[0] && e[0] != '0';
    }();
...
int stelnettts_ogg_opus_decode_glint(const char* path, int want_channels, float** out_buf, int* out_frames,
                                   int* out_channels) {
    (void)want_channels; // 16 kHz ASR path is mono; downmix here
    float* itl = nullptr;
    int frames = 0, ch = 0;
    int rc = stelnettts_ogg_opus_decode_glint_16k(path, &itl, &frames, &ch);
    if (rc != 0)
        return rc;
...
CA_EXPORT int stelnettts_audio_load(const char* path, float** out_pcm, int* out_samples, int* out_sample_rate) {
    ...
    *out_samples = 0;
    ...
    // libopus required, so .opus reads even in builds without STELNETTTS_OPUS.
    ...
    // Non-Opus Ogg (Vorbis) and non-Ogg inputs return -2 → normal path below.
    {
        float* g_buf = nullptr;
        int g_fr = 0, g_ch = 0;
        if (stelnettts_ogg_opus_decode_glint(path, 1, &g_buf, &g_fr, &g_ch) == 0) {
            *out_pcm = g_buf;
            *out_samples = g_fr;
            if (out_sample_rate)
    ...
    if (ma_decoder_init_file(path, &cfg, &decoder) != MA_SUCCESS) {
        ...
        float* itl = nullptr;
        int fr = 0, ch = 0;
        if (stelnettts_at_decode(path, 1, &itl, &fr, &ch) == 0) {
            *out_pcm = itl;
            *out_samples = fr;
            if (out_sample_rate)
                *out_sample_rate = kTargetSampleRate;
        ...
        // AU / .snd fallback (inline, no external deps)
        {
            ...
            int au_fr = 0, au_ch = 0;
            if (stelnettts_au_decode(path, 1, &au_buf, &au_fr, &au_ch) == 0) {
                *out_pcm = au_buf;
                *out_samples = au_fr;
                if (out_sample_rate)
                    *out_sample_rate = kTargetSampleRate;
        ...
        // AMR-NB/WB fallback (opencore-amr, Apache-2.0)
        {
            ...
            if (stelnettts_amr_decode(path, 1, &amr_buf, &amr_fr, &amr_ch) == 0) {
                *out_pcm = amr_buf;
                *out_samples = amr_fr;
                if (out_sample_rate)
                    *out_sample_rate = kTargetSampleRate;
        ...
        // STELNETTTS_OPUS_DECODER=libopus; Vorbis via stb_vorbis). Now always
        // available — no libopus required.
        {
            float* webm_buf = nullptr;
            int webm_fr = 0, webm_ch = 0;
            if (stelnettts_webm_decode(path, 1, &webm_buf, &webm_fr, &webm_ch) == 0) {
                *out_pcm = webm_buf;
                *out_samples = webm_fr;
                if (out_sample_rate)
                    *out_sample_rate = kTargetSampleRate;
        ...
        // M4A / AAC / ADTS fallback (fdk-aac via dlopen, if installed)
        {
            ...
            if (stelnettts_m4a_decode(path, 1, &aac_buf, &aac_fr, &aac_ch) == 0) {
                *out_pcm = aac_buf;
                *out_samples = aac_fr;
                if (out_sample_rate)
                    *out_sample_rate = kTargetSampleRate;
        ...
        // Windows Media Foundation fallback for AAC/M4A
        {
            ...
            int mf_fr = 0, mf_ch = 0;
            if (stelnettts_mf_decode(path, 1, &mf_buf, &mf_fr, &mf_ch) == 0) {
                *out_pcm = mf_buf;
                *out_samples = mf_fr;
                if (out_sample_rate)
                    *out_sample_rate = kTargetSampleRate;
        ...
        // Android NDK MediaCodec fallback for AAC/M4A
        {
            ...
            if (stelnettts_ndk_decode(path, 1, &ndk_buf, &ndk_fr, &ndk_ch) == 0) {
                *out_pcm = ndk_buf;
                *out_samples = ndk_fr;
                if (out_sample_rate)
                    *out_sample_rate = kTargetSampleRate;
...
CA_EXPORT int stelnettts_audio_load_at_rate(const char* path, int target_rate, float** out_pcm, int* out_samples,
                                          int* out_sample_rate) {
    const int prev = kTargetSampleRate;
    kTargetSampleRate = (target_rate > 0) ? target_rate : 16000;
    const int rc = stelnettts_audio_load(path, out_pcm, out_samples, out_sample_rate);
...
/// with `stelnettts_audio_free`.
///
/// Returns 0 on success, negative on error (same codes as
/// `stelnettts_audio_load`).
CA_EXPORT int stelnettts_audio_load_stereo(const char* path, float** out_left, float** out_right, int* out_samples,
    ...
    *out_channels = 0;
    ...
    // STELNETTTS_OPUS_DECODER=libopus; non-Opus Ogg / non-Ogg fall through.
    {
        float* itl = nullptr;
        int fr = 0, ch = 0;
        if (stelnettts_ogg_opus_decode_glint_16k(path, &itl, &fr, &ch) == 0) {
    ...
    STELNETTTS_OPUS_DECODER_CONFIG(probe_cfg);
    ma_decoder probe;
    if (ma_decoder_init_file(path, &probe_cfg, &probe) != MA_SUCCESS) {
        ...
        float* itl = nullptr;
        int fr = 0, ch = 0;
        if (stelnettts_at_decode(path, 0, &itl, &fr, &ch) == 0) {
            float* left = (float*)std::malloc((size_t)fr * sizeof(float));
            float* right = (float*)std::malloc((size_t)fr * sizeof(float));
            if (!left || !right) {
                std::free(itl);
        ...
        if (split_fallback(stelnettts_adts_decode_glint) == 0)
            return 0;
        // AU / .snd fallback
        if (split_fallback(stelnettts_au_decode) == 0)
            return 0;
#if defined(STELNETTTS_HAVE_AMR)
        if (split_fallback(stelnettts_amr_decode) == 0)
            return 0;
#endif
        // WebM/Matroska (glint Opus / stb_vorbis) — always available, no libopus.
        if (split_fallback(stelnettts_webm_decode) == 0)
            return 0;
#if !defined(__APPLE__) && !defined(__EMSCRIPTEN__)
        if (split_fallback(stelnettts_m4a_decode) == 0)
            return 0;
#endif
#if defined(STELNETTTS_HAVE_MEDIA_NDK)
        if (split_fallback(stelnettts_ndk_decode) == 0)
            return 0;
#endif
        return -2;
    }
...
/// Release a buffer allocated by `stelnettts_audio_load`.
CA_EXPORT void stelnettts_audio_free(float* pcm) {
    if (pcm)
        std::free(pcm);
}
