...
#include "stelnettts_vad_encdec.h"
...
#include "core/gpu_backend_pref.h" // stelnettts_init_gpu_backend (#305)
...
static std::vector<float> wvad_compute_mel(const float* pcm, int n_samples, const float* hann, const float* fb,
    ...
    if (getenv("STELNETTTS_VAD_ENCDEC_SERIAL_MEL") || n_mel_threads <= 1 || n_frames < 64) {
...
extern "C" struct whisper_vad_encdec_context* whisper_vad_encdec_init(const char* path) {
    ...
    // fall back to CPU. Opt out with STELNETTTS_VAD_ENCDEC_CPU=1 for A/B.
    ctx->backend = nullptr;
    if (!getenv("STELNETTTS_VAD_ENCDEC_CPU"))
    ...
    // identical (same F16 values). Opt out with STELNETTTS_VAD_ENCDEC_CONV_CAST=1.
    if (!getenv("STELNETTTS_VAD_ENCDEC_CONV_CAST") && m.conv1_w && m.conv2_w) {
...
// across windows; the STELNETTTS_VAD_ENCDEC_REBUILD path rebuilds per window (A/B).
...
static wvad_result wvad_forward(whisper_vad_encdec_context* ctx, const float* mel_data, int n_frames) {
    ...
    // regress those backends. Enable + measure with STELNETTTS_VAD_ENCDEC_PERSIST=1
    ...
    const bool rebuild = getenv("STELNETTTS_VAD_ENCDEC_PERSIST") == nullptr;
...
extern "C" int whisper_vad_encdec_detect(struct whisper_vad_encdec_context* ctx, const float* samples, int n_samples,
    ...
    const bool dbg = getenv("STELNETTTS_VAD_ENCDEC_DEBUG") != nullptr; // #305 A/B timing
    int64_t t_fwd_us = 0;
