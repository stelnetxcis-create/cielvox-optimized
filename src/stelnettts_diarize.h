// stelnettts_diarize.h — shared speaker diarization post-step.
//
...
//                 stelnettts `(speaker 0/1)` labelling.
...
// (examples/cli/stelnettts_diarize_cli.cpp). It shells out to an
// externally installed sherpa binary, which is CLI-shaped UX policy
// rather than a library responsibility.
//
// Shared by: the CLI, the C-ABI wrapper `stelnettts_diarize_segments` in
// stelnettts_c_api.cpp, and every language binding that calls through
// that wrapper.
