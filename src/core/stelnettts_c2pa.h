// stelnettts_c2pa.h — C2PA (Content Credentials) manifest signing for TTS output.
//
// Compile-time gated on STELNETTTS_HAVE_C2PA. When enabled, signs synthesized
...
//
// When STELNETTTS_HAVE_C2PA is not defined, the functions are no-ops that return
...
// Generate with scripts/generate-c2pa-cert.sh, or let StelnetTTS auto-provision a
// per-install self-signed cert (see stelnettts_c2pa_autocert() below).
...
#include "stelnettts_c2pa_default_cert.h" // bundled self-signed default cert (baked in)

// Native pure-C++ C2PA signer for WAV (no c2pa-rs). Always available; it is the
// primary path for WAV on every platform. Define STELNETTTS_NO_C2PA_NATIVE to opt
// out (e.g. a build that only wants MP3/M4A via c2pa-rs).
#ifndef STELNETTTS_NO_C2PA_NATIVE
#include "stelnettts_c2pa_native.h"
#endif

#ifdef STELNETTTS_HAVE_C2PA
#include <c2pa.h>
#endif

// The C2PA manifest JSON. digitalSourceType per IPTC vocabulary marks this as
// trained-algorithmic (AI-generated) media.
inline const char* stelnettts_c2pa_manifest_json() {
    return R"({
  "claim_generator": "StelnetTTS",
...
inline const char* stelnettts_c2pa_format_for_ext(const std::string& ext) {
    if (ext == "wav")
        return "audio/wav";
    if (ext == "mp3")
        return "audio/mpeg";
...
namespace stelnettts_c2pa_detail {
...
// C2PA MIME string (see stelnettts_c2pa_format_for_ext). On success `data` is
...
// signing fails.
inline bool stelnettts_c2pa_sign_pem(std::string& data, const char* format, const std::string& cert_pem,
...
#ifdef STELNETTTS_HAVE_C2PA
    using namespace stelnettts_c2pa_detail;
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    ...
    if (!signer) {
        fprintf(stderr, "stelnettts: C2PA signer init failed: %s\n", c2pa_error() ? c2pa_error() : "unknown");
        return false;
    }

    C2paBuilder* builder = c2pa_builder_from_json(stelnettts_c2pa_manifest_json());
    if (!builder) {
        fprintf(stderr, "stelnettts: C2PA builder init failed: %s\n", c2pa_error() ? c2pa_error() : "unknown");
...
#endif
    if (rc < 0) {
        fprintf(stderr, "stelnettts: C2PA signing failed (%s): %s\n", format, c2pa_error() ? c2pa_error() : "unknown");
        return false;
    }
    data.swap(out);
    return true;
...
inline bool stelnettts_c2pa_sign_buffer(std::string& data, const char* format, const std::string& cert_path,
    ...
    // of STELNETTTS_HAVE_C2PA (see stelnettts_c2pa_detail below).
    std::string cert = stelnettts_c2pa_detail::read_file(cert_path);
    std::string key = stelnettts_c2pa_detail::read_file(key_path);
    if (cert.empty() || key.empty()) {
        fprintf(stderr, "stelnettts: C2PA cert/key unreadable ('%s' / '%s')\n", cert_path.c_str(), key_path.c_str());
        return false;
    }
    return stelnettts_c2pa_sign_pem(data, format, cert, key);
}

// Back-compat WAV wrapper (existing call sites).
inline bool stelnettts_c2pa_sign_wav(std::string& wav, const std::string& cert_path, const std::string& key_path) {
    return stelnettts_c2pa_sign_buffer(wav, "audio/wav", cert_path, key_path);
}
...
inline bool stelnettts_c2pa_sign_auto(std::string& data, const char* format, const std::string& user_cert,
    ...
    if (!user_cert.empty() && !user_key.empty())
        return stelnettts_c2pa_sign_buffer(data, format, user_cert, user_key);
    return stelnettts_c2pa_sign_pem(data, format, stelnettts_c2pa_default_cert_pem(), stelnettts_c2pa_default_key_pem());
}

// Print a one-time startup note about C2PA capabilities.
inline void stelnettts_c2pa_startup_check() {
#if !defined(STELNETTTS_HAVE_C2PA) && defined(STELNETTTS_NO_C2PA_NATIVE)
    static bool warned = false;
    if (!warned) {
        fprintf(stderr, "stelnettts: C2PA signing disabled (no native signer and c2pa-c not found; "
                        "run scripts/fetch-c2pa.sh and rebuild, or enable the native signer)\n");
        warned = true;
    }
#elif !defined(STELNETTTS_HAVE_C2PA)
    // The native signer covers WAV, MP3 (ID3v2.4 GEOB), M4A/MP4 (ISO BMFF
    // uuid) and FLAC — see stelnettts_c2pa_sign_pem() above. c2pa-rs is only a
    // fallback for formats outside that set. No warning: signing works out of
    // the box for every container StelnetTTS emits a manifest into, and the
