// stelnettts_watermark.h — spread-spectrum audio watermark for AI-generated speech.
//
...
//   3. A PRNG seeded with STELNETTTS_WATERMARK_KEY selects which frequency
...
#ifndef STELNETTTS_WATERMARK_KEY
#define STELNETTTS_WATERMARK_KEY 0x437269737041535FULL // "StelnetTTS" in hex-ish
#endif
...
#ifndef STELNETTTS_WATERMARK_NBINS
#define STELNETTTS_WATERMARK_NBINS 32
#endif

// Which detection statistic is the DEFAULT when STELNETTTS_WATERMARK_DETECT is
...
// (tools/watermark_detect_ab.cpp; table in stelnettts_watermark_stats.h). The
// sign test stays reachable with STELNETTTS_WATERMARK_DETECT=sign — it is the
// regression-bisection path, not dead code.
#ifndef STELNETTTS_WATERMARK_DETECT_FRAMES_DEFAULT
#define STELNETTTS_WATERMARK_DETECT_FRAMES_DEFAULT 1
#endif

namespace stelnettts_wm {
...
//
// STELNETTTS_WATERMARK_LEGACY=1 restores the pre-#260 wideband/loud behavior so
...
inline void wm_params(int n_fft, int& lo_bin, int& hi_bin, float& default_alpha) {
    ...
    if (std::getenv("STELNETTTS_WATERMARK_LEGACY")) {
        ...
        // voiced speech (~0.88 on real cielvox2-tts vs 0.94 legacy).
        hi_bin = n_fft / 5;
        default_alpha = 0.05f;
    }
}
...
// Embed the StelnetTTS watermark into `pcm` (float32 mono, any sample rate).
...
inline void stelnettts_watermark_embed_impl(float* pcm, int n_samples, float alpha = -1.0f) {
    ...
    int lo_bin, hi_bin;
    float default_alpha;
    stelnettts_wm::wm_params(n_fft, lo_bin, hi_bin, default_alpha);
    if (alpha < 0.0f)
        alpha = default_alpha;

    const auto bins =
        stelnettts_wm::generate_bin_pattern(STELNETTTS_WATERMARK_KEY, n_fft, STELNETTTS_WATERMARK_NBINS, lo_bin, hi_bin);
    if (bins.empty())
        return;
    ...
    for (int start = 0; start + n_fft <= n_samples; start += hop) {
        ...
        // Inverse FFT
        stelnettts_wm::fft_radix2(re.data(), im.data(), n_fft, true);
...
// indicate the StelnetTTS watermark is present. Values below 0.4 indicate
...
inline float stelnettts_watermark_detect_impl(const float* pcm, int n_samples) {
    const int n_fft = 1024;
    ...
    stelnettts_wm::wm_params(n_fft, lo_bin, hi_bin, default_alpha_unused);
    const auto bins =
        stelnettts_wm::generate_bin_pattern(STELNETTTS_WATERMARK_KEY, n_fft, STELNETTTS_WATERMARK_NBINS, lo_bin, hi_bin);
    if (bins.empty())
        return 0.0f;
...
// the 0.65 threshold sits 1.7 sigma above chance. stelnettts_watermark_stats.h
...
// any StelnetTTS, CrispTTS or StelnetVox release still verifies through this, and
// the three projects can read each other's marks. Do not "improve" the embed to
// suit the detector.
//
// Selected by STELNETTTS_WATERMARK_DETECT=frames|sign (see detect_uses_frames()).
namespace stelnettts_wm {
...
#ifndef STELNETTTS_WM_MAX_DECOY_RATIO
#define STELNETTTS_WM_MAX_DECOY_RATIO 0.70
#endif
inline constexpr double kDetectMaxDecoyRatio = STELNETTTS_WM_MAX_DECOY_RATIO;
...
inline float stelnettts_watermark_detect_frames_impl(const float* pcm, int n_samples) {
    const int n_fft = 1024;
    const int hop = n_fft / 2;
    if (n_samples < n_fft)
        return 0.0f;
    ...
    int lo_bin, hi_bin;
    float alpha_unused;
    stelnettts_wm::wm_params(n_fft, lo_bin, hi_bin, alpha_unused);
    const auto bins =
        stelnettts_wm::generate_bin_pattern(STELNETTTS_WATERMARK_KEY, n_fft, STELNETTTS_WATERMARK_NBINS, lo_bin, hi_bin);
    if (bins.empty())
        return 0.0f;
    ...
    for (int start = 0; start + n_fft <= n_samples; start += hop)
        n_frames++;
    if (n_frames < stelnettts_wm::kDetectMinFrames)
        return 0.0f;

    // Decoy sign patterns over the SAME bins, from keys never embedded with.
    // Mirrors CrispTTS: key ^ (0x9E3779B97F4A7C15 * (k + 1)).
    const int n_pat = 1 + stelnettts_wm::kDetectDecoys;
    std::vector<double> pats((size_t)n_pat * nb);
    for (int j = 0; j < nb; j++)
        pats[j] = sgn[j];
    for (int k = 0; k < stelnettts_wm::kDetectDecoys; k++) {
        const uint64_t dk = (uint64_t)STELNETTTS_WATERMARK_KEY ^ (0x9E3779B97F4A7C15ULL * (uint64_t)(k + 1));
        stelnettts_wm::prng rng(dk);
        for (int j = 0; j < nb; j++)
    ...
    const double scale = std::max(1.4826 * mad, stelnettts_wm::kDetectMinScale);
    ...
    const double ratio = std::min({t_all[0] / stelnettts_wm::kDetectTMin, z / stelnettts_wm::kDetectZMin,
                                   decoy_ratio / stelnettts_wm::kDetectMaxDecoyRatio});
    return (float)stelnettts_wm::t_to_confidence(ratio);
}
...
//   STELNETTTS_WATERMARK_DETECT=sign    -> averaged-spectrum sign agreement
//   STELNETTTS_WATERMARK_DETECT=frames  -> per-frame t + decoy specificity
inline bool stelnettts_watermark_detect_uses_frames() {
    const char* e = std::getenv("STELNETTTS_WATERMARK_DETECT");
    if (!e || !*e)
        return STELNETTTS_WATERMARK_DETECT_FRAMES_DEFAULT;
...
inline float stelnettts_watermark_detect_select(const float* pcm, int n_samples) {
    return stelnettts_watermark_detect_uses_frames() ? stelnettts_watermark_detect_frames_impl(pcm, n_samples)
                                                   : stelnettts_watermark_detect_impl(pcm, n_samples);
}
