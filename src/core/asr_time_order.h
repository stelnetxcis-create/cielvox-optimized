...
// different structs (stelnettts_segment in the CLI/server, stelnettts_session_seg
...
// stelnettts_to_timestamp: this header has to be includable from src/, which
...
// putting it in warn_if_backward() below would give the CLI's stelnettts_segment
// and the session's stelnettts_session_seg a flag each and warn twice in a host
// that drives both.
...
// stage. STELNETTTS_ORDER_WARN=0 silences it.
//
...
template <typename SegT> void warn_if_backward(const std::vector<SegT>& segments, const char* where) {
    if (warn_once_flag())
        return;
    if (const char* e = getenv("STELNETTTS_ORDER_WARN"))
        if (atoi(e) == 0)
            return;
    ...
    fprintf(stderr,
            "stelnettts: warning: transcript is not in time order after %s -- segment %d starts at %s, "
