#pragma once
// core/stelnettts_env.h — unified environment-variable access (issue #265).
//
// StelnetTTS standardizes tunable/debug env vars on the
// STELNETTTS_<BACKEND>_<FEATURE> convention (e.g. STELNETTTS_OMNIVOICE_CODEC_GPU,
// matching STELNETTTS_IRODORI_CODEC_GPU). Many backends historically shipped
// bare-prefixed names (OMNIVOICE_CODEC_GPU, CIELVOX2_TTS_CODEC_GPU, CHATTERBOX_BENCH,
...
//
// The alias is derived automatically: get("STELNETTTS_FOO_BAR") first reads
// STELNETTTS_FOO_BAR, and if unset, falls back to the prefix-stripped legacy name
// FOO_BAR. So a call site only ever names the canonical STELNETTTS_-prefixed
...
// STELNETTTS_SUPPRESS_ENV_DEPRECATION=1 (for pipelines that scrape stderr and
// can't yet migrate).
//
...
namespace stelnettts_env {

inline constexpr char kCanonicalPrefix[] = "STELNETTTS_";
...
// the `canonical` replacement. Silenced by STELNETTTS_SUPPRESS_ENV_DEPRECATION.
inline void warn_deprecated(const char* legacy, const char* canonical) {
    if (std::getenv("STELNETTTS_SUPPRESS_ENV_DEPRECATION"))
        return;
    std::lock_guard<std::mutex> lock(warn_mutex());
    if (!warned().insert(legacy).second)
        return; // already warned for this name
    std::fprintf(stderr,
                 "[stelnettts] warning: environment variable '%s' is deprecated and "
...
// with "STELNETTTS_", falls back to the prefix-stripped legacy name; if a distinct
...
inline const char* get(const char* canonical, const char* legacy = nullptr) {
    if (const char* v = std::getenv(canonical))
        return v;
    constexpr size_t plen = sizeof(kCanonicalPrefix) - 1; // strlen("STELNETTTS_")
