// stelnettts_vad.cpp — Silero VAD segmentation + stitching for shared use.
// See stelnettts_vad.h for the interface contract.
//
// Extracted from examples/cli/stelnettts_vad.cpp so that the CLI, the C-ABI
// wrapper `stelnettts_session_transcribe_vad`, and every language binding
...
#include "stelnettts.h"    // whisper_vad_* API (Silero VAD)
...
#endif
#if __has_include("stelnettts_vad_encdec.h")
#include "stelnettts_vad_encdec.h" // Whisper-encoder + decoder VAD (ONNX-converted)
...
// (stelnettts_server.cpp), so this mutex is the only thing serializing
...
static std::vector<stelnettts_audio_slice> compute_firered_vad_slices(const float* samples, int n_samples,
    ...
    if (!vctx) {
        fprintf(stderr, "stelnettts: warning: failed to load FireRedVAD model '%s'\n", vad_model_path);
        return slices;
    }
...
std::vector<stelnettts_audio_slice> stelnettts_compute_vad_slices(const float* samples, int n_samples, int sample_rate,
                                                              ...
                                                              const stelnettts_vad_options& opts, bool* out_load_failed) {
    std::vector<stelnettts_audio_slice> slices;
    ...
    if (is_firered_vad_model(vad_model_path)) {
        ...
        // Default: Silero VAD via stelnettts API.
        ...
        if (!vctx) {
            fprintf(stderr, "stelnettts: warning: failed to load VAD model '%s'\n", vad_model_path);
            if (out_load_failed)
    ...
    slices = stelnettts_post_merge_vad_slices(slices, sample_rate, opts);
    ...
    // Disable with STELNETTTS_VAD_FAILOVER=0.
    {
        const char* off = std::getenv("STELNETTTS_VAD_FAILOVER");
        if (!(off && off[0] == '0') && sample_rate > 0) {
            ...
            if (verdict.failover) {
                fprintf(stderr, "stelnettts[vad]: %s — falling back to full-clip chunks\n", verdict.reason.c_str());
                const int chunk = opts.chunk_seconds > 0 ? opts.chunk_seconds : 30;
                return stelnettts_fixed_chunk_slices(n_samples, sample_rate, chunk);
            }
        }
    }

    // Post-split: break any VAD segment that exceeds chunk_seconds into
    // sub-segments. See stelnettts_rechunk_slices — the same step is reused on the
    ...
    if (opts.chunk_seconds > 0)
        slices = stelnettts_rechunk_slices(slices, samples, n_samples, sample_rate, opts.chunk_seconds);
...
std::vector<stelnettts_audio_slice> stelnettts_rechunk_slices(const std::vector<stelnettts_audio_slice>& in,
    ...
    std::vector<stelnettts_audio_slice> split;
...
std::vector<stelnettts_audio_slice> stelnettts_energy_chunk_slices(const float* samples, int n_samples, int sample_rate,
    ...
    if (chunk_seconds <= 0 || search_window_seconds <= 0.0f)
        return stelnettts_fixed_chunk_slices(n_samples, sample_rate, chunk_seconds);
...
stelnettts_stitched_audio stelnettts_stitch_vad_slices(const float* samples, int /*n_samples*/, int sample_rate,
    ...
    stelnettts_stitched_audio result;
    if (slices.empty())
        return result;
