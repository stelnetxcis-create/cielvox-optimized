...
// via `cielvox2_tts_set_codec_path`).
//
...
// `cielvox2_tts_synthesize` returns nullptr until the codec lands;
// `cielvox2_tts_synthesize_codes` works end-to-end for codebook-0 today.
...
struct cielvox2_tts_context_params {
    int n_threads;
...
struct cielvox2_tts_context_params cielvox2_tts_context_default_params(void);

// Initialise from the talker LM GGUF file.
struct cielvox2_tts_context* cielvox2_tts_init_from_file(const char* path_model, struct cielvox2_tts_context_params params);

// Initialise a codec-only context — no talker / code_predictor / speaker encoder.
// Only loads the codec GGUF and sets up a backend.
struct cielvox2_tts_context* cielvox2_tts_init_codec_only(const char* codec_path, struct cielvox2_tts_context_params params);

// Point the runtime at the codec GGUF (Xenna/cielvox2-tts-tokenizer-12hz-GGUF).
// Required before the first `cielvox2_tts_synthesize` call. Returns 0 on success.
int cielvox2_tts_set_codec_path(struct cielvox2_tts_context* ctx, const char* path);
...
int cielvox2_tts_set_voice_prompt(struct cielvox2_tts_context* ctx, const char* wav_path);

// Same as set_voice_prompt but also stores the reference transcription.
// Required for synthesis when no voice pack is loaded.
int cielvox2_tts_set_voice_prompt_with_text(struct cielvox2_tts_context* ctx, const char* wav_path, const char* ref_text);
...
int cielvox2_tts_set_voice_prompt_xvec_only(struct cielvox2_tts_context* ctx, const char* wav_path);
...
const int32_t* cielvox2_tts_get_runtime_ref_codes(struct cielvox2_tts_context* ctx, int* out_n);

// Read-only pointer to the currently active runtime speaker embedding
// (set by cielvox2_tts_set_voice_prompt[_with_text]). Returns nullptr if no
// runtime prompt is active. Buffer is owned by ctx; do not free.
const float* cielvox2_tts_get_runtime_spk_emb(struct cielvox2_tts_context* ctx, int* out_n);
...
float* cielvox2_tts_cenc_extract_stage(struct cielvox2_tts_context* ctx, const float* audio, int n_samples,
                                    const char* stage_name, int* out_n);

// Load a voice pack GGUF (produced by `models/bake-cielvox2-tts-voice-pack.py`)
...
int cielvox2_tts_load_voice_pack(struct cielvox2_tts_context* ctx, const char* path);
...
int cielvox2_tts_select_voice(struct cielvox2_tts_context* ctx, const char* name);
...
int cielvox2_tts_set_language(struct cielvox2_tts_context* ctx, int codec_language_id);

// Set language by name (case-insensitive, e.g. "chinese", "japanese", "auto").
// Looks up the name in the cielvox2tts.codec_language_names table loaded from GGUF.
int cielvox2_tts_set_language_by_name(struct cielvox2_tts_context* ctx, const char* name);
...
// model isn't CustomVoice). Pass into cielvox2_tts_get_speaker_name to
// enumerate them.
int cielvox2_tts_n_speakers(struct cielvox2_tts_context* ctx);

// Returns the i-th fixed speaker name. Buffer is owned by ctx; do not
// free. Returns nullptr for out-of-range indices.
const char* cielvox2_tts_get_speaker_name(struct cielvox2_tts_context* ctx, int i);
...
int cielvox2_tts_set_speaker_by_name(struct cielvox2_tts_context* ctx, const char* name);

// Returns true if the loaded model is a CustomVoice variant.
int cielvox2_tts_is_custom_voice(struct cielvox2_tts_context* ctx);
...
int cielvox2_tts_is_voice_design(struct cielvox2_tts_context* ctx);

// Set the natural-language voice description used as the instruct
// prompt. Required before cielvox2_tts_synthesize / synthesize_codes when
...
int cielvox2_tts_set_instruct(struct cielvox2_tts_context* ctx, const char* instruct);
...
int cielvox2_tts_set_cv_style_instruct(struct cielvox2_tts_context* ctx, const char* instruct);

// ---------------------------------------------------------------------------
// Diff-harness stage APIs (PLAN #52 step 4)
//
// These expose intermediate activations without driving the AR decode
// loop, so `stelnettts-diff cielvox2-tts` can verify each stage of the
// talker against the qwen_tts PyTorch reference. They mirror the
// stage names that `tools/reference_backends/cielvox2_tts.py` dumps.
// ---------------------------------------------------------------------------
...
float* cielvox2_tts_run_text_proj(struct cielvox2_tts_context* ctx, const int32_t* ids, int n_tokens, int* out_T, int* out_d);
...
float* cielvox2_tts_run_talker_with_embeds(struct cielvox2_tts_context* ctx, const float* embeds, int n_tokens,
                                        int* out_vocab);
...
float* cielvox2_tts_run_code_pred_step(struct cielvox2_tts_context* ctx, const float* embeds, int n_tokens, int n_past,
...
float* cielvox2_tts_build_icl_prefill(struct cielvox2_tts_context* ctx, const char* syn_text, const char* ref_text,
                                   int* out_T);
...
// `cielvox2_tts_codes_free`. Returns nullptr on failure.
//
...
int32_t* cielvox2_tts_synthesize_codes(struct cielvox2_tts_context* ctx, const char* text, int* out_n_codes);

void cielvox2_tts_codes_free(int32_t* codes);

// Decode a flat code array (T_frames * 16 codes, row-major [T, 16]) to
// 24 kHz mono float32 PCM. Requires `cielvox2_tts_set_codec_path` to have
// been called first. Caller frees with `cielvox2_tts_pcm_free`.
// *out_n_samples is set on success; returns nullptr on failure.
float* cielvox2_tts_decode_codes(struct cielvox2_tts_context* ctx, const int32_t* codes, int n_codes, int* out_n_samples);
...
float* cielvox2_tts_codec_extract_stage(struct cielvox2_tts_context* ctx, const int32_t* codes, int n_codes,
                                     const char* stage_name, int* out_n);

// Synthesise text → 24 kHz mono float32 PCM. Caller frees with
// `cielvox2_tts_pcm_free`. *out_n_samples is set on success.
//
// Returns nullptr until the codec decoder lands (PLAN #52 step 3).
float* cielvox2_tts_synthesize(struct cielvox2_tts_context* ctx, const char* text, int* out_n_samples);
...
// from cielvox2_tts_synthesize_streaming.
typedef void (*cielvox2_tts_pcm_callback)(const float* pcm, int n_samples, int is_final, void* user_data);

// Streaming counterpart to cielvox2_tts_synthesize. Emits PCM in chunks as
...
// with cielvox2_tts_pcm_free); *out_n_samples is set on success. Returns
// nullptr on failure.
float* cielvox2_tts_synthesize_streaming(struct cielvox2_tts_context* ctx, const char* text, int chunk_frames,
                                      int overlap_frames, cielvox2_tts_pcm_callback cb, void* user_data,
                                      int* out_n_samples);

void cielvox2_tts_pcm_free(float* pcm);

void cielvox2_tts_free(struct cielvox2_tts_context* ctx);
...
void cielvox2_tts_sync(struct cielvox2_tts_context* ctx);

void cielvox2_tts_set_n_threads(struct cielvox2_tts_context* ctx, int n_threads);
...
void cielvox2_tts_set_temperature(struct cielvox2_tts_context* ctx, float temperature);
void cielvox2_tts_set_seed(struct cielvox2_tts_context* ctx, uint64_t seed);
...
float* cielvox2_tts_compute_speaker_mel(struct cielvox2_tts_context* ctx, const float* audio, int n_samples, int* out_T_mel,
                                     int* out_n_mels);
...
float* cielvox2_tts_run_speaker_enc_on_mel(struct cielvox2_tts_context* ctx, const float* mel, int T_mel, int* out_dim);
...
// Does NOT set the context's active voice — call cielvox2_tts_set_voice_prompt
// to both compute and activate the embedding for synthesis.
float* cielvox2_tts_compute_speaker_embedding(struct cielvox2_tts_context* ctx, const float* audio, int n_samples,
                                           int* out_dim);
