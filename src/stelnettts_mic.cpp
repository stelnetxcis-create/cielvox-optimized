// stelnettts_mic.cpp — library-level mic capture (PLAN #62d).
//
...
// stelnettts_audio.cpp companion already drops MA_NO_DEVICE_IO on these
// platforms, so the ma_device_* symbols don't exist in libstelnettts
...
// visionOS); macOS leaves it 0. Mirrors the gate in stelnettts_audio.cpp
// so both files agree on which platforms strip miniaudio device IO.
#if TARGET_OS_IPHONE
#define STELNETTTS_MIC_STUB_ONLY 1
#endif
#endif

#ifndef STELNETTTS_MIC_STUB_ONLY
...
struct stelnettts_mic {
    ma_device device;
    stelnettts_mic_callback cb = nullptr;
...
// device's opaque pointer (we set it to stelnettts_mic*); pInput is
...
void mic_data_cb(ma_device* pDevice, void* /*pOutput*/, const void* pInput, ma_uint32 frameCount) {
    auto* m = (stelnettts_mic*)pDevice->pUserData;
    if (!m || !m->cb || !pInput)
        return;
...
extern "C" struct stelnettts_mic* stelnettts_mic_open(int sample_rate, int channels, stelnettts_mic_callback cb,
                                                  void* userdata) {
    if (!cb || sample_rate <= 0 || channels < 1 || channels > 2)
        return nullptr;

    auto* m = new (std::nothrow) stelnettts_mic();
    if (!m)
        return nullptr;
    m->cb = cb;
    m->userdata = userdata;
...
extern "C" int stelnettts_mic_start(struct stelnettts_mic* m) {
    if (!m || m->started)
        return -1;
...
extern "C" int stelnettts_mic_stop(struct stelnettts_mic* m) {
    if (!m)
        return -1;
    if (!m->started)
        return 0;
...
extern "C" void stelnettts_mic_close(struct stelnettts_mic* m) {
    if (!m)
        return;
    if (m->started) {
        ma_device_stop(&m->device);
...
extern "C" const char* stelnettts_mic_default_device_name(void) {
...
#else // STELNETTTS_MIC_STUB_ONLY — iOS / tvOS / watchOS

// Stub the C ABI so libstelnettts links on iOS / tvOS / watchOS. Host
// apps that need mic capture on these platforms call AVAudioEngine
// directly and hand PCM into `stelnettts_stream_feed`. The host has the
...
#endif // STELNETTTS_MIC_STUB_ONLY
