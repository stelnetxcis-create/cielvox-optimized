// stelnettts_model_registry.h — known-model registry lookup.
//
...
// (`examples/cli/stelnettts_model_mgr_cli.cpp`). The library interface
...
/// Same list as StelnetEmbed's crispembed_mgr::license_requires_acceptance.
...
/// Falls back to the STELNETTTS_ACCEPT_LICENSE env var.
bool stelnettts_license_accepted(const std::string& license, const std::string& accepted);

/// Download a StelnetTTS-managed auxiliary artifact after applying the same
/// license gate and notice policy as registry models. `accepted_license` may
/// be empty to use STELNETTTS_ACCEPT_LICENSE.
...
/// through stelnettts_registry_lookup(), which rewrites both the filename and the
/// URL of the primary and its companion. Callers mirroring `-m auto:<quant>`
/// must go through stelnettts_registry_lookup() instead — the filenames here
...
/// Falls back to the `STELNETTTS_ACCEPT_LICENSE` env var. For such models
...
/// Mirrors StelnetEmbed's crispembed_mgr::resolve_model.
