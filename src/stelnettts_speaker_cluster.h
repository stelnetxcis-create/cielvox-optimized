// stelnettts_speaker_cluster.h — agglomerative single-linkage clustering
...
std::vector<int> stelnettts_agglomerative_cluster(const std::vector<float>& embeddings, // N * dim floats, row-major
...
// product). `labels` is the output of stelnettts_agglomerative_cluster
...
// zero vector.
std::vector<float> stelnettts_cluster_centroids(const std::vector<float>& embeddings, const std::vector<int>& labels,
                                              int n, int dim);
