...
static bool wavtok_bench_enabled() {
    static int v = -1;
    if (v < 0) {
        const char* e = stelnettts_env::get("STELNETTTS_WAVTOK_BENCH");
        v = (e && *e && *e != '0') ? 1 : 0;
    }
    return v != 0;
}
...
extern "C" float* wavtok_decoder_decode(struct wavtok_decoder_ctx* ctx, const int32_t* codes, int n_codes,
    ...
    const char* env_dump = stelnettts_env::get("STELNETTTS_WAVTOK_DUMP_DIR");
    ...
    const char* env_codes = stelnettts_env::get("STELNETTTS_WAVTOK_FIXED_CODES");
