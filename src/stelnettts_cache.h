// stelnettts_cache.h — shared cache directory + download helper.
//
// Three places in the unified CLI need to download a small companion file
// from HuggingFace on first use and cache it under ~/.cache/stelnettts/:
//
//   * stelnettts_model_mgr — `-m auto` model resolution
//   * stelnettts_lid       — whisper-tiny LID model auto-download
//   * stelnettts_vad       — Silero VAD model auto-download
//
...
// consumer is a one-liner over `stelnettts_cache::ensure_cached_file(...)`.
...
//   • Otherwise honor STELNETTTS_CACHE_DIR, then STELNETTTS_MODELS_DIR.
//   • If neither environment variable is set, use the platform default:
//       - POSIX : $HOME/.cache/stelnettts
//       - Windows: %USERPROFILE%/.cache/stelnettts
...
// Search all well-known locations (canonical cache, STELNETTTS_MODELS_DIR env,
...
std::string ensure_cached_file(const std::string& filename, const std::string& url, bool quiet,
                               const char* pretty_label = "stelnettts", const std::string& cache_dir_override = "");
