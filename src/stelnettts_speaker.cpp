// stelnettts_speaker.cpp — local speaker playback via miniaudio.
//
// Thin wrapper around miniaudio's ma_device playback mode. The
// MINIAUDIO_IMPLEMENTATION lives in stelnettts_audio.cpp, so this TU
// just consumes the API. Mirrors the structure of stelnettts_mic.cpp.
//
...
// Speakers). In stelnettts_speaker_play() we pre-resample the app's mono
...
#if TARGET_OS_IPHONE
#define STELNETTTS_SPEAKER_STUB_ONLY 1
#endif
#endif

#ifndef STELNETTTS_SPEAKER_STUB_ONLY
...
struct stelnettts_speaker {
    ...
    // f32 interleaved. Owned by stelnettts_speaker_play(); the callback reads it.
...
void speaker_data_cb(ma_device* dev, void* output, const void* /*input*/, ma_uint32 frames) {
    auto* s = (stelnettts_speaker*)dev->pUserData;
...
extern "C" struct stelnettts_speaker* stelnettts_speaker_open(int /*sample_rate*/, int /*channels*/, int device_index) {
    auto* s = new (std::nothrow) stelnettts_speaker();
    if (!s)
        return nullptr;
...
extern "C" int stelnettts_speaker_play(struct stelnettts_speaker* s, const float* pcm, int n_samples, int app_sample_rate,
                                     int app_channels) {
...
extern "C" int stelnettts_speaker_wait(struct stelnettts_speaker* s) {
    if (!s || !s->started)
        return -1;
...
extern "C" int stelnettts_speaker_stop(struct stelnettts_speaker* s) {
    if (!s)
        return -1;
    s->stopped.store(true, std::memory_order_relaxed);
    if (s->event_init)
...
extern "C" void stelnettts_speaker_close(struct stelnettts_speaker* s) {
    if (!s)
        return;
    if (s->started) {
        s->stopped.store(true, std::memory_order_relaxed);
...
extern "C" const char* stelnettts_speaker_default_device_name(void) {
...
#else // STELNETTTS_SPEAKER_STUB_ONLY — iOS / tvOS / watchOS

struct stelnettts_speaker {};

extern "C" struct stelnettts_speaker* stelnettts_speaker_open(int, int, int) {
    return nullptr;
}
extern "C" int stelnettts_speaker_play(struct stelnettts_speaker*, const float*, int, int, int) {
    return -1;
}
extern "C" int stelnettts_speaker_wait(struct stelnettts_speaker*) {
    return -1;
}
extern "C" int stelnettts_speaker_stop(struct stelnettts_speaker*) {
    return -1;
}
extern "C" void stelnettts_speaker_close(struct stelnettts_speaker*) {}
extern "C" const char* stelnettts_speaker_default_device_name(void) {
    return "";
}

#endif // STELNETTTS_SPEAKER_STUB_ONLY
