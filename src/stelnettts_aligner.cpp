// stelnettts_aligner.cpp — shared CTC / forced-alignment implementation.
// See stelnettts_aligner.h.
//
// Extracted from examples/cli/stelnettts_aligner.cpp so every StelnetTTS
// consumer can reach both the canary-ctc and cielvox2-forced-aligner paths
...
// the GGUF model on every stelnettts_align_words call — alignment models
...
static void aligner_cache_free_locked() {
    ...
    case AlignerType::Wav2Vec2:
        ...
        // stelnettts_aligner_free_cache() at CLI shutdown (stelnettts_run.cpp),
...
void stelnettts_aligner_free_cache() {
...
static std::vector<StelnetAsrAlignedWord> align_qwen3_fa_one_chunk(cielvox2_asr_context* ctx,
    ...
    int rc = cielvox2_asr_align_words(ctx, samples, n_samples, word_ptrs.data(), (int)words.size(), start_ms.data(),
                                   end_ms.data());
    if (rc != 0) {
        fprintf(stderr, "stelnettts[aligner-qwen3]: align_words rc=%d (chunk at %.1fs)\n", rc,
...
std::vector<StelnetAsrAlignedWord> align_qwen3_fa(const std::string& model_path, const std::vector<std::string>& words,
    ...
    if (!ctx) {
        ...
        if (!ctx) {
            fprintf(stderr, "stelnettts[aligner-qwen3]: failed to load '%s'\n", model_path.c_str());
            return out;
        }
    ...
    if (cielvox2_asr_lm_head_dim(ctx) == 0 || cielvox2_asr_lm_head_dim(ctx) > 10000) {
        fprintf(stderr,
                "stelnettts[aligner-qwen3]: model '%s' lm_head dim is %d "
    ...
    fprintf(stderr, "stelnettts[aligner-qwen3]: audio %.1fs exceeds %ds limit, splitting into %d chunks\n",
    ...
    for (int c = 0; c < n_chunks; c++) {
        ...
        if (chunk_aligned.empty()) {
            fprintf(stderr, "stelnettts[aligner-qwen3]: chunk %d/%d failed (%d words, %.1fs audio)\n", c + 1, n_chunks,
...
std::vector<StelnetAsrAlignedWord> align_wav2vec2_ctc(const std::string& model_path,
    ...
    // resident server / repeated bindings-API calls (stelnettts_align_words is a
    ...
    if (!model) {
        ...
        if (!wav2vec2_load(model_path.c_str(), *model)) {
            fprintf(stderr, "stelnettts[aligner-wav2vec2]: failed to load '%s'\n", model_path.c_str());
            delete model;
            return out;
        }
    ...
    if (logits.empty()) {
        fprintf(stderr, "stelnettts[aligner-wav2vec2]: compute_logits failed\n");
        return out;
    }
    ...
    if (aligned.empty()) {
        fprintf(stderr, "stelnettts[aligner-wav2vec2]: align_words failed\n");
        return out;
    }
...
std::vector<std::string> stelnettts_tokenise_align_words(const std::string& text) {
    return tokenise_words(text);
}

std::vector<std::string> stelnettts_parse_srt_cues(const std::string& raw) {
...
// the server. `stelnettts_align_words` is what all of them call.
//
...
static void align_sentinel_check(std::vector<StelnetAsrAlignedWord>& words, int n_samples, int64_t t_offset_cs) {
    if (words.empty())
        return;
    {
        const char* off = std::getenv("STELNETTTS_ALIGN_SENTINEL");
        if (off && off[0] == '0')
            return;
    }
    ...
    fprintf(stderr, "stelnettts[aligner]: WARNING: %s\n", core_align_sentinel::describe(a).c_str());

    const char* fix = std::getenv("STELNETTTS_ALIGN_SENTINEL_REDISTRIBUTE");
    if (!(fix && fix[0] == '1'))
        return;
    if (audio_sec <= 0.0f)
        return;
...
static std::vector<StelnetAsrAlignedWord> align_words_impl(const std::string& aligner_model,
    ...
    // Disable with STELNETTTS_ALIGN_NO_ROMANIZE=1 to pass raw script through.
    std::string eff_transcript = transcript;
    {
        const char* no_rom = std::getenv("STELNETTTS_ALIGN_NO_ROMANIZE");
        if (!(no_rom && no_rom[0] == '1') && core_uroman::needs_romanization(transcript)) {
            eff_transcript = core_uroman::romanize(transcript);
            const char* dbg = std::getenv("STELNETTTS_ALIGN_DEBUG");
            if (dbg && dbg[0] == '1')
                fprintf(stderr, "stelnettts[aligner]: romanized → \"%s\"\n", eff_transcript.c_str());
        }
    }
    ...
    if (!actx) {
        ...
        if (!actx) {
            fprintf(stderr, "stelnettts[aligner]: failed to load '%s'\n", aligner_model.c_str());
            if (out_load_failed)
    ...
    if (rc != 0) {
        fprintf(stderr, "stelnettts[aligner]: compute_logits failed (rc=%d)\n", rc);
        return out;
    }
    ...
    if (rc != 0) {
        fprintf(stderr, "stelnettts[aligner]: align_words failed (rc=%d)\n", rc);
        return out;
    }
