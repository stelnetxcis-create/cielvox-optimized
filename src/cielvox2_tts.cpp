// cielvox2_tts.cpp — runtime for Qwen/Qwen3-TTS-12Hz-{0.6B,1.7B}-Base.
//
// Status (April 2026):
//
...
//     loaded via `cielvox2_tts_set_codec_path`. `cielvox2_tts_synthesize`
...
//     Root cause isolated via CIELVOX2_TTS_CODEC_FORCE_METAL=1 + per-op
...
//       harness flags a discrepancy (see StelnetEmbed decoder_embed.cpp
...
//
//   CIELVOX2_TTS_BENCH=1     — print per-stage wall-clock timings on stderr.
//   CIELVOX2_TTS_DEBUG=1     — verbose per-step trace (prompt ids, sampled
//                           code at every AR step, stop reason).
//   CIELVOX2_TTS_DUMP_DIR=/d — dump key intermediate tensors to /d as
...
//
// These knobs follow the existing StelnetTTS pattern (GEMMA4_E2B_BENCH,
...
// ===========================================================================
// Bench instrumentation — `CIELVOX2_TTS_BENCH=1` for per-stage timings.
// ===========================================================================

static bool cielvox2_tts_bench_enabled() {
    static int v = -1;
    if (v < 0) {
        const char* e = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_BENCH");
        v = (e && *e && *e != '0') ? 1 : 0;
    }
    return v != 0;
}

struct cielvox2_tts_bench_stage {
    const char* name;
    std::chrono::steady_clock::time_point t0;
    explicit cielvox2_tts_bench_stage(const char* n) : name(n), t0(std::chrono::steady_clock::now()) {}
    ~cielvox2_tts_bench_stage() {
        if (!cielvox2_tts_bench_enabled())
            return;
        auto t1 = std::chrono::steady_clock::now();
        double ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
        std::fprintf(stderr, "  cielvox2_tts_bench: %-22s %.2f ms\n", name, ms);
    }
};
...
void dump_f32(const char* dir, const char* name, const float* data, size_t n) {
    ...
    if (!f) {
        fprintf(stderr, "cielvox2_tts: dump open '%s' failed\n", path);
        return;
    }
    std::fwrite(data, sizeof(float), n, f);
    std::fclose(f);
    fprintf(stderr, "cielvox2_tts: dumped %s (%zu floats)\n", path, n);
}
void dump_i32(const char* dir, const char* name, const int32_t* data, size_t n) {
    ...
    std::fclose(f);
    fprintf(stderr, "cielvox2_tts: dumped %s (%zu i32)\n", path, n);
}
...
static void qwen3_prof_print(const char* tag, const qwen3_prof_state& ps, int n_calls) {
    ...
    const int64_t total = ps.mul_mat.t_us + ps.flash_attn.t_us + ps.norm.t_us + ps.rope.t_us + ps.add.t_us +
                          ...
                          ps.other.t_us;
    fprintf(stderr, "cielvox2_tts: ---- %s op profile (CIELVOX2_TTS_PROF, %d calls) ----\n", tag, n_calls);
    fprintf(stderr, "cielvox2_tts:  %-12s %7.1f ms %5.1f%% n=%d\n", "mul_mat", ps.mul_mat.t_us / 1e3,
    ...
    fprintf(stderr, "cielvox2_tts:  %-12s %7.1f ms\n", "TOTAL", total / 1e3);
    fprintf(stderr, "cielvox2_tts: -----------------------------------------------\n");
}
...
struct g3t_hp {
    ...
    // convert-cielvox2-tts-to-gguf.py as cielvox2tts.codec_language_names /
    // cielvox2tts.codec_language_ids. Drives cielvox2_tts_set_language_by_name.
...
struct cielvox2_tts_context {
    cielvox2_tts_context_params params{};
    int n_threads = 4;
    ...
    // Default ON for 1.7B; opt out with CIELVOX2_TTS_CP_MTP_NOFUSE=1. Disabled when
    ...
    // skip rebuild/reset/alloc on cache hits. Gated on CIELVOX2_TTS_O15 (default
    ...
    // SIGSEGV). Gated on CIELVOX2_TTS_LK_BUCKET.
    struct TalkerBucket {
        int lk = 0;
    ...
    g3t_spk_enc spk_enc;
    std::vector<float> runtime_spk_emb; // set by cielvox2_tts_set_voice_prompt
    ...
    // cielvox2_tts_set_instruct. Tokenised to instruct_ids via
    ...
    // cielvox2_tts_set_cv_style_instruct. When non-empty, an instruct block is
...
static bool cielvox2_tts_codec_use_gpu_by_default(const cielvox2_tts_context* c) {
...
static bool cielvox2_tts_codec_backend_is_cuda(const cielvox2_tts_context* c) {
    if (!c || !c->backend || c->backend == c->backend_cpu) {
        return false;
    }
...
static bool cielvox2_tts_codec_decode_uses_cuda(const cielvox2_tts_context* c) {
    const bool force_cpu = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_CPU") != nullptr;
    if (force_cpu || !cielvox2_tts_codec_backend_is_cuda(c)) {
        return false;
    }
    return stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_FORCE_METAL") != nullptr ||
           stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_GPU") != nullptr || cielvox2_tts_codec_use_gpu_by_default(c);
}

// #304: the cielvox2-tts talker LM (and code_predictor) miscompute on the Vulkan
...
// The codec is already CPU-pinned (see cielvox2_tts_codec_use_gpu_by_default); this
...
// STELNETTTS_CIELVOX2_TTS_VULKAN_NATIVE=1.
static void cielvox2_tts_route_off_vulkan(cielvox2_tts_context* c, int verbosity) {
    if (c->backend == c->backend_cpu || !std::strstr(ggml_backend_name(c->backend), "Vulkan")) {
        return;
    }
    const char* keep = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_VULKAN_NATIVE");
    if (keep && keep[0] == '1') {
        return;
    }
    if (verbosity >= 1) {
        fprintf(stderr, "cielvox2_tts: Vulkan backend detected — running talker on CPU (#304 Vulkan "
                        "miscompute; set STELNETTTS_CIELVOX2_TTS_VULKAN_NATIVE=1 to override)\n");
    }
...
ggml_tensor* try_get(cielvox2_tts_context* c, const char* name) {
    auto it = c->tensors.find(name);
    return it == c->tensors.end() ? nullptr : it->second;
}

ggml_tensor* require(cielvox2_tts_context* c, const char* name) {
    auto* t = try_get(c, name);
    if (!t) {
        fprintf(stderr, "cielvox2_tts: required tensor missing: %s\n", name);
    }
    return t;
}

bool load_talker(cielvox2_tts_context* c) {
    auto& t = c->talker;
...
bool load_code_predictor(cielvox2_tts_context* c) {
    auto& p = c->code_pred;
...
bool kv_alloc(cielvox2_tts_context* c, int max_ctx) {
    if (c->kv_k) {
        return true;
    }
    const auto& hp = c->hp;
    ...
    const auto kv_pair = core_attn::kv_dtype_pair_from_env("cielvox2_tts");
    ...
    ggml_backend_t kv_backend = core_attn::kv_backend_from_env(c->backend, c->backend_cpu, "cielvox2_tts");
    ...
    if (c->params.verbosity >= 1) {
        fprintf(stderr, "cielvox2_tts: kv cache %d MiB (head_dim=%d max_ctx=%d n_kv=%d n_layers=%d)\n",
...
ggml_cgraph* build_graph_embed_text(cielvox2_tts_context* c, int n_tokens) {
...
ggml_cgraph* build_graph_embed_audio(cielvox2_tts_context* c, int n_tokens) {
...
ggml_cgraph* build_graph_talker_kv(cielvox2_tts_context* c, int n_past, int n_tokens, int fixed_kv_len = 0,
...
ggml_cgraph* build_graph_code_pred_kv(cielvox2_tts_context* c, int n_past, int n_tokens, ggml_tensor* lm_head,
    ...
    // O15 (default OFF since 2026-05-04; opt-in with CIELVOX2_TTS_O15=1):
    ...
    // M1 Metal users who want the speedup should set CIELVOX2_TTS_O15=1.
    ...
    const bool o15 = o15_force >= 0 ? o15_force != 0 : env_bool_default("STELNETTTS_CIELVOX2_TTS_O15", false);
...
static ggml_backend_sched_t code_pred_pick_sched(cielvox2_tts_context* c) {
    const char* cp_be = env_str("STELNETTTS_CIELVOX2_TTS_CP_BACKEND");
...
static bool code_pred_reserve_sched(cielvox2_tts_context* c, ggml_backend_sched_t sched) {
    ...
    if (!ggml_backend_sched_reserve(sched, gf)) {
        fprintf(stderr, "cielvox2_tts: code_pred reserve failed\n");
        return false;
    }
...
ggml_cgraph* build_graph_talker_kv(cielvox2_tts_context* c, int n_past, int n_tokens, int fixed_kv_len,
...
float* run_embed_text(cielvox2_tts_context* c, const int32_t* ids, int n) {
...
float* run_embed_audio(cielvox2_tts_context* c, const int32_t* ids, int n) {
    const int d = (int)c->hp.d_model;
...
static bool graph_ops_supported(ggml_backend_t be, ggml_cgraph* gf, const char* tag) {
    for (int i = 0; i < ggml_graph_n_nodes(gf); i++) {
        ...
        if (!ggml_backend_supports_op(be, node)) {
            fprintf(stderr, "cielvox2_tts: %s disabled — op %s unsupported on %s; using sched path\n", tag,
...
// Lk bucketing helpers (PLAN #52 step 4). Gated on CIELVOX2_TTS_LK_BUCKET=1.
...
// code_predictor's CIELVOX2_TTS_O15 path already uses.
...
static ggml_cgraph* talker_bucket_get_or_build(cielvox2_tts_context* c, int idx) {
    ...
    bk.ctx = ggml_init(ip);
    if (!bk.ctx) {
        fprintf(stderr, "cielvox2_tts: bucket[%d] arena init failed\n", idx);
        return nullptr;
    }
    ...
    if (!bk.gf) {
        fprintf(stderr, "cielvox2_tts: bucket[%d] build_graph failed\n", idx);
        ggml_free(bk.ctx);
        bk.ctx = nullptr;
...
static float* run_talker_kv_bucket(cielvox2_tts_context* c, const float* embeds, int n_past, float** out_hidden_d);
static float* run_talker_kv_dynamic(cielvox2_tts_context* c, const float* embeds, int n_tokens, int n_past,
                                    float** out_hidden_d);

float* run_talker_kv(cielvox2_tts_context* c, const float* embeds, int n_tokens, int n_past, float** out_hidden_d) {
    ...
    if (n_tokens == 1 && env_bool("STELNETTTS_CIELVOX2_TTS_LK_BUCKET") && c->talker_dir_state >= 0) {
...
static float* run_talker_kv_dynamic(cielvox2_tts_context* c, const float* embeds, int n_tokens, int n_past,
    ...
    if (n_past + n_tokens > c->kv_max_ctx) {
        fprintf(stderr, "cielvox2_tts: kv overflow (%d+%d > %d)\n", n_past, n_tokens, c->kv_max_ctx);
        return nullptr;
    }
    const auto& hp = c->hp;
    const int d = (int)hp.d_model;
    ...
    const bool bench = env_bool("STELNETTTS_CIELVOX2_TTS_BENCH");
    const bool prof = env_bool("STELNETTTS_CIELVOX2_TTS_PROF");
    ...
    const bool use_direct =
        !stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_TALKER_SCHED") && !core_cpu_backend::is_cpu(c->backend) && c->kv_k &&
        c->kv_k->buffer &&
    ...
    if (use_direct) {
        ...
        if (!direct_alloc || !ggml_gallocr_alloc_graph(direct_alloc, gf)) {
            fprintf(stderr, "cielvox2_tts: direct talker graph allocation failed\n");
            if (direct_alloc)
    ...
    if (compute_status != GGML_STATUS_SUCCESS) {
        ...
        fprintf(stderr, "cielvox2_tts: talker compute failed\n");
        if (direct_alloc)
            ggml_gallocr_free(direct_alloc);
        return nullptr;
    }
    ...
    if (bench) {
        ...
        if (count == 14) {
            fprintf(stderr,
                    "cielvox2_tts: talker_kv    (%d calls): build=%.1f ms  alloc=%.1f ms  compute=%.1f ms  read=%.1f ms\n",
...
static float* run_talker_kv_bucket(cielvox2_tts_context* c, const float* embeds, int n_past, float** out_hidden_d) {
    ...
    if (n_past + 1 > c->kv_max_ctx) {
        fprintf(stderr, "cielvox2_tts: kv overflow (%d+1 > %d)\n", n_past, c->kv_max_ctx);
        return nullptr;
    }
    const auto& hp = c->hp;
    const int d = (int)hp.d_model;
    ...
    if (!bk.galloc) {
        ...
        if (c->kv_k && c->kv_k->buffer &&
            ggml_backend_buffer_get_type(c->kv_k->buffer) != ggml_backend_get_default_buffer_type(c->backend)) {
            fprintf(stderr, "cielvox2_tts: talker_bucket disabled — KV cache not on the talker backend\n");
        ...
        if (!bk.galloc || !ggml_gallocr_alloc_graph(bk.galloc, gf)) {
            fprintf(stderr, "cielvox2_tts: bucket[%d] gallocr alloc failed\n", idx);
            if (bk.galloc) {
    ...
    if (ggml_backend_graph_compute(c->backend, gf) != GGML_STATUS_SUCCESS) {
        fprintf(stderr, "cielvox2_tts: talker bucket compute failed\n");
        c->talker_dir_state = -1;
        return nullptr;
    }
    const double t_compute1 = bench ? now_ms() : 0.0;
    ...
    if (bench) {
        ...
        if (count == 14) {
            fprintf(stderr,
                    "cielvox2_tts: talker_bucket (%d calls): build=%.1f ms  alloc=%.1f ms  "
...
float* lookup_rows(cielvox2_tts_context* c, ggml_tensor* weight, const int32_t* ids, int n_ids);
bool apply_small_to_mtp(cielvox2_tts_context* c, const float* in, float* out);
...
static ggml_cgraph* cp_step0_get_or_build(cielvox2_tts_context* c) {
    if (c->cp_step0_gf) {
...
static ggml_backend_sched_t cp_step0_pick_sched(cielvox2_tts_context* c) {
    if (c->cp_step0_sched) {
...
static bool cp_direct_init(cielvox2_tts_context* c) {
    if (c->cp_dir_state != 0) {
    ...
    if (!c->cp_dir_galloc || !c->cp_dir0_galloc || !ggml_gallocr_alloc_graph(c->cp_dir_galloc, c->cp_dir_gf) ||
        ...
        fprintf(stderr, "cielvox2_tts: cp_direct gallocr alloc failed; using sched path\n");
        return fail();
    }
    c->cp_dir_state = 1;
    if (c->params.verbosity >= 1) {
        fprintf(stderr, "cielvox2_tts: cp_direct active (persistent sched-free code_pred graphs)\n");
    }
    return true;
}
...
static float* run_code_pred_direct(cielvox2_tts_context* c, const float* embeds, int n_tokens, int n_past,
                                   ggml_tensor* lm_head) {
    if (!cp_direct_init(c)) {
        return nullptr;
    }
    ...
    if (ggml_backend_graph_compute(c->backend, gf) != GGML_STATUS_SUCCESS) {
        fprintf(stderr, "cielvox2_tts: cp_direct compute failed; using sched path\n");
        c->cp_dir_state = -1;
        return nullptr;
    }
    ...
    if (bench) {
        ...
        count++;
        if (count == 15) {
            fprintf(stderr, "cielvox2_tts: cp_direct bench (%d calls): set=%.2f ms  compute=%.1f ms  read=%.2f ms\n",
...
float* run_code_pred_kv(cielvox2_tts_context* c, const float* embeds, int n_tokens, int n_past, ggml_tensor* lm_head,
                        bool skip_plan = false) {
    if (!lm_head) {
        return nullptr;
    }
    if (n_past + n_tokens > c->cp_kv_max_ctx) {
        fprintf(stderr, "cielvox2_tts: cp_kv overflow (%d+%d > %d)\n", n_past, n_tokens, c->cp_kv_max_ctx);
        return nullptr;
    }
    ...
    if (env_bool_default("STELNETTTS_CIELVOX2_TTS_CP_DIRECT", cp_direct_default) && !c->cp_cpu_pinned &&
    ...
    const bool o15 = env_bool_default("STELNETTTS_CIELVOX2_TTS_O15", false);
    ...
    // Step-0 cache (PLAN #52 step 4 follow-on, gated CIELVOX2_TTS_CP_STEP0_CACHE=1):
    ...
    const bool use_step0_cache =
        (o15 && n_tokens == 2 && n_past == 0 && !c->cp_cpu_pinned && env_bool("STELNETTTS_CIELVOX2_TTS_CP_STEP0_CACHE"));
    if (use_step0_cache) {
        ggml_cgraph* s0_gf = cp_step0_get_or_build(c);
        ggml_backend_sched_t s0_sched = cp_step0_pick_sched(c);
        if (s0_gf && s0_sched) {
            const bool bench_s0 = env_bool("STELNETTTS_CIELVOX2_TTS_BENCH");
            ...
            if (!c->cp_step0_reserved) {
                ...
                if (!ggml_backend_sched_alloc_graph(s0_sched, s0_gf)) {
                    fprintf(stderr, "cielvox2_tts: cp_step0 alloc_graph failed\n");
                    return nullptr;
                }
            ...
            if (ggml_backend_sched_graph_compute(s0_sched, s0_gf) != GGML_STATUS_SUCCESS) {
                fprintf(stderr, "cielvox2_tts: cp_step0 compute failed\n");
                return nullptr;
            }
            ...
            if (bench_s0) {
                ...
                if (count == 14) {
                    fprintf(stderr,
                            "cielvox2_tts: cp_step0_cache (%d calls): alloc=%.2f ms  compute=%.1f ms  read=%.2f ms\n",
    ...
    // per-step rebuild). The skip is opt-in via CIELVOX2_TTS_O15_SKIP_REALLOC=1.
    //
    // WARNING (verified on M1 Metal, 2026-06-20, cielvox2-tts-0.6b): with the
    ...
    const bool skip_realloc = can_skip && c->cp_t1_allocated && env_bool("STELNETTTS_CIELVOX2_TTS_O15_SKIP_REALLOC");
    ...
    if (ggml_backend_sched_graph_compute(sched, gf) != GGML_STATUS_SUCCESS) {
        ...
        fprintf(stderr, "cielvox2_tts: code_pred compute failed\n");
        return nullptr;
    }
    if (prof) {
        ggml_backend_sched_set_eval_callback(sched, nullptr, nullptr);
    ...
    if (bench) {
        ...
        if (count == 15) {
            fprintf(stderr,
                    "cielvox2_tts: code_pred_kv bench (%d calls): build=%.2f ms  reset=%.2f ms  alloc=%.2f ms  "
...
bool code_pred_generate_15(cielvox2_tts_context* c, const float* past_hidden_d, const float* last_id_hidden_d,
    ...
    // (`cielvox2_tts_set_temperature`, added 0.6.2) actually takes
    // effect. cparams.temperature defaults to 0.9 in
    // cielvox2_tts_context_default_params(), so untouched callers keep
    // the historical behaviour.
    const float temperature = c->params.temperature > 0 ? c->params.temperature : 0.9f;
    const char* dump_dir = env_str("STELNETTTS_CIELVOX2_TTS_DUMP_DIR");
    ...
    if (stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_EMBD_CHECK")) {
        float ph_l1 = 0.0f, lih_l1 = 0.0f;
        for (int j = 0; j < d; j++)
            ph_l1 += std::abs(past_hidden_d[j]);
        for (int j = 0; j < d; j++)
            lih_l1 += std::abs(last_id_hidden_d[j]);
        fprintf(stderr, "cielvox2_tts: cp_step0 past_hidden_l1=%.4f last_id_hidden_l1=%.4f\n", ph_l1, lih_l1);
        fprintf(stderr, "cielvox2_tts: cp_step0 ph[0..3]=%.8f %.8f %.8f %.8f\n", past_hidden_d[0], past_hidden_d[1],
                past_hidden_d[2], past_hidden_d[3]);
        fprintf(stderr, "cielvox2_tts: cp_step0 lih[0..3]=%.8f %.8f %.8f %.8f\n", last_id_hidden_d[0], last_id_hidden_d[1],
                last_id_hidden_d[2], last_id_hidden_d[3]);
    }

    if (!cp.lm_head[0]) {
        fprintf(stderr, "cielvox2_tts: code_pred.lm_head[0] missing\n");
        return false;
    }
    ...
    if (stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_EMBD_CHECK")) {
        ...
        fprintf(stderr, "cielvox2_tts: cp logits0 l1=%.4f top=%d(%.4f) rng=%llu\n", l1, top_idx, top_val,
    ...
    for (int i = 1; i < n_groups - 1; i++) {
        if (!cp.lm_head[i]) {
            fprintf(stderr, "cielvox2_tts: code_pred missing lm_head[%d]\n", i);
            return false;
        }
        int32_t prev = out_codes15[i - 1];
        bool ok = false;
        const bool use_cache = !env_bool("STELNETTTS_CIELVOX2_TTS_NO_EMBD_CACHE");
        if (use_cache && i - 1 < (int)c->codec_embd_cache.size() && c->codec_embd_cache[i - 1]) {
            ...
            if (!cp.codec_embd[i - 1]) {
                fprintf(stderr, "cielvox2_tts: code_pred missing codec_embd[%d]\n", i - 1);
                return false;
            }
...
bool cp_kv_alloc(cielvox2_tts_context* c) {
    if (c->cp_kv_k) {
        return true;
    }
    const auto& hp = c->hp;
...
//   ref/Qwen3-TTS/qwen_tts/inference/cielvox2_tts_model.py
//
...
std::vector<int32_t> tokenise_assistant_text(cielvox2_tts_context* c, const std::string& text) {
...
// qwen_tts/inference/cielvox2_tts_model.py.
std::vector<int32_t> tokenise_user_instruct(cielvox2_tts_context* c, const std::string& instruct) {
    std::vector<int32_t> ids;
    const auto& v = c->vocab;
    push_special(v, ids, "<|im_start|>");
...
std::vector<int32_t> tokenise_ref_text(cielvox2_tts_context* c, const std::string& ref_text) {
    std::vector<int32_t> ids;
    const auto& v = c->vocab;
    push_special(v, ids, "<|im_start|>");
    push_text_block(v, ids, "assistant");
...
// ICL prefill builder
//
// Mirrors `Qwen3TTSForConditionalGeneration.generate` (modeling_cielvox2_tts.py
...
static float* lookup_rows_sched(cielvox2_tts_context* c, ggml_backend_sched_t sched, ggml_tensor* weight,
...
static float* run_codec_embed_sum_ar(cielvox2_tts_context* c, const int32_t* codes16, float* out_cb0_emb) {
    const int d = (int)c->hp.d_model;
...
static bool copy_cp_weights_to_cpu(cielvox2_tts_context* c, enum ggml_type dst_type) {
    ...
    if (!c->cp_cpu_buf) {
        fprintf(stderr, "cielvox2_tts: copy_cp_to_cpu: alloc failed (%zu MB)\n", total_bytes >> 20);
        return false;
    }
    ...
    for (auto* pp : ptrs) {
        ...
        if (ggml_tallocr_alloc(&talloc, cpu_t) != GGML_STATUS_SUCCESS) {
            fprintf(stderr, "cielvox2_tts: copy_cp_to_cpu: alloc failed for tensor %s\n",
                    orig->name[0] ? orig->name : "(unnamed)");
            return false;
        }
        if (!materialize_tensor_copy(cpu_t, orig, tmp_src, tmp_dst, row_f32)) {
            fprintf(stderr, "cielvox2_tts: copy_cp_to_cpu: copy failed for tensor %s (%s -> %s)\n",
    ...
    if (c->talker.token_embd_w && embd_bytes > 0) {
        ...
        ggml_tensor* cpu_t = ggml_new_tensor(c->cp_cpu_ctx, cpu_type, GGML_MAX_DIMS, orig->ne);
        if (ggml_tallocr_alloc(&talloc, cpu_t) != GGML_STATUS_SUCCESS) {
            fprintf(stderr, "cielvox2_tts: copy_cp_to_cpu: alloc failed for talker.token_embd_w\n");
            return false;
        }
        if (!materialize_tensor_copy(cpu_t, orig, tmp_src, tmp_dst, row_f32)) {
            fprintf(stderr, "cielvox2_tts: copy_cp_to_cpu: copy failed for talker.token_embd_w (%s -> %s)\n",
    ...
    if (c->params.verbosity >= 1) {
        ...
        fprintf(stderr, "cielvox2_tts: code_pred CPU-pinned (%zu tensors, %zu MB, copy=%s)\n", n_tensors,
...
float* sum_codec_embeds(cielvox2_tts_context* c, const int32_t* ref_code_TC, int T_codec) {
    const int d = (int)c->hp.d_model;
    const int n_groups = (int)c->hp.n_code_groups;

    auto& cp = c->code_pred;
    if ((int)cp.codec_embd.size() < n_groups - 1) {
        fprintf(stderr, "cielvox2_tts: code_predictor codec_embedding tables missing (%zu/%d)\n", cp.codec_embd.size(),
                n_groups - 1);
        return nullptr;
    }
...
bool build_icl_prefill_embeds(cielvox2_tts_context* c, const std::string& syn_text, const std::string& ref_text,
    ...
    if (!c->runtime_ref_codes.empty() && (int)c->runtime_spk_emb.size() == d) {
        ...
        if (c->vp_active < 0) {
            fprintf(stderr, "cielvox2_tts: no voice — call cielvox2_tts_load_voice_pack or cielvox2_tts_set_voice_prompt\n");
            return false;
        }
        ...
        if (!spk_t || !code_t) {
            fprintf(stderr, "cielvox2_tts: voice '%s' missing spk_embd / ref_code\n", voice_name.c_str());
            return false;
        }
        T_codec = (int)code_t->ne[1];
        if ((int)code_t->ne[0] != n_groups) {
            fprintf(stderr, "cielvox2_tts: ref_code groups mismatch: %d vs %d\n", (int)code_t->ne[0], n_groups);
            return false;
        }
    ...
    if ((int)syn_ids.size() < 8 || (int)ref_ids.size() < 5) {
        fprintf(stderr, "cielvox2_tts: prompt too short (syn=%zu ref=%zu)\n", syn_ids.size(), ref_ids.size());
        return false;
    }
    ...
    if (const char* dd = env_str("STELNETTTS_CIELVOX2_TTS_DUMP_DIR")) {
    ...
    if (c->params.verbosity >= 1) {
        fprintf(stderr,
                "cielvox2_tts: ICL prefill: role=3 + bridge=%d + icl=%d (text_lens=%d codec_lens=%d) = T=%d  "
                "trailing=%d\n",
...
// `Qwen3TTSForConditionalGeneration.generate` (modeling_cielvox2_tts.py
...
// `c->runtime_spk_emb` by cielvox2_tts_set_speaker_by_name).
//
// Layout (per upstream):
...
bool build_customvoice_prefill_embeds(cielvox2_tts_context* c, const std::string& syn_text,
                                      std::vector<float>& prefill_embeds, int& T_prefill,
                                      std::vector<float>& trailing_text_hidden, int& M_trailing) {
    const auto& hp = c->hp;
    const int d = (int)hp.d_model;
    ...
    if ((int)c->runtime_spk_emb.size() != d) {
        fprintf(stderr, "cielvox2_tts[customvoice]: no speaker — call cielvox2_tts_set_speaker_by_name first\n");
        return false;
    }

    auto syn_ids = tokenise_assistant_text(c, syn_text);
    if ((int)syn_ids.size() < 9) {
        fprintf(stderr, "cielvox2_tts[customvoice]: prompt too short (%zu)\n", syn_ids.size());
        return false;
    }
    ...
    // codec bridge. Mirrors modeling_cielvox2_tts.py lines 2076-2095 for the
    ...
    if (N <= 0) {
        fprintf(stderr, "cielvox2_tts[customvoice]: empty text content\n");
        free(tts_special_emb);
        free(role_emb);
        return false;
    }
    ...
    if (c->params.verbosity >= 1) {
        if (M_instruct > 0) {
            fprintf(stderr,
                    "cielvox2_tts[customvoice]: prefill instruct=%d + role=3 + bridge=%d + text=%d + final=1 = T=%d "
                    "trailing=1\n",
                    M_instruct, L_bridge, L_text, T_prefill);
        } else {
            fprintf(stderr,
                    "cielvox2_tts[customvoice]: prefill role=3 + bridge=%d + text=%d + final=1 = T=%d trailing=1\n",
...
//   [text_block(N+1)]  text_proj(text_content)+tts_eos | codec_pad×(N+1)
//   [final(1)]         tts_pad | codec_bos_emb
bool build_voicedesign_prefill_embeds(cielvox2_tts_context* c, const std::string& instruct_text,
    ...
    if (instruct_text.empty()) {
        fprintf(stderr, "cielvox2_tts[voicedesign]: empty instruct — call cielvox2_tts_set_instruct first\n");
        return false;
    }

    auto syn_ids = tokenise_assistant_text(c, syn_text);
    if ((int)syn_ids.size() < 9) {
        fprintf(stderr, "cielvox2_tts[voicedesign]: prompt too short (%zu)\n", syn_ids.size());
        return false;
    }
    auto instruct_ids = tokenise_user_instruct(c, instruct_text);
    if (instruct_ids.empty()) {
        fprintf(stderr, "cielvox2_tts[voicedesign]: instruct tokenised to empty\n");
        return false;
    }
    ...
    const int N = (int)text_content.size();
    if (N <= 0) {
        fprintf(stderr, "cielvox2_tts[voicedesign]: empty text content\n");
        free(instruct_emb);
        free(tts_special_emb);
        free(role_emb);
        return false;
    ...
    if (c->params.verbosity >= 1) {
        fprintf(stderr,
                "cielvox2_tts[voicedesign]: prefill instruct=%d + role=3 + bridge=%d + text=%d + final=1 = T=%d "
                "trailing=1\n",
                M_instruct, L_bridge, L_text, T_prefill);
    }
...
// ---------------------------------------------------------------------------
// §232 FASTCONV gate (default ON; opt out with CIELVOX2_TTS_CODEC_FASTCONV=0
...
// hot spots found by CIELVOX2_TTS_CODEC_TRACE on M1 Metal (im2col of K=1
...
static bool codec_fastconv_enabled() {
    return env_bool_default("STELNETTTS_CIELVOX2_TTS_CODEC_FASTCONV", true);
}
...
// ---------------------------------------------------------------------------
static ggml_cgraph* build_graph_codec_decode(cielvox2_tts_context* c, int T, bool keep_stage_outputs = false) {
...
static bool load_cenc(cielvox2_tts_context* c);

// ---------------------------------------------------------------------------
// Load codec GGUF into g3t_codec.
// ---------------------------------------------------------------------------
static bool load_codec(cielvox2_tts_context* c, const char* path) {
    auto& codec = c->codec;
    auto& hp = codec.hp;
    ...
    if (!meta) {
        fprintf(stderr, "cielvox2_tts: codec: cannot open '%s'\n", path);
        return false;
    }
    hp.n_layers = core_gguf::kv_u32(meta, "cielvox2tts_codec.dec.n_layers", hp.n_layers);
    hp.d_model = core_gguf::kv_u32(meta, "cielvox2tts_codec.dec.d_model", hp.d_model);
    hp.n_heads = core_gguf::kv_u32(meta, "cielvox2tts_codec.dec.n_heads", hp.n_heads);
    hp.head_dim = core_gguf::kv_u32(meta, "cielvox2tts_codec.dec.head_dim", hp.head_dim);
    hp.ff_dim = core_gguf::kv_u32(meta, "cielvox2tts_codec.dec.ff_dim", hp.ff_dim);
    hp.n_q = core_gguf::kv_u32(meta, "cielvox2tts_codec.dec.n_quantizers", hp.n_q);
    hp.codebook_size = core_gguf::kv_u32(meta, "cielvox2tts_codec.dec.codebook_size", hp.codebook_size);
    hp.latent_dim = core_gguf::kv_u32(meta, "cielvox2tts_codec.dec.latent_dim", hp.latent_dim);
    hp.decoder_dim = core_gguf::kv_u32(meta, "cielvox2tts_codec.dec.decoder_dim", hp.decoder_dim);
    hp.sliding_window = core_gguf::kv_u32(meta, "cielvox2tts_codec.dec.sliding_window", hp.sliding_window);
    hp.max_pos = core_gguf::kv_u32(meta, "cielvox2tts_codec.dec.max_pos", hp.max_pos);
    hp.rope_theta = core_gguf::kv_f32(meta, "cielvox2tts_codec.dec.rope_theta", hp.rope_theta);
    hp.rms_norm_eps = core_gguf::kv_f32(meta, "cielvox2tts_codec.dec.rms_norm_eps", hp.rms_norm_eps);
    core_gguf::free_metadata(meta);

    // Pass 2: weights — default to GPU on all GPU backends. Env overrides:
    //   CIELVOX2_TTS_CODEC_FORCE_METAL=1 — force Metal GPU path with per-op
    //                                   trace callback (instrumentation).
    //   CIELVOX2_TTS_CODEC_GPU=1         — explicit GPU override (no-op when
    //                                   GPU is already the default).
    //   CIELVOX2_TTS_CODEC_CPU=1         — force CPU codec for A/B timing.
    const bool force_metal = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_FORCE_METAL") != nullptr;
    const bool force_gpu = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_GPU") != nullptr;
    const bool force_cpu = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_CPU") != nullptr;
    const bool default_gpu = cielvox2_tts_codec_use_gpu_by_default(c);
    ...
    if (c->params.verbosity >= 1) {
        const char* why =
            force_metal
                ? "CIELVOX2_TTS_CODEC_FORCE_METAL=1"
                : (force_gpu ? "CIELVOX2_TTS_CODEC_GPU=1"
                             : (force_cpu ? "CIELVOX2_TTS_CODEC_CPU=1" : (default_gpu ? "GPU default" : "CPU default")));
        fprintf(stderr, "cielvox2_tts: codec: %s - loading weights onto %s\n", why, ggml_backend_name(weight_backend));
    }
    core_gguf::WeightLoad wl;
    if (!core_gguf::load_weights(path, weight_backend, "codec", wl)) {
        fprintf(stderr, "cielvox2_tts: codec: failed to load weights from '%s'\n", path);
        return false;
    }
    codec.ctx_w = wl.ctx;
    codec.buf_w = wl.buf;
    ...
    // ---------- permute tconv weights for causal-trans-conv (mul_mat + col2im) ----------
    {
        ...
        if (!codec.ctx_perm) {
            fprintf(stderr, "cielvox2_tts: codec: failed to init permuted weight context\n");
            return false;
        }
        ...
        if (!codec.buf_perm) {
            fprintf(stderr, "cielvox2_tts: codec: failed to alloc permuted weight buffer\n");
            return false;
        }
        ...
        if (!to_cast.empty()) {
            ...
            if (codec.ctx_conv32) {
                ...
                if (codec.buf_conv32) {
                    ...
                    fprintf(stderr, "cielvox2_tts: codec: fastconv F32 kernel bake alloc failed (non-fatal)\n");
    ...
    // CPU-only scheduler for the codec graph (see cielvox2_tts_context).
    if (!c->codec_sched) {
        ggml_backend_t cpu_backends[1] = {c->backend_cpu};
        c->codec_sched = ggml_backend_sched_new(cpu_backends, nullptr, 1, 8192, false, false);
        if (!c->codec_sched) {
            fprintf(stderr, "cielvox2_tts: codec: failed to create CPU scheduler\n");
            return false;
        }
    }
    ...
    codec.loaded = true;
    if (c->params.verbosity >= 1) {
        fprintf(stderr, "cielvox2_tts: codec loaded from '%s'  (%uL d=%u/%u  rvq=%u)\n", path, hp.n_layers, hp.d_model,
...
// CIELVOX2_TTS_CODEC_TRACE=1 (or CIELVOX2_TTS_CODEC_FORCE_METAL=1, which implies
...
//   CIELVOX2_TTS_CODEC_FORCE_METAL=1 — force Metal GPU path with per-op tracing.
//   CIELVOX2_TTS_CODEC_GPU=1         — explicit GPU override (usually no-op).
//   CIELVOX2_TTS_CODEC_CPU=1         — force CPU codec for A/B timing.
static ggml_backend_sched_t codec_pick_sched(cielvox2_tts_context* c) {
    const bool force_metal = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_FORCE_METAL") != nullptr;
    const bool force_gpu = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_GPU") != nullptr;
    const bool force_cpu = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_CPU") != nullptr;
    const bool codec_gpu = force_metal || force_gpu || (!force_cpu && cielvox2_tts_codec_use_gpu_by_default(c));
    if (codec_gpu) {
...
// ---------------------------------------------------------------------------
static float* codec_decode_window(cielvox2_tts_context* c, const int32_t* codes_t, int n_q, int T_full, int window_start,
    ...
    ggml_backend_sched_reset(sched);
    if (!ggml_backend_sched_alloc_graph(sched, gf)) {
        fprintf(stderr, "cielvox2_tts: codec: graph alloc failed\n");
        return nullptr;
    }
    ...
    if (stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_TRACE")) {
        fprintf(stderr, "cielvox2_tts: codec: tracing %d nodes\n", ts.total);
    ...
    if (st != GGML_STATUS_SUCCESS) {
        fprintf(stderr, "cielvox2_tts: codec: compute failed (status=%d)\n", (int)st);
        return nullptr;
    }
...
// rolling state, so we decode in chunks of CIELVOX2_TTS_CODEC_CHUNK frames, each
// prefixed with CIELVOX2_TTS_CODEC_CTX left-context frames whose PCM is discarded.
// Peak VRAM depends on (ctx + chunk), not total length.
// chunk <= 0 or T_codec <= chunk → single full-sequence pass (legacy path).
// Env-tunable: CIELVOX2_TTS_CODEC_CHUNK (default 150), CIELVOX2_TTS_CODEC_CTX (default 128).
// ---------------------------------------------------------------------------
static float* codec_decode_codes(cielvox2_tts_context* c, const int32_t* codes, int T_codec, int* out_n_samples) {
    if (out_n_samples) {
        *out_n_samples = 0;
    }
    const auto& codec = c->codec;
    ...
    const bool cuda_codec = cielvox2_tts_codec_decode_uses_cuda(c);
    const int cuda_chunk_cap = 64;
    int chunk = cuda_codec ? cuda_chunk_cap : 150;
    int ctx = cuda_codec ? std::max(window, 96) : 128;
    if (const char* e = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_CHUNK"))
        chunk = atoi(e);
    if (const char* e = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_CTX"))
        ctx = atoi(e);
    if (ctx < window)
        ctx = window; // left-context must cover the sliding window
    if (cuda_codec && !stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_ALLOW_FULL")) {
        if (chunk <= 0) {
    ...
    if (c->params.verbosity >= 1) {
        fprintf(stderr, "cielvox2_tts: codec: chunked decode T=%d chunk=%d ctx=%d%s\n", T_codec, chunk, ctx,
                cuda_codec ? " (CUDA cap)" : "");
    }
...
// ---------------------------------------------------------------------------
static float* codec_extract_stage(cielvox2_tts_context* c, const int32_t* codes, int T_codec, const char* stage_name,
                                  int* out_n) {
    if (out_n) {
        *out_n = 0;
    }
    ...
    codec_trace_state ts{sched, 0, ggml_graph_n_nodes(gf)};
    const bool trace = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_TRACE") ||
                       stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_CODEC_FORCE_METAL");
    if (trace) {
        fprintf(stderr, "cielvox2_tts: codec: tracing %d nodes (sched=%s, stage=%s)\n", ts.total,
    ...
    if (st != GGML_STATUS_SUCCESS) {
        fprintf(stderr, "cielvox2_tts: codec: compute failed (status=%d, last_op_idx=%d)\n", (int)st, ts.idx);
        return nullptr;
    }

    ggml_tensor* t = ggml_graph_get_tensor(gf, stage_name);
    if (!t) {
        fprintf(stderr, "cielvox2_tts: codec: stage '%s' not found in graph\n", stage_name);
        return nullptr;
    }
...
static bool cenc_rvq_encode(cielvox2_tts_context* c, const float* emb, int T, std::vector<int32_t>& out_codes);
...
static ggml_cgraph* build_cenc_seanet_only_graph(cielvox2_tts_context* c, int n_samples) {
...
static ggml_cgraph* build_cenc_xfmr_chunk_graph(cielvox2_tts_context* c, int chunk_T) {
    size_t mem = c->cenc_compute_meta.size();
    ggml_init_params ip = {mem, c->cenc_compute_meta.data(), true};
    ggml_context* ctx0 = ggml_init(ip);
    ggml_cgraph* gf = ggml_new_graph_custom(ctx0, 8192, false);
...
static ggml_cgraph* build_cenc_downsample_graph(cielvox2_tts_context* c, int T_enc) {
    size_t mem = c->cenc_compute_meta.size();
    ggml_init_params ip = {mem, c->cenc_compute_meta.data(), true};
    ggml_context* ctx0 = ggml_init(ip);
    ggml_cgraph* gf = ggml_new_graph_custom(ctx0, 8192, false);
...
static bool run_cenc_chunked(cielvox2_tts_context* c, const float* audio, int n_samples, std::vector<int32_t>& codes,
                             int& T_frames) {
    const int T_enc = n_samples / 960;
    fprintf(stderr, "cielvox2_tts: cenc: chunked encoder T_enc=%d (%.1fs ref audio, chunk=%d ctx=%d)\n", T_enc,
    ...
    std::vector<float> seanet_buf((size_t)512 * T_enc);
    {
        ...
        ggml_backend_sched_reset(c->sched);
        if (!ggml_backend_sched_alloc_graph(c->sched, gf)) {
            fprintf(stderr, "cielvox2_tts: cenc: seanet alloc failed\n");
            return false;
        }
        ggml_backend_tensor_set(ggml_graph_get_tensor(gf, "pcm_input"), audio, 0, (size_t)n_samples * sizeof(float));
        if (ggml_backend_sched_graph_compute(c->sched, gf) != GGML_STATUS_SUCCESS) {
            fprintf(stderr, "cielvox2_tts: cenc: seanet compute failed\n");
            return false;
        }
    ...
    std::vector<float> xfmr_out((size_t)512 * T_enc);
    {
        ...
        while (result_start < T_enc) {
            ...
            ggml_backend_sched_reset(c->sched);
            if (!ggml_backend_sched_alloc_graph(c->sched, gf)) {
                fprintf(stderr, "cielvox2_tts: cenc: xfmr chunk alloc failed (chunk_T=%d)\n", chunk_T);
                return false;
            }
            ...
            if (ggml_backend_sched_graph_compute(c->sched, gf) != GGML_STATUS_SUCCESS) {
                fprintf(stderr, "cielvox2_tts: cenc: xfmr chunk compute failed (chunk_T=%d start=%d)\n", chunk_T,
                        result_start);
                return false;
            }
    ...
    // ── Phase 3: Downsample + RVQ ──
    {
        ...
        ggml_backend_sched_reset(c->sched);
        if (!ggml_backend_sched_alloc_graph(c->sched, gf)) {
            fprintf(stderr, "cielvox2_tts: cenc: downsample alloc failed\n");
            return false;
        }
        ggml_backend_tensor_set(ggml_graph_get_tensor(gf, "cenc_ds_in"), xfmr_out.data(), 0,
                                xfmr_out.size() * sizeof(float));
        if (ggml_backend_sched_graph_compute(c->sched, gf) != GGML_STATUS_SUCCESS) {
            fprintf(stderr, "cielvox2_tts: cenc: downsample compute failed\n");
            return false;
        }
...
static ggml_cgraph* build_cenc_graph(cielvox2_tts_context* c, int n_samples) {
    const auto& ce = c->cenc;
    size_t mem = c->cenc_compute_meta.size();
    ggml_init_params ip = {mem, c->cenc_compute_meta.data(), true};
    ggml_context* ctx0 = ggml_init(ip);
...
static bool run_cenc(cielvox2_tts_context* c, const float* audio, int n_samples, std::vector<int32_t>& codes,
                     int& T_frames) {
    ...
    ggml_backend_sched_reset(c->sched);
    if (!ggml_backend_sched_alloc_graph(c->sched, gf)) {
        fprintf(stderr, "cielvox2_tts: cenc: graph alloc failed\n");
        return false;
    }
    ggml_backend_tensor_set(ggml_graph_get_tensor(gf, "pcm_input"), audio, 0, (size_t)n_samples * sizeof(float));
    ...
    if (ggml_backend_sched_graph_compute(c->sched, gf) != GGML_STATUS_SUCCESS) {
        fprintf(stderr, "cielvox2_tts: cenc: compute failed\n");
        return false;
    }
    ggml_tensor* emb_t = ggml_graph_get_tensor(gf, "enc_emb");
    T_frames = (int)emb_t->ne[1];
...
static bool load_cenc(cielvox2_tts_context* c) {
    ...
    auto need = [&](const char* n) -> ggml_tensor* {
        auto* t = G(n);
        if (!t) {
            fprintf(stderr, "cielvox2_tts: cenc: missing '%s'\n", n);
        }
        return t;
    };
    ...
    ce.loaded = true;
    if (c->params.verbosity >= 1) {
        fprintf(stderr, "cielvox2_tts: codec encoder loaded (SEANet+xfmr+RVQ)\n");
    }
    return true;
}
...
static ggml_cgraph* build_graph_spk_enc(cielvox2_tts_context* c, int T_mel) {
...
static bool load_spk_enc(cielvox2_tts_context* c) {
    ...
    if (!c->hp.spk_names.empty()) {  // FIX #3c: only skip for confirmed CustomVoice; VoiceDesign/Base fall through safely (soft load)
        return true;
    }
    auto& spk = c->spk_enc;
    char buf[256];
    ...
    if (!spk.blk0.w) {
        if (c->hp.n_layers == 28) {  // 1.7B: skip speaker encoder if missing
            fprintf(stderr, "cielvox2_tts: warning: 1.7B model has no speaker encoder tensors; running voiceless\n");
            spk.loaded = false;
            return true;
        }
        return false;
    ...
    spk.loaded = true;
    if (c->params.verbosity >= 1) {
        fprintf(stderr, "cielvox2_tts: speaker encoder loaded (ECAPA-TDNN 128→%u)\n", c->hp.spk_enc_dim);
    }
    return true;
}
...
static std::vector<float> run_spk_enc(cielvox2_tts_context* c, const float* mel_TC, int T_mel) {
...
extern "C" struct cielvox2_tts_context_params cielvox2_tts_context_default_params(void) {
    cielvox2_tts_context_params p{};
    p.n_threads = 4;
    p.verbosity = 1;
    p.use_gpu = true;
    p.temperature = 0.0f;
...
extern "C" void cielvox2_tts_set_temperature(struct cielvox2_tts_context* ctx, float temperature) {
    if (!ctx)
        return;
    if (temperature < 0.0f)
        temperature = 0.0f;
...
extern "C" void cielvox2_tts_set_seed(struct cielvox2_tts_context* ctx, uint64_t seed) {
    if (!ctx)
        return;
    ctx->params.seed = seed;
}

static void build_embd_caches(cielvox2_tts_context* c) {
...
extern "C" struct cielvox2_tts_context* cielvox2_tts_init_codec_only(const char* codec_path,
                                                               struct cielvox2_tts_context_params params) {
    auto* c = new cielvox2_tts_context();
    c->params = params;
    c->n_threads = params.n_threads > 0 ? params.n_threads : 4;

    c->backend_cpu = core_cpu_backend::init();
    if (!c->backend_cpu) {
        fprintf(stderr, "cielvox2_tts: failed to init CPU backend\n");
        delete c;
        return nullptr;
    }
    ...
    cielvox2_tts_route_off_vulkan(c, params.verbosity);

    if (!load_codec(c, codec_path)) {
        fprintf(stderr, "cielvox2_tts: codec-only init: failed to load codec\n");
        cielvox2_tts_free(c);
        return nullptr;
    }
...
extern "C" struct cielvox2_tts_context* cielvox2_tts_init_from_file(const char* path_model,
                                                              struct cielvox2_tts_context_params params) {
    auto* c = new cielvox2_tts_context();
    c->params = params;
    c->n_threads = params.n_threads > 0 ? params.n_threads : 4;
    ...
    // ---- pass 1: read hparams + vocab via metadata-only context ----
    {
        ...
        if (!g) {
            fprintf(stderr, "cielvox2_tts: failed to read GGUF '%s'\n", path_model);
            delete c;
            return nullptr;
        }
        auto& hp = c->hp;
        hp.n_layers = kv_u32(g, "cielvox2tts.talker.n_layers", hp.n_layers);
        hp.d_model = kv_u32(g, "cielvox2tts.talker.d_model", hp.d_model);
        hp.n_heads = kv_u32(g, "cielvox2tts.talker.n_heads", hp.n_heads);
        hp.n_kv_heads = kv_u32(g, "cielvox2tts.talker.n_kv_heads", hp.n_kv_heads);
        hp.head_dim = kv_u32(g, "cielvox2tts.talker.head_dim", hp.head_dim);
        hp.ff_dim = kv_u32(g, "cielvox2tts.talker.ff_dim", hp.ff_dim);
        hp.vocab_size = kv_u32(g, "cielvox2tts.talker.vocab_size", hp.vocab_size);
        hp.text_vocab_size = kv_u32(g, "cielvox2tts.talker.text_vocab_size", hp.text_vocab_size);
        hp.text_hidden_size = kv_u32(g, "cielvox2tts.talker.text_hidden_size", hp.text_hidden_size);
        hp.n_code_groups = kv_u32(g, "cielvox2tts.talker.n_code_groups", hp.n_code_groups);
        hp.max_pos = kv_u32(g, "cielvox2tts.talker.max_pos", hp.max_pos);
        hp.rope_theta = kv_f32(g, "cielvox2tts.talker.rope_theta", hp.rope_theta);
        hp.rms_norm_eps = kv_f32(g, "cielvox2tts.talker.rms_norm_eps", hp.rms_norm_eps);
        hp.rope_interleaved = kv_bool(g, "cielvox2tts.talker.rope_interleaved", hp.rope_interleaved);
        {
            int mr = gguf_find_key(g, "cielvox2tts.talker.mrope_section");
            if (mr >= 0) {
        ...
        hp.cp_n_layers = kv_u32(g, "cielvox2tts.code_pred.n_layers", hp.cp_n_layers);
        hp.cp_d_model = kv_u32(g, "cielvox2tts.code_pred.d_model", hp.cp_d_model);
        hp.cp_n_heads = kv_u32(g, "cielvox2tts.code_pred.n_heads", hp.cp_n_heads);
        hp.cp_n_kv_heads = kv_u32(g, "cielvox2tts.code_pred.n_kv_heads", hp.cp_n_kv_heads);
        hp.cp_n_code_groups = kv_u32(g, "cielvox2tts.code_pred.n_code_groups", hp.cp_n_code_groups);
        hp.cp_vocab_size = kv_u32(g, "cielvox2tts.code_pred.vocab_size", hp.cp_vocab_size);
        hp.cp_max_length = kv_u32(g, "cielvox2tts.code_pred.max_length", hp.cp_max_length);
        hp.spk_enc_dim = kv_u32(g, "cielvox2tts.speaker.enc_dim", hp.spk_enc_dim);
        hp.spk_sample_rate = kv_u32(g, "cielvox2tts.speaker.sample_rate", hp.spk_sample_rate);
        hp.tts_bos_id = kv_u32(g, "cielvox2tts.tts_bos_token_id", hp.tts_bos_id);
        hp.tts_eos_id = kv_u32(g, "cielvox2tts.tts_eos_token_id", hp.tts_eos_id);
        hp.tts_pad_id = kv_u32(g, "cielvox2tts.tts_pad_token_id", hp.tts_pad_id);
        hp.im_start_id = kv_u32(g, "cielvox2tts.im_start_token_id", hp.im_start_id);
        hp.im_end_id = kv_u32(g, "cielvox2tts.im_end_token_id", hp.im_end_id);
        hp.assistant_id = kv_u32(g, "cielvox2tts.assistant_token_id", hp.assistant_id);
        hp.codec_bos_id = kv_u32(g, "cielvox2tts.talker.codec_bos_id", hp.codec_bos_id);
        hp.codec_eos_id = kv_u32(g, "cielvox2tts.talker.codec_eos_token_id", hp.codec_eos_id);
        hp.codec_pad_id = kv_u32(g, "cielvox2tts.talker.codec_pad_id", hp.codec_pad_id);
        hp.codec_think_id = kv_u32(g, "cielvox2tts.talker.codec_think_id", hp.codec_think_id);
        hp.codec_nothink_id = kv_u32(g, "cielvox2tts.talker.codec_nothink_id", hp.codec_nothink_id);
        hp.codec_think_bos_id = kv_u32(g, "cielvox2tts.talker.codec_think_bos_id", hp.codec_think_bos_id);
        hp.codec_think_eos_id = kv_u32(g, "cielvox2tts.talker.codec_think_eos_id", hp.codec_think_eos_id);

        // Model variant + CustomVoice speaker table (Phase 1 of PLAN #57).
        hp.tts_model_type = core_gguf::kv_str(g, "cielvox2tts.tts_model_type", "");
        if (hp.tts_model_type.empty()) hp.tts_model_type = core_gguf::kv_str(g, "qwen3tts.tts_model_type", "base");
        hp.spk_names = core_gguf::kv_str_array(g, "cielvox2tts.spk_names");
        if (hp.spk_names.empty()) hp.spk_names = core_gguf::kv_str_array(g, "qwen3tts.spk_names");
        {
            int64_t k = gguf_find_key(g, "cielvox2tts.spk_token_ids");
            if (k < 0) k = gguf_find_key(g, "qwen3tts.spk_token_ids");
            if (k >= 0) {
                int n = gguf_get_arr_n(g, k);
                const auto* d = (const uint32_t*)gguf_get_arr_data(g, k);
                hp.spk_token_ids.assign(d, d + n);
            }
            k = gguf_find_key(g, "cielvox2tts.spk_dialect_token_ids");
            if (k < 0) k = gguf_find_key(g, "qwen3tts.spk_dialect_token_ids");
            if (k >= 0) {
                int n = gguf_get_arr_n(g, k);
                const auto* d = (const uint32_t*)gguf_get_arr_data(g, k);
        ...
        // cielvox2_tts_set_language_by_name. Optional — older GGUFs lack it.
        hp.codec_language_names = core_gguf::kv_str_array(g, "cielvox2tts.codec_language_names");
        {
            int64_t k = gguf_find_key(g, "cielvox2tts.codec_language_ids");
            if (k >= 0) {
                int n = gguf_get_arr_n(g, k);
                const auto* d = (const uint32_t*)gguf_get_arr_data(g, k);
    ...
    core_gguf::WeightLoad wl;
    if (!core_gguf::load_weights(path_model, c->backend, "cielvox2_tts", wl)) {
        fprintf(stderr, "cielvox2_tts: failed to load weights from '%s'\n", path_model);
        delete c;
        return nullptr;
    }
    c->ctx_w = wl.ctx;
    ...
    if (!load_talker(c)) {
        fprintf(stderr, "cielvox2_tts: talker weights incomplete\n");
        cielvox2_tts_free(c);
        return nullptr;
    }
    ...
    // F16 case untested locally; flip with CIELVOX2_TTS_FUSED_QKV=1 to bench.
    if (env_bool("STELNETTTS_CIELVOX2_TTS_FUSED_QKV")) {
            ...
            if (c->fused_ctx) {
                ...
                if (c->fused_buf) {
                    ...
                    if (params.verbosity >= 1) {
                        fprintf(stderr, "cielvox2_tts: fused QKV for %zu talker layers (%d+%d+%d→%d, type=%s)\n",
    ...
    if (cp_be && std::strncmp(cp_be, "cpu", 3) == 0) {
        if (!copy_cp_weights_to_cpu(c, code_pred_cpu_copy_type_from_env(cp_be))) {
            fprintf(stderr, "cielvox2_tts: code_pred CPU pin requested but copy failed; using main backend\n");
        }
    }
    ...
    // projection), or via CIELVOX2_TTS_CP_MTP_NOFUSE=1 for A/B bisection.
    c->cp_mtp_fused = c->code_pred.small_to_mtp_w && !c->cp_cpu_pinned && !env_bool("STELNETTTS_CIELVOX2_TTS_CP_MTP_NOFUSE");
    if (c->cp_mtp_fused && c->params.verbosity >= 1) {
        fprintf(stderr, "cielvox2_tts: small_to_mtp fused into code_pred graph (no per-step projection dispatch)\n");
    }
    ...
    if (!cp_kv_alloc(c)) {
        fprintf(stderr, "cielvox2_tts: cp_kv pre-alloc failed\n");
        cielvox2_tts_free(c);
        return nullptr;
    }
    ggml_backend_sched_t cp_sched = code_pred_pick_sched(c);
    if (!code_pred_reserve_sched(c, cp_sched)) {
        fprintf(stderr, "cielvox2_tts: code_pred sched reserve failed\n");
        cielvox2_tts_free(c);
        return nullptr;
    }
    ...
    if (!kv_alloc(c, /*max_ctx=*/4096)) {
        fprintf(stderr, "cielvox2_tts: kv allocation failed\n");
        cielvox2_tts_free(c);
        return nullptr;
    }

    if (params.verbosity >= 1) {
        fprintf(stderr, "cielvox2_tts: loaded %s  (talker %uL/%u  code_pred %uL  vocab %u)\n", path_model, c->hp.n_layers,
                c->hp.d_model, c->hp.cp_n_layers, c->hp.vocab_size);
    }
    return c;
}

extern "C" int cielvox2_tts_set_codec_path(struct cielvox2_tts_context* ctx, const char* path) {
    if (!ctx || !path) {
        return -1;
    }
    ctx->codec_path = path;
...
extern "C" int cielvox2_tts_set_voice_prompt(struct cielvox2_tts_context* ctx, const char* wav_path) {
    if (!ctx) {
        return -1;
    }
    ...
    if (!ctx->spk_enc.loaded) {
        fprintf(stderr, "cielvox2_tts: set_voice_prompt: speaker encoder not loaded\n");
        return -1;
    }
    ...
    if (!f) {
        fprintf(stderr, "cielvox2_tts: cannot open wav '%s'\n", wav_path);
        return -1;
    }
    // RIFF header
    char riff[4];
    ...
    if (riff[0] != 'R' || riff[1] != 'I' || riff[2] != 'F' || riff[3] != 'F') {
        std::fclose(f);
        fprintf(stderr, "cielvox2_tts: not a RIFF WAV\n");
        return -1;
    }
    ...
    while (!found_data) {
        ...
        if (id[0] == 'f' && id[1] == 'm' && id[2] == 't' && id[3] == ' ') {
            ...
            if (sr != 24000) {
                std::fclose(f);
                fprintf(stderr, "cielvox2_tts: voice prompt must be 24kHz, got %d Hz\n", sr);
                return -1;
            }
            ...
            if (bits == 16 && !is_float) {
                ...
                std::fclose(f);
                fprintf(stderr, "cielvox2_tts: unsupported WAV format: %d bits, float=%d\n", bits, (int)is_float);
                return -1;
            }
            found_data = true;
        } else {
    ...
    std::fclose(f);
    if (samples.empty()) {
        fprintf(stderr, "cielvox2_tts: no audio in wav\n");
        return -1;
    }
    ...
    if (mel.empty()) {
        fprintf(stderr, "cielvox2_tts: mel computation failed\n");
        return -1;
    }

    auto emb = !ctx->spk_enc.loaded ? std::vector<float>() : run_spk_enc(ctx, mel.data(), T_mel);
    if (!ctx->spk_enc.loaded) {
        fprintf(stderr, "cielvox2_tts: no speaker encoder — running voiceless synthesis\n");
    } else if (emb.empty()) {
        fprintf(stderr, "cielvox2_tts: ECAPA forward failed\n");
        return -1;
    }
    ...
    if (ctx->cenc.loaded) {
        ...
        if (!run_cenc(ctx, samples.data(), (int)samples.size(), ctx->runtime_ref_codes, T_frames)) {
            fprintf(stderr, "cielvox2_tts: voice prompt codec encode failed\n");
            return -1;
        }
        if (ctx->params.verbosity >= 1) {
            fprintf(stderr, "cielvox2_tts: voice prompt encoded: %d codec frames\n", T_frames);
        }
    } else {
        fprintf(stderr, "cielvox2_tts: codec encoder not loaded — ref_codes not available\n");
        return -1;
    }

    if (ctx->params.verbosity >= 1) {
        fprintf(stderr, "cielvox2_tts: voice prompt set from '%s'  (%d samples, T_mel=%d)\n", wav_path,
                (int)samples.size(), T_mel);
    }
    return 0;
}

extern "C" int cielvox2_tts_set_voice_prompt_with_text(struct cielvox2_tts_context* ctx, const char* wav_path,
                                                    const char* ref_text) {
    if (!ctx) {
        return -1;
    }
    ...
    return cielvox2_tts_set_voice_prompt(ctx, wav_path);
}

extern "C" int cielvox2_tts_set_voice_prompt_xvec_only(struct cielvox2_tts_context* ctx, const char* wav_path) {
    if (!ctx) {
        return -1;
    }
    // Use the same WAV loading + ECAPA pipeline as set_voice_prompt.
    int rc = cielvox2_tts_set_voice_prompt(ctx, wav_path);
    if (rc != 0) {
        return rc;
    }
    ...
    ctx->xvec_only = true;
    if (ctx->params.verbosity >= 1) {
        fprintf(stderr, "cielvox2_tts: voice prompt (xvec_only) set from '%s'\n", wav_path);
    }
    return 0;
}
...
extern "C" const int32_t* cielvox2_tts_get_runtime_ref_codes(struct cielvox2_tts_context* ctx, int* out_n) {
    if (out_n) {
        *out_n = 0;
    }
...
extern "C" const float* cielvox2_tts_get_runtime_spk_emb(struct cielvox2_tts_context* ctx, int* out_n) {
    if (out_n) {
        *out_n = 0;
    }
...
extern "C" float* cielvox2_tts_cenc_extract_stage(struct cielvox2_tts_context* ctx, const float* audio, int n_samples,
    ...
    if (!ctx->cenc.loaded) {
        fprintf(stderr, "cielvox2_tts: cenc_extract_stage: encoder not loaded\n");
        return nullptr;
    }
    if (std::strcmp(stage_name, "cenc_codes") == 0) {
        ...
        if (!run_cenc(ctx, audio, n_samples, codes_i32, T_frames)) {
            fprintf(stderr, "cielvox2_tts: cenc_extract_stage: run_cenc failed for cenc_codes\n");
            return nullptr;
        }
    ...
    ggml_tensor* t = ggml_graph_get_tensor(gf, stage_name);
    if (!t) {
        fprintf(stderr, "cielvox2_tts: cenc_extract_stage: '%s' not in graph\n", stage_name);
        return nullptr;
    }
...
extern "C" int cielvox2_tts_load_voice_pack(struct cielvox2_tts_context* ctx, const char* path) {
    if (!ctx || !path) {
        return -1;
    }
    ...
    // backend boundaries during graph build).
    {
        ...
        if (!g) {
            fprintf(stderr, "cielvox2_tts: failed to read voice pack '%s'\n", path);
            return -1;
        }
    ...
    core_gguf::WeightLoad wl;
    if (!core_gguf::load_weights(path, ctx->backend, "cielvox2_tts.voicepack", wl)) {
        fprintf(stderr, "cielvox2_tts: failed to load voice pack tensors from '%s'\n", path);
        return -1;
    }
    ...
    if (ctx->params.verbosity >= 1) {
        fprintf(stderr, "cielvox2_tts: loaded voice pack %s with %zu voices\n", path, ctx->vp_names.size());
...
extern "C" int cielvox2_tts_select_voice(struct cielvox2_tts_context* ctx, const char* name) {
    if (!ctx || !name) {
        return -1;
    }
    ...
    for (size_t i = 0; i < ctx->vp_names.size(); i++) {
        if (ctx->vp_names[i] == name) {
            ctx->vp_active = (int)i;
            if (ctx->params.verbosity >= 1) {
                fprintf(stderr, "cielvox2_tts: selected voice [%zu] '%s'\n", i, name);
            }
            return 0;
        }
    }
...
extern "C" int cielvox2_tts_set_language(struct cielvox2_tts_context* ctx, int codec_language_id) {
    if (!ctx) {
        return -1;
    }
...
extern "C" int cielvox2_tts_set_language_by_name(struct cielvox2_tts_context* ctx, const char* name) {
    if (!ctx || !name) {
        return -1;
    }
...
// FIX #3b: same stale-metadata issue as the internal synthesis check --
// these public accessors also read hp.tts_model_type directly, so the CLI
// layer (which calls these BEFORE reaching the internal override) was still
// seeing stale base for VoiceDesign/CustomVoice 1.7B ggufs. Apply the same
// spk_names / runtime_instruct inference here.
static std::string cielvox2_tts_effective_model_type(struct cielvox2_tts_context* ctx) {
    if (!ctx) return "";
    std::string t = ctx->hp.tts_model_type;
    if (t == "base") {
        if (!ctx->hp.spk_names.empty()) {
            t = "custom_voice";
        } else if (!ctx->spk_enc.loaded) {
            t = "voice_design";
        }
    }
    return t;
...
extern "C" int cielvox2_tts_is_custom_voice(struct cielvox2_tts_context* ctx) {
    return (ctx && cielvox2_tts_effective_model_type(ctx) == "custom_voice") ? 1 : 0;
}

extern "C" int cielvox2_tts_is_voice_design(struct cielvox2_tts_context* ctx) {
    return (ctx && cielvox2_tts_effective_model_type(ctx) == "voice_design") ? 1 : 0;
}

extern "C" int cielvox2_tts_set_instruct(struct cielvox2_tts_context* ctx, const char* instruct) {
    if (!ctx) {
        return -1;
    }
    if (cielvox2_tts_effective_model_type(ctx) != "voice_design") {  // FIX #3d: same override as is_voice_design()
        fprintf(stderr, "cielvox2_tts: model is not VoiceDesign — set_instruct not applicable\n");
        return -1;
    }
    ctx->runtime_instruct = instruct ? instruct : "";
    if (ctx->params.verbosity >= 1 && !ctx->runtime_instruct.empty()) {
        fprintf(stderr, "cielvox2_tts: VoiceDesign instruct set (%zu chars)\n", ctx->runtime_instruct.size());
    }
    return 0;
}

extern "C" int cielvox2_tts_set_cv_style_instruct(struct cielvox2_tts_context* ctx, const char* instruct) {
    if (!ctx) {
        return -1;
    }
    if (ctx->hp.spk_names.empty()) {  // FIX #3c: CustomVoice confirmed via spk_names, not stale tts_model_type
        fprintf(stderr, "cielvox2_tts: model is not CustomVoice — set_cv_style_instruct not applicable\n");
        return -1;
    }
    ctx->runtime_cv_style_instruct = instruct ? instruct : "";
    if (ctx->params.verbosity >= 1 && !ctx->runtime_cv_style_instruct.empty()) {
        fprintf(stderr, "cielvox2_tts: CustomVoice style instruct set (%zu chars)\n",
                ctx->runtime_cv_style_instruct.size());
    }
    return 0;
}

extern "C" int cielvox2_tts_n_speakers(struct cielvox2_tts_context* ctx) {
    if (!ctx) {
        return 0;
    }
...
extern "C" const char* cielvox2_tts_get_speaker_name(struct cielvox2_tts_context* ctx, int i) {
...
extern "C" int cielvox2_tts_set_speaker_by_name(struct cielvox2_tts_context* ctx, const char* name) {
    if (!ctx || !name) {
        return -1;
    }
    if (ctx->hp.spk_names.empty()) {  // FIX #3c: same signal, tts_model_type unreliable
        fprintf(stderr, "cielvox2_tts: model is not CustomVoice — set_speaker_by_name not applicable\n");
        return -1;
    }
    ...
    if (idx < 0) {
        fprintf(stderr, "cielvox2_tts: unknown speaker '%s'. Available: ", name);
        for (size_t i = 0; i < ctx->hp.spk_names.size(); i++) {
    ...
    if (idx >= (int)ctx->hp.spk_token_ids.size()) {
        fprintf(stderr, "cielvox2_tts: spk_token_ids array malformed in GGUF\n");
        return -1;
    }
    ...
    // codec_language_id token. Mirrors modeling_cielvox2_tts.py:2118-2122.
    if (idx < (int)ctx->hp.spk_dialect_token_ids.size()) {
        ...
        if (dialect_tok > 0) {
            ...
            if (ctx->language_id <= 0) {
                ...
                if (ctx->params.verbosity >= 1) {
                    fprintf(stderr, "cielvox2_tts: speaker '%s' carries dialect override → language_id=%d\n",
    ...
    if (ctx->params.verbosity >= 1) {
        fprintf(stderr, "cielvox2_tts: selected CustomVoice speaker '%s' (spk_id=%d)\n", ctx->hp.spk_names[idx].c_str(),
                spk_id);
    }
    return 0;
}

extern "C" float* cielvox2_tts_run_text_proj(struct cielvox2_tts_context* ctx, const int32_t* ids, int n_tokens, int* out_T,
                                          int* out_d) {
    if (out_T) {
        *out_T = 0;
    }
...
extern "C" float* cielvox2_tts_build_icl_prefill(struct cielvox2_tts_context* ctx, const char* syn_text, const char* ref_text,
                                              int* out_T) {
    if (out_T) {
        *out_T = 0;
    }
...
extern "C" float* cielvox2_tts_run_talker_with_embeds(struct cielvox2_tts_context* ctx, const float* embeds, int n_tokens,
                                                   int* out_vocab) {
    if (out_vocab) {
        *out_vocab = 0;
    }
...
extern "C" float* cielvox2_tts_run_code_pred_step(struct cielvox2_tts_context* ctx, const float* embeds, int n_tokens,
    ...
    if (lm_head_idx < 0 || lm_head_idx >= (int)cp.lm_head.size() || !cp.lm_head[lm_head_idx]) {
        fprintf(stderr, "cielvox2_tts: code_pred.lm_head[%d] missing\n", lm_head_idx);
        return nullptr;
    }
...
// Shared talker AR generation loop used by both cielvox2_tts_synthesize_codes
// (whole-clip) and cielvox2_tts_synthesize_streaming (windowed). Builds the
...
// inline cielvox2_tts_synthesize_codes body for the codes it produces.
...
static bool cielvox2_tts_sum_frame_embed(cielvox2_tts_context* ctx, const int32_t* codes16, int n_groups, int d,
                                      std::vector<float>& row_buf, float* out) {
    std::fill(out, out + d, 0.0f);
    const bool embd_cache_enabled = !env_bool("STELNETTTS_CIELVOX2_TTS_NO_EMBD_CACHE");
    for (int cb = 0; cb < n_groups; cb++) {
...
static bool cielvox2_tts_generate_codes_ar(cielvox2_tts_context* ctx, const char* text, std::vector<int32_t>& all_codes,
                                        int* out_frames, OnFrame on_frame) {
    if (out_frames) {
        *out_frames = 0;
    }
    if (ctx->vocab.id_to_token.empty()) {
        fprintf(stderr, "cielvox2_tts: vocab empty — re-convert with the updated converter\n");
        return false;
    }
    // FIX #3: tts_model_type metadata is unreliable for some rebranded 1.7B ggufs
    ...
    if (is_custom_voice) {
        ...
        if ((int)ctx->runtime_spk_emb.size() != (int)ctx->hp.d_model) {
            fprintf(stderr, "cielvox2_tts: no speaker — call cielvox2_tts_set_speaker_by_name\n");
            return false;
        }
    } else if (is_voice_design) {
        // VoiceDesign: need a non-empty natural-language instruct.
        if (ctx->runtime_instruct.empty()) {
            fprintf(stderr, "cielvox2_tts: VoiceDesign needs a voice description — call cielvox2_tts_set_instruct\n");
            return false;
        }
    } else {
    ...
    static int g_cielvox2_tts_synth_counter = 0;
    const int g_cielvox2_tts_synth_index = g_cielvox2_tts_synth_counter++;

    const bool bench = env_bool("STELNETTTS_CIELVOX2_TTS_BENCH");
    const bool dbg = env_bool("STELNETTTS_CIELVOX2_TTS_DEBUG");
    const char* dump_dir = env_str("STELNETTTS_CIELVOX2_TTS_DUMP_DIR");
    ...
    // backend arithmetic — neither --no-flash-attn nor STELNETTTS_KV_QUANT_*=f32
    ...
    // `max_codec_steps` and STELNETTTS_CIELVOX2_TTS_MAX_FRAMES both still override.
    int n_codepoints = 0;
    ...
    if (capped_by_text)
        max_frames = text_cap;
    if (const char* mf = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_MAX_FRAMES")) {
    ...
    uint64_t rng = 42;
    if (const char* s = env_str("STELNETTTS_CIELVOX2_TTS_SEED")) {
    ...
    if (is_custom_voice) {
        ...
        if (ref_text.empty()) {
            if (ctx->xvec_only) {
                ...
                fprintf(
                    stderr,
                    "cielvox2_tts: no ref_text — call cielvox2_tts_set_voice_prompt with ref_text or load a voice pack\n");
                return false;
            }
        }
    ...
    if (bench) {
        ...
        fprintf(stderr, "cielvox2_tts: prefill %7.1f ms (T=%d, %s)\n", now_ms() - t0, T_pre, label);
    }
    if (dump_dir) {
    ...
    if (bench) {
        fprintf(stderr, "cielvox2_tts: talker_pre %7.1f ms\n", now_ms() - t1);
    }
    if (dump_dir) {
    ...
    if (dbg) {
        const char* cp_be = env_str("STELNETTTS_CIELVOX2_TTS_CP_BACKEND");
        fprintf(stderr, "cielvox2_tts: code_pred backend=%s\n", (cp_be && *cp_be) ? cp_be : "main");
    }
    ...
    if (!cp_kv_alloc(ctx)) {
        fprintf(stderr, "cielvox2_tts: cp_kv allocation failed\n");
        free(logits);
        free(past_hidden);
        return false;
    }
    ...
    //
    // (a) `cielvox2_tts_set_temperature` reached the code predictor (see
    ...
    //     STELNETTTS_CIELVOX2_TTS_GREEDY=1 forces top_k=1, i.e. argmax, which makes
    ...
    //     miscompute. Same lever as STELNETTTS_COSYVOICE3_GREEDY, which is what
    //     settled the #304 native-Vulkan post-mortem (see LEARNINGS).
    const bool talker_greedy = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_GREEDY") != nullptr;
    ...
    // #337 / diff-harness replay mode. `STELNETTTS_CIELVOX2_TTS_REPLAY_TOKENS=<file>`
    ...
    // `STELNETTTS_CIELVOX2_TTS_REPLAY_CODES=<file>` is the STRONGER form: 16
    ...
    if (const char* rc = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_REPLAY_CODES")) {
        if (FILE* rf = std::fopen(rc, "r")) {
            long v = 0;
            while (std::fscanf(rf, "%ld", &v) == 1)
                replay_codes.push_back((int32_t)v);
            std::fclose(rf);
            fprintf(stderr, "cielvox2_tts: replaying %zu full frames (16 codes each) from %s\n", replay_codes.size() / 16,
                    rc);
        } else {
            fprintf(stderr, "cielvox2_tts: could not open STELNETTTS_CIELVOX2_TTS_REPLAY_CODES='%s'\n", rc);
        }
    }

    std::vector<int32_t> replay_tokens;
    if (const char* rp = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_REPLAY_TOKENS")) {
        if (FILE* rf = std::fopen(rp, "r")) {
            long v = 0;
            while (std::fscanf(rf, "%ld", &v) == 1)
                replay_tokens.push_back((int32_t)v);
            std::fclose(rf);
            fprintf(stderr, "cielvox2_tts: replaying %zu codebook-0 tokens from %s (sampling disabled)\n",
                    replay_tokens.size(), rp);
        } else {
            fprintf(stderr, "cielvox2_tts: could not open STELNETTTS_CIELVOX2_TTS_REPLAY_TOKENS='%s'\n", rp);
        }
    }
    ...
    for (frame = 0; frame < max_frames; frame++) {
        ...
        if (const char* dump_dir = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_DUMP_LOGITS")) {
            char path[1024];
            ...
            std::snprintf(path, sizeof(path), "%s/talker_s%02d_%04d.f32", dump_dir, g_cielvox2_tts_synth_index, frame);
            ...
            fprintf(stderr, "cielvox2_tts: LOGITS frame=%d top5=", frame);
            for (int k = 0; k < 5; k++)
        ...
        if (stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_EMBD_CHECK"))
            fprintf(stderr, "cielvox2_tts: frame=%d cb0=%d rng=%llu\n", frame, cb0, (unsigned long long)rng);
        free(logits);
        logits = nullptr;
        if (cb0 == eos) {
            if (dbg) {
                fprintf(stderr, "cielvox2_tts: codec_eos at frame %d\n", frame);
            }
            free(past_hidden);
            past_hidden = nullptr;
            break;
        ...
        const double t_next = bench ? now_ms() : 0.0;
        {
            ...
            if (!cielvox2_tts_sum_frame_embed(ctx, frame_codes, n_groups, d, next_emb_row_buf, next_emb.data()))
                return false;
        }
        ...
        if (stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_EMBD_CHECK") && frame < 1) {
            fprintf(stderr, "cielvox2_tts: frame=%d codes: cb0=%d cb1..4=%d %d %d %d\n", frame, cb0, cb1_15[0], cb1_15[1],
                    cb1_15[2], cb1_15[3]);
            float s = 0.0f;
            for (int j = 0; j < d; j++)
                s += std::abs(next_emb[j]);
            fprintf(stderr, "cielvox2_tts: frame=%d next_emb[0..3]=%.6f %.6f %.6f %.6f  l1=%.4f\n", frame, next_emb[0],
        ...
        if (frame + 1 >= max_frames && capped_by_text) {
            fprintf(stderr,
                    "cielvox2_tts: ERROR: talker hit the text-proportional frame cap (%d frames for %d input "
                    ...
                    "STELNETTTS_CIELVOX2_TTS_MAX_FRAMES if the text genuinely needs more.\n",
        ...
        if (n_past >= ctx->kv_max_ctx - 1) {
            ...
            fprintf(stderr,
                    "cielvox2_tts: ERROR: talker ran to the KV ceiling without emitting EOS — stopped at frame %d "
                    ...
                    "STELNETTTS_CIELVOX2_TTS_GREEDY=1 to remove sampling from the comparison.\n",
    ...
    if (bench) {
        fprintf(stderr, "cielvox2_tts: ar_loop    %7.1f ms (%d frames, %.1f ms/frame)\n", now_ms() - t_loop, frame,
                frame > 0 ? (now_ms() - t_loop) / frame : 0.0);
    }
    if (bench) {
        fprintf(stderr,
                "cielvox2_tts: ar_breakdown lookup=%7.1f ms  code_pred=%7.1f ms  next_emb=%7.1f ms  talker=%7.1f ms\n",
                t_loop_lookup, t_loop_code_pred, t_loop_next_emb, t_loop_talker);
    }
    if (ctx->params.verbosity >= 1) {
        fprintf(stderr, "cielvox2_tts: produced %d frames × 16 codebooks = %zu codes\n", frame, all_codes.size());
    }
    if (dump_dir && !all_codes.empty()) {
        ...
        char gc_name[64];
        std::snprintf(gc_name, sizeof(gc_name), "generated_codes_s%02d", g_cielvox2_tts_synth_index);
...
extern "C" int32_t* cielvox2_tts_synthesize_codes(struct cielvox2_tts_context* ctx, const char* text, int* out_n_codes) {
    if (out_n_codes) {
        *out_n_codes = 0;
    }
    if (!ctx || !text) {
    ...
    if (!cielvox2_tts_generate_codes_ar(ctx, text, all_codes, &frames, [](int) { return true; })) {
        return nullptr;
    }
    if (out_n_codes) {
...
extern "C" void cielvox2_tts_codes_free(int32_t* codes) {
    free(codes);
}
...
// was only freed at cielvox2_tts_free. The main sched (talker/prefill/embed)
...
static void cielvox2_tts_reset_scratch_sched(cielvox2_tts_context* c) {
    if (!c)
        return;
    ...
    // backend list as at creation (cielvox2_tts.cpp init: {backend, backend_cpu}).
    if (c->sched) {
...
extern "C" float* cielvox2_tts_decode_codes(struct cielvox2_tts_context* ctx, const int32_t* codes, int n_codes,
                                         int* out_n_samples) {
    if (out_n_samples) {
        *out_n_samples = 0;
    }
    ...
    if (!ctx->codec.loaded) {
        fprintf(stderr, "cielvox2_tts: decode_codes() requires codec — call cielvox2_tts_set_codec_path() first.\n");
        return nullptr;
    }
    const int n_q = (int)ctx->codec.hp.n_q;
    if (n_codes % n_q != 0) {
        fprintf(stderr, "cielvox2_tts: decode_codes: n_codes %d not divisible by n_q %d\n", n_codes, n_q);
        return nullptr;
    }
...
static bool cielvox2_tts_get_active_ref_codes(cielvox2_tts_context* ctx, std::vector<int32_t>& ref_codes, int& T_ref) {
    T_ref = 0;
    ref_codes.clear();
    ...
    if (!ctx->runtime_ref_codes.empty()) {
        if ((int)ctx->runtime_ref_codes.size() % n_q != 0) {
            fprintf(stderr, "cielvox2_tts: runtime_ref_codes size %zu not divisible by n_q=%d\n",
    ...
    if (it == ctx->vp_tensors.end() || !it->second) {
        fprintf(stderr, "cielvox2_tts: active voice '%s' missing ref_code tensor\n", voice_name.c_str());
        return false;
    }

    ggml_tensor* code_t = it->second;
    if ((int)code_t->ne[0] != n_q) {
        fprintf(stderr, "cielvox2_tts: voicepack ref_code groups mismatch: %d vs %d\n", (int)code_t->ne[0], n_q);
        return false;
    }
...
extern "C" float* cielvox2_tts_codec_extract_stage(struct cielvox2_tts_context* ctx, const int32_t* codes, int n_codes,
                                                const char* stage_name, int* out_n) {
    if (out_n) {
        *out_n = 0;
    }
    ...
    if (!ctx->codec.loaded) {
        fprintf(stderr, "cielvox2_tts: codec_extract_stage() requires codec.\n");
        return nullptr;
    }
    const int n_q = (int)ctx->codec.hp.n_q;
    if (n_codes % n_q != 0) {
...
extern "C" float* cielvox2_tts_synthesize(struct cielvox2_tts_context* ctx, const char* text, int* out_n_samples) {
    if (out_n_samples) {
        *out_n_samples = 0;
    }
    if (!ctx) {
    ...
    if (!ctx->codec.loaded) {
        fprintf(stderr, "cielvox2_tts: synthesize() requires the codec — call cielvox2_tts_set_codec_path() first.\n");
        return nullptr;
    }
    cielvox2_tts_bench_stage _bs_synth("synthesize");

    const double t_total0 = now_ms();
    int n_codes = 0;
    int32_t* codes;
    {
        cielvox2_tts_bench_stage _bs("synthesize_codes");
        codes = cielvox2_tts_synthesize_codes(ctx, text, &n_codes);
    }
    ...
    const int n_q = (int)ctx->codec.hp.n_q;
    if (n_codes % n_q != 0) {
        fprintf(stderr, "cielvox2_tts: synthesize: unexpected code count %d (n_q=%d)\n", n_codes, n_q);
        free(codes);
        return nullptr;
    }
    ...
    int T_ref = 0;
    if (!cielvox2_tts_get_active_ref_codes(ctx, ref_codes, T_ref)) {
        free(codes);
        return nullptr;
    }
    ...
    // unskipped path on Apple Silicon Metal, cielvox2-tts-customvoice
    ...
    //
    // Set CIELVOX2_TTS_SKIP_REF_DECODE=0 to opt out (e.g. for A/B
    // verification, or in case a future codec graph variant grows
    // rolling state and the equivalence breaks).
    const char* skip_env = stelnettts_env::get("STELNETTTS_CIELVOX2_TTS_SKIP_REF_DECODE");
    const bool skip_ref = !skip_env || skip_env[0] != '0';

    cielvox2_tts_bench_stage _bs_codec("codec_decode");
    ...
    if (out_n_samples && *out_n_samples > 0) {
        ...
        fprintf(stderr,
                "cielvox2_tts: perf — codes=%.0f ms  codec=%.0f ms  total=%.0f ms  "
                "audio=%.1f s  rtf=%.3f\n",
    ...
    cielvox2_tts_reset_scratch_sched(ctx);
    return pcm;
}

// ---------------------------------------------------------------------------
// Streaming synthesis. Mirrors cielvox2_tts_synthesize_codes' AR loop verbatim
...
// PCM as a pre-roll. See the equivalence note in cielvox2_tts_synthesize.
extern "C" float* cielvox2_tts_synthesize_streaming(struct cielvox2_tts_context* ctx, const char* text, int chunk_frames,
                                                 int overlap_frames, cielvox2_tts_pcm_callback cb, void* user_data,
                                                 int* out_n_samples) {
    if (out_n_samples) {
        *out_n_samples = 0;
    }
    ...
    if (!ctx->codec.loaded) {
        fprintf(stderr,
                "cielvox2_tts: synthesize_streaming() requires the codec — call cielvox2_tts_set_codec_path() first.\n");
        return nullptr;
    }
    if (ctx->vocab.id_to_token.empty()) {
        fprintf(stderr, "cielvox2_tts: vocab empty — re-convert with the updated converter\n");
        ...
        if ((frame + 1) - frames_emitted >= chunk_frames) {
            ...
            if (bench) {
                ...
                if (!first_chunk_logged) {
                    fprintf(stderr, "cielvox2_tts: [stream] time-to-first-chunk %7.1f ms (%d frames)\n", now_ms() - t0,
                            frame + 1);
    ...
    int frames = 0;
    if (!cielvox2_tts_generate_codes_ar(ctx, text, all_codes, &frames, on_frame)) {
        return nullptr;
    }
    if (emit_failed) {
        return nullptr;
    ...
    const int total_frames = (int)(all_codes.size() / n_groups);
    {
        ...
        if (bench) {
            t_codec += now_ms() - tc;
            fprintf(stderr, "cielvox2_tts: [stream] codec %7.1f ms (final emit incl.)\n", t_codec);
        }
    }

    if (ctx->params.verbosity >= 1) {
        fprintf(stderr, "cielvox2_tts: [stream] produced %d frames × 16 codebooks → %zu samples\n", total_frames,
                full_pcm.size());
    }
...
extern "C" void cielvox2_tts_pcm_free(float* pcm) {
    free(pcm);
}

extern "C" void cielvox2_tts_sync(struct cielvox2_tts_context* ctx) {
    if (!ctx)
        return;
...
extern "C" void cielvox2_tts_free(struct cielvox2_tts_context* ctx) {
    if (!ctx) {
        return;
    }
    if (ctx->codec_sched) {
...
extern "C" void cielvox2_tts_set_n_threads(struct cielvox2_tts_context* ctx, int n_threads) {
    if (!ctx || n_threads <= 0) {
        return;
    }
...
extern "C" float* cielvox2_tts_compute_speaker_mel(struct cielvox2_tts_context* /*ctx*/, const float* audio, int n_samples,
...
extern "C" float* cielvox2_tts_run_speaker_enc_on_mel(struct cielvox2_tts_context* ctx, const float* mel, int T_mel,
                                                   int* out_dim) {
    if (out_dim) {
        *out_dim = 0;
    }
...
extern "C" float* cielvox2_tts_compute_speaker_embedding(struct cielvox2_tts_context* ctx, const float* audio, int n_samples,
                                                      int* out_dim) {
    if (out_dim) {
        *out_dim = 0;
    }
    ...
    if (!ctx->spk_enc.loaded) {
        fprintf(stderr, "cielvox2_tts: compute_speaker_embedding: ECAPA not loaded\n");
        return nullptr;
    }
    int T_mel = 0;
