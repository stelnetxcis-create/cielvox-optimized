// stelnettts_imatrix.cpp — see stelnettts_imatrix.h.
//
// File format (GGUF-based; consumed by stelnettts-quantize --imatrix):
//   KV  general.architecture = "stelnettts-imatrix"
...
//
// Ported from StelnetEmbed src/imatrix.cpp.
...
// Optional named-tensor dump (parity harness). When STELNETTTS_ACTDUMP_OUT is
// set, the FIRST computation of the tensor named STELNETTTS_ACTDUMP_TENSOR
...
bool is_active() {
    if (!g_checked) {
        const char* p = getenv("STELNETTTS_IMATRIX_OUT");
        if (p && p[0]) {
            g_imatrix_active = true;
            g_path = p;
        }
        const char* d = getenv("STELNETTTS_ACTDUMP_OUT");
        if (d && d[0]) {
            g_dump_active = true;
            g_dump_path = d;
            const char* tn = getenv("STELNETTTS_ACTDUMP_TENSOR");
...
void stelnettts_imatrix_install(ggml_backend_sched_t sched) {
    if (!is_active() || !sched)
        return;
    ggml_backend_sched_set_eval_callback(sched, eval_cb, nullptr);
    if (!g_atexit) {
        atexit(stelnettts_imatrix_flush);
        g_atexit = true;
    }
}

void stelnettts_imatrix_flush(void) {
    std::lock_guard<std::mutex> lk(g_mu);
    ...
    gguf_set_val_str(g, "general.architecture", "stelnettts-imatrix");
