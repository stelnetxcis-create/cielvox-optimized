// stelnettts_lid.cpp — shared LID implementation.
// See stelnettts_lid.h.
//
// Extracted from examples/cli/stelnettts_lid.cpp. The whisper-tiny and
...
// thread count; freed by stelnettts_lid_free_cache() at shutdown.
...
bool detect_whisper(const float* samples, int n_samples, const CrispasrLidOptions& opts, CrispasrLidResult& out) {
    ...
    if (cache_miss) {
        ...
        if (!c.ctx) {
            if (opts.verbose)
                fprintf(stderr, "stelnettts[lid]: failed to load '%s'\n", opts.model_path.c_str());
            return false;
        }

        if (!whisper_is_multilingual(c.ctx)) {
            if (opts.verbose)
                fprintf(stderr, "stelnettts[lid]: model '%s' is English-only\n", opts.model_path.c_str());
            whisper_free(c.ctx);
            c.ctx = nullptr;
            return false;
        }
    ...
    if (whisper_pcm_to_mel(ctx, pcm.data(), NEED, opts.n_threads) != 0) {
        if (opts.verbose)
            fprintf(stderr, "stelnettts[lid]: pcm_to_mel failed\n");
        return false;
    }
    if (whisper_encode(ctx, 0, opts.n_threads) != 0) {
        if (opts.verbose)
            fprintf(stderr, "stelnettts[lid]: encode failed\n");
        return false;
    }
    ...
    if (lang_id < 0 || lang_id >= n_langs) {
        if (opts.verbose)
            fprintf(stderr, "stelnettts[lid]: whisper_lang_auto_detect failed\n");
        return false;
    }
    ...
    if (opts.verbose) {
        fprintf(stderr, "stelnettts[lid]: detected '%s' (p=%.3f) via whisper\n", out.lang_code.c_str(), out.confidence);
    }
    return true;
}
...
bool detect_silero(const float* samples, int n_samples, const CrispasrLidOptions& opts, CrispasrLidResult& out) {
    ...
    if (!sc.ctx || sc.model_path != opts.model_path || sc.n_threads != opts.n_threads) {
        ...
        if (!sc.ctx) {
            if (opts.verbose)
                fprintf(stderr, "stelnettts[lid]: silero_lid_init('%s') failed\n", opts.model_path.c_str());
            return false;
        }
    ...
    if (code.empty()) {
        if (opts.verbose)
            fprintf(stderr, "stelnettts[lid]: silero_lid_detect returned no code\n");
        return false;
    }
    ...
    out.source = "silero";
    if (opts.verbose) {
        fprintf(stderr, "stelnettts[lid]: detected '%s' (p=%.3f) via silero\n", out.lang_code.c_str(), out.confidence);
    }
    return true;
}
...
bool detect_firered(const float* samples, int n_samples, const CrispasrLidOptions& opts, CrispasrLidResult& out) {
    ...
    if (!fc.ctx || fc.model_path != opts.model_path || fc.n_threads != opts.n_threads) {
        ...
        if (!fc.ctx) {
            if (opts.verbose)
                fprintf(stderr, "stelnettts[lid]: firered_lid_init('%s') failed\n", opts.model_path.c_str());
            return false;
        }
    ...
    if (code.empty()) {
        if (opts.verbose)
            fprintf(stderr, "stelnettts[lid]: firered_lid_detect returned no code\n");
        return false;
    }
    ...
    out.source = "firered";
    if (opts.verbose) {
        fprintf(stderr, "stelnettts[lid]: detected '%s' (p=%.3f) via firered\n", out.lang_code.c_str(), out.confidence);
    }
    return true;
}
...
bool detect_ecapa(const float* samples, int n_samples, const CrispasrLidOptions& opts, CrispasrLidResult& out) {
    ...
    if (!ec.ctx || ec.model_path != opts.model_path || ec.n_threads != opts.n_threads) {
        ...
        if (!ec.ctx) {
            if (opts.verbose)
                fprintf(stderr, "stelnettts[lid]: ecapa_lid_init('%s') failed\n", opts.model_path.c_str());
            return false;
        }
    ...
    if (code.empty()) {
        if (opts.verbose)
            fprintf(stderr, "stelnettts[lid]: ecapa_lid_detect returned no code\n");
        return false;
    }
    ...
    out.source = "ecapa";
    if (opts.verbose)
        fprintf(stderr, "stelnettts[lid]: detected '%s' (p=%.3f) via ecapa\n", out.lang_code.c_str(), out.confidence);
    return true;
}
...
std::vector<float> stelnettts_lid_speech_prefix(const std::vector<float>& pcm,
...
void stelnettts_lid_free_cache() {
    WhisperLidCache& c = whisper_lid_cache();
    if (c.ctx) {
        whisper_free(c.ctx);
        c.ctx = nullptr;
