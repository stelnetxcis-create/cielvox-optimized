// stelnettts_cache.cpp — implementation of stelnettts_cache.h.
...
#ifdef STELNETTTS_USE_LIBCURL
#include <curl/curl.h>
#endif

namespace stelnettts_cache {
...
static bool fetch_winhttp(const std::string& url, const std::string& dest, bool quiet) {
    ...
    for (int hop = 0; hop < 10; ++hop) {
        ...
        if (!WinHttpCrackUrl(wurl.c_str(), 0, 0, &uc)) {
            if (!quiet)
                fprintf(stderr, "stelnettts: WinHTTP: WinHttpCrackUrl failed for %s\n", current_url.c_str());
            return false;
        }

        HINTERNET hSess = WinHttpOpen(L"StelnetTTS/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME,
        ...
        if (status == 200) {
            ...
            if (!f && !quiet) {
                fprintf(stderr, "stelnettts: WinHTTP: cannot create '%s': %s\n", dest.c_str(), strerror(errno));
            }
            if (f) {
                BYTE buf[65536];
                DWORD got = 0;
        ...
        if (!quiet)
            fprintf(stderr, "stelnettts: WinHTTP: server returned %lu for %s\n", static_cast<unsigned long>(status),
                    current_url.c_str());
        WinHttpCloseHandle(hReq);
        WinHttpCloseHandle(hConn);
        WinHttpCloseHandle(hSess);
    ...
    if (!quiet)
        fprintf(stderr, "stelnettts: WinHTTP: too many redirects\n");
    return false;
}
...
static bool fetch_libcurl(const std::string& url, const std::string& dest, bool quiet) {
    ...
    if (!f) {
        if (!quiet)
            fprintf(stderr, "stelnettts: libcurl: cannot create '%s': %s\n", dest.c_str(), strerror(errno));
    ...
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "StelnetTTS/1.0");
    ...
    if (res != CURLE_OK) {
        if (!quiet)
            fprintf(stderr, "stelnettts: libcurl: %s\n", curl_easy_strerror(res));
        return false;
    }
    return true;
}
#endif // STELNETTTS_USE_LIBCURL
...
std::string dir(const std::string& cache_dir_override) {
    ...
    if (selected.empty()) {
        if (const char* env = std::getenv("STELNETTTS_CACHE_DIR"); env && *env)
            selected = env;
    }
    if (selected.empty()) {
        if (const char* env = std::getenv("STELNETTTS_MODELS_DIR"); env && *env)
            selected = env;
    }
    if (selected.empty())
...
static bool fetch_download(const std::string& url, const std::string& dest, bool quiet) {
    ...
    (void)dest;
    (void)quiet;
    fprintf(stderr, "stelnettts: download not available in WASM — load model via JS FS.writeFile\n");
    return false;
#endif
#ifdef _WIN32
    // ── WinHTTP: built-in Windows HTTPS stack, no shell-quoting issues ──────
    if (!quiet)
        fprintf(stderr, "stelnettts: downloading via WinHTTP...\n");
    if (fetch_winhttp(url, dest, quiet) && file_present(dest))
        return true;
    if (!quiet)
        fprintf(stderr, "stelnettts: WinHTTP failed, falling back...\n");
#endif

#ifdef STELNETTTS_USE_LIBCURL
    // ── libcurl API (preferred on POSIX; fallback after WinHTTP on Windows) ──
    if (!quiet)
        fprintf(stderr, "stelnettts: downloading via libcurl...\n");
    if (fetch_libcurl(url, dest, quiet) && file_present(dest))
        return true;
    if (!quiet)
        fprintf(stderr, "stelnettts: libcurl failed, falling back to curl CLI...\n");
#endif
    ...
    fprintf(stderr,
#if defined(_WIN32) && defined(STELNETTTS_USE_LIBCURL)
            "stelnettts: download failed (WinHTTP + libcurl + curl + wget all rejected). "
#elif defined(_WIN32)
            "stelnettts: download failed (WinHTTP + curl + wget all rejected). "
#elif defined(STELNETTTS_USE_LIBCURL)
            "stelnettts: download failed (libcurl + curl + wget all rejected). "
#else
            "stelnettts: download failed (curl + wget both rejected). "
#endif
...
// Caller's cache_dir_override (or the canonical ~/.cache/stelnettts) is
...
//   2. $STELNETTTS_MODELS_DIR (set by users with a dedicated model SSD).
//   3. /Volumes/backups/ai/stelnettts-models  (macOS dev convention).
//   4. ~/.cache/stelnettts-models           (legacy alt cache).
...
static std::vector<std::string> well_known_search_dirs(const std::string& cache_dir_override) {
    ...
    append_unique(dirs, "/Volumes/backups/ai/stelnettts-models");
...
#endif
    if (home && *home) {
        append_unique(dirs, std::string(home) + "/.cache/stelnettts-models");
