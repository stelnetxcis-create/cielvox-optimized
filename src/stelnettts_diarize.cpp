// stelnettts_diarize.cpp — shared speaker diarization post-step.
// See stelnettts_diarize.h for the interface contract.
//
// Extracted from examples/cli/stelnettts_diarize.cpp so the CLI, the
// C-ABI wrapper `stelnettts_diarize_segments`, and every language
// binding use the same implementation. The sherpa-ONNX subprocess
// path stays in the CLI (external binary, CLI-shaped UX).

#include "stelnettts_diarize.h"
...
void stelnettts_diarize_free_pyannote_cache() {
...
bool apply_pyannote(const float* mono, int n_samples, int64_t slice_t0_cs, std::vector<CrispasrDiarizeSegment>& segs,
    ...
    stelnettts_diarize_internal::assign_speakers_from_log_posteriors(probs, T_seg, frame_dur_s, slice_t0_cs, segs);
    std::free(probs);
    return true;
}
...
namespace stelnettts_diarize_internal {
...
bool apply_foxnose(const float* left, int n_samples, const CrispasrDiarizeOptions& opts,
    ...
    if (const char* e = std::getenv("STELNETTTS_SPEAKER_EMBED_THREADS")) {
    ...
    if (!ctx) {
        fprintf(stderr, "stelnettts_diarize: failed to load embedder '%s'\n", opts.foxnose_embedder_path.c_str());
        return false;
    }
    FoxnoseEmbedder emb;
    ...
    if (const char* e = std::getenv("STELNETTTS_DIARIZE_EMBED_WORKERS")) {
        const int v = std::atoi(e);
        if (v > 0)
            want_workers = v;
    }
    ...
    // STELNETTTS_DIARIZE_SPAN_EMBED=1.
    //
    ...
    if (const char* e = std::getenv("STELNETTTS_DIARIZE_SPAN_EMBED"))
        if (*e && *e != '0')
    ...
    if (std::getenv("STELNETTTS_DIARIZE_DEBUG"))
        fprintf(stderr, "stelnettts[diarize]: foxnose windows=%d skipped=%d -> %d speakers (%s), %zu turns\n",
