...
// ===========================================================================
// Bench instrumentation — `CIELVOX2_ASR_BENCH=1` for per-stage timings.
// ===========================================================================

static bool cielvox2_asr_bench_enabled() {
    static int v = -1;
    if (v < 0) {
        const char* e = stelnettts_env::get("STELNETTTS_CIELVOX2_ASR_BENCH");
        v = (e && *e && *e != '0') ? 1 : 0;
    }
    return v != 0;
}
...
static bool cielvox2_asr_load_model(cielvox2_asr_model& model, cielvox2_asr_vocab& vocab, const char* path,
    ...
    // ---- pass 1: read hparams + vocab via metadata-only context ----
    {
        ...
        hp.sample_rate = core_gguf::kv_u32(gctx, "cielvox2asr.sample_rate", hp.sample_rate);
        hp.n_mels = core_gguf::kv_u32(gctx, "cielvox2asr.n_mels", hp.n_mels);
        hp.n_fft = core_gguf::kv_u32(gctx, "cielvox2asr.n_fft", hp.n_fft);
        hp.win_length = core_gguf::kv_u32(gctx, "cielvox2asr.win_length", hp.win_length);
        hp.hop_length = core_gguf::kv_u32(gctx, "cielvox2asr.hop_length", hp.hop_length);
        hp.audio_n_layers = core_gguf::kv_u32(gctx, "cielvox2asr.audio.n_layers", hp.audio_n_layers);
        hp.audio_d_model = core_gguf::kv_u32(gctx, "cielvox2asr.audio.d_model", hp.audio_d_model);
        hp.audio_n_heads = core_gguf::kv_u32(gctx, "cielvox2asr.audio.n_heads", hp.audio_n_heads);
        hp.audio_head_dim = core_gguf::kv_u32(gctx, "cielvox2asr.audio.head_dim", hp.audio_head_dim);
        hp.audio_ff_dim = core_gguf::kv_u32(gctx, "cielvox2asr.audio.ff_dim", hp.audio_ff_dim);
        hp.audio_conv_ch = core_gguf::kv_u32(gctx, "cielvox2asr.audio.conv_channels", hp.audio_conv_ch);
        hp.audio_proj_dim = core_gguf::kv_u32(gctx, "cielvox2asr.audio.proj_dim", hp.audio_proj_dim);
        hp.audio_max_pos = core_gguf::kv_u32(gctx, "cielvox2asr.audio.max_source_pos", hp.audio_max_pos);

        hp.llm_n_layers = core_gguf::kv_u32(gctx, "cielvox2asr.llm.n_layers", hp.llm_n_layers);
        hp.llm_d_model = core_gguf::kv_u32(gctx, "cielvox2asr.llm.d_model", hp.llm_d_model);
        hp.llm_n_heads = core_gguf::kv_u32(gctx, "cielvox2asr.llm.n_heads", hp.llm_n_heads);
        hp.llm_n_kv_heads = core_gguf::kv_u32(gctx, "cielvox2asr.llm.n_kv_heads", hp.llm_n_kv_heads);
        hp.llm_head_dim = core_gguf::kv_u32(gctx, "cielvox2asr.llm.head_dim", hp.llm_head_dim);
        hp.llm_ff_dim = core_gguf::kv_u32(gctx, "cielvox2asr.llm.ff_dim", hp.llm_ff_dim);
        hp.llm_rope_theta = core_gguf::kv_f32(gctx, "cielvox2asr.llm.rope_theta", hp.llm_rope_theta);
        hp.llm_rms_eps = core_gguf::kv_f32(gctx, "cielvox2asr.llm.rms_norm_eps", hp.llm_rms_eps);
        hp.llm_vocab_size = core_gguf::kv_u32(gctx, "cielvox2asr.llm.vocab_size", hp.llm_vocab_size);
        hp.llm_max_pos = core_gguf::kv_u32(gctx, "cielvox2asr.llm.max_pos", hp.llm_max_pos);

        hp.audio_start_token_id = core_gguf::kv_u32(gctx, "cielvox2asr.audio_start_token_id", hp.audio_start_token_id);
        hp.audio_end_token_id = core_gguf::kv_u32(gctx, "cielvox2asr.audio_end_token_id", hp.audio_end_token_id);
        hp.audio_pad_token_id = core_gguf::kv_u32(gctx, "cielvox2asr.audio_pad_token_id", hp.audio_pad_token_id);
        hp.eos_token_id = core_gguf::kv_u32(gctx, "cielvox2asr.eos_token_id", hp.eos_token_id);
        hp.pad_token_id = core_gguf::kv_u32(gctx, "cielvox2asr.pad_token_id", hp.pad_token_id);
    ...
    if (do_split) {
        ...
        fprintf(stderr, "cielvox2_asr: layer offload: gpu=[0,%d), cpu=[%d,%d) (STELNETTTS_N_GPU_LAYERS=%d)\n",
...
extern "C" cielvox2_asr_context* cielvox2_asr_init_from_file(const char* path, cielvox2_asr_context_params params) {
    ...
    // Opt-out: STELNETTTS_CIELVOX2_ASR_FUSED_QKV=0.
    {
        const char* fuse_env = getenv("STELNETTTS_CIELVOX2_ASR_FUSED_QKV");
...
extern "C" bool cielvox2_asr_kv_init(cielvox2_asr_context* ctx, int max_ctx) {
    ...
    // why this only ever appeared with STELNETTTS_KV_QUANT=q8_0, and only on CUDA.
...
extern "C" float* cielvox2_asr_embed_tokens(cielvox2_asr_context* ctx, const int32_t* input_ids, int n_tokens) {
    ...
    // Gated by STELNETTTS_CIELVOX2_ASR_EMBED_FAST (default ON).
    static int use_fast = -1;
    if (use_fast < 0) {
        const char* e = std::getenv("STELNETTTS_CIELVOX2_ASR_EMBED_FAST");
        use_fast = (!e || *e != '0') ? 1 : 0;
    }
