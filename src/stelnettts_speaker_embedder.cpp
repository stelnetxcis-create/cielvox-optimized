// stelnettts_speaker_embedder.cpp — concrete embedder adapters and the
...
std::string resolve_indextts_bigvgan(const std::string& model_spec, const std::string& cache_dir, bool quiet) {
    ...
    return stelnettts_managed_download("indextts-bigvgan.gguf",
                                     ...
                                     "stelnettts[diarize-embedder]", cache_dir);
}
...
std::unique_ptr<StelnetAsrSpeakerEmbedder> stelnettts_make_speaker_embedder(const std::string& model_spec, int n_threads,
    ...
    //
    // STELNETTTS_SPEAKER_EMBED_THREADS overrides, for A/B against a
    ...
    if (want_indextts) {
        ...
        if (resolved.empty()) {
            fprintf(stderr, "stelnettts[diarize]: failed to resolve IndexTTS-BigVGAN model for '%s'\n",
                    model_spec.c_str());
            return nullptr;
        }
        indextts_voc_context* ctx = indextts_voc_init(resolved.c_str(), n_threads, /*use_gpu=*/false);
        if (!ctx) {
            fprintf(stderr, "stelnettts[diarize]: failed to load IndexTTS-BigVGAN from '%s'\n", resolved.c_str());
            return nullptr;
        }
    ...
    if (resolved == "auto" || resolved.find('/') == std::string::npos) {
        resolved = stelnettts_resolve_model(resolved == "auto" ? std::string("auto") : resolved, "titanet",
                                          /*no_prints=*/false, cache_dir, /*auto_download=*/true, "");
    }
    if (resolved.empty()) {
        fprintf(stderr, "stelnettts[diarize]: failed to resolve speaker embedder model '%s'\n", model_spec.c_str());
        return nullptr;
    }

    titanet_context* ctx = titanet_init(resolved.c_str(), n_threads);
    if (!ctx) {
        fprintf(stderr, "stelnettts[diarize]: failed to load speaker embedder from '%s'\n", resolved.c_str());
        return nullptr;
    }
