...
namespace StelnetASR.Tests
{
    ...
    /// (stelnet_asr_session_synthesize_raw + stelnet_asr_session_accept_marking_responsibility).
    ///
    ...
    ///      libstelnet_asr; the native layer is null-safe, so no model is needed.
    ...
    public class MarkingAttestationTests
    {
        ...
        public void AcceptMarkingResponsibility_NativeEntryPoint_ResolvesAndIsNullSafe()
        {
            ...
            int rc = NativeMethods.stelnet_asr_session_accept_marking_responsibility(IntPtr.Zero, "csharp ci smoke");
            Assert.Equal(-1, rc);
        }
        ...
        public void SynthesizeRaw_NativeEntryPoint_ResolvesAndIsNullSafe()
        {
            ...
            IntPtr ptr = NativeMethods.stelnet_asr_session_synthesize_raw(IntPtr.Zero, "hello", out int n);
        ...
        {
            if (!CanLoadLibrary()) return;
            var modelPath = Environment.GetEnvironmentVariable("STELNET_ASR_MODEL_TTS");
