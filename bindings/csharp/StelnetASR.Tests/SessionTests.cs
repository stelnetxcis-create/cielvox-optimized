...
namespace StelnetASR.Tests
{
    ...
    /// stelnet_asr library to be loadable at runtime. Tests that can't
    ...
    public class SessionTests
    {
        ...
        public void DetectBackendFromGguf_DetectsWhisper()
        {
            ...
            var modelPath = Environment.GetEnvironmentVariable("STELNET_ASR_MODEL_WHISPER");
    ...
    /// Gated by STELNET_ASR_MODEL_WHISPER env var — skip cleanly when unset.
    /// </summary>
    public class LiveSessionTests
    {
        private static string? WhisperModel =>
            Environment.GetEnvironmentVariable("STELNET_ASR_MODEL_WHISPER");
        ...
        private static string? VadModel =>
            Environment.GetEnvironmentVariable("STELNET_ASR_VAD_MODEL");
        ...
        {
            if (!CanLoadLibrary()) return;
            StelnetASR.Logging.Silence();
            StelnetASR.Logging.SetMinLevel(StelnetASR.LogLevel.Warn);
            var captured = new System.Collections.Generic.List<string>();
            StelnetASR.Logging.SetCallback((lvl, msg) => captured.Add(msg));
            // Trigger some native logging via a backend query, then restore default.
            _ = Session.AvailableBackends();
            StelnetASR.Logging.SetCallback(null);
        }
        ...
        // real tabcnn model when STELNET_ASR_MODEL_TABCNN is set, asserting the flat
        ...
        private static string? TabModel =>
            Environment.GetEnvironmentVariable("STELNET_ASR_MODEL_TABCNN");
