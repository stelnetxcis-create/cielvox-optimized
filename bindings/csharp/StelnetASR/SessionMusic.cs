...
namespace StelnetASR
{
    ...
    public sealed partial class Session
    {
        ...
        {
            int n = NativeMethods.stelnet_asr_session_tab(Handle, pcm, pcm.Length, sampleRate);
            if (n < 0) throw new InvalidOperationException($"tab failed (rc={n})");
            IntPtr p = NativeMethods.stelnet_asr_session_tab_emissions(Handle, out int nf, out int nStr, out int nCls);
            var logits = CopyView(p, nf * nStr * nCls);
            var open = new int[nStr];
            for (int i = 0; i < nStr; i++)
                open[i] = NativeMethods.stelnet_asr_session_tab_string_open_midi(Handle, i);
            return new TabEmissions(nf, nStr, nCls, logits,
                NativeMethods.stelnet_asr_session_tab_silent_class(Handle),
                NativeMethods.stelnet_asr_session_tab_frame_period(Handle), open);
        }

        /// <summary>Native input rate the loaded beat model expects, or 0 if the backend has no beat arm.</summary>
        public int BeatsSampleRate => NativeMethods.stelnet_asr_session_beats_sample_rate(Handle);
        ...
        {
            int n = NativeMethods.stelnet_asr_session_beats(Handle, pcm, pcm.Length, sampleRate);
            if (n < 0) throw new InvalidOperationException($"beats failed (rc={n})");
            IntPtr p = NativeMethods.stelnet_asr_session_beats_events(Handle, out int ne);
            var raw = CopyView(p, ne * 2);          // {time_s, is_downbeat}
            var beats = new BeatEvent[ne];
            for (int i = 0; i < ne; i++)
                beats[i] = new BeatEvent(raw[i * 2], raw[i * 2 + 1] != 0.0f);
            return new BeatResult(beats, NativeMethods.stelnet_asr_session_beats_tempo_bpm(Handle));
        }

        /// <summary>Chord-vocabulary size: 25 (maj/min + N) or 170 (full quality set).</summary>
        public int ChordsVocabSize => NativeMethods.stelnet_asr_session_chords_vocab_size(Handle);

        /// <summary>Chord recognition: labelled time spans over the audio.</summary>
        public ChordSpan[] Chords(float[] pcm, int sampleRate)
        {
            int n = NativeMethods.stelnet_asr_session_chords(Handle, pcm, pcm.Length, sampleRate);
            if (n < 0) throw new InvalidOperationException($"chords failed (rc={n})");
            IntPtr p = NativeMethods.stelnet_asr_session_chords_spans(Handle, out int ns);
            var raw = CopyView(p, ns * 4);          // {start_ms, end_ms, label, confidence}
            var spans = new ChordSpan[ns];
            for (int i = 0; i < ns; i++)
            {
                int label = (int)raw[i * 4 + 2];
                IntPtr namePtr = NativeMethods.stelnet_asr_session_chords_span_name(Handle, i);
        ...
        public int PianoSampleRate => NativeMethods.stelnet_asr_session_piano_sample_rate(Handle);
        ...
        {
            int n = NativeMethods.stelnet_asr_session_piano(Handle, pcm16k, pcm16k.Length);
            if (n < 0) throw new InvalidOperationException($"piano failed (rc={n})");
            IntPtr p = NativeMethods.stelnet_asr_session_piano_notes(Handle, out int nn);
        ...
        public int PitchSampleRate => NativeMethods.stelnet_asr_session_pitch_sample_rate(Handle);
        ...
        {
            int n = NativeMethods.stelnet_asr_session_pitch(Handle, pcm16k, pcm16k.Length, hopMs);
            if (n < 0) throw new InvalidOperationException($"pitch failed (rc={n})");
            IntPtr p = NativeMethods.stelnet_asr_session_pitch_frames(Handle, out int nf);
        ...
        public int SeparateSampleRate => NativeMethods.stelnet_asr_session_separate_sample_rate(Handle);
        ...
        {
            int n = NativeMethods.stelnet_asr_session_separate(Handle, pcmStereoInterleaved, pcmStereoInterleaved.Length);
            if (n < 0) throw new InvalidOperationException($"separate failed (rc={n})");
            var stems = new Stem[n];
            for (int i = 0; i < n; i++)
            {
                IntPtr namePtr = NativeMethods.stelnet_asr_session_separate_stem_name(Handle, i);
                string name = namePtr == IntPtr.Zero ? $"stem{i}" : (Marshal.PtrToStringUTF8(namePtr) ?? $"stem{i}");
                IntPtr pcmPtr = NativeMethods.stelnet_asr_session_separate_stem(Handle, i, out int perChan);
        ...
        public int ConvertContentDim => NativeMethods.stelnet_asr_session_convert_content_dim(Handle);
        /// <summary>Number of target speakers in the loaded RVC model.</summary>
        public int ConvertSpeakerCount => NativeMethods.stelnet_asr_session_convert_n_speakers(Handle);
        /// <summary>Native output rate of the RVC model (32k/40k/48k — not a constant).</summary>
        public int ConvertSampleRate => NativeMethods.stelnet_asr_session_convert_sample_rate(Handle);
                               ...
                               float[]? noiseZp = null, float[]? noiseSine = null)
        {
            ...
            int n = NativeMethods.stelnet_asr_session_convert(Handle, content, nFrames, f0Hz, speakerId, noiseZp, noiseSine);
            if (n < 0) throw new InvalidOperationException($"convert failed (rc={n})");
            IntPtr p = NativeMethods.stelnet_asr_session_convert_audio(Handle, out int nSamples);
