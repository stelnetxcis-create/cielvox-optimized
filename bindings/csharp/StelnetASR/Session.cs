...
{
    /// <summary>
    /// Unified StelnetASR Session — wraps the C ABI for ASR transcription,
    ...
    public sealed partial class Session : IDisposable
    {
        ...
        {
            var p = NativeMethods.stelnet_asr_session_open(modelPath, nThreads);
            if (p == IntPtr.Zero)
        ...
        {
            var p = NativeMethods.stelnet_asr_session_open_explicit(modelPath, backend, nThreads);
            if (p == IntPtr.Zero)
        ...
        public static string[] AvailableBackends()
        {
            ...
            NativeMethods.stelnet_asr_session_available_backends(buf, buf.Length);
        ...
        {
            var buf = new byte[32];
            NativeMethods.stelnet_asr_session_detected_language(Handle, buf, buf.Length);
        ...
        public void Dispose()
        {
            if (_handle != IntPtr.Zero)
            {
                NativeMethods.stelnet_asr_session_close(_handle);
                _handle = IntPtr.Zero;
            }
        }
        ...
        public void SetCodecPath(string path)
            => Check(NativeMethods.stelnet_asr_session_set_codec_path(Handle, path), "set_codec_path");
        ...
        public void SetVoice(string path, string? refText = null)
            => Check(NativeMethods.stelnet_asr_session_set_voice(Handle, path, refText), "set_voice");

        /// <summary>Select a preset speaker by name (orpheus).</summary>
        public void SetSpeakerName(string name)
        {
            int rc = NativeMethods.stelnet_asr_session_set_speaker_name(Handle, name);
        ...
        public void SetSpeakerId(int id)
            => Check(NativeMethods.stelnet_asr_session_set_speaker_id(Handle, id), "set_speaker_id");

        /// <summary>Return the list of preset speaker names for the active backend.</summary>
        public string[] Speakers()
        {
            int n = NativeMethods.stelnet_asr_session_n_speakers(Handle);
            var result = new string[n];
            for (int i = 0; i < n; i++)
                result[i] = NativeMethods.PtrToUtf8(NativeMethods.stelnet_asr_session_get_speaker_name(Handle, i)) ?? "";
            return result;
        }
        ...
        {
            int rc = NativeMethods.stelnet_asr_session_set_instruct(Handle, instruct);
        ...
        {
            int rc = NativeMethods.stelnet_asr_session_set_tts_phonemes(Handle, phonemes ?? "");
        ...
        public bool IsCustomVoice => NativeMethods.stelnet_asr_session_is_custom_voice(Handle) != 0;

        /// <summary>Whether the loaded model is a qwen3-tts VoiceDesign variant.</summary>
        public bool IsVoiceDesign => NativeMethods.stelnet_asr_session_is_voice_design(Handle) != 0;

        /// <summary>Select + load a punctuation-restoration model.</summary>
        public void SetPuncModel(string puncModel)
            => Check(NativeMethods.stelnet_asr_session_set_punc_model(Handle, puncModel), "set_punc_model");

        /// <summary>Comma-separated hotwords for contextual biasing.</summary>
        public void SetHotwords(string hotwords, float boost)
            => Check(NativeMethods.stelnet_asr_session_set_hotwords(Handle, hotwords, boost), "set_hotwords");
        ...
        public void SetSensitivity(string preset)
        {
            ...
            int rc = NativeMethods.stelnet_asr_session_set_sensitivity(Handle, preset);
            if (rc == -2)
        ...
        public void SetG2pDict(string source)
            => Check(NativeMethods.stelnet_asr_session_set_g2p_dict(Handle, source), "set_g2p_dict");

        /// <summary>Sticky source-language hint.</summary>
        public void SetSourceLanguage(string? lang)
            => Check(NativeMethods.stelnet_asr_session_set_source_language(Handle, lang ?? ""), "set_source_language");

        /// <summary>Sticky target-language. Different from source triggers translation on canary/cohere.</summary>
        public void SetTargetLanguage(string? lang)
            => Check(NativeMethods.stelnet_asr_session_set_target_language(Handle, lang ?? ""), "set_target_language");
        ...
        public void SetTtsReferenceLanguage(string? lang)
            => Check(NativeMethods.stelnet_asr_session_set_tts_reference_language(Handle, lang ?? ""),
        ...
        public void SetPunctuation(bool enable)
            => Check(NativeMethods.stelnet_asr_session_set_punctuation(Handle, enable ? 1 : 0), "set_punctuation");

        /// <summary>Whisper sticky --translate.</summary>
        public void SetTranslate(bool enable)
            => Check(NativeMethods.stelnet_asr_session_set_translate(Handle, enable ? 1 : 0), "set_translate");
        ...
        public void SetTemperature(float temperature, ulong seed = 0)
            => Check(NativeMethods.stelnet_asr_session_set_temperature(Handle, temperature, seed), "set_temperature");

        /// <summary>Reseed TTS backends that support runtime seed control.</summary>
        public void SetTtsSeed(ulong seed)
            => Check(NativeMethods.stelnet_asr_session_set_tts_seed(Handle, seed), "set_tts_seed");

        /// <summary>Generated-token cap for autoregressive session backends.</summary>
        public void SetMaxNewTokens(int n)
            => Check(NativeMethods.stelnet_asr_session_set_max_new_tokens(Handle, n), "set_max_new_tokens");
        ...
        public void SetFrequencyPenalty(float penalty)
            => Check(NativeMethods.stelnet_asr_session_set_frequency_penalty(Handle, penalty), "set_frequency_penalty");

        /// <summary>Diffusion / CFM step count (chatterbox).</summary>
        public void SetTtsSteps(int steps)
            => Check(NativeMethods.stelnet_asr_session_set_tts_steps(Handle, steps), "set_tts_steps");

        /// <summary>Flow-matching timing candidates ranked per token (TADA).</summary>
        public void SetTtsNumCandidates(int n)
            => Check(NativeMethods.stelnet_asr_session_set_tts_num_candidates(Handle, n), "set_tts_num_candidates");

        /// <summary>Top-p nucleus-sampling threshold.</summary>
        public void SetTopP(float topP)
            => Check(NativeMethods.stelnet_asr_session_set_top_p(Handle, topP), "set_top_p");

        /// <summary>Top-k sampling cutoff (0 = disabled). Honoured by TADA.</summary>
        public void SetTopK(int topK)
            => Check(NativeMethods.stelnet_asr_session_set_top_k(Handle, topK), "set_top_k");

        /// <summary>Enable/disable sampling (false = greedy). Honoured by TADA.</summary>
        public void SetDoSample(bool enable)
            => Check(NativeMethods.stelnet_asr_session_set_do_sample(Handle, enable ? 1 : 0), "set_do_sample");

        /// <summary>Min-p sampling threshold.</summary>
        public void SetMinP(float minP)
            => Check(NativeMethods.stelnet_asr_session_set_min_p(Handle, minP), "set_min_p");

        /// <summary>Repetition penalty (1.0 = no penalty).</summary>
        public void SetRepetitionPenalty(float r)
            => Check(NativeMethods.stelnet_asr_session_set_repetition_penalty(Handle, r), "set_repetition_penalty");

        /// <summary>Classifier-free-guidance weight (chatterbox).</summary>
        public void SetCfgWeight(float cfgWeight)
            => Check(NativeMethods.stelnet_asr_session_set_cfg_weight(Handle, cfgWeight), "set_cfg_weight");
        ...
        public void SetTtsNoiseTemp(float noiseTemp)
            => Check(NativeMethods.stelnet_asr_session_set_tts_noise_temp(Handle, noiseTemp), "set_tts_noise_temp");

        /// <summary>Emotion-exaggeration scalar (chatterbox).</summary>
        public void SetExaggeration(float exaggeration)
            => Check(NativeMethods.stelnet_asr_session_set_exaggeration(Handle, exaggeration), "set_exaggeration");

        /// <summary>Upper bound on speech tokens per synthesize call.</summary>
        public void SetMaxSpeechTokens(int n)
            => Check(NativeMethods.stelnet_asr_session_set_max_speech_tokens(Handle, n), "set_max_speech_tokens");
        ...
        public void SetMinSpeechTokens(int n)
            => Check(NativeMethods.stelnet_asr_session_set_min_speech_tokens(Handle, n), "set_min_speech_tokens");

        /// <summary>Per-phoneme length-scale / speaking-rate scalar (kokoro).</summary>
        public void SetLengthScale(float scale)
            => Check(NativeMethods.stelnet_asr_session_set_length_scale(Handle, scale), "set_length_scale");

        /// <summary>Best-of-N sampling count for ASR backends.</summary>
        public void SetBestOf(int n)
            => Check(NativeMethods.stelnet_asr_session_set_best_of(Handle, n), "set_best_of");

        /// <summary>Beam-search width for ASR backends that support it.</summary>
        public void SetBeamSize(int n)
            => Check(NativeMethods.stelnet_asr_session_set_beam_size(Handle, n), "set_beam_size");
        ...
        public void SetReturnLogits(bool enable)
            => Check(NativeMethods.stelnet_asr_session_set_return_logits(Handle, enable ? 1 : 0), "set_return_logits");
        ...
        {
            int rc = NativeMethods.stelnet_asr_session_set_grammar_text(Handle, gbnfText, rootRule, penalty);
        ...
        public void SetFallbackThresholds(float entropyThold, float logprobThold, float noSpeechThold, float temperatureInc)
            => Check(NativeMethods.stelnet_asr_session_set_fallback_thresholds(Handle,
        ...
        public void SetAltN(int n)
            => Check(NativeMethods.stelnet_asr_session_set_alt_n(Handle, n), "set_alt_n");
        ...
        public void SetWhisperDecodeExtras(bool suppressNst, string? suppressRegex = null, bool carryInitialPrompt = false)
            => Check(NativeMethods.stelnet_asr_session_set_whisper_decode_extras(Handle,
        ...
        public void SetAsk(string prompt)
            => Check(NativeMethods.stelnet_asr_session_set_ask(Handle, prompt), "set_ask");

        /// <summary>Drop the kokoro per-session phoneme cache.</summary>
        public void ClearPhonemeCache()
            => Check(NativeMethods.stelnet_asr_session_kokoro_clear_phoneme_cache(Handle), "clear_phoneme_cache");
        ...
        {
            var ptr = NativeMethods.stelnet_asr_session_synthesize(Handle, text, out int nSamples);
            ...
            {
                NativeMethods.stelnet_asr_pcm_free(ptr);
            }
        }
        ...
        public void AcceptMarkingResponsibility(string attestation = "")
            => Check(NativeMethods.stelnet_asr_session_accept_marking_responsibility(Handle, attestation ?? ""),
        ...
        {
            int rc = NativeMethods.stelnet_asr_session_set_speaker_identity(Handle, identity ?? "");
            if (rc == -2)
                throw new ArgumentException(
        ...
        {
            var ptr = NativeMethods.stelnet_asr_session_synthesize_raw(Handle, text, out int nSamples);
            if (ptr == IntPtr.Zero || nSamples <= 0)
        ...
        public static void WatermarkEmbed(float[] pcm)
        {
            if (pcm == null || pcm.Length == 0)
                return;
            NativeMethods.stelnet_asr_watermark_embed(pcm, pcm.Length, -1.0f);
        }
        ...
        public static float WatermarkDetect(float[] pcm)
        {
            if (pcm == null || pcm.Length == 0)
                return 0.0f;
            return NativeMethods.stelnet_asr_watermark_detect(pcm, pcm.Length);
        }
        ...
        {
            var ptr = NativeMethods.stelnet_asr_session_speech_to_speech(
            ...
            try
            {
                ...
                if (textPtr != IntPtr.Zero)
                {
                    ...
                    NativeMethods.stelnet_asr_session_translate_text_free(textPtr);
                }
        ...
        /// </summary>
        public int InputSampleRate() => NativeMethods.stelnet_asr_session_input_sample_rate(Handle);
        ...
        /// </summary>
        public int OutputSampleRate() => NativeMethods.stelnet_asr_session_output_sample_rate(Handle);

        /// <summary>
        /// Channel count for audio input: 1 (mono) for every current backend,
        /// 0 on error. Source separation is the stereo exception. (#332)
        /// </summary>
        public int InputChannels() => NativeMethods.stelnet_asr_session_input_channels(Handle);

        /// <summary>
        /// Channel count for synthesized / s2s output audio: 1 (mono), or 0
        /// when the backend has no audio output. (#332)
        /// </summary>
        public int OutputChannels() => NativeMethods.stelnet_asr_session_output_channels(Handle);
        ...
        {
            var r = NativeMethods.stelnet_asr_session_transcribe_lang(Handle, pcm, pcm.Length, language);
            if (r == IntPtr.Zero) throw new InvalidOperationException("Transcription failed");
            try { return ExtractSegments(r); }
            finally { NativeMethods.stelnet_asr_session_result_free(r); }
        }
        ...
        {
            var r = NativeMethods.stelnet_asr_session_transcribe_chunked_lang(
        ...
        public (Segment[] Segments, CtcLogits? Logits) TranscribeWithLogits(float[] pcm, string? language = null)
        {
            ...
            finally { SetReturnLogits(false); NativeMethods.stelnet_asr_session_result_free(r); }
        }
        ...
        {
            int nFrames = NativeMethods.stelnet_asr_session_result_n_logit_frames(r);
            int nVocab = NativeMethods.stelnet_asr_session_result_n_logit_vocab(r);
            var ptr = NativeMethods.stelnet_asr_session_result_logits(r);
        ...
        {
            int n = NativeMethods.stelnet_asr_session_n_vocab(Handle);
            if (n <= 0) return null;
            var vocab = new string[n];
            for (int i = 0; i < n; i++)
                vocab[i] = NativeMethods.PtrToUtf8(NativeMethods.stelnet_asr_session_token_text(Handle, i)) ?? "";
            return vocab;
        }
        ...
        {
            var r = NativeMethods.stelnet_asr_session_transcribe_vad(
        ...
        {
            int nSegs = NativeMethods.stelnet_asr_session_result_n_segments(r);
            var segs = new Segment[nSegs];
            for (int i = 0; i < nSegs; i++)
            {
                string text = NativeMethods.PtrToUtf8(
                    NativeMethods.stelnet_asr_session_result_segment_text(r, i)) ?? "";
                long t0 = NativeMethods.stelnet_asr_session_result_segment_t0(r, i);
                long t1 = NativeMethods.stelnet_asr_session_result_segment_t1(r, i);

                int nWords = NativeMethods.stelnet_asr_session_result_n_words(r, i);
                var words = new Word[nWords];
                for (int j = 0; j < nWords; j++)
                {
                    int nAlts = NativeMethods.stelnet_asr_session_result_word_n_alts(r, i, j);
                    var alts = new AltToken[nAlts];
                    for (int k = 0; k < nAlts; k++)
                    {
                        alts[k] = new AltToken(
                            NativeMethods.PtrToUtf8(NativeMethods.stelnet_asr_session_result_word_alt_text(r, i, j, k)) ?? "",
                            NativeMethods.stelnet_asr_session_result_word_alt_p(r, i, j, k));
                    }
                    words[j] = new Word(
                        NativeMethods.PtrToUtf8(NativeMethods.stelnet_asr_session_result_word_text(r, i, j)) ?? "",
                        NativeMethods.stelnet_asr_session_result_word_t0(r, i, j),
                        NativeMethods.stelnet_asr_session_result_word_t1(r, i, j),
                        NativeMethods.stelnet_asr_session_result_word_p(r, i, j),
                        alts);
                }
                float noSpeechProb = NativeMethods.stelnet_asr_session_result_segment_no_speech_prob(r, i);
                string speaker = NativeMethods.PtrToUtf8(NativeMethods.stelnet_asr_session_result_segment_speaker(r, i)) ?? "";
        ...
        public LanguageDetection DetectLanguage(float[] pcm, string lidModelPath, int method = 0)
        {
            ...
            int rc = NativeMethods.stelnet_asr_session_detect_language(
        ...
        public static LanguageDetection DetectLanguagePcm(float[] pcm, int method, string modelPath, int nThreads = 4)
        {
            var outLang = new byte[16];
            var outProb = new float[1];
            int rc = NativeMethods.stelnet_asr_detect_language_pcm(
        ...
        {
            var ptr = NativeMethods.stelnet_asr_session_translate_text(Handle, text, srcLang, tgtLang, maxTokens);
            if (ptr == IntPtr.Zero)
            ...
            {
                NativeMethods.stelnet_asr_session_translate_text_free(ptr);
            }
        }
        ...
        {
            var p = NativeMethods.stelnet_asr_session_stream_open(
        ...
        {
            var r = NativeMethods.stelnet_asr_align_words_abi(alignerModel, transcript, pcm, pcm.Length, tOffsetCs, nThreads);
            if (r == IntPtr.Zero) throw new InvalidOperationException("Alignment failed");
            try
            {
                int n = NativeMethods.stelnet_asr_align_result_n_words(r);
                var words = new AlignedWord[n];
                for (int i = 0; i < n; i++)
                {
                    words[i] = new AlignedWord(
                        NativeMethods.PtrToUtf8(NativeMethods.stelnet_asr_align_result_word_text(r, i)) ?? "",
                        NativeMethods.stelnet_asr_align_result_word_t0(r, i),
                        NativeMethods.stelnet_asr_align_result_word_t1(r, i));
                }
                return words;
            }
            finally
            {
                NativeMethods.stelnet_asr_align_result_free(r);
            }
        }
                                             ...
                                             int minSilenceMs = 100, int nThreads = 4)
        {
            ...
            int n = NativeMethods.stelnet_asr_vad_segments(vadModelPath, pcm, pcm.Length,
            ...
            try
            {
                ...
                // The native stelnet_asr_vad_segments ABI returns CENTISECONDS
                // (start_cs, end_cs — see stelnet_asr.h), the raw whisper.cpp VAD
            ...
            {
                NativeMethods.stelnet_asr_vad_free(outSpans[0]);
            }
        }
        ...
        public static string? DetectBackendFromGguf(string path)
        {
            ...
            int rc = NativeMethods.stelnet_asr_detect_backend_from_gguf(path, buf, buf.Length);
        ...
        public static float[] EnhanceAudioRnnoise(float[] pcm)
        {
            ...
            int rc = NativeMethods.stelnet_asr_enhance_audio_rnnoise(pcm, pcm.Length, outPcm, outPcm.Length);
        ...
        public static KokoroResolved KokoroResolveForLang(string modelPath, string lang)
        {
            ...
            int rc = NativeMethods.stelnet_asr_kokoro_resolve_model_for_lang_abi(
            ...
            rc = NativeMethods.stelnet_asr_kokoro_resolve_fallback_voice_abi(
    ...
    public sealed class StreamDecoder : IDisposable
    {
        ...
        public int Feed(float[] pcm)
        {
            ...
            int rc = NativeMethods.stelnet_asr_stream_feed(Handle, pcm, pcm.Length);
        ...
        public StreamingUpdate GetText()
        {
            ...
            int rc = NativeMethods.stelnet_asr_stream_get_text(Handle, buf, buf.Length,
        ...
        public void Flush()
        {
            int rc = NativeMethods.stelnet_asr_stream_flush(Handle);
        ...
        public void Dispose()
        {
            if (_handle != IntPtr.Zero)
            {
                NativeMethods.stelnet_asr_stream_close(_handle);
                _handle = IntPtr.Zero;
            }
        }
    }
    ...
    public sealed class TitaNet : IDisposable
    {
        ...
        {
            var p = NativeMethods.stelnet_asr_titanet_init(modelPath, nThreads);
        ...
        public float[] Embed(float[] pcm16k)
        {
            ...
            int rc = NativeMethods.stelnet_asr_titanet_embed(_handle, pcm16k, pcm16k.Length, emb);
        ...
        public static float CosineSim(float[] a, float[] b)
        {
            ...
            return NativeMethods.stelnet_asr_titanet_cosine_sim(a, b, a.Length);
        }
        ...
        public void Dispose()
        {
            if (_handle != IntPtr.Zero)
            {
                NativeMethods.stelnet_asr_titanet_free(_handle);
                _handle = IntPtr.Zero;
            }
        }
    }
    ...
    public sealed class SpeakerDb : IDisposable
    {
        ...
        public static SpeakerDb Open(string dirPath, string expectedNames, bool consentAttested)
        {
            ...
            var p = NativeMethods.stelnet_asr_speaker_db_open(dirPath, expectedNames, 1);
            if (p == IntPtr.Zero) throw new InvalidOperationException($"Failed to open speaker db from {dirPath}");
            return new SpeakerDb(p);
        }

        public int Count => NativeMethods.stelnet_asr_speaker_db_count(_handle);
        ...
        public (string? Name, float Score) Match(float[] embedding, float threshold = 0.5f)
        {
            ...
            float score = NativeMethods.stelnet_asr_speaker_db_match(
        ...
        public static void Enroll(string dirPath, string name, float[] embedding, bool consentAttested)
        {
            ...
            int rc = NativeMethods.stelnet_asr_speaker_db_enroll2(dirPath, name, embedding, embedding.Length, 1);
        ...
        public void Dispose()
        {
            if (_handle != IntPtr.Zero)
            {
                NativeMethods.stelnet_asr_speaker_db_free(_handle);
                _handle = IntPtr.Zero;
            }
        }
    }
