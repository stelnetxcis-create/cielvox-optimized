 ...
 * <p>Usage:
 * <pre>{@code
 * StelnetAsrSession.Resolved r = StelnetAsrSession.kokoroResolveForLang(
 *     "/models/kokoro-82m-q8_0.gguf", "de");
 * try (StelnetAsrSession s = StelnetAsrSession.open(r.modelPath, 4)) {
 ...
 * }</pre>
 */
public final class StelnetAsrSession implements AutoCloseable {

    public interface Lib extends Library {
        Lib INSTANCE = Native.load("stelnet_asr", Lib.class);

        Pointer stelnet_asr_session_open(String modelPath, int nThreads);
        void    stelnet_asr_session_close(Pointer session);
        int     stelnet_asr_session_set_codec_path(Pointer session, String path);
        int     stelnet_asr_session_set_voice(Pointer session, String path, String refTextOrNull);
        int     stelnet_asr_session_set_speaker_name(Pointer session, String name);
        int     stelnet_asr_session_set_speaker_id(Pointer session, int id);
        int     stelnet_asr_session_set_punc_model(Pointer session, String puncModel);
        int     stelnet_asr_session_set_hotwords(Pointer session, String hotwords, float boost);
        int     stelnet_asr_session_set_sensitivity(Pointer session, String preset);
        int     stelnet_asr_session_set_g2p_dict(Pointer session, String source);
        int     stelnet_asr_session_n_speakers(Pointer session);
        String  stelnet_asr_session_get_speaker_name(Pointer session, int i);
        int     stelnet_asr_session_set_instruct(Pointer session, String instruct);
        // #316: synthesize these phonemes verbatim, skipping the G2P. Empty clears. kokoro and piper only (rc=-2 otherwise).
        int          stelnet_asr_session_set_tts_phonemes(Pointer session, String phonemes);
        int     stelnet_asr_session_is_custom_voice(Pointer session);
        int     stelnet_asr_session_is_voice_design(Pointer session);
        Pointer stelnet_asr_session_synthesize(Pointer session, String text, IntByReference outNSamples);
        // UNMARKED synthesis — refused unless stelnet_asr_session_accept_marking_responsibility() was called first.
        Pointer stelnet_asr_session_synthesize_raw(Pointer session, String text, IntByReference outNSamples);
        int     stelnet_asr_session_accept_marking_responsibility(Pointer session, String attestation);
        // Whose voice a PRESET voice is (EU AI Act Art. 50(4)); -2 on a bad value.
        int     stelnet_asr_session_set_speaker_identity(Pointer session, String identity);
        // Speech-to-speech (lfm2-audio, mini-omni2, sidon, voxcpm2-vae). outText receives the intermediate transcript.
        Pointer stelnet_asr_session_speech_to_speech(Pointer session, float[] inSamples, int nInSamples,
                                                  PointerByReference outText, IntByReference outNSamples);
        int     stelnet_asr_session_input_sample_rate(Pointer session);
        // #332: output-side counterparts (rate of synthesize/s2s PCM; mono channels).
        int     stelnet_asr_session_output_sample_rate(Pointer session);
        int     stelnet_asr_session_input_channels(Pointer session);
        int     stelnet_asr_session_output_channels(Pointer session);
        void    stelnet_asr_pcm_free(Pointer pcm);
        // AI-content marking (EU AI Act Art. 50(2)) — the other half of synthesize_raw.
        void    stelnet_asr_watermark_embed(float[] pcm, int nSamples, float alpha);
        float   stelnet_asr_watermark_detect(float[] pcm, int nSamples);
        int     stelnet_asr_session_kokoro_clear_phoneme_cache(Pointer session);
        int     stelnet_asr_session_set_source_language(Pointer session, String lang);
        int     stelnet_asr_session_set_target_language(Pointer session, String lang);
        int     stelnet_asr_session_set_tts_reference_language(Pointer session, String lang);
        int     stelnet_asr_session_set_punctuation(Pointer session, int enable);
        int     stelnet_asr_session_set_translate(Pointer session, int enable);
        int     stelnet_asr_session_set_temperature(Pointer session, float temperature, long seed);
        int     stelnet_asr_session_set_tts_seed(Pointer session, long seed);
        int     stelnet_asr_session_set_max_new_tokens(Pointer session, int maxNewTokens);
        int     stelnet_asr_session_set_frequency_penalty(Pointer session, float penalty);
        int     stelnet_asr_session_set_tts_steps(Pointer session, int steps);
        int     stelnet_asr_session_set_tts_num_candidates(Pointer session, int n);
        int     stelnet_asr_session_set_top_p(Pointer session, float topP);
        int     stelnet_asr_session_set_top_k(Pointer session, int topK);
        int     stelnet_asr_session_set_do_sample(Pointer session, int enable);
        int     stelnet_asr_session_set_min_p(Pointer session, float minP);
        int     stelnet_asr_session_set_repetition_penalty(Pointer session, float r);
        int     stelnet_asr_session_set_cfg_weight(Pointer session, float cfgWeight);
        int     stelnet_asr_session_set_tts_noise_temp(Pointer session, float noiseTemp);
        int     stelnet_asr_session_set_exaggeration(Pointer session, float exaggeration);
        int     stelnet_asr_session_set_max_speech_tokens(Pointer session, int n);
        int     stelnet_asr_session_set_min_speech_tokens(Pointer session, int n);
        int     stelnet_asr_session_set_length_scale(Pointer session, float scale);
        int     stelnet_asr_session_set_best_of(Pointer session, int n);
        int     stelnet_asr_session_set_beam_size(Pointer session, int n);
        int     stelnet_asr_session_set_return_logits(Pointer session, int enable);
        int     stelnet_asr_session_set_grammar_text(Pointer session, String gbnfText, String rootRule, float penalty);
        int     stelnet_asr_session_set_fallback_thresholds(Pointer session, float entropyThold,
                                                          float logprobThold, float noSpeechThold,
                                                          float temperatureInc);
        int     stelnet_asr_session_set_alt_n(Pointer session, int n);
        int     stelnet_asr_session_set_whisper_decode_extras(Pointer session, int suppressNst,
                                                            String suppressRegex, int carryInitialPrompt);
        int     stelnet_asr_session_set_ask(Pointer session, String prompt);
        int     stelnet_asr_session_detect_language(Pointer session, float[] pcm, int n_samples,
        ...
        int stelnet_asr_kokoro_resolve_fallback_voice_abi(
                String modelPath, String lang,
                byte[] outPath, int outPathLen,
                byte[] outPicked, int outPickedLen);

        // --- ASR transcription (PLAN #59) ---
        Pointer stelnet_asr_session_transcribe(Pointer session, float[] pcm, int nSamples);
        Pointer stelnet_asr_session_transcribe_lang(Pointer session, float[] pcm, int nSamples, String language);
        Pointer stelnet_asr_session_transcribe_chunked_lang(Pointer session, float[] pcm, int nSamples,
                                                         int chunkSeconds, int overlapSeconds, String language);
        int     stelnet_asr_get_progress();
        Pointer stelnet_asr_session_transcribe_vad(Pointer session, float[] pcm, int nSamples,
                                                 int sampleRate, String vadModelPath, Pointer opts);
        int          stelnet_asr_session_result_n_segments(Pointer result);
        String       stelnet_asr_session_result_segment_text(Pointer result, int i);
        long         stelnet_asr_session_result_segment_t0(Pointer result, int i);
        long         stelnet_asr_session_result_segment_t1(Pointer result, int i);
        int          stelnet_asr_session_result_n_words(Pointer result, int iSeg);
        String       stelnet_asr_session_result_word_text(Pointer result, int iSeg, int iWord);
        long         stelnet_asr_session_result_word_t0(Pointer result, int iSeg, int iWord);
        long         stelnet_asr_session_result_word_t1(Pointer result, int iSeg, int iWord);
        float        stelnet_asr_session_result_word_p(Pointer result, int iSeg, int iWord);
        float        stelnet_asr_session_result_segment_no_speech_prob(Pointer result, int iSeg);
        // #300: native per-segment speaker label ("(Speaker N) "), or "" when the
        // backend does not diarize natively. Never NULL.
        String       stelnet_asr_session_result_segment_speaker(Pointer result, int i);
        // Per-frame CTC logits (opted in via stelnet_asr_session_set_return_logits)
        ...
        int          stelnet_asr_session_result_n_logit_frames(Pointer result);
        int          stelnet_asr_session_result_n_logit_vocab(Pointer result);
        Pointer      stelnet_asr_session_result_logits(Pointer result);
        void         stelnet_asr_session_result_free(Pointer result);

        // --- Alignment (PLAN #59) ---
        Pointer stelnet_asr_align_words_abi(String alignerModel, String transcript,
                                         float[] samples, int nSamples, long tOffsetCs, int nThreads);
        int    stelnet_asr_align_result_n_words(Pointer result);
        String stelnet_asr_align_result_word_text(Pointer result, int i);
        long   stelnet_asr_align_result_word_t0(Pointer result, int i);
        long   stelnet_asr_align_result_word_t1(Pointer result, int i);
        void   stelnet_asr_align_result_free(Pointer result);

        // --- Standalone LID (PLAN #59) ---
        int stelnet_asr_detect_language_pcm(float[] samples, int nSamples, int method,
                                         String modelPath, int nThreads, int useGpu,
                                         int gpuDevice, int flashAttn,
                                         byte[] outLang, int outLangCap, float[] outProb);

        // --- VAD (PLAN #59) ---
        int stelnet_asr_vad_segments(String vadModelPath, float[] pcm, int nSamples,
                                  int sampleRate, float threshold, int minSpeechMs, int minSilenceMs,
                                  int nThreads, int useGpu, Pointer[] outSpans);
        void stelnet_asr_vad_free(Pointer spans);

        // --- Punctuation (PLAN #59) ---
        Pointer stelnet_asr_punc_init(String modelPath);
        String  stelnet_asr_punc_process(Pointer ctx, String text);
        void    stelnet_asr_punc_free_text(String text);
        void    stelnet_asr_punc_free(Pointer ctx);

        // --- Registry + cache (PLAN #59) ---
        int stelnet_asr_registry_lookup_abi(String backend, byte[] outFilename, int filenameCap,
                                         byte[] outUrl, int urlCap, byte[] outSize, int sizeCap);
        int stelnet_asr_registry_default_bundle_info_abi(String backend, byte[] outBackend, int backendCap,
                                                       byte[] outLicense, int licenseCap,
                                                       int[] outRequiresAcceptance);
        int stelnet_asr_registry_default_bundle_artifact_abi(String backend, int index, int[] outKind,
                                                           byte[] outFilename, int filenameCap,
                                                           byte[] outUrl, int urlCap,
                                                           byte[] outSize, int sizeCap);
        int stelnet_asr_cache_ensure_file_abi(String filename, String url, int quiet,
                                           String cacheDirOverride, byte[] outBuf, int outCap);
        int stelnet_asr_cache_dir_abi(String cacheDirOverride, byte[] outBuf, int outCap);

        // --- Streaming (PLAN #62b) — rolling-window decoder, whisper-only today.
        Pointer stelnet_asr_session_stream_open(Pointer session, int nThreads, int stepMs,
                                             int lengthMs, int keepMs, String language, int translate);
        int     stelnet_asr_stream_feed(Pointer stream, float[] pcm, int nSamples);
        int     stelnet_asr_stream_get_text(Pointer stream, byte[] outText, int outCap,
                                         DoubleByReference outT0, DoubleByReference outT1,
                                         LongByReference outCounter);
        int     stelnet_asr_stream_flush(Pointer stream);
        void    stelnet_asr_stream_close(Pointer stream);

        // --- Microphone capture (PLAN #62d) — miniaudio-backed cross-platform.
        Pointer stelnet_asr_mic_open(int sampleRate, int channels, MicCallback cb, Pointer userdata);
        int     stelnet_asr_mic_start(Pointer mic);
        int     stelnet_asr_mic_stop(Pointer mic);
        void    stelnet_asr_mic_close(Pointer mic);
        String  stelnet_asr_mic_default_device_name();

        // --- Full C-ABI parity additions ---
        void stelnet_asr_stream_set_live_decode(Pointer stream, int enabled);

        // params_set_* on whisper_full_params
        void stelnet_asr_params_set_language(Pointer p, String lang);
        void stelnet_asr_params_set_translate(Pointer p, int v);
        void stelnet_asr_params_set_detect_language(Pointer p, int v);
        void stelnet_asr_params_set_token_timestamps(Pointer p, int v);
        void stelnet_asr_params_set_n_threads(Pointer p, int n);
        void stelnet_asr_params_set_max_len(Pointer p, int n);
        void stelnet_asr_params_set_best_of(Pointer p, int n);
        void stelnet_asr_params_set_split_on_word(Pointer p, int v);
        void stelnet_asr_params_set_no_context(Pointer p, int v);
        void stelnet_asr_params_set_single_segment(Pointer p, int v);
        void stelnet_asr_params_set_print_realtime(Pointer p, int v);
        void stelnet_asr_params_set_print_progress(Pointer p, int v);
        void stelnet_asr_params_set_print_timestamps(Pointer p, int v);
        void stelnet_asr_params_set_print_special(Pointer p, int v);
        void stelnet_asr_params_set_suppress_blank(Pointer p, int v);
        void stelnet_asr_params_set_temperature(Pointer p, float t);
        void stelnet_asr_params_set_max_tokens(Pointer p, int n);
        void stelnet_asr_params_set_initial_prompt(Pointer p, String prompt);
        void stelnet_asr_params_set_alt_n(Pointer p, int n);
        void stelnet_asr_params_set_vad(Pointer p, int v);
        void stelnet_asr_params_set_vad_model_path(Pointer p, String path);
        void stelnet_asr_params_set_vad_threshold(Pointer p, float t);
        void stelnet_asr_params_set_vad_min_speech_ms(Pointer p, int ms);
        void stelnet_asr_params_set_vad_min_silence_ms(Pointer p, int ms);
        void stelnet_asr_params_set_tdrz(Pointer p, int v);

        // Token-level accessors
        long  stelnet_asr_token_t0(Pointer ctx, int iSeg, int iTok);
        long  stelnet_asr_token_t1(Pointer ctx, int iSeg, int iTok);
        float stelnet_asr_token_p(Pointer ctx, int iSeg, int iTok);
        int   stelnet_asr_token_n_alts(Pointer ctx, int iSeg, int iTok);
        int   stelnet_asr_token_alt_id(Pointer ctx, int iSeg, int iTok, int iAlt);
        float stelnet_asr_token_alt_p(Pointer ctx, int iSeg, int iTok, int iAlt);
        int   stelnet_asr_token_alt_text(Pointer ctx, int iSeg, int iTok, int iAlt, byte[] out, int outCap);

        // Language detection (whisper context)
        float stelnet_asr_detect_language(Pointer ctx, float[] pcm, int nSamples,
                                       int nThreads, byte[] outCode, int outCap);

        // VAD slices
        int stelnet_asr_vad_slices(String vadModelPath, float[] pcm, int nSamples,
                                int sampleRate, float threshold, int minSpeechMs, int minSilenceMs,
                                int speechPadMs, float maxChunkDurationS, int nThreads,
                                Pointer[] outSpans);

        // LCS dedup
        int stelnet_asr_lcs_dedup_prefix_count(int[] prevTailTokens, int nPrev,
                                            int[] currTokens, int nCurr, int minLcsLength);

        // Direct Parakeet API
        Pointer stelnet_asr_parakeet_init(String modelPath, int nThreads, int useFlash);
        void    stelnet_asr_parakeet_free(Pointer ctx);
        Pointer stelnet_asr_parakeet_transcribe(Pointer ctx, float[] pcm, int nSamples, String language);
        String  stelnet_asr_parakeet_result_text(Pointer r);
        int     stelnet_asr_parakeet_result_n_words(Pointer r);
        String  stelnet_asr_parakeet_result_word_text(Pointer r, int i);
        long    stelnet_asr_parakeet_result_word_t0(Pointer r, int i);
        long    stelnet_asr_parakeet_result_word_t1(Pointer r, int i);
        int     stelnet_asr_parakeet_result_n_tokens(Pointer r);
        String  stelnet_asr_parakeet_result_token_text(Pointer r, int i);
        long    stelnet_asr_parakeet_result_token_t0(Pointer r, int i);
        long    stelnet_asr_parakeet_result_token_t1(Pointer r, int i);
        float   stelnet_asr_parakeet_result_token_p(Pointer r, int i);
        void    stelnet_asr_parakeet_result_free(Pointer r);

        // TitaNet
        Pointer stelnet_asr_titanet_init(String modelPath, int nThreads);
        void    stelnet_asr_titanet_free(Pointer ctx);
        int     stelnet_asr_titanet_embed(Pointer ctx, float[] pcm16k, int nSamples, float[] out);
        float   stelnet_asr_titanet_cosine_sim(float[] a, float[] b, int dim);
        ...
        Pointer stelnet_asr_speaker_db_open(String dirPath, String expectedNamesCsv, int consentAttested);
        void    stelnet_asr_speaker_db_free(Pointer db);
        int     stelnet_asr_speaker_db_count(Pointer db);
        float   stelnet_asr_speaker_db_match(Pointer db, float[] embedding, int dim,
                                          float threshold, byte[] outName, int outCap);
        int     stelnet_asr_speaker_db_enroll2(String dirPath, String name, float[] embedding, int dim,
                                            int consentAttested);

        // Pluggable speaker embedder + clustering + pyannote cache
        Pointer stelnet_asr_speaker_embedder_make_abi(String modelSpec, int nThreads, String cacheDir);
        void    stelnet_asr_speaker_embedder_free_abi(Pointer embedder);
        int     stelnet_asr_speaker_embedder_dim_abi(Pointer embedder);
        int     stelnet_asr_speaker_embedder_embed_abi(Pointer embedder, float[] pcm16k, int nSamples, float[] out);
        String  stelnet_asr_speaker_embedder_name_abi(Pointer embedder);
        int     stelnet_asr_speaker_cluster_abi(float[] embeddings, int n, int dim,
                                            float mergeThreshold, int maxSpeakers, int[] labelsOut);
        Pointer stelnet_asr_pyannote_cache_compute_abi(float[] fullAudio, int nSamples,
                                                    String modelPath, int nThreads);
        void    stelnet_asr_pyannote_cache_free_abi(Pointer cache);
        int     stelnet_asr_pyannote_cache_apply_abi(Pointer cache, long sliceT0Cs,
                                                  Pointer segs, int nSegs);

        // Kokoro lang helpers
        boolean stelnet_asr_kokoro_lang_is_german_abi(String lang);
        boolean stelnet_asr_kokoro_lang_has_native_voice_abi(String lang);

        // Backend detection
        int stelnet_asr_detect_backend_from_gguf(String path, byte[] outName, int outCap);

        // RNNoise audio enhancement
        int stelnet_asr_enhance_audio_rnnoise(float[] inPcm, int nSamples, float[] outPcm, int outCap);

        // Diarization
        int stelnet_asr_diarize_segments_abi(float[] leftPcm, float[] rightPcm, int nSamples,
                                          int isStereo, Pointer segs, int nSegs, Pointer opts);

        // Text-LID
        int stelnet_asr_text_detect_language(String text, String modelPath, int nThreads,
                                          byte[] outLabel, int outLabelCap, float[] outConfidence);

        // Registry extras
        int stelnet_asr_registry_lookup_by_filename_abi(String filename, byte[] outFilename, int filenameCap,
                                                     byte[] outUrl, int urlCap, byte[] outSize, int sizeCap);
        int stelnet_asr_registry_list_backends_abi(byte[] outCsv, int outCap);

        // Session extras
        int     stelnet_asr_session_available_backends(byte[] outCsv, int outCap);
        int     stelnet_asr_session_detected_language(Pointer session, byte[] outBuf, int outCap);
        ...
        int     stelnet_asr_session_n_vocab(Pointer session);
        String  stelnet_asr_session_token_text(Pointer session, int id);
        Pointer stelnet_asr_session_open_explicit(String modelPath, String backendName, int nThreads);
        Pointer stelnet_asr_session_open_with_params(String modelPath, String backendName, Pointer params);
        Pointer stelnet_asr_session_transcribe_vad_lang(Pointer session, float[] pcm, int nSamples,
                                                     int sampleRate, String vadModelPath, Pointer opts,
                                                     String language);
        Pointer stelnet_asr_session_translate_text(Pointer session, String text, String srcLang,
                                                String tgtLang, int maxTokens);
        void    stelnet_asr_session_translate_text_free(Pointer text);
        int     stelnet_asr_session_result_word_n_alts(Pointer result, int iSeg, int iWord);
        String  stelnet_asr_session_result_word_alt_text(Pointer result, int iSeg, int iWord, int iAlt);
        float   stelnet_asr_session_result_word_alt_p(Pointer result, int iSeg, int iWord, int iAlt);

        // Streaming (whisper context)
        Pointer stelnet_asr_stream_open(Pointer ctx, int nThreads, int stepMs,
                                     int lengthMs, int keepMs, String language, int translate);
    }

    /**
     * JNA callback for {@code stelnet_asr_mic_callback}. Fired on
    ...
    private StelnetAsrSession(Pointer handle) {
        this.handle = handle;
    }
     ...
     */
    public static StelnetAsrSession open(String modelPath, int nThreads) {
        Pointer p = Lib.INSTANCE.stelnet_asr_session_open(modelPath, nThreads);
        if (p == null) {
            throw new IllegalStateException("stelnet_asr_session_open: failed to open " + modelPath);
        }
        return new StelnetAsrSession(p);
    }
    ...
    public void clearPhonemeCache() {
        int rc = Lib.INSTANCE.stelnet_asr_session_kokoro_clear_phoneme_cache(handle);
    ...
    public void setSourceLanguage(String lang) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_source_language(handle, lang == null ? "" : lang);
    ...
    public void setTargetLanguage(String lang) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_target_language(handle, lang == null ? "" : lang);
    ...
    public void setTtsReferenceLanguage(String lang) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_tts_reference_language(handle, lang == null ? "" : lang);
    ...
    public void setPunctuation(boolean enable) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_punctuation(handle, enable ? 1 : 0);
    ...
    public void setTranslate(boolean enable) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_translate(handle, enable ? 1 : 0);
    ...
    public void setTemperature(float temperature, long seed) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_temperature(handle, temperature, seed);
    ...
    public void setTtsSeed(long seed) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_tts_seed(handle, seed);
    ...
    public void setMaxNewTokens(int maxNewTokens) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_max_new_tokens(handle, maxNewTokens);
    ...
    public void setFrequencyPenalty(float penalty) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_frequency_penalty(handle, penalty);
    ...
    public void setTtsSteps(int steps) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_tts_steps(handle, steps);
    ...
    public void setTtsNumCandidates(int n) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_tts_num_candidates(handle, n);
    ...
    public void setTopP(float topP) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_top_p(handle, topP);
    ...
    public void setTopK(int topK) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_top_k(handle, topK);
    ...
    public void setDoSample(boolean enable) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_do_sample(handle, enable ? 1 : 0);
    ...
    public void setMinP(float minP) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_min_p(handle, minP);
    ...
    public void setRepetitionPenalty(float r) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_repetition_penalty(handle, r);
    ...
    public void setCfgWeight(float cfgWeight) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_cfg_weight(handle, cfgWeight);
    ...
    public void setTtsNoiseTemp(float noiseTemp) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_tts_noise_temp(handle, noiseTemp);
    ...
    public void setExaggeration(float exaggeration) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_exaggeration(handle, exaggeration);
    ...
    public void setMaxSpeechTokens(int n) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_max_speech_tokens(handle, n);
    ...
    public void setMinSpeechTokens(int n) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_min_speech_tokens(handle, n);
    ...
    public void setLengthScale(float scale) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_length_scale(handle, scale);
    ...
    public void setBestOf(int n) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_best_of(handle, n);
    ...
    public void setBeamSize(int n) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_beam_size(handle, n);
    ...
    public void setReturnLogits(boolean enable) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_return_logits(handle, enable ? 1 : 0);
    ...
    public void setGrammarText(String gbnfText, String rootRule, float penalty) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_grammar_text(handle, gbnfText, rootRule, penalty);
    ...
    public void setFallbackThresholds(float entropyThold, float logprobThold, float noSpeechThold, float temperatureInc) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_fallback_thresholds(handle,
    ...
    public void setAltN(int n) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_alt_n(handle, n);
    ...
    public void setWhisperDecodeExtras(boolean suppressNst, String suppressRegex, boolean carryInitialPrompt) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_whisper_decode_extras(handle,
    ...
    public void setAsk(String prompt) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_ask(handle, prompt);
    ...
    public String detectLanguage(float[] pcm, String lidModelPath, int method, float[] outConfidence) {
        ...
        int rc = Lib.INSTANCE.stelnet_asr_session_detect_language(
    ...
    public void setCodecPath(String path) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_codec_path(handle, path);
    ...
    public void setVoice(String path, String refText) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_voice(handle, path, refText);
    ...
    public void setSpeakerName(String name) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_speaker_name(handle, name);
    ...
    public void setSpeakerId(int id) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_speaker_id(handle, id);
    ...
    public void setPuncModel(String puncModel) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_punc_model(handle, puncModel);
    ...
    public void setHotwords(String hotwords, float boost) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_hotwords(handle, hotwords, boost);
    ...
    public void setSensitivity(String preset) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_sensitivity(handle, preset);
        if (rc == -2) {
    ...
    public void setG2pDict(String source) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_g2p_dict(handle, source);
    ...
    public String[] speakers() {
        int n = Lib.INSTANCE.stelnet_asr_session_n_speakers(handle);
        String[] out = new String[n];
        for (int i = 0; i < n; i++) {
            String s = Lib.INSTANCE.stelnet_asr_session_get_speaker_name(handle, i);
            out[i] = (s == null) ? "" : s;
        }
        return out;
    }
    ...
    public void setInstruct(String instruct) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_instruct(handle, instruct);
    ...
    public void setTtsPhonemes(String phonemes) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_tts_phonemes(handle, phonemes == null ? "" : phonemes);
    ...
    public boolean isCustomVoice() {
        return Lib.INSTANCE.stelnet_asr_session_is_custom_voice(handle) != 0;
    }
    ...
    public boolean isVoiceDesign() {
        return Lib.INSTANCE.stelnet_asr_session_is_voice_design(handle) != 0;
    }
    ...
    public float[] synthesize(String text) {
        ...
        Pointer pcm = Lib.INSTANCE.stelnet_asr_session_synthesize(handle, text, n);
        ...
        try {
            ...
            Lib.INSTANCE.stelnet_asr_pcm_free(pcm);
        }
    }
    ...
    public float[] synthesizeRaw(String text) {
        IntByReference n = new IntByReference(0);
        Pointer pcm = Lib.INSTANCE.stelnet_asr_session_synthesize_raw(handle, text, n);
        if (pcm == null || n.getValue() <= 0) {
    ...
    public void acceptMarkingResponsibility(String attestation) {
        int rc = Lib.INSTANCE.stelnet_asr_session_accept_marking_responsibility(
    ...
    public void setSpeakerIdentity(String identity) {
        int rc = Lib.INSTANCE.stelnet_asr_session_set_speaker_identity(
    ...
    public static void watermarkEmbed(float[] pcm) {
        ...
        Lib.INSTANCE.stelnet_asr_watermark_embed(pcm, pcm.length, -1.0f);
    }
    ...
    public static float watermarkDetect(float[] pcm) {
        ...
        return Lib.INSTANCE.stelnet_asr_watermark_detect(pcm, pcm.length);
    }
    ...
    public SpeechToSpeechResult speechToSpeech(float[] inSamples) {
        ...
        Pointer pcm = Lib.INSTANCE.stelnet_asr_session_speech_to_speech(
        ...
        if (tp != null) {
            text = tp.getString(0);
            Lib.INSTANCE.stelnet_asr_session_translate_text_free(tp);
        }
        try {
    ...
    public int inputSampleRate() {
        return Lib.INSTANCE.stelnet_asr_session_input_sample_rate(handle);
    }
    ...
    public int outputSampleRate() {
        return Lib.INSTANCE.stelnet_asr_session_output_sample_rate(handle);
    }
    ...
    public int inputChannels() {
        return Lib.INSTANCE.stelnet_asr_session_input_channels(handle);
    }
    ...
    public int outputChannels() {
        return Lib.INSTANCE.stelnet_asr_session_output_channels(handle);
    }
    ...
    public Stream streamOpen(int stepMs, int lengthMs, int keepMs, String language, boolean translate) {
        Pointer p = Lib.INSTANCE.stelnet_asr_session_stream_open(
                handle, 4, stepMs, lengthMs, keepMs, language == null ? "" : language, translate ? 1 : 0);
        if (p == null) {
            throw new IllegalStateException(
                    "stelnet_asr_session_stream_open failed (whisper-only today)");
        }
        return new Stream(p);
    }
    ...
    public static final class Stream implements AutoCloseable {
        ...
        public int feed(float[] pcm) {
            ...
            int rc = Lib.INSTANCE.stelnet_asr_stream_feed(handle, pcm, pcm.length);
            if (rc < 0) throw new IllegalStateException("stelnet_asr_stream_feed failed (rc=" + rc + ")");
            return rc;
        }
        ...
        public StreamingUpdate getText() {
            ...
            int rc = Lib.INSTANCE.stelnet_asr_stream_get_text(handle, buf, buf.length, t0, t1, counter);
            if (rc < 0) throw new IllegalStateException("stelnet_asr_stream_get_text failed (rc=" + rc + ")");
            int n = 0;
        ...
        public void flush() {
            if (handle == null) throw new IllegalStateException("stream is closed");
            int rc = Lib.INSTANCE.stelnet_asr_stream_flush(handle);
            if (rc < 0) throw new IllegalStateException("stelnet_asr_stream_flush failed (rc=" + rc + ")");
        }

        @Override
        public void close() {
            if (handle != null) {
                Lib.INSTANCE.stelnet_asr_stream_close(handle);
                handle = null;
            }
        }
    }
    ...
    public void close() {
        if (handle != null) {
            Lib.INSTANCE.stelnet_asr_session_close(handle);
            handle = null;
        }
    }
    ...
    public Segment[] transcribeLang(float[] pcm, String lang) {
        Pointer r = Lib.INSTANCE.stelnet_asr_session_transcribe_lang(handle, pcm, pcm.length, lang);
        if (r == null) throw new RuntimeException("transcription failed");
        try { return extractSegments(r); } finally { Lib.INSTANCE.stelnet_asr_session_result_free(r); }
    }
    ...
    public TranscriptionWithLogits transcribeWithLogits(float[] pcm, String lang) {
        ...
        try {
            ...
            setReturnLogits(false);
            Lib.INSTANCE.stelnet_asr_session_result_free(r);
        }
    }
    ...
    public Segment[] transcribeChunked(float[] pcm, int chunkSeconds, int overlapSeconds, String lang) {
        Pointer r = Lib.INSTANCE.stelnet_asr_session_transcribe_chunked_lang(handle, pcm, pcm.length,
    ...
    private static Segment[] extractSegments(Pointer r) {
        int nSegs = Lib.INSTANCE.stelnet_asr_session_result_n_segments(r);
        Segment[] segs = new Segment[nSegs];
        for (int i = 0; i < nSegs; i++) {
            String text = Lib.INSTANCE.stelnet_asr_session_result_segment_text(r, i);
            long t0 = Lib.INSTANCE.stelnet_asr_session_result_segment_t0(r, i);
            long t1 = Lib.INSTANCE.stelnet_asr_session_result_segment_t1(r, i);
            int nWords = Lib.INSTANCE.stelnet_asr_session_result_n_words(r, i);
            Word[] words = new Word[nWords];
            for (int j = 0; j < nWords; j++) {
                words[j] = new Word(
                    Lib.INSTANCE.stelnet_asr_session_result_word_text(r, i, j),
                    Lib.INSTANCE.stelnet_asr_session_result_word_t0(r, i, j),
                    Lib.INSTANCE.stelnet_asr_session_result_word_t1(r, i, j),
                    Lib.INSTANCE.stelnet_asr_session_result_word_p(r, i, j));
            }
            float noSpeechProb = Lib.INSTANCE.stelnet_asr_session_result_segment_no_speech_prob(r, i);
            String speaker = Lib.INSTANCE.stelnet_asr_session_result_segment_speaker(r, i);
    ...
    public String detectedLanguage() {
        ...
        Lib.INSTANCE.stelnet_asr_session_detected_language(handle, buf, buf.length);
    ...
    private static CtcLogits extractLogits(Pointer r) {
        int nFrames = Lib.INSTANCE.stelnet_asr_session_result_n_logit_frames(r);
        int nVocab = Lib.INSTANCE.stelnet_asr_session_result_n_logit_vocab(r);
        Pointer ptr = Lib.INSTANCE.stelnet_asr_session_result_logits(r);
    ...
    public String[] ctcVocab() {
        int n = Lib.INSTANCE.stelnet_asr_session_n_vocab(handle);
        if (n <= 0) return null;
        String[] vocab = new String[n];
        for (int i = 0; i < n; i++) {
            String p = Lib.INSTANCE.stelnet_asr_session_token_text(handle, i);
            vocab[i] = p != null ? p : "";
        }
        return vocab;
    }
    ...
    public static AlignedWord[] alignWords(String alignerModel, String transcript,
        ...
        Pointer r = Lib.INSTANCE.stelnet_asr_align_words_abi(alignerModel, transcript,
                pcm, pcm.length, tOffsetCs, nThreads);
        if (r == null) throw new RuntimeException("alignment failed");
        try {
            int n = Lib.INSTANCE.stelnet_asr_align_result_n_words(r);
            AlignedWord[] words = new AlignedWord[n];
            for (int i = 0; i < n; i++) {
                words[i] = new AlignedWord(
                    Lib.INSTANCE.stelnet_asr_align_result_word_text(r, i),
                    Lib.INSTANCE.stelnet_asr_align_result_word_t0(r, i),
                    Lib.INSTANCE.stelnet_asr_align_result_word_t1(r, i));
            }
            return words;
        } finally {
            Lib.INSTANCE.stelnet_asr_align_result_free(r);
        }
    }
    ...
    public static VADSpan[] vadSegments(String vadModelPath, float[] pcm, int sampleRate,
        ...
        int n = Lib.INSTANCE.stelnet_asr_vad_segments(vadModelPath, pcm, pcm.length,
        ...
        try {
            ...
            Lib.INSTANCE.stelnet_asr_vad_free(outSpans[0]);
        }
    }
    ...
    public static String[] detectLanguagePcm(float[] pcm, int method, String modelPath, int nThreads) {
        ...
        int rc = Lib.INSTANCE.stelnet_asr_detect_language_pcm(pcm, pcm.length, method,
     ...
     * what the StelnetASR CLI does for {@code --backend kokoro -l <lang>}
    ...
    public static Resolved kokoroResolveForLang(String modelPath, String lang) {
        ...
        int rc = Lib.INSTANCE.stelnet_asr_kokoro_resolve_model_for_lang_abi(
        ...
        rc = Lib.INSTANCE.stelnet_asr_kokoro_resolve_fallback_voice_abi(
