...
class StelnetAsrLogitsTest {
    ...
    void sessionOmniCtcLogits() throws Exception {
        ...
        Pointer session = StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_open_explicit(modelPath, "omniasr", 4);
        assertNotNull(session, "session open omniasr");
        try {
            assertEquals(0,
                    StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_set_return_logits(session, 1),
                    "set_return_logits");
            Pointer r = StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_transcribe_lang(session, pcm, pcm.length, null);
            assertNotNull(r, "transcribe_lang");
            try {
                int nFrames = StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_result_n_logit_frames(r);
                int nVocab = StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_result_n_logit_vocab(r);
                Pointer ptr = StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_result_logits(r);
                ...
                StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_result_free(r);
            }
        } finally {
            StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_close(session);
        }
    }
    ...
    void sessionOmniCtcVocab() throws Exception {
        ...
        assertNotNull(session, "session open omniasr");
        try {
            int nVocab = StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_n_vocab(session);
            assertTrue(nVocab > 1000, "unexpectedly small vocab: " + nVocab);
            String[] vocab = new String[nVocab];
            boolean boundary = false;
            for (int i = 0; i < nVocab; i++) {
                String p = StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_token_text(session, i);
                vocab[i] = p != null ? p : "";
            ...
            assertEquals(0,
                    StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_set_return_logits(session, 1), "set_return_logits");
            Pointer r = StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_transcribe_lang(session, pcm, pcm.length, null);
            assertNotNull(r, "transcribe_lang");
            try {
                int nSeg = StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_result_n_segments(r);
                StringBuilder textB = new StringBuilder();
                for (int i = 0; i < nSeg; i++) {
                    String t = StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_result_segment_text(r, i);
                    if (t != null && !t.isEmpty()) {
                ...
                int nFrames = StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_result_n_logit_frames(r);
                int nv = StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_result_n_logit_vocab(r);
                Pointer ptr = StelnetAsrSession.Lib.INSTANCE.stelnet_asr_session_result_logits(r);
