...
# under third_party/c2pa, or -DSTELNETTTS_C2PA_FETCH=ON downloads it here (below).

if (DEFINED _STELNETTTS_C2PA_INCLUDED)
    return()
endif()
set(_STELNETTTS_C2PA_INCLUDED ON)

set(_c2pa_hint "${CMAKE_SOURCE_DIR}/third_party/c2pa")

# -DSTELNETTTS_C2PA_FETCH=ON downloads the prebuilt native lib at configure time,
...
option(STELNETTTS_C2PA_FETCH "Download the prebuilt c2pa-rs native lib if absent" OFF)
set(STELNETTTS_C2PA_VERSION "0.89.3" CACHE STRING "c2pa-rs prebuilt version to fetch")
if (STELNETTTS_C2PA_FETCH AND NOT EXISTS "${_c2pa_hint}/include/c2pa.h")
    ...
    # c2pa-rs ships a prebuilt for every StelnetTTS target: desktop
    ...
    set(_c2pa_url
        "https://github.com/contentauth/c2pa-rs/releases/download/c2pa-v${STELNETTTS_C2PA_VERSION}/c2pa-v${STELNETTTS_C2PA_VERSION}-${_c2pa_triple}.zip")
...
set(STELNETTTS_C2PA_STATIC OFF)
if (C2PA_LIBRARY)
    get_filename_component(_c2pa_ext "${C2PA_LIBRARY}" LAST_EXT)
    if (_c2pa_ext STREQUAL ".a")
        set(STELNETTTS_C2PA_STATIC ON)
    endif()
endif()

if (C2PA_LIBRARY AND C2PA_INCLUDE_DIR)
    set(STELNETTTS_C2PA_ENABLED ON)
    if (STELNETTTS_C2PA_STATIC)
        message(STATUS "C2PA signing enabled — static (${C2PA_LIBRARY})")
    else()
        message(STATUS "C2PA signing enabled — dynamic sidecar (${C2PA_LIBRARY})")
    endif()
else()
    set(STELNETTTS_C2PA_ENABLED OFF)
    message(STATUS "C2PA signing disabled (run scripts/fetch-c2pa.sh or pass -DSTELNETTTS_C2PA_FETCH=ON)")
endif()
...
function(stelnettts_enable_c2pa TARGET)
    if (NOT STELNETTTS_C2PA_ENABLED)
        return()
    endif()
    target_link_libraries(${TARGET} PRIVATE ${C2PA_LIBRARY})
    target_include_directories(${TARGET} PRIVATE ${C2PA_INCLUDE_DIR})
    target_compile_definitions(${TARGET} PRIVATE STELNETTTS_HAVE_C2PA=1)
