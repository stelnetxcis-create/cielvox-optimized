// stelnettts_chat.h — public C ABI for text → text chat / LLM inference.
//
// Sits next to stelnettts.h (the ASR + audio surface). Implemented in
// src/chat.cpp on top of the private `stelnettts-llama-core` static lib
// (vendored llama.cpp). Downstream consumers see ONLY POD structs and
// opaque handles — no llama.h types leak into this header.
//
// Threading
// ---------
//   One call at a time per `stelnettts_chat_session_t` — the session
...
//
//   `stelnettts_chat_close` is the exception, and the one call meant to be
...
// Memory
// ------
//   The KV cache persists across `stelnettts_chat_generate` calls inside
//   one session, so multi-turn chats don't re-prefill the full history.
//   Use `stelnettts_chat_reset` to flush.
//
...
// ------------------------------------------------------------
// Everything else in StelnetTTS either records what a human said (ASR) or
...
// there are two duties on it that StelnetTTS does NOT discharge:
//
...
//     `stelnettts_chat_ai_disclosure_text()` is the canonical wording; show it
...
#ifndef STELNETTTS_CHAT_H
#define STELNETTTS_CHAT_H
...
#define STELNETTTS_CHAT_API __declspec(dllexport)
#else
#define STELNETTTS_CHAT_API __declspec(dllimport)
#endif
#else
#define STELNETTTS_CHAT_API __attribute__((visibility("default")))
#endif
#else
#define STELNETTTS_CHAT_API
#endif
...
// Error reporting
// ---------------------------------------------------------------------------
// Every entry point that can fail accepts a `stelnettts_chat_error*` (may be
...
//
// `STELNETTTS_CHAT_ERR_ABORTED` (below) is the one code with a stable,
...
typedef struct stelnettts_chat_error {
    int32_t code;
    char message[256];
} stelnettts_chat_error;
...
typedef struct stelnettts_chat_message {
    const char* role;
    const char* content;
} stelnettts_chat_message;

// ---------------------------------------------------------------------------
// Open params (per-session, model-level)
// ---------------------------------------------------------------------------
// All fields have well-defined defaults via `stelnettts_chat_open_params_default`.
typedef struct stelnettts_chat_open_params {
...
} stelnettts_chat_open_params;

// Populate `out` with sensible defaults. Safe to call before _open.
STELNETTTS_CHAT_API void stelnettts_chat_open_params_default(stelnettts_chat_open_params* out);

// ---------------------------------------------------------------------------
// Generate params (per-call, sampler-level)
// ---------------------------------------------------------------------------
// Phase 2 + 3 fields: top_k / top_p / repeat_penalty / seed.
typedef struct stelnettts_chat_generate_params {
    ...
    bool prefill_only;
} stelnettts_chat_generate_params;

STELNETTTS_CHAT_API void stelnettts_chat_generate_params_default(stelnettts_chat_generate_params* out);

// ---------------------------------------------------------------------------
// Session lifecycle
// ---------------------------------------------------------------------------
typedef struct stelnettts_chat_session stelnettts_chat_session;
typedef stelnettts_chat_session* stelnettts_chat_session_t;
...
STELNETTTS_CHAT_API stelnettts_chat_session_t stelnettts_chat_open(const char* model_path,
                                                             const stelnettts_chat_open_params* params,
...
// close it after that. Every binding shipped with StelnetTTS does exactly this,
...
STELNETTTS_CHAT_API void stelnettts_chat_close(stelnettts_chat_session_t s);
...
STELNETTTS_CHAT_API int32_t stelnettts_chat_reset(stelnettts_chat_session_t s, stelnettts_chat_error* err);
...
// `stelnettts_chat_string_free`. Returns NULL on failure (sets `err`).
STELNETTTS_CHAT_API char* stelnettts_chat_generate(stelnettts_chat_session_t s, const stelnettts_chat_message* messages,
...
typedef void (*stelnettts_chat_on_token)(const char* utf8_chunk, void* user);

STELNETTTS_CHAT_API int32_t stelnettts_chat_generate_stream(stelnettts_chat_session_t s,
                                                        const stelnettts_chat_message* messages, size_t n_messages,
                                                        const stelnettts_chat_generate_params* params,
...
// Cancellation
// ---------------------------------------------------------------------------
// Returned by `stelnettts_chat_generate_stream`, and written to `err.code` by
...
#define STELNETTTS_CHAT_ERR_ABORTED 40
...
// `user` is the pointer handed to `stelnettts_chat_set_abort_callback`.
//
...
typedef bool (*stelnettts_chat_abort_callback)(void* user);
...
// cancelling it.
//
// On abort `stelnettts_chat_generate` returns NULL and
// `stelnettts_chat_generate_stream` returns `STELNETTTS_CHAT_ERR_ABORTED`,
// having already delivered the partial text through `on_token`; both set
// `err.code` to `STELNETTTS_CHAT_ERR_ABORTED`. An abort flushes the session
...
// and no `stelnettts_chat_reset` is needed first.
STELNETTTS_CHAT_API void stelnettts_chat_set_abort_callback(stelnettts_chat_session_t s, stelnettts_chat_abort_callback cb,
                                                        void* user);
...
STELNETTTS_CHAT_API const char* stelnettts_chat_template_name(stelnettts_chat_session_t s);

// Returns the context window in tokens.
STELNETTTS_CHAT_API int32_t stelnettts_chat_n_ctx(stelnettts_chat_session_t s);
...
// straight against `stelnettts_chat_n_ctx` when sizing a context window.
//
...
// `stelnettts_chat_generate` decodes before it samples its first token, and
...
STELNETTTS_CHAT_API int32_t stelnettts_chat_count_tokens(stelnettts_chat_session_t s, const stelnettts_chat_message* messages,
...
// 0 if it could not be estimated. Mirrors the shape of stelnettts's
// existing `stelnettts_memory_estimate_*` family; lets StelnetWeaver's
// pre-flight guard short-circuit on low-RAM devices.
STELNETTTS_CHAT_API size_t stelnettts_chat_memory_estimate(const char* model_path, const stelnettts_chat_open_params* params,
                                                       stelnettts_chat_error* err);

// Free a malloc'd string returned by `stelnettts_chat_generate`.
STELNETTTS_CHAT_API void stelnettts_chat_string_free(char* s);

// ---------------------------------------------------------------------------
// AI disclosure (EU AI Act Art. 50(1))
// ---------------------------------------------------------------------------
// The canonical wording for "you are talking to an AI", the text counterpart of
// stelnettts_session_disclaimer_text() on the audio side. Show it at or before
...
STELNETTTS_CHAT_API const char* stelnettts_chat_ai_disclosure_text(void);

#ifdef __cplusplus
} // extern "C"
#endif

#endif // STELNETTTS_CHAT_H
