//! Integration tests for the StelnetASR Rust wrapper.
//!
//! Requires:
//!   - whisper-tiny model at STELNET_ASR_MODEL env var (or ../models/ggml-tiny.en.bin)
//!   - parakeet model at PARAKEET_MODEL env var (optional, skipped if absent)
//!   - GGUF chat model at STELNET_ASR_CHAT_MODEL env var (optional, skipped if
//!     absent; STELNET_ASR_CHAT_TEST_MODEL, which the C++ chat suite reads, is
...
fn whisper_model() -> String {
    std::env::var("STELNET_ASR_MODEL").unwrap_or_else(|_| {
...
fn assert_real_ctc_grid(lg: &stelnet_asr::CtcLogits) {
...
// ---- StelnetASR (whisper-only) tests ----

#[test]
#[ignore = "StelnetASR (whisper-direct) API crashes in Rust — use Session API instead"]
fn whisper_load_and_transcribe() {
    ...
    let model = stelnet_asr::StelnetASR::new(&model_path).expect("load whisper-tiny");
    let pcm = jfk_pcm();
...
fn whisper_timestamps_valid() {
    ...
    let model = stelnet_asr::StelnetASR::new(&model_path).unwrap();
...
fn session_whisper_auto_detect() {
    ...
    let sess = stelnet_asr::Session::open(&model_path).expect("session open whisper");
...
fn session_available_backends() {
    let backends = stelnet_asr::Session::available_backends();
...
fn session_parakeet_word_timestamps() {
    ...
    let sess = stelnet_asr::Session::open(&model_path).expect("session open parakeet");
...
fn session_omni_ctc_logits() {
    ...
    let sess = stelnet_asr::Session::open_with_backend(&model_path, "omniasr", 4)
...
fn session_canary_ctc_logits() {
    ...
    let sess = stelnet_asr::Session::open_with_backend(&model_path, "canary-ctc", 4)
...
fn session_wav2vec2_ctc_logits() {
    ...
    let sess = stelnet_asr::Session::open_with_backend(&model_path, "wav2vec2", 4)
...
fn assert_ctc_vocab_contract(sess: &stelnet_asr::Session, pcm: &[f32]) {
    let vocab = sess
        .ctc_vocab()
...
fn session_ctc_backend_no_speech_sentinel() {
    ...
    let sess = stelnet_asr::Session::open_with_backend(&model_path, backend, 4)
...
fn registry_lookup_parakeet() {
    let entry = stelnet_asr::registry_lookup("parakeet").expect("registry call");
    if let Some(e) = entry {
...
fn registry_default_bundle_omnivoice() {
    let bundle = stelnet_asr::registry_default_bundle("omnivoice")
        .expect("bundle call")
    ...
    assert_eq!(
        ...
        stelnet_asr::RegistryArtifactKind::Primary
    );
    assert_eq!(bundle.artifacts[0].filename, "omnivoice-f16.gguf");
    assert_eq!(
        bundle.artifacts[1].kind,
        stelnet_asr::RegistryArtifactKind::Companion
    );
...
#[test]
fn cache_dir_exists() {
    let dir = stelnet_asr::cache_dir(None).expect("cache_dir");
    if let Some(d) = dir {
        assert!(!d.is_empty());
    }
}
...
fn lcs_dedup_empty_inputs() {
    assert_eq!(stelnet_asr::lcs_dedup_prefix_count(&[], &[], 1), 0);
    assert_eq!(stelnet_asr::lcs_dedup_prefix_count(&[1, 2, 3], &[], 1), 0);
    assert_eq!(stelnet_asr::lcs_dedup_prefix_count(&[], &[1, 2, 3], 1), 0);
}
...
fn lcs_dedup_overlap() {
    ...
    let drop = stelnet_asr::lcs_dedup_prefix_count(&prev, &curr, 1);
...
fn titanet_cosine_sim_identical() {
    ...
    let sim = stelnet_asr::titanet_cosine_sim(&a, &b);
    assert!(
        (sim - 1.0).abs() < 1e-5,
...
fn kokoro_lang_helpers() {
    assert!(stelnet_asr::kokoro_lang_is_german("de"));
    assert!(stelnet_asr::kokoro_lang_is_german("de-DE"));
    assert!(stelnet_asr::kokoro_lang_is_german("de_AT"));
    // NOT "deu": the C predicate is documented as "de" followed by '\0', '-'
    // or '_' (src/kokoro.h), StelnetASR's language surface is ISO 639-1, and
    ...
    // stelnet_asr/build.rs — so it never failed out loud.
    assert!(!stelnet_asr::kokoro_lang_is_german("deu"));
    assert!(!stelnet_asr::kokoro_lang_is_german("en"));
    // "en" always has a native Kokoro voice
    assert!(stelnet_asr::kokoro_lang_has_native_voice("en"));
}

#[test]
fn speaker_db_missing_dir() {
    // Loading from a non-existent directory should return an error
    let result = stelnet_asr::SpeakerDB::load("/nonexistent/speaker_db_dir_12345");
...
fn vad_segments_null_model() {
    ...
    let result = stelnet_asr::vad_segments(
...
fn vad_slices_null_model() {
    let pcm = vec![0.0f32; 16000];
    let result = stelnet_asr::vad_slices(
        "/nonexistent/vad.gguf",
        &pcm,
        16000,
        0.5,
    ...
    // contains no speech" — stelnet_asr_compute_vad_slices has carried an
...
fn vad_slices_real_model_is_not_a_load_failure() {
    ...
    let Some(model) = std::env::var("STELNET_ASR_VAD_MODEL")
        .ok()
        .filter(|p| !p.is_empty() && Path::new(p).exists())
    else {
        eprintln!("SKIP: set STELNET_ASR_VAD_MODEL to a VAD model");
        return;
    };
    let pcm = vec![0.0f32; 16000];
    let result = stelnet_asr::vad_slices(&model, &pcm, 16000, 0.5, 250, 100, 30, 30.0, 1);
...
//
// Regression: StelnetAsrDiarizeOptsAbi was 24 bytes short of the C layout
...
// 48-byte layout (plus the layout test in stelnet_asr-sys) pin the fix.
// -------------------------------------------------------------------------
...
fn diarize_vad_turns_model_free() {
    ...
    let mut segs = vec![
        stelnet_asr::DiarizeSegment::new(0.0, 1.0),
        stelnet_asr::DiarizeSegment::new(2.0, 3.0), // 1 s gap > 600 ms turn gap
    ];
    let opts = stelnet_asr::DiarizeOptions::default();
    stelnet_asr::diarize_segments(&mut segs, &pcm, None, false, &opts)
...
fn diarize_foxnose_missing_model_errors() {
    ...
    let mut segs = vec![stelnet_asr::DiarizeSegment::new(0.0, 1.0)];
    let mut opts = stelnet_asr::DiarizeOptions::default();
    opts.method = stelnet_asr::DiarizeMethod::FoxNose;
    opts.foxnose_embedder_path = Some("/nonexistent/wespeaker.gguf".to_string());
    let err = stelnet_asr::diarize_segments(&mut segs, &pcm, None, false, &opts)
        .expect_err("foxnose with a missing embedder must fail, not crash");
    assert!(err.contains("load failed"), "unexpected error: {err}");
}

// =========================================================================
// Chat / LLM (stelnet_asr_chat.h)
// =========================================================================

use std::cell::Cell;
use std::panic::AssertUnwindSafe;

use stelnet_asr::{ChatGenerateOptions, ChatMessage, ChatOptions, ChatSession};
...
fn chat_model() -> Option<String> {
    let p = std::env::var("STELNET_ASR_CHAT_MODEL")
        .or_else(|_| std::env::var("STELNET_ASR_CHAT_TEST_MODEL"))
        .unwrap_or_else(|_| {
...
fn chat_open_reports_context_and_template() {
    let model_path = match chat_model() {
        Some(p) => p,
        None => {
            eprintln!("SKIP: chat model not found (set STELNET_ASR_CHAT_MODEL)");
            return;
        }
    };
...
fn chat_memory_estimate_covers_the_weights_and_scales_with_context() {
    ...
    let missing = ChatSession::memory_estimate(
        "/nonexistent/stelnet_asr-memory-estimate.gguf",
        &ChatOptions::default(),
    );
...
fn separate_returns_named_stems_at_the_models_own_rate() {
    ...
    let s = stelnet_asr::Session::open(&model).expect("open separation model");
