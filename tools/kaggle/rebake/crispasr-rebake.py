# %% [markdown]
# # StelnetTTS — reference re-bake (Kaggle, compute-only)
#
...
#
# This kernel:
#   1. Clones StelnetTTS main.
#   2. Sets `MODE=rebake` + `UPLOAD=0` (the latter is the change —
#      previously was 1).
#   3. Execs the canonical `tools/kaggle/stelnettts-regression.py`,
...
# `/Volumes/backups/ai/stelnettts-regression/rebake-out/` then
# `hf upload`s to `Xenna/stelnettts-regression-fixtures` using
...
REPO = WORK / "StelnetTTS-bootstrap"
...
os.environ.setdefault("STELNETTTS_REGRESSION_MODE", "rebake")
...
os.environ.setdefault("STELNETTTS_REGRESSION_UPLOAD", "1")
...
script = REPO / "tools" / "kaggle" / "stelnettts-regression.py"
