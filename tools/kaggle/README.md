# StelnetTTS — Kaggle notebook kernels

Files in this directory are the **Kaggle-side** half of StelnetTTS's
...
│                               (id = chr1str/stelnettts-regression-suite,
│                                code_file = stelnettts-regression.py).
├── stelnettts-regression.py      THE notebook script. Clones the repo,
│                               builds StelnetTTS, runs the regression
...
└── rebake/                     Sibling kernel for AUTO re-bake.
    ├── kernel-metadata.json    id = chr1str/stelnettts-auto-rebake-refs,
    │                           code_file = stelnettts-rebake.py.
    ├── stelnettts-rebake.py      Thin bootstrap: sets MODE=rebake +
...
latest `stelnettts-regression.py` from `main` on every run, so changes to
...
1. **Push the kernel** (uploads + triggers a first run):
   ...
   `https://www.kaggle.com/code/<your-kaggle-username>/stelnettts-regression-suite`.
...
2. **Wait for the first run to complete cleanly** (poll the URL or
   ...
   of GGUF + ref + builds StelnetTTS; ~10–15 min total on Kaggle's
   CPU runner.
...
4. **Enable the schedule** (the manual step the CLI can't do —
   ...
   - Validate kernel
     ([chr1str/stelnettts-regression-suite](https://www.kaggle.com/code/chr1str/stelnettts-regression-suite)):
     Settings → "Schedule a notebook run" → Weekly · Sun · 04:00 UTC.
   - Rebake kernel
     ([chr1str/stelnettts-auto-rebake-refs](https://www.kaggle.com/code/chr1str/stelnettts-auto-rebake-refs)):
...
Each weekly tick, Kaggle re-runs whatever `stelnettts-regression.py`
version was last pushed. The script itself does:

1. `git clone --recursive https://github.com/Cyna/StelnetTTS.git`
...
3. For each backend in the manifest, downloads the pinned-revision
   GGUF + pinned-revision reference dump, runs `stelnettts` +
   `stelnettts-diff`, compares against the manifest's thresholds.
...
"Add-ons → Variables" or by editing `tools/kaggle/stelnettts-regression.py`
before a push:

| Env var | Default | Effect |
|---|---|---|
| `STELNETTTS_REGRESSION_MODE` | `validate` | `validate` runs the contract check against pinned references. `rebake` regenerates references from the real Python source models. |
| `STELNETTTS_REGRESSION_UPLOAD` | `0` | When `rebake` mode AND `=1`, pushes the new ref.gguf files to `Xenna/stelnettts-regression-suite-fixtures`. Requires `HF_TOKEN` Kaggle secret with write scope. |
| `STELNETTTS_REGRESSION_BACKENDS` | `""` (all) | Comma-separated subset of backend names to process. Useful for one-off ad-hoc runs. |
| `STELNETTTS_REF` | `main` | Which StelnetTTS branch / commit / tag to test. |
| `STELNETTTS_REGRESSION_BUILD` | `cpu` | `cuda` to JIT-build with `-DGGML_CUDA=ON` for GPU-path validation (requires GPU notebook). |

## Re-baking references

Reference dumps in `Xenna/stelnettts-regression-suite-fixtures` are the
...
1. Run this notebook ad-hoc with `STELNETTTS_REGRESSION_MODE=rebake`
   and `STELNETTTS_REGRESSION_UPLOAD=1`. Set both via "Add-ons →
...
needs to change** — i.e. you're editing `stelnettts-regression.py`.
