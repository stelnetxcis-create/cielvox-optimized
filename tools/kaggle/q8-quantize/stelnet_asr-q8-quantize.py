...
STELNET_ASR.chmod(0o755)
QUANT.chmod(0o755)
assert STELNET_ASR.is_file() and QUANT.is_file(), "binaries missing"
...
def do_vibevoice():
    ...
    p = _run([str(STELNET_ASR), "-m", str(q8), "-f", str(WAV), "--backend", "vibevoice"], timeout=900)
...
def do_voxcpm2():
    ...
    p = _run([str(STELNET_ASR), "-m", str(q8), "--backend", "voxcpm2-tts",
    ...
    if not ok_synth:
        ...
        return False
    w = _get_whisper()
    pr = _run([str(STELNET_ASR), "-m", w, "-f", str(wav_out)], timeout=600)
