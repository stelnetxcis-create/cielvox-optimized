...
stelnettts resamples properly (no crude-resample confound). Runs the reporter's
default command under three configs and captures FULL transcripts + loop score:
  ...
  B. STELNETTTS_ARKASR_NO_CHUNK_CONTEXT=1   (windows WITHOUT seed prefix)
  C. STELNETTTS_ARKASR_NO_EOS_SUPPRESS=1    (baseline, no EOS-at-start suppression)
...
subprocess.check_call(["git","clone","--depth","1","--recursive",STELNETTTS_REPO := "https://github.com/Cyna/StelnetTTS.git", str(REPO)])
...
out = {"sha": SHA}
cfgs = [("A_default", {}),
        ("B_no_chunk_context", {"STELNETTTS_ARKASR_NO_CHUNK_CONTEXT": "1"}),
        ("C_no_eos_suppress", {"STELNETTTS_ARKASR_NO_EOS_SUPPRESS": "1"})]
for tag, env in cfgs:
    jstep(f"run_{tag}")
    res = run(tag, env)
