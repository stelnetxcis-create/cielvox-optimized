...
`stelnettts-windowed-attn-cuda.py` builds StelnetTTS with CUDA, downloads
`Xenna/parakeet-tdt-0.6b-v3-GGUF` (a `rel_pos_local_attn`-capable NeMo model),
concatenates `samples/jfk.wav` into a long single-pass clip (forced via
`STELNETTTS_PARAKEET_STREAM_THRESHOLD`), and runs three configs, each with an
`nvidia-smi` peak-memory poller:

| config | env | args |
|---|---|---|
| masked_full_local | `STELNETTTS_FC_WINDOWED_ATTN=0` | `--att-context 64,64` |
| windowed_local    | `STELNETTTS_FC_WINDOWED_ATTN=1` (default) | `--att-context 64,64` |
| full_attention    | — | — |
...
Upload as a script kernel and run. Tunables via env: `STELNETTTS_BRANCH` (default
`main`), `ATT_CONTEXT` (default `64,64`), `CLIP_REPEAT` (default `60` ≈ 660s,
T≈8k). Progress mirrors to `Xenna/stelnettts-kaggle-progress`.
