...
QUANT.chmod(0o755)
assert STELNET_ASR.is_file() and QUANT.is_file(), "binaries missing from tarball"
...
def validate_tts(backbone, codec, backend):
    ...
    with kh.build_heartbeat(f"synth.{Path(backbone).stem}"):
        p = _run([str(STELNET_ASR), "--backend", backend, "-m", str(backbone),
    ...
    with kh.build_heartbeat("roundtrip"):
        pr = _run([str(STELNET_ASR), "-m", w, "-f", str(wav)], timeout=900)
