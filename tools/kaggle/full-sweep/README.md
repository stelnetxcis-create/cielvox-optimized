# StelnetTTS Full Backend Sweep (Kaggle)
...
## Streaming + resume
- Per-backend result → `Xenna/stelnettts-kaggle-progress` dataset under
...
- On restart the kernel lists existing files for `<RUN>` and **skips** done
  backends. `STELNETTTS_SWEEP_RUN` (default `latest`) is the stable run tag —
  bump it for a clean run. `STELNETTTS_SWEEP_REPO` overrides the dataset (default `Xenna/stelnettts-kaggle-progress`).
