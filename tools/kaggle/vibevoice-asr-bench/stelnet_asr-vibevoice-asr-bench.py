...
STELNET_ASR = BIN_DIR / "stelnettts"
...
assert STELNET_ASR.is_file(), f"binary missing after download"
step("binary-download.done", binary=str(STELNET_ASR))
...
def transcribe(gguf: Path, label: str) -> dict:
    ...
    try:
        proc = subprocess.run(
            [str(STELNET_ASR), "-m", str(gguf), "-f", str(WAV),
             "--backend", "vibevoice"],
