#!/usr/bin/env python3
"""
Audit `stelnettts-diff` coverage across all registered reference backends.
...
Reports, per backend:
  ...
  - whether the C++ diff harness in examples/cli/stelnettts_diff_main.cpp has
...
ARCHIVE_ROOTS = [
    ...
    Path("/Volumes/backups/ai/stelnettts"),
    Path("/Volumes/backups/ai/stelnettts/ref-dumps"),
...
ARCHIVE_HINTS = {
    ...
    "cielvox2-tts": ["cielvox2-tts-ref.gguf"],
    "cielvox2-tts-codec": ["cielvox2-tts-codec-ref.gguf"],
    "cielvox2-tts-spk": ["cielvox2-tts-spk-ref.gguf"],
    "cielvox2-tts-cenc": ["cielvox2-tts-cenc-ref.gguf"],
...
def main() -> int:
    ...
    harness_branches = parse_harness_branches(REPO / "examples" / "cli" / "stelnettts_diff_main.cpp")
    ...
    lines: list[str] = []
    lines.append("# stelnettts-diff coverage")
    lines.append("")
    ...
    lines.append(f"- {n_with_harness} have a `stelnettts-diff` C++ harness branch")
    ...
    lines.append("`examples/cli/stelnettts_diff_main.cpp`. Then `tools/bootstrap_ref_env.sh")
    ...
    lines.append("`Xenna/chatterbox-GGUF/diff-harness-ref/chatterbox-ref.gguf`,")
    lines.append("`Xenna/canary-1b-v2-GGUF`, `Xenna/zonos-v0.1-transformer-GGUF`; the")
    lines.append("`orpheus` backend's ref is the SNAC decoder, archived in")
    lines.append("`Xenna/snac-24khz-GGUF`). Fetch one to run the harness without")
    ...
    if needs_archive:
        ...
        lines.append("")
        lines.append("These are wired into `stelnettts-diff` but skip every stage at runtime")
    ...
    if needs_harness:
        ...
        lines.append("")
        lines.append("Archive is on disk but `stelnettts-diff` has no `backend_name == \"<name>\"`")
        lines.append("branch yet — wire one into `examples/cli/stelnettts_diff_main.cpp` to start")
