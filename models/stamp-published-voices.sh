...
# survives being renamed, re-quantised or moved. Everything StelnetTTS has already
...
# `stelnettts --print-speaker-identity`, which runs the same resolution the
...
for f in "$SRC"/*.gguf; do
    ...
    if python "$STAMPER" --input "$f" --output "$OUT/$base" \
        ...
        --evidence "resolved by stelnettts --print-speaker-identity; see examples/cli/stelnettts_speaker_identity_models.h" \
