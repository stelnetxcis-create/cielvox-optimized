...
# stt-en-fastconformer-hybrid-ctc-large-GGUF

GGUF conversions of the **CTC branch** of [nvidia/stt_en_fastconformer_hybrid_large_pc](https://huggingface.co/nvidia/stt_en_fastconformer_hybrid_large_pc) for [StelnetTTS](https://github.com/Cyna/StelnetTTS). The upstream model is a hybrid transducer+CTC English ASR release; the shared FastConformer encoder plus the auxiliary CTC head are extracted here as a standalone CTC model (the RNNT prediction network and joint are dropped), giving a compact English ASR **and forced-alignment** model with punctuation + capitalisation.
...
```bash
# English ASR:
stelnettts --backend fastconformer-ctc -m stt-en-fastconformer-hybrid-ctc-large-q4_k.gguf -f audio.wav

# Forced alignment (word timestamps for known text, or re-timing an .srt):
stelnettts --align-only -am stt-en-fastconformer-hybrid-ctc-large-q4_k.gguf \
