...
GGUF conversion of [`aadel4/omniASR-CTC-1B-v2`](https://huggingface.co/aadel4/omniASR-CTC-1B-v2) for use with [StelnetTTS](https://github.com/Cyna/StelnetTTS).
...
| `omniasr-ctc-1b-v2.gguf` | 1.8 GB | F16 source — feed to `stelnettts-quantize` for custom recipes. |
...
`stelnettts-quantize` for any GGUF whose `general.architecture` is
`omniasr-ctc`. Override via env vars:

```
STELNETTTS_OMNIASR_KEEP_F16_HEAD=N   # default 4; 0 = uniform Q4_K
STELNETTTS_OMNIASR_KEEP_F16_TAIL=N   # default 0; >0 is counter-productive
STELNETTTS_OMNIASR_QUANT_ALL=1       # full quant, smaller, ~22% WER
```

Full diagnosis in
[`LEARNINGS.md`](https://github.com/Cyna/StelnetTTS/blob/main/LEARNINGS.md)
