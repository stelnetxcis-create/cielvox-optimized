...
This GGUF release uses the **always-on robust path**: the Mega-ASR LoRA is merged into the Qwen3-ASR-1.7B weights offline, then converted to a normal Qwen3-ASR GGUF. StelnetTTS runs it through the standard `qwen3` backend; no Python, PEFT, safetensors, or runtime LoRA switching is needed.
...
| `mega-asr-router.gguf` | 2.8 MB | Converted upstream audio-quality router weights; auxiliary artifact, not used by current StelnetTTS inference |
...
```bash
huggingface-cli download Xenna/mega-asr-GGUF \
...
Older StelnetTTS builds can use the same file with the canonical backend name:
...
Current StelnetTTS `main` registers `mega-asr` as an alias for the Qwen3-ASR runtime:

```bash
./build/bin/stelnettts --backend mega-asr -m auto your-audio.wav
```
...
The small `mega-asr-router.gguf` file is included so the router weights are available in GGUF form, but StelnetTTS does not yet use it to switch between base and LoRA models at runtime. Dynamic routing would require either loading both Qwen3-ASR-1.7B variants or adding runtime LoRA switching.

The router's frontend is a `LogMelSpectrogram` using 16 kHz audio, 80 Slaney mel bins, `n_fft=400`, `hop_length=160`, `win_length=400`, `log10(clamp(mel, 1e-10))`, then `(x + 4) / 4`. StelnetTTS already has the reusable pieces for this in `src/core/mel.h` (`build_slaney_fb` + `core_mel::compute()`), so if dynamic routing is added later it should reuse that shared mel path rather than adding another spectrogram implementation.
...
3. Converted the merged Hugging Face checkpoint with StelnetTTS's existing Qwen3-ASR converter.
4. Quantised the F16 GGUF with StelnetTTS's `stelnettts-quantize`.
