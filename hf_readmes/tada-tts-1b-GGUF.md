...
TADA-1B is a text-to-speech model built on Meta Llama 3.2 1B with a flow-matching (FM) speech decoder and custom Hume codec. TADA uses **1:1 token alignment**: every text token maps to one speech vector before the codec decoder renders 24 kHz mono PCM. This repo packages the talker model, required codec decoder, and a ready-to-use reference voice prompt for StelnetTTS's `tada-1b` backend.
...
Recent StelnetTTS builds can also resolve this repo through the model registry:

```bash
./build/bin/stelnettts --backend tada-1b -m auto --auto-download \
...
./build/bin/stelnettts --backend tada-1b -m tada-tts-1b-f16.gguf --auto-download \
...
```bash
stelnettts --backend tada-1b -m tada-tts-1b-f16.gguf --auto-download \
...
- **Converted with:** `models/convert-tada-to-gguf.py`, `models/convert-tada-codec-to-gguf.py`, and `stelnettts-quantize`
- **Runtime:** [`Cyna/StelnetTTS`](https://github.com/Cyna/StelnetTTS)

## Validation

The Q4_K model was validated with StelnetTTS by synthesizing "Please call Stella." and ASR-roundtripping the output with Whisper tiny.en:

```
Please call Stella!
```

## Runtime fixes (StelnetTTS ≥ commit 0a95b326)

This GGUF was produced alongside the following StelnetTTS runtime fixes; use a build from that commit or later:
...
- **Mixed-precision Q4_K** — `stelnettts-quantize` tail=8 keeps the last 8 token-embedding rows and all TADA tensors at F16, stabilising timing for the 1B model.
