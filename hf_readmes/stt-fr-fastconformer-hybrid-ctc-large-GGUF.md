...
```bash
# French ASR:
stelnettts --backend fastconformer-ctc -m stt-fr-fastconformer-hybrid-ctc-large-q4_k.gguf -f audio.wav

# Forced alignment (word timestamps for known text, or re-timing an .srt):
stelnettts --align-only -am stt-fr-fastconformer-hybrid-ctc-large-q4_k.gguf \
