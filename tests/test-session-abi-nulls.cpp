...
//   2. stelnettts_diarize_segments_abi argument validation and the model-free
...
//      layout static_asserts in stelnettts_c_api.cpp fail the build before
...
int stelnettts_session_input_sample_rate(stelnettts_session* s);
int stelnettts_session_output_sample_rate(stelnettts_session* s);
int stelnettts_session_input_channels(stelnettts_session* s);
int stelnettts_session_output_channels(stelnettts_session* s);
int stelnettts_session_set_pcm_sample_rate(stelnettts_session* s, int rate);

// Hand-written mirror of the append-only ABI structs, byte-identical to the
// ones in stelnettts_c_api.cpp (pinned there by static_asserts) and to the
// mirrors in stelnettts-sys, flutter and the Go cgo preamble.
...
int stelnettts_diarize_segments_abi(const float* left_pcm, const float* right_pcm, int32_t n_samples, int32_t is_stereo,
...
TEST_CASE("NULL session: every #332 getter returns its no-data value", "[unit][abi]") {
    CHECK(stelnettts_session_input_sample_rate(nullptr) == 0);
    CHECK(stelnettts_session_output_sample_rate(nullptr) == 0);
    CHECK(stelnettts_session_input_channels(nullptr) == 0);
    CHECK(stelnettts_session_output_channels(nullptr) == 0);
    CHECK(stelnettts_session_set_pcm_sample_rate(nullptr, 16000) == -1);
}
...
TEST_CASE("diarize ABI rejects invalid arguments with -1", "[unit][diarize]") {
    ...
    CHECK(stelnettts_diarize_segments_abi(nullptr, nullptr, 16000, 0, &seg, 1, &opts) == -1);
    CHECK(stelnettts_diarize_segments_abi(pcm.data(), nullptr, 16000, 0, nullptr, 1, &opts) == -1);
    CHECK(stelnettts_diarize_segments_abi(pcm.data(), nullptr, 16000, 0, &seg, 0, &opts) == -1);
    CHECK(stelnettts_diarize_segments_abi(pcm.data(), nullptr, 16000, 0, &seg, 1, nullptr) == -1);

    diarize_opts_abi bad_method = default_opts(5); // one past FoxNose
    CHECK(stelnettts_diarize_segments_abi(pcm.data(), nullptr, 16000, 0, &seg, 1, &bad_method) == -1);
    bad_method.method = -1;
    CHECK(stelnettts_diarize_segments_abi(pcm.data(), nullptr, 16000, 0, &seg, 1, &bad_method) == -1);
}
...
TEST_CASE("VadTurns alternates speakers across a >600 ms gap", "[unit][diarize]") {
    ...
    REQUIRE(stelnettts_diarize_segments_abi(pcm.data(), nullptr, (int32_t)pcm.size(), 0, segs, 2, &opts) == 0);
...
TEST_CASE("Energy picks the louder channel; ties stay unlabelled", "[unit][diarize]") {
    ...
    REQUIRE(stelnettts_diarize_segments_abi(left.data(), right.data(), sr * 3, 1, segs, 3, &opts) == 0);
    CHECK(segs[0].speaker == 0);
...
TEST_CASE("Energy on mono input succeeds but labels nothing", "[unit][diarize]") {
    ...
    REQUIRE(stelnettts_diarize_segments_abi(pcm.data(), nullptr, 16000, 0, &seg, 1, &opts) == 0);
    CHECK(seg.speaker == -1);
}
...
TEST_CASE("slice_t0 maps absolute segment times into the buffer", "[unit][diarize]") {
    ...
    REQUIRE(stelnettts_diarize_segments_abi(left.data(), right.data(), sr * 2, 1, segs, 2, &opts) == 0);
    CHECK(segs[0].speaker == 0);
    CHECK(segs[1].speaker == 1);
}
...
TEST_CASE("FoxNose with a missing embedder fails with 1, not a crash", "[unit][diarize]") {
    ...
    CHECK(stelnettts_diarize_segments_abi(pcm.data(), nullptr, 16000, 0, &seg, 1, &opts) == 1);
}
