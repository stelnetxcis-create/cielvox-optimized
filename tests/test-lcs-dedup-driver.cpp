...
// driver that wraps `stelnettts_lcs::lcs_dedup_prefix_count` and physically
// removes duplicate leading tokens / words / segment-text from adjacent
// overlap-save chunks (issue #89 / #114 follow-up).
//
// These tests exercise the driver on synthetic `stelnettts_segment`
...
using stelnettts_lcs::apply_lcs_chunk_dedup;
using stelnettts_lcs::collect_tail_token_ids;
using stelnettts_lcs::drop_leading_tokens;
...
stelnettts_segment make_seg(const std::vector<std::pair<int32_t, const char*>>& toks, int64_t t0_cs = 0) {
