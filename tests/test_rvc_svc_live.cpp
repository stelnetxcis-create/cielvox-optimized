...
//
// Requires STELNETTTS_MODEL_RVC pointing at an rvc GGUF. SKIPs cleanly when unset.
//
// This drives the SESSION C ABI, not rvc_svc.h directly: RVC has no CLI verb
// (its input is ContentVec features, which StelnetTTS does not produce), so the
...
stelnettts_session* open_session(const char* model) {
    return stelnettts_session_open_explicit(model, "rvc-svc", 2);
}
...
TEST_CASE("rvc: session opens and reports checkpoint geometry", "[integration][rvc]") {
    const char* model = std::getenv("STELNETTTS_MODEL_RVC");
    if (!model || !*model)
        SKIP("STELNETTTS_MODEL_RVC not set");
    ...
    const int cd = stelnettts_session_convert_content_dim(s);
    REQUIRE((cd == 256 || cd == 768));
    REQUIRE(stelnettts_session_convert_n_speakers(s) > 0);
    // The output rate is a property of the checkpoint, not a constant.
    const int sr = stelnettts_session_convert_sample_rate(s);
...
TEST_CASE("rvc: replayed noise makes conversion deterministic", "[integration][rvc]") {
    ...
    const int n1 = stelnettts_session_convert(s, content.data(), kFrames, f0.data(), 0, nz.data(), ns.data());
    REQUIRE(n1 > 0);
    int c1 = 0;
    const float* p1 = stelnettts_session_convert_audio(s, &c1);
    REQUIRE(p1 != nullptr);
    REQUIRE(c1 == n1);
    const std::vector<float> first(p1, p1 + c1);

    const int n2 = stelnettts_session_convert(s, content.data(), kFrames, f0.data(), 0, nz.data(), ns.data());
    REQUIRE(n2 == n1);
    int c2 = 0;
    const float* p2 = stelnettts_session_convert_audio(s, &c2);
    REQUIRE(c2 == c1);
...
TEST_CASE("rvc: NULL noise draws fresh each call", "[integration][rvc]") {
    ...
    REQUIRE(s != nullptr);
    std::vector<float> content, f0;
    make_inputs(stelnettts_session_convert_content_dim(s), content, f0);

    const int n1 = stelnettts_session_convert(s, content.data(), kFrames, f0.data(), 0, nullptr, nullptr);
    REQUIRE(n1 > 0);
    int c1 = 0;
    const float* p1 = stelnettts_session_convert_audio(s, &c1);
    const std::vector<float> first(p1, p1 + c1);

    REQUIRE(stelnettts_session_convert(s, content.data(), kFrames, f0.data(), 0, nullptr, nullptr) == n1);
    int c2 = 0;
    const float* p2 = stelnettts_session_convert_audio(s, &c2);
...
TEST_CASE("rvc: rejects bad arguments", "[integration][rvc]") {
    ...
    REQUIRE(stelnettts_session_convert(s, nullptr, kFrames, f0.data(), 0, nullptr, nullptr) == -1);
    REQUIRE(stelnettts_session_convert(s, content.data(), 0, f0.data(), 0, nullptr, nullptr) == -1);
    REQUIRE(stelnettts_session_convert(s, content.data(), kFrames, nullptr, 0, nullptr, nullptr) == -1);
    REQUIRE(stelnettts_session_convert(nullptr, content.data(), kFrames, f0.data(), 0, nullptr, nullptr) == -1);
    ...
    const int n_spk = stelnettts_session_convert_n_speakers(s);
    REQUIRE(stelnettts_session_convert(s, content.data(), kFrames, f0.data(), n_spk, nullptr, nullptr) == -1);
    REQUIRE(stelnettts_session_convert(s, content.data(), kFrames, f0.data(), -1, nullptr, nullptr) == -1);

    REQUIRE(stelnettts_session_convert_audio(nullptr, nullptr) == nullptr);
    REQUIRE(stelnettts_session_convert_content_dim(nullptr) == 0);
