...
int main(int argc, char** argv) {
    ...
    float* out = nullptr;
    int n_out = 0;
    bool ok = openvoice2_convert(ctx, pcm.data(), (int)pcm.size(), sr, ref_pcm.data(), (int)ref_pcm.size(), ref_sr,
                                 &out, &n_out);
    ...
    if (ok && out && n_out > 0) {
        ...
        // Write WAV for ASR roundtrip
        {
            ...
            if (wf) {
                ...
                uint32_t sr_out = 22050;
                fwrite(&sr_out, 4, 1, wf);
