...
TEST_CASE("splitter empty / whitespace-only input → empty vector", "[unit][chunking]") {
    REQUIRE(stelnettts_tts_split_sentences("").empty());
    REQUIRE(stelnettts_tts_split_sentences("   ").empty());
    REQUIRE(stelnettts_tts_split_sentences("\n\t  \r\n").empty());
}

TEST_CASE("splitter single sentence stays whole (with terminator)", "[unit][chunking]") {
    auto out = stelnettts_tts_split_sentences("Hello there.");
    REQUIRE(out.size() == 1);
...
TEST_CASE("splitter handles all three ASCII terminators", "[unit][chunking]") {
    auto out = stelnettts_tts_split_sentences("Wait. Really? Yes!");
    REQUIRE(out.size() == 3);
    REQUIRE(out[0] == "Wait.");
...
TEST_CASE("splitter trims whitespace from each chunk", "[unit][chunking]") {
    auto out = stelnettts_tts_split_sentences("  First.    Second.   Third.  ");
    REQUIRE(out.size() == 3);
    REQUIRE(out[0] == "First.");
...
TEST_CASE("splitter does NOT split decimal numbers", "[unit][chunking]") {
    ...
    // it must stay intact.
    auto out = stelnettts_tts_split_sentences("The value is 1.5 today.");
    REQUIRE(out.size() == 1);
...
TEST_CASE("splitter over-splits English abbreviations (documented)", "[unit][chunking]") {
    ...
    auto out = stelnettts_tts_split_sentences("Mr. Smith said hi.");
    REQUIRE(out.size() == 2);
    REQUIRE(out[0] == "Mr.");
...
TEST_CASE("splitter handles CJK ideographic full stop U+3002", "[unit][chunking]") {
    ...
    auto out = stelnettts_tts_split_sentences(ja);
    REQUIRE(out.size() == 2);
...
TEST_CASE("splitter handles Devanagari danda U+0964", "[unit][chunking]") {
    ...
    auto out = stelnettts_tts_split_sentences(hi);
    REQUIRE(out.size() == 2);
...
TEST_CASE("splitter max_chars fallback splits run-on input", "[unit][chunking]") {
    ...
    auto out = stelnettts_tts_split_sentences(run_on, /*max_chars=*/20);
    REQUIRE(out.size() >= 2);
...
TEST_CASE("splitter pathological no-whitespace blob still splits at max_chars", "[unit][chunking]") {
    ...
    auto out = stelnettts_tts_split_sentences(blob, /*max_chars=*/50);
    REQUIRE(out.size() >= 4);
    for (const auto& chunk : out)
...
TEST_CASE("server TTS policy keeps VibeVoice voice cloning single-shot", "[unit][chunking]") {
    auto out = stelnettts_tts_plan_chunks_for_backend("First sentence. Second sentence. Third sentence.", "vibevoice");
    REQUIRE(out.size() == 1);
...
TEST_CASE("server TTS policy keeps ALL vibevoice* TTS variants single-shot", "[unit][chunking]") {
    ...
    for (const char* name : {"vibevoice-tts", "vibevoice-1.5b", "vibevoice-tts-1.5b", "vibevoice-tts-base"}) {
        CAPTURE(name);
        auto out = stelnettts_tts_plan_chunks_for_backend(text, name);
        REQUIRE(out.size() == 1);
        REQUIRE(out[0] == text);
    }
}

TEST_CASE("server TTS policy keeps cielvox2-tts variants single-shot", "[unit][chunking]") {
    const char* text = "First sentence. Second sentence. Third sentence.";
    for (const char* name : {"cielvox2-tts", "cielvox2-tts-1.7b-base", "cielvox2-tts-customvoice", "cielvox2-tts-voicedesign"}) {
        CAPTURE(name);
        auto out = stelnettts_tts_plan_chunks_for_backend(text, name);
        REQUIRE(out.size() == 1);
        REQUIRE(out[0] == text);
...
TEST_CASE("server TTS policy keeps dots-tts single-shot", "[unit][chunking]") {
    const char* text = "First sentence. Second sentence. Third sentence.";
    auto out = stelnettts_tts_plan_chunks_for_backend(text, "dots-tts");
    REQUIRE(out.size() == 1);
    REQUIRE(out[0] == text);
}

TEST_CASE("server TTS policy chunks sentence-safe backends", "[unit][chunking]") {
    auto out = stelnettts_tts_plan_chunks_for_backend("First sentence. Second sentence.", "kokoro");
    REQUIRE(out.size() == 2);
...
TEST_CASE("server TTS policy preserves whitespace-only requests", "[unit][chunking]") {
    auto out = stelnettts_tts_plan_chunks_for_backend("   ", "cielvox2-tts");
    REQUIRE(out.size() == 1);
    REQUIRE(out[0] == "   ");
}

// ──────────────────────────────────────────────────────────────────────────
// stelnettts_tts_concat_with_silence
// ──────────────────────────────────────────────────────────────────────────

TEST_CASE("concat empty input → empty output", "[unit][chunking]") {
    auto out = stelnettts_tts_concat_with_silence({}, 4800);
    REQUIRE(out.empty());
}

TEST_CASE("concat single chunk has no silence pad", "[unit][chunking]") {
    std::vector<std::vector<float>> chunks = {{0.1f, 0.2f, 0.3f}};
    auto out = stelnettts_tts_concat_with_silence(chunks, 4800);
    REQUIRE(out.size() == 3);
    REQUIRE(out[0] == 0.1f);
    REQUIRE(out[1] == 0.2f);
    REQUIRE(out[2] == 0.3f);
...
TEST_CASE("concat two chunks inserts silence between but not at boundaries", "[unit][chunking]") {
    ...
    auto out = stelnettts_tts_concat_with_silence(chunks, 5);
...
TEST_CASE("concat with silence_samples=0 just concatenates", "[unit][chunking]") {
    ...
    auto out = stelnettts_tts_concat_with_silence(chunks, 0);
    REQUIRE(out.size() == 3);
    REQUIRE(out[0] == 1.0f);
    REQUIRE(out[1] == 2.0f);
    REQUIRE(out[2] == 3.0f);
...
TEST_CASE("concat negative silence_samples treated as zero", "[unit][chunking]") {
    ...
    auto out = stelnettts_tts_concat_with_silence(chunks, -100);
    REQUIRE(out.size() == 2);
}
