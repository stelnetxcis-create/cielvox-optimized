...
//
// Coverage: stelnettts_session_detected_language,
// stelnettts_session_result_segment_no_speech_prob, stelnettts_session_n_vocab,
// stelnettts_session_token_text — declared in include/stelnettts_session.h.
...
TEST_CASE("session introspection: detected_language null-session → -1", "[unit][introspection]") {
    char buf[8] = {0};
    REQUIRE(stelnettts_session_detected_language(nullptr, buf, sizeof(buf)) == -1);
}
...
TEST_CASE("session introspection: detected_language null-buffer → -1", "[unit][introspection]") {
    ...
    REQUIRE(stelnettts_session_detected_language(nullptr, nullptr, 8) == -1);
}

TEST_CASE("session introspection: detected_language non-positive capacity → -1", "[unit][introspection]") {
    char buf[8] = {0};
    REQUIRE(stelnettts_session_detected_language(nullptr, buf, 0) == -1);
    REQUIRE(stelnettts_session_detected_language(nullptr, buf, -1) == -1);
}
...
TEST_CASE("session introspection: no_speech_prob null-result → -1.0 sentinel", "[unit][introspection]") {
    ...
    REQUIRE(stelnettts_session_result_segment_no_speech_prob(nullptr, 0) == -1.0f);
}

TEST_CASE("session introspection: no_speech_prob negative segment index → -1.0", "[unit][introspection]") {
    REQUIRE(stelnettts_session_result_segment_no_speech_prob(nullptr, -1) == -1.0f);
}
...
TEST_CASE("session introspection: n_vocab null-session → 0", "[unit][introspection]") {
    ...
    REQUIRE(stelnettts_session_n_vocab(nullptr) == 0);
}
...
TEST_CASE("session introspection: token_text null-session → non-null empty string", "[unit][introspection]") {
    ...
    const char* t = stelnettts_session_token_text(nullptr, 0);
    REQUIRE(t != nullptr);
...
TEST_CASE("session introspection: token_text negative id → empty string", "[unit][introspection]") {
    ...
    const char* t = stelnettts_session_token_text(nullptr, -1);
    REQUIRE(t != nullptr);
    REQUIRE(std::strcmp(t, "") == 0);
}
