...
// `stelnettts_lcs_dedup_prefix_count` is exported from `libstelnettts` and
...
TEST_CASE("C ABI: empty inputs return 0", "[unit][lcs][c-abi]") {
    ...
    REQUIRE(stelnettts_lcs_dedup_prefix_count(nullptr, 0, Y.data(), (int)Y.size(), 1) == 0);
    REQUIRE(stelnettts_lcs_dedup_prefix_count(Y.data(), (int)Y.size(), nullptr, 0, 1) == 0);
}
...
TEST_CASE("C ABI: perfect alignment matches the C++ algorithm", "[unit][lcs][c-abi]") {
    ...
    REQUIRE(stelnettts_lcs_dedup_prefix_count(X.data(), (int)X.size(), Y.data(), (int)Y.size(), 1) == 3);
}
...
TEST_CASE("C ABI: min_lcs_length gates short matches", "[unit][lcs][c-abi]") {
    ...
    REQUIRE(stelnettts_lcs_dedup_prefix_count(X.data(), (int)X.size(), Y.data(), (int)Y.size(), 1) == 1);
    REQUIRE(stelnettts_lcs_dedup_prefix_count(X.data(), (int)X.size(), Y.data(), (int)Y.size(), 2) == 0);
}
...
TEST_CASE("C ABI: min_lcs_length <= 0 falls back to the NeMo default", "[unit][lcs][c-abi]") {
    ...
    const int slice = stelnettts_lcs_dedup_prefix_count(X.data(), (int)X.size(), Y.data(), (int)Y.size(), 0);
    REQUIRE(slice == 2);
}
...
TEST_CASE("C ABI: leftmost match is preferred", "[unit][lcs][c-abi]") {
    ...
    REQUIRE(stelnettts_lcs_dedup_prefix_count(X.data(), (int)X.size(), Y.data(), (int)Y.size(), 1) == 2);
}
