...
#
# StelnetTTS #341. The published libstelnettts-linux-x86_64-hip bundle had
...
cat > "$WORK/ext.c" <<'EOF'
int stelnettts_test_ext(void) { return 7; }
EOF
# verify-lib-bundle.sh insists the concrete libstelnettts dlopens AND exports the
...
cat > "$WORK/main.c" <<'EOF'
int stelnettts_test_ext(void);
int stelnettts_test_main(void) { return stelnettts_test_ext(); }
void *stelnettts_session_open_explicit(void) { return 0; }
void stelnettts_session_close(void *s) { (void)s; }
EOF
cat > "$WORK/ggml.c" <<'EOF'
int stelnettts_test_ggml(void) { return 1; }
EOF
...
cc -shared -fPIC -o "$priv/libstelnetttstestext.so" "$WORK/ext.c"
cc -shared -fPIC -Wl,-soname,libstelnettts.so.1 \
   -o "$OUT/src/libstelnettts.so.1.0.0" "$WORK/main.c" \
   -L"$priv" -lstelnetttstestext -Wl,-rpath,"$priv"
ln -s libstelnettts.so.1.0.0 "$OUT/src/libstelnettts.so.1"
ln -s libstelnettts.so.1     "$OUT/src/libstelnettts.so"
cc -shared -fPIC -o "$OUT/ggml/src/libstelnetttstestggml.so" "$WORK/ggml.c"

bash "$PKG" "$OUT" > "$WORK/pkg.log" 2>&1 || { cat "$WORK/pkg.log"; fail "packaging failed"; }

# 1. the external dependency is in the bundle
[ -f "$OUT/lib/libstelnetttstestext.so" ] || {
    cat "$WORK/pkg.log"
    fail "libstelnetttstestext.so was not bundled — the RUNPATH-only dependency was dropped"; }
...
[ -f "$OUT/lib/libstelnettts.so.1.0.0" ] || fail "the real library did not reach lib/"
[ -e "$OUT/lib/libstelnettts.so.1" ] || fail "SOVERSION alias did not survive the flatten"
[ -f "$OUT/lib/libstelnetttstestggml.so" ] || fail "the ggml library did not reach lib/"
...
import ctypes, sys
lib = ctypes.CDLL(sys.argv[1] + "/lib/libstelnettts.so.1")
lib.stelnettts_test_main.restype = ctypes.c_int
v = lib.stelnettts_test_main()
if v != 7:
...
#    declares STELNETTTS_BUNDLE_EXTRA_RPATH would have had it erased one line
#    later — the same shape as the defect this file exists for.
OUT2="$WORK/bundle2"
host="$WORK/opt-fake-toolkit/lib"
mkdir -p "$OUT2/src" "$OUT2/ggml/src" "$host"
cc -shared -fPIC -o "$host/libstelnetttstesthost.so" "$WORK/ext.c"
cc -shared -fPIC -Wl,-soname,libstelnettts.so.1 \
   -o "$OUT2/src/libstelnettts.so.1.0.0" "$WORK/main.c" \
   -L"$host" -lstelnetttstesthost -Wl,-rpath,"$host"
ln -s libstelnettts.so.1.0.0 "$OUT2/src/libstelnettts.so.1"
cc -shared -fPIC -o "$OUT2/ggml/src/libstelnetttstestggml.so" "$WORK/ggml.c"

STELNETTTS_BUNDLE_HOST_DIRS="$WORK/opt-fake-toolkit" \
STELNETTTS_BUNDLE_EXTRA_RPATH="$host" \
    bash "$PKG" "$OUT2" > "$WORK/pkg2.log" 2>&1 || {
    cat "$WORK/pkg2.log"; fail "packaging with a host toolkit failed"; }

if [ -f "$OUT2/lib/libstelnetttstesthost.so" ]; then
    fail "a host-toolkit library must not be copied into the bundle"
fi
rp="$(patchelf --print-rpath "$OUT2/lib/libstelnettts.so.1.0.0")"
case "$rp" in
