...
//
// The two helpers in `examples/cli/stelnettts_stream_partial_decode.h`
...
#include "../examples/cli/stelnettts_stream_partial_decode.h"
...
TEST_CASE("partial-decode: interval = ms * sample_rate / 1000", "[unit][stream-partial-decode]") {
    REQUIRE(stelnettts_stream_partial_decode_interval_samples(1000, /*step_ms=*/500, kSR) == 16000);
    REQUIRE(stelnettts_stream_partial_decode_interval_samples(750, /*step_ms=*/500, kSR) == 12000);
    REQUIRE(stelnettts_stream_partial_decode_interval_samples(500, /*step_ms=*/500, kSR) == 8000);
}
...
TEST_CASE("partial-decode: ms=0 falls back to step_ms (the documented default)", "[unit][stream-partial-decode]") {
    ...
    REQUIRE(stelnettts_stream_partial_decode_interval_samples(0, /*step_ms=*/500, kSR) == 8000);
    REQUIRE(stelnettts_stream_partial_decode_interval_samples(0, /*step_ms=*/3000, kSR) == 48000);
    REQUIRE(stelnettts_stream_partial_decode_interval_samples(0, /*step_ms=*/1, kSR) == 16);
}
...
TEST_CASE("partial-decode: degenerate intervals return 0 safely", "[unit][stream-partial-decode]") {
    ...
    REQUIRE(stelnettts_stream_partial_decode_interval_samples(0, /*step_ms=*/0, kSR) == 0);
    REQUIRE(stelnettts_stream_partial_decode_interval_samples(-100, /*step_ms=*/-1, kSR) == 0);
    REQUIRE(stelnettts_stream_partial_decode_interval_samples(500, /*step_ms=*/500, /*sample_rate=*/0) == 0);
}
...
TEST_CASE("partial-decode: non-JSON streaming never throttles", "[unit][stream-partial-decode]") {
    ...
    REQUIRE(stelnettts_stream_partial_decode_allow(/*stream_json=*/false,
...
TEST_CASE("partial-decode: first step always decodes (last_partial_decode_sample == -1)",
    ...
    REQUIRE(stelnettts_stream_partial_decode_allow(true, -1, false, 0, 16000));
    REQUIRE(stelnettts_stream_partial_decode_allow(true, -1, false, 0, 0)); // even with degenerate interval
}
...
TEST_CASE("partial-decode: final-silence-due bypasses the throttle", "[unit][stream-partial-decode]") {
    ...
    REQUIRE(stelnettts_stream_partial_decode_allow(/*stream_json=*/true,
...
TEST_CASE("partial-decode: interval <= 0 is treated as 'always allow'", "[unit][stream-partial-decode]") {
    ...
    // tolerates anyway.
    REQUIRE(stelnettts_stream_partial_decode_allow(true, 100000, false, 100001, 0));
    REQUIRE(stelnettts_stream_partial_decode_allow(true, 100000, false, 100001, -1));
}
...
TEST_CASE("partial-decode: gate trips only after `interval_samples` of new audio", "[unit][stream-partial-decode]") {
    ...
    REQUIRE_FALSE(stelnettts_stream_partial_decode_allow(true, last, false, last + 1, interval));
    REQUIRE_FALSE(stelnettts_stream_partial_decode_allow(true, last, false, last + interval - 1, interval));

    // delta == interval: allowed.
    REQUIRE(stelnettts_stream_partial_decode_allow(true, last, false, last + interval, interval));

    // delta > interval: allowed.
    REQUIRE(stelnettts_stream_partial_decode_allow(true, last, false, last + interval + 1, interval));
}
...
TEST_CASE("partial-decode: 500 ms step + 1000 ms throttle -> decode every other step",
          "[unit][stream-partial-decode]") {
    const int64_t interval = stelnettts_stream_partial_decode_interval_samples(/*ms=*/1000, /*step_ms=*/500, kSR);
    ...
    for (int step = 0; step < 8; step++) {
        cumulative += 8000;
        const bool allow = stelnettts_stream_partial_decode_allow(
...
TEST_CASE("partial-decode: default 0 + step 500 still throttles every step (no-op default)",
          "[unit][stream-partial-decode]") {
    const int64_t interval = stelnettts_stream_partial_decode_interval_samples(/*ms=*/0, /*step_ms=*/500, kSR);
    ...
    for (int step = 0; step < 6; step++) {
        cumulative += 8000;
        if (stelnettts_stream_partial_decode_allow(true, last_decode, false, cumulative, interval)) {
            decoded++;
            last_decode = cumulative;
        }
    }
