...
int main(int argc, char** argv) {
    ...
    cielvox2_tts_context* ctx = cielvox2_tts_init_from_file(model, p);
    if (!ctx) {
        fprintf(stderr, "init failed\n");
        return 2;
    }
    if (cielvox2_tts_load_voice_pack(ctx, voice) != 0) {
    ...
    int n = 0;
    int32_t* codes = cielvox2_tts_synthesize_codes(ctx, text, &n);
    if (!codes) {
