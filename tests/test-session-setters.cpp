...
TEST_CASE("session setter: set_return_logits null-handle -> -1", "[unit][setters]") {
    REQUIRE(stelnettts_session_set_return_logits(nullptr, 1) == -1);
}

TEST_CASE("session setter: set_alt_n null-handle → -1", "[unit][setters]") {
    REQUIRE(stelnettts_session_set_alt_n(nullptr, 3) == -1);
}

// ─── string / multi-param setters ─────────────────────────────────────────

TEST_CASE("session setter: set_ask null-handle → -1", "[unit][setters]") {
    REQUIRE(stelnettts_session_set_ask(nullptr, "hello") == -1);
}

TEST_CASE("session setter: set_grammar_text null-handle → -1", "[unit][setters]") {
    REQUIRE(stelnettts_session_set_grammar_text(nullptr, nullptr, nullptr, 100.0f) == -1);
}

TEST_CASE("session setter: set_fallback_thresholds null-handle → -1", "[unit][setters]") {
    REQUIRE(stelnettts_session_set_fallback_thresholds(nullptr, 2.4f, -1.0f, 0.6f, 0.2f) == -1);
}

TEST_CASE("session setter: set_whisper_decode_extras null-handle → -1", "[unit][setters]") {
    REQUIRE(stelnettts_session_set_whisper_decode_extras(nullptr, 0, nullptr, 0) == -1);
}

// ─── last_synth_error ─────────────────────────────────────────────────────

TEST_CASE("session: last_synth_error null-handle → empty string", "[unit][setters]") {
    const char* err = stelnettts_session_last_synth_error(nullptr);
    REQUIRE(err != nullptr);
    REQUIRE(err[0] == '\0');
}

// ─── #282: lazy dynamic-backend load path ─────────────────────────────────
// g_open_use_gpu_tls defaults to true, so stelnettts_session_open_explicit runs
...
TEST_CASE("session open: GPU-default open of missing model loads plugins and returns null safely", "[unit][setters]") {
    stelnettts_session* s1 = stelnettts_session_open_explicit("/nonexistent/stelnettts-282.gguf", "whisper", 1);
    REQUIRE(s1 == nullptr);
    stelnettts_session* s2 = stelnettts_session_open_explicit("/nonexistent/stelnettts-282.gguf", "whisper", 1);
    REQUIRE(s2 == nullptr);
}
