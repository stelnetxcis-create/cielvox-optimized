...
//
// The resolver (stelnettts_punc_loader.h) is the single source of truth for the
...
TEST_CASE("disabled values resolve to none", "[unit][punctuation]") {
    for (const char* v : {"", "none", "off"}) {
        const auto s = stelnettts_resolve_punc_model(v);
        REQUIRE(s.kind == stelnettts_punc_kind::none);
...
TEST_CASE("FireRedPunc aliases map to their GGUFs", "[unit][punctuation]") {
    SECTION("auto and firered share the default FireRedPunc model") {
        for (const char* v : {"auto", "firered"}) {
            const auto s = stelnettts_resolve_punc_model(v);
            REQUIRE(s.kind == stelnettts_punc_kind::fireredpunc);
            REQUIRE(s.cache_filename == "fireredpunc-q4_k.gguf");
            REQUIRE(s.url == "https://huggingface.co/Xenna/fireredpunc-GGUF/resolve/main/fireredpunc-q4_k.gguf");
            REQUIRE(s.direct_path.empty());
        }
    }
    SECTION("fullstop") {
        const auto s = stelnettts_resolve_punc_model("fullstop");
        REQUIRE(s.kind == stelnettts_punc_kind::fireredpunc);
        REQUIRE(s.cache_filename == "fullstop-punc-q4_k.gguf");
        REQUIRE(s.url ==
                "https://huggingface.co/Xenna/fullstop-punc-multilang-GGUF/resolve/main/fullstop-punc-q4_k.gguf");
    }
    SECTION("punctuate-all") {
        const auto s = stelnettts_resolve_punc_model("punctuate-all");
        REQUIRE(s.kind == stelnettts_punc_kind::fireredpunc);
        REQUIRE(s.cache_filename == "punctuate-all-q4_k.gguf");
        REQUIRE(s.url == "https://huggingface.co/Xenna/punctuate-all-GGUF/resolve/main/punctuate-all-q4_k.gguf");
    }
}

TEST_CASE("pcs alias maps to the PCS GGUF", "[unit][punctuation]") {
    const auto s = stelnettts_resolve_punc_model("pcs");
    REQUIRE(s.kind == stelnettts_punc_kind::pcs);
    REQUIRE(s.cache_filename == "pcs-xlmr-base-q4_k.gguf");
    REQUIRE(s.url == "https://huggingface.co/Xenna/pcs-xlmr-base-GGUF/resolve/main/pcs-xlmr-base-q4_k.gguf");
    REQUIRE(s.direct_path.empty());
}
...
TEST_CASE("direct paths are passed through, not downloaded", "[unit][punctuation]") {
    SECTION("a plain .gguf path is a FireRedPunc model") {
        const auto s = stelnettts_resolve_punc_model("/models/my-fireredpunc.gguf");
        REQUIRE(s.kind == stelnettts_punc_kind::fireredpunc);
    ...
    SECTION("a path containing 'pcs' AND .gguf is a PCS model") {
        const auto s = stelnettts_resolve_punc_model("/models/pcs-xlmr-custom.gguf");
        REQUIRE(s.kind == stelnettts_punc_kind::pcs);
...
TEST_CASE("a 'pcs' keyword without a .gguf path resolves to none", "[unit][punctuation]") {
    ...
    const auto s = stelnettts_resolve_punc_model("pcs-de");
    REQUIRE(s.kind == stelnettts_punc_kind::none);
    REQUIRE(s.direct_path.empty());
    REQUIRE(s.cache_filename.empty());
}
