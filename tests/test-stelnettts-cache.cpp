// test-stelnettts-cache.cpp — unit tests for stelnettts_cache helpers.
//
...
static std::string make_temp_dir() {
    ...
    if (!base.empty() && (base.back() == '\\' || base.back() == '/'))
        base.pop_back();
    std::string dir = base + "/stelnettts_unit_" + std::to_string(_getpid());
    _mkdir(dir.c_str());
    return dir;
}
...
static std::string make_temp_dir() {
    ...
    std::string pattern = base + "/stelnettts_unit_XXXXXX";
...
TEST_CASE("file_present: nonexistent path returns false", "[unit]") {
    REQUIRE_FALSE(stelnettts_cache::file_present("/this/absolutely/does/not/exist.bin"));
}

TEST_CASE("file_present: empty file (0-byte zombie) returns false", "[unit]") {
    const std::string tmp = make_temp_dir() + "/empty.bin";
    write_file(tmp, "");
    REQUIRE_FALSE(stelnettts_cache::file_present(tmp));
    remove_file(tmp);
}
...
TEST_CASE("file_present: non-empty file returns true", "[unit]") {
    ...
    REQUIRE(stelnettts_cache::file_present(tmp));
    remove_file(tmp);
}
...
TEST_CASE("dir: override path is returned unchanged", "[unit]") {
    ...
    const std::string result = stelnettts_cache::dir(override_dir);
...
TEST_CASE("dir: override creates the leaf directory", "[unit]") {
    ...
    stelnettts_cache::dir(override_dir);
...
TEST_CASE("dir: empty override returns a non-empty platform default", "[unit]") {
    REQUIRE_FALSE(stelnettts_cache::dir("").empty());
}

TEST_CASE("dir: environment and CLI destination precedence", "[unit]") {
    const char* old_cache = std::getenv("STELNETTTS_CACHE_DIR");
    const char* old_models = std::getenv("STELNETTTS_MODELS_DIR");
    ...
    set_env("STELNETTTS_CACHE_DIR", cache_env.c_str());
    set_env("STELNETTTS_MODELS_DIR", models_env.c_str());
    REQUIRE(stelnettts_cache::dir("") == cache_env);
    REQUIRE(stelnettts_cache::dir(cli_override) == cli_override);

    set_env("STELNETTTS_CACHE_DIR", nullptr);
    REQUIRE(stelnettts_cache::dir("") == models_env);

    set_env("STELNETTTS_CACHE_DIR", had_cache ? saved_cache.c_str() : nullptr);
    set_env("STELNETTTS_MODELS_DIR", had_models ? saved_models.c_str() : nullptr);
...
TEST_CASE("ensure_cached_file: returns existing file path without fetching", "[unit]") {
    ...
    const std::string result =
        stelnettts_cache::ensure_cached_file(filename,
...
TEST_CASE("ensure_cached_file: matching source sidecar reuses the cached file", "[unit]") {
    ...
    const std::string url = "https://huggingface.co/Xenna/moonshine-tiny-GGUF/resolve/main/tokenizer.bin";
    ...
    const std::string result = stelnettts_cache::ensure_cached_file(filename, url, /*quiet=*/true, "test", cache_dir);
    REQUIRE(result == path);
...
TEST_CASE("ensure_cached_file: cross-repo same-basename file is NOT reused (issue #250)", "[unit]") {
    ...
    const std::string filename = "tokenizer.bin";
    const std::string path = cache_dir + "/" + filename;
    const std::string url_en = "https://huggingface.co/Xenna/moonshine-tiny-GGUF/resolve/main/tokenizer.bin";
    const std::string url_de = "https://huggingface.co/Xenna/moonshine-tiny-de-fidoriel-GGUF/resolve/main/tokenizer.bin";
    ...
    const std::string result =
        stelnettts_cache::ensure_cached_file(filename, "file:///stelnettts-test-nonexistent/tokenizer.bin",
                                           /*quiet=*/true, "test", cache_dir);
    ...
    REQUIRE(stelnettts_cache::file_present(path));
    remove_file(path + ".src");
    remove_file(path);
    (void)url_de;
}
...
TEST_CASE("ensure_cached_file: wrong-source hit is skipped in favor of a correct-source dir", "[unit]") {
    ...
    // (STELNETTTS_MODELS_DIR) holds the DE tokenizer with a matching sidecar.
    ...
    set_env("STELNETTTS_MODELS_DIR", models_dir.c_str());
    const std::string result = stelnettts_cache::ensure_cached_file(filename, url_de, /*quiet=*/true, "test",
...
TEST_CASE("ensure_cached_file: no-sidecar legacy flat file is still trusted (backward compat)", "[unit]") {
    ...
    const std::string result = stelnettts_cache::ensure_cached_file(filename, "https://example.invalid/codec.gguf",
                                                                  /*quiet=*/true, "test", cache_dir);
...
TEST_CASE("ensure_cached_file: a leftover .part temp is never treated as a cache hit", "[unit]") {
    ...
    REQUIRE(stelnettts_cache::probe_cached_file("model.gguf", cache_dir).empty());
