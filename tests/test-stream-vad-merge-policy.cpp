...
#include "../src/stelnettts_vad.h"

#include <vector>

static stelnettts_audio_slice slice(int start, int end) {
    return stelnettts_audio_slice{start, end, start / 160, end / 160};
}

static stelnettts_vad_options stream_opts(int merge_gap_ms, int final_silence_ms = 800) {
    stelnettts_vad_options opts;
    opts.post_merge_policy = stelnettts_vad_post_merge_policy::streaming_json;
...
TEST_CASE("stream-vad-merge-policy: effective gap clamps below final silence", "[unit][stream-json]") {
    REQUIRE(stelnettts_stream_vad_effective_merge_gap_ms(250, 800) == 250);
    REQUIRE(stelnettts_stream_vad_effective_merge_gap_ms(500, 300) == 299);
    REQUIRE(stelnettts_stream_vad_effective_merge_gap_ms(250, 1) == 0);
    REQUIRE(stelnettts_stream_vad_effective_merge_gap_ms(250, 0) == 250);
    REQUIRE(stelnettts_stream_vad_effective_merge_gap_ms(0, 800) == 0);
}

TEST_CASE("stream-vad-merge-policy: close jitter gap merges below threshold", "[unit][stream-json]") {
    const std::vector<stelnettts_audio_slice> in = {
        slice(0, 16000),
        slice(16000 + 3999, 32000),
    };
    const auto out = stelnettts_post_merge_vad_slices(in, 16000, stream_opts(250));
...
TEST_CASE("stream-vad-merge-policy: hard final silence boundary is not hidden", "[unit][stream-json]") {
    ...
    const auto out = stelnettts_post_merge_vad_slices(in, 16000, stream_opts(500, 300));
...
TEST_CASE("stream-vad-merge-policy: offline policy still merges short previous slice", "[unit][stream-json]") {
    ...
    stelnettts_vad_options opts;
    const auto out = stelnettts_post_merge_vad_slices(in, 16000, opts);
...
TEST_CASE("stream-vad-merge-policy: final silence disabled uses configured gap", "[unit][stream-json]") {
    ...
    const auto out = stelnettts_post_merge_vad_slices(in, 16000, stream_opts(500, 0));
