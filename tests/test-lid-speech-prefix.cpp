...
TEST_CASE("LID speech prefix compacts VAD slices", "[unit][lid]") {
    ...
    const std::vector<stelnettts_audio_slice> slices = {{10, 20, 0, 0}, {40, 50, 0, 0}};

    const auto got = stelnettts_lid_speech_prefix(pcm, slices);
...
TEST_CASE("LID speech prefix clamps slices and caps at 15 seconds", "[unit][lid]") {
    ...
    const std::vector<stelnettts_audio_slice> slices = {{-10, 16 * 16000 + 10, 0, 0}};
...
TEST_CASE("LID speech prefix ignores empty slices", "[unit][lid]") {
    ...
    const std::vector<stelnettts_audio_slice> slices = {{4, 4, 0, 0}, {20, 30, 0, 0}};
    CHECK(stelnettts_lid_speech_prefix(pcm, slices).empty());
}
