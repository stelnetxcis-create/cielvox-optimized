...
TEST_CASE("paraformer: init and free", "[paraformer][.live]") {
    std::string model = get_env("STELNETTTS_PARAFORMER_MODEL");
    if (model.empty()) {
...
TEST_CASE("paraformer: Chinese transcription matches reference", "[paraformer][.live]") {
    ...
    std::string audio = get_env("STELNETTTS_PARAFORMER_AUDIO_ZH");
    if (audio.empty()) {
...
TEST_CASE("paraformer: English JFK matches reference", "[paraformer][.live]") {
    ...
    std::string audio = get_env("STELNETTTS_PARAFORMER_AUDIO_EN", "samples/jfk.wav");
...
TEST_CASE("paraformer: Q4_K transcript matches F16", "[paraformer][.live]") {
    std::string model = get_env("STELNETTTS_PARAFORMER_MODEL_Q4K");
    if (model.empty()) {
