...
# The CLI binary.
_CLI = Path("build/bin/stelnettts")
if not _CLI.exists():
    _CLI = Path("build-shared/bin/stelnettts")
...
def _run_cv(speaker: str, instruct: str | None, text: str, extra_flags: list[str] | None = None) -> subprocess.CompletedProcess:
    """Run stelnettts with cielvox2-tts CustomVoice and return the completed process."""
    cmd = [
        str(_CLI),
        "-m", str(_CV_MODEL),
...
class TestCVStyleNullGuard(unittest.TestCase):
    ...
    def test_cli_passes_instruct_to_cv(self):
        ...
        backend_cpp = Path("examples/cli/stelnettts_backend_cielvox2_tts.cpp")
...
class TestCVStyleIntegration(unittest.TestCase):
    ...
    def test_fast_speech_fewer_frames_than_slow(self):
        ...
        the 'stelnettts: n_codes' line from stderr to extract the frame count.
