...
int main() {
    ...
    const char* whisper_model = env_or("STELNETTTS_MODEL_WHISPER", STELNETTTS_MODEL_PATH);
    ...
    for (float thold : thresholds) {
        struct whisper_full_params wparams = whisper_full_default_params(STELNETTTS_SAMPLING_GREEDY);
        wparams.vad = true;
