...
//
// Pure CPU, no model load. Exercises stelnettts_segments_to_diarized_json() and
// stelnettts_make_disp_segments() with synthetic segments.
...
// ---------------------------------------------------------------------------
static stelnettts_segment make_seg(const std::string& text, int64_t t0, int64_t t1, const std::string& speaker = {},
                                 const std::vector<stelnettts_word>& words = {}) {
...
TEST_CASE("diarized_json: empty segments produce valid JSON", "[unit][diarized_json]") {
    ...
    std::string out = stelnettts_segments_to_diarized_json(segs, 0.0, "en", "transcribe", 0.0f);
...
TEST_CASE("diarized_json: speaker labels normalised to letters", "[unit][diarized_json]") {
    ...
    std::string out = stelnettts_segments_to_diarized_json(segs, 3.0, "en", "transcribe", 0.0f);
...
TEST_CASE("diarized_json: empty speaker defaults to A", "[unit][diarized_json]") {
    ...
    std::string out = stelnettts_segments_to_diarized_json(segs, 5.0, "en", "transcribe", 0.0f);
    REQUIRE(out.find("\"speaker\": \"A\"") != std::string::npos);
}
...
TEST_CASE("diarized_json: timestamps in seconds", "[unit][diarized_json]") {
    ...
    std::string out = stelnettts_segments_to_diarized_json(segs, 2.61, "en", "transcribe", 0.0f);
...
TEST_CASE("diarized_json: type field present", "[unit][diarized_json]") {
    ...
    std::string out = stelnettts_segments_to_diarized_json(segs, 1.0, "en", "transcribe", 0.0f);
...
TEST_CASE("diarized_json: full text field present", "[unit][diarized_json]") {
    ...
    std::string out = stelnettts_segments_to_diarized_json(segs, 2.0, "en", "transcribe", 0.0f);
...
TEST_CASE("diarized_json: word-level timestamps included when present", "[unit][diarized_json]") {
    std::vector<stelnettts_word> words = {{"Hello", 0, 50}, {"there", 50, 100}};
    std::vector<stelnettts_segment> segs;
...
TEST_CASE("make_disp_segments: max_len splits when words present", "[unit][max-len]") {
    ...
    std::vector<stelnettts_word> words = {
        {"The", 0, 25},
        {"quick", 25, 50},
        {"brown", 50, 75},
        {"fox", 75, 100},
    ...
    auto disp = stelnettts_make_disp_segments(segs, 10);
    REQUIRE(disp.size() >= 2);
...
TEST_CASE("make_disp_segments: max_len=0 does not split", "[unit][max-len]") {
    std::vector<stelnettts_word> words = {{"Hello", 0, 50}, {"world", 50, 100}};
    std::vector<stelnettts_segment> segs;
    segs.push_back(make_seg("Hello world", 0, 100, "", words));

    auto disp = stelnettts_make_disp_segments(segs, 0);
    REQUIRE(disp.size() == 1);
}
...
TEST_CASE("make_disp_segments: max_len=1 gives one segment per word", "[unit][max-len]") {
    ...
    auto disp = stelnettts_make_disp_segments(segs, 1);
    REQUIRE(disp.size() == 2);
...
TEST_CASE("diarized_json: chunk_id emitted only when >= 0", "[unit][diarized_json][issue-292]") {
    ...
    b.chunk_id = 2;
    std::vector<stelnettts_segment> segs = {a, b};
    std::string out = stelnettts_segments_to_diarized_json(segs, 130.0, "en", "transcribe", 0.0f);
...
TEST_CASE("diarized_json: single-pass run emits NO chunk_id", "[unit][diarized_json][issue-292]") {
    ...
    std::vector<stelnettts_segment> segs = {s};
    std::string out = stelnettts_segments_to_diarized_json(segs, 5.0, "en", "transcribe", 0.0f);
...
TEST_CASE("verbose_json: speaker label is emitted when diarization produced one", "[unit][diarized_json][issue-326]") {
    std::vector<stelnettts_segment> segs = {make_seg("hello there", 0, 100, "(speaker 0) "),
                                          make_seg("and hello to you", 100, 250, "(speaker 1) ")};
    std::string out = stelnettts_segments_to_openai_verbose_json(segs, 2.5, "en", "transcribe", 0.0f);
...
TEST_CASE("verbose_json: no speaker key when diarization did not run", "[unit][diarized_json][issue-326]") {
    ...
    std::vector<stelnettts_segment> segs = {make_seg("hello there", 0, 100), make_seg("nobody labelled this", 100, 250)};
    std::string out = stelnettts_segments_to_openai_verbose_json(segs, 2.5, "en", "transcribe", 0.0f);
    REQUIRE(out.find("\"speaker\"") == std::string::npos);
...
TEST_CASE("verbose_json: a partially diarized run labels only what was labelled", "[unit][diarized_json][issue-326]") {
    ...
    // named "".
    std::vector<stelnettts_segment> segs = {make_seg("labelled", 0, 100, "(speaker 0) "),
