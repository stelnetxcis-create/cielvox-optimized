// test-crisp-audio-mapping-released.cpp — stelnet_audio must not outlive its
// weight mapping either.
//
// stelnet_audio's load_model() takes its backend buffer from
// core_gguf::load_weights and moves it into stelnet_audio_context::model_buf.
// stelnet_audio_free() destroyed that handle with ggml_backend_buffer_free(),
...
// path: src/cielvox2_asr.cpp and src/higgs_stt.cpp both link stelnet_audio, and
...
// the consumer: that stelnet_audio's handle actually reaches
...
//     guard the weaker claim that stelnet_audio releases at all.
//
...
// embedding stelnet_audio precomputes at init costs nothing; d_model must stay
...
// A GGUF stelnet_audio loads successfully: hparams under the default
// `stelnet_audio.` key prefix plus the full tensor set for a one-layer tower.
void write_tower_gguf(const std::string& path) {
    ...
    gguf_set_val_str(g, "general.architecture", "stelnet_audio_test");
    ...
    gguf_set_val_u32(g, "stelnet_audio.d_model", kDModel);
    gguf_set_val_u32(g, "stelnet_audio.n_layers", kLayers);
    gguf_set_val_u32(g, "stelnet_audio.n_heads", 1);
    gguf_set_val_u32(g, "stelnet_audio.head_dim", kDModel);
    gguf_set_val_u32(g, "stelnet_audio.ff_dim", kDModel);
    gguf_set_val_u32(g, "stelnet_audio.conv_channels", kDModel);
    gguf_set_val_u32(g, "stelnet_audio.max_source_pos", 8);
    gguf_set_val_u32(g, "stelnet_audio.output_dim", kDModel);
...
// A GGUF the loader maps but stelnet_audio then rejects: valid hparams, no tower
// tensors. load_model() returns false after load_weights() has already mapped
// the file, and stelnet_audio_init_from_file cleans up through stelnet_audio_free.
...
// True when this machine has the GPU device stelnet_audio_init_from_file would
...
stelnet_audio_params quiet_params(bool use_gpu) {
...
TEST_CASE("stelnet_audio_free unmaps the zero-copy GPU path's weight region", "[unit][crisp-audio-mapping]") {
    ...
    Fixture fx("stelnettts_test_stelnet_audio_gpu.gguf", /*loadable=*/true);
    REQUIRE(count_regions_backed_by(fx.abs) == 0);

    stelnet_audio_params p = quiet_params(/*use_gpu=*/true);
    stelnet_audio_context* ctx = stelnet_audio_init_from_file(fx.rel.c_str(), &p);
    REQUIRE(ctx != nullptr);
    // Positive control: stelnet_audio really took the zero-copy branch. Without
...
TEST_CASE("repeated stelnet_audio init/free cycles leave no mapping behind", "[unit][crisp-audio-mapping]") {
    if (!region_probe_available()) {
        SUCCEED("region enumeration unavailable on this platform");
        return;
    }
    ...
    Fixture fx("stelnettts_test_stelnet_audio_loop.gguf", /*loadable=*/true);
    stelnet_audio_params p = quiet_params(/*use_gpu=*/true);
...
TEST_CASE("a stelnet_audio load rejected after mapping leaves no region", "[unit][crisp-audio-mapping]") {
    if (!region_probe_available()) {
        SUCCEED("region enumeration unavailable on this platform");
        return;
    }
    ...
    // what fail, one step later. stelnet_audio_init_from_file's own cleanup call
    // is the release path here, and it is the same one the success case uses.
    Fixture fx("stelnettts_test_stelnet_audio_reject.gguf", /*loadable=*/false);
    REQUIRE(count_regions_backed_by(fx.abs) == 0);

    stelnet_audio_params p = quiet_params(/*use_gpu=*/true);
    REQUIRE(stelnet_audio_init_from_file(fx.rel.c_str(), &p) == nullptr);
    REQUIRE(count_regions_backed_by(fx.abs) == 0);
...
TEST_CASE("stelnet_audio_free unmaps the CPU mmap path's weight region", "[unit][crisp-audio-mapping]") {
    if (!region_probe_available()) {
        SUCCEED("region enumeration unavailable on this platform");
        return;
    }
    ...
    Fixture fx("stelnettts_test_stelnet_audio_cpu.gguf", /*loadable=*/true);
    REQUIRE(count_regions_backed_by(fx.abs) == 0);

    stelnet_audio_params p = quiet_params(/*use_gpu=*/false);
    stelnet_audio_context* ctx = stelnet_audio_init_from_file(fx.rel.c_str(), &p);
    REQUIRE(ctx != nullptr);
    REQUIRE(count_regions_backed_by(fx.abs) >= 1);
