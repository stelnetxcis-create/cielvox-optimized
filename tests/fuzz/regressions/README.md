...
build, revert the fix and run the single input directly — `libstelnettts` is a
shared library, so rebuild it, not just the harness, or you will test the new
code with an old-looking binary:

```
./build-fuzz/bin/stelnettts-fuzz-audio tests/fuzz/regressions/<file>
```
