# Fuzzing `stelnettts_audio_load`

`stelnettts_audio_load()` runs an attacker-controllable file through the whole
...
cmake -B build-fuzz \
  -DSTELNETTTS_FUZZ=ON \
  -DSTELNETTTS_SANITIZE_ADDRESS=ON \
  -DSTELNETTTS_SANITIZE_UNDEFINED=ON \
  -DSTELNETTTS_BUILD_TESTS=ON \
  -DCMAKE_BUILD_TYPE=Debug
cmake --build build-fuzz --target stelnettts-fuzz-audio
```

`-DSTELNETTTS_FUZZ=ON` adds `-fsanitize=fuzzer-no-link` (coverage) to every TU;
...
```bash
mkdir -p /tmp-nonsmall/stelnettts-fuzz-corpus   # any writable dir off a real fs
cp samples/jfk.wav samples/jfk.au samples/jfk.amr samples/jfk.webm \
   samples/jfk-vorbis.webm samples/jfk.m4a /tmp-nonsmall/stelnettts-fuzz-corpus/

./build-fuzz/bin/stelnettts-fuzz-audio -max_len=1048576 /tmp-nonsmall/stelnettts-fuzz-corpus
```

A crash writes `crash-<hash>`; reproduce with
`./build-fuzz/bin/stelnettts-fuzz-audio crash-<hash>`. The harness writes each
input to `stelnettts_fuzz_input.bin` in the cwd and caps size at 8 MB (the
parsers have their own 500 MB caps; the cap just keeps the fuzzer itself fast).

## What it exercises

Every format-detection + fallback path in `stelnettts_audio_load`, i.e. the
`stelnettts_au_decode` / `stelnettts_amr_decode` / `stelnettts_webm_decode` /
`stelnettts_m4a_decode` parsers plus miniaudio's WAV/MP3/FLAC/OGG handling.

## GGUF metadata harness (`stelnettts-fuzz-gguf`)
...
./build-fuzz/bin/stelnettts-fuzz-gguf -max_len=2097152 gguf-corpus
```
...
## Text tokenizer harness (`stelnettts-fuzz-tokenizer`)
...
./build-fuzz/bin/stelnettts-fuzz-tokenizer -max_len=65536 tok-corpus
```
