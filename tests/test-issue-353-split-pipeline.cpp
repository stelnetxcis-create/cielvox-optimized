...
//
//  - the original STELNETTTS_SLICE_PIPELINE override re-derived the decision and
...
//    STELNETTTS_PARAKEET_VAD_SLICE_CAP=12 --vad: SIGABRT on
...
stelnettts_split::Inputs good() {
    stelnettts_split::Inputs in;
...
TEST_CASE("issue #353: a plain multi-slice run pipelines", "[unit][split-pipeline][issue-353]") {
    REQUIRE(stelnettts_split::available(good()));
    REQUIRE(stelnettts_split::enabled(good(), nullptr));
}
...
TEST_CASE("issue #353: every hazard refuses the pipeline", "[unit][split-pipeline][issue-353]") {
    SECTION("a single slice has nothing to overlap") {
        ...
        REQUIRE_FALSE(stelnettts_split::available(in));
    }
...
TEST_CASE("issue #353: STELNETTTS_SLICE_PIPELINE=0 always turns it off", "[unit][split-pipeline][issue-353]") {
    REQUIRE_FALSE(stelnettts_split::enabled(good(), "0"));
}

TEST_CASE("issue #353: STELNETTTS_SLICE_PIPELINE=1 cannot override a safety condition",
    ...
    for (int which = 0; which < 3; which++) {
        ...
        REQUIRE_FALSE(stelnettts_split::enabled(in, "1"));
    }
    // It may still enable a run that is already safe.
    REQUIRE(stelnettts_split::enabled(good(), "1"));
}
