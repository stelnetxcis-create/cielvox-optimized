...
//
// The helpers in `examples/cli/stelnettts_stream_punc.h` are pure
...
#include "../examples/cli/stelnettts_stream_punc.h"
...
TEST_CASE("stream-punc: partials_enabled is true ONLY for 'partial'", "[unit][stream-punc]") {
    REQUIRE_FALSE(stelnettts_stream_punc_partials_enabled("off"));
    REQUIRE_FALSE(stelnettts_stream_punc_partials_enabled("final"));
    REQUIRE(stelnettts_stream_punc_partials_enabled("partial"));
}

TEST_CASE("stream-punc: finals_enabled is true for 'final' and 'partial', false for 'off'", "[unit][stream-punc]") {
    REQUIRE_FALSE(stelnettts_stream_punc_finals_enabled("off"));
    REQUIRE(stelnettts_stream_punc_finals_enabled("final"));
    REQUIRE(stelnettts_stream_punc_finals_enabled("partial"));
}

TEST_CASE("stream-punc: mode_valid accepts the three documented values", "[unit][stream-punc]") {
    REQUIRE(stelnettts_stream_punc_mode_valid("off"));
    REQUIRE(stelnettts_stream_punc_mode_valid("final"));
    REQUIRE(stelnettts_stream_punc_mode_valid("partial"));
}

TEST_CASE("stream-punc: mode_valid rejects unknown / case-mismatched / empty values", "[unit][stream-punc]") {
    REQUIRE_FALSE(stelnettts_stream_punc_mode_valid(""));
    REQUIRE_FALSE(stelnettts_stream_punc_mode_valid("FINAL")); // case-sensitive
    REQUIRE_FALSE(stelnettts_stream_punc_mode_valid("Partial"));
    REQUIRE_FALSE(stelnettts_stream_punc_mode_valid("on"));
    REQUIRE_FALSE(stelnettts_stream_punc_mode_valid("yes"));
    REQUIRE_FALSE(stelnettts_stream_punc_mode_valid("partials"));
}
...
TEST_CASE("stream-punc: default 'final' implies finals=on, partials=off", "[unit][stream-punc]") {
    ...
    REQUIRE(stelnettts_stream_punc_mode_valid(default_mode));
    REQUIRE(stelnettts_stream_punc_finals_enabled(default_mode));
    REQUIRE_FALSE(stelnettts_stream_punc_partials_enabled(default_mode));
}
