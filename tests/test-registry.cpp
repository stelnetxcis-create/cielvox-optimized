// test-registry.cpp — unit tests for stelnettts_model_registry.
//
...
TEST_CASE("registry: lookup known backend returns valid entry", "[unit][registry]") {
    ...
    bool found = stelnettts_registry_lookup("whisper", e);
    REQUIRE(found);
...
TEST_CASE("registry: default bundle resolves aliases and includes extras", "[unit][registry]") {
    StelnetAsrRegistryBundle bundle;
    REQUIRE(stelnettts_registry_default_bundle("cosyvoice3", bundle));
    REQUIRE(bundle.backend == "cosyvoice3-tts");
    REQUIRE(bundle.artifacts.size() == 6);
    REQUIRE(bundle.artifacts[0].kind == StelnetAsrRegistryArtifactKind::Primary);
    REQUIRE(bundle.artifacts[1].kind == StelnetAsrRegistryArtifactKind::Companion);
    for (size_t i = 2; i < bundle.artifacts.size(); ++i) {
        REQUIRE(bundle.artifacts[i].kind == StelnetAsrRegistryArtifactKind::Extra);
...
TEST_CASE("registry: default bundle preserves license metadata", "[unit][registry]") {
    StelnetAsrRegistryBundle bundle;
    REQUIRE(stelnettts_registry_default_bundle("voxtral-tts", bundle));
...
TEST_CASE("registry: non-permissive supported weights carry policy metadata", "[unit][registry][license]") {
    ...
    for (const auto& c : cases) {
        StelnetAsrRegistryEntry e;
        REQUIRE(stelnettts_registry_lookup(c.backend, e));
        REQUIRE(std::string(e.license).find(c.needle) != std::string::npos);
        REQUIRE(stelnettts_license_requires_acceptance(e.license) == c.requires_acceptance);
    }
}

TEST_CASE("registry: default bundle rejects unknown backends", "[unit][registry]") {
    StelnetAsrRegistryBundle bundle;
    REQUIRE_FALSE(stelnettts_registry_default_bundle("nonexistent-backend-xyz", bundle));
}

TEST_CASE("registry: every default bundle starts with its lookup result", "[unit][registry]") {
    for (int i = 0; i < stelnettts_registry_count(); ++i) {
        StelnetAsrRegistryEntry entry;
        REQUIRE(stelnettts_registry_get_at(i, entry));

        StelnetAsrRegistryEntry canonical;
        REQUIRE(stelnettts_registry_lookup(entry.backend, canonical));
        StelnetAsrRegistryBundle bundle;
        REQUIRE(stelnettts_registry_default_bundle(entry.backend, bundle));
        REQUIRE_FALSE(bundle.artifacts.empty());
        REQUIRE(bundle.backend == canonical.backend);
        REQUIRE(bundle.license == canonical.license);
        REQUIRE(bundle.requires_license_acceptance == stelnettts_license_requires_acceptance(canonical.license));
        REQUIRE(bundle.artifacts[0].kind == StelnetAsrRegistryArtifactKind::Primary);
...
TEST_CASE("registry: parakeet entry has correct filename", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    bool found = stelnettts_registry_lookup("parakeet", e);
    REQUIRE(found);
...
TEST_CASE("registry: mimo-asr has entry (added in #63)", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    bool found = stelnettts_registry_lookup("mimo-asr", e);
    REQUIRE(found);
...
TEST_CASE("registry: omniasr has entry", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    bool found = stelnettts_registry_lookup("omniasr", e);
    REQUIRE(found);
}

TEST_CASE("registry: omniasr-300m has entry", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    bool found = stelnettts_registry_lookup("omniasr-300m", e);
    REQUIRE(found);
...
TEST_CASE("registry: omniasr-llm has entry", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    bool found = stelnettts_registry_lookup("omniasr-llm", e);
    REQUIRE(found);
}

TEST_CASE("registry: omniasr-llm-1b has entry", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    bool found = stelnettts_registry_lookup("omniasr-llm-1b", e);
    REQUIRE(found);
...
TEST_CASE("registry: granite-4.1 has entry", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    bool found = stelnettts_registry_lookup("granite-4.1", e);
    REQUIRE(found);
...
TEST_CASE("registry: gemma4-e2b has entry", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    bool found = stelnettts_registry_lookup("gemma4-e2b", e);
    REQUIRE(found);
}

TEST_CASE("registry: vibevoice has entry", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    bool found = stelnettts_registry_lookup("vibevoice", e);
    REQUIRE(found);
}

TEST_CASE("registry: wav2vec2 aligner aliases resolve", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    REQUIRE(stelnettts_registry_lookup("wav2vec2-aligner", e));
    REQUIRE(e.backend == "wav2vec2-aligner");
    REQUIRE(e.filename.find("wav2vec2") != std::string::npos);

    REQUIRE(stelnettts_registry_lookup("wav2vec2-aligner-en", e));
    REQUIRE(e.filename == "wav2vec2-xlsr-en-q4_k.gguf");

    REQUIRE(stelnettts_registry_lookup("wav2vec2-aligner-de", e));
    ...
    for (const auto& [alias, filename_part] : {
        ...
        REQUIRE(stelnettts_registry_lookup(alias, e));
...
TEST_CASE("registry: preferred quant rewrites primary filename", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    bool found = stelnettts_registry_lookup("chatterbox", e, "q4_k");
    REQUIRE(found);
...
TEST_CASE("registry: non-quantized companion remains unchanged", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    bool found = stelnettts_registry_lookup("cielvox2-tts", e, "q4_k");
    REQUIRE(found);
...
TEST_CASE("registry: companion filename lookup resolves the companion entry", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    bool found = stelnettts_registry_lookup_by_filename("cielvox2-tts-tokenizer-12hz.gguf", e);
    REQUIRE(found);
...
TEST_CASE("registry: quantized companion filename lookup preserves the requested quant", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    bool found = stelnettts_registry_lookup_by_filename("cielvox2-tts-tokenizer-12hz-q8_0.gguf", e);
    REQUIRE(found);
    REQUIRE(e.backend == "cielvox2-tts");
...
TEST_CASE("registry: companion_approx_size populated for orpheus", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    REQUIRE(stelnettts_registry_lookup("orpheus", e));
    REQUIRE(!e.companion_approx_size.empty());
    REQUIRE(e.companion_approx_size != e.approx_size);
}
...
TEST_CASE("registry: companion_approx_size populated for chatterbox", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    REQUIRE(stelnettts_registry_lookup("chatterbox", e));
    REQUIRE(!e.companion_approx_size.empty());
    REQUIRE(e.companion_approx_size != e.approx_size);
}
...
TEST_CASE("registry: chatterbox family keeps multilingual and finetunes separate", "[unit][registry]") {
    ...
    REQUIRE(stelnettts_registry_lookup("kartoffelbox-turbo", e));
    ...
    REQUIRE(stelnettts_registry_lookup("lahgtna-chatterbox", e));
...
TEST_CASE("registry: companion_approx_size empty for backends without companion", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    REQUIRE(stelnettts_registry_lookup("whisper", e));
    REQUIRE(e.companion_filename.empty());
    REQUIRE(e.companion_approx_size.empty());
}
...
TEST_CASE("registry: cielvox2-tts tokenizer filename lookup uses companion size (#146)", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    REQUIRE(stelnettts_registry_lookup_by_filename("cielvox2-tts-tokenizer-12hz.gguf", e));
...
TEST_CASE("registry: every entry fills without crash (#152)", "[unit][registry]") {
    ...
    for (int i = 0; i < n; ++i) {
        StelnetAsrRegistryEntry e;
        bool ok = stelnettts_registry_get_at(i, e);
        REQUIRE(ok);
...
TEST_CASE("registry: fill with preferred quant does not crash on any entry (#152)", "[unit][registry]") {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        stelnettts_registry_get_at(i, e);
        StelnetAsrRegistryEntry eq;
        stelnettts_registry_lookup(e.backend, eq, "q4_k");
...
TEST_CASE("registry: cosyvoice3 short alias resolves via -tts fallback", "[unit][registry]") {
    StelnetAsrRegistryEntry e;
    REQUIRE(stelnettts_registry_lookup("cosyvoice3", e));
...
TEST_CASE("registry: cosyvoice3-tts-rl swaps only the talker", "[unit][registry]") {
    StelnetAsrRegistryEntry base;
    StelnetAsrRegistryEntry rl;
    REQUIRE(stelnettts_registry_lookup("cosyvoice3-tts", base));
    REQUIRE(stelnettts_registry_lookup("cosyvoice3-tts-rl", rl));
