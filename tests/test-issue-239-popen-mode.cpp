...
// the POSIX branch of stelnettts_popen before calling ::popen().
//
// These tests verify stelnettts_popen can successfully open a real
...
TEST_CASE("stelnettts_popen: mode 'r' opens a subprocess", "[unit][issue-239][popen]") {
    FILE* p = stelnettts::stelnettts_popen("echo hello", "r");
    REQUIRE(p != nullptr);
    char buf[64] = {};
    (void)fgets(buf, sizeof(buf), p);
    stelnettts::stelnettts_pclose(p);
    std::string out(buf);
    REQUIRE(out.find("hello") != std::string::npos);
}

TEST_CASE("stelnettts_popen: mode 'rb' opens a subprocess on all platforms", "[unit][issue-239][popen]") {
    // Before the fix this returned nullptr on Linux (errno=EINVAL).
    FILE* p = stelnettts::stelnettts_popen("echo hello", "rb");
    REQUIRE(p != nullptr);
    char buf[64] = {};
    (void)fgets(buf, sizeof(buf), p);
    stelnettts::stelnettts_pclose(p);
...
TEST_CASE("stelnettts_popen: 'wb' mode strips 'b' cleanly", "[unit][issue-239][popen]") {
    // Write a byte to /dev/null via popen to confirm 'w'/'wb' normalise the same way.
    FILE* p = stelnettts::stelnettts_popen("cat > /dev/null", "wb");
    REQUIRE(p != nullptr);
    (void)fputc('x', p);
    stelnettts::stelnettts_pclose(p);
}
