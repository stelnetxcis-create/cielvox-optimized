...
// mirrors it). stelnettts_gap_fill_resolve_overlaps now splits the covering
...
stelnettts_word make_word(const char* text, int64_t t0_cs, int64_t t1_cs) {
    stelnettts_word w;
    w.text = text;
    w.t0 = t0_cs;
    w.t1 = t1_cs;
...
stelnettts_segment make_segment(std::vector<stelnettts_word> words) {
    stelnettts_segment seg;
    seg.words = std::move(words);
    seg.text = stelnettts_rebuild_text_from_words(seg.words);
...
public:
    ...
    std::vector<stelnettts_segment> transcribe(const float*, int n_samples, int64_t t_offset_cs,
...
private:
    std::vector<stelnettts_word> script_;
};

void require_monotone(const std::vector<stelnettts_segment>& segs) {
...
// stelnettts_make_disp_segments, which — for any segment that carries words —
...
void require_words_monotone_in_reading_order(const std::vector<stelnettts_segment>& segs) {
    int64_t prev_t0 = -1;
...
void require_words_inside_their_cue(const std::vector<stelnettts_segment>& segs) {
    for (size_t i = 0; i < segs.size(); i++)
        for (const auto& w : segs[i].words) {
...
TEST_CASE("issue #356: recovery inside a straddling segment splits it, output monotone",
    ...
    const stelnettts_audio_slice sl = {3250 * kSampleRate / 100, 4330 * kSampleRate / 100, 3250, 4330};
    ...
    stelnettts_gap_fill_slice(be, params, samples.data(), sl.end, kSampleRate, sl, segs);
...
TEST_CASE("issue #356: the merge case is reachable end-to-end through gap_fill_slice", "[unit][gap-fill][issue-356]") {
    ...
    const stelnettts_audio_slice sl = {0, 1000 * kSampleRate / 100, 0, 1000};
    std::vector<float> samples((size_t)sl.end, 0.0f);
