// test_slice_timestamps.cpp — unit tests for stelnettts_fixed_chunk_slices
// t0_cs / t1_cs timestamp math.
//
// PR #96 changed the server to route all audio through
// stelnettts_compute_audio_slices (which falls back to
// stelnettts_energy_chunk_slices / stelnettts_fixed_chunk_slices when no VAD
...
// ──────────────────────────────────────────────────────────────────────────────
// stelnettts_fixed_chunk_slices
// ──────────────────────────────────────────────────────────────────────────────

TEST_CASE("fixed_chunk_slices: empty input returns no slices", "[unit][slice-ts]") {
    auto slices = stelnettts_fixed_chunk_slices(0, SR, 30);
    REQUIRE(slices.empty());

    auto neg = stelnettts_fixed_chunk_slices(-1, SR, 30);
    REQUIRE(neg.empty());
}
...
TEST_CASE("fixed_chunk_slices: audio shorter than chunk → single slice", "[unit][slice-ts]") {
    ...
    const int n = 5 * SR;
    auto slices = stelnettts_fixed_chunk_slices(n, SR, 30);
