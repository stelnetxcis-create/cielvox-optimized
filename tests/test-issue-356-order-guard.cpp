...
// opt-in and documented as "never reorders". stelnettts_first_backward_segment
// is that missing check.
//
// The subtlety it has to get right is WHICH timestamp orders the output:
// stelnettts_make_disp_segments builds every cue from the WORD timestamps of a
...
stelnettts_segment with_words(std::vector<stelnettts_word> words) {
...
stelnettts_segment text_only(const char* text, int64_t t0_cs, int64_t t1_cs) {
    stelnettts_segment seg;
    seg.text = text;
    seg.t0 = t0_cs;
    seg.t1 = t1_cs;
...
TEST_CASE("issue #356: an in-order transcript is not flagged", "[unit][order-guard][issue-356]") {
    ...
    REQUIRE(stelnettts_first_backward_segment(segs) == -1);
}
...
TEST_CASE("issue #356: the reported backward jump is flagged", "[unit][order-guard][issue-356]") {
    ...
    REQUIRE(stelnettts_first_backward_segment(segs, &prev, &cur) == 1);
    REQUIRE(prev == 3946);
    REQUIRE(cur == 3318);
}
...
// The session C ABI carries its own segment struct (stelnettts_session_seg, a
// private type in src/stelnettts_c_api.cpp) and reimplements every backend's
...
TEST_CASE("issue #356: the guard is shared with the session ABI's segment type", "[unit][order-guard][issue-356]") {
    ...
    std::vector<stelnettts_segment> cli;
    cli.push_back(with_words({make_word("a", 400, 500)}));
    cli.push_back(with_words({make_word("b", 200, 250)}));
    REQUIRE(core_time_order::first_backward(cli) == stelnettts_first_backward_segment(cli));
    REQUIRE(stelnettts_first_backward_segment(cli) == 1);
}
