// chat_test.dart — tests for the StelnetAsrChatSession Dart binding.
//
// Two layers:
//   1. Symbol-resolution test (always runs): confirms every entry point of
//      include/stelnet_asr_chat.h is exported by the loaded libstelnet_asr.
//   2. Real-model tests (gated on STELNET_ASR_CHAT_TEST_MODEL pointing at a
...
//
// Run with:
//   STELNET_ASR_LIB=../../build-tests/src/libstelnet_asr.dylib \
//   STELNET_ASR_CHAT_TEST_MODEL=/path/to/gemma-3-1b-it-Q4_K_M.gguf \
...
import 'package:stelnet_asr/stelnet_asr.dart';
...
DynamicLibrary _openLib() {
  ...
  if (Platform.isMacOS || Platform.isIOS) {
    return DynamicLibrary.open(
      '../../build-ninja-compile/src/libstelnet_asr.dylib',
    );
  }
  return DynamicLibrary.open('../../build-ninja-compile/src/libstelnet_asr.so');
}
...
void main() {
  ...
  test('chat ABI symbols resolve', () {
    // Every entry point declared in include/stelnet_asr_chat.h.
    const symbols = [
      'stelnet_asr_chat_open_params_default',
      'stelnet_asr_chat_generate_params_default',
      'stelnet_asr_chat_open',
      'stelnet_asr_chat_close',
      'stelnet_asr_chat_reset',
      'stelnet_asr_chat_generate',
      'stelnet_asr_chat_generate_stream',
      'stelnet_asr_chat_set_abort_callback',
      'stelnet_asr_chat_template_name',
      'stelnet_asr_chat_n_ctx',
      'stelnet_asr_chat_count_tokens',
      'stelnet_asr_chat_memory_estimate',
    ...
    for (final s in symbols) {
      expect(
        ...
        isTrue,
        reason: 'missing $s in libstelnet_asr',
      );
    }
  });
  ...
  test('an interior NUL in an open argument is rejected by field name', () {
    ...
    final libPath = Platform.environment['STELNET_ASR_LIB'];
    expect(
      () => StelnetAsrChatSession.open('mo\u0000del.gguf', libPath: libPath),
      throwsA(isA<ArgumentError>().having((e) => e.name, 'name', 'modelPath')),
    );
    expect(
      () => StelnetAsrChatSession.open(
        'model.gguf',
    ...
    expect(
      () => StelnetAsrChatSession.memoryEstimate(
        'mo\u0000del.gguf',
        libPath: libPath,
      ),
      throwsA(isA<ArgumentError>().having((e) => e.name, 'name', 'modelPath')),
  ...
  test('the error message is decoded as UTF-8, not Latin-1', () {
    ...
    final libPath = Platform.environment['STELNET_ASR_LIB'];
    const path = '/tmp/stelnet_asr-naïve-模型-нет.gguf';
    expect(
      () => StelnetAsrChatSession.open(path, libPath: libPath),
      throwsA(
        isA<ChatException>()
  ...
  test("ChatGenerateParams' defaults are the ABI's own defaults", () {
    // No model needed — stelnet_asr_chat_generate_params_default is a pure
    ...
    // hazard: an edit to stelnet_asr_chat_generate_params_default in
    ...
    // include/stelnet_asr_chat.h documents.
  ...
  group('real model', () {
    final modelPath = Platform.environment['STELNET_ASR_CHAT_TEST_MODEL'];
    final libPath = Platform.environment['STELNET_ASR_LIB'];

    StelnetAsrChatSession openSession() {
      final s = StelnetAsrChatSession.open(
        modelPath!,
    ...
    Future<bool> isPinnedStopBaseline(StelnetAsrChatSession s) async {
    ...
    setUp(() {
      if (modelPath == null || modelPath.isEmpty) {
        markTestSkipped('STELNET_ASR_CHAT_TEST_MODEL not set');
      }
    });
    ...
    test(
      ...
      () {
        ...
        int at(int nCtx) => StelnetAsrChatSession.memoryEstimate(
              modelPath,
        ...
        expect(
          StelnetAsrChatSession.memoryEstimate(modelPath, libPath: libPath),
    ...
    test(
      ...
      () async {
        ...
        session.close();
        // Repeated closes must not reach stelnet_asr_chat_close a second time. A
    ...
    test(
      ...
      () {
        ...
        if (libPath == null || libPath.isEmpty) {
          markTestSkipped('STELNET_ASR_LIB not set — nothing to override with');
          return;
        }

        // A copy of libstelnet_asr in a temp directory is a separate file, so dyld
        // loads it as a second image: its stelnet_asr_chat_close sits at a
        ...
        final dir = Directory.systemTemp.createTempSync('stelnet_asr-libcopy');
        addTearDown(() => dir.deleteSync(recursive: true));
        final copyPath = '${dir.path}/libstelnet_asr-copy.dylib';
        File(libPath).copySync(copyPath);

        final primaryClose = DynamicLibrary.open(
          libPath,
        ).lookup<NativeFinalizerFunction>('stelnet_asr_chat_close');
        ...
        final viaCopy = StelnetAsrChatSession.open(
          modelPath,
          params: tiny,
          libPath: copyPath,
        );
        ...
        final viaPrimary = StelnetAsrChatSession.open(
          modelPath,
          params: tiny,
          libPath: libPath,
        );
