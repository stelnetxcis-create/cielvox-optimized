...
//
// Opens a StelnetAsrSession against ggml-tiny.en.bin, sets altN=3,
...
//       list by design).
//
// Skips silently when STELNET_ASR_LIB / STELNET_ASR_MODEL are absent —
...
void main() {
  final libEnv = Platform.environment['STELNET_ASR_LIB'];
  final modelEnv = Platform.environment['STELNET_ASR_MODEL'];
  final libPath =
  ...
  final skipReason = canRun
      ? null
      : 'set STELNET_ASR_LIB + STELNET_ASR_MODEL (or drop ggml-tiny.en.bin '
  ...
  group('§5.1.11 alt-token capture — live whisper-tiny', () {
    test('setAltN(3) populates Word.alts on JFK clip', () {
      final s = StelnetAsrSession.open(modelPath, libPath: libPath);
      try {
