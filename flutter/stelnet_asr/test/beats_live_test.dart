...
//
// Opens a StelnetAsrSession against a beat-this GGUF, runs [beats] on a
...
//
// Skips silently when STELNET_ASR_LIB / STELNET_ASR_BEAT_MODEL are absent — same
// pattern as transcription_test.dart + alt_tokens_live_test.dart so `dart
// test` stays green on model-less runners.
//
// Run locally with:
//   STELNET_ASR_LIB=../../build/src/libstelnet_asr.dylib \
//     STELNET_ASR_BEAT_MODEL=/path/to/beat-this-f16.gguf \
...
void main() {
  ...
  final modelPath = Platform.environment['STELNET_ASR_BEAT_MODEL'];

  test('beats: order, downbeat subset, and 120 BPM on a 120 BPM fixture', () {
    if (libPath == null || modelPath == null) {
      // ignore: avoid_print
      print('skip: set STELNET_ASR_LIB and STELNET_ASR_BEAT_MODEL to run');
      return;
    }
    final s =
        StelnetAsrSession.open(modelPath, libPath: libPath, backend: 'beat-this');
    try {
