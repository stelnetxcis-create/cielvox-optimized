// chat.dart — Dart binding for the stelnet_asr_chat_* C ABI (text → text LLM).
//
...
// `stelnet_asr_chat_close` is attached as a backstop for a session whose owner is
...
// returns it, optionally under an abort predicate. `stelnet_asr_chat_count_tokens`
// is bound as `countTokens`. `stelnet_asr_chat_generate_stream` is not bound —
// see the note above `close()` for why and what the alternative is.
//
// The header lives in include/stelnet_asr_chat.h and is the authority for every
// shape in this file.
...
import 'stelnet_asr.dart' show StelnetASR;
...
///
/// Each default below is the value `stelnet_asr_chat_generate_params_default`
...
class ChatGenerateParams {
  ...
  /// The generate defaults the loaded libstelnet_asr reports, read from
  /// `stelnet_asr_chat_generate_params_default`.
  ///
  ...
  static ChatGenerateParams abiDefaults(
    ...
    lib ??= DynamicLibrary.open(libPath ?? StelnetASR.defaultLibName());
...
/// A chat call failed. [code] is the C side's `stelnet_asr_chat_error.code`,
...
/// [StelnetAsrChatSession.reset] first.
...
/// the garbage collector running the attached `stelnet_asr_chat_close` while a
/// native call is still blocked on the same handle.
class StelnetAsrChatSession implements Finalizable {
  StelnetAsrChatSession._(this._lib, this._closeFn, this._handle,
  ...
  /// `stelnet_asr_chat_close` in the library this session was opened from — the
  ...
  /// rest of StelnetASR — "a CLI transcription tool is obvious to a reasonably
  ...
  /// Address of the `stelnet_asr_chat_close` that frees this session, on both the
  ...
  // callback is the stelnet_asr_chat_close of the very library this session was
  ...
  /// `stelnet_asr_chat_open` (predates the chat ABI). Throws
  /// [ChatException] when the underlying load fails (missing file,
  /// unsupported architecture, …).
  factory StelnetAsrChatSession.open(
    String modelPath, {
    ChatOpenParams params = const ChatOpenParams(),
    String? libPath,
  }) {
    final lib = DynamicLibrary.open(libPath ?? StelnetASR.defaultLibName());
    if (!lib.providesSymbol('stelnet_asr_chat_open')) {
      throw UnsupportedError(
        'stelnet_asr_chat_open not found in this libstelnet_asr — '
        'needs StelnetASR 0.8.16+ with the chat ABI.',
      );
    }
    ...
    final open = lib.lookupFunction<
        ...
        Pointer<Void> Function(
            Pointer<Utf8>, Pointer<_ChatOpenParams>, Pointer<_ChatError>)>(
      'stelnet_asr_chat_open',
    );
    final tmplName = lib.lookupFunction<Pointer<Utf8> Function(Pointer<Void>),
        Pointer<Utf8> Function(Pointer<Void>)>('stelnet_asr_chat_template_name');
    final nCtxFn = lib.lookupFunction<Int32 Function(Pointer<Void>),
        int Function(Pointer<Void>)>('stelnet_asr_chat_n_ctx');
    // `void stelnet_asr_chat_close(stelnet_asr_chat_session_t)` is exactly the
    ...
    final closeFn = lib.lookup<NativeFinalizerFunction>('stelnet_asr_chat_close');
    ...
    try {
      ...
      if (handle == nullptr) {
        throw _chatError(errPtr, 0, 'stelnet_asr_chat_open failed');
      }
      ...
      return StelnetAsrChatSession._(
  ...
  ///
  /// Throws [UnsupportedError] when the loaded dylib doesn't expose
  /// `stelnet_asr_chat_memory_estimate`, [ArgumentError] for an interior NUL, and
  ...
  static int memoryEstimate(
    ...
    final lib = DynamicLibrary.open(libPath ?? StelnetASR.defaultLibName());
    if (!lib.providesSymbol('stelnet_asr_chat_memory_estimate')) {
      throw UnsupportedError(
        'stelnet_asr_chat_memory_estimate not found in this libstelnet_asr — '
        'needs StelnetASR 0.8.16+ with the chat ABI.',
      );
    }
    final estimate = lib.lookupFunction<
        ...
        int Function(Pointer<Utf8>, Pointer<_ChatOpenParams>,
            Pointer<_ChatError>)>('stelnet_asr_chat_memory_estimate');
    ...
    try {
      ...
      if (bytes == 0) {
        throw _chatError(errPtr, 0, 'stelnet_asr_chat_memory_estimate failed');
      }
      return bytes;
    } finally {
      calloc.free(pathPtr);
  ...
  void reset() {
    ...
    final reset = _lib.lookupFunction<
        ...
        int Function(
            Pointer<Void>, Pointer<_ChatError>)>('stelnet_asr_chat_reset');
    final errPtr = calloc<_ChatError>();
    try {
      final rc = reset(_handle, errPtr);
      if (rc != 0) {
        throw _chatError(errPtr, rc, 'stelnet_asr_chat_reset failed');
      }
    } finally {
      calloc.free(errPtr);
    }
  ...
  int countTokens(List<ChatMessage> messages) {
    ...
    final countFn = _lib.lookupFunction<
        ...
        int Function(Pointer<Void>, Pointer<_ChatMessage>, int,
            Pointer<_ChatError>)>('stelnet_asr_chat_count_tokens');
    ...
    try {
      ...
      if (n < 0) {
        ...
        throw _chatError(errPtr, 0, 'stelnet_asr_chat_count_tokens failed');
      }
      return n;
    } finally {
      calloc.free(errPtr);
  ...
  /// to abort it — the polarity of `stelnet_asr_chat_abort_callback`, forwarded
  ...
  String _generateSync(
    ...
    final generate = _lib.lookupFunction<
        ...
        Pointer<Utf8> Function(
            ...
            Pointer<_ChatError>)>('stelnet_asr_chat_generate');
    ...
    try {
      ...
      if (out == nullptr) {
        throw _chatError(errPtr, 0, 'stelnet_asr_chat_generate failed');
      }
      try {
  ...
  // by `stelnet_asr --server --chat-model …` (POST /v1/chat/completions with
  ...
  ///
  /// `stelnet_asr_chat_close` waits for calls already inside the session before
  ...
  void _ensureOpen() {
    if (_closed || _handle == nullptr) {
      throw StateError('StelnetAsrChatSession is closed');
    }
  }
...
/// Turn a filled-in `stelnet_asr_chat_error` into the right exception.
///
...
Pointer<Utf8> _fillOpenParams(
  ...
  final defaults = lib.lookupFunction<
      ...
      void Function(
          Pointer<_ChatOpenParams>)>('stelnet_asr_chat_open_params_default');
  defaults(out);
  final pp = out.ref;
