...
- Native improvements that ship with this release (no Dart API change):
  - **Tiron** — a new multi-speaker meeting-ASR model (Whisper large-v3 + inline
    ...
    `StelnetAsrSession.open()` on the whisper path.
  - **Source separation ~26× faster** on Linux/Windows — `StelnetAsrSession.separate()`
...
- Beat and downbeat tracking: `StelnetAsrSession.beats()` returns a beat grid
...
- The native GGUF architecture auto-detect recognises `beat-this`, so plain
  `StelnetAsrSession.open()` works for beat models; `backend: 'beat-this'`
  remains valid and is required against an older native library.

## 0.8.17

- Source separation: `StelnetAsrSession.separate()` splits interleaved stereo
...
- Piano transcription: `StelnetAsrSession.pianoNotes()` returns structured
...
- Pitch (F0) estimation: `StelnetAsrSession.pitch()` returns a monophonic
...
- The native GGUF architecture auto-detect now recognises `crepe` (and
  `htdemucs`), so plain `StelnetAsrSession.open()` works for those models;
...
- **Whisper text-suppression + prompt-carry extras** — three
  ...
  sticky setter
  `StelnetAsrSession.setWhisperDecodeExtras(suppressNonSpeechTokens:,
...
- **Whisper decoder-fallback thresholds** — `wparams`
  ...
  `StelnetAsrSession.setFallbackThresholds(entropyThold:,
