...
# C2PA (Content Credentials) signing of StelnetTTS TTS output.
#
...
#
# Creates:
#   <output-dir>/stelnettts-c2pa.crt  — PEM certificate
#   <output-dir>/stelnettts-c2pa.key  — PEM private key (unencrypted)

set -euo pipefail

OUT_DIR="${1:-.}"
CERT="${OUT_DIR}/stelnettts-c2pa.crt"
KEY="${OUT_DIR}/stelnettts-c2pa.key"
...
EXT_CNF="$(mktemp -t stelnettts-c2pa-ext.XXXXXX)"
...
echo ""
echo "Use with StelnetTTS:"
echo "  stelnettts --tts \"hello\" --c2pa-cert $CERT --c2pa-key $KEY"
echo ""
echo "Or in server mode:"
echo "  stelnettts --server --c2pa-cert $CERT --c2pa-key $KEY"
