#!/bin/bash
# validate-macos.sh - Validate macOS Application with embedded stelnettts.xcframework using SwiftUI
...
if [ -d "${APP_PATH}/Contents/Frameworks/stelnettts.framework" ]; then
...
if [ -f "${APP_PATH}/Contents/Frameworks/stelnettts.framework/Versions/A/stelnettts" ]; then
    echo "✅ Framework binary exists"

    # Further validate framework by checking architecture
    ARCHS=$(lipo -info "${APP_PATH}/Contents/Frameworks/stelnettts.framework/Versions/A/stelnettts" 2>/dev/null | grep -o "arm64\\|x86_64" | tr '\n' ' ')
    if [ -n "$ARCHS" ]; then
