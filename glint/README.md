...
## How StelnetTTS uses it

**Compressed output (TTS / S2S).** The encoders back:

- `stelnettts-server` — `POST /v1/audio/speech` with
  `response_format=mp3` / `aac` / `opus`
- `stelnettts-cli` — `--tts-output out.{mp3,aac,opus}` and
  `--s2s-output`

routed through the header-only writers
`examples/cli/stelnettts_{mp3,aac,opus}_writer.h` and, for M4A/MP4,
`stelnettts_mp4_writer.h`. AI-provenance rides along per container: MP3
and AAC get a prepended ID3v2 tag (`stelnettts_make_id3v2_ai_tag`), Opus
...
**Compressed input (decode).** `src/stelnettts_audio.cpp` uses the
...
an optional *pinned* fallback, built only with `-DSTELNETTTS_OPUS=ON` and
selected at runtime with `STELNETTTS_OPUS_DECODER=libopus`; the default
...
with `STELNETTTS_MP3_ENCODER=lame` for A/B comparisons.

## Syncing

Automated: the `sync-glint` GitHub workflow pulls the committed state
of `Cyna/glint` main (on repository_dispatch from glint pushes,
