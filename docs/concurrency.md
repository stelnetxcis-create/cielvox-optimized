...
How StelnetTTS uses multiple cores, how it handles concurrent requests, and how
...
- **`--server-workers N` / `STELNETTTS_SERVER_WORKERS=N`** loads N independent
...
StelnetTTS parallelises at three levels. They compose: a bulk run typically uses
...
Every inference already runs multi-threaded inside ggml. One `stelnettts`
...
- C-ABI / bindings: `stelnettts_params_set_n_threads()` (default 4).
...
`stelnettts --server` loads the model **once** and reuses it across requests
...
(see `include/stelnettts.h`), so the server holds a `model_mutex` around the
inference call.
...
stelnettts --server -m model.gguf --port 8080 --server-workers 4

# Env form (overrides the flag; the ultimate gate):
STELNETTTS_SERVER_WORKERS=4 stelnettts --server -m model.gguf --port 8080
```
...
run several `stelnettts` processes at once, each transcribing whole files.
...
ls corpus/*.wav | xargs -P 4 -I{} \
  sh -c 'stelnettts -m model.gguf --threads 4 -otxt -of "out/$(basename "$1" .wav)" -f "$1"' _ {}

# GNU parallel is nicer for progress, retries, and remote nodes
# (here {/.} = input basename without extension):
parallel -j 4 --bar \
  'stelnettts -m model.gguf --threads 4 -otxt -of out/{/.} -f {}' ::: corpus/*.wav
```
...
named `stelnettts`. Scale it and put a proxy in front:

```bash
# N independent server containers, each with the model loaded once.
docker compose up --build --scale stelnettts=4
```
...
> compose network (as below) — remove the `ports:` mapping from the `stelnettts`
...
One `stelnettts_context` / session must not be driven from multiple threads at
once (documented in `include/stelnettts.h`). Concurrency comes from **separate
...
That is not where StelnetTTS's ASR throughput goes:
...
revisited *for that backend* — but it would not be a general StelnetTTS feature.
...
- [Server mode (HTTP API)](server.md) — endpoints, form fields, Docker Compose,
  `STELNETTTS_SERVER_WORKERS`, prebuilt CUDA images.
- [Benchmarking StelnetTTS](benchmarking.md) — how to measure throughput honestly
