...
StelnetTTS started as a fork of [whisper.cpp](https://github.com/ggml-org/whisper.cpp) and extends that base into a **unified speech engine** called `stelnettts`, backed by full ggml C++ runtimes for major open-weights ASR *and* TTS architectures. One build, one binary, one consistent CLI — pick the backend at the command line or let StelnetTTS auto-detect it from your GGUF file. See [Text-to-Speech](#text-to-speech-models) for the TTS side.

```console
$ stelnettts -m ggml-base.en.bin          -f samples/jfk.wav                    # OpenAI Whisper
$ stelnettts -m parakeet-tdt-0.6b.gguf    -f samples/jfk.wav                    # NVIDIA Parakeet
$ stelnettts -m canary-1b-v2.gguf         -f samples/jfk.wav                    # NVIDIA Canary
$ stelnettts -m voxtral-mini-3b-2507.gguf -f samples/jfk.wav                    # Mistral Voxtral
$ stelnettts --backend qwen3 -m auto      -f samples/jfk.wav                    # -m auto downloads
$ stelnettts --backend kokoro -m auto --tts "Hello world" --tts-output out.wav  # TTS
```
...
**Demo**: [HuggingFace Space](https://huggingface.co/spaces/Xenna/StelnetTTS) —
...
|---|---|
| **[StelnetTTS](https://github.com/Cyna/StelnetTTS)** | This repo — C++ speech engine. 54 ASR + 52 TTS backends, CLI + HTTP server + C-ABI + Python/Rust/Dart/Go/Ruby/Java bindings. |
| **[StelnetWeaver](https://github.com/Cyna/StelnetWeaver)** | Cross-platform Flutter transcription app built on StelnetTTS. Desktop + mobile,  model browser with download queue, mic capture, SRT/VTT/JSON export, diarization, batch processing. Fully offline. |
| **[StelnetEmbed](https://github.com/Cyna/StelnetEmbed)** | Text-related engine via ggml — same philosophy as StelnetTTS but for embeddings, retrieval, OCR and OMR, Math and Music Notation. Numerous architectures (XLM-R, Qwen3-Embed, Gemma3, ModernBERT, ...), dense + sparse + ColBERT + reranking. PP-OCR, Tesseract, EasyOCR, InternVL2, etc. Python/Rust/Dart bindings. |
| **[StelnetVox](https://github.com/Cyna/StelnetVox)** | Python ASR GUI with 9 backends (faster-whisper, mlx-whisper, voxtral, insanely-fast-whisper, ...). The Python counterpart to StelnetTTS's C++ approach. |
...
- [Environment variables](docs/environment-variables.md) — the `STELNETTTS_<BACKEND>_<FEATURE>` convention, global knobs, and every per-backend variable
- [Language bindings](docs/bindings.md) — Python / Rust / Dart / Go / Java / JavaScript / Ruby / mobile
- [Benchmarking StelnetTTS](docs/benchmarking.md) — how to measure transcribe time (not cold start): server/in-process reps, proof-of-work rules, phase-timing env vars
...
- [Quantize models](docs/quantize.md) — `stelnettts-quantize` for all backends
...
StelnetTTS ships **54 ASR backends** for transcription/translation and
...
| **whisper** | [`distil-whisper/distil-large-v3`](https://huggingface.co/Xenna/distil-large-v3-GGUF) | Distilled Whisper: 32L encoder + 2L decoder (6.3x faster) | English | MIT |
...
| **parakeet** | [`nvidia/parakeet-tdt-0.6b-v2`](https://huggingface.co/Xenna/parakeet-tdt-0.6b-v2-GGUF) | FastConformer + TDT, original Open ASR Leaderboard topper | en (mixed-case + punct) | CC-BY-4.0 |
| **parakeet** | [`nvidia/parakeet-tdt-1.1b`](https://huggingface.co/Xenna/parakeet-tdt-1.1b-GGUF) | 42L FastConformer + TDT, larger English variant | en (lowercase) | CC-BY-4.0 |
| **parakeet** | [`nvidia/parakeet-tdt_ctc-110m`](https://huggingface.co/Xenna/parakeet-tdt_ctc-110m-GGUF) | 17L FastConformer + TDT+CTC hybrid; smallest variant, auto-CTC decode | en | CC-BY-4.0 |
| **parakeet** | [`nvidia/parakeet-tdt_ctc-1.1b`](https://huggingface.co/Xenna/parakeet-tdt_ctc-1.1b-GGUF) | 42L FastConformer + TDT+CTC hybrid; largest, mixed-case + punct | en | CC-BY-4.0 |
| **parakeet** | [`nvidia/parakeet-tdt_ctc-0.6b-ja`](https://huggingface.co/Xenna/parakeet-tdt-0.6b-ja-GGUF) | FastConformer-TDT-CTC, xscaling, 80 mels | Japanese | CC-BY-4.0 |
| **reazonspeech** | [`reazon-research/reazonspeech-nemo-v2`](https://huggingface.co/Xenna/reazonspeech-nemo-v2-GGUF) | FastConformer-RNNT, local attn (w=256), 80 mels, 619M params | Japanese | Apache-2.0 |
| **fastconformer-ctc** | [`nvidia/parakeet-ctc-0.6b`](https://huggingface.co/Xenna/parakeet-ctc-0.6b-GGUF) | 24L FastConformer + CTC, 80 mels (same arch as fc-ctc-xlarge) | en | CC-BY-4.0 |
| **fastconformer-ctc** | [`nvidia/parakeet-ctc-1.1b`](https://huggingface.co/Xenna/parakeet-ctc-1.1b-GGUF) | 42L FastConformer + CTC, 80 mels | en | CC-BY-4.0 |
| **fastconformer-ctc** | [`grider-transwithai/parakeet-ctc-1.1b-ja`](https://huggingface.co/Xenna/parakeet-ctc-1.1b-ja-GGUF) | 42L FastConformer + CTC, 80 mels, Japanese fine-tune | Japanese | Apache-2.0 |
...
| **lfm2-audio** | [`LiquidAI/LFM2.5-Audio-1.5B`](https://huggingface.co/Xenna/lfm2-audio-1.5b-GGUF) | FastConformer + LFM2 hybrid conv+attention backbone (ASR+TTS) | en | LFM Open v1.0 |
| **lfm2-audio** | [`LiquidAI/LFM2.5-Audio-1.5B-JP`](https://huggingface.co/Xenna/lfm2-audio-1.5b-jp-GGUF) | FastConformer + LFM2 hybrid conv+attention backbone (ASR+TTS) | ja | LFM Open v1.0 |
...
| **wav2vec2** | [`facebook/data2vec-audio-base-960h`](https://huggingface.co/Xenna/data2vec-audio-960h-GGUF) | Data2Vec Audio (79 MB Q4_K) | English | Apache-2.0 |
| **wav2vec2** | [`facebook/hubert-large-ls960-ft`](https://huggingface.co/Xenna/hubert-large-ls960-ft-GGUF) | HuBERT Large (212 MB Q4_K) | English | Apache-2.0 |
...
| **moonshine** | [`UsefulSensors/moonshine-{tiny,base}`](https://huggingface.co/Xenna/moonshine-base-GGUF) | Conv + 6L enc + 6L dec; multilingual variants | English + 6 langs | MIT |
| **moonshine&#8209;de** | [`fidoriel/moonshine-base-de`](https://huggingface.co/Xenna/moonshine-base-de-fidoriel-GGUF) | German fine-tune of moonshine-base (6.9% WER CV22) | German | CC&#8209;BY&#8209;NC&#8209;SA&#8209;4.0 |
| **moonshine&#8209;tiny&#8209;de** | [`fidoriel/moonshine-tiny-de`](https://huggingface.co/Xenna/moonshine-tiny-de-fidoriel-GGUF) | German fine-tune of moonshine-tiny (11.4% WER CV22) | German | CC&#8209;BY&#8209;NC&#8209;SA&#8209;4.0 |
| **moonshine-streaming** | [`UsefulSensors/moonshine-streaming-{tiny,small,medium}`](https://huggingface.co/Xenna/moonshine-streaming-tiny-GGUF) | Streaming: sliding-window encoder + AR decoder (34–245M) | English | MIT |
| **gemma4-e2b** | [`google/gemma-4-E2B-it`](https://huggingface.co/Xenna/gemma4-e2b-it-GGUF) | USM Conformer 12L + Gemma4 LLM 35L (GQA, PLE) | 140+ langs | Apache-2.0 |
| **gemma4-e4b** | [`google/gemma-4-E4B-it`](https://huggingface.co/Xenna/gemma4-e4b-it-GGUF) | Same USM Conformer 12L + larger Gemma4 LLM 42L (GQA, PLE); runs on `--backend gemma4-e2b` | 140+ langs | Apache-2.0 |
| **omniasr** | [`omniASR-CTC-1B-v2`](https://huggingface.co/Xenna/omniASR-CTC-1B-v2-GGUF) | wav2vec2 CNN + 48L transformer + CTC ([more](docs/architecture.md#omniasr-ctc--llm--unlimited)) | **1600+** | Apache-2.0 |
| **omniasr&#8209;300m** | [`omniASR-CTC-300M-v2`](https://huggingface.co/Xenna/omniASR-CTC-300M-v2-GGUF) | Same arch, 24L, ~194 MB Q4_K; auto-chunks >7 s ([more](docs/architecture.md#omniasr-ctc--llm--unlimited)) | **1600+** | Apache-2.0 |
| **omniasr-llm** | [`omniASR-LLM-300M-v2`](https://huggingface.co/Xenna/omniasr-llm-300m-v2-GGUF) | Same encoder + 12L LLaMA decoder ([more](docs/architecture.md#omniasr-ctc--llm--unlimited)) | **1600+** | Apache-2.0 |
| **omniasr-llm** | [`omniASR-LLM-Unlimited-300M-v2`](https://huggingface.co/Xenna/omniasr-llm-unlimited-300m-v2-GGUF) | Streaming: 15s segment protocol, unlimited audio ([more](docs/architecture.md#omniasr-ctc--llm--unlimited)) | **1600+** | Apache-2.0 |
| **vibevoice** | [`microsoft/VibeVoice-ASR`](https://huggingface.co/Xenna/vibevoice-asr-GGUF) | σ-VAE ConvNeXt + Qwen2.5-7B ([more](docs/architecture.md#vibevoice)) | 50+ | MIT |
| **vibevoice-bitnet** | [`VibeVoice-ASR-BitNet`](https://huggingface.co/Xenna/vibevoice-asr-bitnet-GGUF) | Same arch, TQ2_0 ternary LM (1.6 GB) ([more](docs/architecture.md#vibevoice)) | 7+ | MIT |
| **mimo-asr** | [`XiaomiMiMo/MiMo-V2.5-ASR`](https://huggingface.co/Xenna/mimo-asr-GGUF) | 6L transformer + 36L Qwen2 LM + RVQ codec ([more](docs/architecture.md#mimo-asr)) | Mandarin + dialects + English | MIT |
| **ark-asr** ⚠️*experimental/WIP* | [`Xenna/ark-asr-3b-GGUF`](https://huggingface.co/Xenna/ark-asr-3b-GGUF) (base [`AutoArk-AI/ARK-ASR-3B`](https://huggingface.co/AutoArk-AI/ARK-ASR-3B)) | Whisper-large-v3 enc (partial RoPE) + Qwen2.5-3B LM ([more](docs/architecture.md#ark-asr)) | 19 (zh, en, de, ja, fr, ko, es, pl, it, ro, hu, cs, nl, fi, hr, sk, sl, et, lt) | see base |
| **moss-audio** | [`OpenMOSS-Team/MOSS-Audio-4B-Instruct`](https://huggingface.co/Xenna/MOSS-Audio-4B-Instruct-GGUF) | 32L Whisper encoder + DeepStack 3-tap + 36L Qwen3 LM; audio understanding + ASR ([more](docs/architecture.md#moss-audio)) | zh, en | Apache-2.0 |
| **moss-transcribe** | [`OpenMOSS-Team/MOSS-Transcribe-preview-2B`](https://huggingface.co/Xenna/MOSS-Transcribe-preview-2B-GGUF) | Qwen3-Omni audio encoder (32L, windowed attn) + GatedMLP adapter + Qwen3-1.7B LM; ASR ([more](docs/architecture.md#moss-transcribe)) | zh, en | Apache-2.0 |
| **moss-diarize** | [`OpenMOSS-Team/MOSS-Transcribe-Diarize-0.9B`](https://huggingface.co/Xenna/MOSS-Transcribe-Diarize-0.9B-GGUF) | Stock Whisper encoder (24L, 80 mel) + 4x merge + VQAdaptor + Qwen3-0.6B LM; joint ASR + speaker diarization + timestamps | multi | Apache-2.0 |
| **whisper** *(tiron)* ⚠️*experimental* | [`Trelis/tiron`](https://huggingface.co/Xenna/tiron-GGML) (base [`Trelis/tiron`](https://huggingface.co/Trelis/tiron)) | Whisper large-v3 with an extended vocab: emits inline `<|speakerN|>` markers + 20 ms timestamps for joint transcription + per-window speaker attribution, via a constrained-decoding grammar; cross-window linking to stable speakers (#295) | multi (en focus) | Apache-2.0 |
| **funasr** | [`FunAudioLLM/Fun-ASR-Nano-2512`](https://huggingface.co/Xenna/funasr-nano-GGUF) | 70-block SANM encoder + 2-block Transformer adaptor + Qwen3-0.6B LLM | zh, yue, en, ja, ko | FunASR Model License v1.1 (commercial OK w/ attribution) |
| **fun-asr-mlt-nano** | [`FunAudioLLM/Fun-ASR-MLT-Nano-2512`](https://huggingface.co/Xenna/funasr-mlt-nano-GGUF) | Same architecture, multilingual decoder | 31 langs incl. de, fr, es, pt, ru, ar, hi, vi, th, ko | FunASR Model License v1.1 |
| **paraformer** | [`funasr/paraformer-zh`](https://huggingface.co/Xenna/paraformer-zh-GGUF) | 50-block SANM encoder + CIF predictor + 16-block NAR decoder (single-pass, non-autoregressive); character-level vocab (8404); 220M params | zh, en | FunASR Model License (commercial OK w/ attribution) |
| **foxnose** *(speaker diarization)* | [`Wespeaker/wespeaker-voxceleb-resnet34-LM`](https://huggingface.co/Xenna/wespeaker-resnet34-lm-GGUF) | Speaker diarization via `--diarize-method foxnose`: WeSpeaker ResNet34-LM 256-d embeddings + GMM/BIC speaker counting + spectral clustering + Viterbi temporal smoothing ([more](docs/architecture.md#foxnose-diarize)). 3.18 % DER on VoxConverse dev vs the upstream reference implementation's 3.07 % | any | weights CC-BY-4.0 |
| **gigaam** | [`ai-sage/GigaAM-v3`](https://huggingface.co/Xenna/gigaam-v3-GGUF) (base [`ai-sage/GigaAM-v3`](https://huggingface.co/ai-sage/GigaAM-v3)) | 16-layer rotary Conformer (220M) + CTC or RNN-T head; four revisions — `e2e_rnnt` / `e2e_ctc` emit punctuation + casing + ITN from a SentencePiece vocab, `rnnt` / `ctc` emit bare lowercase Cyrillic ([more](docs/architecture.md#gigaam)) | en ru | MIT |
| **sensevoice** | [`FunAudioLLM/SenseVoiceSmall`](https://huggingface.co/Xenna/sensevoice-small-GGUF) | 70-block SANM encoder + CTC head; emits transcript + language ID + audio-event in one forward pass (non-AR, 15× faster than Whisper-Large); structured C ABI + `-oj` JSON expose the tags as separate fields. Upstream's emotion classifier is **not exposed** — see [EU AI Act](docs/eu-ai-act.md#41-emotion-recognition--removed-not-gated) | 50+ langs; native LID + audio-event tags | FunASR Model License v1.1 |
...
stelnettts -m models/sidon-v0.1-f16.gguf -f input.wav --s2s --s2s-output restored.wav

python models/convert-voxcpm2-to-gguf.py --input openbmb/VoxCPM2 \
  --output models/voxcpm2-vae-f32.gguf --vae-only
stelnettts -m models/voxcpm2-vae-f32.gguf -f input.wav --s2s \
...
|---------|--------|-------------|-----------|---------|
| **miotts** | [`MioTTS-0.6B`](https://huggingface.co/Xenna/miotts-0.6b-GGUF) | Qwen3 LLM + MioCodec-v2 FSQ codec (25 Hz, 44.1 kHz output) | ja, en | Apache-2.0 |
| **vibevoice-tts** | [`VibeVoice-Realtime-0.5B`](https://huggingface.co/Xenna/vibevoice-realtime-0.5b-GGUF), [`VibeVoice-1.5B`](https://huggingface.co/Xenna/vibevoice-1.5b-GGUF) | DPM-Solver++ + σ-VAE decoder; voice presets or cloning | en, zh | MIT |
| **kugelaudio** | [`kugelaudio-0-open`](https://huggingface.co/Xenna/kugelaudio-0-open-GGUF) | Qwen2.5-7B LM + 4L DiT diffusion + acoustic VAE decoder; voice cloning | multilingual | Apache-2.0 |
| **cielvox2-tts** | [`Qwen3-TTS-12Hz-0.6B-Base`](https://huggingface.co/Xenna/cielvox2-tts-0.6b-base-GGUF), [`1.7B-Base`](https://huggingface.co/Xenna/cielvox2-tts-1.7b-base-GGUF), [`1.7B-VoiceDesign`](https://huggingface.co/Xenna/cielvox2-tts-1.7b-voicedesign-GGUF) | Qwen3 talker LM + 12 Hz RVQ ([more](docs/architecture.md#cielvox2-tts)) | multilingual | Apache-2.0 |
| **cielvox2-tts-customvoice** | [`1.7B-CustomVoice`](https://huggingface.co/Xenna/cielvox2-tts-1.7b-customvoice-GGUF) | Same talker + 9 premium built-in speakers (`--voice <name>`); optional style via `--instruct` (e.g. "spoke very slowly") ([more](docs/architecture.md#cielvox2-tts)) | multilingual | Apache-2.0 |
| **moss-tts** | [`OpenMOSS-Team/MOSS-TTS-v1.5`](https://huggingface.co/Xenna/moss-tts-v1.5-GGUF) | Qwen3-8B backbone emitting 32 RVQ audio codebooks under a delay pattern, decoded by a 1.6B pure-transformer codec companion; voice cloning via `--voice ref.wav`; `--backend moss-tts -m <backbone> --codec-model <codec>` | multilingual | Apache-2.0 |
| **moss-tts-local** | [`OpenMOSS-Team/MOSS-TTS-Local-Transformer-v1.5`](https://huggingface.co/Xenna/moss-tts-local-v1.5-GGUF) | Qwen3-4B backbone; a 1-layer local/depth transformer autoregressively emits 12 RVQ codebooks per frame (RQ-Transformer, no delay), decoded to 48 kHz by MOSS-Audio-Tokenizer-v2 (downmixed to mono); `--backend moss-tts-local -m <backbone> --codec-model <codec>` | multilingual | Apache-2.0 |
| **omnivoice** | [`k2-fsa/OmniVoice`](https://huggingface.co/Xenna/omnivoice-GGUF) | Qwen3-0.6B + masked iterative 8-codebook TTS (SoundStorm-style); voice cloning; 600+ languages ([more](docs/architecture.md#omnivoice)) | 600+ langs | Apache-2.0 |
...
| **orpheus** | [`Orpheus-3B-FT`](https://huggingface.co/Xenna/orpheus-3b-0.1-ft-GGUF) + [`SNAC 24 kHz`](https://huggingface.co/Xenna/snac-24khz-GGUF) | Llama-3.2-3B + SNAC RVQ codec; 8 speakers ([more](docs/architecture.md#orpheus)) | en, de | Llama 3.2 Community License / MIT |
| **chatterbox** | [`Xenna/chatterbox-GGUF`](https://huggingface.co/Xenna/chatterbox-GGUF) + turbo/kartoffelbox/lahgtna variants | T3 AR + S3Gen flow-matching ([more](docs/architecture.md#chatterbox--chatterbox-turbo--kartoffelbox-turbo--lahgtna-chatterbox)) | 23 multilingual; separate Arabic (`lahgtna-chatterbox`) and German turbo (`kartoffelbox-turbo`) fine-tunes | MIT |
| **indextts** | [`Xenna/indextts-1.5-GGUF`](https://huggingface.co/Xenna/indextts-1.5-GGUF) | GPT-2 AR (24L/1280d) + Conformer conditioning + BigVGAN vocoder; voice cloning via reference audio | zh, en | Apache-2.0 |
| **voxcpm2-tts** | [`Xenna/voxcpm2-GGUF`](https://huggingface.co/Xenna/voxcpm2-GGUF) | Tokenizer-free CFM diffusion AR (TSLM + RALM + LocDiT) at 48 kHz native; zero-shot + voice cloning via `--voice <wav>` | 30 languages | Apache-2.0 |
...
| **cosyvoice3-tts** | [`Xenna/cosyvoice3-0.5b-2512-GGUF`](https://huggingface.co/Xenna/cosyvoice3-0.5b-2512-GGUF) | Qwen2-0.5B AR speech-token LM + DiT-CFM (10-step Euler) + HiFT (NSF + iSTFT) at 24 kHz; baked-voice zero-shot cloning via `--voice <name>`, or any WAV via `--voice ref.wav --ref-text "<exact transcript>"`. `--backend cosyvoice3-tts-rl` selects upstream's RL-tuned talker (same companions) | 9 langs + 18 zh dialects | Apache-2.0 |
| **csm** | [`Xenna/csm-1b-GGUF`](https://huggingface.co/Xenna/csm-1b-GGUF) | Sesame CSM-1B conversational TTS: Llama-3.2 1B backbone + 100M depth decoder (32-codebook RVQ) + Kyutai Mimi codec at 24 kHz ([more](docs/architecture.md#csm)) | en | Apache-2.0 |
| **lfm2-audio** | [`Xenna/lfm2-audio-1.5b-GGUF`](https://huggingface.co/Xenna/lfm2-audio-1.5b-GGUF) + [`jp`](https://huggingface.co/Xenna/lfm2-audio-1.5b-jp-GGUF) | LFM2.5-Audio ASR+TTS+S2S: FastConformer enc + LFM2 hybrid backbone + depthformer (8-codebook Mimi) + ISTFT detokenizer at 24 kHz; interleaved text+audio generation | en, ja | LFM Open v1.0 |
| **dia** | [`nari-labs/Dia-1.6B`](https://huggingface.co/Xenna/dia-1.6b-GGUF) | Byte-level text encoder (12L) + AR audio decoder (18L GQA + CFG) → 9 delayed DAC codebooks + 44.1 kHz DAC codec; dialogue style with `[S1]`/`[S2]` tags (use >100-char prompts) | en | Apache-2.0 |
| **zonos-tts** | [`Xenna/zonos-v0.1-transformer-GGUF`](https://huggingface.co/Xenna/zonos-v0.1-transformer-GGUF) + [`Xenna/dac-44khz-GGUF`](https://huggingface.co/Xenna/dac-44khz-GGUF) | Zyphra Zonos-v0.1: 26L GQA AR transformer (2B) + 9-codebook DAC @ 44.1 kHz; CFG-guided; voice cloning via reference WAV ([more](docs/architecture.md#zonos-tts)) | en | Apache-2.0 |
| **bark** | [`Xenna/bark-small-GGUF`](https://huggingface.co/Xenna/bark-small-GGUF) | Suno Bark 3-stage GPT-2 TTS: text→semantic (12L) → coarse EnCodec (12L, 2 codebooks) → fine (12L, 8 codebooks) → EnCodec 24 kHz decoder; speaker conditioning via `.npz` prompts (`--voice <file.npz>`) | multilingual | MIT |
| **speecht5** | [`Xenna/speecht5-tts-GGUF`](https://huggingface.co/Xenna/speecht5-tts-GGUF) | SpeechT5 80M: char-level encoder (12L) + AR mel decoder (6L) + 5-layer conv postnet + HiFi-GAN at 16 kHz; speaker via 512-d x-vector (`--voice <xvector.bin>`) | en | MIT |
| **fastpitch** | [`Xenna/fastpitch-en-GGUF`](https://huggingface.co/Xenna/fastpitch-en-GGUF) | NVIDIA FastPitch 60M: non-autoregressive parallel TTS — 6L encoder + duration/pitch predictors + 6L decoder + HiFi-GAN at 22 kHz; deterministic, single forward pass ([more](docs/architecture.md#fastpitch)) | en | CC-BY-4.0 |
...
| **parler-tts** | [`Xenna/parler-tts-mini-v1.1-GGUF`](https://huggingface.co/Xenna/parler-tts-mini-v1.1-GGUF) | Parler TTS Mini v1.1 (~900M): T5 encoder + MusicGen decoder + DAC 44.1 kHz; prompt-conditioned (describe voice in text via `--instruct`) | en | Apache-2.0 |
| **outetts** | [`Xenna/outetts-0.3-1b-GGUF`](https://huggingface.co/Xenna/outetts-0.3-1b-GGUF) | OLMo-1B talker + WavTokenizer single-codebook VQ-GAN at 24 kHz; voice cloning via speaker profile JSON (`--voice <speaker.json>`) | en | CC-BY-NC-SA-4.0 |
| **pocket-tts** | [`Xenna/pocket-tts-GGUF`](https://huggingface.co/Xenna/pocket-tts-GGUF) | Kyutai Pocket TTS 100M: continuous-latent AR at 12.5 Hz + one-step LSD flow + Mimi VAE 24 kHz; voice cloning via ref audio ([more](docs/architecture.md#pocket-tts)) | en | CC-BY-4.0 + gated-use conditions |
| **tada** | [`Xenna/tada-tts-1b-GGUF`](https://huggingface.co/Xenna/tada-tts-1b-GGUF) + `HumeAI/tada-3b-ml` | Llama-3.2 1B/3B backbone + per-token FM diffusion head + TADA codec at 24 kHz; 1:1 text-to-acoustic alignment; default prompt via `tada-ref.gguf`, custom voices via `--voice <tada-ref.gguf>` built with `models/convert-tada-ref-to-gguf.py` ([more](docs/architecture.md#tada)) | en | Llama 3.2 Community License |
...
| cielvox2-tts | yes* | temp | 24 | yes | yes |
...
`cosyvoice3-tts`, `cielvox2-tts` and `moss-tts` act on it natively. For
...
| Backend | Models | Architecture | Languages | License |
|---|---|---|---|---|
| **m2m100** | [`facebook/m2m100_418M`](https://huggingface.co/Xenna/m2m100-418m-GGUF) | 12L enc + 12L dec transformer, SentencePiece 128K ([more](docs/architecture.md#m2m100--wmt21)) | 100 langs, any-to-any | MIT |
| **m2m100-wmt21** | [`facebook/wmt21-dense-24-wide-en-x`](https://huggingface.co/Xenna/wmt21-dense-24-wide-en-x-GGUF) + [`facebook/wmt21-dense-24-wide-x-en`](https://huggingface.co/Xenna/wmt21-dense-24-wide-x-en-GGUF) | Same as m2m100, scaled to 4.7B (24L enc) ([more](docs/architecture.md#m2m100--wmt21)) | English ↔ 7 langs (separate `en-x` / `x-en` checkpoints) | MIT |
| **madlad** | [`google/madlad400-3b-mt`](https://huggingface.co/Xenna/madlad400-3b-mt-GGUF) | T5 enc-dec (12L+12L, d=2048, gated-GELU, RMSNorm) ([more](docs/architecture.md#madlad)) | 419 languages | Apache-2.0 |
...
./build/bin/stelnettts --backend m2m100-wmt21 \
...
|---|---|---|---|---|---|
| **FireRedPunc** | Punctuation restoration | BERT-base (12L, d=768), 5 classes | Chinese + English | Apache-2.0 | [`Xenna/fireredpunc-GGUF`](https://huggingface.co/Xenna/fireredpunc-GGUF) |
| **fullstop-punc** | Punctuation restoration | XLM-RoBERTa-large (24L, d=1024), 6 classes | EN, DE, FR, IT | MIT | [`Xenna/fullstop-punc-multilang-GGUF`](https://huggingface.co/Xenna/fullstop-punc-multilang-GGUF) |
| **punctuate-all** | Punctuation restoration | XLM-RoBERTa-base (12L, d=768), 6 classes | 12 languages | MIT | [`Xenna/punctuate-all-GGUF`](https://huggingface.co/Xenna/punctuate-all-GGUF) |
...
| **CLD3** | Text language ID | Embedding-bag → FC + ReLU → softmax (~1.5 MB F32) | 109 ISO 639-1 | Apache-2.0 | [`Xenna/cld3-GGUF`](https://huggingface.co/Xenna/cld3-GGUF) |
| **GlotLID-V3** | Text language ID | fastText supervised, flat softmax | 2102 ISO 639-3 + script | Apache-2.0 | [`Xenna/glotlid-GGUF`](https://huggingface.co/Xenna/glotlid-GGUF) |
| **LID-176** | Text language ID | fastText supervised, hierarchical softmax | 176 ISO 639-1 | CC-BY-NC-4.0 | [`Xenna/fasttext-lid176-GGUF`](https://huggingface.co/Xenna/fasttext-lid176-GGUF) |
...
|---|---|---|---|---|---|
| **MioCodec v2** | WavLM encoder → FSQ(12800) → Transformer decoder + AdaLN-Zero + SnakeBeta upsampler + iSTFT | 44.1 kHz | 25 Hz (341 bps) | MIT | [`Xenna/miocodec-v2-44k-GGUF`](https://huggingface.co/Xenna/miocodec-v2-44k-GGUF) |
| **SNAC 24 kHz** | 3-codebook RVQ + decoder blocks (stride 8/8/4/2) | 24 kHz | 3×12.5 Hz | MIT | [`Xenna/snac-24khz-GGUF`](https://huggingface.co/Xenna/snac-24khz-GGUF) |
...
Beyond speech, StelnetTTS runs several music/audio analysis tasks — each a small
...
- **Guitar tablature** (`--tab`) — per-frame fret-per-string grid via **TabCNN**
  ...
  `stelnettts_session_tab_emissions()` for playable output.
...
Run `stelnettts --list-backends` to see it live. Each backend declares capabilities at runtime; if you ask for a feature the selected backend does not support, StelnetTTS prints a warning and silently ignores the flag.

**Sortable / filterable view:** [`docs/feature-matrix.html`](docs/feature-matrix.html) — click any column header to sort, type to filter rows, click cap pills to require a capability. Generated from `stelnettts --list-backends-json` (single source of truth — drift impossible). Regenerate via `python tools/gen-feature-matrix.py`. A Markdown twin lives at [`docs/feature-matrix.md`](docs/feature-matrix.md).
...
<!-- Generated from `stelnettts --list-backends` + cross-cutting features. -->
...
| KV quant (`STELNETTTS_KV_QUANT`, plus per-half `_K` / `_V`) | | | | | ✔ | ✔ | ✔ | ✔ | ✔ | | | ✔ | | | | | | ✔ | | ✔ | ✔ | ✔ | | |
| mmap weights (`STELNETTTS_GGUF_MMAP`) | | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ | ✔ |
| TTS | | | | | | | | | | | | | | | | | | | ✔ | | | | | |

The matrix above covers 24 ASR backends. **Additional ASR backends** not shown: `nemotron` (39-lang streaming ASR with cache-aware FastConformer + RNN-T), `lfm2-audio` (ASR + TTS + S2S in one model), `moss-audio` (audio understanding + ASR), `moss-transcribe` (Qwen3-Omni encoder + Qwen3-1.7B ASR), `mini-omni2` (ASR + TTS + S2S), `kugelaudio` (7B audio understanding). See [`docs/feature-matrix.md`](docs/feature-matrix.md) for the full 106-backend matrix. **TTS-only backends** (`kokoro`, `cielvox2-tts` + variants, `vibevoice-tts`, `orpheus` + DE variants, `chatterbox` / `chatterbox-turbo` / `kartoffelbox-turbo` / `lahgtna-chatterbox`, `dia`, `bark`, `outetts`, `zonos`, `csm`, `f5-tts`, `irodori-tts`, `parler-tts`, `speecht5`, `piper`, `fastpitch`, `pocket-tts`, `melotts`, `cosyvoice3`, `voxcpm2`, `tada-tts`) all carry the TTS, AUTO_DOWNLOAD, TEMPERATURE, and FLASH_ATTN caps; per-backend cloning + voice-pack support is documented in the [Text-to-Speech models](#text-to-speech-models) table above and [`docs/tts.md`](docs/tts.md). The vibevoice and lfm2-audio columns mark dual-mode (ASR + TTS) backends.

**Key:** ✔ = native/built-in, `-am` = via CTC forced aligner (`-am canary-ctc-aligner.gguf` or `-am cielvox2-forced-aligner.gguf`), **LID** = via external language identification pre-step (`-l auto`), **pp** = via `--punc-model` post-processor (FireRedPunc or fullstop-punc), * = experimental or partial support, † = PLUS variant only (native `[T:N]` word timestamps with `-owts`; base uses `-am`). granite-4.1 covers both the regular and `-plus` variants; granite-4.1-nar is a non-autoregressive variant with encoder+projector only (no LLM decode features). The **KV quant** row marks backends that honor `STELNETTTS_KV_QUANT={f16,q8_0,q4_0}` — CTC-style backends without a KV cache (parakeet, fc-ctc, wav2vec2, kyutai-stt, firered, moonshine variants, omniasr-CTC) don't apply. The same backends also honor the per-half `STELNETTTS_KV_QUANT_K` / `STELNETTTS_KV_QUANT_V` overrides (llama.cpp `--cache-type-k` / `--cache-type-v` parity) for asymmetric K-vs-V precision; common recipe `K=q8_0 V=q4_0` saves ~40 % more KV memory than symmetric Q8_0. The **mmap weights** row marks backends consuming `core_gguf::load_weights()` and therefore honoring `STELNETTTS_GGUF_MMAP=1`; whisper itself uses upstream's loader and is unaffected. See [`docs/cli.md`](docs/cli.md) Memory footprint for usage + recommended combos.
...
**Punctuation restoration** (`--punc-model`): CTC-based backends output lowercase without punctuation. Named shortcuts: `auto`/`firered` (Chinese+English), `fullstop` (EN/DE/FR/IT, XLM-R-large), `punctuate-all` (12 languages, XLM-R-base), `pcs` (47 languages, punc + truecasing + sentence boundary detection in one model). Or pass a GGUF path directly. Also available via Python/Rust/Dart wrappers (`stelnettts.PuncModel`).

**Truecasing** (`--truecase-model`): Restore German noun/name capitalization in lowercase ASR output. Three options in ascending quality: `auto` (statistical, 9 MB), `crf` (CRF with context, 24 MB), `lstm` (BiLSTM char-level, 3.2 MB, **recommended** — 97.9% F1, handles adjective/noun distinction and formal "Ihnen"). All auto-download from [`Xenna/truecaser-de`](https://huggingface.co/Xenna/truecaser-de). Or use `--punc-model pcs` for neural punc + truecasing in one pass (47 languages).
...
```bash
stelnettts --backend parakeet -m parakeet.gguf --vad --flush-after 1 -osrt -f long_audio.wav
...
`-l auto` and stelnettts runs an optional LID pre-step before the main
transcribe() call:

```bash
# Downloads ggml-tiny.bin (75 MB, 99 languages) on first use
stelnettts --backend cohere -m $TC/cohere-transcribe-q5_0.gguf \
         -f unknown.wav -l auto
# stelnettts[lid]: detected 'en' (p=0.977) via whisper-tiny
# stelnettts: LID -> language = 'en' (whisper, p=0.977)
```

These LID providers are available:

- `--lid-backend whisper` (default) — uses a small multilingual ggml-*.bin model via the stelnettts C API. Auto-downloads ~75 MB on first use. 99 languages.
- `--lid-backend silero` — native GGUF port of Silero's 95-language classifier. 16 MB F32. Runs as a ggml graph (multi-threaded SIMD on CPU, GPU offload on Metal/CUDA; on Vulkan the graph is routed to CPU pending an upstream kernel fix). Analyzes the first 30 s of audio (`STELNETTTS_SILERO_LID_MAX_S` overrides); `STELNETTTS_SILERO_LID_LEGACY=1` restores the old scalar path.
- `--lid-backend ecapa` — **recommended**: ECAPA-TDNN (Apache-2.0). Purpose-built for language ID. Very high accuracy on TTS benchmark. Two variants via `--lid-model`:
  - [`Xenna/ecapa-lid-107-GGUF`](https://huggingface.co/Xenna/ecapa-lid-107-GGUF) — VoxLingua107, 43 MB F16, 107 languages, ISO codes (en, de, ...). **Default.**
  - [`Xenna/ecapa-lid-commonlanguage-GGUF`](https://huggingface.co/Xenna/ecapa-lid-commonlanguage-GGUF) — CommonLanguage, 40 MB F16, 45 languages, full names (English, German, ...).
...
- **MarbleNet** — NVIDIA 1D separable CNN, 439 KB, 6 languages (EN/DE/FR/ES/RU/ZH). Pass `--vad -vm marblenet` to auto-download. Smallest model. ([`Xenna/marblenet-vad-GGUF`](https://huggingface.co/Xenna/marblenet-vad-GGUF))
- **Whisper-VAD-EncDec** *(experimental)* — Whisper-base encoder + TransformerDecoder head, 22 MB Q4_K. Trained on Japanese ASMR; may not generalise well to all domains. Pass `--vad -vm whisper-vad`. Slower than others (~1s vs ~50ms). ([`Xenna/whisper-vad-encdec-asmr-GGUF`](https://huggingface.co/Xenna/whisper-vad-encdec-asmr-GGUF))
...
|---|---:|---:|---|---|
| **CLD3** (Google compact language detector v3) | 109 ISO 639-1 | **440 KB** | Apache-2.0 | [`Xenna/cld3-GGUF`](https://huggingface.co/Xenna/cld3-GGUF) |
| **GlotLID-V3** (cis-lmu fastText) | 2102 ISO 639-3 + script | 250 MB | Apache-2.0 | [`Xenna/glotlid-GGUF`](https://huggingface.co/Xenna/glotlid-GGUF) |
| **LID-176** (Facebook fastText) | 176 ISO 639-1 | 63 MB | CC-BY-NC-4.0¹ | [`Xenna/fasttext-lid176-GGUF`](https://huggingface.co/Xenna/fasttext-lid176-GGUF) |
...
```bash
stelnettts-lid -m auto --text "Bonjour le monde"        # → Xenna/cld3-GGUF (default, ~440 KB)
stelnettts-lid -m auto:glotlid --text "Bonjour le monde" -k 5
stelnettts-lid -m auto:lid-fasttext176 --text "Hallo Welt"
# Or pass an explicit path / canonical filename (looked up in the registry):
stelnettts-lid -m cld3-f16.gguf --text "你好世界"
# zh	0.997816
echo "Привет мир" | stelnettts-lid -m auto --quiet
# ru	0.907322
```
...
```bash
stelnettts -m ggml-tiny.bin -f speech.wav --lid-on-transcript auto
...
Produces `build/bin/stelnettts` (main CLI), `build/bin/stelnettts-quantize`,
and `build/bin/stelnettts-diff`. No Python, PyTorch, or pip required at
...
curl -L -o parakeet.gguf \
    https://huggingface.co/Xenna/parakeet-tdt-0.6b-v3-GGUF/resolve/main/parakeet-tdt-0.6b-v3-q4_k.gguf

./build/bin/stelnettts -m parakeet.gguf -f samples/jfk.wav
# Auto-detected backend 'parakeet' from GGUF metadata.
# And so, my fellow Americans, ask not what your country can do for you, ask what you can do for your country.

# Word-level timestamps (one line per word)
./build/bin/stelnettts -m parakeet.gguf -f samples/jfk.wav -ml 1
```
...
./build/bin/stelnettts --backend canary -m canary-1b-v2-q5_0.gguf -f audio.de.wav -sl de -tl de

# Translation (German speech → English text)
./build/bin/stelnettts --backend canary -m canary-1b-v2-q5_0.gguf -f audio.de.wav -sl de -tl en

# ...or use the familiar stelnettts flag:
./build/bin/stelnettts --backend canary -m canary-1b-v2-q5_0.gguf -f audio.de.wav -l de --translate
```

### Voxtral (speech-LLM with auto-download)

```bash
# First run downloads ~2.5 GB to ~/.cache/stelnettts/ via curl, then runs
./build/bin/stelnettts --backend voxtral -m auto -f samples/jfk.wav

# Subsequent runs use the cached file
./build/bin/stelnettts --backend voxtral -m auto -f samples/jfk.wav -l en
```
...
```bash
# 0.6B (default, ~500 MB)
./build/bin/stelnettts --backend qwen3 -m auto -f audio.zh.wav

# 1.7B (higher quality, ~1.3 GB) — supports both -hf and non-hf source models
./build/bin/stelnettts --backend qwen3 -m qwen3-1.7b --auto-download -f audio.wav

# Japanese anime/galgame fine-tune (~1.3 GB)
./build/bin/stelnettts --backend qwen3 -m qwen3-ja-anime --auto-download -f anime.wav
```
...
```bash
./build/bin/stelnettts --backend glm-asr -m auto -f audio.wav
...
./build/bin/stelnettts --backend glm-asr -m auto --chunk-seconds 0 -f long.wav
```
...
huggingface-cli download Xenna/mimo-asr-GGUF mimo-asr-q4_k.gguf \
    --local-dir ~/.cache/stelnettts
huggingface-cli download Xenna/mimo-tokenizer-GGUF mimo-tokenizer-q4_k.gguf \
    --local-dir ~/.cache/stelnettts

# Transcribe (auto-discovers tokenizer if it sits next to the LM)
./build/bin/stelnettts \
    --backend mimo-asr \
    -m ~/.cache/stelnettts/mimo-asr-q4_k.gguf \
    --codec-model ~/.cache/stelnettts/mimo-tokenizer-q4_k.gguf \
    -f samples/jfk.wav
...
curl -L -o wav2vec2-en-q4k.gguf \
    https://huggingface.co/Xenna/wav2vec2-large-xlsr-53-english-GGUF/resolve/main/wav2vec2-xlsr-en-q4_k.gguf

./build/bin/stelnettts -m wav2vec2-en-q4k.gguf -f samples/jfk.wav
# and so my fellow americans ask not what your country can do for you ask what you can do for your country

# German
curl -L -o wav2vec2-de-q4k.gguf \
    https://huggingface.co/Xenna/wav2vec2-large-xlsr-53-german-GGUF/resolve/main/wav2vec2-xlsr-de-q4_k.gguf

./build/bin/stelnettts -m wav2vec2-de-q4k.gguf -f audio.de.wav
...
./build/bin/stelnettts-quantize wav2vec2-de.gguf wav2vec2-de-q4k.gguf q4_k
```
...
StelnetTTS has three feature areas that warrant their own docs pages:
...
- **[Text-to-Speech (TTS)](docs/tts.md)** — Kokoro (multilingual,
  ...
  arbitrary-WAV cloning). Voice packs, language routing, and cielvox2-tts
...
stelnettts --backend vibevoice-tts -m auto --tts "Hello world" --tts-output hello.wav

# CosyVoice3 on GPU; companions auto-download beside the LLM
stelnettts --backend cosyvoice3-tts -m auto --tts "Hello world" --tts-output cosy.wav

# CosyVoice3 fast mode: 5 flow steps instead of the quality-default 10
COSYVOICE3_FLOW_STEPS=5 stelnettts --backend cosyvoice3-tts -m auto \
...
stelnettts --server --backend cielvox2-tts-customvoice -m auto --voice-dir ./voices --port 8080
...
```bash
stelnettts -m auto --backend parakeet -f audio.wav --vad -osrt --split-on-punct
```
...
StelnetTTS is structured as a stable C-ABI in `src/` (every algorithm:
...
- **[`docs/contributing.md`](docs/contributing.md)** — adding a new
  ...
  `stelnettts-diff` PyTorch-ground-truth workflow, and the
...
**Shared libraries** (cross-repo with StelnetEmbed):
...
Both are self-contained static libraries with CMakeLists.txt. StelnetEmbed
links them via `add_subdirectory(../StelnetTTS/crisp_*/)`; StelnetTTS uses
...
`build/bin/stelnettts-quantize` is a single, model-agnostic GGUF
...
```bash
./build/bin/stelnettts-quantize input.gguf output.gguf q4_k
```
...
stelnettts --gpu-backend vulkan -m model.gguf -f audio.wav
...
stelnettts -ng -m model.gguf -f audio.wav

# CUDA unified memory (swap to RAM when VRAM exhausted)
GGML_CUDA_ENABLE_UNIFIED_MEMORY=1 stelnettts -m model.gguf -f audio.wav
```
...
- The Windows convenience script `build-vulkan.bat` creates a separate Vulkan-capable binary at `build-vulkan\bin\stelnettts.exe`.
...
plumbed through), set `STELNETTTS_VERBOSE=1` instead.

```bash
# Per-stage timing breakdown (mel / encoder / prefill / decode):
stelnettts -v --backend gemma4-e2b -m model.gguf -f audio.wav
...
# stelnettts: transcribed 11.0s audio in 7.75s (1.4x realtime)

# Hugging Face access for gated models (Voxtral, Gemma4-E2B, …):
HF_TOKEN=hf_xxx stelnettts -m auto --backend gemma4-e2b -f audio.wav
```

The server has its own auth env: `STELNETTTS_API_KEYS` (see
...
| Env var | Purpose |
| --- | --- |
| `STELNETTTS_VERBOSE=1` | Forces verbose mode for any backend (parallel to the `-v` flag). |
| `STELNETTTS_DUMP_DIR=path/` | Generic per-stage F32 tensor dump for the `stelnettts-diff` harness. |
...
| `CIELVOX2_TTS_BENCH=1` / `CIELVOX2_TTS_DEBUG=1` / `CIELVOX2_TTS_DUMP_DIR=` | Qwen3-TTS per-stage timings, diagnostics, and stage dumps. |
...
| `STELNETTTS_VOXTRAL4B_STREAM_TIMING=1` | Per-stage timings for the voxtral4b streaming path (encoder drain / prefill / first-text-token / decode-step p50/p95). |
| `STELNETTTS_VOXTRAL4B_STREAM_CHUNK_MS=N` | Override the internal encoder chunk size (default 240 ms). Must be a multiple of 80 ms. Larger = faster feed (kernel-launch amortisation), longer live-caption latency floor. |
| `STELNETTTS_VOXTRAL4B_STREAM_BATCH_ENCODER=1` | Regression-debug: ignore the streaming encoder's audio_embeds and re-run the whole batch encoder at flush. |
| `STELNETTTS_VOXTRAL4B_STREAM_DEBUG=1` / `STELNETTTS_VOXTRAL4B_STREAM_DIFF=1` | Per-step decode prints / side-by-side encoder cosine vs the batch encoder. |
| `STELNETTTS_VOXTRAL4B_STREAM_LIVE=1` | Live-captions decode-during-feed (PLAN #7 phase 3). `get_text()` polled during feed returns progressive transcript. Default OFF (PTT semantics). Wrappers: Python `Session.stream_open(live=True)`, Rust `stream_open_ex(.., live: true)`. |
| `STELNETTTS_VOXTRAL4B_STREAM_DECODER_THREAD=1` | Decoder worker thread (PLAN #7 phase 4, implies live mode). Lets `feed()` return between encoder chunks without waiting for the decode loop — useful for mic-driven workloads. On M1 the Metal queue serializes encoder and decoder so total wall-clock is unchanged; faster GPUs with kernel-level parallelism see real overlap. |
| `STELNETTTS_VOXTRAL4B_FUSED_QKV=0` | Opt out of the runtime fused-QKV LLM path (default-on, ~7-8 % decode speedup on M1 Q4_K, ~500 MB extra memory). |
| `STELNETTTS_CIELVOX2_ASR_FUSED_QKV=0` | Opt out of the runtime fused-QKV LLM path for cielvox2-asr (default-on; works on F16/F32/Q4_K/Q8_0/...). |
| `STELNETTTS_VOXTRAL_FUSED_QKV=1` | Opt **in** to the runtime fused-QKV LLM path for voxtral 3B. Off by default (no measurable speedup on JFK-shape decodes; useful for long-form workloads where decode dominates). |
| `CIELVOX2_TTS_FUSED_QKV=1` | Opt in to the runtime fused-QKV talker path. |
...
| `STELNETTTS_NO_REL_POS=1` | Ablate the relative-position bias in the Gemma-4 audio encoder (development only). |
| `ECAPA_REF_FBANK=path` | Reference filterbank tensor for the ECAPA-TDNN LID model (regression harness). |
| `STELNETTTS_SHERPA_LID_BIN=path` | Override the auto-detected sherpa-onnx LID binary. |
| `STELNETTTS_ARG_DEVICE=N` | Default GPU device index when `-dev` isn't passed. |
...
- **[glint](https://github.com/Cyna/glint)** (MIT) — in-tree clean-room codec suite: MP3/AAC-LC/Ogg-Opus output encoders (TTS `.mp3`/`.aac`/`.opus`) and cross-platform ADTS AAC-LC + Ogg Opus input decoders (no libopus needed)
- **[Claude Code](https://claude.ai/claude-code)** (Anthropic) — significant portions of the stelnettts integration layer, all model converters, and the FastConformer/attention/mel/FFN/BPE core helpers were co-authored with Claude
...
Per-model weights are covered by their respective HuggingFace model licenses (see [Supported backends](#supported-backends)). The `stelnettts` binary itself links model runtimes that are mostly permissively licensed (MIT / Apache-2.0 / CC-BY-4.0 for weights).
